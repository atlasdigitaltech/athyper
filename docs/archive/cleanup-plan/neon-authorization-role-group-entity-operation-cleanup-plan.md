...
Implementation status (2026-07-27): the runtime/repository/session/context,
evidence, invalidation, fail-closed publication, and shared enforcement
foundation is implemented and covered by the Wave 5 static/runtime suite.
Production publication remains `shadow`. Route-by-route consumer migration is
tracked in
`config/governance/authorization-wave5-consumer-inventory.v1.json`; those
pending rows are an explicit cutover blocker enforced by the database release
guard. See
`docs/architecture/authorization-wave5-runtime-enforcement-session-v2.md`.
...
Implementation status (2026-07-27): the immutable seed ledger, serialized and
plane-isolated provisioners, guarded reset marker/fingerprint, additive seed
cleanup, conflict-fail identity executor, additive Keycloak reconciler,
fresh-target disposition executor, canonical state capture, and Wave 6
verification suite are implemented. The repository/static suite passes.
Live clean/reseed/upgrade captures, concurrent execution evidence, approved
data/identity manifests, source/target reconciliation, and the Keycloak restore
drill remain operational cutover gates. See
`docs/architecture/authorization-wave6-safe-seeds-and-identity-tooling.md`.
...
Implementation status (2026-07-27): the immutable three-way shadow comparison,
mandatory high-risk coverage, exact typed-scope/permission normalization,
closed mismatch taxonomy, plane/permission cohort state, append-only evidence,
projection/readiness gates, legacy freeze guards, validated target guards, and
atomic single-writer epoch switch are implemented for Neon/Admin and the
physically isolated Mesh database. Static/runtime verification passes. Live
shadow traffic, approved mismatch dispositions and active-user parity, load
evidence, deployed target-guard inventory, zero-lag switch rehearsal, any
promised reverse projection, and the observation window remain operational
gates. See
`docs/architecture/authorization-wave7-shadow-evaluation-cohort-cutover.md`.
...
Implementation status (2026-07-28): the fail-closed Wave 8 rehearsal contract,
fresh-target source/target state capture, full evidence certification engine,
gate tests, verifier, and executable runbook are implemented. The tooling
requires distinct exact plane database identities and read-only target
inspection; it cannot certify an in-place reset or infer approval from source
control. Live backup/restore, migration/replay, identity/seed/constraint,
business acceptance, boundary/dispatcher, RTO, connection-switch, rollback,
and stable-observation evidence remain pending because no approved isolated
production-like environment or completed recovery contract was supplied. See
...
Implementation status (2026-07-28): the Wave 9 contraction contract, explicit
plane-scoped removal manifest, `RESTRICT`-only idempotent Neon and Mesh
tombstones, immutable completion receipt, guarded executor, repository
zero-scan, clean-build event-trigger sentinel, read-only clean-baseline capture,
certification evaluator/tests, and operational runbook are implemented. Live
contraction and repository deletion remain deliberately blocked because the
Wave 8 operational certification and rollback-retention completion evidence
are not yet approved. No legacy object or source file has been removed by this
Wave 9 preparation. See
`docs/runbooks/authorization-v2-wave9-contraction-clean-baseline.md`.

Local-development fast path (2026-07-28): Phase 1 is implemented through the
separate `development_clean_reset` profile and decision record. It authorizes
only exact `disposable_local` plane databases carrying the
`LOCAL-AUTH-V2-RESET` guard label, preserves Keycloak users, and explicitly
claims no production Wave 8 completion. See
