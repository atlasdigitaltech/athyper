...
[render-pipeline-config.md](./render-pipeline-config.md). Do not
...
[render-pipeline-config.md](./render-pipeline-config.md).
...
- **Synchronous path (`RenderService`)**: `GotenbergClient` implemented at
  ...
  contract in [render-pipeline-config.md](./render-pipeline-config.md).
...
1. Read per-render overrides from `document.render_output.manifest_json`
   ...
   margins, scale, header/footer, background — see the
   `master.print_profile` column mapping in the metadata docs).
