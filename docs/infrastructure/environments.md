...
The three environments — `local`, `staging`, `production` — share the same compose definitions, application images, and kernel-config schema. They differ only in:
...
| **Secrets source** | Plain env file (`stack/env/.env`) — local-only credentials | Bootstrap env + `/opt/stack/athyper/secrets/*` files | Bootstrap env + SUPERSTAR / Infisical-managed secrets |
| **Default compose profile** | `core` (infra only) — apps run on host | `all` (everything containerized) | `all` (everything containerized) |
...
The same script does the right thing per environment — you do not switch scripts when moving between environments. Examples:

| Script | `ENVIRONMENT=local` | `ENVIRONMENT=staging` / `production` |
|---|---|---|
| `stack/scripts/app/api-up.{bat,sh}` | `pnpm --filter @athyper/runtime-server dev` on host, translates `dbpool-apps` → `127.0.0.1:6432` etc. | `docker compose build` (if `--build`), then `docker compose up -d --no-deps api` |
...
| `stack/scripts/stack-profile/up.{bat,sh}` | Uses `STACK_PROFILE=core` by default → infra only | Uses `STACK_PROFILE=all` (or whatever the env file sets) → everything |
| `stack/scripts/app/planes-up.bat` | Opens 3 host Next.js windows | (refuses) — use `web-up.sh all` |
| `stack/scripts/dev-local.{bat,sh}` | One command: stops stale containers + brings up infra + opens host windows | (refuses) — staging/production must not use this |

`.bat` scripts are explicitly local-only. `.sh` scripts handle all three environments — staging and production deploys always use them.

---

## Local — the canonical recipe
...
**Equivalent explicit form** (three terminals — what `dev-local` automates):
...
:: Terminal 3 — Neon + Mesh + Admin in three spawned windows
stack\scripts\app\local-ui-up.bat
```

**Why not "everything in Docker" for local?** Source edits to `packages/shared/auth-bff` etc. don't propagate into a baked Next.js bundle. A container that was built 30 minutes ago will silently run stale code while you edit and reload — a failure mode that wastes hours. Host-mode `next dev` and `tsx watch` give you sub-second feedback and stack traces in source coordinates.

**Defence in depth.** Even if you bypass `dev-local.bat`, the per-script guards in `api-up.{bat,sh}` and `web-up.{bat,sh}` detect a running `${COMPOSE_PROJECT_NAME}-<svc>-1` container and warn you before starting host dev — pausing for confirmation so you cannot silently shadow yourself.
...
## Staging — Ubuntu, everything containerized

The full server-build runbook (Phases 0–26: user/group setup, two-root layout, hardening, GitHub deploy key, config deployment, secrets, smoke tests, systemd, backups) lives in [`staging-setup.md`](staging-setup.md). The summary:
...
**Why staging matters for testing realm changes.** Local KC has `start-dev --import-realm` which skips imports on existing DBs (per `feedback_kc_reset_approach.md`). Staging has `start --import-realm` with the same skip behavior, so when you ship a realm JSON change, you must wipe the KC DB on staging before restart — same gotcha, same fix. Document the runbook step before promoting realm changes.

---

## Production — same shape as staging, three deltas

Production reuses the staging Ubuntu runbook end-to-end. Three things change:

1. **Override file** — `athyper.override.production.yml` is selected by `up.sh` when `ENVIRONMENT=production`. It pins higher resource limits, disables debug routes, sets log level to `warn`, and may switch to a non-dev `server/Dockerfile.prod` if your pipeline builds one.

2. **Secrets source** — production uses SUPERSTAR (or Infisical, per `project_infra_review_april2026.md`). Secret files are populated by your secrets manager's sync agent rather than copied by `setup-config`. See [`secrets-management.md`](secrets-management.md) §1 for the per-environment matrix.

3. **Managed dependencies where chosen** — production typically moves Postgres and object storage to managed services (RDS / Cloud SQL / S3 / GCS). The compose db / objectstorage services are then omitted from the active profile set, and `DATABASE_URL` / `S3_ENDPOINT` in the env file point to the managed endpoints. Keycloak's PG behind it is the most common candidate for "move to managed" because clustering KC itself stays simpler.

Everything else — two-root layout, service identities (svc-redis 9100, svc-minio 9101, etc.), systemd unit, backups — is identical to staging. The runbook in [`staging-setup.md`](staging-setup.md) applies verbatim.
...
A developer working on a specific concern locally can opt in to extra profiles by running `up.bat <profile>` repeatedly — profiles accumulate. See [§6 Stack Profiles in local-dev-setup.md](local-dev-setup.md#6-stack-profiles) for the full table.
...
| You want to… | Go here |
|---|---|
| Set up Windows for the first time | [`local-dev-setup.md`](local-dev-setup.md) §3 Initial Setup |
| Just start coding after a reboot | [`local-dev-setup.md`](local-dev-setup.md) Quick Start (one command: `dev-local.bat`) |
| Understand `next dev` vs Docker for a plane | [`local-dev-setup.md`](local-dev-setup.md) §4 Options A/B/C |
| Build a fresh staging server | [`staging-setup.md`](staging-setup.md) Part B (Phases 0–26) |
...
| Document | Purpose |
|---|---|
| [`local-dev-setup.md`](local-dev-setup.md) | Windows local development — full setup, Options A/B/C, gotchas |
| [`staging-setup.md`](staging-setup.md) | Ubuntu server runbook — Phases 0–26, v13 permission model |
