...
//
// Usage:
//   pnpm --filter @athyper/neon run runtime:path-check          (warn mode, exit 0)
//   pnpm --filter @athyper/neon run runtime:path-check --strict (fail on any hit)
//
// Wired into apps/neon/package.json "lint" once Phase 5 flips strict on.
...
const ALLOW_FILES = new Set<string>([
  ...
  "apps/neon/scripts/check-runtime-api-paths.ts",
...
async function main(): Promise<number> {
  ...
  if (hits.length === 0) {
    console.log("[runtime:path-check] OK — no legacy runtime API paths found.");
    return 0;
  }

  const label = strict ? "ERROR" : "WARN ";
  for (const hit of hits) {
    console.log(`[runtime:path-check] ${label} ${hit.file}:${hit.line}  (${hit.pattern})  ${hit.text}`);
  }
  console.log(
    `[runtime:path-check] ${hits.length} occurrence(s) of legacy runtime API paths. ` +
...
main().then((code) => process.exit(code)).catch((err: unknown) => {
  console.error("[runtime:path-check] fatal", err);
  process.exit(2);
});
