import { createRelayHandler, EXPERIENCE_BOOTSTRAP_OPERATION, IAM_ME_OPERATION, NEON_OPERATING_ORGANIZATIONS_OPERATION, NEON_WORK_CONTEXTS_OPERATION, PLATFORM_VERIFICATION_RUN_OPERATION, PLATFORM_VERIFICATION_SNAPSHOT_OPERATION, type RelayHandler } from "@athyper/platform-gateway-bff-relay";
...
export const platformRelay: RelayHandler = (request, context) => {
  relay ??= createRelayHandler({
    ...
    operations: [IAM_ME_OPERATION, EXPERIENCE_BOOTSTRAP_OPERATION, NEON_WORK_CONTEXTS_OPERATION, NEON_OPERATING_ORGANIZATIONS_OPERATION, PLATFORM_VERIFICATION_SNAPSHOT_OPERATION, PLATFORM_VERIFICATION_RUN_OPERATION],
    session: {
