...
export default async function ProtectedLayout({ children }: { readonly children: React.ReactNode }) { const [state,cookieStore] = await Promise.all([loadProtectedAppBootstrap("/"),cookies()]); if (state.state === "redirect") redirect(state.location); const initialCollapsed=cookieStore.get("athyper_shell_collapsed")?.value==="true"; return <AppProviders session={state.session} bootstrap={state.bootstrap} dehydratedState={state.dehydratedState}><NeonShell bootstrap={state.bootstrap} principalId={state.session.principalId ?? "Account"} initialCollapsed={initialCollapsed}>{children}</NeonShell></AppProviders>; }
