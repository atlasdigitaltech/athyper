...
import {
  Banner, SectionCard, SkeletonCard, ToggleGroup,
...
type DensityCode    = "compact" | "comfortable" | "spacious";
    ...
    <div className="rounded-md border border-border bg-card">
      ...
      {expanded && (
        <div className="border-t border-border px-3 pb-3 pt-2">
          ...
          <div className="mb-3 flex gap-x-4 gap-y-1 flex-wrap text-xs text-muted-foreground">
          ...
          <div className="flex flex-wrap gap-1.5">
            {(["pin", "star", "share"] as const).map((action) => {
              ...
              const activeClass = { pin: view.is_pinned ? "border-info text-info" : "", star: view.is_starred ? "border-warning text-warning" : "", share: view.is_shared ? "border-success text-success" : "" };
              return (
                <Button key={action} variant="outline" size="sm"
                  className={cn("h-7 gap-1 px-2 text-xs", activeClass[action])}
                  onClick={() => act(action)} disabled={!!busy}
                >
            ...
            <Button variant="outline" size="sm"
              ...
              onClick={() => act("archive")} disabled={!!busy}
            >
...
// ─── Workspace / module options ───────────────────────────────────────────────

const HOME_WORKSPACES = [
  { value: "CORE", label: "Core" }, { value: "FIN",  label: "Finance" },
  { value: "SCM",  label: "Supply Chain" }, { value: "COM", label: "Commercial" },
  { value: "PPL",  label: "People" }, { value: "PRS", label: "Projects" },
  { value: "OPS",  label: "Operations" }, { value: "AST", label: "Assets" },
  { value: "PTR",  label: "Partner" },
];

const HOME_MODULES_BY_WS: Record<string, { value: string; label: string }[]> = {
  CORE: [{ value: "FND", label: "Foundation" }, { value: "META", label: "Metadata" }, { value: "IAM", label: "IAM" }, { value: "AUD", label: "Audit" }, { value: "NTF", label: "Notifications" }],
  FIN:  [{ value: "ACC", label: "Accounting" }, { value: "PAY", label: "Payments" }, { value: "TREASURY", label: "Treasury" }, { value: "BUDGET", label: "Budget" }],
  SCM:  [{ value: "SRM", label: "Supplier Relations" }, { value: "SOURCE", label: "Sourcing" }, { value: "CONTRACT", label: "Contracts" }, { value: "BUY", label: "Purchasing" }, { value: "INVENTORY", label: "Inventory" }],
  COM:  [{ value: "CRM", label: "CRM" }, { value: "SALE", label: "Sales" }],
  PPL:  [{ value: "HR", label: "HR" }, { value: "PAYROLL", label: "Payroll" }],
  PRS:  [{ value: "PRJCOST", label: "Project Costing" }, { value: "ITSM", label: "ITSM" }],
  OPS:  [{ value: "MAINT", label: "Maintenance" }, { value: "MFG", label: "Manufacturing" }],
  AST:  [{ value: "ASSET", label: "Asset Management" }, { value: "ASSETREMS", label: "Real Estate" }, { value: "ASSETFM", label: "Facilities" }],
  PTR:  [{ value: "PCON", label: "Partner Connect" }, { value: "OMI", label: "OMI" }, { value: "IMO", label: "IMO" }, { value: "LOGX", label: "Logistics" }],
};
...
export function PreferencesSection({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    bffFetch<Record<string, unknown>>("/api/user/preferences")
      .then((d) => {
        const nextMode   = (d["appearance_mode"] as AppearanceMode | undefined) ?? "system";
        const nextPreset = String(d["theme_preset"] ?? (d["metadata"] as Record<string, unknown> | undefined)?.["theme_preset"] ?? "neon-base");
        const nextDens   = (d["density_code"] as DensityCode | undefined) ?? "compact";
        const nextLang   = String(d["language_code"] ?? "en");
        const nextTz     = String(d["timezone_code"] ?? "Asia/Dubai");
        const nextDateFmt= String(d["date_format"] ?? "%d/%m/%Y");
        const nextWeek   = String(d["week_start"] ?? "1");
        const nextHomeWs = String(d["home_workspace_code"] ?? "FIN");
        const nextHomeMod= String(d["home_module_code"] ?? "ACC");
        const nextDigest = String(d["notification_digest"] ?? "daily");
        ...
        applyThemePreferences({
          ...
          density_code: nextDens,
          theme_preset: nextPreset,
          language_code: nextLang,
        });
  ...
  async function handleSave() {
    ...
    let autoReset = true;
    try {
      const result = await bffFetch<{ ok: boolean; reason?: string }>("/api/user/preferences", {
        method: "PATCH",
        body: {
  ...
  const densityOptions = [
    { value: "compact" as DensityCode,     label: "Compact",      desc: "Tight spacing — default" },
    { value: "comfortable" as DensityCode, label: "Comfortable",  desc: "Balanced spacing" },
    { value: "spacious" as DensityCode,    label: "Spacious",     desc: "Relaxed — easier reading" },
  ];
    ...
    <div className="w-full">
      ...
      >
        <div className="mb-5">
          ...
          <ToggleGroup value={appearanceMode} onChange={(v) => { setAppearanceMode(v); setDirty(true); }} options={appearanceOptions} />
        </div>
      ...
      >
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            ...
            <Select value={homeWs} onValueChange={(v) => {
              setHomeWs(v);
              setHomeMod(HOME_MODULES_BY_WS[v]?.[0]?.value ?? "");
              setDirty(true);
            }}>
              <SelectTrigger className="h-9 text-sm"><SelectValue /></SelectTrigger>
              <SelectContent>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-medium text-foreground">{bff.activeOrg ? bff.activeOrg.replace("--", " / ") : "—"}</span>
        </div>
      </SectionCard>
