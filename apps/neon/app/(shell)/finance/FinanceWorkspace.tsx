...
export function FinanceWorkspace({ organizationName, periodLabel }: { organizationName: string; periodLabel: string }) {
  return (
    <main className="mx-auto w-full max-w-[1680px] space-y-6 p-5 sm:p-7 lg:p-9">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div><p className="text-sm font-medium text-primary">Finance</p><h1 className="mt-1 text-2xl font-semibold tracking-tight">{organizationName}</h1><p className="mt-1 text-sm text-muted-foreground">{periodLabel} · accounting operations and governance</p></div>
        <Link href="/workbench/finance" className="inline-flex h-10 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-medium text-primary-foreground"><Landmark className="h-4 w-4" />Finance Workbench</Link>
      </header>
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.6fr)]">
        <div className="rounded-xl border bg-card p-5"><div className="mb-4"><h2 className="font-semibold">Finance operations</h2><p className="mt-1 text-sm text-muted-foreground">Fast paths to daily and period-end work.</p></div><div className="grid gap-2 sm:grid-cols-2">{[
