...
export function DashboardHero({ userName }: DashboardHeroProps) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!trimmed) return;
    router.push(`/app/purchase_invoice?q=${encodeURIComponent(trimmed)}`);
  };
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-muted/70 via-muted/40 to-primary/[0.06] p-5 sm:p-7">
        ...
        <div className="relative flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2">
          ...
          >
            ...
            <span className="text-muted-foreground/80">· your Neon copilot</span>
          </button>
        </div>
        ...
        <div className="atlas-enter mt-5" style={{ animationDelay: "200ms" }}>
          ...
          >
            <label htmlFor="neon-search" className="sr-only">Ask Atlas</label>
            <input
              ref={inputRef}
              id="neon-search"
              type="search"
