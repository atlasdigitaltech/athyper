// GET /api/runtime/v1/entities/[entity]/[id]/process-state — current lifecycle state for one record.
...
export async function GET(
  ...
  if (!session) {
    return NextResponse.json(
      { error: "UNAUTHENTICATED", message: "Sign in again to load process state." },
      { status: 401 },
    );
  }
