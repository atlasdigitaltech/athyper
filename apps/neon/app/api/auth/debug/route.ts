// GET /api/auth/debug — local-only diagnostic that mirrors the current session's principal, realm, and token state for the diagnostics panel.
import "server-only";
...
export async function GET(): Promise<NextResponse> {
  // Hard 404 outside local to prevent leaking session/token internals from staging/prod.
