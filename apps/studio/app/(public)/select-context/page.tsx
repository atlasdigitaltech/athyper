...
export default async function SelectContextPage({
  ...
  return (
    <ContextGatePage
      plane="studio"
      returnTo={returnTo}
      contexts={await loadContexts(returnTo)}
    />
  );
}
async function loadContexts(
  returnTo: string,
): Promise<readonly IdentityContextOption[]> {
  const incoming = new Headers(await headers());
  const origin = readAppEnvironment().appOrigin;
  const response = await auth.contexts(
    new Request(`${origin}/api/auth/contexts`, { headers: incoming }),
  );
  if (response.status === 401)
    redirect(
      `/sign-in?reason=expired&returnTo=${encodeURIComponent(returnTo)}`,
    );
  if (!response.ok)
    redirect(
      `/sign-in?reason=service&returnTo=${encodeURIComponent(returnTo)}`,
    );
  const value = (await response.json()) as { contexts?: unknown };
  return Array.isArray(value.contexts)
    ? (value.contexts as IdentityContextOption[])
    : [];
}
