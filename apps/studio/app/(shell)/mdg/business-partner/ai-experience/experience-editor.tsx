...
export function AtlasExperienceEditor() {
  const client = useMemo(() => createAtlasExperienceAdminClient(), []),
    [definition, setDefinition] = useState(DEFAULT_DEFINITION),
  ...
  useEffect(() => {
    ...
    void client
      ...
      .catch(() => {
        ...
        setStatus("ready");
        setMessage(
          "No saved draft is available. Start from the governed Business Partner baseline below.",
        );
      });
    return () => {
      active = false;
      ...
      >
        {definition.searchSources.map((item, index) => (
          <ConfigRow
            key={item.code}
            enabled={item.enabled}
            onEnabled={(enabled) => update("searchSources", index, { enabled })}
            code={item.code}
          >
            <input
