...
export const platformRelay: RelayHandler = (request, context) => {
  relay ??= createRelayHandler({
    ...
    operations: [IAM_ME_OPERATION, EXPERIENCE_BOOTSTRAP_OPERATION, PRINCIPAL_LOCALE_UPDATE_OPERATION, MESH_NETWORK_ACCOUNTS_OPERATION, ENTITY_LIST_DESCRIPTOR_OPERATION, ENTITY_LIST_QUERY_OPERATION, ...RECORD_BOOKMARK_RELAY_OPERATIONS, ...RECORD_TRANSFER_RELAY_OPERATIONS, PLATFORM_VERIFICATION_SNAPSHOT_OPERATION, PLATFORM_VERIFICATION_RUN_OPERATION, ...ACTIVITY_CENTER_RELAY_OPERATIONS],
    session: {
