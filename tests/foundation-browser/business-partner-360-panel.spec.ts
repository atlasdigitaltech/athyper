...
if(code==='identity')return {sectionCode:code,data:{items:[{id:partner,kind:'canonical',displayName:'Cendana Office',code:'ATH-APAC-003',legalName:'Cendana Office Sdn Bhd',legalForm:'private_limited',registrationCountryCode:'MY'},{id:'industry',kind:'classification',industryDomainCode:'isic',industryCode:'4659',industryName:'Wholesale of machinery and equipment',industryCodeId:'technical-id'}]},provenance:[]};
if(['contacts','addresses','identifiers','governance'].includes(code))return {sectionCode:code,data:{items:[{id:code,displayName:code==='contacts'?'Amelia Hart':'Partner details',primary:true,lines:['Suite 12','18 Jalan Cendana'],countryCode:'MY'}]},provenance:[]};
if(code==='network')return {data:{live:{state:'not_requested'}}};
return {data:{items:[],roles:[],providers:[],openWork:{active:2}},provenance:[]};}};
export const useApiClient=()=>client;export const useSessionIdentity=()=>identity;
`
...
test("continuous navigation, real tabs, documents, and browser history", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "Maybank · •••• 4821" }),
  ).toBeVisible();
  const top = await page
    .locator('[data-record-section="banking"]')
