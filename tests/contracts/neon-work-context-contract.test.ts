...
import { parseNeonBusinessContextOptions, parseNeonWorkContextBootstrap } from "../../packages/platform/foundation/api-client/src/work-context";
...
describe("Neon work-context browser contract",()=>{
  ...
  it("parses v2 Legal Entity discovery and rejects company rows outside it",()=>{const options={...payload,schemaVersion:2 as const,actionPermissionCode:"neon.finance.invoice.read",legalEntities:[{legalEntityId:payload.companies[0].legalEntityId,code:payload.companies[0].legalEntityCode,displayName:payload.companies[0].legalEntityName,logoAssetRef:payload.companies[0].logoAssetRef}],organizations:[]};assert.equal(parseNeonBusinessContextOptions(options).legalEntities.length,1);assert.throws(()=>parseNeonBusinessContextOptions({...options,legalEntities:[]}),/discovered legal entity/);});
});
