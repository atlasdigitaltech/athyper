...
describe("Phase 4 hardened BFF relay", () => {
  ...
  it("allowlists governed Atlas conversation history and lifecycle routes",()=>{for(const signature of [["GET","/api/atlas/threads"],["GET","/api/atlas/threads/:threadId/messages"],["PATCH","/api/atlas/threads/:threadId"],["POST","/api/atlas/threads/:threadId/archive"]]as const)assert.ok(ATLAS_ANSWER_RELAY_OPERATIONS.some((item)=>item.method===signature[0]&&item.path===signature[1]));assert.equal(ATLAS_ANSWER_RELAY_OPERATIONS.filter((item)=>item.method==="PATCH"||item.path.endsWith("/archive")).every((item)=>item.idempotency==="required"),true);});
