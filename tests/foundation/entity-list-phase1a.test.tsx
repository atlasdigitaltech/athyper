...
} from "../../packages/platform/entity/runtime/list-view/src/index";
...
test("Phase 1A restores URL state and aborts stale list authority on context change", async () => {
  ...
  try {
    ...
    assert.equal(
      requests.length,
      0,
      "Do not resolve authority before scope bootstrap completes",
    );
    assert.ok(
      dom.window.document.querySelector(".a-entity-list--compact table"),
    );
    assert.equal(
      dom.window.document.querySelectorAll("header a, header button").length,
      0,
    );
    await act(async () =>
      root.render(
    ...
    assert.equal(
      dom.window.document.querySelector('a[href="/requests/new"]')?.textContent,
      "Permohonan baharu",
    );
    assert.doesNotMatch(
      dom.window.document.body.textContent ?? "",
      /Restricted action|Legacy label/,
    );
    const unavailable = dom.window.document.querySelector(
    ...
    assert.equal(
      dom.window.document.getElementById(
        unavailable.getAttribute("aria-describedby")!,
      )?.textContent,
...
test("initial loading reserves header, toolbar and table without claiming action authority", () => {
  const client = {
    request() {
      throw new Error("SSR must not fetch entity authority");
    },
  } as unknown as HttpClient;
  const html = renderToStaticMarkup(
    <EntityListRuntime client={client} entityCode="unknown_entity" />,
  );
  const document = new JSDOM(html).window.document;
  assert.equal(document.querySelectorAll("h1").length, 1);
  assert.ok(
    document.querySelector(".athyper-page-header__description .a-skeleton"),
  );
  ...
  assert.doesNotMatch(
    ...
    /Read-only|Manage · Collection|unknown_entity/,
  );
});
...
test("entity navigation renders one active section, localized overflow and positive attention counts", async () => {
  const { EntityNavigation } =
    await import("../../packages/platform/entity/runtime/list-view/src/navigation");
  ...
  const document = new JSDOM(
    renderToStaticMarkup(
      <EntityNavigation sections={sections} currentSurfaceKey="list" />,
    ),
  ).window.document;
  ...
  assert.equal(
    document.querySelectorAll(".a-management-navigation__count").length,
    1,
  );
  assert.equal(
    document.querySelector("details a")?.getAttribute("href"),
    "/review",
  );
...
test("navigation-only surface does not fetch records or rewrite its destination", async () => {
  const dom = new JSDOM('<div id="root"></div>', {
    url: "https://neon.test/review?status=pending",
  });
  ...
  try {
    const calls: string[] = [];
    const client = {
      request: async (operation: { id?: string; path?: string }) => {
        calls.push(JSON.stringify(operation));
        return descriptor("a".repeat(64));
      },
    } as unknown as HttpClient;
    const root = createRoot(dom.window.document.getElementById("root")!);
    await act(async () =>
...
for (const mode of ["table", "compact"] as const)
  test(`group disclosure collapses and restores ${mode} records without querying again`, async () => {
    const dom = new JSDOM('<div id="root"></div>', {
      url: "https://neon.test/partners?group=status",
      pretendToBeVisual: true,
    });
    const previous = [
      "window",
    ...
    const configured = {
      ...
      surface: {
        ...base.surface,
        supportedModes: ["table", "compact"] as const,
        defaultState: { ...base.surface.defaultState, mode },
      },
    };
    const client = {
      request: async (operation: unknown) => {
        requests++;
        if (operation === entityListDescriptorOperation) return configured;
        if (operation === recordBookmarkMembershipOperation) return new Set();
        if (operation === recordBookmarksOperation) return [];
        return page(digest("b"), "GROUPED-BP");
      },
    } as unknown as HttpClient;
    try {
      await act(async () =>
...
test("entity application keeps one header and scopes each collection's saved views", async () => {
  ...
  const root = createRoot(dom.window.document.getElementById("root")!);
  const base = descriptor(digest("b")),
    label = (en: string) => ({ defaultLocale: "en", values: { en } });
  const app = {
    ...base,
    application: {
      key: "partner_app",
  ...
  const client = {
    request: async (
      operation: unknown,
      options: { params: { entityCode: string } },
    ) => {
      const code = options.params.entityCode;
      if (operation === entityApplicationDescriptorOperation) {
        calls.push("application");
        return app;
      }
      if (operation === entityListDescriptorOperation) {
        calls.push(`descriptor:${code}`);
        return { ...base, entity: { ...base.entity, code } };
      }
      if (operation === recordBookmarkMembershipOperation) return new Set();
      if (operation === recordBookmarksOperation) return [];
      calls.push(`list:${code}`);
      return page(digest("b"), code);
    },
  } as unknown as HttpClient;
  const render = (section: string) =>
    root.render(
      <EntityListRuntime
        ...
        applicationOnly
      >
        <EntityApplicationSection key={section} sectionKey={section} />
      </EntityListRuntime>,
    );
  try {
    ...
    assert.deepEqual(calls, [
      "application",
      "descriptor:business_partner",
      "list:business_partner",
    ]);
    assert.equal(dom.window.document.querySelectorAll("h1").length, 1);
    assert.ok(dom.window.document.querySelector(".a-entity-pulse"));
    ...
    assert.equal(dom.window.document.querySelectorAll("h1").length, 1);
    assert.ok(
      calls.includes("list:business_partner"),
      JSON.stringify({ calls, body: dom.window.document.body.textContent }),
    );
    assert.equal(
      dom.window.localStorage.getItem(
        "athyper.entity-list.views.neon.partner_app.manage.business_partner",
      ),
      "[]",
    );
    await act(async () => render("review"));
    assert.equal(dom.window.document.querySelectorAll("h1").length, 1);
    assert.ok(calls.includes("list:partner_request"));
    assert.equal(calls.filter((call) => call === "application").length, 1);
    assert.ok(dom.window.document.querySelector(".a-entity-list__panel"));
    assert.equal(
      dom.window.localStorage.getItem(
        "athyper.entity-list.views.neon.partner_app.review.partner_request",
      ),
      null,
    );
  } finally {
...
test("canonical entity route preserves saved views and repeated query values", async () => {
  const { entityRouteAlias } =
    await import("../../apps/neon/lib/entity-route-alias");
  assert.equal(
    entityRouteAlias("/mdg/business-partner/manage", {
      vid: "saved-view",
      density: "compact",
      filter: ["active", "uk"],
      unused: undefined,
    }),
    "/mdg/business-partner/manage?vid=saved-view&density=compact&filter=active&filter=uk",
  );
});
...
test("application failure replaces loading header with themed retry state", async () => {
  ...
  const client = {
    request: async () => {
      calls++;
      throw new Error("The requested platform operation is not allowlisted");
    },
  } as unknown as HttpClient;
  try {
    await act(async () =>
      root.render(
        <EntityListRuntime
          ...
          scopeControl={<span>Scope icon</span>}
        />,
      ),
    );
    assert.equal(
      dom.window.document.querySelectorAll(".athyper-page-header").length,
      0,
    );
    assert.doesNotMatch(
      dom.window.document.body.textContent ?? "",
      /Loading application|Loading list|Scope icon/,
    );
    assert.equal(
...
test("server view defaults respect URL, personal, shared and system precedence", async () => {
  const { readListLocation, writeListLocation } =
    await import("../../packages/platform/entity/runtime/list-view/src/location");
  const dom = new JSDOM("<div/>", { url: "https://neon.test/manage" });
  ...
  try {
    const base = descriptor(digest("b")),
      state = { ...base.surface.defaultState, density: "compact" as const };
    const shared = {
      id: "shared",
      name: "Shared",
    ...
    const configured = {
      ...base,
      viewCatalog: {
        views: [shared, { ...shared, id: "mine", scope: "personal" as const }],
        sharedDefault: "shared",
        personalDefault: "mine",
        capabilities: {
          createShared: false,
          manageShared: false,
          setSharedDefault: false,
        },
      },
    };
    ...
    assert.equal(
      readListLocation(
        {
          ...configured,
          viewCatalog: { ...configured.viewCatalog, personalDefault: "system" },
        },
        "",
      ).savedViewId,
      "system",
    ...
    assert.equal(
      readListLocation(configured, "?vid=shared").savedViewId,
      "shared",
    );
    assert.equal(
      readListLocation(configured, "?cols=code").savedViewId,
      undefined,
    );
    assert.equal(
      readListLocation(
    ...
    const reset = { ...base.surface.defaultState, savedViewId: "system" };
    writeListLocation(reset, configured, "replace");
    assert.equal(readListLocation(configured).savedViewId, "system");
    assert.equal(
      dom.window.localStorage.length,
      0,
      "Server views are never cached into unscoped browser storage",
    );
  } finally {
...
test("Manage views exposes System default and permission-gated tenant defaults", async () => {
    ...
    views: [
      {
        ...
        name: "Shared active",
        scope: "shared" as const,
        state: base.surface.defaultState,
        version: 1,
        compatible: true,
      },
    ],
    personalDefault: undefined as string | undefined,
    sharedDefault: "11111111-1111-4111-8111-111111111111",
    capabilities: {
      createShared: false,
      manageShared: false,
      setSharedDefault: false,
  ...
  const client = {
    request: async (
      ...
      if (operation === entityListDescriptorOperation) return base;
      if (operation === entityViewsOperation) return catalog;
      if (operation === entityViewCommandOperation) {
        commands.push(options.body!);
        catalog = { ...catalog, personalDefault: String(options.body!.id) };
        return catalog;
      }
      if (operation === recordBookmarkMembershipOperation) return new Set();
      if (operation === recordBookmarksOperation) return [];
      return page(digest("b"), "VIEW-BP");
    },
  } as unknown as HttpClient;
  const root = createRoot(dom.window.document.getElementById("root")!);
  try {
    ...
    assert.equal(
      new URLSearchParams(dom.window.location.search).get("vid"),
      catalog.sharedDefault,
    );
    await act(async () =>
      dom.window.document
        .querySelector<HTMLButtonElement>('[aria-label="Select view"]')!
        .click(),
    );
    await act(async () =>
      [...dom.window.document.querySelectorAll<HTMLButtonElement>("button")]
        .find((button) => button.textContent === "Manage views…")!
        .click(),
    );
    ...
    assert.deepEqual(
      ...
      ["Available views", "Save current configuration"],
    );
    assert.equal(viewTabs[0]!.getAttribute("aria-selected"), "true");
    assert.ok(
      drawer.querySelector("#entity-list-view-name")?.closest("[hidden]"),
    );
    await act(async () => viewTabs[1]!.click());
    assert.equal(
      drawer.querySelector("#entity-list-view-name")?.closest("[hidden]"),
      null,
    );
    ...
    assert.match(
      drawer.textContent ?? "",
      /Standard views.*My views.*Shared views/,
    );
    assert.match(drawer.textContent ?? "", /System default/);
    assert.doesNotMatch(drawer.textContent ?? "", /Set tenant default/);
    assert.equal(drawer.querySelectorAll('option[value="shared"]').length, 0);
    await act(async () =>
      [...drawer.querySelectorAll<HTMLButtonElement>("button")]
        .find((button) => button.textContent === "Make my default")!
        .click(),
    );
    ...
    await act(async () =>
      [...drawer.querySelectorAll<HTMLButtonElement>("button")]
        .find((button) => button.textContent === "Reset to system default")!
        .click(),
    );
    assert.equal(
      new URLSearchParams(dom.window.location.search).get("vid"),
...
test("column filters preserve other fields, synchronize chips and dismiss without sorting", async () => {
  ...
  const configured = {
    ...
    fields: base.fields.map((field) =>
      field.key === "code" ? { ...field, filterOperators: [] } : field,
    ),
    surface: {
      ...base.surface,
      defaultState: {
        ...base.surface.defaultState,
        filters: [
          { field: "name", operator: "contains" as const, value: "Acme" },
        ],
      },
    },
  };
  ...
  try {
    ...
    assert.ok(
      dom.window.document.querySelector(
        '[aria-label="Filter Status, 1 active"]',
      ),
    );
    assert.match(
      dom.window.document.querySelector('[aria-label="Applied filters"]')
        ?.textContent ?? "",
      /Display Name contains Acme.*Status is Draft/,
    );
    assert.equal(
      dom.window.document
        .querySelector("th[aria-sort]")
    ...
    assert.ok(
      dom.window.document.querySelector('[aria-label="Filter Status"]'),
    );
    assert.match(
      dom.window.document.querySelector('[aria-label="Applied filters"]')
        ?.textContent ?? "",
      /Display Name contains Acme/,
    );
    assert.doesNotMatch(
      dom.window.document.querySelector('[aria-label="Applied filters"]')
        ?.textContent ?? "",
...
test("one drawer selector preserves drafts within a session and uses metadata availability", async () => {
  ...
  try {
    ...
    assert.ok(
      dom.window.document.querySelector(
        '[data-list-drawer="sort"][hidden][inert]',
      ),
    );
    const applyGroup = [
      ...dom.window.document.querySelectorAll<HTMLButtonElement>(
        '[data-list-drawer="group"] button',
      ),
    ].find((button) => button.textContent === "Apply grouping")!;
    await act(async () => {
      applyGroup.focus();
      applyGroup.dispatchEvent(
        new dom.window.KeyboardEvent("keydown", {
          key: "Tab",
          bubbles: true,
          cancelable: true,
    ...
    const { LIST_DRAWERS } =
      await import("../../packages/platform/entity/runtime/list-view/src/drawer-registry");
    const restricted = {
      ...base,
      fields: base.fields.map((field) => ({
        ...field,
    ...
    assert.deepEqual(
      LIST_DRAWERS.filter((item) => item.available(restricted)).map(
        (item) => item.key,
      ),
      ["columns", "display", "views"],
    );
  } finally {
    await act(async () => root.unmount());
    for (const [key, value] of previous) {
...
test("standard view keys remain symbolic through saved configuration and portable URLs", async () => {
  const { encodeListLocationState, decodeListLocationState } =
    await import("../../packages/contracts/platform/entity-list/src/index");
  const { saveableViewState } =
    await import("../../packages/platform/entity/runtime/list-view/src/preferences");
  const base = descriptor(digest("b")),
    state = { ...base.surface.defaultState, standardViewKey: "mine" };
  const saved = saveableViewState(state);
  assert.equal(saved.standardViewKey, "mine");
  assert.equal(JSON.stringify(saved).includes("principalId"), false);
  const url = encodeListLocationState(state, base, { includeViewIds: false });
  assert.equal(url.get("standardView"), "mine");
  assert.equal(decodeListLocationState(url, base).standardViewKey, "mine");
});
...
test("field-aware filters select reference keys, reuse applied choices, and reject invalid dates", async () => {
  const {
    ...
    await import("../../packages/platform/entity/runtime/list-view/src/filter-editor");
  const { filterValueFromInput, filterInputValue, describeFilter } =
    await import("../../packages/platform/entity/runtime/list-view/src/state");
  const base = descriptor("user-a"),
    country = {
      ...base.fields[0]!,
      key: "country",
      label: "Country",
      valueKind: "reference" as const,
      filterOperators: ["eq", "in", "is_null"] as const,
      filterOptions: [
  ...
  const date = {
    ...
    filterOperators: ["between", "relative", "eq"] as const,
  };
  assert.match(
  ...
  assert.equal(
    filterValueFromInput("relative", "last_30_days", "datetime"),
    "last_30_days",
  );
  assert.equal(
    filterInputValue(
      { field: "updated", operator: "relative", value: "this_month" },
      "datetime",
    ),
    "this_month",
  );
  assert.equal(
    filterValueFromInput("eq", "2", "enum", [{ value: 2, label: "Two" }]),
    2,
  );
  assert.equal(
    describeFilter(
      { field: "country", operator: "in", value: ["MY", "SG"] },
      { ...base, fields: [country] },
    ),
    "Country is any of Malaysia, Singapore",
  );
  const dom = new JSDOM('<div id="root"></div>', {
    url: "https://neon.test/manage",
    pretendToBeVisual: true,
  ...
  const root = createRoot(dom.window.document.getElementById("root")!);
  const key = recentFilterKey(base);
  let selected = "";
  const render = async () =>
    act(async () =>
      root.render(
  ...
  try {
    await render();
    const input =
      dom.window.document.querySelector<HTMLInputElement>('[role="combobox"]')!;
    await act(async () =>
      input.dispatchEvent(
        new dom.window.MouseEvent("click", { bubbles: true }),
      ),
    ...
    assert.equal(
      selected,
      "MY",
      "The reference key, not its name, is submitted",
    );
    assert.equal(
      dom.window.sessionStorage.length,
      0,
      "Draft choices are not remembered",
    );
    await render();
    assert.ok(
      dom.window.document.querySelector('[role="option"][aria-selected="true"]'),
    );
    rememberFilters(
      key,
      [{ field: country.key, operator: "in", value: ["MY"] }],
      [country],
    );
    selected = "";
    await render();
    const recent = dom.window.document.querySelector<HTMLElement>(
      '[role="group"][aria-label="Recently selected"] [role="option"]',
    )!;
    assert.match(recent.textContent!, /Malaysia/);
    await act(async () => {
      recent.dispatchEvent(new dom.window.MouseEvent("click", { bubbles: true }));
    });
    ...
    await act(async () =>
      root.render(
        <FilterValueEditor
          ...
          value=""
          filterNumber={1}
          historyKey={recentFilterKey(descriptor("user-b"))}
          onChange={() => {}}
        />,
      ),
    );
    assert.equal(
      dom.window.document.querySelector(
        '[role="group"][aria-label="Recently selected"]',
      ),
      null,
      "History never crosses authorization scopes",
    );
  } finally {
    await act(async () => root.unmount());
    for (const [key, value] of previous) {
...
test("directory filters are metadata-gated and stage company changes until Apply", async () => {
  ...
  const configured = {
    ...base,
    scope: { ...base.scope, filterKinds: ["organization", "company"] as const },
  };
  const client = {
    request: async (operation: unknown) =>
      operation === entityListDescriptorOperation
  ...
  const company = {
    ...
    code: "DE01",
    displayName: "Germany Operations",
    legalEntityId: "legal",
    legalEntityCode: "LE-DE",
    legalEntityName: "Germany Legal Entity",
    countryCode: "DE",
      ...
      <DirectoryFilterContext.Provider
        value={{
          ...
          companies: [
            ...
            {
              ...company,
              companyCodeId: "second",
              code: "DE02",
              displayName: "Germany Sales",
            },
          ],
          organizations: [
            {
              id: "org",
              displayName: "Europe Operations",
              companyAssignments: [],
            },
          ],
          apply: (next) => {
  ...
  try {
    ...
    await clickText("Organization");
    await click(".a-directory-organizations input");
    await clickText("Company");
    assert.equal(
      dom.window.document.querySelector<HTMLInputElement>(
        ".a-company-groups__row input",
      )!.checked,
      true,
    );
    ...
    await act(async () => root.render(<App key="unsupported" />));
    await click('[aria-label="Filters"]');
    assert.ok(
      ![...dom.window.document.querySelectorAll('[role="tab"]')].some(
        (e) => e.textContent === "Company" || e.textContent === "Organization",
      ),
    );
  } finally {
    await act(async () => root.unmount());
    for (const [key, property] of previous) {
...
test("scope quick filters stage values, enforce dependencies and reset", async () => {
  ...
  const configured = {
    ...
    scope: {
      ...base.scope,
      filterKinds: ["organization", "company"] as const,
      quickFilters: [
        {
          key: "partnerRole" as const,
          label: "Role",
          emptyLabel: "All partners",
          options: [{ value: "supplier", label: "Suppliers" }],
        },
        {
          key: "eligibleOperation" as const,
          label: "Eligible for transactions",
          emptyLabel: "Any",
          options: [{ value: "order", label: "Orders" }],
          requires: ["partnerRole", "organization", "company"] as const,
        },
      ],
    },
  };
...
test("child queue resolves its own context without inheriting parent directory filters", async () => {
  const dom = new JSDOM('<div id="root"></div>', {
    url: "https://neon.test/partners/requests",
    pretendToBeVisual: true,
  });
  const previous = [
    "window",
    ...
    navigation: [
      {
        ...
        href: "/partners/requests",
        aliases: [],
        label: { defaultLocale: "en", values: { en: "Review" } },
        content: { kind: "task_list", entityCode: "partner_request" },
      },
    ],
  };
  ...
  const client = {
    request: async (
      operation: unknown,
      options: {
        params: { entityCode: string };
        query?: Record<string, unknown>;
      },
    ) => {
      if (operation === entityApplicationDescriptorOperation) return app;
      if (operation === entityListDescriptorOperation) {
        calls.push(options.query ?? {});
        if (options.query?.operatingOrganizationId === "denied-org")
          throw new Error("Context rejected");
        return {
          ...base,
          entity: { ...base.entity, code: "partner_request" },
          scope: {
            ...base.scope,
            status: options.query?.operatingOrganizationId
              ? "ready"
              : "context_required",
            workContext,
          },
  ...
  try {
    await act(async () =>
      root.render(
        <EntityListRuntime
          ...
          entityCode="business_partner"
          applicationOnly
          scopeCoordinate={{ companyCodeIds: ["parent-company"] }}
          renderScopeControl={({ scope, onChange }) =>
            scope.workContext ? (
              <>
                <button
                  id="choose-queue-context"
                  onClick={() =>
                    onChange({ operatingOrganizationId: "queue-org" })
                  }
                >
                  Choose queue organization
                </button>
                <button
                  id="denied-queue-context"
                  onClick={() =>
                    onChange({ operatingOrganizationId: "denied-org" })
                  }
                >
        ...
        >
          <EntityApplicationSection sectionKey="review" />
        </EntityListRuntime>,
      ),
    );
    assert.equal(listCalls, 0);
    assert.equal(calls[0]?.companyCodeIds, undefined);
    assert.equal(
      dom.window.document.querySelectorAll("#choose-queue-context").length,
      1,
    );
    await act(async () => {
      (
        dom.window.document.querySelector(
          "#choose-queue-context",
        ) as HTMLElement
      ).click();
    });
    assert.equal(calls.at(-1)?.operatingOrganizationId, "queue-org");
    assert.equal(calls.at(-1)?.companyCodeIds, undefined);
    assert.ok(listCalls > 0);
    const beforeRejected = listCalls;
    await act(async () => {
      (
        dom.window.document.querySelector(
          "#denied-queue-context",
        ) as HTMLElement
      ).click();
    });
    assert.equal(listCalls, beforeRejected);
    assert.ok(dom.window.document.querySelector("#choose-queue-context"));
    await act(async () => {
      (
        dom.window.document.querySelector(
          "#choose-queue-context",
    ...
    assert.ok(listCalls > beforeRejected);
  } finally {
    await act(async () => root.unmount());
    for (const [key, value] of previous) {
      if (value) Object.defineProperty(globalThis, key, value);
...
test("embedded chooser reuses directory, selects one record, and leaves parent URL unchanged", async () => {
  const {parseEntityLookupOptions} = await import("../../packages/contracts/platform/entity-runtime/src/lookup-options");
  const dom = new JSDOM("<!doctype html><div id='lookup-root'></div>", {url:"https://neon.test/request/new?vid=unrelated&q=parent"});
  const keys = ["window","document","navigator","Element","HTMLElement","HTMLInputElement","HTMLButtonElement","HTMLAnchorElement","MouseEvent","IS_REACT_ACT_ENVIRONMENT"];
  const prior = keys.map(key => [key,Object.getOwnPropertyDescriptor(globalThis,key)] as const);
  for(const key of keys)Object.defineProperty(globalThis,key,{configurable:true,value:key==="IS_REACT_ACT_ENVIRONMENT"?true:(dom.window as any)[key]});
  const queries: any[] = [];
  const client = {request(operation:unknown,input:any) {
    if(operation === entityListDescriptorOperation)return Promise.resolve(descriptor(digest("1")));
    if(operation === recordBookmarkMembershipOperation)return Promise.resolve(new Set());
    queries.push(input.query);
    const data=page(digest("1"),"A");return Promise.resolve({...data,rows:[...data.rows,...page(digest("1"),"B").rows]});
  }} as HttpClient;
  const root=createRoot(dom.window.document.getElementById("lookup-root")!);
  let chosen: readonly any[]=[];
  const options=parseEntityLookupOptions({mode:"choose",selectionMode:"single",presentation:{viewType:"full",fullViewHost:"inline"}});
  function Harness(){const [rows,setRows]=React.useState<readonly any[]>([]);return <EntityListRuntime client={client} entityCode="business_partner" contentOnly embedding={{options,selectedRows:rows,onSelectionChange:next=>{chosen=next;setRows(next);}}}/>;}
  try {
    await act(async()=>root.render(<Harness/>));
    assert.equal(dom.window.location.search,"?vid=unrelated&q=parent");
    assert.ok(queries.every(q=>q.q!=="parent"));
    assert.equal(dom.window.document.querySelectorAll('input[type="radio"]').length,2);
    assert.equal(dom.window.document.querySelectorAll('.a-entity-list__record-link').length,0);
    const radios=dom.window.document.querySelectorAll<HTMLInputElement>('input[type="radio"]');
    await act(async()=>radios[0]!.click());await act(async()=>radios[1]!.click());
    assert.deepEqual(chosen.map(row=>row.id),["B-id"]);
    assert.equal(dom.window.document.querySelectorAll('input[type="radio"]:checked').length,1);
