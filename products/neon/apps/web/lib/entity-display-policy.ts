...
const HEURISTIC_COLUMNS = ["code", "name", "title", "label", "display_name", "description"];
...
export async function resolveDisplayPolicy(
    ...
    try {
        ...
        if (l2) {
            displayPolicyCache.set(l1Key, { policy: l2, expiresAt: Date.now() + CACHE_TTL_MS });
        ...
        if (!acquired) {
            ...
            if (retry) {
                displayPolicyCache.set(l1Key, { policy: retry, expiresAt: Date.now() + CACHE_TTL_MS });
                return retry;
            }
        }
    } catch {
    ...
    const ui = (entityFeatureFlags as any)?.ui as Record<string, unknown> | undefined;
    ...
    const policy: DisplayPolicy = {
        ...
        fields: !template && fields ? fields.filter((f) => availableCols.has(f)) : undefined,
        resolvedColumns,
        primaryKey,
        isCompositePk,
    };
    ...
    try {
        const ns = await getNamespaceVersion();
        const bytes = await cacheSet(dpKey(ns, schema, table), policy, REDIS_TTL.displayPolicy);
 ...
 */
export async function invalidateDisplayPolicyCache(schema?: string, table?: string): Promise<void> {
    if (schema && table) {
