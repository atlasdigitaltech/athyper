...
const GENERAL_FIELDS = new Set(["code", "name", "title", "description", "label", "display_name"]);
const HIERARCHY_FIELDS = new Set(["parent_id", "org_unit_id", "level", "inherit_from_parent"]);
const STATUS_FIELDS = new Set(["status", "is_active", "activated_at", "sunset_at"]);
...
const SYSTEM_FIELDS = new Set([
    ...
    "entity_type_code", "source_system",
]);
...
export function groupFieldsIntoSections(
    ...
    for (const field of fields) {
        ...
        // Skip system-origin fields in business forms (hidden by default)
        if (field.origin === "system") continue;

        const col = field.columnName;

        // 1. Explicit field override — uiHint.section > validation.ui.section (legacy)
        const uiHint = field.uiHint as Record<string, unknown> | undefined;
        const validationUi = (field.validation as any)?.ui as Record<string, unknown> | undefined;
        const fieldSection = (uiHint?.section ?? validationUi?.section) as string | undefined;
        if (fieldSection) {
            appendToSection(sectionFieldsMap, sectionMetaMap, fieldSection, col, sectionLabels);
            continue;
        }
        ...
        if (entityOverride) {
            appendToSection(sectionFieldsMap, sectionMetaMap, entityOverride, col, sectionLabels);
            continue;
        }
 ...
 *   1. field.isComputed === true → COMPUTED
 *   2. field.writeOnce === true && viewMode=edit → WRITE_ONCE
 *   3. field.isReadOnly === true → EXPLICIT_META
 *   4. uiHint.readOnly === true OR validation.ui.readOnly === true → EXPLICIT_META
 *   5. uiHint.lockOnEdit === true OR validation.ui.lockOnEdit === true && viewMode=edit → LOCK_ON_EDIT
 *   6. field.origin === "system" → SYSTEM_ORIGIN
 *   7. columnName === "level" → DERIVED_HIERARCHY
 *   8. columnName in [activated_at, sunset_at] → LIFECYCLE_MANAGED
 *   9. columnName in [created_at, updated_at, version] → SYSTEM_MANAGED
 *
...
export function getFieldEditBehavior(
    ...
    // Rule 1: Computed fields are always read-only
    if (field.isComputed) {
        return { readOnly: true, reason: "COMPUTED" };
    }

    // Rule 2: Write-once in edit mode
    if (field.writeOnce && viewMode === "edit") {
        return { readOnly: true, reason: "WRITE_ONCE" };
    }

    // Rule 3: Explicit read-only from field flag
    if (field.isReadOnly) {
        return { readOnly: true, reason: "EXPLICIT_META" };
    }

    // Rule 4: Read-only from UI hint or legacy validation.ui
    if (uiHint?.readOnly === true || validationUi?.readOnly === true) {
        return { readOnly: true, reason: "EXPLICIT_META" };
    }

    // Rule 5: Lock on edit (editable on create, locked on edit)
    if ((uiHint?.lockOnEdit === true || validationUi?.lockOnEdit === true) && viewMode === "edit") {
        return { readOnly: true, reason: "LOCK_ON_EDIT" };
    }

    // Rule 6: System-origin fields
    if (field.origin === "system") {
        return { readOnly: true, reason: "SYSTEM_ORIGIN" };
    }

    // Rule 7: Derived hierarchy
    if (DERIVED_HIERARCHY_COLUMNS.has(field.columnName)) {
        return { readOnly: true, reason: "DERIVED_HIERARCHY" };
    }

    // Rule 8: Lifecycle-managed
    if (LIFECYCLE_MANAGED_COLUMNS.has(field.columnName)) {
        return { readOnly: true, reason: "LIFECYCLE_MANAGED" };
    }

    // Rule 9: System-managed
