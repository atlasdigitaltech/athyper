...
import type { InvoiceLineDTO } from "@/lib/finance/types";
import type { JournalLineDTO } from "@/lib/finance/types";
import type { PaymentAllocationDTO, UnpaidInvoiceDTO } from "@/lib/finance/types";
import type { JournalEntryDTO, DecisionEvaluationDTO } from "@/lib/finance/types";
 ...
 */
function mapAllocationRow(dto: PaymentAllocationDTO, unpaidMap: Map<string, UnpaidInvoiceDTO>) {
...
function mapDecisionResult(dto: DecisionEvaluationDTO) {
    return {
        ...
        approvalRoute: dto.approvalRoute as "ZERO_APPROVAL" | "STANDARD" | "ENHANCED" | "EXECUTIVE" | "BLOCKED",
        moduleScores: {} as Record<string, number>,
        exceptions: dto.exceptions.map((ex) => `[${ex.severity}] ${ex.policyCode}: ${ex.message}`),
...
function InvoiceLinesTab({ entityId }: TabPluginProps) {
    ...
    if (loading) return <div className="p-6"><LoadingIndicator /></div>;
...
function JournalLinesTab({ entityId }: TabPluginProps) {
    ...
    const totalDebits = entry?.totalDebit ?? sumStringAmounts(jeLines.map((l) => l.debitAmount));
    const totalCredits = entry?.totalCredit ?? sumStringAmounts(jeLines.map((l) => l.creditAmount));
    const balanceDifference = subtractStrings(totalDebits, totalCredits);
    const isBalanced = balanceDifference === "0.00" || balanceDifference === "-0.00";
...
function PaymentAllocationsTab({ entityId }: TabPluginProps) {
    ...
    const mappedAllocations = allocations.map((a) => mapAllocationRow(a, unpaidMap));

    const totalAllocated = sumStringAmounts(allocations.map((a) => a.allocatedAmount));
    const unallocatedAmount = subtractStrings(paymentTotal, totalAllocated);
    const isFullyAllocated = unallocatedAmount === "0.00" || unallocatedAmount === "-0.00";
