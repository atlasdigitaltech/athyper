...
// Lookup map keyed by record ID for O(1) access in single-record endpoint
export const PURCHASE_INVOICES_BY_ID = Object.fromEntries(
  PURCHASE_INVOICES.map((p) => [p.id, p]),
);
...
> = {
  ...
  "chart-of-accounts": { list: ACCOUNTS, byId: ACCOUNTS_BY_ID },
  "purchase-invoice": {
    list: PURCHASE_INVOICES,
    byId: PURCHASE_INVOICES_BY_ID,
  },
};
