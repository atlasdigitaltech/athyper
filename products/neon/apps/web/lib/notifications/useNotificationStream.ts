  ...
  const connect = useCallback(() => {
    ...
    eventSource.addEventListener("notification.new", (e) => {
      try {
        ...
        console.error(
          "[NotificationStream] Failed to parse notification.new event:",
          err,
        );
      }
    });
    ...
    eventSource.addEventListener("notification.read", (e) => {
      try {
        ...
        console.error(
          "[NotificationStream] Failed to parse notification.read event:",
          err,
        );
      }
    });
    ...
    eventSource.addEventListener("notification.dismissed", (e) => {
      try {
        ...
        console.error(
          "[NotificationStream] Failed to parse notification.dismissed event:",
          err,
        );
      }
    });
    ...
    eventSource.onerror = () => {
      console.warn("[NotificationStream] Connection error");
      eventSource.close();
      ...
      console.warn(
        `[NotificationStream] Reconnecting in ${delay}ms (attempt ${reconnectAttemptsRef.current})`,
      );
