...
export function computeSessionState(
  ...
  const accessExpiresAt =
    typeof session.accessExpiresAt === "number" ? session.accessExpiresAt : 0;
  const lastSeenAt =
    typeof session.lastSeenAt === "number" ? session.lastSeenAt : 0;
  const refreshExpiresAt =
    typeof session.refreshExpiresAt === "number" ? session.refreshExpiresAt : 0;

  // Token remaining (seconds until access token expires)
  const tokenRemaining =
    accessExpiresAt > 0 ? Math.max(0, accessExpiresAt - now) : null;

  // Idle remaining (time until idle timeout fires)
  const idleSince = lastSeenAt > 0 ? now - lastSeenAt : 0;
  const idleRemaining =
    lastSeenAt > 0 ? Math.max(0, IDLE_TIMEOUT_SEC - idleSince) : null;
  ...
  const tokenExpiring =
    !tokenExpired && accessExpiresAt > 0 && accessExpiresAt - now < 120;
  const tokenState: TokenState = tokenExpired
    ? "expired"
    : tokenExpiring
      ? "expiring"
      : "valid";
  ...
  if (lastSeenAt > 0 && idleSince >= IDLE_TIMEOUT_SEC) {
    ...
    lastSeenAt > 0 &&
    idleRemaining !== null &&
    idleRemaining <= IDLE_WARNING_SEC
  ) {
