...
export function useApprovalInstance(
  ...
  const fetchInstance = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/collab/approvals/${encodeURIComponent(instanceId)}`,
        {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          body.error?.message ?? `Failed to load approval (${res.status})`,
        );
      }
  ...
  const currentUserTask =
    instance?.stages
      .flatMap((s) => s.tasks)
      .find((t) => t.status === "pending" && t.taskType === "current_user") ??
    null;
