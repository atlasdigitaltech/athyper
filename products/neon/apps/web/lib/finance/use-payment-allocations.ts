...
import type { PaymentAllocationDTO, UnpaidInvoiceDTO, CreateAllocationInput } from "./types";
...
export interface UsePaymentAllocationsResult {
    ...
    loading: boolean;
    error: string | null;
    saveAllocations: (allocations: CreateAllocationInput[], version: number) => Promise<void>;
    refresh: () => void;
}
...
export function usePaymentAllocations(paymentId: string | undefined): UsePaymentAllocationsResult {
    ...
    const saveAllocations = useCallback(
        async (inputAllocations: CreateAllocationInput[], version: number) => {
            ...
            try {
                ...
                await finPost<{ ok: boolean }>(url, { allocations: inputAllocations, version });
