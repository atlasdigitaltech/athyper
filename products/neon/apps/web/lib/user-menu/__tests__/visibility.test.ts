...
function makeInput(
  overrides: Partial<MenuVisibilityInput> = {},
): MenuVisibilityInput {
  return {
    personas: [],
...
describe("deriveCapabilities", () => {
  ...
  it("grants legal profile to compliance_officer", () => {
    const caps = deriveCapabilities(
      makeInput({ personas: ["compliance_officer"] }),
    );
    expect(caps.has(CAP.LEGAL_PROFILE_READ)).toBe(true);
  });
  ...
  it("does not grant viewport in production for regular users", () => {
    const caps = deriveCapabilities(
      makeInput({ personas: ["viewer"], environment: "production" }),
    );
...
describe("computeVisibility", () => {
  ...
  it("gates savedSuppliers behind feature flag", () => {
    const without = computeVisibility(
      makeInput({ personas: ["tenant_admin"] }),
    );
  ...
  it("grants viewport in local env for any user", () => {
    const result = computeVisibility(
      makeInput({ personas: ["viewer"], environment: "local" }),
    );
