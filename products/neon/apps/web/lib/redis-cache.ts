...
async function getClient(): Promise<RedisClient | null> {
    ...
    _connecting = (async () => {
        try {
            ...
            client.on("error", (err: Error) => console.warn("[redis-cache] error:", err.message));
...
export async function getNamespaceVersion(): Promise<number> {
    if (_nsVersion !== null && Date.now() < _nsVersionExpiresAt) return _nsVersion;
...
export function fieldsKey(ns: number, tenantId: string, entityName: string, versionId: string): string {
...
export function refKey(ns: number, tenantId: string, schema: string, table: string, id: string): string {
    return `ep:ref:ns${ns}:${tenantId}:${schema}.${table}:${id}`;
}

export function queryResultKey(ns: number, tenantId: string, entity: string, gen: number, queryHash: string): string {
    return `ep:qr:ns${ns}:${tenantId}:${entity}:g${gen}:${queryHash}`;
}

export function queryCountKey(ns: number, tenantId: string, entity: string, gen: number, filterHash: string): string {
...
export async function cacheSet(key: string, value: unknown, ttlSeconds: number): Promise<number> {
    const json = JSON.stringify(value);
    const bytes = Buffer.byteLength(json, "utf8");
    if (bytes > PAYLOAD_MAX_BYTES) {
        console.error(`[redis-cache] payload too large, skipping cache: ${key} = ${bytes} bytes`);
        return bytes;
    }
...
export async function cacheMSet(entries: Array<{ key: string; value: unknown; ttl: number }>): Promise<void> {
    const client = await getClient();
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            const bytes = Buffer.byteLength(json, "utf8");
            if (bytes > PAYLOAD_MAX_BYTES) {
                console.error(`[redis-cache] payload too large, skipping: ${entry.key} = ${bytes} bytes`);
                continue;
            }
...
export async function acquireComputeLock(lockKey: string, ttlSec: number): Promise<boolean> {
    const client = await getClient();
