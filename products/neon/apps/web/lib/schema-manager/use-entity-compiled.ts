  ...
  const fetchCompiled = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          body.error?.message ??
            `Failed to load compiled snapshot (${res.status})`,
        );
      }
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load compiled snapshot",
      );
    } finally {
  ...
  const recompile = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const body = (await res.json()) as { error?: { message?: string } };
        throw new Error(
          body.error?.message ?? `Recompilation failed (${res.status})`,
        );
      }
