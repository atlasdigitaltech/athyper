...
export function usePolicyMutations({
  ...
  const duplicateRule = useCallback(
    async (rule: Record<string, unknown>) => {
      ...
      return apiCall(`${base}/rules`, "POST", {
        ...rest,
        priority: ((rest.priority as number) ?? 0) + 1,
      });
    },
    [apiCall, base],
  );
  ...
  const updateRule = useCallback(
    async (ruleId: string, rule: Record<string, unknown>) => {
  ...
  const publishVersion = useCallback(
    async (versionId: string) => {
      return apiCall(`${base}/versions`, "POST", {
        action: "publish",
        versionId,
      });
    },
