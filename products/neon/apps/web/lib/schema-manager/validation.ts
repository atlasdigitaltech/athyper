...
export function validateField(
  ...
  if (field.name && !/^[a-z][a-z0-9_]*$/.test(field.name)) {
    errors.push({
      code: "FIELD_NAME_FORMAT",
      field: "name",
      message: "Field name must be lowercase snake_case",
    });
  }
  ...
  if (RESERVED_FIELD_NAMES.has(field.name)) {
    errors.push({
      ...
      field: "name",
      message: `'${field.name}' is a reserved system field name`,
    });
  }
  ...
  if (nameConflict) {
    errors.push({
      ...
      field: "name",
      message: `A field named '${field.name}' already exists`,
    });
  }
  ...
  if (field.columnName) {
    ...
    if (columnConflict) {
      errors.push({
        ...
        field: "columnName",
        message: `Column name '${field.columnName}' is already in use`,
      });
    }
  }
...
export function validateRelation(
  ...
  if (nameConflict) {
    errors.push({
      code: "RELATION_NAME_DUPLICATE",
      field: "name",
      message: `A relation named '${relation.name}' already exists`,
    });
  }
  ...
  if (
    ...
    relation.targetEntity === entityName
  ) {
    warnings.push({
      code: "SELF_OWNERSHIP",
      field: "targetEntity",
...
export function validateIndex(
  ...
  for (const col of index.columns) {
    if (!fieldNames.has(col)) {
      errors.push({
        ...
        field: "columns",
        message: `Column '${col}' does not exist on this entity`,
      });
    }
  }
  ...
  if (index.columns.length > 16) {
    errors.push({
      ...
      field: "columns",
      message: "Indexes can have at most 16 columns",
    });
  }
  ...
  if (existingCols.includes(newCols)) {
    warnings.push({
      ...
      field: "columns",
      message: "An index with the same columns already exists",
    });
  }
...
export function validatePolicy(
  ...
  if (policy.fieldPath) {
    const fieldExists = fields.some(
      (f) => f.name === policy.fieldPath || f.columnName === policy.fieldPath,
    );
    if (!fieldExists) {
      errors.push({
