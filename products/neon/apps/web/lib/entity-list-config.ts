...
export function buildEntityListConfig(options: {
  ...
  const treeFlag = featureFlags?.treeView as
    | { parentField?: string; levelField?: string; isGroupField?: string }
    | undefined;
  const treeConfig: TreeConfig | undefined = treeFlag?.parentField
    ? {
        parentField: treeFlag.parentField,
        levelField: treeFlag.levelField,
        isGroupField: treeFlag.isGroupField,
      }
    : undefined;
  ...
  return {
    ...
    cardRenderer: (_item) => null, // Will be overridden by EntityCard in the page
    ...
    // Presets
    presets: [{ id: "default", label: "Default", isDefault: true }],
  };
}
...
function buildAccessor(
  field: FieldMeta,
): (item: EntityRecord) => React.ReactNode {
  ...
  return (item: EntityRecord) => {
    ...
    switch (field.dataType) {
      case "number": {
        ...
        if (
          field.columnName.includes("amount") ||
          field.columnName.includes("price") ||
          field.columnName.includes("rate")
        ) {
...
function buildQuickFilters(fields: FieldMeta[]): QuickFilterDef[] {
  ...
  const statusField = fields.find(
    (f) => f.columnName === "status" || f.columnName === "is_active",
  );
...
function naivePluralize(label: string): string {
  if (label.endsWith("s") || label.endsWith("sh") || label.endsWith("ch"))
    return label;
