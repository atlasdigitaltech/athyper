...
async function resolveTenantIdFromDb(tenantCode: string): Promise<string> {
    ...
    try {
        ...
        if (result.rows.length > 0) {
            ...
            _tenantUuidByCode.set(tenantCode, { uuid, expiresAt: Date.now() + TENANT_CACHE_TTL_MS });
            return uuid;
        }
        console.error(
        ...
        console.warn("[getApiContext] Tenant UUID resolution failed, using code as-is:", err);
        return tenantCode;
    } finally {
        await pool.end();
    }
...
export async function getApiContext(): Promise<{ context: ApiContext; redis: Awaited<ReturnType<typeof getRedisClient>> } | { context: null; redis: Awaited<ReturnType<typeof getRedisClient>> }> {
    ...
    try {
        ...
        const session: SessionData & { selectedTenantId?: string } = JSON.parse(raw);
        ...
        const effectiveTenantCode = (isPlatform && session.selectedTenantId)
...
export async function resolveTenantUuid(
    ...
    if (!uuid) {
        throw new Error(`Tenant not found for code "${tenantCode}". Check DEFAULT_TENANT_ID env var and core.tenant seed data.`);
    }

    tenantUuidCache.set(tenantCode, { uuid, expiresAt: Date.now() + TENANT_CACHE_TTL_MS });
    return uuid;
}
