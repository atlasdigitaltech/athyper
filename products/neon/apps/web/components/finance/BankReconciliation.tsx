...
import { Check, Link2, Link2Off, Play } from "lucide-react";
          ...
          <div className="overflow-x-auto rounded-md border">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {lines.map((line) => {
                  const matchConfig =
                    MATCH_STATUS_CONFIG[line.matchStatus] ??
...
function StatPill({
  ...
  return (
    <Badge variant="outline" className={cn("text-xs tabular-nums", className)}>
      {label}: {value}
    </Badge>
  );
}
...
function MatchDetailPanel({
  ...
  const matchConfig =
    MATCH_STATUS_CONFIG[line.matchStatus] ?? MATCH_STATUS_CONFIG.UNMATCHED;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <DetailRow
          label="Amount"
          value={formatAmount(line.amount, currencyCode)}
          className={
...
function DetailRow({
  ...
  return (
    <div className="flex items-baseline justify-between gap-2">
      ...
      <span
        className={cn("text-sm font-medium truncate text-right", className)}
      >
        {value}
      </span>
    </div>
