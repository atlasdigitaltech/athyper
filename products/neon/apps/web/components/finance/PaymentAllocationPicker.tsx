...
export function PaymentAllocationPicker({
  ...
  const unallocatedInvoices = availableInvoices.filter(
    (inv) => !allocatedInvoiceIds.has(inv.id),
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-4 text-sm">
        ...
        {isFullyAllocated ? (
          ...
          <Badge
            variant="outline"
            className="border-amber-600/30 bg-amber-50 text-amber-700 dark:bg-amber-950/30 dark:text-amber-400"
          >
            Allocation mismatch
          </Badge>
        )}
