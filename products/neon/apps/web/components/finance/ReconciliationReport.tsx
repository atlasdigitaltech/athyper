...
export function ReconciliationReport({
    ...
    const statusConfig = SESSION_STATUS_CONFIG[session.status] ?? SESSION_STATUS_CONFIG.OPEN;
    const totalMatched = session.autoMatched + session.manualMatched;
    const matchedPercent = session.totalLines > 0 ? Math.round((totalMatched / session.totalLines) * 100) : 0;
    const unmatchedPercent = session.totalLines > 0 ? Math.round((session.unmatched / session.totalLines) * 100) : 0;
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div className="space-y-2">
                            ...
                            {statement.bankName && <InfoRow label="Bank" value={statement.bankName} />}
                            <InfoRow label="Period" value={`${formatDate(statement.periodStart)} \u2013 ${formatDate(statement.periodEnd)}`} />
                        </div>
                        <div className="space-y-2">
                            <InfoRow label="Opening Balance" value={formatAmount(statement.openingBalance, statement.currencyCode)} mono />
                            <InfoRow label="Closing Balance" value={formatAmount(statement.closingBalance, statement.currencyCode)} mono />
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center justify-between text-xs text-muted-foreground">
                            <span>Matched: {totalMatched} of {session.totalLines} ({matchedPercent}%)</span>
                            <span>Unmatched: {session.unmatched} ({unmatchedPercent}%)</span>
                        </div>
                        <div className="relative h-4 overflow-hidden rounded-full bg-muted">
                            ...
                            {session.autoMatched > 0 && (
                                <div
                                    ...
                                    style={{ width: `${session.totalLines > 0 ? (session.autoMatched / session.totalLines) * 100 : 0}%` }}
                                />
                            )}
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-xs">
                            <LegendDot color="bg-blue-500 dark:bg-blue-400" label={`Auto: ${session.autoMatched}`} />
                            <LegendDot color="bg-green-500 dark:bg-green-400" label={`Manual: ${session.manualMatched}`} />
                            <LegendDot color="bg-gray-400 dark:bg-gray-500" label={`Excluded: ${session.excluded}`} />
                            <LegendDot color="bg-muted" label={`Unmatched: ${session.unmatched}`} />
                        </div>
                    </div>
            ...
            {/* ── Matched Lines Table ── */}
            <div className="space-y-2">
                <h4 className="text-sm font-medium">Matched Lines ({matchedLines.length})</h4>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b bg-muted/50">
                                ...
                                <th className="px-3 py-2 text-left text-xs font-medium text-muted-foreground">Match Type</th>
                        ...
                        <tbody>
                            ...
                            {matchedLines.map((line) => {
                                const matchConfig = MATCH_STATUS_SHORT[line.matchStatus] ?? MATCH_STATUS_SHORT.AUTO_MATCHED;
                                const isDebit = line.direction === "DEBIT";

                                return (
                                    <tr key={`${line.lineNo}-${line.matchedPaymentId}`} className="border-b last:border-b-0 hover:bg-muted/30">
...
function InfoRow({
    ...
    return (
        <div className="flex items-baseline justify-between gap-2">
            ...
            <span className={cn("text-sm font-medium truncate text-right", mono && "font-mono tabular-nums")}>{value}</span>
        </div>
    );
}
