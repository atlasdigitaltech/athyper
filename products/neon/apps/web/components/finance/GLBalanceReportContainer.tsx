...
export function GLBalanceReportContainer({
  ...
  const mappedTrialBalance = useMemo(
    () => trialBalanceRows.map(mapTrialBalanceRow),
    [trialBalanceRows],
  );
  ...
  const trialBalanceIsBalanced =
    trialBalanceTotals.totalDebits === trialBalanceTotals.totalCredits;
  ...
  return (
    <GLBalanceReport
      ...
      onReversalModeChange={(mode) =>
        setFilters({ ...filters, reversalMode: mode })
      }
