...
export function MoneyCell({ amount, currency }: MoneyCellProps) {
    ...
    return (
        <span className="block text-right font-mono text-sm tabular-nums">
            {formatted}
            {currency ? <span className="ml-1 text-xs text-muted-foreground">{currency}</span> : null}
        </span>
    );
}
...
export function DateCell({ date }: DateCellProps) {
    ...
    const formatted = Number.isNaN(parsed.getTime()) ? date : dateFormatter.format(parsed);
...
export function ApprovalRouteBadge({ route }: ApprovalRouteBadgeProps) {
    ...
    const colors = APPROVAL_ROUTE_COLORS[route] ?? { bg: "bg-gray-100", text: "text-gray-600" };
