...
/** Maps score to a CSS color for the circular ring gradient. */
function scoreGradientColor(score: number): string {
  if (score >= 0.9) return "var(--success, oklch(0.7 0.15 160))";
  if (score >= 0.75) return "var(--info, oklch(0.6 0.15 250))";
  if (score >= 0.5) return "var(--warning, oklch(0.75 0.15 85))";
  if (score >= 0.25) return "var(--warning, oklch(0.7 0.17 55))";
  return "var(--destructive, oklch(0.6 0.2 25))";
}

const APPROVAL_ROUTE_CONFIG: Record<
  ApprovalRoute,
...
function CircularScore({ score }: { score: number }) {
  ...
  return (
    <div
      className={cn(
        styles.circularScoreRing,
        "relative flex h-24 w-24 items-center justify-center rounded-full",
      )}
      style={
        {
          "--score-fill": fillColor,
          "--score-pct": percentage,
...
export function DecisionScorePanel({
  result,
  loading,
}: DecisionScorePanelProps) {
  if (loading) {
    return (
      <Card className="p-6">
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-2">
            <Badge
              variant="outline"
              className={cn("text-xs", routeConfig.className)}
            >
              {routeConfig.label}
            </Badge>
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span>
            Pipeline: <span className="font-mono">{result.pipelineId}</span>
          </span>
