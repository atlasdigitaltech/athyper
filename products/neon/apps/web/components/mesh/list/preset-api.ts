...
function getCsrfHeaders(): Record<string, string> {
  ...
  const bootstrap = (
    window as unknown as { __SESSION_BOOTSTRAP__?: SessionBootstrap }
...
function presetToStateJson(preset: ViewPreset): Record<string, unknown> {
  ...
  if (preset.columnVisibility !== undefined)
    state.columnVisibility = preset.columnVisibility;
  if (preset.columnOrder !== undefined) state.columnOrder = preset.columnOrder;
  if (preset.columnSizing !== undefined)
    state.columnSizing = preset.columnSizing;
  if (preset.filterBarFields !== undefined)
    state.filterBarFields = preset.filterBarFields;
...
export async function fetchPresets(entityKey: string): Promise<ViewPreset[]> {
  const res = await fetch(
    `/api/ui/views?entity_key=${encodeURIComponent(entityKey)}`,
  );
 ...
 */
export async function fetchPresetFull(
  viewId: string,
