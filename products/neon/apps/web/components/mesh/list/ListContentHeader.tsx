...
export function ListContentHeader<T>() {
  ...
  const barQuickFilters =
    state.filterBarFields.length > 0
      ? config.quickFilters.filter((qf) =>
          state.filterBarFields.includes(qf.id),
        )
      : config.quickFilters;
          ...
          <h2 className="min-w-0 truncate text-base font-semibold">
            ...
            {totalFilteredCount > 0 && (
              <span className="ml-1.5 text-sm font-normal text-muted-foreground">
                (
                {infiniteScroll
                  ? `${infiniteScroll.loadedCount} of ${totalFilteredCount}`
                  : totalFilteredCount}
                )
              </span>
            )}
          ...
          <div className="min-h-0 overflow-hidden">
            ...
            {!state.advancedOpen && activeFilterCount > 0 && (
              <div className="pb-1 text-xs text-muted-foreground">
                ...
                active: {activeFilterLabels.join(", ")}
              </div>
            )}
            ...
            {state.advancedOpen && hasFilters && (
              <div className="flex flex-wrap items-end gap-2 border-t py-1.5 sm:gap-3">
                {barQuickFilters.map((qf) => (
                  <div
                    key={qf.id}
                    className="flex min-w-0 flex-1 flex-col gap-1 sm:flex-none"
                  >
