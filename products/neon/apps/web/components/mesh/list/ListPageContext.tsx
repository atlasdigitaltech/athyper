...
function createInitialState(config: ListPageConfig<unknown>): ListPageState {
  ...
  const viewMode: ViewMode = config.defaultViewMode ?? "card-grid";
...
function resolveClientViewMode(
...
export function ListPageProvider<T>({
  ...
  useEffect(() => {
    ...
    const storedVersion = getLocalStorageValue(
      `neon:defaultsVersion:${cfg.basePath}`,
    );
  ...
  useEffect(() => {
    ...
    function resolveAndApply() {
      ...
      const desktopDensity: Density =
        cfg.defaultDensityDesktop ?? mobileDensity;

      // Suppress persistence during viewport-triggered changes
      isViewportReset.current = true;
      dispatch({
        type: "SET_VIEW_MODE",
        payload: isDesktop ? desktopMode : mobileMode,
      });
      dispatch({
        type: "SET_DENSITY",
        payload: isDesktop ? desktopDensity : mobileDensity,
      });
      // Re-enable persistence after React processes the dispatches
      setTimeout(() => {
        isViewportReset.current = false;
      }, 0);
    }
  ...
  const capabilities = useMemo(() => computeCapabilities(config), [config]);
  ...
  const { groupedItems, totalGroupCount } = useMemo((): {
    ...
    if (state.groupBy.length === 0)
      return { groupedItems: [], totalGroupCount: 0 };
    ...
    return {
      groupedItems: all.slice(0, MAX_GROUPS),
  ...
  useEffect(() => {
    ...
    debounceTimerRef.current = setTimeout(() => {
      const filtersStr = buildServerFilterString(
        state.filters,
        config.quickFilters,
      );
      ...
      const effectivePageSize =
        state.groupBy.length > 0 ? 1000 : state.pageSize;
  ...
  return useMemo(
    () => ({
      ...
      expandAll: (ids: string[]) =>
        dispatch({ type: "EXPAND_ALL", payload: ids }),
      ...
      openSettings: (
        defaultTab?: "view" | "filter" | "columns" | "sort" | "group",
      ) => {
        if (defaultTab) {
