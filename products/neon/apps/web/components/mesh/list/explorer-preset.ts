  ...
  return {
    ...
    columnSizing:
      Object.keys(state.columnSizing).length > 0
        ...
        : undefined,
    filterBarFields:
      state.filterBarFields.length > 0 ? [...state.filterBarFields] : undefined,
...
export function isPresetDirty(
  ...
  // Group-by
  if (preset.groupBy && !groupRulesEqual(state.groupBy, preset.groupBy))
    return true;
  ...
  if (preset.columnOrder) {
    if (state.columnOrder.join(",") !== preset.columnOrder.join(","))
      return true;
  }
...
function groupRulesEqual(a: GroupRule[], b: GroupRule[]): boolean {
  ...
  return a.every(
    (r, i) =>
      r.fieldId === b[i].fieldId && (r.dir ?? "asc") === (b[i].dir ?? "asc"),
  );
}
