...
export function EntityTreeView<T>() {
    const { state, config, sortedItems, loading, isPendingQuery } = useListPage<T>();
    ...
    const tree = useMemo(
        () => buildTree(sortedItems, config.getId, treeConfig),
    ...
    const allExpanded = allGroupIds.length > 0 && allGroupIds.every((id) => state.expandedIds.has(id));
...
interface TreeNodeRowProps<T> {
    ...
    density: Density;
    config: { getId: (item: T) => string; getItemHref?: (item: T) => string; columns: { id: string; header: string; accessor: (item: T) => React.ReactNode }[] };
...
function TreeNodeRow<T>({
    ...
    const visibleActions = rowActions?.filter((a) => !a.hidden?.(node.item)) ?? [];
...
const STATUS_COLORS: Record<string, string> = {
    ...
    SUNSET: "bg-orange-100 text-orange-700 dark:bg-orange-950 dark:text-orange-400",
