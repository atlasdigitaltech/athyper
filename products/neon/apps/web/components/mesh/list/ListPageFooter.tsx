...
export function ListPageFooter<T>() {
  ...
  if (hasGroups) {
    const groupItemCount = groupedItems.reduce(
    ...
    return (
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t bg-background px-1 py-1">
        <p className="text-xs text-muted-foreground">
          {groupItemCount} {config.entityLabelPlural} in {totalGroupCount} group
          {totalGroupCount !== 1 ? "s" : ""}
        </p>
        <p className="text-xs text-muted-foreground/70">
          Grouping uses in-memory dataset (max 1,000 records). Filter to narrow
          results.
        </p>
      </div>
    );
