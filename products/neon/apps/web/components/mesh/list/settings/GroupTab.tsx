...
export function GroupTab<T>({
  ...
  function addGroup() {
    ...
    onGroupByChange([
      ...groupBy,
      { fieldId: col.id, dir: "asc", collapsed: false },
    ]);
  }
  ...
  function toggleCollapsed(fieldId: string) {
    onGroupByChange(
      groupBy.map((r) =>
        r.fieldId === fieldId ? { ...r, collapsed: !r.collapsed } : r,
      ),
    );
  }
    ...
    <div className="flex flex-col gap-3 p-1">
      ...
      <div className="space-y-2">
        {groupBy.map((rule, idx) => {
          ...
          return (
            <div key={rule.fieldId} className="space-y-2 rounded-md border p-3">
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  title={
                    (rule.dir ?? "asc") === "asc" ? "Ascending" : "Descending"
                  }
                >
                  {(rule.dir ?? "asc") === "asc" ? (
              ...
              <div className="flex items-center gap-3 pl-6">
                <div className="flex items-center gap-1.5">
                  <Checkbox
                    ...
                    onCheckedChange={() => toggleCollapsed(rule.fieldId)}
                    className="size-3.5"
                  />
                  <Label
      ...
      {groupBy.length > 1 && (
        <p className="text-[10px] text-muted-foreground">
          Multi-level grouping: items are grouped by level 1, then sub-grouped
          by level 2.
        </p>
      )}
    </div>
