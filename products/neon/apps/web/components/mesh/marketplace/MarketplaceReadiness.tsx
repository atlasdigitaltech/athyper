...
import {
  CHECK_STATUS_COLOR,
  ITEM_TYPE_BADGE,
  PUBLISHING_STATUS_BADGE,
  readinessScoreClasses,
...
function ReadinessScore({ score }: { score: number }) {
  ...
  return (
    <div
      className={cn("flex items-center gap-1 px-2 py-1 rounded", classes.bg)}
    >
...
function PublishingStatusBadge({
  status,
}: {
  status: PublishableItem["publishingStatus"];
}) {
  ...
  return (
    <Badge className={PUBLISHING_STATUS_BADGE[status] ?? ""}>
      {labels[status]}
    </Badge>
  );
}
...
function ItemCard({ item }: { item: PublishableItem }) {
  ...
  const criticalIssues = item.checks.filter(
    (c) => c.status === "failed" && c.blocksPublish,
  ).length;
        ...
        {item.marketplaceStats && (
          <div className="flex items-center gap-4 text-xs text-muted-foreground pt-2 border-t">
            <div className="flex items-center gap-1">
              ...
              <span>
                {item.marketplaceStats.downloads.toLocaleString()} downloads
              </span>
            </div>
            <div className="flex items-center gap-1">
              <Package className="h-3 w-3" />
              <span>
                {item.marketplaceStats.activeInstallations.toLocaleString()}{" "}
                active
              </span>
            </div>
            {item.marketplaceStats.rating !== null && (
              <div className="flex items-center gap-1">
                ...
                <span>
                  {item.marketplaceStats.rating.toFixed(1)} (
                  {item.marketplaceStats.reviews})
                </span>
              </div>
            )}
          </div>
...
export function MarketplaceReadiness() {
  ...
  const [filter, setFilter] = useState<
    PublishableItem["publishingStatus"] | "all"
  >("all");
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold mt-1 text-categorical-4">
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold mt-1">
                {data.summary.totalDownloads.toLocaleString()}
              </p>
              {data.summary.averageRating !== null && (
                <div className="flex items-center gap-1 mt-1">
                  <Star className="h-3 w-3 fill-warning text-warning" />
                  <span className="text-xs text-muted-foreground">
                    {data.summary.averageRating.toFixed(1)} avg
                  </span>
                </div>
              )}
            </div>
