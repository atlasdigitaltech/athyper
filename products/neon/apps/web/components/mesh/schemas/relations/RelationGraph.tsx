...
function buildNodesAndEdges(
  ...
  const nodes: Node[] = entities.map((name, i) => ({
    ...
    style: {
      background:
        name === entityName ? "oklch(var(--primary))" : "oklch(var(--muted))",
      color:
        name === entityName
  ...
  const edges: Edge[] = relations.map((rel) => ({
    ...
    style: {
      stroke:
        selectedId === rel.id
          ? "oklch(var(--chart-4))"
          : (EDGE_COLORS[rel.relationKind] ?? "oklch(var(--muted-foreground))"),
    ...
    markerEnd:
      rel.relationKind !== "m2m" ? { type: "arrowclosed" as const } : undefined,
  }));
