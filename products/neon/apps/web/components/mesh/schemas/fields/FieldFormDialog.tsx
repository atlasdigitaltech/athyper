...
function buildFieldSchema(
  ...
  return z.object({
    name: z
      ...
      .regex(
        /^[a-z][a-z0-9_]*$/,
        "Must be snake_case (lowercase, numbers, underscores)",
      )
      .refine(
...
export function FieldFormDialog({
  ...
  // Type-specific state
  const existingLookup = field?.validation as
  ...
  const [lookupConfig, setLookupConfig] = useState<LookupConfig>({
    ...
    onDelete:
      (existingLookup?.onDelete as LookupConfig["onDelete"]) ?? "restrict",
  });
  ...
  const handleSubmit = (values: FieldFormValues) => {
    ...
    if (dt === "reference" && lookupConfig.targetEntity) {
      ...
      finalValues.validationJson = JSON.stringify({
  ...
  const typeChangeWarning =
    isEditing && field?.dataType && watchDataType !== field.dataType
            ...
            <FormField
              ...
              render={({ field: f }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    JSON object with validation constraints (minLength,
                    maxLength, pattern, min, max, etc.).
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
