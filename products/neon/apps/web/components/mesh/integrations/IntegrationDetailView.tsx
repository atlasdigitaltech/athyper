    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1 max-w-sm">
          Create data flows to synchronize entity records with this integration
          endpoint.
        </p>
      ...
      <Table>
        ...
        <TableBody>
          {flows.map((flow) => (
            <TableRow
              key={flow.id}
              className={cn(!flow.isActive && "opacity-50")}
            >
              <TableCell>
                <div>
              ...
              <TableCell>
                <TriggerBadge
                  mode={flow.triggerMode}
                  cron={flow.scheduleCron}
                />
              </TableCell>
              <TableCell>
  ...
  if (webhooks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1 max-w-sm">
          Subscribe to events and deliver notifications to external systems via
          webhooks.
        </p>
        <Button size="sm" className="mt-4">
          <Plus className="mr-1.5 size-3.5" />
      ...
      <Table>
        ...
        <TableBody>
          {webhooks.map((webhook) => (
            <TableRow
              key={webhook.id}
              className={cn(!webhook.isActive && "opacity-50")}
            >
              <TableCell>
  ...
  if (deliveries.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1 max-w-sm">
          Delivery logs will appear here once data flows or webhooks have
          executed.
        </p>
      </div>
    );
      ...
      {integration.retryPolicy && (
        <Card>
          ...
          <CardContent className="space-y-3">
            <ConfigRow
              label="Max Attempts"
              value={String(integration.retryPolicy.maxAttempts)}
            />
            <ConfigRow
              label="Backoff"
              value={integration.retryPolicy.backoffStrategy}
            />
            <ConfigRow
              label="Initial Delay"
              value={`${integration.retryPolicy.initialDelayMs}ms`}
            />
          </CardContent>
        </Card>
      )}
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <ConfigRow
              label="Status"
              value={
                integration.circuitBreaker.enabled ? "Enabled" : "Disabled"
              }
            />
            <ConfigRow
              label="Failure Threshold"
              value={String(integration.circuitBreaker.failureThreshold)}
            />
            <ConfigRow
              label="Success Threshold"
              value={String(integration.circuitBreaker.successThreshold)}
            />
            <ConfigRow
              label="Timeout"
              value={`${integration.circuitBreaker.timeoutMs}ms`}
            />
          </CardContent>
        </Card>
      )}
...
): UseIntegrationDetailResult {
  const [integration, setIntegration] = useState<IntegrationDetail | null>(
    null,
  );
  ...
  const fetchIntegration = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/mesh/integration-studio/${encodeURIComponent(integrationId)}`,
        {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          body.error?.message ?? `Failed to load integration (${res.status})`,
        );
      }
...
export function IntegrationDetailView({
  ...
  const { integration, loading, error, refresh } =
    useIntegrationDetail(integrationId);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          {integration.isActive ? (
            <Badge
              variant="outline"
              className={cn("text-xs shrink-0", ACTIVE_BADGE)}
            >
              Active
            </Badge>
          ) : (
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
              ...
              <span>
                {
                  integration.flows.filter((f) => f.flowDirection === "inbound")
                    .length
                }{" "}
                inbound
              </span>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {integration.webhooks.filter((w) => w.isActive).length} active
              subscriptions
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {integration.recentDeliveries.length}
            </div>
            <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1">
                <CheckCircle2 className="size-3 text-success" />
                {
                  integration.recentDeliveries.filter(
                    (d) => d.status === "success",
                  ).length
                }
              </span>
      ...
      <nav className="flex items-center gap-1 border-b" role="tablist">
        {TABS.map((tab) => {
          ...
          if (tab.id === "deliveries")
            count = integration.recentDeliveries.length;
      ...
      <div className="min-h-[200px]">
        ...
        {activeTab === "webhooks" && (
          <WebhooksTab webhooks={integration.webhooks} />
        )}
        {activeTab === "deliveries" && (
          <DeliveryLogTab deliveries={integration.recentDeliveries} />
        )}
