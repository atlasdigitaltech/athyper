      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="space-y-1 rounded-md border p-3">
          ...
          {data.checks.redis.latencyMs != null && (
            <DebugRow
              label={t("diag.health.latency")}
              value={`${data.checks.redis.latencyMs}ms`}
            />
          )}
          {data.checks.redis.sessionTtl != null && (
            <DebugRow
              label={t("diag.health.session_ttl")}
              value={`${data.checks.redis.sessionTtl}s`}
            />
          )}
        ...
        <div className="space-y-1 rounded-md border p-3">
          ...
          {data.checks.keycloak.latencyMs != null && (
            <DebugRow
              label={t("diag.health.latency")}
              value={`${data.checks.keycloak.latencyMs}ms`}
            />
          )}
          {data.checks.keycloak.keyCount != null && (
            <DebugRow
              label={t("diag.health.jwks_keys")}
              value={String(data.checks.keycloak.keyCount)}
            />
          )}
        ...
        <div className="space-y-1 rounded-md border p-3">
          ...
          {data.checks.apiMesh.latencyMs != null && (
            <DebugRow
              label={t("diag.health.latency")}
              value={`${data.checks.apiMesh.latencyMs}ms`}
            />
          )}
