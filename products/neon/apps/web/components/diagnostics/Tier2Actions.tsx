...
export function Tier2Actions({ onActionComplete }: Tier2ActionsProps) {
  ...
  const runHealthCheck = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setHealthResult({
          ok: true,
          message: `${t("diag.health.overall_label")}: ${data.overall}`,
        });
      } else {
        setHealthResult({
          ok: false,
          message: (data.error as string) ?? t("diag.result.failed"),
        });
      }
    } catch (err) {
      setHealthResult({
        ok: false,
        message: err instanceof Error ? err.message : t("diag.result.failed"),
      });
    } finally {
  ...
  const [downloadResult, setDownloadResult] = useState<ActionResult | null>(
    null,
  );
    ...
    try {
      const bootstrap =
        typeof window !== "undefined"
          ? ((window as any).__SESSION_BOOTSTRAP__ as
              | SessionBootstrap
              | undefined)
          : undefined;
      ...
      if (!res.ok) {
        ...
        setDownloadResult({
          ok: false,
          message: (errData.error as string) ?? t("diag.result.failed"),
        });
        return;
      }
      ...
      setDownloadResult({
        ok: true,
        message: t("diag.tier2.report_downloaded"),
      });
    } catch (err) {
      setDownloadResult({
        ok: false,
  ...
  return (
    <div className="space-y-3">
      ...
      {healthData && <HealthPanel data={healthData as any} />}
    </div>
  );
}
