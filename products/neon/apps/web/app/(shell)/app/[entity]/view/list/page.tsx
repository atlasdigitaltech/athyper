...
import { slugToEntityName, entityNameToDisplayName } from "@/lib/entity-meta-utils";
...
import type { InfiniteScrollState, ScrollMode } from "@/components/mesh/list/types";
...
export default function ListViewPage() {
    ...
    // Fetch metadata
    const { fields, entityMeta, loading: fieldsLoading } = useEntityFields(entitySlug);
    const { capabilities, loading: capsLoading } = useEntityCapabilities(entityName);
    const { primaryAction, rowActions, bulkActions, loading: actionsLoading } =
        ...
        (query: UseEntityDataOptions) => {
            if (scrollMode === "infinite") {
                ...
                if (prevResetKey === "") {
                    ...
                    setServerQuery((prev) => ({ ...query, page: 1, pageSize: prev.pageSize }));
                    return;
                }
    ...
    const hasMoreItems =
        ...
        (paginationMeta.total > 0 && accumulatedItems.length < paginationMeta.total);
    ...
    const itemsForProvider =
        scrollMode === "infinite" ? accumulatedItems : data;
    ...
    }, [scrollMode, paginationMeta, accumulatedItems.length, serverQuery.pageSize]);
    ...
    const infiniteScrollState = useMemo<InfiniteScrollState | undefined>(() => {
        ...
        return {
            scrollMode,
            loadMore: hasMoreItems && accumulatedItems.length < INFINITE_ITEM_CAP
                ? manualLoadMore
                : null,
        ...
        scrollMode, hasMoreItems, accumulatedItems.length,
        manualLoadMore, loadingMore, loadMoreError, paginationMeta.total,
        scrollToTop, serverQuery.pageSize, onPageSizeChange,
    ]);
    ...
    const isServerFiltered = paginationMeta.serverFiltered === true || scrollMode === "infinite";
    const isServerSorted = paginationMeta.serverSorted === true || scrollMode === "infinite";
    ...
    const config = useMemo(() => {
        ...
        cfg.cardRenderer = (item: Record<string, unknown>) => (
            <EntityCard
                item={item}
                basePath={`/app/${entitySlug}`}
                fields={fields}
            />
        );

        return cfg;
    }, [fields, entityMeta, entityName, entitySlug, capabilities, primaryAction, rowActions, bulkActions]);
    ...
    if (metaLoading && !config) {
        return (
            <div className="flex items-center justify-center py-16">
                ...
                <span className="ml-2 text-sm text-muted-foreground">Loading entity metadata...</span>
            </div>
        );
    }
