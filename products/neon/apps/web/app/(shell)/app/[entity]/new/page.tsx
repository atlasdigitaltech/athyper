...
import { entityNameToDisplayName, slugToEntityName } from "@/lib/entity-meta-utils";
...
export default function CreateRecordPage({ params }: CreateRecordPageProps) {
    ...
    const { fields, loading: fieldsLoading, error: fieldsError } = useEntityFields(entity);
    ...
    useEffect(() => {
        ...
        async function fetchSections() {
            try {
                ...
                const res = await fetch(`/api/entity-page/${encodeURIComponent(entity)}`, {
                    headers,
    ...
    const handleSuccess = useCallback(
        (savedRecord: Record<string, unknown>) => {
            ...
            if (recordId) {
                router.push(`/app/${encodeURIComponent(entity)}/${encodeURIComponent(recordId)}`);
            } else {
        ...
        return (
            <Card className="p-6">
                ...
                <Button
                    variant="ghost"
                    className="mt-4"
                    onClick={() => router.push(`/app/${encodeURIComponent(entity)}/view/list`)}
                >
