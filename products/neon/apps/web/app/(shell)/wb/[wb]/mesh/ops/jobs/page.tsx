...
function statusIcon(status: string) {
  switch (status) {
    ...
    case "delayed":
      return <Timer className="size-3.5 text-yellow-500" />;
    default:
      return <Clock className="size-3.5 text-muted-foreground" />;
  }
}
...
export default function JobsPage() {
  ...
  const {
    ...
    error: jobsError,
    refresh: refreshJobs,
  } = useJobList(selectedQueue, statusFilter);
  ...
  const handleTogglePause = useCallback(
    async (queueName: string, isPaused: boolean) => {
      try {
        await fetch(
          `/api/admin/mesh/jobs/queues/${encodeURIComponent(queueName)}`,
          {
            method: "POST",
  ...
  const handleTriggerJob = useCallback(async () => {
    ...
    try {
      let data: unknown = {};
      try {
        data = JSON.parse(triggerPayload);
      } catch {
        /* use empty object */
      }

      await fetch(
        `/api/admin/mesh/jobs/queues/${encodeURIComponent(selectedQueue)}`,
        {
          method: "POST",
          headers: { ...buildHeaders(), "Content-Type": "application/json" },
          credentials: "same-origin",
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setTriggerPriority(parseInt(e.target.value, 10) || 0)
                }
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!triggerName.trim() || triggerLoading}
            >
              {triggerLoading ? (
