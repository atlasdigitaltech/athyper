  ...
  const fetchIndexes = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          body.error?.message ?? `Failed to load indexes (${res.status})`,
        );
      }
          ...
          {indexes.map((idx) => (
            <TableRow key={idx.id}>
              ...
              <TableCell>
                <Badge
                  variant="outline"
                  className={`text-xs font-mono ${METHOD_COLORS[idx.method] ?? ""}`}
                >
                  {idx.method}
                </Badge>
              </TableCell>
              <TableCell className="text-center">
                {idx.isUnique ? (
                  <Badge
                    variant="outline"
                    className="text-xs text-purple-600 border-purple-300 dark:text-purple-400 dark:border-purple-700"
                  >
                    unique
                  </Badge>
                ) : (
