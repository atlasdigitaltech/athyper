  ...
  const fetchPolicies = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          body.error?.message ?? `Failed to load policies (${res.status})`,
        );
      }
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        ...
        {entityPolicies.length > 0 ? (
          ...
          <p className="text-sm text-muted-foreground">
            No entity-level policies configured.
          </p>
        )}
      </section>
      ...
      <section className="space-y-3">
        ...
        {fieldSecurityPolicies.length > 0 ? (
              ...
              {fieldSecurityPolicies.map((fsp) => (
                <TableRow key={fsp.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant="outline"
                      className={`text-xs font-mono ${MASK_COLORS[fsp.maskStrategy] ?? ""}`}
                    >
                      {fsp.maskStrategy}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="text-center">
                    <div
                      className={`inline-block size-2 rounded-full ${fsp.isActive ? "bg-green-500" : "bg-muted-foreground/30"}`}
                    />
                  </TableCell>
                </TableRow>
              ))}
          ...
          <p className="text-sm text-muted-foreground">
            No field-level security rules configured.
          </p>
        )}
      </section>
    </div>
