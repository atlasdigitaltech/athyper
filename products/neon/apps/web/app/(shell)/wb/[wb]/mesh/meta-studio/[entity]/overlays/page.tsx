  ...
  const fetchOverlays = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          body.error?.message ?? `Failed to load overlays (${res.status})`,
        );
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        Drag to reorder overlay priority. Higher priority overlays are applied
        last and can override earlier ones.
      </p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0 space-y-2">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Badge
                  variant="outline"
                  className={`text-xs ${CONFLICT_COLORS[overlay.conflictMode] ?? ""}`}
                >
                  {overlay.conflictMode}
                </Badge>
                <StatusDot
                  status={overlay.isActive ? "published" : "archived"}
                />
              </div>
              ...
              {overlay.changes.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {overlay.changes.map((change) => (
                    ...
                    >
                      {CHANGE_KIND_LABELS[change.kind] ?? change.kind}:{" "}
                      <code className="ml-1 font-mono">{change.path}</code>
                    </Badge>
                  ))}
                </div>
              )}
