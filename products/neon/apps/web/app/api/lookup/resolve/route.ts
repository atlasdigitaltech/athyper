...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (!body.refs || !Array.isArray(body.refs)) {
            return errorResponse("INVALID_BODY", "Expected { refs: [{ schema, table, ids }] }", 400);
        }
        ...
        async function shouldCacheLabels(schema: string, table: string): Promise<boolean> {
            const key = `${schema}.${table}`;
            if (allowCache.has(key)) return allowCache.get(key)!;
            const targetMeta = await resolveEntityMetaByTable(db!, schema, table, tenantUuid);
            const allowed = (targetMeta?.featureFlags as any)?.ui?.cacheRefLabels === true;
        ...
        for (const ref of body.refs) {
            if (!ref.schema || !ref.table || !Array.isArray(ref.ids) || ref.ids.length === 0) {
                continue;
            }
            ...
            const policy = await resolveDisplayPolicy(db, ref.schema, ref.table, undefined, metrics);
            ...
            if (cacheLabels) {
                try {
                    const redisKeys = ids.map((id) => refKey(ns, tenantUuid, ref.schema, ref.table, id));
            ...
            if (uncachedIds.length > 0) {
                const hasTenant = await tableHasColumn(db, ref.schema, ref.table, "tenant_id");
