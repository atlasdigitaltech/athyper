...
export async function POST(req: Request) {
    ...
    if (!hasPlatformPermission(auth.platformRoles, "tenant:switch")) {
        return NextResponse.json(
            { error: "FORBIDDEN", message: "Insufficient platform role for tenant:switch" },
            { status: 403 },
        );
    }
    ...
    if (!targetTenantId || typeof targetTenantId !== "string" || targetTenantId.trim().length === 0) {
        return NextResponse.json(
    ...
    try {
        ...
        if (validationRes.ok) {
            const data = (await validationRes.json()) as { tenants?: Array<{ id?: string }> };
            const exists = data.tenants?.some((t) => t.id === targetTenantId);
            if (!exists) {
                return NextResponse.json(
                    { error: "TENANT_NOT_FOUND", message: `Tenant ${targetTenantId} not found` },
                    { status: 404 },
                );
            }
        }
    ...
    try {
        ...
        await redis.set(`sess:platform:${auth.sid}`, JSON.stringify(session), { EX: effectiveTtl });
