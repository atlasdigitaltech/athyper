...
export async function GET(req: Request) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      new URL(`/?error=${encodeURIComponent(errorDescription)}`, url.origin),
    );
  }
  ...
  try {
    ...
    if (
      ...
      clientRoles.push(
        ...resourceAccess[clientId].roles.filter(
    ...
    const resolvedPersona: string | null =
    ...
    let tenantDisplayName: string = isPlatformLogin
      ? "Platform"
      : tokenTenantId;
    if (!isPlatformLogin) {
      try {
    ...
    const session = {
      ...
      persona: isPlatformLogin
        ? "platform_admin"
        : (resolvedPersona ??
          (roles.includes("admin") ? "tenant_admin" : "requester")),
    ...
    try {
      ...
      if (mfaRes.ok) {
        const mfaData = (await mfaRes.json()) as {
          data?: { isEnabled?: boolean; requiresSetup?: boolean };
        };
