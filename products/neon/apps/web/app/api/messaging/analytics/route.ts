 ...
 */
function generateDailyMessages(
  days: number,
): Array<{ date: string; sent: number; read: number }> {
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!VALID_RANGES.has(rangeParam)) {
      console.warn(
        `[GET /api/messaging/analytics] Invalid range: ${rangeParam}`,
      );
      return NextResponse.json(
        {
          error: {
            message: `Invalid range: ${rangeParam}. Must be 7d, 30d, or 90d.`,
          },
        },
        { status: 400 },
      );
    ...
    const analytics = {
      ...
      readRate:
        totalMessages > 0
          ? Math.round((totalRead / totalMessages) * 100) / 100
          : 0,
      messagesByDay,
