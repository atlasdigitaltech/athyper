...
import {
  CACHE_PATTERNS,
  scanAndDelete,
} from "@/lib/diagnostics/cache-patterns";
  ...
  try {
    ...
    try {
      if (!session.refreshToken) {
        ...
        // Rotate sid
        const rotatedSid = createHash("sha256")
        ...
        if (session.userId) {
          await redis.sRem(
            `user_sessions:${tenantId}:${session.userId}`,
            currentSid,
          );
          await redis.sAdd(
            `user_sessions:${tenantId}:${session.userId}`,
        ...
        steps.refreshToken = {
          ok: true,
          message: "Token refreshed and session rotated",
        };
      }
    } catch (err) {
    ...
    try {
      ...
      steps.clearAppCache = {
        ok: true,
        message: `Cleared ${keysDeleted} keys`,
        keysDeleted,
      };
    } catch (err) {
    ...
    try {
      if (!session.accessToken) {
        ...
        await redis.set(
          `sess:${tenantId}:${currentSid}`,
          JSON.stringify(session),
          { EX: 28800 },
        );
        steps.syncProfile = {
          ok: true,
          message: `Updated: ${fieldsUpdated.length > 0 ? fieldsUpdated.join(", ") : "no changes"}`,
          fieldsUpdated,
        };
      }
    } catch (err) {
