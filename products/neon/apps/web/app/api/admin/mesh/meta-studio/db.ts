...
export async function listEntitiesDirect(): Promise<{
    ...
    const versions = entityIds.length > 0
        ? await db.selectFrom("meta.entity_version")
            .selectAll()
    ...
    const data = entities.map((e) => {
        ...
        return {
            ...
            relationCount: versionId ? (relationCountByVersion.get(versionId) ?? 0) : 0,
    ...
    return {
        success: true,
        data,
        meta: { page: 1, pageSize: total, total, totalPages: 1, hasNext: false, hasPrev: false },
    };
}
...
export async function getEntityDirect(entityName: string): Promise<{
    ...
    const entity = await db.selectFrom("meta.entity")
        .selectAll()
    ...
    // Latest version
    const latestVersion = await db.selectFrom("meta.entity_version")
        .selectAll()
...
export async function createEntityDirect(data: {
    ...
    const entity = await db.insertInto("meta.entity")
        .values({
 ...
 */
export async function updateEntityDirect(entityName: string, updates: Record<string, unknown>): Promise<{
    success: boolean;
    data: unknown | null;
}> {
    const db = await getDb();
    ...
    if (updates.tableSchema !== undefined) dbUpdates.table_schema = updates.tableSchema;
    ...
    if (updates.governanceLevel !== undefined) dbUpdates.governance_level = updates.governanceLevel;
    if (updates.engineTag !== undefined) dbUpdates.engine_tag = updates.engineTag;

    const entity = await db.updateTable("meta.entity")
 ...
 */
export async function deleteEntityDirect(entityName: string): Promise<{ success: boolean }> {
    const db = await getDb();
...
async function resolveLatestVersionId(entityName: string): Promise<{ entityId: string; versionId: string } | null> {
    const db = await getDb();
    const entity = await db.selectFrom("meta.entity").select(["id"]).where("name", "=", entityName).executeTakeFirst();
    if (!entity) return null;
    const version = await db.selectFrom("meta.entity_version").select(["id"]).where("entity_id", "=", entity.id).orderBy("version_no", "desc").limit(1).executeTakeFirst();
...
export async function listFieldsDirect(entityName: string) {
    ...
    const rows = await db.selectFrom("meta.field").selectAll().where("entity_version_id", "=", resolved.versionId).orderBy("sort_order", "asc").execute();
...
export async function listRelationsDirect(entityName: string) {
    ...
    const resolved = await resolveLatestVersionId(entityName);
    if (!resolved) return { success: false, data: [] };
    const rows = await db.selectFrom("meta.relation").selectAll().where("entity_version_id", "=", resolved.versionId).execute();
...
export async function listIndexesDirect(entityName: string) {
    ...
    const resolved = await resolveLatestVersionId(entityName);
    if (!resolved) return { success: false, data: [] };
    const rows = await db.selectFrom("meta.index_def").selectAll().where("entity_version_id", "=", resolved.versionId).execute();
    return {
        success: true,
        data: rows.map((r: any) => ({
            id: r.id, name: r.name, isUnique: r.is_unique ?? false,
            method: r.method ?? "btree", columns: r.columns ?? [],
            whereClause: r.where_clause ?? null, createdAt: r.created_at,
        })),
    };
}
...
export async function listVersionsDirect(entityName: string) {
    ...
    const rows = await db.selectFrom("meta.entity_version").selectAll().where("entity_id", "=", entity.id).orderBy("version_no", "desc").execute();
    return {
        success: true,
        data: rows.map((r: any) => ({
            id: r.id, versionNo: r.version_no ?? 1, status: r.status ?? "draft",
            label: r.label ?? null, publishedAt: r.published_at ?? null,
            publishedBy: r.published_by ?? null, createdAt: r.created_at,
        })),
    };
}
...
export async function getPoliciesDirect(entityName: string) {
    const db = await getDb();
    const entity = await db.selectFrom("meta.entity").select(["id"]).where("name", "=", entityName).executeTakeFirst();
    if (!entity) return { success: false, data: { entityPolicy: null, fieldSecurityPolicies: [] } };
    const entityPolicy = await db.selectFrom("meta.entity_policy").selectAll().where("entity_id", "=", entity.id).executeTakeFirst();
    const fieldSecurityPolicies = await db.selectFrom("meta.field_security_policy").selectAll().where("entity_id", "=", entity.id).where("is_active", "=", true).orderBy("priority", "asc").execute();
    return {
        success: true,
        data: {
            entityPolicy: entityPolicy ? {
                id: entityPolicy.id, accessMode: (entityPolicy as any).access_mode ?? "default_deny",
            ...
            fieldSecurityPolicies: fieldSecurityPolicies.map((r: any) => ({
                id: r.id, fieldPath: r.field_path, policyType: r.policy_type,
                roleList: r.role_list ?? null, abacCondition: r.abac_condition ?? null,
                maskStrategy: r.mask_strategy ?? "null", maskConfig: r.mask_config ?? null,
                scope: r.scope ?? "entity", priority: r.priority ?? 100, isActive: r.is_active ?? true,
            })),
        },
    };
}
...
export async function getCompiledDirect(entityName: string) {
    ...
    const compiled = await db.selectFrom("meta.entity_compiled").selectAll().where("entity_version_id", "=", resolved.versionId).orderBy("generated_at", "desc").limit(1).executeTakeFirst();
    if (!compiled) return { success: true, data: null };
    return {
        success: true,
        data: {
            id: compiled.id, entityVersionId: (compiled as any).entity_version_id,
            compiledJson: (compiled as any).compiled_json, compiledHash: (compiled as any).compiled_hash,
...
export async function getValidationDirect(entityName: string) {
    ...
    const fieldsWithValidation = await db.selectFrom("meta.field").select(["name", "validation"]).where("entity_version_id", "=", resolved.versionId).where("validation", "is not", null).execute();
    const rules = fieldsWithValidation.filter((f) => f.validation).map((f) => ({ fieldName: f.name, rules: f.validation }));
