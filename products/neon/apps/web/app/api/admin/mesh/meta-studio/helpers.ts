...
export async function requireAdminSession(): Promise<
  AdminAuthContext | AdminAuthError
> {
  ...
  if (!runtimeApiUrl && !process.env.DATABASE_URL) {
    return {
      ok: false,
      response: NextResponse.json(
        {
          ...
          error: {
            code: "NOT_CONFIGURED",
            message: "Neither Runtime API nor DATABASE_URL configured",
          },
        },
        { status: 503 },
      ),
...
export function validateBody<T>(
  body: unknown,
  schema: ZodSchema<T>,
): ValidateBodyOk<T> | ValidateBodyError {
        ...
        response: NextResponse.json(
          {
            ...
            error: {
              ...
              message: `Request body exceeds ${MAX_BODY_SIZE / 1024}KB limit`,
            },
          },
          { status: 413 },
        ),
...
export async function proxyGet(
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: "FORBIDDEN_PATH",
          message: "Requested upstream path is not allowed",
        },
      },
      { status: 400 },
    );
  ...
  try {
    ...
    // Forward upstream response as-is (it already has { success, data, meta } shape)
...
export async function proxyMutate(
  ...
  try {
    ...
    const statusForResponse =
      method === "POST" && res.status === 201 ? 201 : res.status;
    ...
    // Forward upstream response as-is (it already has { success, data } shape)
