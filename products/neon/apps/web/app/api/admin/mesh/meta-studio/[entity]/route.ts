...
import { hasDirectDb, getEntityDirect, updateEntityDirect, deleteEntityDirect } from "../db";
        ...
        try {
            ...
            if (!result.success) {
                return NextResponse.json(
                    { success: false, error: { code: "ENTITY_NOT_FOUND", message: `Entity '${entity}' not found` } },
                    { status: 404 },
                );
            }
...
export async function PUT(request: NextRequest, context: RouteContext) {
    ...
    if (hasDirectDb()) {
        try {
            const result = await updateEntityDirect(entity, validated.data as Record<string, unknown>);
            if (!result.success) {
                return NextResponse.json(
                    { success: false, error: { code: "ENTITY_NOT_FOUND", message: `Entity '${entity}' not found` } },
                    { status: 404 },
