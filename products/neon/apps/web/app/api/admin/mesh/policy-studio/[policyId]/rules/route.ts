...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    const res = await fetch(
      `${auth.runtimeApiUrl}/api/policies/${encodeURIComponent(policyId)}/rules`,
      {
        method: "POST",
        headers: {
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            message: `Failed to create rule (${res.status})`,
          },
        },
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  ...
  if (!body.ruleId) {
    return NextResponse.json(
      {
        success: false,
        error: { code: "MISSING_RULE_ID", message: "ruleId is required" },
      },
      { status: 400 },
    );
  }
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            message: `Failed to delete rule (${res.status})`,
          },
        },
        { status: res.status >= 500 ? 502 : res.status },
      );
