...
export async function POST() {
  ...
  try {
    ...
    const groups = Array.isArray(userinfo.groups)
      ...
      : [];
    const persona = roles.includes("tenant_admin")
      ? "tenant_admin"
      : "requester";
    ...
    await redis.set(`sess:${tenantId}:${newSid}`, JSON.stringify(newSession), {
      EX: 28800,
    });
    ...
    if (newSession.userId) {
      ...
      await redis.sAdd(
        `user_sessions:${tenantId}:${newSession.userId}`,
        newSid,
      );
    }
