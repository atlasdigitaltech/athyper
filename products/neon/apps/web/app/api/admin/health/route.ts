...
export async function GET() {
  ...
  try {
    ...
    const [redisResult, keycloakResult, apiMeshResult] =
    ...
    const keycloakHealth =
      keycloakResult.status === "fulfilled"
        ? keycloakResult.value
        : {
            status: "down" as const,
            error: String((keycloakResult as PromiseRejectedResult).reason),
          };

    const apiMeshHealth =
      apiMeshResult.status === "fulfilled"
        ? apiMeshResult.value
        : {
            status: "down" as const,
            error: String((apiMeshResult as PromiseRejectedResult).reason),
          };

    // Compute overall status
    const statuses = [redisHealth.status, keycloakHealth.status].filter(
      (s) => s !== "not_configured",
    );
