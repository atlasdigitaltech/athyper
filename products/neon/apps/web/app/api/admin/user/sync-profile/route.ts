...
export async function POST() {
  ...
  try {
    ...
    const newUsername =
      (userinfo.preferred_username as string) ?? session.username;

    const realmAccess = userinfo.realm_access as
      | { roles?: string[] }
      | undefined;
    const newRoles = Array.isArray(realmAccess?.roles)
      ? realmAccess.roles
      : session.roles;

    const resourceAccess = userinfo.resource_access as
    ...
    const newGroups = Array.isArray(userinfo.groups)
      ? (userinfo.groups as string[])
      : session.groups;
    ...
    if (JSON.stringify(newRoles) !== JSON.stringify(session.roles))
      fieldsUpdated.push("roles");
    if (JSON.stringify(newClientRoles) !== JSON.stringify(session.clientRoles))
      fieldsUpdated.push("clientRoles");
    if (JSON.stringify(newGroups) !== JSON.stringify(session.groups))
      fieldsUpdated.push("groups");

    // Derive persona from roles
    const newPersona = newRoles.includes("tenant_admin")
      ? "tenant_admin"
      : "requester";
    ...
    await redis.set(`sess:${tenantId}:${sid}`, JSON.stringify(patchedSession), {
      EX: 28800,
    });
