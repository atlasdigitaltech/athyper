...
export async function GET() {
  ...
  try {
    ...
    const worstState =
      sessionState === "idle_expired" || tokenState === "expired"
        ? "reauth_required"
    ...
    const report = {
      ...
      session: redact({
        ...
        issuedAt: session.createdAt
          ? new Date(session.createdAt * 1000).toISOString()
          : null,
        expiresAt: session.accessExpiresAt
          ? new Date(session.accessExpiresAt * 1000).toISOString()
          : null,
        lastSeenAt: lastSeenAt
          ? new Date(lastSeenAt * 1000).toISOString()
          : null,
