...
# Export absolute paths for MESH_CONFIG / MESH_DATA.
# Shell env vars take precedence over --env-file in Docker Compose, so these
# override the relative ../../config values in .env regardless of CWD.
export MESH_CONFIG="$MESH_DIR/config"
export MESH_DATA="$MESH_DIR/data"
...
add_file "$COMPOSE_DIR/mail/mesh-mailhog.yml"
