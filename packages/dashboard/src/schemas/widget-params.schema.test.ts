...
describe("headingParamsSchema", () => {
  it("accepts valid heading params", () => {
    expect(
      headingParamsSchema.safeParse({
        text_key: "dashboard.title",
        level: "h1",
      }).success,
    ).toBe(true);
  });
  ...
  it("accepts all valid levels", () => {
    for (const level of ["h1", "h2", "h3", "h4"]) {
      expect(
        headingParamsSchema.safeParse({ text_key: "k", level }).success,
      ).toBe(true);
    }
  });
  ...
  it("rejects invalid level", () => {
    expect(
      headingParamsSchema.safeParse({ text_key: "k", level: "h5" }).success,
    ).toBe(false);
  });

  it("rejects empty text_key", () => {
    expect(
      headingParamsSchema.safeParse({ text_key: "", level: "h1" }).success,
    ).toBe(false);
  });
...
describe("shortcutParamsSchema", () => {
  ...
  it("rejects empty label_key", () => {
    expect(
      shortcutParamsSchema.safeParse({ label_key: "", href: "/x" }).success,
    ).toBe(false);
  });

  it("rejects empty href", () => {
    expect(
      shortcutParamsSchema.safeParse({ label_key: "k", href: "" }).success,
    ).toBe(false);
  });
...
describe("kpiParamsSchema", () => {
  ...
  it("accepts all format types", () => {
    for (const format of ["number", "currency", "percent"]) {
      expect(kpiParamsSchema.safeParse({ ...valid, format }).success).toBe(
        true,
      );
    }
  });
  ...
  it("rejects invalid format", () => {
    expect(
      kpiParamsSchema.safeParse({ ...valid, format: "text" }).success,
    ).toBe(false);
  });

  it("accepts optional trend_query_key", () => {
    expect(
      kpiParamsSchema.safeParse({
        ...valid,
        trend_query_key: "crm.orders_prev",
      }).success,
    ).toBe(true);
  });
  ...
  it("rejects missing query_key", () => {
    expect(
      kpiParamsSchema.safeParse({ label_key: "k", format: "number" }).success,
    ).toBe(false);
  });

  it("rejects missing label_key", () => {
    expect(
      kpiParamsSchema.safeParse({ query_key: "q", format: "number" }).success,
    ).toBe(false);
  });
});
...
describe("listParamsSchema", () => {
  ...
  it("accepts optional link_template", () => {
    expect(
      listParamsSchema.safeParse({ ...valid, link_template: "/orders/{id}" })
        .success,
    ).toBe(true);
  });
  ...
  it("rejects page_size below 1", () => {
    expect(listParamsSchema.safeParse({ ...valid, page_size: 0 }).success).toBe(
      false,
    );
  });
  ...
  it("rejects page_size above 100", () => {
    expect(
      listParamsSchema.safeParse({ ...valid, page_size: 101 }).success,
    ).toBe(false);
  });

  it("accepts page_size at boundaries (1 and 100)", () => {
    expect(listParamsSchema.safeParse({ ...valid, page_size: 1 }).success).toBe(
      true,
    );
    expect(
      listParamsSchema.safeParse({ ...valid, page_size: 100 }).success,
    ).toBe(true);
  });

  it("rejects empty columns array", () => {
    expect(listParamsSchema.safeParse({ ...valid, columns: [] }).success).toBe(
      false,
    );
  });
...
describe("chartParamsSchema", () => {
  ...
  it("accepts all chart types", () => {
    for (const chart_type of ["bar", "line", "area", "pie"]) {
      expect(
        chartParamsSchema.safeParse({ ...valid, chart_type }).success,
      ).toBe(true);
    }
  });
  ...
  it("rejects invalid chart_type", () => {
    expect(
      chartParamsSchema.safeParse({ ...valid, chart_type: "scatter" }).success,
    ).toBe(false);
  });
