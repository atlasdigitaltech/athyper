...
function splitLightDark(css: string): { light: string; dark: string } {
  // Anchor to selector start to avoid matching comments or variable values.
  const match = /^(?:\.dark[:\s[]|\[data-theme="dark"\])/m.exec(css);
  if (!match) {
    return { light: css, dark: "" };
  }
  return {
    light: css.substring(0, match.index),
    dark: css.substring(match.index),
  };
}
...
export function validatePresetCSS(css: string, presetName: string): PresetValidationResult {
  ...
  if (dark.length === 0) {
    warnings.push("No .dark / [data-theme=\"dark\"] block found — dark mode will inherit light values.");
  }
