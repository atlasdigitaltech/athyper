...
export function WorkbenchModeSwitcher({
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent, index: number) => {
      const focusable = visibleModes.filter((m) => !m.disabled);
      const currentFocusable = focusable.findIndex((m) => m.key === visibleModes[index]?.key);
      let next = -1;
      if (e.key === "ArrowRight" || e.key === "ArrowDown") {
        e.preventDefault();
        next = (currentFocusable + 1) % focusable.length;
      } else if (e.key === "ArrowLeft" || e.key === "ArrowUp") {
        e.preventDefault();
        next = (currentFocusable - 1 + focusable.length) % focusable.length;
      }
      if (next >= 0) {
        const targetKey = focusable[next]?.key;
        const targetIdx = visibleModes.findIndex((m) => m.key === targetKey);
  ...
  const content = visibleModes.map((mode, index) => {
