    ...
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content
          align={align}
          sideOffset={4}
          className="z-popover min-w-[10rem] overflow-hidden rounded-md border bg-popover p-1 shadow-md animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95"
        >
          {items.map((item) => (
            <Fragment key={item.key}>
              ...
              >
                {item.icon && (
                  <span aria-hidden="true" className="size-4 shrink-0 [&>svg]:size-4">{item.icon}</span>
                )}
                {item.label}
              </DropdownMenu.Item>
            </Fragment>
