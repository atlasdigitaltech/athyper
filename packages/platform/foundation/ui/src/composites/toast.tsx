...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const dismiss = useCallback((id: string) => {
    ...
    // Mark as exiting so CSS exit animation plays, then remove from DOM.
    setToasts((prev) => prev.map((item) => item.id === id ? { ...item, exiting: true } : item));
    timers.current.set(`${id}-exit`, setTimeout(() => removeNow(id), 200));
  }, [removeNow]);
...
function ToastCard({
  ...
  const isUrgent = item.intent === "error" || item.intent === "warning";
  ...
  return (
    <div
      ...
      className={cn(
        "flex w-80 max-w-[calc(100vw-2rem)] items-start gap-3 rounded-lg border bg-background p-3.5 shadow-lg",
        item.exiting
