...
export function WorkbenchMatrix({
  ...
  return (
    <section className={cn("flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border bg-card", className)}>
      ...
      {selection && (
        <div className="flex flex-wrap items-center gap-2 border-t bg-primary px-3 py-2 text-primary-foreground">
          <div className="min-w-0 flex-1">
            <div className="truncate text-sm font-medium">{selection.title}</div>
            {selection.detail && <div className="truncate text-xs text-primary-foreground/70">{selection.detail}</div>}
          </div>
