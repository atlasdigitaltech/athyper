...
export function AppliedFilters({
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      const tops = [...new Set(children.map((child) => child.offsetTop))].sort(
        (a, b) => a - b,
      );
      const visible = children.filter(
        (child) => child.offsetTop < (tops[2] ?? Infinity),
      ).length;
      ...
      setLayout((previous) =>
        previous.visible === visible && previous.height === height
          ? previous
          : { visible, height },
      );
    };
  ...
  return (
    <section className="a-applied-filters" aria-label="Applied filters">
      ...
      <div
        ref={list}
        id={id}
        className="a-applied-filters__list"
        style={
          !expanded && overflowing ? { maxHeight: layout.height } : undefined
        }
      >
        {chips.map((chip, index) => {
          const hidden = !expanded && overflowing && index >= layout.visible;
          return (
            <button
              type="button"
              key={chip.key}
      ...
      <div className="a-applied-filters__actions">
        {overflowing ? (
          <button
            type="button"
