...
export function ScopePickerToolbar({
  query,
  onQueryChange,
  searchLabel,
  allLabel,
...
export function CompanyGroups({
  ...
  const toggle = (key: string) =>
    multiple
      ...
      : onSelect?.(key);
  const id = useId(),
    [collapsed, setCollapsed] = useState<ReadonlySet<string>>(new Set());
  const matches = companies.filter((c) =>
    `${c.displayName} ${c.code} ${c.legalEntityName} ${c.legalEntityCode}`
      .toLocaleLowerCase()
      .includes(query.trim().toLocaleLowerCase()),
  );
  const groups = new Map<string, CompanyChoice[]>();
  for (const company of matches)
    groups.set(company.legalEntityId, [
      ...(groups.get(company.legalEntityId) ?? []),
      company,
    ]);
  return (
    <div className="a-company-groups">
      {onQueryChange ? (
        <ScopePickerToolbar
          showControls={showAll && !multiple}
          query={query}
          ...
          onAll={() =>
            multiple ? onSelectionChange?.([]) : onSelect?.(undefined)
          }
        ...
        <>
          {showAll ? (
            <button
              type="button"
              className="a-company-groups__all"
      ...
      {[...groups.entries()]
        .sort(([, a], [, b]) =>
          a[0]!.legalEntityName.localeCompare(b[0]!.legalEntityName),
        )
        .map(([key, rows]) => {
          const legal = rows[0]!,
            expanded = !!query || !collapsed.has(key);
          return (
            <section key={key} className="a-company-groups__group">
              <button
                type="button"
                className="a-company-groups__heading"
              ...
              >
                <span className="a-company-groups__chevron" aria-hidden="true">
                  {expanded ? (
                ...
                <span className="a-company-groups__avatar">
                  {legal.legalEntityLogoUrl ? (
                    <img src={legal.legalEntityLogoUrl} alt="" />
                  ) : (
              ...
              {expanded ? (
                <div>
                  {rows.map((company) => (
                    <label
                      key={company.companyCodeId}
                      className="a-company-groups__row"
                      data-selected={
                        selected.includes(company.companyCodeId) || undefined
                      }
                    >
                      <input
                        data-context-picker-select={closeOnSelect || undefined}
                        ...
                        disabled={
                          multiple &&
                          selected.length >= 100 &&
                          !selected.includes(company.companyCodeId)
                        }
                        checked={selected.includes(company.companyCodeId)}
                        onChange={() => toggle(company.companyCodeId)}
                      />
                      <span>
