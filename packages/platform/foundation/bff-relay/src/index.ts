...
// Headers derived from the session. passthroughHeaders entries that collide
// with any of these are silently dropped so callers cannot spoof auth values.
const SESSION_OWNED_HEADERS = new Set([
  "authorization",
  "x-realm",
  "x-plane",
  "x-org",
...
export interface RelayHandlerOptions {
   ...
   * Must not begin with "api/" — the relay prepends that automatically.
...
export function buildRelayHandler(options: RelayHandlerOptions): RelayHandler {
  ...
  if (routePrefix && /^api(?:\/|$)/i.test(routePrefix)) {
    throw new Error(
      `Invalid relay routePrefix: "${routePrefix}" must not begin with "api/" — the relay adds that automatically`,
    );
  }
  ...
  return async function relay(req: NextRequest, { params }: RelayParams): Promise<NextResponse> {
    ...
    // Session headers are authoritative. passthroughHeaders entries that collide
    // with a session-owned key are skipped so callers cannot spoof tenant/auth values.
    const headers = new Headers(Object.entries(buildRuntimeHeaders(session)));
    for (const headerName of passthroughHeaders) {
      if (SESSION_OWNED_HEADERS.has(headerName.toLowerCase())) continue;
      const value = req.headers.get(headerName);
      if (value) headers.set(headerName, value);
    }

    const init: RequestInit = { method: req.method, headers, cache: "no-store" };
    let uploadCapExceeded = false;
      ...
      if (contentType?.startsWith("multipart/form-data")) {
        ...
        if (req.body) {
          if (Number.isFinite(contentLength)) {
            // Content-Length present — header check above already enforces the cap.
            init.body = req.body as BodyInit;
          } else {
            // No Content-Length (chunked transfer) — enforce cap by counting streamed bytes.
            let received = 0;
            const capBytes = uploadCapBytes;
            init.body = req.body.pipeThrough(new TransformStream<Uint8Array, Uint8Array>({
              transform(chunk, ctrl) {
                received += chunk.byteLength;
                if (received > capBytes) {
                  uploadCapExceeded = true;
                  ctrl.error(new RangeError("Upload cap exceeded"));
                } else {
                  ctrl.enqueue(chunk);
                }
              },
        ...
        // Only set Content-Type when a body is actually present, and only default
        // to application/json when the caller did not supply an explicit type.
        // Bodyless DELETE/PATCH requests should not advertise a content type.
        if (contentType) {
          headers.set("Content-Type", contentType);
        } else if (body) {
    ...
    // Declared outside the try block so the catch can clear them on any error path.
    ...
    try {
      if (streamingRequest) {
        ...
        streamTimeout = setTimeout(
          () => streamController!.abort(new DOMException(
      ...
      // Compose the client-disconnect signal on all paths so upstream fetches are
      // cancelled when the caller navigates away, regardless of streaming mode.
      const fetchSignal = streamController
      ...
      // Connection established — clear the connection-phase timeout on all paths.
      // For SSE, the client-disconnect leg of fetchSignal takes over from here.
      if (streamTimeout) {
      ...
      if (error instanceof Error) {
        ...
        if (error.name === "AbortError") {
          // The client disconnected before the upstream responded — nothing to return.
