...
export function formatBusinessDate(value: string, options: FormatBusinessDateOptions): string {
  ...
  if (options.template && !localeUsesAlternativeCalendar(options.locale)) {
 ...
 *
 * Seconds are included when the `localDateTime` carries them
 * ("2026-06-30T18:00:30" → "18:00:30").
 */
export function formatZonedDateTime(value: ZonedDateTimeValue, options: FormatZonedDateTimeOptions): string {
  ...
  const mi = Number(timePieces[1]);
  const hasSec = timePieces.length >= 3;
  const sec = hasSec ? Number(timePieces[2]) : null;
  ...
  let timeText = `${String(h).padStart(2, "0")}:${String(mi).padStart(2, "0")}`;
  if (sec !== null) {
    timeText += `:${String(sec).padStart(2, "0")}`;
  }
