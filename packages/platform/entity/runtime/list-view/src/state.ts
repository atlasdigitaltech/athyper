...
export function filterInputValue(filter: ListFilterV1): string {
  return Array.isArray(filter.value) ? filter.value.map(String).join(", ") : filter.value === undefined || filter.value === null ? "" : String(filter.value);
}

export function describeFilter(filter: ListFilterV1, descriptor: EntityListDescriptorV1): string {
  const label = descriptor.fields.find((field) => field.key === filter.field)?.label ?? filter.field;
  ...
  const value = filterInputValue(filter);
