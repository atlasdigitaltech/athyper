...
import { PageFrame, PageHeader, EntityPageLayout, useEntityBreadcrumbBinding } from "@athyper/platform-shell";
...
function EntityApplicationRuntime({client,entityCode,scopeCoordinate,scopeControl,scopePending=false,children,onNavigate,activePath}:EntityListRuntimeProps) {
  ...
  return <EntityApplicationContext.Provider value={{client,descriptor,scopeCoordinate,scopeControl}}><PageFrame width="wide" className="a-entity-application"><EntityPageLayout collectionHeader={<PageHeader level="collection" title={title} description={description} icon={<Icon/>} actions={actions.length?<>{actions.map(action=>{const label=action.localizedLabel?resolveEntityText(action.localizedLabel,locale):action.label;return action.state==="enabled"&&action.href?<a key={action.key} className={`a-button a-button--${action.placement}`} href={action.href} onClick={event=>{if(onNavigate&&event.button===0&&!event.metaKey&&!event.ctrlKey&&!event.shiftKey&&!event.altKey){event.preventDefault();onNavigate(action.href!);}}}>{label}</a>:<span className="a-entity-list__unavailable-action" key={action.key}><button className={`a-button a-button--${action.placement}`} disabled>{label}</button><small>{action.disabledMessage?resolveEntityText(action.disabledMessage,locale):undefined}</small></span>;})}</>:undefined}/>} collectionNavigation={<EntityNavigation sections={descriptor.navigation} onNavigate={onNavigate} activePath={activePath}/>}>{descriptor.scope.status === "context_required" ? scopeControl : children}</EntityPageLayout></PageFrame></EntityApplicationContext.Provider>;
}
