import type { ClipboardImageUploader } from "./clipboard-converter.js";
  ...
  return {
    async upload(file) {
      ...
      const staged = await json<{ attachmentId: string; uploadUrl: string }>(request, `${base}/stage`, {
        method: "POST", headers: headers(options, true), body: JSON.stringify({ attachmentId, fileName: file.name || "pasted-image", contentType: file.type || "application/octet-stream", sizeBytes: file.size, entityType: "document.comment" }),
      });
