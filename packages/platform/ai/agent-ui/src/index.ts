...
import { AtlasBusinessContextStore, AtlasBusinessContextProvider, useAtlasBusinessContext } from "./business-context";
...
export interface AtlasAnswerState {
  ...
  readonly actionReceipts: readonly { readonly action: AtlasGovernedAction; readonly context?: AtlasBusinessContextV1; readonly outcome: string; readonly commandId?: string }[];
...
export interface UseAtlasAnswerOptions { readonly proactiveBriefsEnabled?: boolean; readonly client?: AtlasAnswerClient; readonly scopeKey?: string; readonly locale?: string; }
const initialState: AtlasAnswerState = Object.freeze({ status: "idle", text: "", actionReceipts: Object.freeze([]), citations: Object.freeze([]), attachmentCitations:Object.freeze([]), actions: Object.freeze([]), historyVisible: false, historyStatus: "idle", history: Object.freeze([]), experienceStatus: "loading",threadsStatus:"idle",threads:Object.freeze([]),messages:Object.freeze([]) });

const AtlasAnswerContext=createContext<AtlasAnswerController|undefined>(undefined);
export function AtlasAnswerProvider({children,options}:{readonly children:ReactNode;readonly options?:UseAtlasAnswerOptions}){
  const store=useMemo(()=>new AtlasBusinessContextStore(options?.locale),[options?.scopeKey,options?.locale]);
  return createElement(AtlasBusinessContextProvider,{store,children:createElement(AtlasControllerProvider,{key:`${options?.scopeKey}:${options?.locale}`,options,store,children})});
}
function AtlasControllerProvider({children,options,store}:{readonly children:ReactNode;readonly options?:UseAtlasAnswerOptions;readonly store:AtlasBusinessContextStore}){
  const value=useAtlasAnswerState(options,store);
  useEffect(()=>{if(window.location.pathname!=="/atlas")return;const token=new URLSearchParams(window.location.search).get("context");if(token&&options?.scopeKey){const restoredThread=store.restore(options.scopeKey,token);if(restoredThread)void value.selectThread(restoredThread);}return()=>store.remove("fullscreen");},[store,options?.scopeKey]);
...
function useAtlasAnswerState(options: UseAtlasAnswerOptions = {}, store: AtlasBusinessContextStore): AtlasAnswerController {
  ...
  const mountWorkspace = useCallback(() => {
    ...
    return () => {
      ...
      queueMicrotask(() => {
        if (!workspaceOwners.current.size && automaticRun.current && automaticRun.current === active.current) {
          active.current?.abort();
          lastAutomaticKey.current = undefined;
          setState(current => ({...current, status: "idle", messages: updatePendingMessage(current.messages, "cancelled")}));
        }
      });
    };
  }, []);
  ...
  useEffect(() => store.subscribe(() => {
    active.current?.abort(); active.current=undefined; threadId.current=undefined;
    setState(current=>({...current,threadId:undefined,status:"idle",text:"",message:undefined,citations:[],attachmentCitations:[],actions:[],messages:[]}));
  }),[store]);
  ...
  const ask = useCallback(async (question: string, agentCode?: string, attachmentContext?: { readonly contextId:string; readonly attachmentIds:readonly string[] }) => {
    ...
    const requestKey = JSON.stringify([capturedContext?.generationId, question, agentCode, attachmentContext]);
    if (active.current && !active.current.signal.aborted && activeKey.current === requestKey) return;
    conversationVersion.current++;
    activeKey.current = requestKey;
    const continuingThreadId=threadId.current&&(!capturedContext||store.isThreadBound(threadId.current,capturedContext.generationId))?threadId.current:undefined;
    ...
    setState((current) => ({ ...current, threadId:continuingThreadId, status: "answering", text: "", citations: Object.freeze([]), attachmentCitations:Object.freeze([]), actions: Object.freeze([]), message: undefined, actionMessage: undefined,messages:Object.freeze([...(resetTranscript?[]:current.messages),userMessage,assistantMessage]) }));
    try {
      ...
      const answer = await client.answer(question, { signal: controller.signal,...(capturedContext?{businessContext:capturedContext}:{}),...(continuingThreadId?{threadId:continuingThreadId}:{}), ...(agent?{agent}:{}),...(attachmentContext?{attachmentContextId:attachmentContext.contextId,attachmentIds:attachmentContext.attachmentIds}:{}), onProgress(progress) {
        if (controller.signal.aborted || store.snapshot()?.generationId !== capturedContext?.generationId) return;
      ...
      if (!controller.signal.aborted && store.snapshot()?.generationId === capturedContext?.generationId){threadId.current=answer.threadId;if(capturedContext)store.bindThread(answer.threadId,capturedContext.generationId);setState((current) => ({ ...current,threadId:answer.threadId, status: "complete", text: answer.text, citations: answer.citations, attachmentCitations:answer.attachmentCitations, actions: answer.actions, publicModelId: answer.publicModelId,messages:completePendingMessage(current.messages,answer.text,"completed",answer) }));void client.threads("active").then((page)=>setState((current)=>({...current,threadsStatus:"ready",threads:page.items}))).catch(()=>undefined);}
    } catch (error) {
      if (controller.signal.aborted || store.snapshot()?.generationId !== capturedContext?.generationId) return;
      const unavailable = isUnavailable(error);
      const message=unavailable ? "No Atlas answer mode is available for this session. Navigation results remain available." : atlasAnswerErrorMessage(error);setState((current) => ({ ...current, status: unavailable ? "unavailable" : "error", text: "", citations: Object.freeze([]), actions: Object.freeze([]), message,messages:completePendingMessage(current.messages,message,"failed") }));
  ...
  const refreshBrief = useCallback(async () => {
    ...
    if (!options.proactiveBriefsEnabled || !page || !key || active.current && !active.current.signal.aborted) return;
    ...
    try { await pending; }
    finally { if (automaticRun.current === controller) automaticRun.current = undefined; }
  }, [ask, store, options.proactiveBriefsEnabled]);

  useEffect(() => {
    if (!options.proactiveBriefsEnabled || !automaticBriefsEnabled || !workspaceCount || state.experienceStatus === "loading") return;
    const key = automaticBriefKey(businessContext);
    if (!key || key === lastAutomaticKey.current || state.status === "answering") return;
    const version = conversationVersion.current;
    const timer = setTimeout(() => {
      if (conversationVersion.current === version) void refreshBrief();
    }, 300);
    return () => clearTimeout(timer);
  }, [options.proactiveBriefsEnabled, automaticBriefsEnabled, workspaceCount, businessContext, state.status, state.experienceStatus, refreshBrief]);

  useEffect(() => {
    if (!options.proactiveBriefsEnabled) {
      setAutomaticBriefs(false);
      if (automaticRun.current && automaticRun.current === active.current) cancel();
    }
  ...
  const transitionAction = useCallback(async (action: AtlasGovernedAction, kind: "confirm" | "decline") => {
    ...
    try {
      ...
      setState((current) => ({ ...current, actionReceipts:[...current.actionReceipts,{action,context:actionContext,outcome:result.outcome,commandId:result.commandId}], actionBusy: undefined, actionMessage: store.snapshot()!==actionContext?undefined:result.outcome === "completed" ? "Action completed and audit evidence recorded." : "Action declined and recorded in the audit history.", actions: updateAction(current.actions, action.proposalId, result.outcome === "completed" ? "completed" : result.outcome) }));
    } catch (error) {
      setState((current) => ({ ...current, actionBusy: undefined, actionMessage: store.snapshot()===actionContext?actionError(error):undefined, actions: updateAction(current.actions, action.proposalId, "failed") }));
    }
  }, [client]);
  ...
  const selectThread=useCallback(async(nextThreadId:string)=>{const version=++conversationVersion.current;const captured=store.snapshot();active.current?.abort();setState((current)=>({...current,threadsStatus:"loading",status:"idle",message:undefined}));try{const page=await client.messages(nextThreadId);if(store.snapshot()!==captured||version!==conversationVersion.current)return;threadId.current=nextThreadId;setState((current)=>({...current,threadId:nextThreadId,threadsStatus:"ready",messages:page.items,text:"",citations:Object.freeze([]),attachmentCitations:Object.freeze([]),actions:Object.freeze([])}));}catch{setState((current)=>({...current,threadsStatus:"error"}));}},[client]);
  const newConversation=useCallback(()=>{conversationVersion.current++;active.current?.abort();threadId.current=undefined;setState((current)=>({...current,threadId:undefined,status:"idle",text:"",message:undefined,citations:Object.freeze([]),attachmentCitations:Object.freeze([]),actions:Object.freeze([]),messages:Object.freeze([])}));},[]);
  ...
  return {
    ...
    setAutomaticBriefsEnabled: enabled => {
      ...
      if (!enabled && automaticRun.current && automaticRun.current === active.current) cancel();
    },
    mountWorkspace,
    refreshBrief,
    historyContextUnbound:!!(businessContext&&state.threadId&&!store.isThreadBound(state.threadId,businessContext.generationId)),
    fullscreenHref: from => { const token=options.scopeKey?store.handoff(options.scopeKey,threadId.current):undefined;return `/atlas?from=${encodeURIComponent(from)}${token?`&context=${token}`:""}`; },
    ask,
    cancel,
...
function isUnavailable(error: unknown): boolean { return error instanceof AtlasClientError && ["ATLAS_NOT_ADMITTED", "ATLAS_MODE_UNAVAILABLE"].includes(error.code); }
export function atlasAnswerErrorMessage(error: unknown): string { if (statusOf(error) === 403) return "Atlas could not access the selected record or transaction context. Check your selected organization, company and role, then try again."; if (statusOf(error) === 404) return "The requested Atlas resource is unavailable. Refresh the page and start a new conversation."; if (statusOf(error) === 503) return "Atlas is temporarily unavailable. Try again shortly."; if (statusOf(error) === 429) return "Atlas usage limit reached. Wait for the quota to reset or contact your administrator."; if (error instanceof AtlasClientError) return error.message; return "Atlas could not answer this question. Try again or use the matching destinations below."; }
...
export { useAtlasContextNavigation, useAtlasBusinessContextPublisher, type AtlasBusinessContextInput } from "./business-context";

export { parseAtlasInsightResult, type AtlasBusinessContextV1, parseAtlasAnswerEnvelope, type AtlasAnswerEnvelope, type AtlasAnswerAuthority, type AtlasGroundedAnswer } from "@athyper/platform-ai-agent-runtime";
