...
export async function resolveExistingSessionLanding(input: ExistingSessionLandingDependencies): Promise<string | undefined> {
  ...
  if (session.state === "authenticated") return sanitizeReturnTo(input.returnTo ?? input.home);
  if (session.state === "context_required") return gateLocation("/select-context", input.returnTo ?? input.home);
  if (session.state === "required_action") return gateLocation("/auth/required-action", input.returnTo ?? input.home);
  return undefined;
}
...
export async function readProtectedBootstrap(input: ProtectedBootstrapDependencies): Promise<ProtectedBootstrapResult> {
  ...
  const returnTo = sanitizeReturnTo(input.returnTo ?? `${url.pathname}${url.search}`);
  const sessionResponse = await input.readSession(input.request.clone());
  if (sessionResponse.status === 401) return Object.freeze({ state: "redirect", location: loginLocation(returnTo), reason: "unauthenticated" });
  ...
  if (session.state === "anonymous") return Object.freeze({ state: "redirect", location: loginLocation(returnTo), reason: "unauthenticated" });
  if (session.state === "required_action") return Object.freeze({ state: "redirect", location: gateLocation("/auth/required-action", returnTo), reason: "required_action" });
  const scope = principalQueryScope(session);
  if (!scope) return Object.freeze({ state: "redirect", location: gateLocation("/select-context", returnTo), reason: "context_required" });
  ...
  if (experienceResponse.status === 401) return Object.freeze({ state: "redirect", location: loginLocation(returnTo), reason: "unauthenticated" });
...
function loginLocation(returnTo: string): string { return `/api/auth/login?returnTo=${encodeURIComponent(sanitizeReturnTo(returnTo))}`; }
...
export function bootstrapRevalidationReason(previous: Readonly<{ session: SanitizedSession; bootstrap: ExperienceBootstrap }>, next: Readonly<{ session: SanitizedSession; bootstrap: ExperienceBootstrap }>, now = Date.now(), expirySkewMs = 60_000): BootstrapRevalidationReason | undefined {
  ...
  const expiry = next.session.expiresAt ? parseInstant(next.session.expiresAt) : Number.NaN;
