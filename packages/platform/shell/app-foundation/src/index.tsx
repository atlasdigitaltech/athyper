...
const SessionExpiryContext = createContext<Readonly<{ expiresAt?: string; idleExpiresAt?: string; absoluteExpiresAt?: string }> | undefined>(undefined);
            ...
            <AccessProvider snapshot={access}><PermissionProvider permissions={bootstrap.permissions}>
              <FeatureProvider features={bootstrap.features}>
                <ToastProvider><SurfaceStackProvider><ShellStateProvider>
                  ...
                  <SessionExpiryWarning absoluteExpiresAt={props.session.absoluteExpiresAt} />
                  {props.children}
...
function SessionProvider({ session, lifecycle, children }: { readonly session: SanitizedSession; readonly lifecycle: PrincipalQueryLifecycle; readonly children: ReactNode }) {
  ...
  const expiry = useMemo(() => Object.freeze({ ...(session.expiresAt ? { expiresAt: session.expiresAt } : {}), ...(session.idleExpiresAt ? { idleExpiresAt: session.idleExpiresAt } : {}), ...(session.absoluteExpiresAt ? { absoluteExpiresAt: session.absoluteExpiresAt } : {}) }), [session.expiresAt, session.idleExpiresAt, session.absoluteExpiresAt]);
  ...
  return <SessionIdentityContext.Provider value={identity}><SessionExpiryContext.Provider value={expiry}><SessionActionsContext.Provider value={actions}>{children}</SessionActionsContext.Provider></SessionExpiryContext.Provider></SessionIdentityContext.Provider>;
}
...
function BootstrapFreshnessBridge({ session, onRevalidate }: { readonly session: SanitizedSession; readonly onRevalidate?: (reason: BootstrapRevalidationReason) => void }) {
  useEffect(() => {
    ...
    const expiresAt = session.expiresAt ? Date.parse(session.expiresAt) : Number.NaN, delay = Number.isFinite(expiresAt) ? Math.max(0, expiresAt - Date.now() - 60_000) : undefined;
  ...
  }, [session.expiresAt, session.authEpoch, session.tenantId, session.plane, onRevalidate]);
  return null;
}

const ABSOLUTE_EXPIRY_WARNING_MS = 5 * 60_000;
export function sessionExpiryWarningDelay(absoluteExpiresAt: string | undefined, currentTime = Date.now(), leadTimeMs = ABSOLUTE_EXPIRY_WARNING_MS): number | undefined { const expiry = absoluteExpiresAt ? Date.parse(absoluteExpiresAt) : Number.NaN; if (!Number.isFinite(expiry) || expiry <= currentTime) return undefined; return Math.max(0, expiry - currentTime - leadTimeMs); }
function SessionExpiryWarning({ absoluteExpiresAt }: { readonly absoluteExpiresAt?: string }) {
  const { push } = useToasts(); const warnedFor = useRef<string | undefined>(undefined);
  useEffect(() => {
    const delay = sessionExpiryWarningDelay(absoluteExpiresAt); if (delay === undefined || !absoluteExpiresAt) return;
    const notify = () => { if (warnedFor.current === absoluteExpiresAt) return; warnedFor.current = absoluteExpiresAt; const time = new Date(absoluteExpiresAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }); push({ tone: "warning", title: "Session ending soon", detail: `Your security session ends at ${time}. Save your work before signing in again.` }); };
    const timer = window.setTimeout(notify, delay); return () => window.clearTimeout(timer);
  }, [absoluteExpiresAt, push]);
  return null;
}
