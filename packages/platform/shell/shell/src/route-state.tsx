...
export interface RecordBreadcrumbBinding { readonly pathname: string; readonly label: string; }
...
export function deriveEntityBreadcrumbs(navigation:DerivedShellNavigation,pathname:string,binding?:EntityBreadcrumbBinding,record?:RecordBreadcrumbBinding) {
  ...
  if(record?.pathname===path){ const crumbs=deriveBreadcrumbs(navigation,path); return [...crumbs.slice(0,-1),{label:record.label,href:path}]; }
...
export function useRecordBreadcrumb(label: string) {
  const route=useShellRoute(), pathname=route?.pathname, setRecord=route?.setRecord;
  useEffect(()=>{ if(!setRecord||!pathname)return; const record={pathname:pathname.split(/[?#]/)[0]!,label}; setRecord(record); return()=>setRecord(current=>current===record?undefined:current); },[setRecord,pathname,label]);
}
