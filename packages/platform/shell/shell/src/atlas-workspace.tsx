...
import { useAtlasAnswer, type AtlasResponseFeedbackV1, type AtlasVocabularyCorrection, type AtlasConversationMessage, type AtlasExperienceAgent, type AtlasGovernedAction } from "@athyper/platform-ai-agent-ui";
...
function ConversationBubble({message,onFeedback,onCorrection}:{readonly message:AtlasConversationMessage;readonly onFeedback?:(value:AtlasResponseFeedbackV1)=>Promise<void>;readonly onCorrection?:(value:AtlasVocabularyCorrection)=>Promise<void>}){return <article className="athyper-atlas-workspace__message" data-role={message.role} data-status={message.status}><header><strong>{message.role==="user"?"You":message.role==="tool"?"Authorized result":"Atlas AI"}</strong><time dateTime={message.createdAt}>{timeLabel(message.createdAt)}</time></header>{message.answer?.intent?.kind === "clarify" ? <p role="status">Clarification needed</p> : message.answer?.intent?.kind === "denied" ? <p role="status">Unavailable in the current context</p> : null}{message.answer?.envelope&&message.status==="completed"?<AtlasValidatedAnswer answer={message.answer}/>:message.text||!message.results.length?message.role==="assistant"&&message.status==="completed"?<AtlasSafeProse text={message.text||"No text was returned."}/>:<p>{message.text|| (message.status==="pending"?"Working on it…":"No text was returned.")}</p>:null}{message.results.map((result,index)=><StructuredResult key={index} value={result}/>)}{onFeedback && message.role==="assistant" && message.status==="completed" && message.runId && !message.messageId.startsWith("local-") ? <ResponseFeedback message={message} submit={onFeedback} propose={onCorrection}/> : null}{message.status==="failed"||message.status==="cancelled"?<small>{message.status==="failed"?"This response did not complete.":"Response stopped."}</small>:null}</article>;}
...
function ResponseFeedback({message,submit,propose}:{message:AtlasConversationMessage;submit:(value:AtlasResponseFeedbackV1)=>Promise<void>;propose?:(value:AtlasVocabularyCorrection)=>Promise<void>}) {
  const [category,setCategory]=useState<AtlasResponseFeedbackV1["category"]>("intent");
  const [verdict,setVerdict]=useState<AtlasResponseFeedbackV1["verdict"]>("wrong");
  const [status,setStatus]=useState<"idle"|"saving"|"saved"|"error">("idle");
  const receipt=useRef<{key:string;id:string}|undefined>(undefined);
  const send=async()=>{const key=`${category}:${verdict}`;if(receipt.current?.key!==key)receipt.current={key,id:crypto.randomUUID()};setStatus("saving");try{await submit({schemaVersion:1,feedbackId:receipt.current.id,runId:message.runId!,messageId:message.messageId,category,verdict});setStatus("saved");}catch{setStatus("error");}};
  return <details><summary>Response feedback</summary><fieldset disabled={status==="saving"||status==="saved"}><label>Feedback about <select value={category} onChange={e=>setCategory(e.target.value as AtlasResponseFeedbackV1["category"])}>{[["vocabulary","Wording"],["intent","What I meant"],["missing_context","Missing work context"],["unsupported_capability","Unavailable capability"],["owner_failure","Read or service failure"],["evidence","Evidence"],["presentation","Presentation"]].map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label>Assessment <select value={verdict} onChange={e=>setVerdict(e.target.value as AtlasResponseFeedbackV1["verdict"])}>{["correct","wrong","partial","missing"].map(value=><option key={value}>{value}</option>)}</select></label><button type="button" onClick={()=>void send()}>Send feedback</button></fieldset><p role="status">{status==="saved"?"Feedback recorded for review.":status==="error"?"Feedback could not be recorded. Try again.":status==="saving"?"Saving feedback…":"Feedback is reviewed before vocabulary changes."}</p>{status === "saved" && propose && (category === "intent" || category === "vocabulary") && verdict !== "correct" ? <VocabularyCorrection feedbackId={receipt.current!.id} propose={propose}/> : null}</details>;
}

function VocabularyCorrection({feedbackId,propose}:{feedbackId:string;propose:(value:AtlasVocabularyCorrection)=>Promise<void>}) {
  const [phrase,setPhrase]=useState("");
  const [status,setStatus]=useState<"idle"|"saving"|"saved"|"error">("idle");
  const receipt=useRef<{phrase:string;id:string}|undefined>(undefined);
  const send=async()=>{if(receipt.current?.phrase!==phrase)receipt.current={phrase,id:crypto.randomUUID()};setStatus("saving");try{await propose({schemaVersion:1,candidateId:receipt.current.id,feedbackId,locale:"en",phrase,capabilityId:"entity_read_record"});setStatus("saved");}catch{setStatus("error");}};
  return <section aria-label="Suggest vocabulary"><p>Teach a short English term for the record summary, such as “company snapshot”. Studio reviewers will receive the term and its intended meaning.</p><fieldset disabled={status==="saving"||status==="saved"}><label>Term for record summary <input maxLength={80} value={phrase} onChange={event=>setPhrase(event.target.value)}/></label><button type="button" disabled={!phrase.trim()} onClick={()=>void send()}>Share term with Studio</button></fieldset><p role="status">{status==="saved"?"Correction sent for review. It takes effect after publication.":status==="error"?"Correction could not be sent. Retry, or ask again on the current record if its definition changed.":status==="saving"?"Sending correction…":"Use a general term, without names or confidential record values."}</p></section>;
}
