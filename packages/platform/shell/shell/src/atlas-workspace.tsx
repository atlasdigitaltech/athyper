...
export function AtlasWorkspace({mode,planeName,currentPath="/home",pinned=false,onPinnedChange,onClose}:AtlasWorkspaceProps){
  ...
  const contextLabel=useMemo(()=>contextForPath(effectivePath),[effectivePath]);
  ...
  return <section className={`athyper-atlas-workspace athyper-atlas-workspace--${mode}`} aria-label="Atlas AI workspace">
    <header className="athyper-atlas-workspace__header"><span className="athyper-atlas-workspace__mark"><SparklesIcon size={18}/></span><div><strong>Atlas AI</strong><small>{planeName} · {contextLabel}</small></div><nav aria-label="Atlas workspace controls">
      ...
      {mode==="dock"?<><button type="button" title={pinned?"Unpin Atlas":"Keep Atlas open"} aria-label={pinned?"Unpin Atlas":"Keep Atlas open"} aria-pressed={pinned} onClick={()=>onPinnedChange?.(!pinned)}><PanelsTopLeftIcon size={16}/></button><a href={`/atlas?from=${encodeURIComponent(currentPath)}`} title="Open full-screen Atlas" aria-label="Open full-screen Atlas"><LayoutIcon size={16}/></a></>:null}
    ...
    <div className="athyper-atlas-workspace__body" data-history={historyOpen}>
      {historyOpen?<ConversationHistory atlas={atlas}/>:null}
      <main className="athyper-atlas-workspace__conversation">
        ...
        <div className="athyper-atlas-workspace__messages" aria-live="polite">{atlas.messages.length?atlas.messages.filter((message)=>message.role==="user"||message.role==="assistant"||message.role==="tool").map((message)=><ConversationBubble key={message.messageId} message={message}/>):<EmptyConversation agentName={agent?.name}/>}</div>
        ...
        <AtlasPromptComposer ref={composer} draftKey={`${scope.storageKey}:atlas-workspace-prompt`} value={draft} onChange={setDraft} onSubmit={submit} busy={atlas.status==="answering"} agents={atlas.experience?.agents} selectedAgent={selectedAgent} onAgentChange={setSelectedAgent}/>
      </main>
