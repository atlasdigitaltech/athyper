...
export interface ShellChromeProps { readonly applicationName: string; readonly tenantLabel: string; readonly accountLabel: string; readonly navigation: DerivedShellNavigation; readonly contexts?: readonly ShellContextOption[]; readonly children: ReactNode; }
export function ShellChrome({ applicationName, tenantLabel, accountLabel, navigation, contexts, children }: ShellChromeProps) {
  ...
  return <div className="athyper-shell" data-collapsed={collapsed} data-drawer-open={drawerOpen}>
    ...
    <div className="athyper-shell__body">
      ...
      <main id="main-content" tabIndex={-1} className="athyper-shell__main">{!navigation.routes.length ? <EmptyEntitlement /> : routeAllowed ? children : <ForbiddenRoute />}</main>
    </div>
  </div>;
}
...
function AccountMenu({ accountLabel }: { readonly accountLabel: string }) { return <details className="athyper-shell__account"><summary aria-label="Open account menu">{accountLabel}</summary><div><span>{accountLabel}</span><a href="/logout">Sign out</a></div></details>; }
