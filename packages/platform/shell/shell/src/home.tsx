...
export interface AtlasPromptComposerHandle { focus(): void; setText(value: string): void; }
...
export const AtlasPromptComposer = React.forwardRef<AtlasPromptComposerHandle, AtlasPromptComposerProps>(function AtlasPromptComposer(props, forwardedRef) {
  ...
  const addFiles=React.useCallback(async(files:FileList|readonly File[])=>{
    ...
    const remaining=Math.max(0,5-attachments.length);if(!remaining){setFormatMessage("You can add up to 5 files.");return;}
    const candidates=selected.slice(0,remaining),accepted=candidates.filter((file)=>file.size>0&&file.size<=25*1024*1024&&supportedAtlasFile(file));
    if(!accepted.length){setFormatMessage(candidates.some((file)=>file.size>25*1024*1024)?"This file exceeds the 25 MB limit.":"Choose a supported file up to 25 MB.");return;}
    setFormatMessage(accepted.length<candidates.length?"Some files couldn’t be added. Check their type and size.":undefined);
    ...
    await Promise.all(accepted.map(async(file,index)=>{
      ...
      try{
        ...
        setAttachments((current)=>replaceAttachment(current,item.localId,{attachmentId:uploaded.attachmentId,status:status.extractionStatus==="extracted"?"ready":"error",...(status.extractionStatus==="extracted"?{}:{message:"We couldn’t prepare this file. Remove it and try again."})}));
      }catch(error){setAttachments((current)=>replaceAttachment(current,item.localId,{status:"error",message:attachmentMessage(error)}));}
    }));
    if(selected.length>remaining)setFormatMessage("You can add up to 5 files.");
  },[attachments.length]);
...
async function waitForAttachment(client:ReturnType<typeof createAttachmentApiClient>,attachmentId:string):Promise<AttachmentProcessingStatus>{for(let attempt=0;attempt<40;attempt+=1){const status=await client.status(attachmentId);if(status.extractionStatus==="extracted"||status.extractionStatus==="failed"||status.extractionStatus==="skipped")return status;await new Promise((resolve)=>setTimeout(resolve,1_500));}throw new Error("This file is taking longer than expected.");}
function attachmentMessage(error:unknown):string{if(error instanceof AttachmentApiError){if(error.status===413)return"This file exceeds the 25 MB limit.";if(error.status===429)return"Attachment storage is full. Remove a file or contact your administrator.";if(error.status===403)return"You don’t have permission to use this file.";}if(error instanceof Error&&error.message==="This file is taking longer than expected.")return error.message;return"We couldn’t prepare this file. Remove it and try again.";}
function attachmentStatusLabel(item:AtlasPromptAttachment):string{return item.status==="uploading"?"Uploading and virus scanning…":item.status==="processing"?"Preparing governed content…":item.status==="ready"?"Ready to use":item.message??"We couldn’t prepare this file. Remove it and try again.";}
...
const ATLAS_FILE_EXTENSIONS=new Set(["pdf","txt","md","csv","doc","docx","xls","xlsx","ppt","pptx"]);
function supportedAtlasFile(file:File):boolean{const extension=file.name.toLocaleLowerCase().split(".").pop();return Boolean(extension&&ATLAS_FILE_EXTENSIONS.has(extension));}
