...
export function ReportTitle({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <span className={cn("shrink-0 text-sm font-semibold leading-5 text-foreground", className)}>
      {children}
    </span>
  );
}
...
export function ReportHeaderRow({
  ...
  return (
    <div className={cn("-mx-2 border-b sm:-mx-4", className)}>
      <div className="flex h-10 min-w-0 items-center gap-3 overflow-x-auto px-2 py-1 text-sm text-muted-foreground sm:px-4">
...
export function ReportMetricCard({
  ...
  return (
    <div className={cn("space-y-1 rounded-lg border bg-background px-3 py-2.5", className)}>
      ...
      <div className={cn("text-sm font-semibold tabular-nums", reportToneClass(tone))}>
        {value}
      </div>
...
export function StatementColumnHeader({
  ...
  return (
    <div className="flex items-center border-b bg-muted/20 px-2 py-2 text-xs font-semibold text-muted-foreground">
