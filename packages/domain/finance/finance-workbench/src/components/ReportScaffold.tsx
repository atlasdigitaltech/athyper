  ...
  return (
    <span
      className={cn(
        ...
        "text-[12px] font-medium leading-none text-info",
        className,
      )}
    >
      Live
...
export function ReportTitle({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <span className={cn("shrink-0 text-[13px] font-semibold leading-5 text-foreground", className)}>
      {children}
    </span>
  );
}
...
export function ReportHeaderRow({
  ...
  return (
    <div className={cn("-mx-2 border-b sm:-mx-4", className)}>
      <div className="flex h-10 min-w-0 items-center gap-3 overflow-x-auto px-2 py-1 text-[13px] leading-5 text-muted-foreground sm:px-4">
...
export function ReportMetaRow({
  ...
  return (
    <div className={cn("flex flex-wrap items-center justify-between gap-2 text-[12px] leading-4 text-muted-foreground", className)}>
      <span>
...
export function ReportMetricCard({
  ...
  return (
    <div className={cn("space-y-1 rounded-lg border bg-background px-3 py-2.5", className)}>
      ...
      <div className={cn("text-[13px] font-semibold leading-5 tabular-nums", reportToneClass(tone))}>
        {value}
      </div>
      {detail && <div className="text-[12px] leading-4 text-muted-foreground">{detail}</div>}
    </div>
  );
}
...
export function ReportGraphMetric({
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-md border bg-background px-3 py-2 text-[13px] leading-5">
      <span className="truncate text-muted-foreground">{label}</span>
      <span className={cn("shrink-0 font-semibold tabular-nums", reportToneClass(tone))}>
...
export function StatementColumnHeader({
  ...
  return (
    <div className="flex items-center border-b bg-muted/20 px-2 py-2 text-[12px] font-semibold leading-4 text-muted-foreground">
