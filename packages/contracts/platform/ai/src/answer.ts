...
} from "@athyper/contract-platform-ai/insights";
...
export interface AtlasAnswerEnvelope {
  ...
  readonly kind:
    "brief" | "explanation" | "comparison" | "worklist" | "unavailable";
...
export function parseAtlasAnswerEnvelope(
  value: unknown,
  authority: AtlasAnswerAuthority,
): AtlasAnswerEnvelope {
  const fail = (): never => {
    throw new TypeError("Invalid Atlas answer envelope or reference.");
  };
  ...
  if (
    ...
    !["brief", "explanation", "comparison", "worklist", "unavailable"].includes(
      item.kind as string,
    )
  )
    return fail();
  ...
  const insight = authority.insight
    ? parseAtlasInsightResult(authority.insight)
    : undefined;
  const refs = (
    value: unknown,
  ...
  const findingIds = refs(
    item.findingIds,
    insight?.findings.map((f) => f.id) ?? [],
  );
  const evidenceIds = refs(item.evidenceIds, [
    ...authority.evidenceIds,
    ...(insight?.evidence.map((e) => e.id) ?? []),
  ]);
  // Every selected finding carries its owner evidence; prose cannot detach the supporting sources.
  for (const finding of insight?.findings.filter((f) =>
    findingIds.includes(f.id),
  ) ?? []) {
    if (finding.evidenceIds.some((id) => !evidenceIds.includes(id)))
      return fail();
  }
  if (item.nextActionId !== undefined) {
    ...
    if (
      typeof item.nextActionId !== "string" ||
      !authority.actionIds.includes(item.nextActionId)
    )
      return fail();
  ...
  return Object.freeze({
    ...
    ...(item.explanation === undefined
      ? {}
      : { explanation: prose(item.explanation, 24000) }),
    findingIds,
    evidenceIds,
    ...(item.nextActionId === undefined
      ? {}
      : { nextActionId: item.nextActionId as string }),
  });
}
