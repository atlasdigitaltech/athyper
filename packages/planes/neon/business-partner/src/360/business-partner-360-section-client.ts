...
import type { RoleLens, SummaryQuery } from "./business-partner-360-client";

export type CommonSectionCode =
  "identity" | "contacts" | "addresses" | "identifiers-tax" | "governance";
export interface CommonSectionItem {
  readonly id: string;
  ...
  readonly industryDomainCode?: string;
  ...
  readonly validationProvider?: string;
  ...
  readonly memberBusinessPartnerId?: string;
  readonly memberCountryCode?: string;
  readonly ownershipPercent?: number;
  readonly votingPercent?: number;
  readonly beneficialOwnershipPercent?: number;
  readonly appointedDate?: string;
  readonly endOfTerm?: string;
  readonly status?: string;
}
export interface CommonSection {
  readonly schemaVersion: 1;
  readonly sectionCode: CommonSectionCode;
  readonly state: "ready" | "empty" | "partial" | "stale" | "unavailable";
  readonly data: Readonly<{
    items: readonly CommonSectionItem[];
    nextCursor?: string;
  }>;
  ...
  readonly redactions: readonly Readonly<{
    fieldCode: string;
    behavior: string;
    reasonCode: string;
  }>[];
}
export interface SectionQuery extends SummaryQuery {
  readonly sectionCode: CommonSectionCode;
  readonly cursor?: string;
  readonly limit?: number;
}
export interface TaxReveal {
  readonly taxRegistrationId: string;
  readonly value: string;
  readonly expiresAt: string;
}
const routes: Readonly<Record<CommonSectionCode, string>> = {
  identity: "identity",
  contacts: "contacts",
  addresses: "addresses",
...
const readSection = createOperation<CommonSection>({
  method: "GET",
...
const revealTax = createOperation<
  TaxReveal,
  Readonly<{
...
>({
  ...
  path: ({ businessPartnerId }) =>
    `/api/neon/business-partners/${encodePathSegment(businessPartnerId)}/360/identifiers-tax/reveal`,
  parse: (value) => record(value) as unknown as TaxReveal,
});
export function createBusinessPartner360SectionClient(http: HttpClient) {
  return Object.freeze({
    read: (query: SectionQuery, signal?: AbortSignal) =>
      http.request(readSection, {
        params: {
          businessPartnerId: query.businessPartnerId,
          section: routes[query.sectionCode],
        },
    ...
    revealTax: (
      ...
      http.request(revealTax, {
        params: { businessPartnerId },
        body: { taxRegistrationId, purpose, ...revealClaim() },
        query: {
...
export function sectionQueryKey(query: SectionQuery) {
  return [
...
function coordinates(query: SectionQuery) {
  return {
...
function revealClaim() {
  return {
    revealId: crypto.randomUUID(),
    purposeExpiresAt: new Date(Date.now() + 30_000).toISOString(),
  };
}
function parseSection(value: unknown): CommonSection {
  const body = record(value),
    serialized = JSON.stringify(body);
  if (
    /"(?:identifierValue|registrationNumber|protectedValueToken|dateOfBirth|nationalId)"\s*:/i.test(
      serialized,
    )
  )
    throw new TypeError("Business Partner section contains a restricted field");
  if (
    body["schemaVersion"] !== 1 ||
    !record(body["data"]) ||
    !Array.isArray(record(body["data"])["items"])
  )
    throw new TypeError("Business Partner section contract is invalid");
  return Object.freeze(body as unknown as CommonSection);
}
function record(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new TypeError("Business Partner section response must be an object");
