...
import { useNeonOperatingOrganization, useNeonWorkContext, type NeonOperatingOrganizationValue } from "@athyper/product-neon-shell";
...
export function BusinessPartner360Shell({ businessPartnerId }: { readonly businessPartnerId: string }) {
  ...
  const organization = resolveBusinessPartner360Organization(organizations, url.roleLens);
...
export function resolveBusinessPartner360Organization(organizations:Pick<NeonOperatingOrganizationValue,"selected"|"compatible">,roleLens:RoleLens){
  const capabilities=roleLens==="customer"?["sales"]as const:roleLens==="supplier"?["procurement"]as const:["shared_services","procurement","sales"]as const;
  for(const capability of capabilities){const selected=organizations.selected(capability);if(selected)return selected;}
  const compatible=[...new Map(capabilities.flatMap(capability=>organizations.compatible(capability)).map(organization=>[organization.id,organization])).values()];
...
function isCommon(value:string):value is CommonSectionCode{return value==="identity"||value==="contacts"||value==="addresses"||value==="identifiers-tax"||value==="governance";}
