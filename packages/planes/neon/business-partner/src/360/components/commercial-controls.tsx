        ...
        <Card className="bp360-section-card">
          {sharedFactsOnly ? (
            <>
              ...
              <a
                href={`/mdg/business-partner/${encodeURIComponent(businessPartnerId)}?tab=roles&section=roles-scope&roleTab=banks`}
              >
                Configure company usage &amp; acceptance
              </a>
            </>
          ) : (
            <>
  ...
  return (
    <Card className="bp360-section-card">
      ...
      <dl>
        ...
        {!sharedFactsOnly ? (
          <>
            <Field label="Company usage" value={item["companyUsage"]} />
            {item["companyApplicable"] !== undefined ? (
              <Field
                label="Available to selected company"
                value={
                  item["companyApplicable"] ? "Yes" : "Assignment required"
                }
              />
            ) : null}
            <Field label="Acceptance" value={item["acceptance"]} />
          </>
        ) : null}
        {item["effectiveFrom"] ? (
          <Field
      ...
      {!sharedFactsOnly && rows(item["companyAssignments"]).length ? (
        <div className="bp360-company-acceptance">
          <h3>Company assignments</h3>
          {rows(item["companyAssignments"]).map((assignment, index) => (
            <dl
              key={`${assignment["companyCodeId"]}:${assignment["purpose"]}:${index}`}
            >
              ...
              <Field
                label="Company usage effective from"
                value={assignment["effectiveFrom"]}
              />
              {assignment["effectiveUntil"] ? (
                <Field
                  label="Company usage effective until"
                  value={assignment["effectiveUntil"]}
                />
              ) : null}
              <Field
                label="Preferred for purpose"
      ...
      {allowCompanySettings &&
      item["companyCodeId"] &&
      !String(item["linkId"]).startsWith("mesh:") ? (
        <details>
          <summary>Company usage settings</summary>
          <p>
            Availability does not approve this account for payment. Acceptance
            uses the bank verification workflow.
          </p>
          <form
            onSubmit={(event) => {
              ...
              setUsageBusy(true);
              void client
                .configureCompanyUsage(String(item["linkId"]), {
                  companyCodeId: String(item["companyCodeId"]),
                  purpose: String(fields.get("purpose")),
                  effectiveFrom: String(fields.get("effectiveFrom")),
                  ...(fields.get("effectiveUntil")
                    ? { effectiveUntil: String(fields.get("effectiveUntil")) }
                    : {}),
                  primary: fields.get("primary") === "on",
                })
                .then(() => {
                  setUsageSaved(true);
                  onChanged?.();
          ...
          >
            <label>
              Purpose <input name="purpose" defaultValue="default" required />
            </label>
            <label>
              Effective from{" "}
              <input
                name="effectiveFrom"
                type="date"
                defaultValue={String(item["effectiveFrom"] ?? "")}
                required
              />
            </label>
            <label>
              Effective until <input name="effectiveUntil" type="date" />
            </label>
            <label>
              <input name="primary" type="checkbox" /> Preferred for this
              purpose
            </label>
            <button type="submit" disabled={usageBusy}>
              Save company assignment
            </button>
            {usageError ? (
              <p role="alert">
                Company assignment could not be saved. Check dates, eligibility
                and permissions.
              </p>
            ) : null}
            {usageSaved ? (
              <p role="status">
                Company assignment saved. Payment acceptance is unchanged.
              </p>
            ) : null}
          </form>
        </details>
