...
const directoryVisibility = {
  ...
  organization_company: "Restricted to authorized organizations and companies",
};
interface Decision {
  eligible: boolean;
...
export function AccessScopeCard({
  ...
  const needsCompany =
    !requiredCoordinates || requiredCoordinates.includes("companyCodeId");
  const unsupported = requiredCoordinates?.some(
    (key) => !["companyCodeId", "operatingOrganizationId"].includes(key),
  );
  ...
  useEffect(() => {
    setOrganizationId(summary.scope.operatingOrganizationId ?? "");
    setCompanyId(summary.scope.companyCodeId ?? "");
  }, [
    summary.identity.id,
    summary.scope.operatingOrganizationId,
  ...
  const org = operating.organizations.find(
    ...
    company = work.companies.find((item) => item.companyCodeId === companyId);
  ...
  useEffect(() => {
    setDecisions({});
    if (!eligibilityOpen || !org || !company) return;
  ...
  }, [http, key, eligibilityOpen, org?.id, company?.companyCodeId]);
  return (
    <Card className="bp360-section-card">
      <dl className="bp360-fields"><div><dt>Directory visibility</dt><dd>{summary.directoryScope ? directoryVisibility[summary.directoryScope] : "Published directory policy"}</dd></div></dl>
      <h2>Transaction context</h2>
      <p>
        Choose an organization and company for transactions and setup requests.
      </p>
      <div className="bp360-fields">
        <label>
          Organization
          <Select
            aria-label="Transaction organization"
        ...
        <label>
          Company
          <Select
            aria-label="Transaction company"
            value={companyId}
            disabled={!org}
          ...
          >
            ...
            {work.companies
              .filter((item) =>
                org?.companyAssignments.some(
                  (assignment) =>
                    assignment.companyCodeId === item.companyCodeId,
                ),
              )
              .map((item) => (
                <option key={item.companyCodeId} value={item.companyCodeId}>
      ...
      {unsupported ? (
        <p role="status">
          This operation requires a context that cannot be selected here.
        </p>
      ) : null}
      {selectScope ? (
        <button
          ...
          onClick={() =>
            selectScope(org!.id, needsCompany ? company!.companyCodeId : "")
          }
        >
          Use this context
        </button>
      ...
      <details
        onToggle={(event) => setEligibilityOpen(event.currentTarget.open)}
      >
        <summary>Check transaction eligibility</summary>
        <label>
          Transaction{" "}
          <Select
            value={operation}
