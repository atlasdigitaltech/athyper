...
export function CustomerControls({businessPartnerId}:{readonly businessPartnerId:string}){
  ...
  async function createCredit(event:FormEvent<HTMLFormElement>){
    ...
    await run("credit",()=>api.createCustomerCredit(businessPartnerId,{customerId:customer.id,operatingOrganizationId:organizationId,companyCodeId,reviewTypeCode:String(data.get("reviewTypeCode")||"initial"),requestedCreditLimit:Number.isFinite(limit)?limit:undefined,requestedCurrencyCode:String(data.get("requestedCurrencyCode")||"").toUpperCase(),riskClassCode:String(data.get("riskClassCode")||"")}),"Credit review created");
  }
  async function decide(review:CustomerCreditReview,decision:"approved"|"conditional"|"rejected"){
    await run(`decision-${review.id}`,()=>api.decideCustomerCredit(review.id,{expectedVersion:review.rowVersion,decision,reason:`CUSTOMER_CREDIT_${decision.toUpperCase()}`,...(decision!=="rejected"&&review.requestedCreditLimit!==undefined&&review.requestedCurrencyCode?{approvedCreditLimit:review.requestedCreditLimit,approvedCurrencyCode:review.requestedCurrencyCode}:{})}),`Credit review ${decision}`);
  }
  ...
  return <PageSurface title="Customer credit and lifecycle" description="Credit approval is independent from registration approval; activation is pinned to current sales and AR readiness." actions={<a className="a-button a-button--secondary" href={`/mdg/business-partner/${encodeURIComponent(businessPartnerId)}`}>Back to partner</a>}>
