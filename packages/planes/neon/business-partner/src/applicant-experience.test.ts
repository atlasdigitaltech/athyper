...
describe("R3 restricted supplier applicant experience",()=>{
  ...
  it("projects only applicant-editable supplier identity values",()=>{expect(projectApplicantPayload({name:"Supplier",registrationNumber:"REG-1",internalWorkflow:{approver:"secret"},bankAccount:"secret"})).toEqual({name:"Supplier",registrationNumber:"REG-1"});});
});
