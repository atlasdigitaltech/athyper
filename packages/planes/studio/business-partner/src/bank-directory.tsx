...
export function BankDirectoryWorkspace() {
  ...
  return (
    <ManagementWorkspace header={<PageHeader level="collection" title="Bank directory" description="Governed bank institutions, branches and routing identifiers." icon={<Building2Icon/>} actions={canAuthor?<Button onClick={openImport}>Import bank data</Button>:undefined}/>} navigation={<ManagementNavigation label="Bank directory sections" currentKey={tab} items={[{key:"overview",label:"Overview",href:"?tab=overview"},{key:"manage",label:"Manage",href:"?tab=manage"},{key:"review",label:"Review & Approval",href:"?tab=review",count:revisions.filter(r=>!r.decision).length}]} onNavigate={navigate}/>}>
    <div className="bank-directory" aria-busy={busy}>
      <div className="bank-directory__sync" role="status"><span>{status?.expected?`Release ${status.expected.version} · ${status.planes.length===3&&status.planes.every(p=>p.state==="current")?"Up to date across all three planes":"Publication needs attention"}`:busy?"Checking publication status…":"No published release"}</span><Button variant="secondary" disabled={busy} onClick={()=>void run(refresh)}>Refresh status</Button></div>
