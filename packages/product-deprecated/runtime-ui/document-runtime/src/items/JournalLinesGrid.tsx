...
function ReferenceBadge({ references }: { references: unknown }) {
  ...
  return (
    <span className="inline-flex max-w-full items-center gap-1 rounded-full border border-border/70 bg-muted/40 px-2 py-0.5 text-xs text-muted-foreground">
          ...
          <div className="max-h-56 overflow-y-auto p-1">
            {filteredOptions.length === 0 ? (
              ...
              filteredOptions.map((option) => (
                ...
                >
                  <span className="w-8 shrink-0 tabular-nums font-medium">{option.code}</span>
...
function ReferenceSummaryCell({
  ...
  return (
    <div className="min-w-0" title={display.title}>
      ...
      {display.secondary && (
        <div className={cn(READONLY_SECONDARY_TEXT, "truncate tabular-nums leading-tight")}>
          {display.secondary}
        </div>
      )}
    </div>
      ...
      {requiresExchangeRate && (
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/40 bg-muted/20 px-3.5 py-2">
          <div className="min-w-0">
            ...
            <div className="text-xs text-muted-foreground">{transactionCurrency} to {baseCurrency}</div>
          </div>
          <Input
      ...
      <div className="flex justify-between items-center px-4 py-3 border-t border-border/40 bg-muted/20 text-xs flex-wrap gap-3">
        <div className="flex gap-6">
          <span>
            ...
            <span className="font-medium tabular-nums">{fmtDisplayMoney(totalDebit, transactionCurrency || currencyCode, amountScale, amountCurrencyPosition)}</span>
          </span>
          <span>
            <span className="text-muted-foreground mr-1.5">Total Credit</span>
            <span className="font-medium tabular-nums">{fmtDisplayMoney(totalCredit, transactionCurrency || currencyCode, amountScale, amountCurrencyPosition)}</span>
          </span>
        </div>
        <span className={cn(
          "text-xs font-medium",
      ...
      <div>
        ...
        <div className="flex justify-between items-center px-4 py-3 border-t border-border/40 bg-muted/20 text-xs flex-wrap gap-3">
          <div className="flex gap-6">
            <span>
              <span className="text-muted-foreground mr-1.5">Total Debit</span>
              <span className="font-medium tabular-nums">{fmtDisplayMoney(totalDebit, activeCurrencyCode, amountScale, amountCurrencyPosition)}</span>
            </span>
            <span>
              <span className="text-muted-foreground mr-1.5">Total Credit</span>
              <span className="font-medium tabular-nums">{fmtDisplayMoney(totalCredit, activeCurrencyCode, amountScale, amountCurrencyPosition)}</span>
            </span>
          </div>
          <span className={cn(
            "text-xs font-medium",
