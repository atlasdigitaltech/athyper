...
import { appEntityDetailHref, appEntityNewHref } from "@athyper/runtime-shared/core";
import {
    ...
    switch (action.handlerType) {
      case "NAVIGATE":
        if (action.handlerTarget) {
          ...
          router.push(appEntityNewHref(entityCode));
        } else if (action.permissionCode.endsWith(".edit") && recordId) {
          router.push(appEntityDetailHref(entityCode, recordId, "edit"));
        }
        break;
      case "API":
        void callApi(action);
