...
const FULL_WIDTH_FORM_UI_TYPES = new Set([
  "textarea", "rich_text", "json_editor", "markdown", "code_editor",
]);

function formGroupGridClass(columns: 1 | 2 | 3 | undefined): string {
  if (columns === 1) return "grid grid-cols-1 gap-4";
  if (columns === 3) return "grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3";
  return "grid grid-cols-1 gap-4 md:grid-cols-2";
}

function isFormFieldFullWidth(field: EntityField): boolean {
  const uiType = field.ui_type ?? field.data_type;
  if (BOOLEAN_FULL_WIDTH_UI_TYPES.has(uiType)) return true;
  if (FULL_WIDTH_FORM_UI_TYPES.has(uiType)) return true;
...
export const EntityForm = forwardRef<EntityFormHandle, EntityFormProps>(
  function EntityForm({
    ...
    const multiSection = formConfig.sections.length > 1;
    const formBody = (
        ...
        {formConfig.sections.map((section) => (
          <Card key={section.group.group_key}>
            ...
            <CardContent className={multiSection ? undefined : "pt-6"}>
              <div className={formGroupGridClass(section.group.columns as 1 | 2 | 3 | undefined)}>
                {section.fields.map((field) => {
                  ...
                  const isFullWidth = isFormFieldFullWidth(field);
                  return (
