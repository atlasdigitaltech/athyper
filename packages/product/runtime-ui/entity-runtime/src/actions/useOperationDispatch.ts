...
export function useOperationDispatch({
  ...
  const refreshRecordQueries = useCallback(async () => {
    ...
    await Promise.all([
      ...
      qc.invalidateQueries({ queryKey: queryKeys.entityOperations.byEntity(entityCode) }),
      qc.invalidateQueries({ queryKey: ["runtime-process-state", entityCode, recordId] }),
  ...
  const dispatch = useCallback(
    (actionCode: string, operations: EntityOperation[], opts?: { remarks?: string }): Promise<void> | void => {
      ...
      if (!op || (op.handler_type !== "MODAL" && op.handler_type !== "API")) return;

      // API operations POST directly to the action endpoint — no modal, no flow bundle.
      if (op.handler_type === "API") {
        const uuid = recordUuid;
        if (!uuid) return;
        return (async () => {
          setIsSubmitting(true);
          try {
            const idempotencyKey = getIdempotencyKey(uuid, actionCode);
            ...
            if (!res.ok) {
              clearIdempotencyKey(uuid, actionCode);
  ...
  const submitModal = useCallback(
    async (draft: Record<string, unknown>) => {
      ...
      try {
        ...
        const idempotencyKey = getIdempotencyKey(targetId, activeOpCode);
        const res = await fetch(
        ...
        if (!res.ok) {
          clearIdempotencyKey(targetId, activeOpCode);
          const body = await res.json().catch(() => ({})) as Record<string, unknown>;
    ...
    [activeOpCode, clearIdempotencyKey, entityCode, getIdempotencyKey, recordId, recordUuid, statusFieldName, applyStatusToDetailCache, refreshRecordQueries],
  );
