            ...
            <div key={`${node.node_type}-${i}`} className="flex items-center gap-1">
              ...
              {i > 0 && (
                <div className="flex flex-col items-center px-1">
                  ...
                  {node.fulfilled_pct != null && (
                    <span className="mt-0.5 text-xs text-muted-foreground">{node.fulfilled_pct}%</span>
                  )}
                </div>
              )}
              ...
              >
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-medium">{nodeShortLabel}</span>
                  {node.count > 1 && (
                    <Badge variant="outline" className="h-4 px-1 text-xs">×{node.count}</Badge>
                  )}
                </div>
                ...
                {node.has_exceptions && (
                  <span className="mt-0.5 text-xs text-destructive font-medium">⚠</span>
                )}
              </button>
            </div>
          );
