 ...
 *
 * Two modes:
 *   Standard  — pass `entityCode`. Fetches from runtime automatically.
...
import { resolveReferenceLabel } from "./reference-label-resolver";
...
export function resolveEntityPickerOptionConfig(referenceConfig: unknown): EntityPickerOptionConfig | undefined {
  ...
  return {
    valueField:       textConfig(picker?.["value_field"] ?? picker?.["valueField"] ?? config["value_field"] ?? config["valueField"]),
...
export const EntityPicker = forwardRef<HTMLDivElement, EntityPickerProps>(
  (
    ...
    const [resolvedDisplayLabel, setResolvedDisplayLabel] = useState<string | null>(null);
    ...
    useEffect(() => {
      if (!value || !entityCode) {
        setResolvedDisplayLabel(null);
        return;
      }
      const controller = new AbortController();
      void (async () => {
        const resolved = await resolveReferenceLabel({
          entity: entityCode,
          value,
          valueField: optionConfig?.valueField ?? "id",
          labelField: optionConfig?.labelField ?? "name",
          codeField: optionConfig?.codeField ?? "code",
        });
        if (resolved && !controller.signal.aborted) {
          const label = resolved.label?.trim() ?? "";
          const code = resolved.code?.trim() ?? "";
          setResolvedDisplayLabel(label && code && label.toLowerCase() !== code.toLowerCase()
            ? `${label} (${code})`
            : label || code || null);
        }
      })().catch(() => undefined);
      return () => controller.abort();
    }, [displayLabel, entityCode, optionConfig, value]);
    ...
    const handleValueChange = useCallback(
      (next: string | null) => {
        ...
        setResolvedDisplayLabel(selected ? selectedReferenceLabel(selected) : null);
    ...
    if (optionConfig?.variant === "advanced") {
      return (
        <AdvancedEntityCombobox
          ...
          id={id}
          value={value}
          displayLabel={resolvedDisplayLabel ?? displayLabel}
...
EntityPicker.displayName = "entity-picker";
