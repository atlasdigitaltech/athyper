 ...
 * Calls GET runtimePath.list(entityCode) + query string for filters/limit.
 *
...
import type { EntityPickerOption, EntityPickerOptionConfig } from "./entity-picker";
...
export function entityRowToPickerOption(
  ...
  const optionValue = firstText(row, [
    optionConfig?.valueField ?? undefined,
    "id",
  ]);
    ...
    (
      ...
      debounceRef.current = setTimeout(async () => {
        ...
        try {
          ...
          // page_size only — `limit` gets rewritten to `filter.limit` by the
          // BFF normaliser, which is a bogus column filter on entities like
          // cost_center/project/gl_account that have no `limit` column.
          ...
          if (!controller.signal.aborted) {
            // BFF route returns `records`; legacy backend-direct returns `data`.
            const rows = body.data ?? body.records ?? [];
            const nextOptions = rows.map((row) => entityRowToPickerOption(row, entityCode, optionConfig));
