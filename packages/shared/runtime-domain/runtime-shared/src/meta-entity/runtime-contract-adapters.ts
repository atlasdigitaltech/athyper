...
export function adaptField(field: MetaEntityField): EntityField {
  return {
    ...
    // P5 primary-field markers. MetaEntityField (the older runtime shape)
    // doesn't carry these yet; default to false. The compiled-entity route
    // returns the real values via EntityCompilerService.
...
export function adaptOperation(operation: MetaEntityOperation): EntityOperation {
  return {
    ...
    execution_target: operation.executionTarget ?? null,
