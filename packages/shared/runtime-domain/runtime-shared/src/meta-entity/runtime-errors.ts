...
function parseFieldErrors(body: Record<string, unknown>): Record<string, string> {
  ...
  // Source-change validation shape:
  //   { error: "FIELD_DEPENDENCY_*", fields: { <target>: { action, sources, reason, message } } }
  // See server/packages/services/records/source-change-validation.ts
  const fieldsMap = asRecord(body["fields"]);
  if (fieldsMap) {
    const out: Record<string, string> = {};
    for (const [field, detail] of Object.entries(fieldsMap)) {
      const detailRecord = asRecord(detail);
      const message = text(detailRecord?.["message"]) ?? text(detail);
