...
import type {
  DocumentEditSubmitRequestV1,
  DocumentEditSubmitResponseV1,
...
export interface DocumentEditDraftLoadCallback {
  (): Promise<DocumentWorkspaceDraftContext>;
}

export interface DocumentEditDraftSaveResult {
  type: "ok";
  response: DocumentEditSubmitResponseV1;
}

export interface DocumentEditDraftConflictResult {
  type: "conflict";
  currentEtag?: string;
  message?: string;
}

export interface DocumentEditDraftValidationResult {
  type: "validation";
  message?: string;
  fieldErrors: Record<string, string>;
}

export interface DocumentEditDraftNetworkErrorResult {
  type: "error";
  message: string;
}

export type DocumentEditDraftSaveOutcome =
  | DocumentEditDraftSaveResult
  | DocumentEditDraftConflictResult
  | DocumentEditDraftValidationResult
  | DocumentEditDraftNetworkErrorResult;

export interface DocumentEditDraftSaveCallback {
  (changes: DocumentEditSubmitRequestV1["changes"], etag: string): Promise<DocumentEditDraftSaveOutcome>;
}

export interface UseDocumentEditDraftOptions {
  /** Master switch. When false, the hook is a no-op (isEditing always false). */
  enabled?: boolean;
  /** Load the context returned by workspace OPEN. Called on `enterEdit()`. */
  loadWorkspaceContext: DocumentEditDraftLoadCallback;
  /** Submit bundled changes. Called on `save()`; transport is workspace SUBMIT. */
  submitWorkspaceChanges: DocumentEditDraftSaveCallback;
...
export interface UseDocumentEditDraftReturn {
  // ── Mode ──
  isEditing: boolean;
  ...
  /**
   * Refresh the workspace draft context (etag, masks, status) without
   ...
   * timer. Used while the caller is resolving a concurrency conflict.
   */
  pauseAutosave: () => void;
 ...
 *
 * The hook owns local workspace-draft state and delegates OPEN/SUBMIT transport
 ...
 *
 * @example
 * const draft = useDocumentEditDraft({
 *   enabled: isObjectPage,
 *   loadWorkspaceContext: () => openWorkspace(entityCode, recordId),
 *   submitWorkspaceChanges: (changes, etag) => submitWorkspace(entityCode, recordId, changes, etag),
 ...
 * )}
 */
export function useDocumentEditDraft(
  options: UseDocumentEditDraftOptions,
): UseDocumentEditDraftReturn {
  const { enabled = true, loadWorkspaceContext, submitWorkspaceChanges, onSaveSuccess, autosave } = options;
  ...
  // External pause/resume lets conflict recovery park autosave.
  ...
  const callbacksRef = useRef({ loadWorkspaceContext, submitWorkspaceChanges, onSaveSuccess });
  callbacksRef.current = { loadWorkspaceContext, submitWorkspaceChanges, onSaveSuccess };
  ...
  const enterEdit = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const ctx = await callbacksRef.current.loadWorkspaceContext();
      setEtag(ctx.etag);
  ...
  // changes when `save()` is called. Workspace SUBMIT executes the bundle
  ...
  const save = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const outcome = await callbacksRef.current.submitWorkspaceChanges(
        {
      ...
      if (outcome.type === "conflict") {
        ...
        if (retryEtag && retryEtag !== etag) {
          setEtag(retryEtag);
          const retry = await callbacksRef.current.submitWorkspaceChanges(
            {
              ...(headerPart ? { header: headerPart } : {}),
              ...(linesPart ? { lines: linesPart } : {}),
            },
          ...
          latestConflict = retry;
          // retry.type === "conflict" — fall through to the dialog
        }
        setSaveStatus("conflict");
        setSaveError(latestConflict.message ?? "Document changed on the server.");
        setConflictEtag(latestConflict.currentEtag ?? null);
        if (pauseOnError) autosavePausedRef.current = true;
        return false;
      }
