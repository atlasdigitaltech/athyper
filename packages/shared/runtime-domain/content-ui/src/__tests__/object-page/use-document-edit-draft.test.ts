...
import { useDocumentEditDraft } from "../../object-page/use-document-edit-draft";
import type {
...
import type {
  DocumentEditDraftSaveOutcome,
} from "../../object-page/use-document-edit-draft";

function makeContext(overrides: Partial<DocumentWorkspaceDraftContext> = {}): DocumentWorkspaceDraftContext {
  return {
    recordId:       "rec-1",
...
function makeOkResponse(overrides: Partial<DocumentEditSubmitResponseV1> = {}): DocumentEditSubmitResponseV1 {
  return {
    ok: true,
    intent: "save",
...
type SaveFn = (body: DocumentEditSubmitRequestV1["changes"], etag: string) => Promise<DocumentEditDraftSaveOutcome>;
type OnSaveSuccessFn = (response: DocumentEditSubmitResponseV1) => void;

function setup(overrides?: {
  context?: DocumentWorkspaceDraftContext;
  loadContextImpl?: () => Promise<DocumentWorkspaceDraftContext>;
  saveImpl?: SaveFn;
  ...
  const hook = renderHook(() => useDocumentEditDraft({
    enabled:       true,
    loadWorkspaceContext: loadContext,
    submitWorkspaceChanges: saveChanges,
    onSaveSuccess,
    autosave,
  }));
...
describe("use-document-edit-draft", () => {
  ...
  describe("save", () => {
    ...
    it("save() reuses the in-flight request when re-entered", async () => {
      let resolveSave: (value: DocumentEditDraftSaveOutcome) => void = () => {};
      const saveChanges = vi.fn<SaveFn>(() => new Promise((resolve) => {
        resolveSave = resolve;
      }));
      ...
      resolveSave({ type: "ok", response: makeOkResponse() });
      await act(async () => {
    ...
    it("save() reports the retry conflict etag when auto-recovery also conflicts", async () => {
      const saveChanges = vi.fn<SaveFn>()
        .mockResolvedValueOnce({ type: "conflict", currentEtag: "13", message: "First stale" })
        .mockResolvedValueOnce({ type: "conflict", currentEtag: "14", message: "Still stale" });
      const { hook } = setup({ saveImpl: saveChanges });
      await act(async () => { await hook.result.current.enterEdit(); });
      act(() => hook.result.current.setHeaderField("description", "Updated"));
      ...
      expect(saveChanges.mock.calls.map(([, etag]) => etag)).toEqual(["12", "13"]);
      expect(hook.result.current.saveStatus).toBe("conflict");
      expect(hook.result.current.conflictEtag).toBe("14");
      expect(hook.result.current.saveError).toBe("Still stale");
    });
  ...
  describe("Phase 10 #3 — autosave", () => {
    ...
    it("manual save() after failure clears pause and re-arms autosave", async () => {
      let nextResponse: DocumentEditDraftSaveOutcome = { type: "error", message: "first" };
