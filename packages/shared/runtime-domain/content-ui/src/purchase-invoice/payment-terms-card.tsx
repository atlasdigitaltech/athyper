        ...
        {application.clause_type === "DUE_DATE" ? (
          application.resolved_due_date ? (
            <span className="inline-flex items-center gap-1">
              ...
              {fmtDate(application.resolved_due_date)}
            </span>
          ) : (
      ...
      <div className="min-w-0 flex flex-col gap-0.5">
        ...
        <div className="text-[11px] text-muted-foreground">
          ...
          {result.qualification_date && (
            <span> · {fmtDate(result.qualification_date)}</span>
          )}
        </div>
      </div>
    ...
    <div className={cn("rounded-md border border-border bg-card", className)}>
      ...
      <div className="px-3 py-3 flex flex-col gap-2">
        ...
        <div className="text-xs text-muted-foreground flex items-center gap-3">
          {baselineDate && (
            <span>Baseline: {fmtDate(baselineDate)}</span>
          )}
          {dueDateApp?.resolved_due_date && (
            <span className="inline-flex items-center gap-1">
              <Calendar className="h-3 w-3" aria-hidden />
              Due: {fmtDate(dueDateApp.resolved_due_date)}
