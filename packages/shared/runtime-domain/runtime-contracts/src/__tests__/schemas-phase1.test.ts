...
describe("MetaEntityOperationSchema — selection actions", () => {
  it("accepts a metadata-driven contextual selection contract", () => {
...
function buildDescriptor(overrides: {
  ...
  relations: Array<{
    ...
    mutationOwner?: "direct_crud" | "workspace";
  }>;
}) {
  return {
...
describe("MetaEntityRuntimeDescriptorSchema — renderer invariants", () => {
  it("rejects a child surface whose mutation owner differs from its relation", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues.some((issue) => issue.message.includes("mutation owner"))).toBe(true);
    }
  });

  it("rejects a writable ledger descriptor", () => {
    const base = buildDescriptor({ operations: [], relations: [] });
    const result = MetaEntityRuntimeDescriptorSchema.safeParse({
      ...base,
      renderer: "ledger",
      capabilities: {
  ...
  it("rejects an editable descriptor without versioned concurrency", () => {
    const base = buildDescriptor({ operations: [], relations: [] });
    const result = MetaEntityRuntimeDescriptorSchema.safeParse({
      ...base,
    ...
    if (!result.success) {
      expect(result.error.issues.some((issue) =>
        issue.path.join(".") === "concurrency.strategy"
        && issue.message.includes("require version_only"),
      )).toBe(true);
    }
  });
});
