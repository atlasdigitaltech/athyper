...
import type { MetaEntityLineItemsSurface, MetaEntityOperation } from "@athyper/runtime-contracts";
...
export interface LineFieldChangeResolveInput {
  fieldName: string;
  newValue: unknown;
...
export interface LineFieldChangeResolveResult {
  accepted?: boolean;
  patch?: Record<string, unknown>;
  clearedFields?: string[];
}

export type LineFieldChangeResolver = (
  input: LineFieldChangeResolveInput,
) => Promise<void | LineFieldChangeResolveResult>;
...
export interface LineItemEditorProps extends LineItemSheetContext {
  ...
  onOpenChange:       (open: boolean) => void;
  line:               DocumentLine;
  /** Presentation mode for the shared existing-line drawer. */
  mode?:              "view" | "edit";
...
export interface LineItemPanel {
   ...
   */
  groupKeys: string[];
  /** Drawer modes in which this panel participates. Omit for all modes. */
  modes?: Array<LineItemPanelContext["mode"]>;
...
export interface LinesGridProps {
  ...
  distributions:   AccountingDistribution[];
  /** Pricing components used to derive the authoritative accounting cost target. */
  ...
  lineFieldChangeResolver?: LineFieldChangeResolver;
  /** Commit staged line updates through the document workspace SUBMIT lifecycle. */
  submitWorkspaceChanges?: DocumentWorkspaceLineSubmit;
   ...
   * expansion (default).
   */
  renderRowExpansion?: (line: LineRecord, componentLaunch?: LinePricingComponentLaunch) => ReactNode;
  /** Metadata operations eligible for the contextual line-selection bar. */
  selectionOperations?: ReadonlyArray<MetaEntityOperation>;
}

export type LinePricingComponentKind = "discount" | "charge" | "tax" | "withholding";

export interface LinePricingComponentLaunch {
  kind: LinePricingComponentKind;
  /** Changes for every user launch so an already-mounted row can react again. */
  requestId: number;
}
...
export interface DocumentWorkspaceLineSubmitInput {
  etag: string;
  lines: {
...
export interface DocumentWorkspaceLineSubmitResult {
  etag: string;
  record?: unknown;
}

export type DocumentWorkspaceLineSubmit = (
  input: DocumentWorkspaceLineSubmitInput,
) => Promise<DocumentWorkspaceLineSubmitResult>;
