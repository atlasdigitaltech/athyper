...
const FIELD_LABEL_CLASS = "text-sm font-semibold leading-normal text-muted-foreground";
...
function ReadonlyFieldValue({
  ...
  return (
    <span className={cn("text-base font-normal", text === "-" ? "text-muted-foreground/40" : "text-foreground")}>
      {text}
    </span>
  );
}
  ...
  return (
    <label className="flex min-w-0 flex-col gap-1.5">
      <span className={FIELD_LABEL_CLASS}>
        {fieldLabel(field)}
        {!isReadOnly && field.is_required && (
          <span className="ml-0.5 text-sm font-semibold text-destructive">*</span>
        )}
      </span>
      {isReadOnly ? (
        ...
        <MetaFieldInput
          field={field}
          value={value}
          onChange={(v) => onFieldChange ? onFieldChange(field.name, v) : onDraftChange({ ...draft, [field.name]: v })}
          disabled={disabled}
