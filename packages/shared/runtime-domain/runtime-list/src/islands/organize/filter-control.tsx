...
function AsyncOptionFilterEditor({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setMessage("");
    // Base href is the v1 entity root (`/api/runtime/v1/entities/<entity>`); append the
    // field-options sub-resource path. The adapter docs this contract in entityFieldOptionsApiHrefBase.
    fetch(`${trimTrailingSlash(fieldOptionsApiHrefBase)}/fields/${encodeURIComponent(field.name)}/options?${params.toString()}`, {
      cache: "no-store",
...
function DateFilterEditor({
  ...
  // Extract active preset key from the wire value so the sidebar chip shows
  // as selected. The wire format for a preset is "@<key>" — server resolves
  // to a date range at query time.
  ...
  // sidebar; the standalone chip row is retired.
  const presets = showShortcuts ? FISCAL_PRESETS : [];
