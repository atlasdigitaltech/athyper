// Phase E2 — D-E2.8 golden fixture invariant.
//
...
// (packages/shared/auth-bff/src/auth-pipeline.ts) call. Because both surfaces
// dispatch through this single function, "API ↔ BFF byte-identical" reduces
...
// MatrixMatch output. Any change to the algorithm — accidental or intentional
// — must be acknowledged by updating the fixture, which forces a reviewer to
...
// against drift on either side — if the runtime values change, this test
...
// `expected` field is the EXACT object the algorithm should produce — deep
// equality is checked.
...
const GOLDEN: readonly GoldenScenario[] = [
  // ─ baseline: no pending actions ────────────────────────────────────────────
  {
    label: "server / empty actions / mutating finance route → open",
    surface: "server",
    requiredActions: [],
  ...
  {
    label: "bff / empty actions / mutating workflow route → open",
    surface: "bff",
    requiredActions: [],
  ...
  // ─ no route info (gateway / verify endpoint posture) ──────────────────────
  {
    label: "server / VERIFY_EMAIL alone / no route → blocked on first",
    surface: "server",
  ...
  {
    label: "bff / two pending actions / no route → blocked on first",
    surface: "bff",
  ...
  // ─ reads stay open (so the user can see WHY they're blocked) ──────────────
  {
    label: "server / UPDATE_PASSWORD / GET /api/finance/x → open (read)",
    surface: "server",
  ...
  {
    label: "server / VERIFY_EMAIL / HEAD /api/ap/x → open (read)",
    surface: "server",
    requiredActions: ["VERIFY_EMAIL"],
  ...
  {
    label: "bff / VERIFY_EMAIL / OPTIONS /finance/x → open (preflight)",
    surface: "bff",
    requiredActions: ["VERIFY_EMAIL"],
  ...
  // ─ UPDATE_PASSWORD blocks everything mutating ─────────────────────────────
  {
    label: "server / UPDATE_PASSWORD / POST /api/records → blocked",
    surface: "server",
    requiredActions: ["UPDATE_PASSWORD"],
  ...
  {
    label: "bff / UPDATE_PASSWORD / DELETE / → blocked",
    surface: "bff",
    requiredActions: ["UPDATE_PASSWORD"],
  ...
  // ─ VERIFY_EMAIL: scope limited to finance + journal/invoice records ───────
  {
    label: "server / VERIFY_EMAIL / POST /api/records/journal_entry/x → blocked",
    surface: "server",
    requiredActions: ["VERIFY_EMAIL"],
  ...
  {
    label: "server / VERIFY_EMAIL / PATCH /api/records/customer/x → open",
    surface: "server",
    requiredActions: ["VERIFY_EMAIL"],
  ...
  {
    label: "bff / VERIFY_EMAIL / PUT /finance/post → blocked",
    surface: "bff",
    requiredActions: ["VERIFY_EMAIL"],
  ...
  {
    label: "bff / VERIFY_EMAIL / POST /settings/profile → open",
    surface: "bff",
    requiredActions: ["VERIFY_EMAIL"],
  ...
  // ─ CONFIGURE_TOTP: scope limited to workflow + iam grants/groups/roles ────
  {
    label: "server / CONFIGURE_TOTP / POST /api/iam/grants → blocked",
    surface: "server",
  ...
  {
    label: "server / CONFIGURE_TOTP / POST /api/iam/users → open (not in scope)",
    surface: "server",
    requiredActions: ["CONFIGURE_TOTP"],
  ...
  {
    label: "bff / CONFIGURE_TOTP / POST /settings/security → blocked (exact match)",
    surface: "bff",
    requiredActions: ["CONFIGURE_TOTP"],
  ...
  {
    label: "bff / CONFIGURE_TOTP / PATCH /settings/profile → open",
    surface: "bff",
    requiredActions: ["CONFIGURE_TOTP"],
  ...
  // ─ multiple pending: matches the first action whose prefix hits ───────────
  {
    label: "server / two pending / POST /api/finance/post → blocked on UPDATE_PASSWORD",
    surface: "server",
  ...
  {
    label: "server / two pending (reverse order) / POST /api/finance/post → blocked on VERIFY_EMAIL first",
    surface: "server",
  ...
  // ─ unknown action: ignored even when listed ───────────────────────────────
  {
    label: "server / unknown action / POST /api/finance/x → open",
    surface: "server",
  ...
  {
    label: "bff / unknown + known action / POST /workflow/x → blocked on the known",
    surface: "bff",
  ...
  // ─ method casing: lowercase mutating must still block ─────────────────────
  {
    label: "server / UPDATE_PASSWORD / lowercase 'post' /api/x → blocked",
    surface: "server",
    requiredActions: ["UPDATE_PASSWORD"],
...
describe("matchRequiredActions — golden fixture (D-E2.8)", () => {
