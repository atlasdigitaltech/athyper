// packages/shared/auth-common/src/required-actions.ts
//
// Phase G — Shared required-actions matrix algorithm.
//
// Both the server runtime pipeline (server/src/auth/auth-pipeline.ts) and
// the BFF session pipeline (packages/shared/auth-bff/src/auth-pipeline.ts)
// implemented their own copy of the prefix-matching algorithm. This module
// is the single source of truth — surface-specific wrappers translate the
...
/**
 * Result of a matrix lookup. Generic shape — callers map to per-surface
...
export interface MatrixMatchInput {
  ...
  /** Matrix: action code → list of path prefixes that block on it. */
 ...
 *
 * Semantics (locked):
 *   - Empty requiredActions ⇒ never blocked.
 *   - No route ⇒ ANY pending action blocks (most conservative; matches
 *     /api/auth/verify and validatePlaneServerSession behaviour).
 *   - Non-mutating method (GET / HEAD / OPTIONS) ⇒ never blocked. Reads
 *     stay open so users can see WHY they're blocked.
 *   - Mutating method ⇒ blocked iff requiredActions ∩ matrix.keys yields
 ...
 * `required_actions` (snake_case — KC standard) and `requiredActions`
 * (camelCase — legacy tooling). Snake_case wins when both are present.
 */
...
// ─── Matrix loader (env JSON with safe default fallback) ─────────────────────

/** Minimal env-bag shape — kept node-agnostic so auth-common stays portable. */
 ...
 * (AUTH_BFF_REQUIRED_ACTIONS_MATRIX) — the per-surface module passes its
...
export function loadMatrixFromEnv(
  ...
  try {
    ...
    // bad JSON falls back to the surface default — the auth-flag validator
