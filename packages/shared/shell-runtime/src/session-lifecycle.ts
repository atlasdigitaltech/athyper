...
export function usePlaneSessionLifecycle(plane: PlaneKey): {
  ...
  const [sessionPolicy, setSessionPolicy] = useState<SessionPolicyDefaults>(SESSION_POLICY_DEFAULTS);
  ...
  const touchSession = useCallback(async (force = false): Promise<boolean> => {
    const now = Date.now();
    if (!force && now - lastHeartbeatAtRef.current < sessionPolicy.heartbeatIntervalMs) {
      return true;
    }
  ...
  }, [plane, sessionPolicy.heartbeatIntervalMs]);
  ...
  }, [logoutNow, scheduleTokenRefresh]);
