/**
 * @athyper/content-ui — AccountingDistributionPanel (Band 4)
 *
 * Spec v1.1 §4.3 Band 4 + §6.5 (balance reconciler) + acceptance §A6.
 *
 * Renders accounting_distribution rows scoped to one PIL. Each row
 * surfaces account_source as a "resolved via X" tag and exposes a
 * drill-in to accounting_distribution_resolution_audit when present.
 *
 * Empty state (per §A6): when no AD rows in draft, this panel
 * displays a policy-driven placeholder ("Will resolve by policy at
 * posting") with an optional hint (FROM_CATEGORY / FROM_INTENT) when
 * the line has resolution hints set. Per UI-P6, the panel stays
 * VISIBLE because AD is required for posting — not hidden.
 *
 * Balance reconciler footer compares sum(distributed_amount) to
 * line gross. Submit must be gated on `balanced` status by callers.
 */
"use client";

import { CheckCircle2, AlertCircle, Info, Plus, FileSearch } from "lucide-react";
import { cn } from "@athyper/theme/utils";
import { resolveSemanticColors } from "@athyper/theme/semantic-colors";
import { CurrencyTriad } from "../money/CurrencyTriad";
import type {
  AccountingDistribution,
  AdAccountSource,
  EditAffordance,
} from "../../purchase-invoice/types";

export interface AccountingDistributionPanelProps {
  /** AD rows for this line. */
  distributions: AccountingDistribution[];
  /** Line gross — the balance target. */
  lineGrossAmount: number;
  /** Line currency triad. */
  currencyCode: string;
  baseCurrencyCode: string;
  exchangeRate: number;

  /** Resolved edit gate. */
  affordance: EditAffordance;

  /** Add a new split. */
  onAdd?: () => void;
  /** Edit a split (draft + approval pre-post). */
  onEdit?: (adId: string) => void;
  /** Delete a split (draft only). */
  onDelete?: (adId: string) => void;
  /** Open the per-row resolution audit sheet. */
  onViewAudit?: (adId: string) => void;

  /**
   * Optional policy-driven placeholder hint when distributions is empty.
   * Examples: "Commodity: Office Supplies", "Intent: Capital Acquisition".
   * When omitted, renders "Will resolve by policy at posting".
   */
  emptyResolutionHint?: string;
  /**
   * When emptyResolutionHint is known, name the resolution path
   * (e.g. "FROM_CATEGORY", "FROM_INTENT"). Surfaces inline.
   */
  emptyResolutionPath?: AdAccountSource;

  className?: string;
}

const SOURCE_LABEL: Record<AdAccountSource, string> = {
  POSTING_ROLE:  "POSTING_ROLE",
  FIXED:         "FIXED",
  FROM_INTENT:   "FROM_INTENT",
  FROM_CATEGORY: "FROM_CATEGORY",
};

// ── Balance state derivation ───────────────────────────────────────

type BalanceState =
  | { kind: "balanced"; sum: number }
  | { kind: "short"; sum: number; delta: number }
  | { kind: "over"; sum: number; delta: number };

function computeBalance(
  distributions: AccountingDistribution[],
  lineGrossAmount: number,
  tolerance: number = 0.005,
): BalanceState {
  const sum = distributions.reduce((acc, d) => acc + d.distributed_amount, 0);
  const delta = sum - lineGrossAmount;
  if (Math.abs(delta) <= tolerance) return { kind: "balanced", sum };
  if (delta < 0) return { kind: "short", sum, delta: -delta };
  return { kind: "over", sum, delta };
}

// ── Balance footer ─────────────────────────────────────────────────

function BalanceFooter({
  balance,
  lineGrossAmount,
  currencyCode,
  baseCurrencyCode,
  exchangeRate,
}: {
  balance: BalanceState;
  lineGrossAmount: number;
  currencyCode: string;
  baseCurrencyCode: string;
  exchangeRate: number;
}) {
  if (balance.kind === "balanced") {
    const colors = resolveSemanticColors("success");
    const percentage = lineGrossAmount === 0 ? 100 : (balance.sum / lineGrossAmount) * 100;
    return (
      <div className={cn(
        "flex items-center justify-between gap-3 px-3 py-2 border-t border-border bg-muted/40",
      )}>
        <div className="text-xs text-muted-foreground">
          AD TOTAL <span className="ml-1 font-medium text-foreground">
            <CurrencyTriad
              amount={balance.sum}
              currencyCode={currencyCode}
              baseCurrencyCode={baseCurrencyCode}
              exchangeRate={exchangeRate}
            />
          </span>
          <span className="ml-2 tabular-nums">· {percentage.toFixed(2)}%</span>
        </div>
        <div className={cn("inline-flex items-center gap-1 text-xs font-medium", colors.text)}>
          <CheckCircle2 className="h-3 w-3" aria-hidden /> balanced to gross
        </div>
      </div>
    );
  }

  const colors = resolveSemanticColors("error");
  const label = balance.kind === "short" ? "short by" : "over by";

  return (
    <div className="flex items-center justify-between gap-3 px-3 py-2 border-t border-border bg-muted/40">
      <div className="text-xs text-muted-foreground">
        AD TOTAL <span className="ml-1 font-medium text-foreground">
          <CurrencyTriad
            amount={balance.sum}
            currencyCode={currencyCode}
            baseCurrencyCode={baseCurrencyCode}
            exchangeRate={exchangeRate}
          />
        </span>
      </div>
      <div className={cn("inline-flex items-center gap-1 text-xs font-medium", colors.text)}>
        <AlertCircle className="h-3 w-3" aria-hidden />
        {label}{" "}
        <CurrencyTriad
          amount={balance.delta}
          currencyCode={currencyCode}
          baseCurrencyCode={baseCurrencyCode}
          exchangeRate={exchangeRate}
        />
        <span className="ml-1">· cannot submit</span>
      </div>
    </div>
  );
}

// ── Resolution audit chip ──────────────────────────────────────────

function ResolutionChip({
  source,
  auditCount,
  onClick,
}: {
  source: AdAccountSource;
  auditCount: number;
  onClick?: () => void;
}) {
  const colors = resolveSemanticColors("info");

  const content = (
    <>
      <span className="font-medium">{SOURCE_LABEL[source]}</span>
      {auditCount > 0 && (
        <span className="text-muted-foreground">· {auditCount} audit</span>
      )}
    </>
  );

  const sharedClass = cn(
    "inline-flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-[11px]",
    colors.subtleBadge,
  );

  if (onClick && auditCount > 0) {
    return (
      <button
        type="button"
        onClick={onClick}
        className={cn(sharedClass, "transition-opacity hover:opacity-85")}
        aria-label={`Account resolved via ${SOURCE_LABEL[source]}, ${auditCount} audit records. View details.`}
      >
        <FileSearch className="h-3 w-3" aria-hidden />
        {content}
      </button>
    );
  }

  return <span className={sharedClass}>{content}</span>;
}

// ── One AD row ─────────────────────────────────────────────────────

function DistributionRow({
  distribution,
  currencyCode,
  baseCurrencyCode,
  exchangeRate,
  lineGrossAmount,
  affordance,
  onEdit,
  onDelete,
  onViewAudit,
}: {
  distribution: AccountingDistribution;
  currencyCode: string;
  baseCurrencyCode: string;
  exchangeRate: number;
  lineGrossAmount: number;
  affordance: EditAffordance;
  onEdit?: (id: string) => void;
  onDelete?: (id: string) => void;
  onViewAudit?: (id: string) => void;
}) {
  const showEdit = affordance === "edit" || affordance === "replace";
  const showDelete = affordance === "edit";

  // Derive % from distributed_amount if PERCENT basis is set, else compute.
  const pct = distribution.split_pct
    ?? (lineGrossAmount === 0 ? 0 : (distribution.distributed_amount / lineGrossAmount) * 100);

  return (
    <div className="grid grid-cols-[2rem_1fr_auto_auto_auto_auto_auto] items-center gap-3 px-3 py-2 text-sm border-b border-border/40 last:border-b-0">
      <div className="text-xs tabular-nums text-muted-foreground">#{distribution.distribution_no}</div>

      <div className="min-w-0 flex flex-col gap-0.5">
        <div className="font-medium truncate">
          {distribution.gl_account_label ?? <span className="italic text-muted-foreground">unresolved</span>}
        </div>
        <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
          {distribution.cost_center_label && <span>CC: {distribution.cost_center_label}</span>}
          {distribution.profit_center_label && <span>PC: {distribution.profit_center_label}</span>}
          {distribution.project_label && <span>Proj: {distribution.project_label}</span>}
          {distribution.is_capex && <span className="font-medium">CapEx</span>}
        </div>
      </div>

      {/* Resolution source */}
      <div>
        <ResolutionChip
          source={distribution.account_source}
          auditCount={distribution.resolution_audit_count}
          onClick={onViewAudit ? () => onViewAudit(distribution.id) : undefined}
        />
      </div>

      {/* % */}
      <div className="text-xs tabular-nums text-muted-foreground">{pct.toFixed(2)}%</div>

      {/* Amount */}
      <div className="text-sm tabular-nums font-medium text-right">
        <CurrencyTriad
          amount={distribution.distributed_amount}
          currencyCode={currencyCode}
          baseCurrencyCode={baseCurrencyCode}
          exchangeRate={exchangeRate}
        />
      </div>

      {/* Edit */}
      <div>
        {showEdit && onEdit && (
          <button
            type="button"
            onClick={() => onEdit(distribution.id)}
            className="text-[11px] font-medium text-muted-foreground hover:text-foreground rounded-md border border-border px-2 py-0.5"
          >
            Edit
          </button>
        )}
      </div>

      {/* Delete */}
      <div>
        {showDelete && onDelete && (
          <button
            type="button"
            onClick={() => onDelete(distribution.id)}
            className="text-[11px] font-medium text-muted-foreground hover:text-foreground rounded-md border border-border px-2 py-0.5"
          >
            Delete
          </button>
        )}
      </div>
    </div>
  );
}

// ── Empty-state placeholder ────────────────────────────────────────

function EmptyPlaceholder({
  affordance,
  emptyResolutionHint,
  emptyResolutionPath,
  onAdd,
}: {
  affordance: EditAffordance;
  emptyResolutionHint?: string;
  emptyResolutionPath?: AdAccountSource;
  onAdd?: () => void;
}) {
  const pathLabel = emptyResolutionPath
    ? `via ${SOURCE_LABEL[emptyResolutionPath]}`
    : "by policy";

  return (
    <div className="flex flex-col gap-3 px-4 py-6 text-sm">
      <div className="flex items-start gap-2 text-muted-foreground">
        <Info className="h-4 w-4 mt-0.5 shrink-0" aria-hidden />
        <div className="flex flex-col gap-1">
          <div>
            No distribution rows. Will resolve {pathLabel} at posting.
          </div>
          {emptyResolutionHint && (
            <div className="text-xs italic">{emptyResolutionHint}</div>
          )}
        </div>
      </div>
      {affordance === "edit" && onAdd && (
        <div>
          <button
            type="button"
            onClick={onAdd}
            className={cn(
              "inline-flex items-center gap-1 rounded-md border border-border px-2 py-1 text-xs font-medium",
              "text-muted-foreground hover:text-foreground hover:bg-muted/60",
            )}
          >
            <Plus className="h-3 w-3" aria-hidden /> Add split manually
          </button>
        </div>
      )}
    </div>
  );
}

// ── Main component ─────────────────────────────────────────────────

export function AccountingDistributionPanel({
  distributions,
  lineGrossAmount,
  currencyCode,
  baseCurrencyCode,
  exchangeRate,
  affordance,
  onAdd,
  onEdit,
  onDelete,
  onViewAudit,
  emptyResolutionHint,
  emptyResolutionPath,
  className,
}: AccountingDistributionPanelProps) {
  // Per §A6: keep band visible when AD is required-but-unresolved.
  if (distributions.length === 0) {
    return (
      <div className={cn("rounded-md border border-border bg-card", className)}>
        <div className="grid grid-cols-[2rem_1fr_auto_auto_auto_auto_auto] items-center gap-3 px-3 py-2 text-xs font-medium text-muted-foreground border-b border-border bg-muted/40">
          <div>#</div>
          <div>GL Account</div>
          <div>Source</div>
          <div className="text-right">%</div>
          <div className="text-right">Amount</div>
          <div />
          <div />
        </div>
        <EmptyPlaceholder
          affordance={affordance}
          emptyResolutionHint={emptyResolutionHint}
          emptyResolutionPath={emptyResolutionPath}
          onAdd={onAdd}
        />
      </div>
    );
  }

  const sorted = [...distributions].sort((a, b) => a.distribution_no - b.distribution_no);
  const balance = computeBalance(distributions, lineGrossAmount);

  return (
    <div className={cn("rounded-md border border-border bg-card", className)}>
      {/* Header row */}
      <div className="grid grid-cols-[2rem_1fr_auto_auto_auto_auto_auto] items-center gap-3 px-3 py-2 text-xs font-medium text-muted-foreground border-b border-border bg-muted/40">
        <div>#</div>
        <div>GL Account</div>
        <div>Source</div>
        <div className="text-right">%</div>
        <div className="text-right">Amount</div>
        <div />
        <div />
      </div>

      {/* Rows */}
      <div>
        {sorted.map((distribution) => (
          <DistributionRow
            key={distribution.id}
            distribution={distribution}
            currencyCode={currencyCode}
            baseCurrencyCode={baseCurrencyCode}
            exchangeRate={exchangeRate}
            lineGrossAmount={lineGrossAmount}
            affordance={affordance}
            onEdit={onEdit}
            onDelete={onDelete}
            onViewAudit={onViewAudit}
          />
        ))}
      </div>

      {/* Add split affordance */}
      {affordance === "edit" && onAdd && (
        <div className="px-3 py-2 border-t border-border/40">
          <button
            type="button"
            onClick={onAdd}
            className={cn(
              "inline-flex items-center gap-1 rounded-md border border-border px-2 py-1 text-xs font-medium",
              "text-muted-foreground hover:text-foreground hover:bg-muted/60",
            )}
          >
            <Plus className="h-3 w-3" aria-hidden /> Add split
          </button>
        </div>
      )}

      {/* Balance reconciler footer */}
      <BalanceFooter
        balance={balance}
        lineGrossAmount={lineGrossAmount}
        currencyCode={currencyCode}
        baseCurrencyCode={baseCurrencyCode}
        exchangeRate={exchangeRate}
      />
    </div>
  );
}
