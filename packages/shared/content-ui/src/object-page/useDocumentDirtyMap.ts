...
export interface UseDocumentDirtyMapOptions {
  ...
  /** Pending header patch — e.g. `editSession.pendingHeaderPatch`. */
  pendingPatch: Record<string, unknown>;
  /** Field-level validation errors — e.g. `editSession.fieldErrors`. */
...
/**
 * Partitions a document-level edit session's dirty + error state by section.
 *
 ...
 * `Record` shapes that match `useDocumentEditSession`'s public surface. Used
