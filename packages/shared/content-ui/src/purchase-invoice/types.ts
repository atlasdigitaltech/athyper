 ...
 *      • Display: hydrated *_label fields, resolution_audit_count,
...
/** pricing_component.account_source (mirrors ad enum). */
export type PcAccountSource = "POSTING_ROLE" | "FIXED" | "FROM_INTENT" | "FROM_CATEGORY";
...
export type AdAccountSource = "POSTING_ROLE" | "FIXED" | "FROM_INTENT" | "FROM_CATEGORY";
 ...
 *
 * `resolution_audit_count` surfaces the per-row drill-in count to
 * accounting_distribution_resolution_audit. Click drills into the
 * audit sheet to answer "why this account?".
 */
export interface AccountingDistribution {
  ...
  /** Count of audit records for drill-in. */
...
export interface PurchaseInvoiceHeader {
  ...
  /** Display label (from invoice_party_snapshot post-submit, supplier master pre-submit). */
