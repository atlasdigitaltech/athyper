...
export function parseFilterSigil(raw: string): FilterEntry {
  ...
  if (raw.startsWith("between:")) {
    const parts = raw.slice("between:".length).split(",");
    if (parts.length === 2 && parts[0] !== undefined && parts[1] !== undefined) {
      return { op: "between", value: [coerceNumOrStr(parts[0]), coerceNumOrStr(parts[1])] };
    }
    // Malformed between: (missing second bound) — discard rather than misinterpret
    // as a comma-separated string array, which would produce a silent wrong filter.
    return [];
  }
...
export function serializeFilterEntry(entry: FilterEntry): string {
  ...
  switch (entry.op) {
    ...
    default: {
      // TypeScript exhaustiveness: `entry` is `never` here.
      // If reached at runtime (e.g. a cast from `unknown`), return "" so the
      // param is silently dropped by the `if (sigil)` guard in the client.
