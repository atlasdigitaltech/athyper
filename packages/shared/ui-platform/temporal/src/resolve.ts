/**
 * resolveTemporalKind â€” metadata-first resolver with strict inference fallback.
 *
 * Order of precedence:
 *   1. field.temporalKind (explicit) â†’ wins, no inference attempted.
 *   2. Inference by data_type:
 *        date         â†’ businessDate
 *        timestamptz  â†’ instant
 *        timestamp    â†’ ERROR (must be tagged explicitly; audit script enforces)
 *        anything else â†’ undefined (not a temporal field)
 *
...
import type { MetaEntityField, TemporalKind, TemporalDisplayMode } from "@athyper/runtime-contracts";
...
export type TemporalResolution =
  ...
  | {
      status: "unresolved";
      reason: "ambiguous_timestamp" | "unknown_kind_string";
      dataType: string;
    };

/**
 * Picker contract: pass a MetaEntityField (from runtime-contracts) and get
 * back either a resolved kind + display mode, or "unresolved" â€” caller must
 * not silently fall back to a default (that's how time gets dropped from
 * datetime fields). Surface unresolved as a hard error in dev, audit in CI.
 */
export function resolveTemporalKind(field: MetaEntityFieldLike): TemporalResolution {
  ...
  if (dt === "timestamptz" || dt === "datetime" || dt === "timestamp") {
    ...
    //   'timestamptz' â€” Postgres TIMESTAMPTZ (canonical UTC storage)
    //   'timestamp'   â€” pre-normalisation literal used in seed VALUES blocks;
    //                   045_control_entity_field_data_type_normalization_contract.sql maps it
    //                   to 'datetime' at the end of the seed phase
    //   'datetime'    â€” post-normalisation form
...
export interface MetaEntityFieldLike {
  dataType: MetaEntityField["dataType"];
