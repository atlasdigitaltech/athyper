...
/**
 * Shared identity-and-navigation header for section, collection, record, and
 * workspace surfaces. Product packages provide semantic content while this
 * component owns the visual anatomy and responsive behavior.
 */
export function SurfaceHeader({
  ...
  const rootClassName = [
    "w-full min-w-0 overflow-hidden rounded-xl border bg-card shadow-sm",
    className,
  ].filter(Boolean).join(" ");

  return (
    <header className={rootClassName} data-surface-header-kind={kind}>
      <div className="flex min-h-[76px] flex-col gap-4 px-4 py-4 md:flex-row md:items-center md:justify-between">
        <div className="flex min-w-0 items-center gap-3">
          {back && (
            ...
            >
              <span aria-hidden>{back.icon ?? "←"}</span>
              ...
              <dl className="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2">
                {facts.map((fact) => (
                  <div key={fact.key} className="flex min-w-0 items-center gap-1.5 text-sm">
                    {fact.icon && <span className="shrink-0 text-muted-foreground" aria-hidden>{fact.icon}</span>}
