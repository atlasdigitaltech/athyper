// packages/shared/auth-bff/src/auth-pipeline.ts
//
// Phase B + Phase G — BFF mirror of server/src/auth/auth-pipeline.ts.
//
...
// matrix. The MATCHING ALGORITHM lives in @athyper/auth-common — this module
...
// ─── BFF matrix (intentionally distinct from the API matrix) ─────────────────

/**
 * Default matrix for the BFF surface — paths are relative to the app's BFF
...
// ─── Step: required-actions enforcement ──────────────────────────────────────
...
// ─── Step: AUTHORIZED role check (against cached access-token claims) ────────
...
export interface EnforceSessionPipelineInput {
   ...
   */
  readonly resourceAccess?: unknown;
  /** `${planeKey}-web` — derived by the caller from session.plane. */
  readonly clientId?: string;
 ...
 * (no route info → any pending action blocks) and by individual route handlers
