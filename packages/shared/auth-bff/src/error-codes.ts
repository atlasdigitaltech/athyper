// packages/shared/auth-bff/src/error-codes.ts
//
// Phase A — Single source of truth for the auth-failure contract.
//
...
// response — and historically this mapping was duplicated per app, drifting
...
export interface AuthFailurePresentation {
  ...
  /** Short, user-friendly message. No correlation ID — those go in `detail`. */
   ...
   * Lets observability correlate UI symptom → server log via requestId.
   */
...
// ─── The table ───────────────────────────────────────────────────────────────
//
// Order: most common first → least common. New codes go at the bottom.
...
const RAW_PRESENTATIONS = {
  ...
  // Authorization (claim/context mismatches) ─────────────────────────────────
 ...
 * Fatal severities don't render as toasts — callers should detect and handle.
 */
...
// ─── Aggregate view (telemetry / CLI introspection) ──────────────────────────
