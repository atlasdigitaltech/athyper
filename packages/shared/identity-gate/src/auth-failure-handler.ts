// packages/shared/identity-gate/src/auth-failure-handler.ts
//
// Phase A — Auth failure dispatcher.
//
...
export interface AuthFailureDispatcherDeps {
   ...
   * the request id (if known) so dashboards can join UI symptom → server log.
   */
...
/**
 * Adapt a fetch Response → outcome. Use this in the fetch interceptor to
...
export async function dispatchAuthFailureFromResponse(
  ...
  try {
    ...
    // non-JSON body — no auth code available
  }
