...
// packages/shared/identity-gate/src/AuthFailureBridge.tsx
//
// Phase A — Glue component mounted inside <ToastProvider> + react-query
...
//
//   - Recognized AuthFailureCode → toast / auto-navigate per the contract
//   - Unknown code → no-op (the host's existing error boundary handles it)
//
...
export interface AuthFailureBridgeProps {
  ...
  /**
   * Toast emitter — typically `useToast().toast` adapted to the dispatcher's
  ...
  /**
   * Router push fn — usually `useRouter().push` from `next/navigation`.
