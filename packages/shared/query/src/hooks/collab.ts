...
/**
 * @athyper/query — Collaboration hooks
 *
 * Ported from F1 lib/collab/* (SWR → TanStack Query).
 * All hooks call the BFF `/api/collab/` routes directly.
 * No typed API client — the BFF is the boundary.
 */
...
export function useTimeline(options?: UseTimelineOptions) {
  ...
  const query = useQuery<TimelinePage>({
    queryKey: queryKeys.collab.timeline(options?.entityType, options?.entityId),
    queryFn: ({ signal }) =>
      collabGet<TimelinePage>(`/api/collab/timeline?${params}`, signal),
    enabled,
    staleTime: 30 * 1000,
  });
...
export function useReactions(commentId: string | null) {
  ...
  const query = useQuery<ReactionsResponse>({
    queryKey: qk,
    queryFn: ({ signal }) =>
      collabGet<ReactionsResponse>(`/api/collab/comments/${commentId}/reactions`, signal),
    enabled: !!commentId,
    staleTime: 30 * 1000,
  });
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ reactionType }: { reactionType: string }) =>
      collabMutate(`/api/collab/comments/${commentId}/reactions`, "POST", { reactionType }),
      ...
      if (prev) {
        ...
        const optimistic: ReactionSummary[] = existing?.reacted
          ...
          : [
              ...
              { reactionType, emoji: existing?.emoji ?? reactionType, count: (existing?.count ?? 0) + 1, reacted: true },
            ];
        queryClient.setQueryData<ReactionsResponse>(qk, { ok: true, data: optimistic });
      }
  ...
  return {
    ...
    isLoading: query.isLoading,
    error: query.error,
    toggleReaction: (reactionType: string) => toggleMutation.mutate({ reactionType }),
  };
}
...
export function useCommentActions(entityType: string, entityId: string) {
  ...
  const replyMutation = useMutation({
    ...
    onSuccess: (_data, { parentId }) => {
      queryClient.invalidateQueries({ queryKey: listKey });
      queryClient.invalidateQueries({ queryKey: queryKeys.collab.replies(parentId) });
    },
  });
...
export function useCollabUnreadCount(
  ...
  const query = useQuery<UnreadCountResponse>({
    queryKey: qk,
    queryFn: ({ signal }) =>
      collabGet<UnreadCountResponse>(`/api/collab/comments/unread-count?${params}`, signal),
    enabled,
    staleTime: 30 * 1000,
