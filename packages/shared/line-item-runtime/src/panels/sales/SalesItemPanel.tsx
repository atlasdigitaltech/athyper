...
export function SalesItemPanel({
  ...
  const { itemFields, qtyPriceFields, pricingFields } = useMemo(() => {
    ...
    const rawItem = fieldsForGroups(entity, ["item", "product", "service"]).filter(isEditableLineField);
    const rawPricing = fieldsForGroups(entity, ["financial", "pricing"]).filter(isEditableLineField);
