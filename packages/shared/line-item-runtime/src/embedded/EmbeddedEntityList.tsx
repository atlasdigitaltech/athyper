...
/**
 * @athyper/line-item-runtime — EmbeddedEntityList
 *
 ...
 *
 * Lives in `@athyper/line-item-runtime` for now because the line-items
...
export function EmbeddedEntityList({
  ...
  const [sort, setSort] = useState<EntityListSortEntry[] | undefined>(initialSort);
  ...
  const { data: entity, isLoading: descriptorLoading } = useCompiledEntity(entityCode);
  ...
  const columns = useMemo(
    () => columnsOverride ?? (entity
      ...
      : []),
    [columnsOverride, entity, currencyCode, visibleColumnKeys, disableHeaderSort],
  );
