...
function CellToken({
  ...
  if (!cell) {
    return (
      <span className="inline-flex h-6 min-w-9 items-center justify-center rounded-md border border-dashed bg-muted/40 px-1 text-[10px] font-semibold text-muted-foreground">
        {emptyCellLabel}
      </span>
    );
  }
  ...
  return (
    <span
      className={cn(
        "inline-flex min-h-7 min-w-7 items-center justify-center rounded-md border px-1.5 text-[11px] font-semibold leading-none",
        toneClass(cell.tone),
      )}
      title={cell.title}
    >
...
function LegendToken({ item }: { item: WorkbenchMatrixLegendItem }) {
  return (
    <span className="inline-flex items-center gap-1.5">
      <span
        className={cn(
          "inline-flex size-5 items-center justify-center rounded border px-1 text-[10px] font-semibold leading-none",
          toneClass(item.tone),
        )}
      >
        {item.marker ?? ""}
    ...
    <section className={cn("flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border bg-card", className)}>
      ...
      <div className="min-h-0 flex-1 overflow-auto bg-card">
        <table className="min-w-full border-separate border-spacing-0 text-sm">
          <thead>
            <tr>
              <th className="sticky left-0 top-0 z-40 w-64 min-w-64 border-b border-r bg-muted px-3 py-2 text-left text-xs font-semibold text-muted-foreground shadow-sm">
                {rowHeaderLabel}
              </th>
              {columns.map((column) => {
              ...
              <th className="sticky right-0 top-0 z-40 w-20 min-w-20 border-b border-l bg-muted px-2 py-2 text-center text-xs font-semibold text-muted-foreground shadow-sm">
                {coverageHeaderLabel}
              </th>
            </tr>
          </thead>
            ...
            {rows.length === 0 ? (
              ...
              return (
                <tr key={row.key} className={cn(rowSelected && "bg-primary/5")}>
                  ...
                  <td className="sticky right-0 z-10 w-20 min-w-20 border-b bg-card px-2 py-2 text-center align-middle text-sm font-semibold text-foreground">
          ...
          {columns.length > 0 && rows.length > 0 && (
            <tfoot>
              <tr>
                <th className="sticky bottom-0 left-0 z-40 border-r border-t bg-muted px-3 py-2 text-left text-xs font-semibold text-muted-foreground shadow-[0_-1px_0_hsl(var(--border))]">
                  Column coverage
                </th>
                {columns.map((column) => (
                  <td
                    key={column.key}
                    className={cn(
                      "sticky bottom-0 z-30 border-t bg-muted px-2 py-2 text-center text-sm font-semibold text-foreground shadow-[0_-1px_0_hsl(var(--border))]",
                ...
                <td className="sticky bottom-0 right-0 z-40 border-l border-t bg-muted px-2 py-2 text-center text-sm font-semibold text-foreground shadow-[0_-1px_0_hsl(var(--border))]">
