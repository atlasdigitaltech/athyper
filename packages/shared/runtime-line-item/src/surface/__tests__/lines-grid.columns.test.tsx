...
function buildProps(over: Partial<LinesGridProps> = {}): LinesGridProps {
  return {
    ...
    record: { id: "inv-1" },
    lines: [
      { id: "line-1", line_no: 1, item_description: "Q2 Consulting", quantity: 5, uom_code: "EA", net_amount: 600 } as unknown as LineRecord,
    ],
    distributions: [],
    isLoading: false,
    onRefresh: vi.fn(),
...
describe("LinesGrid — columns from display_config.list_columns via the embedded path", () => {
  ...
  it("renders quantity as plain text with muted UOM in view mode", () => {
    useCompiledEntityMetadataMock.mockReturnValue(PI_LINE_ENTITY);
    useCompiledEntityMock.mockReturnValue({ data: PI_LINE_ENTITY, isLoading: false });
    useEntityListMock.mockReturnValue({ data: undefined, isLoading: false });

    const { container } = renderGrid(buildProps({ editMode: false }));
    const quantityCell = container.querySelector('[title="5 EA"]');

    expect(quantityCell).not.toBeNull();
    expect(quantityCell?.className).toContain("items-baseline");
    expect(quantityCell?.className).not.toContain("h-7");
  });

  it("renders quantity as plain text in edit mode too because the grid cell is display-only", () => {
    useCompiledEntityMetadataMock.mockReturnValue(PI_LINE_ENTITY);
    useCompiledEntityMock.mockReturnValue({ data: PI_LINE_ENTITY, isLoading: false });
    useEntityListMock.mockReturnValue({ data: undefined, isLoading: false });

    const { container } = renderGrid(buildProps({ editMode: true }));
    const quantityCell = container.querySelector('[title="5 EA"]');
  ...
  it("toolbar still renders while the descriptor is loading (null) and the grid does not crash", () => {
    ...
    expect(screen.getByText(/Lines \(0\)/i)).toBeInTheDocument();
...
describe("LinesGrid — column visibility menu (descriptor-driven, persisted)", () => {
  ...
  it("toggling a hidden field then clicking Discard leaves the grid unchanged", async () => {
    ...
    expect(screen.queryByRole("columnheader", { name: /^Type/ })).toBeNull();
    expect(window.localStorage.getItem("line-grid:purchase_invoice_line:columns:v4-authoring-grid")).toBeNull();
  });
  ...
  it("Apply persists the user choice to localStorage under a line-entity-keyed slot", async () => {
    ...
    await waitFor(() => {
      const raw = window.localStorage.getItem("line-grid:purchase_invoice_line:columns:v4-authoring-grid");
