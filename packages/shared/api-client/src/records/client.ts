...
export function createRecordsClient(fetch: ApiFetch) {
  return {
    async list(
      ...
      }>(`/api/records/${encodePathSegment(entityCode)}${query}`);
    },

    async get(entityCode: string, id: string): Promise<MasterRecord> {
      return fetch(`/api/records/${encodePathSegment(entityCode)}/${encodePathSegment(id)}`);
    },

    async create(entityCode: string, body: MasterRecordWrite): Promise<MasterRecord> {
      return fetch(`/api/records/${encodePathSegment(entityCode)}`, { method: "POST", body: JSON.stringify(body) });
    },

    async update(entityCode: string, id: string, body: MasterRecordWrite): Promise<MasterRecord> {
      return fetch(`/api/records/${encodePathSegment(entityCode)}/${encodePathSegment(id)}`, { method: "PUT", body: JSON.stringify(body) });
    },

    async remove(entityCode: string, id: string): Promise<void> {
      return fetch(`/api/records/${encodePathSegment(entityCode)}/${encodePathSegment(id)}`, { method: "DELETE" });
    },
    ...
    async bulkPreflight(
      ...
      return fetch(`/api/records/${encodePathSegment(entityCode)}/bulk-preflight`, {
        method: "POST",
    ...
    async bulkAction(
      ...
      return fetch(`/api/records/${encodePathSegment(entityCode)}/bulk-action`, {
        method: "POST",
        body: JSON.stringify(body),
      });
    },
