...
function FactCell({ fact }: { fact: HeaderFact }) {
  ...
  return (
    <div className={cn("flex flex-col gap-0.5 min-w-0 shrink-0", fact.xl && "pl-2 border-l border-border")}>
      <span className="text-2xs font-medium text-muted-foreground uppercase tracking-wider whitespace-nowrap">
        {fact.label}
      </span>
      <span className={cn(
        "whitespace-nowrap leading-tight",
        fact.xl ? "text-base font-semibold" : "text-sm font-medium",
        fact.valueType === "code" && "font-mono",
