...
function EnumFilter({
  ...
  return (
    <div className="space-y-1">
      {facetValues.length > 10 && (
        <div className="relative mb-1">
          ...
          <input
            ...
            className="h-6 w-full rounded border border-input bg-background pl-6 pr-2 text-2xs focus:outline-none focus:ring-1 focus:ring-ring"
          />
          {valueSearch && (
            <button
      ...
      {facetValues.length === 0 && (
        <p className="text-2xs text-muted-foreground italic px-1">No values available</p>
      )}
      {facetValues.length > 0 && displayValues.length === 0 && valueSearch && (
        <p className="text-2xs text-muted-foreground italic px-1">No values match "{valueSearch}"</p>
      )}
    </div>
  );
}
...
function DateFilter({
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-1">
        <p className="text-2xs text-muted-foreground">Custom range</p>
      ...
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {selected.map((id) => (
            <span
              key={id}
              className="inline-flex items-center gap-1 rounded-full border border-primary/30 bg-primary/8 px-2 py-0.5 text-2xs font-medium"
            >
          ...
          {results.map(({ id, label }) => {
            ...
            return (
              ...
              >
                ...
                <span className="font-mono text-2xs text-muted-foreground/50">{id.slice(0, 6)}</span>
              </button>
            );
          })}
        </div>
