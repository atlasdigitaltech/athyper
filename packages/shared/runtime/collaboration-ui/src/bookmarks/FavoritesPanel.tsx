...
function GroupHeader({ label, count }: { label: string; count: number }) {
  return (
    <div className="section-label flex h-7 items-center justify-between px-3">
      <span className="truncate">{label}</span>
      <span className="ml-2 rounded-full border bg-muted/40 px-1.5 py-0.5 text-doc-badge font-semibold tabular-nums text-foreground/70">
        {count}
      </span>
    </div>
  );
    ...
    <div className={cn(
      ...
      >
        <span className="min-w-0">
          ...
          {subtitle && (
            <span className="block truncate text-doc-support font-medium text-muted-foreground">
              {subtitle}
            </span>
          )}
          {tertiary && (
            <span className="block truncate text-doc-support text-muted-foreground/80">
              {tertiary}
            </span>
          )}
        </span>
      ...
      {tab === "recent" && (
        <div className="flex h-9 shrink-0 items-center gap-1 border-b px-3">
          {([
            ...
            >
              <span>{label}</span>
              <span className="rounded-full bg-background/80 px-1.5 text-doc-badge font-semibold tabular-nums text-muted-foreground">
                {count}
              </span>
            </button>
          ))}
