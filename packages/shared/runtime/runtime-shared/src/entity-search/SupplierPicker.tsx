...
/**
 * SupplierPicker — reusable supplier search picker.
 *
 ...
 * for every field whose target_entity = 'supplier'.
 *
 * Stored value is the supplier UUID (proper FK reference).
 * Defaults to the "active" tab so inactive/suspended suppliers stay out of view
...
export interface SupplierPickerProps {
  /** Current stored value (supplier UUID). */
  value?: string | null;
  ...
  /**
   * EntityField metadata for the supplier reference field.
   * Drives variant, tabs, badges. Falls back to bare "supplier" search when omitted.
   */
  field?: SupplierFieldMeta | null;
  /**
...
function resolveTargetEntity(field?: SupplierFieldMeta | null): string {
  const t = field?.reference_config?.["target_entity"];
  return typeof t === "string" && t.trim() ? t.trim() : "supplier";
}
...
export function SupplierPicker({
  ...
  return (
    <EntityPicker
      ...
      optionActionLabel={optionConfig?.optionActionLabel ?? "Open supplier"}
      loadOnOpen
      placeholder={placeholder ?? `Search ${field?.label ?? "supplier"}…`}
      disabled={disabled}
      error={error}
      clearable
      className={className}
