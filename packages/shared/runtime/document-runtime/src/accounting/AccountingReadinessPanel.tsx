  ...
  return (
    <div className={SUMMARY_BOX}>
      ...
      <div className="mt-2 flex items-center gap-1.5">
        ...
        <span className={cn("truncate text-sm font-semibold", status === "warning" ? "text-warning" : "text-foreground")}>
          {value}
        </span>
      </div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border/60 pb-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="text-base font-semibold text-foreground">Accounting Readiness</h2>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border/50">
              {packet.components.map((component) => (
                <tr key={component.id} className={component.componentType === "net_payable" ? "bg-muted/20 font-semibold" : undefined}>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-muted/30 text-xs font-semibold text-muted-foreground">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-left">Category / Intent</th>
                      ...
                      {item.distributions.length === 0 ? (
                        ...
                        <tr key={dist.id}>
                          ...
                          <td className="px-3 py-2 text-xs text-muted-foreground">
                            {dist.commodity_category_id ? shortId(dist.commodity_category_id) : "category ?"}
                            <MoveRight className="mx-1 inline h-3 w-3" />
                            {dist.business_intent_id ? shortId(dist.business_intent_id) : "intent ?"}
                          </td>
        ...
        <div className="overflow-hidden rounded-lg border border-border/70">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border/50">
              {packet.previewLines.map((line) => (
                <tr key={line.seq}>
                  ...
                  <td className={cn("px-3 py-2 text-center font-semibold", line.side === "Dr" ? "text-info" : "text-destructive")}>{line.side}</td>
            ...
            <tfoot className="border-t border-border/70 bg-muted/20 text-xs">
              <tr>
                <td colSpan={3} className="px-3 py-2 font-semibold">Projected balance</td>
              ...
              >
                ...
                {item.actionLabel && (
                  <button
                    ...
                    className="shrink-0 text-xs font-semibold text-info transition-colors hover:text-foreground"
                  >
                    {item.actionLabel}
                  </button>
                )}
