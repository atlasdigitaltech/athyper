...
describe("Phase 3 — capability reason fields", () => {
  ...
  it("uses the compiled deletion mode and lifecycle flags for UI capabilities", () => {
    const descriptor = compileMetaEntityRuntimeDescriptor(
      baseEntity({
        feature_flags: {},
        capability_manifest: {
          deletionMode: "retire",
          mutation: { delete: { enabled: true, kind: "generic" } },
          lifecycle: {
            states: {
              active: { isEditable: true, isCommitted: false, isTerminal: false, isDeletable: true, isReversible: false },
              retired: { isEditable: false, isCommitted: false, isTerminal: true, isDeletable: false, isReversible: false },
            },
          },
        },
      }),
      {
        operations: [{
          id: "op-delete", entity_name: "supplier", permission_code: "delete",
          surface: "DETAIL", placement: "OVERFLOW", handler_type: "MODAL",
          handler_target: "delete", is_record_required: true, sort_order: 30, is_enabled: true,
        }],
      },
    );
    expect(descriptor.capabilities.canDelete).toBe(true);
    expect(descriptor.lifecycleStateMasks).toEqual(expect.arrayContaining([
      expect.objectContaining({ recordStatus: "active", canEdit: true, canDelete: true }),
      expect.objectContaining({ recordStatus: "retired", canEdit: false, canDelete: false }),
    ]));
  });
