 ...
 * ColumnPickerBase
 *
 * Router-free, state-callback column visibility picker. Owns the entire
 * drawer UI — trigger button, search, Visible/Hidden sections, drag-to-
 * reorder, Apply/Reset/Discard — but doesn't know how the apply result
 * gets persisted.
 *
 * Two consumers wrap it differently:
 *   - `ColumnControl` (this directory) → wraps with `useRouter` + URL state
 *     for production list pages.
 *   - `LinesGrid` (in `@athyper/runtime-line-item`) → wraps with
 *     `useLocalStoragePreference` for the embedded line items grid.
 *
 * Panel open/close state is controlled via `open` + `onOpenChange` props
 * so URL-mode callers can drive it from `useOrganizePanel` while
 * embedded-mode callers can drive it from `useState`. The base never
 * mounts `useOrganizePanel` itself.
 */
...
import { ReorderActionsBar, ReorderHandle } from "./reorder-handle";
import {
...
export interface ColumnPickerBaseProps {
  /** Currently visible columns, in display order. */
  columns:        ResolvedColumn[];
  /** Full candidate pool. */
  allColumns:     ResolvedColumn[];
  /** Default visible columns (e.g. descriptor's list_columns). */
  defaultColumns: ResolvedColumn[];
  /** When false (or `allColumns` is empty), the picker renders nothing. */
  enabled:        boolean;
  /** Controlled open state. */
  open:           boolean;
  onOpenChange:   (next: boolean) => void;
  /**
   * Apply handler. Called with the draft's ordered visible-column names
   * when the user clicks Apply. The caller decides how to persist
   * (URL params, localStorage, etc.).
   */
  onApply:        (visible: string[]) => void;
  /**
   *   - `"button"` (default): render a `PaletteButton` trigger.
   *   - `"hidden"`: render a hidden anchor only — the parent supplies its
   *     own trigger and drives `open`.
   */
  trigger?:       "button" | "hidden";
  /**
   * Trigger button size, forwarded to `PaletteButton`. Embedded consumers
   * (e.g. the line items grid in `@athyper/runtime-line-item`) should pass
   * `"compact"` to fit a tighter parent toolbar; the default `"default"`
   * matches the entity-list page header chrome.
   */
  triggerSize?:   "default" | "compact";
  /**
   * Trigger button chrome, forwarded to `PaletteButton`. Use `"plain"` in
   * dense toolbars where the default border + active highlight would
   * compete with neighbouring affordances (search input, etc.).
   */
  triggerChrome?: "default" | "plain";
  /**
   * Backdrop behind the drawer, forwarded to `PaletteDrawer`. Embedded
   * consumers (line items grid in a document page) typically pass
   * `"transparent"` so the parent document chrome underneath stays at
   * full clarity instead of being dimmed.
   */
  backdrop?:      "dim" | "transparent";
  /**
   * When false, the trigger button omits the hidden-column count badge.
   * Embedded consumers in dense toolbars often suppress this — the count
   * is already surfaced inside the drawer header ("List Columns N/M"),
   * and the trigger's `active` state alone is enough to signal "some
   * columns are hidden." Default `true` preserves the entity-list page
   * look.
   */
  showHiddenCount?: boolean;
}

export function ColumnPickerBase({
  columns,
  allColumns,
  defaultColumns,
  enabled,
  ...
  // Re-seed draft each time the drawer opens so stale edits don't bleed
  // across open/close cycles. Same semantics as the original ColumnControl.
  useEffect(() => {
    if (open) {
    ...
    <>
      {trigger === "button" ? (
        <PaletteButton
          ...
          badge={showHiddenCount ? hiddenCount : undefined}
          expanded={open}
          size={triggerSize}
          chrome={triggerChrome}
