...
export interface RuntimeEntityIdentityBarProps {
  ...
  statusDimensions?: HeaderStatusDimension[];
  editMode?: boolean;
  /**
   * Suppresses the built-in "EDITING" badge when true. Used by callers that
   * mount their own mode indicator (e.g. the View|Edit pill in the document
   * workspace's actionLeadingSlot), so the same signal isn't shown twice.
   * Defaults to false to preserve the classic-form path.
   */
...
export function RuntimeEntityIdentityBar({
  ...
  // Primary row = the canonical identifier (code). Copy-to-clipboard hangs off
  // here because the code is what callers paste into tickets / search.
  const codePrimary = canCopy ? (
    <button
      type="button"
  ...
  // Secondary row = the human-readable name; only when it differs from the code
  // (records without a separate name collapse to a single-row identity).
  const nameSecondary = hasDistinctName ? (
    <span
      title={identity.name}
      className="block min-w-0 max-w-full truncate text-sm font-medium leading-tight text-muted-foreground"
    >
      {identity.name}
    </span>
  ) : null;
  ...
  const statusBadge = (
    <span className={cn("inline-flex shrink-0 items-center rounded-sm border px-2.5 py-1 text-[11px] font-semibold uppercase leading-none tracking-wide", statusIntentClasses(identity.status.intent))}>
      {identity.status.label}
    </span>
  );

  const renderStatusDimensionBadges = () => statusDimensions.map((dimension) => (
    <span
      key={dimension.id}
      title={dimension.label}
      className={cn("inline-flex shrink-0 items-center rounded-sm border px-2.5 py-1 text-[11px] font-semibold uppercase leading-none tracking-wide", statusIntentClasses(dimension.intent))}
    >
      {dimension.value}
    </span>
  ));

  const editingBadge = editMode && !hideEditingBadge ? (
    <span className="inline-flex shrink-0 items-center gap-1 rounded-sm border border-primary/40 bg-primary/10 px-2.5 py-1 text-[11px] font-semibold uppercase leading-none tracking-wide text-primary">
  ...
  return (
    <div className={cn(headerIdentityPaddingClass, className)}>
      ...
      <div className="flex flex-col gap-1.5 sm:hidden">
        ...
        <div className="flex min-w-0 flex-wrap items-center gap-1.5 px-0.5 leading-none">
          {codePrimary}
          {classificationText && (
            <>
              <span aria-hidden className="h-1 w-1 shrink-0 rounded-full bg-border" />
              <span className="min-w-0 truncate text-xs text-muted-foreground">{classificationText}</span>
            </>
          )}
          {identity.version && (
            <>
