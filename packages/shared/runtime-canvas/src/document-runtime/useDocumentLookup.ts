 ...
 * (`/api/document-runtime/lookup/<lookup_code>`) resolves the code
  ...
  const query: UseQueryResult<ReadonlyArray<RuntimeRecordRow>, Error> = useQuery({
    ...
    queryFn:  async () => {
      ...
      const url = params.toString().length > 0
        ? `/api/document-runtime/lookup/${encodeURIComponent(lookupCode)}?${params.toString()}`
        : `/api/document-runtime/lookup/${encodeURIComponent(lookupCode)}`;
