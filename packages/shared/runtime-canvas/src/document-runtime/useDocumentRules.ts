...
/**
 * Projection shape returned by /api/document-runtime/<entity>/rules.
 *
...
export function useDocumentRules(opts: UseDocumentRulesOptions): DocumentRulesResult {
  const { entityCode, enabled = true, staleTimeMs = 5 * 60_000 } = opts;

  const query: UseQueryResult<DocumentRuleSet, Error> = useQuery({
    queryKey: ["doc-rules", entityCode],
    queryFn:  async () => {
      const res = await fetch(
        `/api/document-runtime/${encodeURIComponent(entityCode)}/rules`,
        { cache: "no-store" },
      );
      if (!res.ok) {
