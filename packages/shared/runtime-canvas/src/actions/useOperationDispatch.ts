...
export function useOperationDispatch({
  ...
  const postAction = useCallback(async (
    ...
    const response = await fetch(
      `/api/relay/api/records/${encodeURIComponent(entityCode)}/${encodeURIComponent(targetId)}/action/${encodeURIComponent(actionCode)}`,
      {
        method: "POST",
        headers: {
  ...
  const dispatch = useCallback(
    (actionCode: string, operations: EntityOperation[], opts?: { remarks?: string }): Promise<void> | void => {
      ...
      if (operation.handler_type === "API") {
        ...
        return (async () => {
          ...
          try {
            await postAction(actionCode, uuid, { remarks: opts?.remarks });
          } finally {
    ...
    [entityCode, isSubmitting, postAction, recordUuid, userPermissions],
  );
