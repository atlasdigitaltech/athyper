...
import { toEntityEditState } from "../record/document-edit-session-adapter";

/**
 * Builds a stub `UseDocumentEditSessionReturn` with the four properties the
 ...
 */
function stubSession(over: Partial<UseDocumentEditSessionReturn> = {}): UseDocumentEditSessionReturn {
  const base: Partial<UseDocumentEditSessionReturn> = {
    isEditing: true,
  ...
  return { ...base, ...over } as UseDocumentEditSessionReturn;
}
