 ...
 *
 * Stored value defaults to the account CODE (matching the field-registry
 * convention for `gl_account` references). Surfaces that bind by UUID
 * (e.g. `document.accounting_distribution.gl_account_id`) pass
 * `valueKind="id"` to switch the picker to emitting the row id instead.
 *
 * The CoA scoping filter is extracted from formData by searching common
 * paths for chart_of_account_id.
 *
...
import { readLookupFilters, searchLookupOptions } from "./lookup-config";
import type { EntityPickerOption, EntityPickerSearchContext } from "./entity-picker";
import type { LookupFilterValue } from "./lookup-config";
...
export interface GlAccountPickerProps {
  /** Current stored value (account code, or UUID when valueKind="id"). */
  value?: string | null;
  /** Display label shown when the picker is closed. */
  displayLabel?: string | null;
  /** Called on selection / clear with (value, label). */
  onChange: (value: string | null, label: string | null) => void;
  /**
   * Which field on the gl_account row is used as the stored value.
   * - "code" (default) — emits account code; matches the historical
   *   `gl_account` reference-field convention.
   * - "id" — emits the row's UUID; required for FK columns like
   *   `accounting_distribution.gl_account_id`.
   */
  valueKind?: "code" | "id";
  /**
...
export function GlAccountPicker({
  ...
  // valueKind="id" forces the picker to emit UUIDs regardless of the
  // field's metadata default. This is the right shape for FK columns
  // (e.g. accounting_distribution.gl_account_id).
  const valueField = useMemo(
    () => valueKind === "id" ? "id" : resolveValueField(field, optionConfig),
    [valueKind, field, optionConfig],
  );
