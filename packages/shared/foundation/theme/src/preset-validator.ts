...
// ── CLI Runner ──────────────────────────────────────────────────
// Run with: tsx src/preset-validator.ts --check-shipped

if (typeof process !== "undefined" && process.argv?.includes("--check-shipped")) {
  void (async () => {
    const fs = await import("node:fs");
    const path = await import("node:path");
    const presetsDir = path.join(import.meta.dirname ?? ".", "presets");

    const files = fs.readdirSync(presetsDir).filter((f: string) => f.endsWith(".css"));
    let allValid = true;

    console.log(`Validating ${files.length} presets against contract v${THEME_CONTRACT_VERSION}\n`);

    for (const file of files) {
      const css = fs.readFileSync(path.join(presetsDir, file), "utf-8");
