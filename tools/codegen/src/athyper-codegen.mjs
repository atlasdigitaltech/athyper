...
const CONTRACTS_GEN_DIR = path.join(
  REPO_ROOT,
  "packages",
  "contracts",
  "src",
...
async function writeEntryPoints() {
  ...
  await ensureDir(path.join(genRoot, "prisma", "kysely"));
  ...
  // Contracts side re-export (Kysely types only)
...
if (!WATCH) {
  ...
  const schemaPath = path.join(DB_DIR, "src", "prisma", "schema.prisma");
