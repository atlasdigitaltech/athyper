...
export interface CompiledRule {
  ...
  assignTo:   { type: string; value?: string };
}
  ...
  async compileTemplate(
    ...
    for (const r of ruleRows) {
      ...
      rulesByStage.get(stageId)!.push({
        ...
        assignTo:   r["assign_to"] as { type: string; value?: string } ?? { type: "manual" },
      });
    }

    const stages: CompiledStage[] = stageRows.map((s) => ({
      stageNo:         s["stage_no"] as number,
      name:            s["name"] as string | null,
      mode:            (s["mode"] as "serial" | "parallel") ?? "serial",
      quorum:          s["quorum"] as { strategy: string; required?: number } | null,
      slaPolicyId:     s["sla_policy_id"] as string | null,
      templateStageId: s["id"] as string,
    ...
    for (const rule of compiledStage.rules) {
      ...
      await this.db
        ...
        .values({
          ...
          assigned_to:        rule.assignTo.value,
          item_type:          "approval",
          status:             "pending",
...
function diffTemplates(
  ...
  const aStages = new Set(a.stages.map((s) => `stage_${s.stageNo}`));
  const bStages = new Set(b.stages.map((s) => `stage_${s.stageNo}`));
  ...
  for (const s of a.stages) {
    const bStage = b.stages.find((x) => x.stageNo === s.stageNo);
    if (bStage && JSON.stringify(s) !== JSON.stringify(bStage)) {
      changed.push(`stage_${s.stageNo}`);
    }
  }
