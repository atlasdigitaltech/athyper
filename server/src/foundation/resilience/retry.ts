 ...
 *
 * Ported from @athyper-F1/framework/core/src/resilience/retry.ts
 */
...
/** Suitable for Kysely / PostgreSQL pool errors (connection reset, timeout). */
...
export async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= cfg.maxAttempts; attempt++) {
    try {
      ...
      // Don't retry if the error is not classified as retryable
      if (cfg.retryableError && !cfg.retryableError(lastErr)) throw lastErr;

      // Don't sleep after the last attempt — just throw
...
export function isTransientDbError(err: Error): boolean {
  ...
  // PG error codes: 08* = connection errors, 53* = resource/pool exhaustion
