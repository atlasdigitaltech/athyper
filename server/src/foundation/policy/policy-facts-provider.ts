 ...
 * framework/runtime/services/policy/engine.ts).
 *
...
export class PolicyFactsProvider {
  ...
  async getEntityFacts(entityType: string, entityId: string): Promise<EntityFacts | null> {
    // Resolve entity state from entity_lifecycle_instance if lifecycle-enabled
    const liRow = await this.db
      .selectFrom("control.entity_lifecycle_instance as eli" as never)
      .select(["eli.current_state", "eli.metadata"] as never[])
      .where("eli.entity_name" as never, "=", entityType as never)
      .where("eli.entity_id" as never, "=", entityId as never)
      .where("eli.tenant_id" as never, "=", this.session.tenantId as never)
