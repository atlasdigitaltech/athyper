 ...
 *
 * Targets the self-hosted Healthchecks service at HEALTHCHECKS_BASE_URL.
 ...
 * a Healthchecks check per slug they want to monitor; unmatched pings
 * are silently dropped server-side (no client-side coordination needed).
 *
 * Design notes:
 *   - No-op when HEALTHCHECKS_BASE_URL is unset (returns immediately).
...
const BASE_URL = process.env.HEALTHCHECKS_BASE_URL?.trim().replace(/\/$/, "") ?? "";
