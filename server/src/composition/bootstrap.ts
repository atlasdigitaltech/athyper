// server/src/composition/bootstrap.ts
//
...
import {
  ...
  recordAuthFlagPostureWarning,
  recordTenantStampSkipped,
} from "../runtimes/metrics.js";
import { applyAuthFlagPostureValidation } from "@athyper/svc-iam";
import { runWithJobContext, tryGetContext } from "@athyper/server-foundation/context";
...
} from "@athyper/adapter-communications";
import {
  AtlasThreadService,
...
import { CircuitBreaker } from "@athyper/server-foundation/kernel/resilience";
...
import { ServiceRegistry, type HealthCheck, type HealthContribution } from "@athyper/server-foundation/observability";
import { createFeatureFlagService, createNotificationOrchestrator } from "@athyper/svc-platform";
import { createMetadataApprovalBridge, createEntityCompilerService } from "@athyper/svc-metadata";
...
import { invalidateDescriptorCache } from "@athyper/svc-metadata";
...
import { RenderDocumentService } from "@athyper/plane-neon-document-rendering";
import { createHttpConnectorClient, createOAuth2TokenCache } from "@athyper/svc-integration";
import { createMentionService } from "@athyper/svc-collab";
...
import { createLogger } from "@athyper/server-foundation/observability";
import { Lifecycle } from "@athyper/server-foundation/kernel";
import {
  createConsoleAuditWriter,
  createLifecycleAuditWriter,
  type AuditWriter,
} from "@athyper/svc-audit";
...
function databaseAuthorityIdentity(connectionString: string): string {
  try {
...
export async function bootstrap(
  ...
  if (config.env !== "local") {
    audit = createLifecycleAuditWriter(_db, { plane: config.env }) as unknown as AuditWriter;
  }
  ...
  // Atlas retention is the only cross-tenant persistence path. It is enabled
  // solely in scheduler/worker runtimes and uses an independently configured
  // athyperadmin connection. A missing, reused, unreachable, or wrong-role
  // connection leaves the purge scheduler and worker inactive.
  const atlasPersistence = config.atlasAgent?.persistence;
  let atlasConversationPurge:
    import("@athyper/svc-jobs").JobsServiceDeps["atlasConversationPurge"];
  let atlasToolInvocationRecovery:
    import("@athyper/svc-jobs").JobsServiceDeps[
      "atlasToolInvocationRecovery"
    ];
  if (
    atlasPersistence?.enabled
    && (runtimeMode === "scheduler" || runtimeMode === "worker")
  ) {
    const maintenanceDatabaseUrl = atlasPersistence.maintenanceDatabaseUrl;
    if (
      !maintenanceDatabaseUrl
      || databaseAuthorityIdentity(maintenanceDatabaseUrl)
        === databaseAuthorityIdentity(config.db.url)
    ) {
      logger.error("atlas_conversation_maintenance_authority_rejected", {
        reason: !maintenanceDatabaseUrl
          ? "maintenance_database_url_missing"
          : "application_database_authority_reused",
        runtimeMode,
      });
    } else {
      ...
      try {
        const identity = await maintenanceDb.getPool().query<{
          role_name: string;
          atlas_maintenance: boolean;
          application_role: boolean;
        }>(
          "SELECT current_user AS role_name, "
          + "pg_has_role(current_user, 'athyperadmin_atlas_maintenance', 'USAGE') "
          + "AS atlas_maintenance, "
          + "pg_has_role(current_user, 'athyperapp', 'USAGE') AS application_role",
        );
        const dbIdentity = identity.rows[0];
        if (
          dbIdentity?.atlas_maintenance !== true
          || dbIdentity.application_role === true
        ) {
          throw new Error(
            "Atlas maintenance connection must use the isolated "
            + "athyperadmin_atlas_maintenance authority; received "
            + `${dbIdentity?.role_name ?? "unknown"}.`,
          );
        }

        const maintenanceAuthority = new SqlAtlasThreadMaintenanceAuthority(
          maintenanceDb.kysely as never,
          {
            authority: "athyperadmin_atlas_maintenance",
            systemPrincipalId: SYSTEM_ACTOR_ID,
          },
        );
        const threadService = new AtlasThreadService({
          repository: new SqlAtlasThreadRepository(_db),
          retentionPolicies: new SqlAtlasRetentionPolicyResolver(_db as never, {
            defaultRetentionDays: atlasPersistence.defaultRetentionDays,
            minimumRetentionDays: atlasPersistence.minimumRetentionDays,
            maximumRetentionDays: atlasPersistence.maximumRetentionDays,
          }),
          authorizers: createPrincipalPrivateAtlasThreadAuthorizers(),
          maintenanceAuthority,
          purgeBatchSize: atlasPersistence.purgeBatchSize,
          contextMaxMessages: atlasPersistence.contextMaxMessages,
          contextMaxCharacters: atlasPersistence.contextMaxCharacters,
          staleRunTimeoutMs: atlasPersistence.staleRunTimeoutMs,
        });
        atlasConversationPurge = {
          service: threadService,
          batchSize: atlasPersistence.purgeBatchSize,
          metrics: {
            observe: recordAtlasConversationPurge,
          },
        };
        if (config.atlasAgent?.tools.enabled) {
          const toolMaintenanceAuthority =
            new SqlAtlasToolInvocationMaintenanceAuthority(
              maintenanceDb.kysely as never,
              {
                authority: "athyperadmin_atlas_tool_maintenance",
              },
            );
          const toolRecoveryService =
            new SqlAtlasToolInvocationRecoveryAdminService(
              maintenanceDb.kysely as never,
              toolMaintenanceAuthority,
              {
                authority: "athyperadmin_atlas_tool_maintenance",
              },
            );
          atlasToolInvocationRecovery = {
            ...
            staleRunTimeoutMs: atlasPersistence.staleRunTimeoutMs,
            metrics: {
              observe: recordAtlasToolInvocationRecovery,
            },
          };
        }
        lifecycle.onShutdown(() => maintenanceDb.close());
        logger.info("atlas_conversation_maintenance_authority_ready", {
          runtimeMode,
          role: dbIdentity.role_name,
          purgeBatchSize: atlasPersistence.purgeBatchSize,
          toolRecoveryEnabled: atlasToolInvocationRecovery != null,
          staleRunTimeoutMs: atlasPersistence.staleRunTimeoutMs,
        });
      } catch (error) {
        await maintenanceDb.close().catch(() => undefined);
        logger.error("atlas_conversation_maintenance_authority_unavailable", {
          runtimeMode,
  ...
  const jobs = createJobsService({
    ...
    attachmentStorage,
    backupStorage,
    ...(atlasConversationPurge ? { atlasConversationPurge } : {}),
    ...(atlasToolInvocationRecovery
      ? { atlasToolInvocationRecovery }
      : {}),
  ...
  const renderService = new RenderDocumentService(_db, pdfRenderer, objectStorage);
