-- Retires Technostat's former functional shared-service selector records.
DO $retire_technostat_legacy_operating_organizations$
DECLARE
  v_tenant_id uuid; v_actor uuid;
BEGIN
  SELECT id INTO v_tenant_id FROM master.tenant WHERE realm_key='athyper' AND code='technostat';
  ...
  IF v_tenant_id IS NULL OR v_actor IS NULL THEN RAISE EXCEPTION '[203_retire_legacy_operating_organizations] Technostat tenant or seed principal missing'; END IF;
  PERFORM set_config('app.current_principal_id',v_actor::text,true);
  IF EXISTS (
    SELECT 1 FROM master.operating_organization org
    WHERE org.tenant_id=v_tenant_id AND org.code IN ('tech.finance','tech.people','tech.procurement')
  ...
  ) THEN RAISE EXCEPTION '[203_retire_legacy_operating_organizations] legacy Technostat organizations still have business-record references'; END IF;
  UPDATE master.operating_organization_company_assignment assignment
   ...
   WHERE org.tenant_id=v_tenant_id AND assignment.tenant_id=org.tenant_id AND assignment.operating_organization_id=org.id
     AND org.code IN ('tech.finance','tech.people','tech.procurement');
  UPDATE master.operating_organization
   ...
   WHERE tenant_id=v_tenant_id AND code IN ('tech.finance','tech.people','tech.procurement');
END;
$retire_technostat_legacy_operating_organizations$;
