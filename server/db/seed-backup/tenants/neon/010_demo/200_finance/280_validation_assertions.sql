...
BEGIN
    ...
    -- ══════════════════════════════════════════════════════════════════════
    -- §23.1b  No group/local COA assignment rows; COA-GROUP is internal taxonomy
    -- ══════════════════════════════════════════════════════════════════════
    SELECT count(*) INTO v_count
    FROM (
        SELECT company_code_id
        ...
        WHERE tenant_id = v_tid
          AND assignment_type IN ('group', 'local')
          AND is_active = true
    ) stale;
    IF v_count > 0 THEN
        v_errors := array_append(v_errors, format('§23.1b FAIL: %s active group/local COA assignments should be removed', v_count));
    END IF;
    ...
    WHERE ga.tenant_id = v_tid
      ...
      AND COALESCE(coa.metadata->>'_chart_role', '') <> 'reporting_taxonomy'
    ...
    IF v_count <> 2 THEN
        v_errors := array_append(v_errors, format('CUSTOM FAIL: Expected exactly 2 active selectable operating COAs, got %s', v_count));
    END IF;
