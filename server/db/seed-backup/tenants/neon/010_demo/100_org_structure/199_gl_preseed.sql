...
BEGIN
    ...
    ('LE-ASAC', 'Athyper Saudi Construction',                  'Saudi construction subsidiary',                         'SA', 'SAR', 'SAR', 'subsidiary', 'LE-ATHQ', 'ifrs'),
    ...
    ('LE-ASAH', 'Athyper Saudi Hospitality',                   'Saudi hospitality subsidiary',                          'SA', 'SAR', 'SAR', 'subsidiary', 'LE-ATHQ', 'ifrs'),
    ...
    ('LE-AITM', 'Athyper India Textile & Leather Mfg',         'India textile & leather manufacturing subsidiary',      'IN', 'INR', 'INR', 'subsidiary', 'LE-ATHQ', 'ifrs'),
    ...
    ('LE-ADPM', 'Athyper Germany Pharmaceutical Mfg',          'Germany pharmaceutical manufacturing subsidiary',       'DE', 'EUR', 'EUR', 'subsidiary', 'LE-ATHQ', 'ifrs'),
    ...
    ('LE-AJED', 'Athyper Japan Education Services',            'Japan education services subsidiary',                   'JP', 'JPY', 'JPY', 'subsidiary', 'LE-ATHQ', 'ifrs'),
    ...
    INSERT INTO tmp_cc (code, name, le_code, functional_currency, fiscal_year_start_month, regulatory_framework, industry_pack, industry_vertical) VALUES
    ('ATHQ', 'Athyper Group Holdings',            'LE-ATHQ', 'MYR',  1, 'ifrs',    NULL,                    NULL),
    ('AMRE', 'Athyper Malaysia Real Estate',       'LE-AMRE', 'MYR',  1, 'ifrs',    'pack_real_estate',      'real_estate'),
    ('AQTU', 'Athyper Qatar Utilities',            'LE-AQTU', 'QAR',  4, 'ifrs',    'pack_utilities',        'utilities'),
    ('ASAC', 'Athyper Saudi Construction',         'LE-ASAC', 'SAR',  1, 'ifrs',    'pack_construction',     'construction'),
    ('AQTS', 'Athyper Qatar Transport',            'LE-AQTS', 'QAR',  4, 'ifrs',    'pack_transport',        'transport'),
    ('AUET', 'Athyper UAE Trading',                'LE-AUET', 'AED',  1, 'ifrs',    'pack_trading',          'trading'),
    ('ASAH', 'Athyper Saudi Hospitality',          'LE-ASAH', 'SAR',  1, 'ifrs',    'pack_hospitality',      'hospitality'),
    ('AUIC', 'Athyper US InfoComm',                'LE-AUIC', 'USD',  1, 'us_gaap', 'pack_infocomm',         'infocomm'),
    ('ASGF', 'Athyper Singapore Financial',        'LE-ASGF', 'SGD',  1, 'ifrs',    'pack_financial',        'financial'),
    ('AITM', 'Athyper India Textile Mfg',          'LE-AITM', 'INR',  4, 'ifrs',    'pack_mfg_textile',      'mfg_textile'),
    ('ACFB', 'Athyper Canada Food & Bev Mfg',      'LE-ACFB', 'CAD',  1, 'ifrs',    'pack_mfg_food_bev',     'mfg_food_bev'),
    ('ADPM', 'Athyper Germany Pharma Mfg',         'LE-ADPM', 'EUR',  1, 'ifrs',    'pack_mfg_pharma',       'mfg_pharma'),
    ('ATEM', 'Athyper Taiwan Electronics Mfg',     'LE-ATEM', 'TWD',  1, 'ifrs',    'pack_mfg_electronics',  'mfg_electronics'),
    ('ASPE', 'Athyper SA Petroleum Extraction',    'LE-ASPE', 'ZAR',  3, 'ifrs',    'pack_mining_petroleum', 'mining_petroleum'),
    ('AUKA', 'Athyper UK Agriculture',             'LE-AUKA', 'GBP',  4, 'ifrs',    'pack_agriculture',      'agriculture'),
    ('AJED', 'Athyper Japan Education',            'LE-AJED', 'JPY',  4, 'ifrs',    'pack_education',        'education'),
    ('APHS', 'Athyper Philippines Hospital',       'LE-APHS', 'PHP',  1, 'ifrs',    'pack_healthcare',       'healthcare');
    ...
    -- Tag demo company codes so the shared industry org template can add
    -- vertical-specific leaves without hard-coded demo mappings.
    UPDATE master.company_code cc
    SET metadata = cc.metadata || jsonb_build_object(
            '_industry_pack',      t.industry_pack,
            '_industry_vertical',  t.industry_vertical,
            '_industry_packs',     jsonb_build_array(t.industry_pack),
            '_industry_verticals', jsonb_build_array(t.industry_vertical)
        ),
        updated_at = now(),
        updated_by = v_su
    FROM tmp_cc t
    WHERE cc.tenant_id = v_tid
      AND cc.code = t.code
      AND t.industry_pack IS NOT NULL
      AND (
          cc.metadata->>'_industry_pack' IS DISTINCT FROM t.industry_pack
          OR cc.metadata->>'_industry_vertical' IS DISTINCT FROM t.industry_vertical
      );
