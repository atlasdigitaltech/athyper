...
Owned by @athyper/server-db. Run from server/db:
    python3 prisma/fix-prisma-relations.py

(invoked automatically by `pnpm run db:codegen:sync` after the three db pulls.)

Processes all three plane schemas (neon, mesh, studio). Fixes are no-ops
...
# Schemas sit beside this script in server/db/prisma/.
