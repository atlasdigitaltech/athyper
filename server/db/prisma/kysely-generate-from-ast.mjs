 ...
 *
 * Owned by @athyper/server-db. Run from server/db:
 *   node prisma/kysely-generate-from-ast.mjs --schema prisma/schema.neon.prisma
 *   node prisma/kysely-generate-from-ast.mjs --schema prisma/schema.mesh.prisma
 *   node prisma/kysely-generate-from-ast.mjs --schema prisma/schema.studio.prisma
 * (or via the package scripts: `pnpm run db:codegen:{neon,mesh,studio}` / `pnpm run codegen`.)
 */
...
// Resolve @mrleebo/prisma-ast. It is a direct devDependency of @athyper/server-db,
// so require.resolve from this file finds it via the workspace install. Fall back
// to a global `npm install -g prisma-kysely` for contributors without one.
function loadPrismaAst() {
  try {
    return require("@mrleebo/prisma-ast");
  } catch {}
  try {
    const globalRoot = execSync("npm root -g", { encoding: "utf-8" }).trim();
    for (const sub of [
      "@mrleebo/prisma-ast",
      "prisma-kysely/node_modules/@mrleebo/prisma-ast",
    ]) {
      const p = resolve(globalRoot, sub);
      if (existsSync(p)) return require(p);
    }
  } catch {}
  throw new Error("Cannot locate @mrleebo/prisma-ast — run `pnpm install` at the repo root, or `npm install -g prisma-kysely`.");
}

const { getSchema } = loadPrismaAst();
