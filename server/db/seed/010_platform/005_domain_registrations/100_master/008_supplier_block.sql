...
SELECT
    ...
    'supplier_block', 'SBK', 'supplier_block',
...
WHERE NOT EXISTS (
    ...
    WHERE table_schema = 'master' AND table_name = 'supplier_block'
      AND entity_code = 'supplier_block' AND tenant_id IS NULL
);
...
SET display_config        = COALESCE(display_config, '{}'::jsonb) || jsonb_build_object(
        ...
        'list_columns',       '["block_type","block_reason","blocked_at","status"]'::jsonb,
