...
FROM (VALUES
            (v_ev,'category_id',       'category_id',       'Category',       'uuid',  'reference','one','standard',false,true, false,false,'{"ref_entity":"item_category"}'::jsonb,  110,v_su),
            (v_ev,'spend_category_id', 'spend_category_id', 'Spend Category', 'uuid',  'reference','one','standard',false,true, false,false,'{"ref_entity":"spend_category"}'::jsonb,  120,v_su),
...
SELECT entity_version_id, name, column_name, label, data_type, ui_type,
    ...
    CASE WHEN data_type = 'enum' THEN '{}'::jsonb END
FROM (VALUES
            -- code/name/description — mirror 000_common_fields Pass 3a/3b/3c;
            -- explicit here so they are present even when the table was absent
            -- during the 000_common_fields run (information_schema guard miss).
            ...
            (v_ev,'category_id',       'category_id',       'Category',        'uuid',   'reference','one','standard',false,true, false,false,'{"ref_entity":"item_category"}'::jsonb,   130,v_su),
            (v_ev,'spend_category_id', 'spend_category_id', 'Spend Category',  'uuid',   'reference','one','standard',false,true, false,false,'{"ref_entity":"spend_category"}'::jsonb,  140,v_su),
...
SELECT entity_version_id, name, column_name, label, data_type, ui_type,
    ...
    CASE WHEN data_type = 'enum' THEN '{}'::jsonb END
FROM (VALUES
            (v_ev,'parent_id',           'parent_id',           'Parent Category', 'uuid',   'reference','one','standard',false,true, false,false,'{"ref_entity":"item_category"}'::jsonb, 110,v_su),
            (v_ev,'level_no',            'level_no',            'Level',           'integer','number',   'one','system',  false,true, true, false,NULL::jsonb,                              120,v_su),
            (v_ev,'default_tax_group_id','default_tax_group_id','Default Tax Group','uuid',  'reference','one','standard',false,true, false,false,'{"ref_entity":"tax_group"}'::jsonb,      130,v_su),
            (v_ev,'sort_order',          'sort_order',          'Sort Order',      'integer','number',   'one','standard',false,false,true, false,NULL::jsonb,                              140,v_su)
) AS v(entity_version_id, name, column_name, label, data_type, ui_type,
       cardinality, origin, is_required, is_filterable, is_sortable,
       is_searchable, validation, sort_order, created_by)
        ON CONFLICT DO NOTHING;
...
FROM (VALUES
            ...
            (v_ev,'name',                       'name',                       'Name',                    'string', 'text',    'one','standard',true, true, true, true, NULL::jsonb,                                  40, v_su),
            (v_ev,'description',                'description',                'Description',             'text',   'textarea','one','standard',false,false,false,true, NULL::jsonb,                                  50, v_su),
            (v_ev,'parent_id',                  'parent_id',                  'Parent',                  'uuid',   'reference','one','standard',false,true, false,false,'{"ref_entity":"spend_category"}'::jsonb,    110,v_su),
            (v_ev,'procurement_type',           'procurement_type',           'Procurement Type',        'enum',   'select',  'one','standard',true, true, true, false,NULL::jsonb,                                  120,v_su),
            (v_ev,'visibility',                 'visibility',                 'Visibility',              'enum',   'select',  'one','standard',true, true, true, false,NULL::jsonb,                                  130,v_su),
            (v_ev,'is_classification_required', 'is_classification_required', 'Classification Required', 'boolean','hidden',  'one','standard',false,true, false,false,NULL::jsonb,                                  140,v_su),
            (v_ev,'is_hs_required',             'is_hs_required',             'HS Code Required',        'boolean','hidden',  'one','standard',false,true, false,false,NULL::jsonb,                                  150,v_su),
            (v_ev,'is_regulated',               'is_regulated',               'Regulated',               'boolean','hidden',  'one','standard',false,true, false,false,NULL::jsonb,                                  160,v_su),
            (v_ev,'default_intent_id',          'default_intent_id',          'Default Intent',          'uuid',   'reference','one','standard',false,true, false,false,'{"ref_entity":"business_intent"}'::jsonb,   170,v_su),
            (v_ev,'sort_order',                 'sort_order',                 'Sort Order',              'integer','number',  'one','standard',false,false,true, false,NULL::jsonb,                                  180,v_su)
) AS v(entity_version_id, name, column_name, label, data_type, ui_type,
       cardinality, origin, is_required, is_filterable, is_sortable,
       is_searchable, validation, sort_order, created_by)
        ON CONFLICT DO NOTHING;
...
FROM (VALUES
            ...
            (v_ev,'mapping_mode',        'mapping_mode',        'Mode',            'enum',   'select',  'one','standard',true, true, true, true, NULL::jsonb,                                  130,v_su),
            ...
            (v_ev,'capex_screening_currency','capex_screening_currency','CapEx Currency','string','currency','one','standard',false,true,true, true, NULL::jsonb,                              190,v_su),
            ...
            (v_ev,'override_visibility', 'override_visibility', 'Override Visibility','enum','select',  'one','standard',false,true, true, true, NULL::jsonb,                                  210,v_su),
       ...
       is_searchable, validation, sort_order, created_by)
        ...
        UPDATE control.entity_field ef
           SET is_searchable = true,
               updated_at = now(),
               updated_by = v_su
          FROM control.entity_version ev2
          JOIN control.entity e2 ON e2.id = ev2.entity_id
         WHERE ef.entity_version_id = ev2.id
           AND e2.name = 'company_code_spend_policy'
           AND ev2.version_no = 1
           AND ef.name IN ('mapping_mode', 'capex_screening_currency', 'override_visibility', 'status')
           AND ef.is_searchable = false;
    END IF;

    -- ── Patch existing rows missing ref_entity (idempotent UPDATE) ────────────
    -- Covers rows inserted before the validation column was added to this file.
    UPDATE control.entity_field ef
       SET validation = ref_fix.val
      FROM (VALUES
        ('product',       'category_id',        '{"ref_entity":"item_category"}'::jsonb),
        ('product',       'spend_category_id',  '{"ref_entity":"spend_category"}'::jsonb),
        ('item',          'company_code_id',     '{"ref_entity":"company_code"}'::jsonb),
        ('item',          'product_id',          '{"ref_entity":"product"}'::jsonb),
        ('item',          'category_id',         '{"ref_entity":"item_category"}'::jsonb),
        ('item',          'spend_category_id',   '{"ref_entity":"spend_category"}'::jsonb),
        ('item_category', 'parent_id',           '{"ref_entity":"item_category"}'::jsonb),
        ('item_category', 'default_tax_group_id','{"ref_entity":"tax_group"}'::jsonb),
        ('spend_category','parent_id',           '{"ref_entity":"spend_category"}'::jsonb),
        ('spend_category','default_intent_id',   '{"ref_entity":"business_intent"}'::jsonb),
        ('company_code_spend_policy','company_code_id',       '{"ref_entity":"company_code"}'::jsonb),
        ('company_code_spend_policy','spend_category_id',     '{"ref_entity":"spend_category"}'::jsonb),
        ('company_code_spend_policy','default_gl_account_id', '{"ref_entity":"gl_account"}'::jsonb),
        ('company_code_spend_policy','default_tax_group_id',  '{"ref_entity":"tax_group"}'::jsonb),
        ('company_code_spend_policy','default_intent_id',     '{"ref_entity":"business_intent"}'::jsonb),
        ('company_code_spend_policy','asset_class_id',        '{"ref_entity":"asset_class"}'::jsonb)
