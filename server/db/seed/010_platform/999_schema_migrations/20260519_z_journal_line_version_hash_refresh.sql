-- 20260519_z_journal_line_version_hash_refresh.sql
-- Refresh journal_line version_hash after GL account lookup metadata changes so
-- compiled descriptor caches invalidate immediately.
...
BEGIN
    WITH version_payload AS (
        ...
        WHERE ev.tenant_id IS NULL
          ...
          AND e.entity_code = 'journal_line'
          AND ev.version_no = 1
    ),
    computed_hash AS (
        SELECT
