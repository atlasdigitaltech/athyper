-- Table-owned seed for control.lifecycle
...
-- ============================================================
-- SOURCE: server/db/seed/010_platform/004_entity_engine/010_lifecycles.sql
-- ============================================================

-- 010_lifecycles/000_all_lifecycles.sql
-- Unified platform lifecycle seed — all 21 lifecycle definitions
-- Tables: control.lifecycle, control.lifecycle_state, control.lifecycle_transition
-- Idempotent: cleanup at top, ON CONFLICT DO NOTHING throughout

-- ─── Cleanup ─────────────────────────────────────────────────────────────────
DO $$
BEGIN
    DELETE FROM control.lifecycle_transition
    WHERE lifecycle_id IN (
        SELECT id FROM control.lifecycle WHERE tenant_id IS NULL
    );
    DELETE FROM control.lifecycle_state
    WHERE lifecycle_id IN (
        SELECT id FROM control.lifecycle WHERE tenant_id IS NULL
    );
    DELETE FROM control.lifecycle WHERE tenant_id IS NULL;
END $$;
...
-- States: onboarding → active ⇌ on_leave / suspended → terminated
DO $$
DECLARE
    v_lc_id uuid;
    v_s     jsonb;
...
-- States: uploading → active ⇌ flagged → archived
DO $$
DECLARE
    v_lc_id uuid;
    v_s     jsonb;
...
-- States: draft → published ⇌ flagged → hidden → deleted
DO $$
DECLARE
    v_lc_id uuid;
    v_s     jsonb;
...
-- States: draft → active → void / archived
DO $$
DECLARE
    v_lc_id uuid;
    v_s     jsonb;
...
-- States: draft → in_service ⇌ under_maintenance → disposed
DO $$
DECLARE
    v_lc_id uuid;
    v_s     jsonb;
...
-- States: draft → approved → active ⇌ closed → archived
DO $$
DECLARE
    v_lc_id uuid;
    v_s     jsonb;
...
-- States: pending_verification → active ⇌ frozen → closed
DO $$
DECLARE
    v_lc_id uuid;
    v_s     jsonb;
...
-- States: pending → active → expired (auto) / revoked
DO $$
DECLARE
    v_lc_id uuid;
    v_s     jsonb;
...
-- ============================================================
-- SOURCE: server/db/seed/010_platform/005_domain_registrations/100_master/002_supplier_lifecycle.sql
-- ============================================================
...
-- ============================================================
-- SOURCE: server/db/seed/010_platform/005_domain_registrations/100_master/004_customer_lifecycle.sql
-- ============================================================
...
-- ============================================================
-- SOURCE: server/db/seed/010_platform/005_domain_registrations/200_document/002_document_lifecycles.sql
-- ============================================================
