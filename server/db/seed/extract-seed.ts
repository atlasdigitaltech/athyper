#!/usr/bin/env tsx
// server/db/seed/extract-seed.ts
//
// Rebuilds 900_seed_data from the live database after a successful --reset --all run.
//
...
const BACKUP    = path.join(SQL_DIR, "900_seed_data_backup");
const OUTPUT    = path.join(SQL_DIR, "900_seed_data");
...
function main(): void {
  if (!fs.existsSync(BACKUP)) {
    process.stderr.write(
      ...
      + `Run first: Rename-Item server\\db\\sql\\900_seed_data server\\db\\sql\\900_seed_data_backup\n`,
    );
    process.exit(1);
  }
  ...
  console.log(`Scanning ${files.length} SQL files in 900_seed_data_backup …`);
