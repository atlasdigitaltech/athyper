-- ============================================================================
-- INDUSTRY PACK — HEALTHCARE SERVICES
-- ============================================================================
...
BEGIN
    ...
    ('BI-COGS', 'Healthcare Cost of Sales',     'Direct cost of medical supplies, drugs, and clinical consumables',  'COST_OF_SALES', 'HEALTHCARE', 'BI-COGS',  48),
    ('BI-REG',  'Healthcare Regulatory Expense','Licensing fees, inspections, accreditation, and regulatory filings','REGULATORY',    'HLTH_CERT',  'BI-REG',   18);
    ...
    WHERE sc.tenant_id = v_tid AND bi.tenant_id = v_tid
      AND (sc.code, bi.code) IN (
          ('SC-HLTH-MED',    'BI-COGS'), ('SC-HLTH-DRUG',   'BI-COGS'),
          ('SC-HLTH-SUPPLY', 'BI-COGS'), ('SC-HLTH-LAB',    'BI-COGS'),
          ('SC-HLTH-IT',     'BI-COGS')
      );
...
IF (SELECT count(*) FROM master.commodity_classification WHERE tenant_id = v_tid AND owner_type = 'spend_category' AND metadata->'_seed'->>'pack' = v_pack) < 1 THEN
    ...
    RAISE NOTICE '[115_pack_healthcare] Pack loaded: % spend cats, domain intents, % bridge rows, % routing rules',
