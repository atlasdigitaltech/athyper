-- ============================================================================
-- INDUSTRY PACK — MINING & PETROLEUM
-- ============================================================================
...
BEGIN
    ...
    ('BI-COGS',  'Petroleum Cost of Sales',    'Direct extraction, processing, and catalyst costs',                 'COST_OF_SALES', 'PETROLEUM',  'BI-COGS',  45),
    ('BI-CAPEX', 'Petroleum Capital Equipment','Capital investment in drilling rigs, wellheads, and refinery plant', 'CAPEX',         'PETRO_PLANT','BI-CAPEX', 35);
    ...
    WHERE sc.tenant_id = v_tid AND bi.tenant_id = v_tid
      AND (sc.code, bi.code) IN (
          ('SC-MNPET-DRILL',  'BI-COGS'), ('SC-MNPET-EXPL',   'BI-COGS'),
          ('SC-MNPET-REFINE', 'BI-COGS'), ('SC-MNPET-ENV',    'BI-COGS'),
          ('SC-MNPET-SAFETY', 'BI-COGS')
      );
...
IF (SELECT count(*) FROM master.commodity_classification WHERE tenant_id = v_tid AND owner_type = 'spend_category' AND metadata->'_seed'->>'pack' = v_pack) < 1 THEN
    ...
    RAISE NOTICE '[112_pack_mining_petroleum] Pack loaded: % spend cats, domain intents, % bridge rows, % routing rules',
