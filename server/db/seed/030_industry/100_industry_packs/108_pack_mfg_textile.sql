-- ============================================================================
-- INDUSTRY PACK — MFG TEXTILE (Textile & Leather Mfg)
-- ============================================================================
...
BEGIN
    ...
    ('BI-COGS', 'Textile Manufacturing Cost of Sales', 'Direct material and conversion costs for textile production', 'COST_OF_SALES', 'TEXTILE', 'BI-COGS', 41),
    ('BI-CAPEX','Textile Capital Equipment',           'Capital investment in looms, spinning frames, and dyeing plant','CAPEX',       'TEXTILE', 'BI-CAPEX', 31);
    ...
    WHERE sc.tenant_id = v_tid AND bi.tenant_id = v_tid
      AND (sc.code, bi.code) IN (
          ('SC-TX-FIBER', 'BI-COGS'), ('SC-TX-YARN',    'BI-COGS'),
          ('SC-TX-FABRIC','BI-COGS'),  ('SC-TX-APPAREL', 'BI-COGS'),
          ('SC-TX-FINISH','BI-COGS')
      );
...
IF (SELECT count(*) FROM master.commodity_classification WHERE tenant_id = v_tid AND owner_type = 'spend_category' AND metadata->'_seed'->>'pack' = v_pack) < 1 THEN
    ...
    RAISE NOTICE '[108_pack_mfg_textile] Pack loaded: % spend cats, domain intents, % bridge rows, % routing rules',
