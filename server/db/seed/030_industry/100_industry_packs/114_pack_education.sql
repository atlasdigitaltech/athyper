-- ============================================================================
-- INDUSTRY PACK — EDUCATION SERVICES
-- ============================================================================
...
BEGIN
    ...
    ('BI-COGS', 'Education Cost of Sales',     'Direct content, platform, and instructor delivery costs',           'COST_OF_SALES', 'EDUCATION', 'BI-COGS',  47),
    ('BI-REG',  'Education Regulatory Expense','Accreditation fees, regulatory filings, and compliance training',   'REGULATORY',    'EDU_CERT',  'BI-REG',   17);
    ...
    WHERE sc.tenant_id = v_tid AND bi.tenant_id = v_tid
      AND (sc.code, bi.code) IN (
          ('SC-EDU-CONTENT',  'BI-COGS'), ('SC-EDU-PLATFORM', 'BI-COGS'),
          ('SC-EDU-FACILITY', 'BI-COGS'), ('SC-EDU-STAFF',    'BI-COGS'),
          ('SC-EDU-CERT',     'BI-REG')
      );
...
IF (SELECT count(*) FROM master.commodity_classification WHERE tenant_id = v_tid AND owner_type = 'spend_category' AND metadata->'_seed'->>'pack' = v_pack) < 1 THEN
    ...
    RAISE NOTICE '[114_pack_education] Pack loaded: % spend cats, domain intents, % bridge rows, % routing rules',
