...
BEGIN
    ...
    VALUES (
        ...
        'Internal tenant-scoped reporting taxonomy used by framework COA _group_map metadata',
        'reporting_taxonomy',
        NULL,
        'GRP-*',
        1,
    ...
    CREATE TEMP TABLE tmp_gl (
        ...
        group_map      text           -- NULL for taxonomy accounts (they are the group-map targets)
    ) ON COMMIT DROP;
    ...
    SELECT
        md5('wave5:coa-group-account:' || v_tid::text || ':' || t.code)::uuid,
        ...
        v_meta || jsonb_build_object(
            ...
            '_journal_postable', false,
            '_reporting_taxonomy', true,
            '_reporting_leaf', t.node_type = 'posting'
        ),
        'active', v_su
    FROM tmp_gl t
    ...
    ON CONFLICT (tenant_id, chart_of_account_id, code) DO UPDATE SET
        ...
        metadata       = master.gl_account.metadata
                         ...
                         || jsonb_build_object(
                                '_display_no', EXCLUDED.sort_order::text,
                                '_journal_postable', false,
                                '_reporting_taxonomy', true,
                                '_reporting_leaf', EXCLUDED.node_type = 'posting'
                            ),
        updated_at = now(),
        updated_by = v_su
    WHERE (master.gl_account.name,
       ...
       OR master.gl_account.metadata->>'_reporting_leaf' IS DISTINCT FROM
          CASE WHEN EXCLUDED.node_type = 'posting' THEN 'true' ELSE 'false' END;
    ...
    SELECT count(*) INTO v_flag_missing
    FROM master.gl_account
    WHERE tenant_id = v_tid
      AND chart_of_account_id = v_coa_id
      AND metadata->'_seed'->>'pack' = v_pack
      AND (
          metadata->>'_journal_postable' IS DISTINCT FROM 'false'
          OR metadata->>'_reporting_taxonomy' IS DISTINCT FROM 'true'
          OR metadata->>'_reporting_leaf' IS DISTINCT FROM
             CASE WHEN node_type = 'posting' THEN 'true' ELSE 'false' END
      );
    ...
    IF v_flag_missing <> 0 THEN
        RAISE EXCEPTION '[210_group_chart] Expected all group accounts to be non-postable reporting taxonomy accounts, got % missing flags', v_flag_missing;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        WHERE id = v_coa_id
          AND framework = 'reporting_taxonomy'
          AND metadata->>'_chart_role' = 'reporting_taxonomy'
          AND metadata->>'_selectable' = 'false'
          AND metadata->>'_operating_coa' = 'false'
          AND metadata->>'_journal_postable' = 'false'
    ) THEN
