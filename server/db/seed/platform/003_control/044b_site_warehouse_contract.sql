-- master.site + master.warehouse metadata contract. Run after 044_control_entity_operation_contract.sql.
-- Sections map to control.* tables:
--   Â§1 control.entity registry patches
--   Â§2 control.entity_field versioned fields
--   Â§3 enum_domain_code assignments
--   Â§4 reference / validation config
--   Â§5 lookup / dependent-filter config
--   Â§6 control.entity_relation UI behavior
--   Â§7 site entity_operation rows
--   Â§8 warehouse entity_operation rows
--   Â§9 field group_key assignments

-- Â§1 Entity registry patches
-- site has_hierarchy=true â†’ parent_site_id + level_no tree; compiler then enables tree
-- controls, hierarchy list rendering, and exclude_current_record on parent pickers.
UPDATE control.entity
SET
    feature_flags  = COALESCE(feature_flags, '{}'::jsonb) || '{"has_hierarchy":true}'::jsonb,
    display_config = COALESCE(display_config, '{}'::jsonb)
                     || '{"title_field":"name","subtitle_field":"code","list_columns":["id","code","name","company_code_id","site_type","country_code","status"],"detail_renderer":"master","detail_profile":"simple","list_renderer":"table","runtime_canvas_flags":{"descriptorSurfaceShell":true,"groupedForms":true}}'::jsonb,
...
SET
    ...
    display_config = COALESCE(display_config, '{}'::jsonb)
                     || '{"title_field":"name","subtitle_field":"code","list_columns":["id","code","name","site_id","warehouse_type","status"],"detail_renderer":"master","detail_profile":"simple","list_renderer":"table","runtime_canvas_flags":{"descriptorSurfaceShell":true,"groupedForms":true}}'::jsonb,
    search_config  = COALESCE(search_config, '{}'::jsonb)
                     || '{"enabled":true,"fields":["code","name","description"]}'::jsonb,
    updated_at     = now()
WHERE table_schema = 'master'
...
-- Â§2 Versioned entity fields (10 site fields + 2 warehouse fields + site_id validation upgrade)
...
BEGIN
    ...
    -- â”€â”€ site â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    IF v_ev IS NOT NULL THEN
        ...
        FROM (VALUES
            -- â”€â”€ Â§A  Ownership â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            -- company_code_id: UUID FK â†’ company_code; required; scoped to
            ...
            -- â”€â”€ Â§B  Classification â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            -- site_type: enum select; enum_domain_code patched in Â§3 below.
            ...
            -- â”€â”€ Â§C  Hierarchy â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            -- â”€â”€ Â§D  Location context (code-valued references) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            -- country_code: CHAR(2); FK â†’ shared.country(code); required.
            ...
            -- timezone_code: text; FK-like â†’ shared.timezone(code); optional.
            ...
            -- â”€â”€ Â§E  Management â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            -- â”€â”€ Â§F  Capacity â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            -- capacity_uom: code-valued â†’ shared.uom(code); optional.
            ...
            -- capacity_value: must pair with capacity_uom; numeric â‰¥ 0.
        ...
        RAISE NOTICE '044b Â§2 site entity fields: %', (
    ...
    ELSE
        RAISE WARNING '044b Â§2 site entity_version not found â€” skipping field registration';
    END IF;
    ...
    IF v_ev IS NOT NULL THEN
        ...
        -- label resolution hints. Safe to replay â€” only fires when the value
        ...
        RAISE NOTICE '044b Â§2 warehouse fields: description + is_negative_stock_allowed added; site_id validation upgraded';
    ELSE
        RAISE WARNING '044b Â§2 warehouse entity_version not found â€” skipping';
    END IF;

    RAISE NOTICE '044b Â§2 versioned entity fields: done';
END $$;


-- Â§3 Enum domain assignment (sets enum_domain_code, clears placeholder enum_config).
...
-- Â§4 Reference / validation config â€” full reference_config for every picker field.

WITH field_updates(entity_name, field_name, reference_config) AS (
  VALUES
    -- â”€â”€ site: UUID references â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    -- â”€â”€ site: code-valued references (UI book Â§739â€“829) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    -- â”€â”€ warehouse: UUID references â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- Â§5 Lookup config â€” drives runtime-options: search fields, scope filters, dependent cascades.

-- â”€â”€ site â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
WITH site_lookup(field_name, lookup_config) AS (
  VALUES
    -- company_code_id: search by code or name; active records only;
    -- legal-entity scope (no further dependent filter â€” this IS the scope key).
    ('company_code_id'::text,
     '{"search_fields":["code","name"],"filters":{"status":"active"},"scope_mode":"legal_entity"}'::jsonb),

    -- parent_site_id: cascades off company_code_id â€” only shows sites in the
...
-- â”€â”€ warehouse â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- Â§6 Relation UI behavior â€” merge chooser/scope/child-label hints (|| preserves other keys).

-- warehouse â†’ site: annotate belongs_to with picker chooser + legal_entity scope.
...
-- site â†’ warehouses: annotate has_many so detail surfaces know the tab label,
...
-- site â†’ cost_centers: same pattern for the cost centers child tab.
UPDATE control.entity_relation er
SET    ui_behavior = COALESCE(er.ui_behavior, '{}'::jsonb)
...
-- Â§7 site operations â€” mirrors Set B (business_unit / cost_center / profit_center) + import.
...
-- Â§8 warehouse operations â€” fills in the import op missing from 044's 6-op seed.
...
-- Â§9 Field group_key â€” versioned (entity-specific) rows only. Canonical fields are
-- bound to groups by 042b_control_field_group_member_contract.sql. Group keys defined in 020_control_field_group_contract.sql.
