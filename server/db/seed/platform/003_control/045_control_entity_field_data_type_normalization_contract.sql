-- Final metadata field type normalization + filterability overrides.
...
-- runtime data_type values. Also applies the document-runtime + address-picker
-- is_filterable overrides (folded from former 051_document_runtime_field_overrides.sql).
--
-- Layout:
--   Â§a Data-type alias repair
--   Â§b Legacy AP/master array-alias repair
--   Â§c Hierarchy/grouping metadata
--   Â§d Document-runtime filterability overrides (folded)
--   Â§e Compiled-descriptor invalidation (final step)
...
-- Â§c Hierarchy/grouping metadata â€” runs late because it depends on settled data_type values.
UPDATE control.entity_field ef
SET
    is_groupable = true,
    updated_at = now(),
...
-- Â§d Document-runtime + address-picker filterability overrides.
-- 042's auto-coverage only marks well-known names (id/code/name/*_id/is_*) filterable, so
-- runtime-registry tables and address-picker views lose legitimate filter clauses silently.
UPDATE control.entity_field f
  ...
  JOIN control.entity e
    ON e.id = ev.entity_id
 WHERE f.entity_version_id = ev.id
   AND ev.tenant_id IS NULL
   AND ev.status = 'EFFECTIVE'
   AND e.tenant_id IS NULL
   AND e.status = 'ACTIVE'
   AND e.entity_code IN (
       'polymorphic_child_binding',
       'document_lookup',
       'entity_action_rule'
   )
   AND f.name IN (
   ...
   AND f.is_filterable IS DISTINCT FROM TRUE;

-- Address-picker views: PI header (billto/billfrom/remitto) + line (ship_to/ship_from)
-- carry lookup_config.filters that target purpose / address_status / role_qualifier.
UPDATE control.entity_field f
   SET is_filterable = TRUE,
       updated_at    = now(),
       updated_by    = '00000000-0000-0000-0000-000000000000'
...
DO $diag$
DECLARE
    v_filterable_count integer;
    v_address_filterable_count integer;
BEGIN
    SELECT count(*) INTO v_filterable_count
      FROM control.entity_field f
      JOIN control.entity_version ev ON ev.id = f.entity_version_id
      JOIN control.entity e ON e.id = ev.entity_id
     WHERE ev.tenant_id IS NULL
       AND e.tenant_id IS NULL
       AND e.entity_code IN ('polymorphic_child_binding','document_lookup','entity_action_rule')
       AND f.name IN ('binding_code','lookup_code','action_code',
                      'parent_entity_code','child_entity_code','entity_code')
       AND f.is_filterable = TRUE;

    SELECT count(*) INTO v_address_filterable_count
      FROM control.entity_field f
      JOIN control.entity_version ev ON ev.id = f.entity_version_id
      JOIN control.entity e ON e.id = ev.entity_id
     WHERE ev.tenant_id IS NULL
       AND e.tenant_id IS NULL
       AND e.entity_code IN (
           'v_company_code_address','v_supplier_address',
           'v_business_partner_address','v_site_address'
       )
       AND f.name = 'purpose'
       AND f.is_filterable = TRUE;

    RAISE NOTICE '[045 doc-runtime overrides] % filterable lookup-key columns', v_filterable_count;
    RAISE NOTICE '[045 doc-runtime overrides] % address-view rows with purpose filterable', v_address_filterable_count;

    IF v_filterable_count = 0 THEN
        RAISE WARNING '[045 doc-runtime overrides] no rows updated â€” verify '
                    'control.entity rows exist for polymorphic_child_binding / document_lookup / entity_action_rule '
                    '(run 040a_control_all_schema_entity_coverage_contract.sql first).';
    END IF;
    IF v_address_filterable_count = 0 THEN
        RAISE WARNING '[045 doc-runtime overrides] no address-view rows updated â€” verify '
                    'v_company_code_address / v_supplier_address entity registrations exist.';
    END IF;
END $diag$;


-- Â§e Invalidate compiled descriptors whose field payload diverges from active entity_field
-- rows. snapshot.entity_compiled is append-only at runtime; immutability triggers are
-- disabled for this seed-side repair path. Startup compiler will repopulate.
DO $$
DECLARE
