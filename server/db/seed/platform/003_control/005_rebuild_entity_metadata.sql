-- Explicit entity-metadata rebuild gate.
-- When app.rebuild_entity_metadata is true, clear the control metadata graph
-- before any table-owned seeds in this folder run. The provisioner forces seed
-- files in this mode, so the following files recreate the seed-owned rows.
--
-- CASCADE intentionally clears additional FK dependents that point at these
-- roots. Seed-owned dependents are listed here so the scope is visible; any
-- other FK children are database-dependent and are cleared by PostgreSQL.
