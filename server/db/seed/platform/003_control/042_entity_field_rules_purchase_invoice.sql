...
-- The visibility evaluator (field-visibility.ts) supports the {field, eq/ne/isNull/notNull}
-- predicate shape — store under ui_hint.display.visible_when per the contract
-- registry's phase-4 migration target.
WITH pi_version AS (
    SELECT ev.id AS version_id
      FROM control.entity_version ev
      JOIN control.entity e ON e.id = ev.entity_id
