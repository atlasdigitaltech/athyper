...
BEGIN
    ...
    -- ── STAGE C.5: Pre-check — all sc_codes in tmp_route must exist ─────────
    -- INNER JOIN in Stage D silently drops unresolved codes; catch them here.
    IF EXISTS (
        SELECT 1 FROM tmp_route r
        WHERE NOT EXISTS (SELECT 1 FROM tmp_sc_map sm WHERE sm.code = r.sc_code)
    ) THEN
        RAISE EXCEPTION '[024_base] tmp_route references unknown spend_category codes: %',
    ...
    IF (SELECT count(*) FROM control.commodity_to_spend_category_rule
        WHERE tenant_id = v_tid
          AND metadata->'_seed'->>'pack' = v_pack) < 90 THEN
        RAISE EXCEPTION '[024_base] Routing rule load incomplete: expected ≥90, got %',
