...
BEGIN
  ...
  SELECT md5('athyper:p5-e5:neon:'||code)::uuid,'neon.business_partner.'||code,'entity_operation',
    'neon.business_partner',code,v_module,risk::authz.risk_tier_d,false,false,shareable,delegable,overridable,
  ...
  SELECT id,CASE operation_code WHEN 'shared_read' THEN 'resource'::authz.scope_kind_d ELSE 'tenant'::authz.scope_kind_d END,'exact',v_actor
    FROM authz.permission WHERE canonical_code IN ('neon.business_partner.list','neon.business_partner.shared_read','neon.business_partner.override_execute')
