...
--           metadata._group_map.
-- Depends:  company codes, 200_chart_catalog.sql, selected framework accounts
...
DECLARE
    ...
    v_pack     text := '201_technostat_coa';
...
BEGIN
    ...
    IF v_tid IS NULL THEN
        RAISE EXCEPTION '[201_technostat_coa] technostat tenant not found';
    END IF;
    ...
    IF v_oper <> v_expected THEN
        RAISE EXCEPTION '[201_technostat_coa] Expected % active operating assignments, got %', v_expected, v_oper;
    END IF;

    IF v_stale <> 0 THEN
        RAISE EXCEPTION '[201_technostat_coa] Expected no active group/local COA assignments, got %', v_stale;
    END IF;
