-- 08_functions/002_control.sql
-- Depends on: 04_tables/002_control.sql, 05_pre_constraint_functions/001_shared (shared.current_tenant_id)
...
--   FIXED        → gl_account by code
--   FROM_INTENT  → business_intent.default_gl_account_id
--   FROM_CATEGORY→ spend_category → intent → gl_account
    ...
    CASE p_account_source
        ...
        WHEN 'FROM_INTENT' THEN
            IF p_intent_id IS NOT NULL THEN
                BEGIN
                    SELECT default_gl_account_id INTO v_gl_account_id
                    FROM master.business_intent
        ...
        WHEN 'FROM_CATEGORY' THEN
            IF p_spend_category_id IS NOT NULL THEN
                BEGIN
                    SELECT bi.default_gl_account_id INTO v_gl_account_id
...
COMMENT ON FUNCTION control.resolve_entry_account IS
    ...
    'POSTING_ROLE → resolve_posting_role_account() → account_fallback, '
    'FIXED → gl_account by code, FROM_INTENT → business_intent.default_gl_account_id, '
    'FROM_CATEGORY → spend_category → intent → gl_account. '
    'Called by the JE generation orchestrator for each entry template line. '
    'Returns JSONB: gl_account_id, method, fallback_used, resolved.';
...
--
-- Resolution chain:
--   1. control.posting_role_account_map (when populated)
--   2. Returns NULL if no mapping found (caller handles fallback)
--
-- This is the stub implementation. Phase 2 will add the mapping table
-- (control.posting_role_account_map) and full resolution logic.
-- ────────────────────────────────────────────────────────────────────────────
