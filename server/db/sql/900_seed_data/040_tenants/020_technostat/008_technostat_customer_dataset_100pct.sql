...
BEGIN
    ...
    IF v_profiled_count <> v_customer_count THEN
        RAISE EXCEPTION '[technostat_customer_100pct] company_code_customer_profile coverage %/%',
            v_profiled_count, v_customer_count;
    END IF;

    IF v_qualified_count <> v_customer_count THEN
        RAISE EXCEPTION '[technostat_customer_100pct] customer_qualification coverage %/%',
            v_qualified_count, v_customer_count;
    END IF;

    IF v_block_history_count <> v_customer_count THEN
        RAISE EXCEPTION '[technostat_customer_100pct] customer_block history coverage %/%',
            v_block_history_count, v_customer_count;
    END IF;

    RAISE NOTICE
        '[technostat_customer_100pct] customers=%, app_index=%, profiled=%, qualified=%, block_history=%',
        v_customer_count, v_index_count, v_profiled_count, v_qualified_count, v_block_history_count;
