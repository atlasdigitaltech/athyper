...
-- Depends : 199_gl_preseed, 211_framework_ifrs_accounts, 311_ledger_books,
...
DECLARE
    ...
    -- GL Account IDs — resolved once from COA-IFRS
...
BEGIN
    ...
    -- ── B: Resolve IFRS GL account IDs (from COA-IFRS chart, one-time) ──────
    CREATE TEMP TABLE _acct (code text PRIMARY KEY, id uuid) ON COMMIT DROP;

    INSERT INTO _acct (code, id)
    SELECT ga.code, ga.id
    ...
    WHERE coa.tenant_id = v_tid
      ...
      AND ga.code IN (
        'IFRS-A-CASH-LOCAL', 'IFRS-A-CASH-PAYROLL',
        'IFRS-A-AR-TRADE',
        'IFRS-A-OAR-ADVANCE', 'IFRS-A-OAR-INPUT-TAX',
        'IFRS-A-INV-FG',
        'IFRS-A-DEP-BLDG', 'IFRS-A-DEP-IT',
        'IFRS-L-AP-TRADE', 'IFRS-L-AP-RETENTION', 'IFRS-L-AP-ACCRUED',
        'IFRS-L-TAX-WHT',
        'IFRS-L-EMP-SAL', 'IFRS-L-EMP-BONUS', 'IFRS-L-EMP-EOS',
        'IFRS-R-SALES-GOODS',
        'IFRS-E-COGS-MAT', 'IFRS-E-COGS-FREIGHT',
        'IFRS-E-HR-SAL', 'IFRS-E-HR-EOS', 'IFRS-E-HR-BONUS',
        'IFRS-E-GA-OFFICE', 'IFRS-E-GA-CONSULT', 'IFRS-E-GA-AUDIT',
        'IFRS-E-DA-BLDG', 'IFRS-E-DA-IT',
        'IFRS-Q-RE-OPENING', 'IFRS-Q-CAP-ISSUED',
        'IFRS-L-ACCR-GEN'
      );

    SELECT id INTO v_cash_local    FROM _acct WHERE code = 'IFRS-A-CASH-LOCAL';
    SELECT id INTO v_cash_payroll  FROM _acct WHERE code = 'IFRS-A-CASH-PAYROLL';
    SELECT id INTO v_ar_trade      FROM _acct WHERE code = 'IFRS-A-AR-TRADE';
    SELECT id INTO v_oar_advance   FROM _acct WHERE code = 'IFRS-A-OAR-ADVANCE';
    SELECT id INTO v_oar_input_tax FROM _acct WHERE code = 'IFRS-A-OAR-INPUT-TAX';
    SELECT id INTO v_inv_fg        FROM _acct WHERE code = 'IFRS-A-INV-FG';
    SELECT id INTO v_dep_bldg      FROM _acct WHERE code = 'IFRS-A-DEP-BLDG';
    SELECT id INTO v_dep_it        FROM _acct WHERE code = 'IFRS-A-DEP-IT';
    SELECT id INTO v_ap_trade      FROM _acct WHERE code = 'IFRS-L-AP-TRADE';
    SELECT id INTO v_ap_retention  FROM _acct WHERE code = 'IFRS-L-AP-RETENTION';
    SELECT id INTO v_ap_accrued    FROM _acct WHERE code = 'IFRS-L-AP-ACCRUED';
    SELECT id INTO v_tax_wht       FROM _acct WHERE code = 'IFRS-L-TAX-WHT';
    SELECT id INTO v_emp_sal_pay   FROM _acct WHERE code = 'IFRS-L-EMP-SAL';
    SELECT id INTO v_emp_bonus_pay FROM _acct WHERE code = 'IFRS-L-EMP-BONUS';
    SELECT id INTO v_emp_eos_pay   FROM _acct WHERE code = 'IFRS-L-EMP-EOS';
    SELECT id INTO v_r_goods       FROM _acct WHERE code = 'IFRS-R-SALES-GOODS';
    SELECT id INTO v_e_cogs_mat    FROM _acct WHERE code = 'IFRS-E-COGS-MAT';
    SELECT id INTO v_e_cogs_frgt   FROM _acct WHERE code = 'IFRS-E-COGS-FREIGHT';
    SELECT id INTO v_e_hr_sal      FROM _acct WHERE code = 'IFRS-E-HR-SAL';
    SELECT id INTO v_e_hr_eos      FROM _acct WHERE code = 'IFRS-E-HR-EOS';
    SELECT id INTO v_e_hr_bonus    FROM _acct WHERE code = 'IFRS-E-HR-BONUS';
    SELECT id INTO v_e_ga_office   FROM _acct WHERE code = 'IFRS-E-GA-OFFICE';
    SELECT id INTO v_e_ga_consult  FROM _acct WHERE code = 'IFRS-E-GA-CONSULT';
    SELECT id INTO v_e_ga_audit    FROM _acct WHERE code = 'IFRS-E-GA-AUDIT';
    SELECT id INTO v_e_da_bldg     FROM _acct WHERE code = 'IFRS-E-DA-BLDG';
    SELECT id INTO v_e_da_it       FROM _acct WHERE code = 'IFRS-E-DA-IT';
    SELECT id INTO v_q_re_open     FROM _acct WHERE code = 'IFRS-Q-RE-OPENING';
    SELECT id INTO v_q_cap_issued  FROM _acct WHERE code = 'IFRS-Q-CAP-ISSUED';
    SELECT id INTO v_l_accr_gen    FROM _acct WHERE code = 'IFRS-L-ACCR-GEN';

    IF v_cash_local IS NULL OR v_ar_trade IS NULL OR v_ap_trade IS NULL THEN
        RAISE EXCEPTION '[500_je] Core IFRS GL accounts not resolved — run 211_framework_ifrs_accounts.sql first';
    END IF;
