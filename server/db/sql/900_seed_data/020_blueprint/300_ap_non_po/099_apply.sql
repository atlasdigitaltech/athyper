-- ============================================================================
-- FILE: blueprint/099_apply.sql
-- Purpose: Record application of pack_ap_non_po per tenant + final validation
-- Depends on: all prior 010..090 files successfully executed
...
BEGIN
    ...
    FOR v_tenant IN
        SELECT id AS tenant_id, code AS tenant_code
          FROM master.tenant WHERE status = 'active'
    LOOP
        ...
        SELECT count(*) INTO v_rule_count
         ...
         WHERE tenant_id = v_tenant.tenant_id
           AND resolved_intent_id IN (
                SELECT id FROM master.business_intent
                 WHERE tenant_id = v_tenant.tenant_id
                   AND code IN ('OPEX_GENERAL','CAPEX_GENERAL','ADMIN_GENERAL'));
        ...
        SELECT count(*) INTO v_iprr_count
          ...
          JOIN master.accounting_profile ap ON ap.id = apc.accounting_profile_id
         WHERE iprr.tenant_id = v_tenant.tenant_id
           AND ap.code IN ('AP_NON_PO_STANDARD','AP_NON_PO_CAPEX',
                           'AP_ADVANCE_VENDOR','AP_RETENTION_RELEASE');
        ...
        INSERT INTO control.tenant_blueprint_application (
            ...
            v_tenant.tenant_id, 'pack_ap_non_po', v_version,
            now(), v_sys,
            'applied',
        ...
        IF cardinality(v_warnings) > 0 THEN
            RAISE WARNING 'pack_ap_non_po applied with warnings for tenant %: %',
                v_tenant.tenant_code, array_to_string(v_warnings, '; ');
        ELSE
            RAISE NOTICE 'pack_ap_non_po applied cleanly for tenant % (% profiles, % configs, % events, % templates, % rules)',
                v_tenant.tenant_code, v_profile_count, v_config_count,
