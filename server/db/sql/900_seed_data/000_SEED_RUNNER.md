...
TIER 1 — Foundation        (always apply, no selection)
  [1/5] base               universal spend taxonomy
  [2/5] foundation_tax     tax jurisdictions, types, rates, FX
  [3/5] foundation_pay     holiday calendars, payment terms
  [4/5] foundation_assets  asset class hierarchy
  [5/5] foundation_bank    bank format & payment rail rules
...
#### Step 2 — TIER 1: Foundation (always apply all 5, in this order)

```sql
SET app.seed_tenant_id = '<tenant_uuid>';

-- [1/5] base — universal spend taxonomy (prerequisite)
...
-- [5/5] foundation_bank — payment rail format rules
...
#### Step 5 — Tenant Instance Data (after all blueprint tiers complete)

Order is strict within each client folder.
...
040_tenants/{client}/100_org_structure/300_operating_units.sql
040_tenants/{client}/100_org_structure/301_cost_centers.sql
040_tenants/{client}/100_org_structure/302_profit_centers.sql
...
| Rule | Detail |
|---|---|
| Foundation always applied | All 5 foundation packs run automatically — no choice |
| One COA framework | Select exactly one: `coa_ifrs` (gaap, mfrs planned) |
