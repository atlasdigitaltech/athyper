...
-- Version : 3.0.0    (comprehensive coverage — 2025-Q4)
--
-- ── Rationale ────────────────────────────────────────────────────────────────
...
--
-- 2) FY2026 Q1 PERIOD_END (Jan 31, Feb 28, Mar 31) for ALL 150 active
--    currencies. This is the "comprehensive layer": every ISO 4217 currency
--    in shared.currency (excluding MYR itself, deprecated HRK/ZWL, and the
--    non-monetary ISO codes XAU/XAG/XPT/XPD/XDR) gets full Q1 coverage so
--    that any new tenant in any country has working FX.
--
-- 3) SPOT rate (effective today, anchored at 2026-03-31) for all 150
--    currencies — for live transaction posting (purchase invoices, sales
--    invoices, payments) where rate_type='SPOT' is the default.
--
-- ── Coverage summary ─────────────────────────────────────────────────────────
--    FY2025 quarterly PERIOD_END :  5 dates × 13 ccy =  65 rows
--    FY2026 Q1 PERIOD_END        :  3 dates × 150 ccy = 450 rows
--    SPOT (today)                :  1 date  × 150 ccy = 150 rows
--    ──────────────────────────────────────────────  ───────
--    TOTAL                                              665 rows
--
...
--
-- ── Q1 2026 progression (all currencies) ─────────────────────────────────────
-- A small uniform appreciation of MYR is modelled across Q1 2026:
--    Jan 31 = Mar 31 × 1.00690   (≈ +0.69%)
--    Feb 28 = Mar 31 × 1.00230   (≈ +0.23%)
--    Mar 31 = anchor (also drives SPOT)
-- This matches the original USD progression (4.3800 → 4.3600 → 4.3500) and
-- is applied uniformly so all currencies tell a consistent story.
--
-- ── Idempotency ──────────────────────────────────────────────────────────────
...
DECLARE
    ...
    v_meta      jsonb := '{"_seed": {"pack": "330_org", "version": "3.0.0"}}'::jsonb;
    v_inserted  int;
    v_active    int;
    v_missing   text;
BEGIN
    ...
    -- ── LAYER 1 — FY2026 Q1 PERIOD_END for ALL 150 currencies (3 dates each) ─
    -- Generated from tmp_base × {Jan 31, Feb 28, Mar 31} with progression.
    INSERT INTO tmp_fx (from_curr, rate, eff_date, rate_type)
    SELECT
        b.from_curr,
        ROUND(
            b.rate * CASE d.eff_date
                WHEN DATE '2026-01-31' THEN 1.00690
                WHEN DATE '2026-02-28' THEN 1.00230
                ELSE 1.00000
            END,
            10
        ) AS rate,
    ...
    -- ── LAYER 2 — SPOT (current transaction rate, anchored at 2026-03-31) ────
    -- One row per currency. Mirrors the Mar 31 PERIOD_END anchor.
    INSERT INTO tmp_fx (from_curr, rate, eff_date, rate_type)
    SELECT b.from_curr, b.rate, DATE '2026-03-31', 'SPOT'
    FROM tmp_base b;
    ...
    -- A4: Exact total row count (665 = 65 FY2025 + 450 FY2026 Q1 + 150 SPOT)
    IF v_inserted != 665 THEN
        RAISE EXCEPTION '330 FAIL: expected exactly 665 FX rates, inserted %', v_inserted;
    END IF;

    -- A5: Sub-counts by rate_type
    IF (SELECT count(*) FROM master.fx_rate
        WHERE tenant_id = v_tid AND metadata->'_seed'->>'pack' = '330_org'
          AND rate_type = 'PERIOD_END') != 515
    THEN
        RAISE EXCEPTION '330 FAIL: expected 515 PERIOD_END rates, got %',
    ...
    RAISE NOTICE '330: % FX rates inserted (% PERIOD_END, % SPOT) covering % active currencies → MYR (Q1 2026 + SPOT comprehensive; FY2025 quarterly for 13 demo-driver currencies)',
        v_inserted,
        (SELECT count(*) FROM master.fx_rate
         WHERE tenant_id = v_tid AND metadata->'_seed'->>'pack' = '330_org'
           AND rate_type = 'PERIOD_END'),
