...
-- R7-B: primary lookup — all suppliers for a company × jurisdiction × year
CREATE INDEX IF NOT EXISTS wva_company_jur_year_idx
    ON aggregate.wht_supplier_accumulator
    (tenant_id, company_code_id, jurisdiction_id, tax_type_id, fiscal_year);

-- Partial: find suppliers that have not yet reached threshold (for threshold-check query)
