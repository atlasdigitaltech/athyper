...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'athyperapp') THEN
        ...
        GRANT SELECT, INSERT, UPDATE ON event.notification_outbox_state TO athyperapp;
...
REVOKE ALL ON FUNCTION event.fn_notification_claim_deliveries(uuid, text, integer, integer) FROM PUBLIC;
...
END;
$$;
REVOKE ALL ON event.invalidation_dead_letter FROM PUBLIC;
GRANT SELECT,INSERT ON event.invalidation_dead_letter TO athyperadmin;
