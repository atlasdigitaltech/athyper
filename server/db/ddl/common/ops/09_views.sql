...
CREATE VIEW ops.identity_admission_shadow_gate AS
SELECT plane_code,workflow,resolver_revision,
  count(*) AS comparison_count,
  count(*) FILTER(WHERE comparison_status='match') AS match_count,
  count(*) FILTER(WHERE comparison_status='mismatch') AS mismatch_count,
  count(*) FILTER(WHERE comparison_status='candidate_error') AS candidate_error_count,
  count(*) FILTER(WHERE severity='critical') AS critical_count,
  min(observed_at) AS window_started_at,max(observed_at) AS window_ended_at,
  bool_and(comparison_status='match') AS zero_mismatch
FROM ops.identity_admission_shadow_comparison
GROUP BY plane_code,workflow,resolver_revision;
COMMENT ON VIEW ops.identity_admission_shadow_gate IS 'Per-plane and workflow evidence gate. Cutover requires a reviewed observation window with zero mismatches and candidate errors.';
