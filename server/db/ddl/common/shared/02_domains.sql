...
CREATE DOMAIN shared.classification_kind_d AS text
    CHECK (VALUE IN ('commodity', 'industry'));

COMMENT ON DOMAIN shared.classification_kind_d IS
  'Sealed classification family for shared.classification_scheme.';

CREATE DOMAIN shared.uom_quantity_type_d AS text
    CHECK (VALUE IN (
        'angle',
        'area',
        'concentration',
...
COMMENT ON DOMAIN shared.uom_quantity_type_d IS
  'Seed-owned physical or commercial quantity family used to classify UN/ECE units.';
