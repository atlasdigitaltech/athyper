...
CREATE TABLE shared.country (
    ...
    CONSTRAINT country_metadata_object_chk CHECK (jsonb_typeof(metadata) = 'object'),
    CONSTRAINT country_status_audit_pair_chk
        CHECK ((status_changed_at IS NULL) = (status_changed_by IS NULL)),
    CONSTRAINT country_update_audit_pair_chk
...
CREATE TABLE shared.currency (
    ...
    CONSTRAINT currency_metadata_object_chk CHECK (jsonb_typeof(metadata) = 'object'),
    CONSTRAINT currency_status_audit_pair_chk
        CHECK ((status_changed_at IS NULL) = (status_changed_by IS NULL)),
    CONSTRAINT currency_update_audit_pair_chk
        CHECK ((updated_at IS NULL) = (updated_by IS NULL))
);
...
CREATE TABLE shared.language (
    ...
    CONSTRAINT language_metadata_object_chk CHECK (jsonb_typeof(metadata) = 'object'),
    CONSTRAINT language_status_audit_pair_chk
        CHECK ((status_changed_at IS NULL) = (status_changed_by IS NULL)),
    CONSTRAINT language_update_audit_pair_chk
        CHECK ((updated_at IS NULL) = (updated_by IS NULL))
);
...
CREATE TABLE shared.locale (
    ...
    CONSTRAINT locale_dir_chk        CHECK (direction = ANY (ARRAY['ltr', 'rtl'])),
    CONSTRAINT locale_name_nonempty  CHECK (btrim(name) <> ''),
    CONSTRAINT locale_metadata_object_chk CHECK (jsonb_typeof(metadata) = 'object'),
    CONSTRAINT locale_status_audit_pair_chk
        CHECK ((status_changed_at IS NULL) = (status_changed_by IS NULL)),
    CONSTRAINT locale_update_audit_pair_chk
        CHECK ((updated_at IS NULL) = (updated_by IS NULL))
);
...
CREATE TABLE shared.timezone (
    ...
    CONSTRAINT timezone_name_chk
        CHECK (name IS NULL OR btrim(name) <> ''),
    CONSTRAINT timezone_metadata_object_chk CHECK (jsonb_typeof(metadata) = 'object'),
    CONSTRAINT timezone_status_audit_pair_chk
        CHECK ((status_changed_at IS NULL) = (status_changed_by IS NULL)),
    CONSTRAINT timezone_update_audit_pair_chk
        CHECK ((updated_at IS NULL) = (updated_by IS NULL))
);
...
COMMENT ON COLUMN shared.timezone.utc_offset_minutes IS
  'Standard non-DST display hint only. Business calculations must use the IANA code with AT TIME ZONE.';
...
CREATE TABLE shared.uom (
    ...
    -- Table-specific
    symbol          text,
    quantity_type   shared.uom_quantity_type_d NOT NULL,
    ...
    CONSTRAINT uom_metadata_object_chk CHECK (jsonb_typeof(metadata) = 'object'),
    CONSTRAINT uom_status_audit_pair_chk
        CHECK ((status_changed_at IS NULL) = (status_changed_by IS NULL)),
    CONSTRAINT uom_update_audit_pair_chk
        CHECK ((updated_at IS NULL) = (updated_by IS NULL))
);

COMMENT ON TABLE shared.uom IS
  'ARCHETYPE=A;SCOPE=N. UN/ECE Rec 20 unit-of-measure register. PK: uuidv7 id. Natural key: 1-4 char code. quantity_type uses the sealed shared domain.';

-- §7 state_region
CREATE TABLE shared.state_region (
    -- Identity
    id              uuid              NOT NULL DEFAULT shared.uuidv7(),
    code            text              NOT NULL,
    name            text              NOT NULL,
    ...
    CONSTRAINT state_region_category_chk
        CHECK (category IS NULL OR btrim(category) <> ''),
    CONSTRAINT state_region_metadata_object_chk CHECK (jsonb_typeof(metadata) = 'object'),
    CONSTRAINT state_region_status_audit_pair_chk
        CHECK ((status_changed_at IS NULL) = (status_changed_by IS NULL)),
    CONSTRAINT state_region_update_audit_pair_chk
        CHECK ((updated_at IS NULL) = (updated_by IS NULL))
);
...
CREATE TABLE shared.classification_scheme (
    id                  uuid                          NOT NULL DEFAULT shared.uuidv7(),
    code                text                          NOT NULL,
    scheme_kind         shared.classification_kind_d  NOT NULL,
    name                text                          NOT NULL,
    ...
    CONSTRAINT classification_scheme_code_uq UNIQUE (code),
    CONSTRAINT classification_scheme_code_fmt
        CHECK (code ~ '^[a-z][a-z0-9_.-]{1,62}$'),
    CONSTRAINT classification_scheme_name_chk CHECK (btrim(name) <> ''),
    CONSTRAINT classification_scheme_publisher_chk CHECK (btrim(publisher) <> ''),
    CONSTRAINT classification_scheme_edition_chk CHECK (btrim(edition) <> ''),
    CONSTRAINT classification_scheme_uri_chk
        CHECK (reference_uri IS NULL OR reference_uri ~ '^https?://'),
    CONSTRAINT classification_scheme_metadata_object_chk
        CHECK (jsonb_typeof(metadata) = 'object'),
    CONSTRAINT classification_scheme_status_audit_pair_chk
        CHECK ((status_changed_at IS NULL) = (status_changed_by IS NULL)),
    CONSTRAINT classification_scheme_update_audit_pair_chk
        CHECK ((updated_at IS NULL) = (updated_by IS NULL))
);

COMMENT ON TABLE shared.classification_scheme IS
  'Seed-owned registry of commodity and industry classification systems and editions.';

-- §8 commodity_code
CREATE TABLE shared.commodity_code (
    -- Identity
    id              uuid              NOT NULL DEFAULT shared.uuidv7(),
    code            text              NOT NULL,
    name            text              NOT NULL,
    ...
    CONSTRAINT commodity_code_domain_fmt
    ...
    CONSTRAINT commodity_code_level_chk       CHECK (level_no >= 1),
    CONSTRAINT commodity_code_root_chk        CHECK (
        (parent_code IS NULL AND level_no = 1)
        OR (parent_code IS NOT NULL AND level_no > 1)
    ),
    CONSTRAINT commodity_code_name_nonempty   CHECK (btrim(name) <> ''),
    CONSTRAINT commodity_code_description_chk
        CHECK (description IS NULL OR btrim(description) <> ''),
    CONSTRAINT commodity_code_metadata_object_chk CHECK (jsonb_typeof(metadata) = 'object'),
    CONSTRAINT commodity_code_status_audit_pair_chk
        CHECK ((status_changed_at IS NULL) = (status_changed_by IS NULL)),
    CONSTRAINT commodity_code_update_audit_pair_chk
        CHECK ((updated_at IS NULL) = (updated_by IS NULL))
);
...
-- §9 industry_code
CREATE TABLE shared.industry_code (
    -- Identity
    id              uuid              NOT NULL DEFAULT shared.uuidv7(),
    code            text              NOT NULL,
    name            text              NOT NULL,
    ...
    CONSTRAINT industry_code_domain_fmt
        CHECK (domain_code ~ '^[a-z][a-z0-9_.-]{1,62}$'),
    ...
    CONSTRAINT industry_code_level_chk       CHECK (level_no >= 1),
    CONSTRAINT industry_code_root_chk        CHECK (
        (parent_code IS NULL AND level_no = 1)
        OR (parent_code IS NOT NULL AND level_no > 1)
    ),
    CONSTRAINT industry_code_name_nonempty   CHECK (btrim(name) <> ''),
    CONSTRAINT industry_code_description_chk
        CHECK (description IS NULL OR btrim(description) <> ''),
    CONSTRAINT industry_code_metadata_object_chk CHECK (jsonb_typeof(metadata) = 'object'),
    CONSTRAINT industry_code_status_audit_pair_chk
        CHECK ((status_changed_at IS NULL) = (status_changed_by IS NULL)),
    CONSTRAINT industry_code_update_audit_pair_chk
        CHECK ((updated_at IS NULL) = (updated_by IS NULL))
);
...
-- Reverse tenant routing resolves through Neon master.commodity_code_assignment
-- category rows marked is_code_routing_default.
-- FKs to shared.commodity_code (source + target) → 06_constraints/001_shared.sql
-- Indexes → 07_indexes/001_shared.sql
-- =============================================================================

CREATE TABLE shared.commodity_crosswalk (
    -- Identity
    id                  uuid        NOT NULL DEFAULT shared.uuidv7(),
    ...
    CONSTRAINT ccw_verified_chk
        CHECK (is_verified = (verified_by IS NOT NULL AND verified_at IS NOT NULL)),
    CONSTRAINT ccw_notes_chk CHECK (notes IS NULL OR btrim(notes) <> ''),
    CONSTRAINT ccw_metadata_object_chk CHECK (jsonb_typeof(metadata) = 'object'),
    CONSTRAINT ccw_status_audit_pair_chk
        CHECK ((status_changed_at IS NULL) = (status_changed_by IS NULL)),
    CONSTRAINT ccw_update_audit_pair_chk
        CHECK ((updated_at IS NULL) = (updated_by IS NULL))
);
...
CREATE TABLE shared.industry_crosswalk (
    -- Identity
    id                  uuid        NOT NULL DEFAULT shared.uuidv7(),
    ...
    CONSTRAINT icw_confidence_chk    CHECK (confidence IS NULL
        OR (confidence >= 0 AND confidence <= 100)),
    CONSTRAINT icw_verified_chk
        CHECK (is_verified = (verified_by IS NOT NULL AND verified_at IS NOT NULL)),
    CONSTRAINT icw_notes_chk CHECK (notes IS NULL OR btrim(notes) <> ''),
    CONSTRAINT icw_metadata_object_chk CHECK (jsonb_typeof(metadata) = 'object'),
    CONSTRAINT icw_status_audit_pair_chk
        CHECK ((status_changed_at IS NULL) = (status_changed_by IS NULL)),
    CONSTRAINT icw_update_audit_pair_chk
        CHECK ((updated_at IS NULL) = (updated_by IS NULL))
);
