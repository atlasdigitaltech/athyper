...
ALTER TABLE authz.permission_scope_policy ENABLE ROW LEVEL SECURITY;
ALTER TABLE authz.permission_scope_policy FORCE ROW LEVEL SECURITY;
...
CREATE POLICY published_read ON authz.permission_scope_policy
    FOR SELECT USING (
        EXISTS (
            SELECT 1
              FROM authz.permission
             WHERE permission.id = permission_scope_policy.permission_id
               AND permission.status = 'published'
        )
    );
CREATE POLICY seed_write ON authz.permission_scope_policy
    FOR ALL TO CURRENT_USER USING (true) WITH CHECK (true);
