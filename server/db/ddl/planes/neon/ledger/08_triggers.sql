...
CREATE TRIGGER trg_budget_transaction_05_reversal
BEFORE INSERT ON ledger.budget_transaction
FOR EACH ROW EXECUTE FUNCTION ledger.trg_validate_budget_reversal();
...
CREATE TRIGGER trg_budget_balance_15_coordinate
BEFORE UPDATE ON ledger.budget_balance
FOR EACH ROW EXECUTE FUNCTION ledger.trg_guard_budget_balance_identity();
...
CREATE TRIGGER trg_planning_run_70_transition
BEFORE UPDATE OF status ON ledger.planning_run
FOR EACH ROW EXECUTE FUNCTION ledger.trg_enforce_planning_run_transition();
...
FOR EACH ROW EXECUTE FUNCTION master.trg_set_master_created_by();
CREATE TRIGGER trg_planning_output_05_run
BEFORE INSERT ON ledger.planning_output
FOR EACH ROW EXECUTE FUNCTION ledger.trg_validate_planning_output_run();
