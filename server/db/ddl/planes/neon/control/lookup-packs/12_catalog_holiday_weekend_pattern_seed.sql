...
-- seed-pack: neon.control.lookup.catalog_holiday_weekend_pattern
-- seed-pack-version: 1.0.0
-- seed-dataset: neon.control.lookup.catalog_holiday_weekend_pattern
...
-- source-files: server/db/seed/platform/000_lookups/LookupDomain/master/holiday_weekend_pattern.sql

DO $guard$ BEGIN
  IF current_setting('app.database_plane', true) <> 'neon' THEN
    RAISE EXCEPTION 'neon.control.lookup.catalog_holiday_weekend_pattern: invalid database plane';
  END IF;
END $guard$;
...
VALUES
  ('sat_sun', 'Saturday & Sunday', 'master.holiday_weekend_pattern', 'Standard Western weekend (Sat + Sun)', NULL, 10, true, '{}'::jsonb, 'active', '00000000-0000-0000-0000-000000000000'::uuid),
  ('fri_sat', 'Friday & Saturday', 'master.holiday_weekend_pattern', 'GCC / Middle East weekend (Fri + Sat)', NULL, 20, true, '{}'::jsonb, 'active', '00000000-0000-0000-0000-000000000000'::uuid),
...
DO $assertions$ BEGIN
  IF (SELECT count(*) FROM control.lookup_value WHERE tenant_id IS NULL AND domain_code = ANY(ARRAY['master.holiday_weekend_pattern'])) <> 6 THEN
    RAISE EXCEPTION 'neon.control.lookup.catalog_holiday_weekend_pattern: expected-count assertion failed';
  END IF;
  IF EXISTS (SELECT 1 FROM control.lookup_value v LEFT JOIN control.lookup_domain d ON d.code=v.domain_code WHERE v.tenant_id IS NULL AND v.domain_code = ANY(ARRAY['master.holiday_weekend_pattern']) AND d.id IS NULL) THEN
    RAISE EXCEPTION 'neon.control.lookup.catalog_holiday_weekend_pattern: orphan assertion failed';
  END IF;
  IF EXISTS (SELECT domain_code, code FROM control.lookup_value WHERE tenant_id IS NULL AND domain_code = ANY(ARRAY['master.holiday_weekend_pattern']) GROUP BY domain_code, code HAVING count(*) > 1) THEN
    RAISE EXCEPTION 'neon.control.lookup.catalog_holiday_weekend_pattern: uniqueness assertion failed';
  END IF;
  IF EXISTS (SELECT 1 FROM control.lookup_value WHERE tenant_id IS NULL AND domain_code = ANY(ARRAY['master.holiday_weekend_pattern']) AND (code <> lower(btrim(code)) OR btrim(name) = '' OR status NOT IN ('active','inactive','deprecated'))) THEN
    RAISE EXCEPTION 'neon.control.lookup.catalog_holiday_weekend_pattern: semantic assertion failed';
  END IF;
END $assertions$;
