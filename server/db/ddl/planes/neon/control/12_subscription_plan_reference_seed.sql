...
VALUES
  (md5('neon:subscription-plan:finance_free')::uuid,'finance_free','Finance Free',5,10,'{"_seed":{"pack":"neon.subscription-plans","version":"2.0.0"}}','active','00000000-0000-0000-0000-000000000000'),
  (md5('neon:subscription-plan:erp_enterprise')::uuid,'erp_enterprise','ERP Enterprise',NULL,20,'{"_seed":{"pack":"neon.subscription-plans","version":"2.0.0"}}','active','00000000-0000-0000-0000-000000000000')
...
WHERE code NOT IN ('finance_free','erp_enterprise')
  AND metadata #>> '{_seed,pack}' = 'neon.subscription-plans'
  AND status <> 'deprecated';

DO $assertions$ BEGIN
  IF (SELECT count(*) FROM control.subscription_plan WHERE status='active') <> 2 THEN
    RAISE EXCEPTION 'neon subscription plan count mismatch'; END IF;
END $assertions$;
