...
CREATE TABLE document.attachment_quota_usage (
    tenant_id uuid NOT NULL, quota_kind text NOT NULL,
    used_bytes bigint NOT NULL DEFAULT 0, reserved_bytes bigint NOT NULL DEFAULT 0,
    used_items bigint NOT NULL DEFAULT 0, reserved_items bigint NOT NULL DEFAULT 0,
    created_at timestamptz NOT NULL DEFAULT now(), created_by uuid NOT NULL,
    updated_at timestamptz, updated_by uuid,
    PRIMARY KEY (tenant_id, quota_kind),
    CHECK (quota_kind ~ '^[a-z][a-z0-9_.]{1,126}$'),
...
CREATE TABLE document.attachment_quota_reservation (
    id uuid NOT NULL DEFAULT shared.uuidv7(), tenant_id uuid NOT NULL, quota_kind text NOT NULL,
    resource_type text NOT NULL, resource_id uuid NOT NULL, reserved_bytes bigint NOT NULL,
    reserved_items bigint NOT NULL DEFAULT 1, actual_bytes bigint, status text NOT NULL DEFAULT 'reserved',
    ...
    CHECK (reserved_bytes >= 0 AND reserved_items > 0 AND (actual_bytes IS NULL OR actual_bytes >= 0)),
...
CREATE TABLE document.content_quota_usage (
    tenant_id uuid NOT NULL, quota_kind text NOT NULL, used_items bigint NOT NULL DEFAULT 0,
    reserved_items bigint NOT NULL DEFAULT 0, created_at timestamptz NOT NULL DEFAULT now(),
    created_by uuid NOT NULL, updated_at timestamptz, updated_by uuid,
    PRIMARY KEY (tenant_id,quota_kind), CHECK (used_items>=0 AND reserved_items>=0)
);
CREATE TABLE document.content_quota_reservation (
    id uuid NOT NULL DEFAULT shared.uuidv7(), tenant_id uuid NOT NULL, quota_kind text NOT NULL,
    content_item_id uuid NOT NULL, reserved_items bigint NOT NULL DEFAULT 1,
    status text NOT NULL DEFAULT 'reserved', expires_at timestamptz NOT NULL,
    committed_at timestamptz, released_at timestamptz, created_at timestamptz NOT NULL DEFAULT now(),
    created_by uuid NOT NULL, updated_at timestamptz, updated_by uuid,
    PRIMARY KEY(id), UNIQUE(tenant_id,quota_kind,content_item_id),
    CHECK(reserved_items>0), CHECK(status IN ('reserved','committed','released','expired'))
);
