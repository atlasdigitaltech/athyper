...
-- seed-pack: mesh.control-platform-catalog
...
-- seed-expected-row-count: query:wave2_mesh_control_catalog
...
DO $guard$ BEGIN IF current_setting('app.database_plane',true) <> 'mesh' THEN
  RAISE EXCEPTION 'control catalog pack requires app.database_plane=mesh'; END IF; END $guard$;
...
SELECT m.workspace_id,m.id,true,row_number() OVER (PARTITION BY m.workspace_id ORDER BY m.code)::smallint,
       '{"_seed":{"pack":"mesh.control-platform-catalog","version":"2.0.0"}}'::jsonb,'active','00000000-0000-0000-0000-000000000000'::uuid
...
DO $assertions$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM master.workspace m JOIN control.workspace c USING(id) WHERE m.code<>c.code)
    ...
    RAISE EXCEPTION 'mesh control catalog failed UUID-preserving backfill'; END IF;
END $assertions$;
