...
-- seed-id-strategy: deterministic-uuid:athyper-wave4-mesh-subscription-plan-v1
...
SELECT md5('mesh:subscription-plan:' || v.code)::uuid,v.code,v.name,v.max_users,v.sort_order,
       '{"_seed":{"pack":"mesh.subscription-plans","version":"1.0.0"}}'::jsonb,'active','00000000-0000-0000-0000-000000000000'::uuid
FROM (VALUES
...
DO $assertions$ BEGIN IF (SELECT count(*) FROM control.subscription_plan WHERE status='active') <> 5 THEN
  RAISE EXCEPTION 'mesh subscription plan count mismatch'; END IF; END $assertions$;
