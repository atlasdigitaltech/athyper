...
BEGIN
    IF TG_TABLE_NAME = 'permission' THEN
        ...
        NEW.provenance_ref := btrim(NEW.provenance_ref);
...
BEGIN
    IF NEW.status = 'published'
       AND (TG_OP = 'INSERT' OR OLD.status IS DISTINCT FROM NEW.status) THEN
        ...
        IF NOT EXISTS (
            SELECT 1
              FROM authz.permission_scope_policy AS policy
             WHERE policy.permission_id = NEW.id
        ) THEN
            RAISE EXCEPTION
                'permission % cannot be published without at least one scope rule',
                NEW.canonical_code
                USING ERRCODE = 'check_violation';
        END IF;
        ...
        SELECT dependency_kind, dependency_id
          ...
          FROM (
                SELECT 'group_role'::text AS dependency_kind,
                       assignment.id AS dependency_id
                  FROM authz.role_permission AS role_permission
                  JOIN authz.group_role AS assignment
                    ON assignment.tenant_id = role_permission.tenant_id
                   AND assignment.role_id = role_permission.role_id
                  JOIN authz.scope_target AS target
                    ON target.tenant_id = assignment.tenant_id
                   AND target.id = assignment.scope_target_id
                 WHERE role_permission.permission_id = NEW.id
                   ...
                   AND NOT EXISTS (
                        SELECT 1
                          FROM authz.permission_scope_policy AS policy
                         WHERE policy.permission_id = NEW.id
                           AND policy.scope_kind = target.scope_kind
                   )
                UNION ALL
                SELECT 'delegation_grant', grant_row.id
                  ...
                  JOIN authz.scope_target AS target
                    ON target.tenant_id = grant_row.tenant_id
                   AND target.id = grant_row.scope_target_id
                 WHERE grant_row.permission_id = NEW.id
                   AND delegation.status IN ('pending', 'active')
                   AND (
                        NOT NEW.is_delegable
                        OR NOT EXISTS (
                            SELECT 1
                              FROM authz.permission_scope_policy AS policy
                             WHERE policy.permission_id = NEW.id
                ...
                UNION ALL
                SELECT 'deny_rule', deny_rule.id
                  FROM authz.deny_rule AS deny_rule
                  JOIN authz.scope_target AS target
                    ON target.tenant_id = deny_rule.tenant_id
                   AND target.id = deny_rule.scope_target_id
                 WHERE deny_rule.permission_id = NEW.id
                   AND deny_rule.status IN ('active', 'suspended')
                   AND NOT EXISTS (
                        SELECT 1
                          FROM authz.permission_scope_policy AS policy
                         WHERE policy.permission_id = NEW.id
                ...
                SELECT 'override', override_row.id
                  ...
                  JOIN authz.scope_target AS target
                    ON target.tenant_id = override_row.tenant_id
                   AND target.id = override_row.scope_target_id
                 WHERE override_row.permission_id = NEW.id
                   AND override_row.status IN ('pending', 'active')
                   AND (
                        NOT NEW.is_overridable
                        OR NOT EXISTS (
                            SELECT 1
                              FROM authz.permission_scope_policy AS policy
                             WHERE policy.permission_id = NEW.id
                ...
                SELECT 'record_acl', acl.id
                 ...
                 WHERE acl.permission_id = NEW.id
                   ...
                   AND (
                        NOT NEW.is_shareable
                        OR acl.resource_code <> NEW.resource_code
                        OR NOT EXISTS (
                            SELECT 1
                              FROM authz.permission_scope_policy AS policy
                             WHERE policy.permission_id = NEW.id
                               AND policy.scope_kind = 'resource'
                        )
                   )
          ) AS incompatible
         LIMIT 1;
...
CREATE OR REPLACE FUNCTION authz.trg_guard_permission_scope_policy()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = authz, pg_catalog
AS $$
DECLARE
    ...
    v_parent_status authz.catalog_status_d;
BEGIN
    IF TG_OP = 'UPDATE'
       AND (
           NEW.permission_id IS DISTINCT FROM OLD.permission_id
        ...
        RAISE EXCEPTION
            'permission scope-policy identity and creation evidence are immutable'
            USING ERRCODE = 'check_violation';
    END IF;

    v_permission_id := CASE
        WHEN TG_OP = 'DELETE' THEN OLD.permission_id
        ELSE NEW.permission_id
    END;
    ...
    IF v_parent_status IS NULL THEN
        RAISE EXCEPTION
            'permission scope-policy requires an existing permission'
            USING ERRCODE = 'foreign_key_violation';
    END IF;

    IF v_parent_status NOT IN ('draft', 'suspended') THEN
        RAISE EXCEPTION
            'Scope rules for permission % may change only while it is draft or suspended; current status is %',
            v_permission_id, v_parent_status
            USING ERRCODE = 'check_violation';
    END IF;
    ...
    SELECT EXISTS (
        SELECT 1
          ...
          JOIN authz.scope_target AS target
           ...
           AND target.scope_kind = policy.scope_kind
       ...
       AND (TG_OP = 'INSERT' OR OLD.status IS DISTINCT FROM NEW.status) THEN
        ...
        SELECT dependency_kind, dependency_id
          ...
          FROM (
                ...
                UNION ALL
                SELECT 'group_role', assignment.id
                  FROM authz.group_role AS assignment
                  JOIN authz.scope_target AS target
                    ON target.tenant_id = assignment.tenant_id
                   AND target.id = assignment.scope_target_id
                 ...
                 WHERE assignment.tenant_id = NEW.tenant_id
                   AND assignment.role_id = NEW.id
                   AND assignment.status IN ('active', 'suspended')
                   AND NOT EXISTS (
                        SELECT 1
                          FROM authz.permission_scope_policy AS policy
                         WHERE policy.permission_id = role_permission.permission_id
                           AND policy.scope_kind = target.scope_kind
                   )
          ) AS incompatible
         LIMIT 1;
...
BEGIN
    ...
    SELECT is_shareable, is_delegable, is_overridable, resource_code
      INTO v_shareable, v_delegable, v_overridable, v_resource
      FROM authz.permission
    ...
    IF TG_TABLE_NAME = 'record_acl' THEN
        IF NOT v_shareable THEN
            RAISE EXCEPTION 'record_acl permission is not shareable'
                USING ERRCODE = 'check_violation';
        END IF;
        IF NEW.resource_code <> v_resource THEN
            RAISE EXCEPTION
                'record_acl resource_code % does not match permission resource %',
                NEW.resource_code, v_resource
                USING ERRCODE = 'check_violation';
        END IF;
        IF NOT EXISTS (
            SELECT 1
              FROM authz.permission_scope_policy
             WHERE permission_id = NEW.permission_id
               AND scope_kind = 'resource'
        ) THEN
            RAISE EXCEPTION 'record_acl permission has no resource scope policy'
                USING ERRCODE = 'check_violation';
        END IF;
        RETURN NEW;
    END IF;
       ...
       AND (
            NOT EXISTS (
                SELECT 1
                  ...
                  JOIN authz.permission_scope_policy AS policy
                    ON policy.permission_id = grant_row.permission_id
                   AND policy.scope_kind = assignment_target.scope_kind
            ...
            OR EXISTS (
                SELECT 1
                  ...
                  JOIN authz.permission_scope_policy AS policy
                    ON policy.permission_id = deny.permission_id
                   AND policy.scope_kind = deny_target.scope_kind
