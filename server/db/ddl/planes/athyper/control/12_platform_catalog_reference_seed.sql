...
-- seed-pack: athyper.control-platform-catalog
...
-- seed-expected-row-count: query:wave2_athyper_control_catalog
...
DO $guard$ BEGIN IF current_setting('app.database_plane',true) <> 'athyper' THEN
  RAISE EXCEPTION 'control catalog pack requires app.database_plane=athyper'; END IF; END $guard$;

INSERT INTO control.workspace (
  id,code,name,description,sort_order,is_shared_infrastructure,metadata,status,
  status_changed_at,status_changed_by,created_at,created_by,updated_at,updated_by
)
SELECT id,code,name,description,sort_order,is_shared_infrastructure,metadata,status,
       status_changed_at,status_changed_by,created_at,created_by,updated_at,updated_by
FROM master.workspace
ON CONFLICT (code) DO UPDATE SET
  name=excluded.name,description=excluded.description,sort_order=excluded.sort_order,
  is_shared_infrastructure=excluded.is_shared_infrastructure,metadata=excluded.metadata,
  status=excluded.status,updated_at=now(),updated_by=excluded.created_by
WHERE (control.workspace.name,control.workspace.description,control.workspace.sort_order,control.workspace.is_shared_infrastructure,control.workspace.metadata,control.workspace.status)
...
INSERT INTO control.module (
  id,code,name,description,config,metadata,status,status_changed_at,status_changed_by,
  created_at,created_by,updated_at,updated_by
)
SELECT id,code,name,description,config,metadata,status,status_changed_at,status_changed_by,
       created_at,created_by,updated_at,updated_by
FROM master.module
ON CONFLICT (code) DO UPDATE SET
  name=excluded.name,description=excluded.description,config=excluded.config,
  metadata=excluded.metadata,status=excluded.status,updated_at=now(),updated_by=excluded.created_by
WHERE (control.module.name,control.module.description,control.module.config,control.module.metadata,control.module.status)
  IS DISTINCT FROM (excluded.name,excluded.description,excluded.config,excluded.metadata,excluded.status);

INSERT INTO control.workspace_module (
  workspace_id,module_id,is_primary,sort_order,metadata,status,created_by
)
SELECT m.workspace_id,m.id,true,
       row_number() OVER (PARTITION BY m.workspace_id ORDER BY m.code)::smallint,
       '{"_seed":{"pack":"athyper.control-platform-catalog","version":"2.0.0"}}'::jsonb,
       'active','00000000-0000-0000-0000-000000000000'::uuid
FROM master.module m
...
WHERE (control.workspace_module.is_primary,control.workspace_module.sort_order,control.workspace_module.metadata,control.workspace_module.status)
  IS DISTINCT FROM (excluded.is_primary,excluded.sort_order,excluded.metadata,excluded.status);
...
DO $assertions$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM master.workspace m JOIN control.workspace c USING(id) WHERE m.code<>c.code)
    ...
    RAISE EXCEPTION 'athyper control catalog failed UUID-preserving backfill';
  END IF;
END $assertions$;
