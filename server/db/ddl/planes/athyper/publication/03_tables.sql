...
CREATE TABLE publication.artifact (
    ...
    CONSTRAINT publication_artifact_plane_chk CHECK (plane_code IN ('athyper','neon','mesh')),
    ...
    CONSTRAINT publication_artifact_signature_chk CHECK (btrim(signature_algorithm) <> '' AND btrim(signing_key_id) <> '' AND btrim(signature) <> '')
);
...
CREATE TABLE publication.deployment (
    ...
    CONSTRAINT publication_deployment_plane_chk CHECK (target_plane IN ('athyper','neon','mesh')),
