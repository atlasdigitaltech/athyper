...
-- seed-expected-row-count: query:wave4_athyper_platform_catalog
...
VALUES
    (md5('athyper:workspace:' || 'core')::uuid, 'core', 'Core Platform', 'Meta-driven runtime, authentication, and platform services', 10, true, '{"_seed":{"pack":"athyper.platform-catalog","version":"1.0.0"}}'::jsonb, 'active', '00000000-0000-0000-0000-000000000000'::uuid)
...
SELECT md5('athyper:module:' || seed_rows.code)::uuid, seed_rows.code,seed_rows.name,seed_rows.description,workspace.id,seed_rows.config,
       '{"_seed":{"pack":"athyper.platform-catalog","version":"1.0.0"}}'::jsonb,'active','00000000-0000-0000-0000-000000000000'::uuid
FROM (VALUES
...
DO $assertions$ BEGIN
  IF (SELECT count(*) FROM master.workspace WHERE status='active') <> 1
     OR (SELECT count(*) FROM master.module WHERE status='active') <> 13 THEN
    RAISE EXCEPTION 'athyper platform catalog count mismatch';
  END IF;
  IF EXISTS (SELECT 1 FROM master.workspace WHERE code <> lower(code))
    ...
    RAISE EXCEPTION 'athyper platform catalog contains non-canonical codes';
  END IF;
  IF EXISTS (SELECT 1 FROM master.module m LEFT JOIN master.workspace w ON w.id=m.workspace_id WHERE w.id IS NULL) THEN
    RAISE EXCEPTION 'athyper platform catalog contains orphan modules';
  END IF;
END $assertions$;
