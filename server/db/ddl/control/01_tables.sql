...
-- ============================================================================
-- §ACBPT  control.asset_class_book_policy_template — onboarding template
-- ============================================================================
-- Platform/tenant template rows for provisioning concrete company/class/book
-- policies. book_category is abstract here and resolves to real ledger book
-- codes during control.provision_asset_policies().
-- ============================================================================

CREATE TABLE IF NOT EXISTS control.asset_class_book_policy_template (
    -- Identity
    id               uuid         NOT NULL DEFAULT shared.uuidv7(),
    ...
    -- Capitalization threshold multiplier. Provisioning multiplies this by
    -- the company currency's base threshold.
    capitalization_threshold_multiplier numeric(9,4) NOT NULL DEFAULT 1,
    ...
    CONSTRAINT acbpt_pkey PRIMARY KEY (id),
    CONSTRAINT acbpt_scope_uq UNIQUE NULLS NOT DISTINCT
        (tenant_id, template_code, asset_class_code, book_category, effective_from),

    CONSTRAINT acbpt_template_code_chk CHECK (btrim(template_code) <> ''),
    CONSTRAINT acbpt_framework_chk CHECK (btrim(framework) <> ''),
    CONSTRAINT acbpt_class_code_chk CHECK (btrim(asset_class_code) <> ''),
    CONSTRAINT acbpt_book_category_chk CHECK (btrim(book_category) <> ''),
    CONSTRAINT acbpt_threshold_mult_chk CHECK (capitalization_threshold_multiplier >= 0),
    CONSTRAINT acbpt_effective_chk CHECK (
        effective_to IS NULL OR effective_to >= effective_from),
    CONSTRAINT acbpt_depr_method_chk CHECK (
        NOT is_depreciable OR depreciation_method IS NOT NULL),
    CONSTRAINT acbpt_depr_life_chk CHECK (
        NOT is_depreciable OR depreciation_method = 'no_depreciation'
        OR useful_life_months IS NOT NULL),
    CONSTRAINT acbpt_nodepr_life_chk CHECK (
        depreciation_method IS NULL OR depreciation_method != 'no_depreciation'
        OR useful_life_months IS NULL),
    CONSTRAINT acbpt_useful_life_pos CHECK (
        useful_life_months IS NULL OR useful_life_months > 0),
    CONSTRAINT acbpt_life_range_chk CHECK (
        (useful_life_min_months IS NULL AND useful_life_max_months IS NULL)
        OR (
            useful_life_min_months IS NOT NULL
            AND useful_life_max_months IS NOT NULL
            AND useful_life_min_months > 0
            AND useful_life_max_months >= useful_life_min_months
            AND (
                useful_life_months IS NULL
                OR useful_life_months BETWEEN useful_life_min_months AND useful_life_max_months
            )
        )),
    CONSTRAINT acbpt_residual_mode_chk CHECK (
        residual_value_mode IN ('amount','percent','zero')),
    CONSTRAINT acbpt_residual_amount_chk CHECK (
        residual_value_mode != 'amount'
        OR (residual_value_amount IS NOT NULL AND residual_value_amount >= 0)),
    CONSTRAINT acbpt_residual_pct_chk CHECK (
        residual_value_mode != 'percent'
        OR (residual_value_pct IS NOT NULL AND residual_value_pct BETWEEN 0 AND 100)),
    CONSTRAINT acbpt_residual_zero_chk CHECK (
        residual_value_mode != 'zero'
        OR (residual_value_amount IS NULL AND residual_value_pct IS NULL)),
    CONSTRAINT acbpt_start_rule_chk CHECK (
        depreciation_start_rule IN ('in_service_date','capitalization_date','next_period')),
    CONSTRAINT acbpt_depr_roles_chk CHECK (
    ...
    CONSTRAINT acbpt_acq_role_chk CHECK (
        acquisition_posting_role_code IS NOT NULL
        OR cwip_posting_role_code IS NOT NULL),
    CONSTRAINT acbpt_status_chk CHECK (status IN ('active','inactive','superseded'))
);

COMMENT ON TABLE control.asset_class_book_policy_template IS
    'ARCHETYPE=B;SCOPE=G. Platform/tenant template for asset_class_book_policy onboarding. '
    'tenant_id=NULL rows are platform global defaults; tenant rows may override. '
    'book_category is resolved to a real assigned ledger_book.code by '
    'control.provision_asset_policies(). Useful-life min/default/max and source_note '
    'capture best-practice policy guidance without adding a separate reference table.';
...
CREATE TABLE IF NOT EXISTS control.asset_class_book_policy (
    ...
    -- Capitalization threshold (moved from master.asset_class — varies by company functional currency)
    ...
    CONSTRAINT acbp_threshold_pos CHECK (capitalization_threshold >= 0),
    CONSTRAINT acbp_cap_currency_chk CHECK (capitalization_threshold = 0 OR capitalization_currency IS NOT NULL)
);
...
COMMENT ON TABLE control.asset_class_book_policy IS
    ...
    'Carries capitalization_threshold/currency (moved from master.asset_class — varies per company functional currency). '
