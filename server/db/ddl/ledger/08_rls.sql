...
CREATE POLICY tenant_read   ON ledger.gl_balance FOR SELECT USING     (shared.current_tenant_id_soft() IS NOT NULL AND tenant_id = shared.current_tenant_id_soft());
...
CREATE POLICY tenant_read        ON ledger.asset_revaluation_reserve FOR SELECT USING     (shared.current_tenant_id_soft() IS NOT NULL AND tenant_id = shared.current_tenant_id_soft());
...
CREATE POLICY tenant_read        ON ledger.tax_calculation FOR SELECT USING     (shared.current_tenant_id_soft() IS NOT NULL AND tenant_id = shared.current_tenant_id_soft());
...
CREATE POLICY tenant_read        ON ledger.tax_credit_movement FOR SELECT USING     (shared.current_tenant_id_soft() IS NOT NULL AND tenant_id = shared.current_tenant_id_soft());
