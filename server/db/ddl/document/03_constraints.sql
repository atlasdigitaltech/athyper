...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1
          FROM pg_constraint
         WHERE conrelid = 'document.journal_entry'::regclass
           AND conname = 'journal_entry_tenant_id_uq'
    ) THEN
        ALTER TABLE document.journal_entry ADD CONSTRAINT journal_entry_tenant_id_uq
            UNIQUE (tenant_id, id);
    END IF;
...
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
ALTER TABLE document.journal_entry DROP CONSTRAINT IF EXISTS je_reversal_of_fk;
DO $$ BEGIN ALTER TABLE document.journal_entry ADD CONSTRAINT je_reversal_of_fk
    FOREIGN KEY (tenant_id, reversal_of_id) REFERENCES document.journal_entry (tenant_id, id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
ALTER TABLE document.journal_entry DROP CONSTRAINT IF EXISTS je_reversed_by_fk;
DO $$ BEGIN ALTER TABLE document.journal_entry ADD CONSTRAINT je_reversed_by_fk
    FOREIGN KEY (tenant_id, reversed_by_id) REFERENCES document.journal_entry (tenant_id, id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
ALTER TABLE document.journal_entry DROP CONSTRAINT IF EXISTS je_derived_from_fk;
DO $$ BEGIN ALTER TABLE document.journal_entry ADD CONSTRAINT je_derived_from_fk
    FOREIGN KEY (tenant_id, derived_from_je_id) REFERENCES document.journal_entry (tenant_id, id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1
          FROM pg_constraint
         WHERE conrelid = 'document.journal_line'::regclass
           AND conname = 'journal_line_tenant_id_uq'
    ) THEN
        ALTER TABLE document.journal_line ADD CONSTRAINT journal_line_tenant_id_uq
            UNIQUE (tenant_id, id);
    END IF;
EXCEPTION WHEN duplicate_object OR duplicate_table THEN NULL; END $$;
ALTER TABLE document.journal_line DROP CONSTRAINT IF EXISTS jl_je_fk;
...
DO $$ BEGIN ALTER TABLE document.journal_line_reference ADD CONSTRAINT jlr_tenant_fk
    FOREIGN KEY (tenant_id) REFERENCES master.tenant (id) ON DELETE CASCADE;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
ALTER TABLE document.journal_line_reference DROP CONSTRAINT IF EXISTS jlr_line_fk;
DO $$ BEGIN ALTER TABLE document.journal_line_reference ADD CONSTRAINT jlr_line_fk
    FOREIGN KEY (tenant_id, journal_line_id) REFERENCES document.journal_line (tenant_id, id);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
