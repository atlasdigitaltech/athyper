-- Local development only. Run with psql -v ON_ERROR_STOP=1 -v apply=false first.
...
DECLARE
 ...
 actor uuid;
 ref text := 'local-dev:cirrusatlantic-publication-authority:v1';
...
BEGIN
 IF current_database()<>'athyper_studio' OR NOT EXISTS(SELECT 1 FROM master.tenant WHERE id=t AND code='cirrusatlantic' AND status='active') THEN
  RAISE EXCEPTION 'Studio development tenant fingerprint mismatch';
 END IF;
 ...
 SELECT id INTO STRICT admin_group FROM authz.principal_group WHERE tenant_id=t AND code='demo.studio.catl.admin' AND status='active' FOR UPDATE;
 IF (SELECT count(*) FROM authz.group_member WHERE tenant_id=t AND group_id=admin_group AND status='active')<>1 OR NOT EXISTS(SELECT 1 FROM authz.group_member WHERE tenant_id=t AND group_id=admin_group AND principal_id=admin_id AND status='active') THEN
  RAISE EXCEPTION 'Admin group is not exclusive; refusing to affect other actors';
 END IF;
 SELECT id INTO STRICT old_role FROM authz.role WHERE tenant_id=t AND code='demo.studio.testing-admin-tenant-exact' AND status='active';
 ...
 VALUES(t,'local.studio.catl.definition-author','CirrusAtlantic development author','Preserves existing admin grants except definition publish','system','seed',ref,'{"environment":"dev","purpose":"separate-publication-actors"}','draft',actor)
 ON CONFLICT(tenant_id,code) DO NOTHING;
 SELECT id INTO STRICT author_role FROM authz.role WHERE tenant_id=t AND code='local.studio.catl.definition-author' AND source_ref=ref;
 IF EXISTS(SELECT 1 FROM authz.role WHERE id=author_role AND status='draft') THEN
  ...
  SELECT t,author_role,rp.permission_id,actor FROM authz.role_permission rp JOIN authz.permission p ON p.id=rp.permission_id
  WHERE rp.tenant_id=t AND rp.role_id=old_role AND p.canonical_code<>'studio.business_partner_definition.publish';
  UPDATE authz.role SET status='active',updated_by=actor WHERE id=author_role;
 END IF;
 INSERT INTO authz.role(tenant_id,code,name,description,role_kind,source_type,source_ref,metadata,status,created_by)
 VALUES(t,'local.studio.catl.definition-reviewer','CirrusAtlantic development reviewer','Read and independently approve definitions at exact tenant scope','system','seed',ref,'{"environment":"dev","purpose":"separate-publication-actors"}','draft',actor)
 ON CONFLICT(tenant_id,code) DO NOTHING;
 SELECT id INTO STRICT reviewer_role FROM authz.role WHERE tenant_id=t AND code='local.studio.catl.definition-reviewer' AND source_ref=ref;
 ...
 VALUES(t,'local.studio.catl.definition-reviewer','CirrusAtlantic definition reviewer','system','seed',ref,'{"environment":"dev"}','active',actor)
 ON CONFLICT(tenant_id,code) DO NOTHING;
 SELECT id INTO STRICT owner_group FROM authz.principal_group WHERE tenant_id=t AND code='local.studio.catl.definition-reviewer' AND source_ref=ref;
 ...
 -- Identity coordinates of an assignment are immutable: suspend the old grant.
 UPDATE authz.group_role SET status='suspended',updated_by=actor
 WHERE tenant_id=t AND group_id=admin_group AND role_id=old_role AND status='active';
...
END $$;
SELECT jsonb_build_object('schema','athyper.local-publication-authority-provisioning/1','environment','dev','database',current_database(),'observedAt',clock_timestamp(),'executionIdentity',current_user,'nativeApproval',false,'grants',jsonb_agg(to_jsonb(g) ORDER BY code,canonical_code)) FROM actual_definition_grants g;
\if :apply
COMMIT;
\else
ROLLBACK;
