...
BEGIN
  ...
  choices:='{"partner.legalName":"source","partner.legalForm":"local","partner.countryCode":"source","partner.incorporationDate":"local","partner.websiteUrl":"local","partner.description":"source"}';
  proposed:='{"partner.legalName":"R6 Incoming Ltd","partner.legalForm":null,"partner.countryCode":"MY","partner.incorporationDate":null,"partner.websiteUrl":null,"partner.description":"Incoming description"}';
  ...
  payload:=jsonb_build_object('businessPartnerCode','R4.BP.TEST','name','R6 Incoming Ltd','ownershipClass','external','expectedBusinessPartnerVersion',1,'priorStatus','active','reasonCode','MESH_PROFILE_CHANGE','meshChangeResolutionId',resolution_id,'meshChangeFingerprint',repeat('f',64),'meshChangeDecisions',choices,'meshChangePreview','{}'::jsonb,'operatingOrganizationId',org_id,'registrationCountryCode','MY','description','Incoming description');
  ...
  IF NOT EXISTS(SELECT 1 FROM master.business_partner WHERE id=bp_id AND record_version=1 AND name='R4 BP')
  ...
  IF NOT EXISTS(SELECT 1 FROM master.business_partner WHERE id=bp_id AND code='R4.BP.TEST' AND status='active' AND record_version=2 AND name='R6 Incoming Ltd' AND description='Incoming description' AND registration_country_code='MY') THEN RAISE EXCEPTION 'Amendment did not preserve identity and update selected fields'; END IF;
