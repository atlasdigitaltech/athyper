-- Synthetic draft contract and request fixtures; every write rolls back.
BEGIN;
DO $$
...
BEGIN
 ...
 VALUES(contract_id,t,contract_entity_id,'master.business_partner',release_id,revision_id,1,'athyper.entity-contract','1.0.0',repeat('a',64),'{"type":"object","required":["country"],"additionalProperties":false,"properties":{"name":{"type":"string"},"country":{"type":"string"}}}'::jsonb,'g1.probe','ed25519','g1-key','probe',clock_timestamp()-interval '1 second','published',clock_timestamp());
 SELECT * INTO r FROM document.command_entity_case_draft(t,case_id,0,NULL,'G1.PROBE','master.business_partner','register',NULL,'g1-probe',contract_id,repeat('a',64),form_id,1,repeat('b',64),'{"country":"MY"}', 'g1-draft-create-0001',a,NULL);
 IF r.row_version<>1 OR r.disposition<>'accepted' OR r.replayed THEN RAISE EXCEPTION 'G1 create failed';END IF;s1:=r.snapshot_id;
 IF EXISTS(SELECT 1 FROM snapshot.entity_snapshot WHERE snapshot_id=s1 AND payload_json?'name') THEN RAISE EXCEPTION 'Empty draft invented a registered name'; END IF;
 SELECT * INTO r FROM document.command_entity_case_draft(t,case_id,0,NULL,'G1.PROBE','master.business_partner','register',NULL,'g1-probe',contract_id,repeat('a',64),form_id,1,repeat('b',64),'{"country":"MY"}', 'g1-draft-create-0001',a,NULL);
 ...
 SELECT * INTO r FROM document.command_entity_case_draft(t,case_id,1,s1,'G1.PROBE','master.business_partner','register',NULL,'g1-probe',contract_id,repeat('a',64),form_id,1,repeat('b',64),'{"country":"SG"}', 'g1-draft-merge-0003',a,NULL);
...
SELECT * INTO r FROM document.command_entity_case_draft(t,case_id,3,NULL,'G1.PROBE','master.business_partner','register',NULL,'g1-probe',contract_id,repeat('a',64),form_id,1,repeat('b',64),'{"country":"SG"}', 'g1-draft-clear-0006',a,NULL);
 IF r.row_version<>4 OR EXISTS(SELECT 1 FROM snapshot.entity_snapshot WHERE snapshot_id=r.snapshot_id AND payload_json?'name') THEN RAISE EXCEPTION 'Cleared name was retained'; END IF;
 IF NOT EXISTS(SELECT 1 FROM snapshot.entity_snapshot WHERE snapshot_id=s2 AND payload_json->>'name'='Beta') THEN RAISE EXCEPTION 'Previous draft was overwritten'; END IF;
END $$;
ROLLBACK;
SELECT 'BP_INCOMPLETE_DRAFT_PERSISTENCE_ROLLBACK_OK';
