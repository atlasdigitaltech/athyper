...
BEGIN
 ...
 FOR item IN SELECT value FROM jsonb_array_elements('[{"addressKind":"street","purpose":"default","buildingName":"Example Tower","floor":"31","unit":"BC13","line1":"Example Tower","city":"Kuala Lumpur","countryCode":"MY","normalizedHash":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},{"addressKind":"po_box","purpose":"default","poBox":"127","city":"Kuala Lumpur","postalCode":"50470","countryCode":"MY","normalizedHash":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"}]'::jsonb) LOOP
 address_id:=gen_random_uuid();
  INSERT INTO master.address(id,tenant_id,address_type,address_kind,building_name,floor,unit,house_number,street_name,po_box,po_box_city,po_box_postal_code,line1,line2,city,region,postal_code,country_code,normalized_hash,
 ...
 IF item->>'addressKind'='street' THEN
  IF saved.building_name<>'Example Tower' OR saved.floor<>'31' OR saved.unit<>'BC13' OR saved.formatted_address<>'Example Tower, Kuala Lumpur, MY' THEN RAISE EXCEPTION 'Structured street persistence or formatting failed'; END IF;
 ELSE
  IF saved.po_box<>'127' OR saved.po_box_city<>'Kuala Lumpur' OR saved.po_box_postal_code<>'50470' OR saved.formatted_address<>'PO Box 127, Kuala Lumpur, 50470, MY' THEN RAISE EXCEPTION 'PO box persistence failed'; END IF;
 END IF;
 END LOOP;
END $test$;
ROLLBACK;
