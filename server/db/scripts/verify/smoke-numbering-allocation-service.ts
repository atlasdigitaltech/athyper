...
} from "../../../packages/services/numbering-runtime/src/postgres-numbering-allocation.service.js";
...
async function createPolicy(code: string, options: { reset?: string; format?: string; maximum?: number | null } = {}): Promise<string> {
  const id = randomUUID();
  policyIds.push(id);
  ...
  await sql`INSERT INTO control.numbering_policy (
    ...
    ${options.maximum ?? null},'tenant',${reset},${timezone},'active',now(),${systemPrincipal}::uuid,${systemPrincipal}::uuid)`.execute(db);
  return id;
}

function command(policyCode: string, occurredAt = "2026-08-02T00:00:00Z", tenantId = systemTenant) {
  return {
    allocationId: randomUUID(), tenantId, principalId: systemPrincipal, targetPlane: "neon" as const,
    policyCode, policyRevision: 1, occurredAt,
  };
}
...
try {
  ...
  if (JSON.stringify(values) !== JSON.stringify(Array.from({ length: 20 }, (_, index) => index + 1))) {
  ...
  try {
    await db.transaction().execute(async (transaction) => {
      await service.allocateWithinTransaction(transaction, command(rollbackCode));
  ...
  catch (error) { exhausted = error instanceof NumberingAllocationError && error.code === "NUMBERING_POLICY_EXHAUSTED"; }
  ...
  process.stdout.write("NUMBERING_ALLOCATION_SMOKE_OK sequential=2 concurrent=20 rollback=1 rollover=2 exhaustion=blocked tenants=2\n");
} finally {
  if (policyIds.length) {
