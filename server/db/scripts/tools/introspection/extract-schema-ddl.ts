...
import pg from "pg";
import { generateTablesSql } from "./extract-schema-tables.js";
...
const requiredConnectionString: string = connectionString;
...
async function main() {
  ...
  let db = new Client(clientOptions(requiredConnectionString));
  try {
    await db.connect();
  } catch (error) {
    const fallbackUrl = new URL(requiredConnectionString);
    const canRetry =
      plane === "mesh" &&
  ...
  try {
    ...
    const files = new Map<string, string>([
      ...
      ["01_tables.sql", await generateTablesSql(db, schema, header("01_tables.sql", "Tables reconstructed from the live catalog."))],
