...
async function provisionMesh(client: Client, dryRun: boolean) {
  ...
  try {
    ...
    for(const [index,pair] of unique.entries()){
      ...
      await client.query("SELECT set_config('app.current_tenant_id',$1,true),set_config('app.current_principal_id',$2,true)",[pair.buyer.tenant_id,principal.id]);
      const id=deterministicUuid(`relationship:${pair.buyer.id}:${pair.supplier.id}:commercial`),metadata=JSON.stringify({_seed:{pack:"development.mesh-network-relationship-list.v1",version:"1.0.0"},sequence:index+1});
      await client.query("INSERT INTO mesh.network_relationship(id,buyer_tenant_id,buyer_account_id,supplier_tenant_id,supplier_account_id,relationship_kind,effective_from,metadata,status,created_by_tenant_id,created_by) VALUES($1::uuid,$2::uuid,$3::uuid,$4::uuid,$5::uuid,'commercial','2026-01-01',$6::jsonb,'requested',$2::uuid,$7::uuid) ON CONFLICT(buyer_account_id,supplier_account_id,relationship_kind) DO NOTHING",[id,pair.buyer.tenant_id,pair.buyer.id,pair.supplier.tenant_id,pair.supplier.id,metadata,principal.id]);
      const actual=await one<{id:string}>(client,"SELECT id::text FROM mesh.network_relationship WHERE buyer_account_id=$1::uuid AND supplier_account_id=$2::uuid AND relationship_kind='commercial'",[pair.buyer.id,pair.supplier.id]);if(actual.id!==id)throw new Error(`network relationship fixture conflict: ${pair.buyer.account_code}/${pair.supplier.account_code}`);
    }
