...
test("the historical compact request guard is normalized before the validation transition", async () => {
  const [compatibility, manifest] = await Promise.all([
    readFile(resolve(databaseRoot, "migrations/20260829_neon_business_partner_request_guard_compatibility.sql"), "utf8"),
    readFile(resolve(databaseRoot, "migrations/manifests/neon.txt"), "utf8"),
  ]);
  assert.match(compatibility, /OLD\.status NOT IN\(''draft'',''validation_failed'',''returned''\)/);
  assert.match(compatibility, /OLD\.status NOT IN \(''draft'', ''validation_failed'', ''returned''\)/);
  assert.ok(
    manifest.indexOf("20260829_neon_business_partner_request_guard_compatibility.sql") <
      manifest.indexOf("20260828_neon_business_partner_request_validation_transition.sql"),
  );
});
