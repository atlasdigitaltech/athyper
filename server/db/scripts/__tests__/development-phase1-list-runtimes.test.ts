...
test("Phase 1 Mesh and Studio list publications are deterministic and plane-safe", () => {
  ...
  assert.deepEqual(mesh.projection.descriptor.compiled_json.listPresentation.dataOperations.importFormats,["xlsx","csv","json"]);
  ...
  assert.deepEqual(mesh.projection.descriptor.compiled_json.listPresentation.filterPresentation.quickFields.map((item) => item.field), ["status", "relationship_kind", "effective_from", "updated_at"]);
  ...
  assert.deepEqual(studio.projection.descriptor.compiled_json.listPresentation.dataOperations.importFormats,["xlsx","csv","json"]);
  assert.deepEqual(studio.projection.descriptor.compiled_json.listPresentation.filterPresentation.quickFields.map((item) => item.field), ["status", "entity_class", "ownership_model", "updated_at"]);
