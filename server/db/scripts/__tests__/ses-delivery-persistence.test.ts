...
describe("SES delivery persistence contract", () => {
  it("installs the forward migration on all three planes", async () => {
    for (const plane of ["studio", "neon", "mesh"]) {
      ...
      assert.equal(manifest.match(/20260825_ses_delivery_persistence\.sql/g)?.length, 1);
    }
  });
  ...
  it("keeps provider receipts append-only, unique, and payload-redacted", async () => {
    const [tables, triggers, migration] = await Promise.all([
      read("ddl/common/event/03_tables.sql"),read("ddl/common/event/08_triggers.sql"),
    ...
    for (const source of [tables, migration]) {
      ...
      const receipt = source.match(/CREATE TABLE event\.notification_provider_event([\s\S]*?)\n\);/)?.[1] ?? "";
      assert.doesNotMatch(receipt, /raw_payload|recipient_addr\s+text|email_address/);
    }
  ...
  it("uses hash-only global and tenant suppression projections", async () => {
    ...
    assert.doesNotMatch(tables, /notification_email_suppression[\s\S]{0,1200}(email_address|recipient_addr)/);
