...
test("keeps rejected migration tooling out of the active authorization build", async () => {
  ...
  assert.equal(Object.keys(packageJson.scripts).some((name) => name.includes("legacy-retirement") || name.includes("inventory:authorization:legacy")), false);
  for (const path of [
...
test("publishes only exact catalogs and zero-grant authority", async () => {
  for (const plane of ["studio", "neon", "mesh"] as const) {
    const catalog = JSON.parse(await readFile(resolve(dbRoot, `seed/contracts/authorization/catalog/${plane}/catalog.v2.json`), "utf8")) as Record<string, unknown> & { permissions: Array<{ canonicalCode: string }> };
    assert.equal("blockedLegacyPermissionCount" in catalog, false);
    assert.equal("excludedNonPermissionCount" in catalog, false);
    assert.equal(catalog.permissions.every((permission) => permission.canonicalCode.split(".").length === 4 && permission.canonicalCode.startsWith(`${plane}.`)), true);
    const pack = JSON.parse(await readFile(resolve(dbRoot, `seed/packs/authorization-v2/${plane}/seed-pack.v1.json`), "utf8")) as {
      contractVersion: string;
      authority: { version: string; roles: unknown[]; groups: Array<{ zeroGrant: boolean }> };
      tenantAuthorityProjection: { definitions: { roles: unknown[]; rolePermissions: unknown[] }; assignments: { groupRoles: unknown[] } };
    };
    assert.equal(pack.contractVersion, "athyper.authorization.clean-slate-seed-pack.v1");
    assert.equal(pack.authority.version, "authorization.clean-slate.v1");
    assert.equal(pack.authority.roles.length, 0);
    assert.deepEqual(pack.authority.groups.map((group) => group.zeroGrant), [true]);
    assert.equal(pack.tenantAuthorityProjection.definitions.roles.length, 0);
    assert.equal(pack.tenantAuthorityProjection.definitions.rolePermissions.length, 0);
    assert.equal(pack.tenantAuthorityProjection.assignments.groupRoles.length, 0);
  }
});

test("guards the destructive reset and reapplies the deny-all pack", async () => {
  const reset = await readFile(resolve(dbRoot, "scripts/operations/authorization/reset-authorization-clean-slate.ts"), "utf8");
...
test("accepts an explicit authorization-pack database target", async () => {
  const applicator = await readFile(resolve(dbRoot, "scripts/apply-authorization-seed-pack.ts"), "utf8");
  const planeApplicator = await readFile(resolve(dbRoot, "scripts/provisioning/authorization-pack-applicator.ts"), "utf8");
