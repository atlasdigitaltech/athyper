...
it("publishes a legacy-free, zero-grant clean-slate baseline", async () => {
  ...
  assert.equal(
    (
      report.gates.noPublishedLegacyPermissions?.evidence as {
        legacySeedRepublishRisk: boolean;
      }
    ).legacySeedRepublishRisk,
    false,
  );
  ...
  assert.equal(
    report.gates.applicationPermissionDemandCoverage?.status,
    "fail",
  );
  assert.equal(
    report.gates.neonPhysicalTableAuthorizationCoverage?.status,
    "fail",
  );
  assert.equal(
    report.gates.meshPhysicalTableAuthorizationCoverage?.status,
    "fail",
  );
  assert.equal(
    report.gates.inventoryPermissionPromotionQualification?.status,
    "fail",
  );
});
...
it("keeps three-plane snapshot parity authoritative and excludes operational receipts", async () => {
  ...
  const command =
    packageJson.scripts["db:verify:authorization:release:idempotency"] ?? "";
  ...
  assert.match(
    applicator,
    /current\.rows\[0\]\.exact\) return current\.rows\[0\]\.id/,
  );
  assert.match(
    applicator,
    /authz\.permission\.permission_kind[\s\S]*IS DISTINCT FROM[\s\S]*'published'::authz\.catalog_status_d/,
  );
  assert.match(
    applicator,
    /authz\.plane_membership[\s\S]*metadata IS DISTINCT FROM \$5::jsonb/,
  );
  assert.match(
    applicator,
    /master\.legal_entity\.metadata[\s\S]*IS DISTINCT FROM/,
  );
  assert.match(
    applicator,
    /mesh\.network_account\.metadata[\s\S]*IS DISTINCT FROM/,
  );
});
...
it("runs one mandatory idempotency command for compiler, contract, and pack changes", async () => {
  ...
  const workflow = await readFile(
    resolve(repositoryRoot, ".github/workflows/authorization-idempotency.yml"),
    "utf8",
  );
  ...
  assert.match(
    workflow,
    /Create separate plane databases[\s\S]*athyper_studio[\s\S]*athyper_neon[\s\S]*athyper_mesh/,
  );
  assert.equal((workflow.match(/-AuthorizationIdempotency/g) ?? []).length, 3);
  const invocations =
    workflow.match(/db:verify:authorization:release:idempotency/g) ?? [];
