...
import {
  ...
  validateExactScopeCompatibility,
} from "../../seed/exact-scope-compatibility-model.js";
...
test("declares exact permission-specific Neon scopes", () => {
  ...
  assert.deepEqual(contract.permissions[0], {
    permissionCode: code,
    scopes: [
...
test("declares independent Neon catalog visibility coordinates without member-company propagation", () => {
  ...
  assert.equal(
    contract.permissions[0]?.scopes.some(
      (scope) => scope.propagation === "member_companies",
    ),
    false,
  );
});
...
test("rejects non-canonical permission identities", () => {
  assert.throws(
    () =>
      buildExactScopeCompatibility({
        plane: "neon",
        permissionCodes: ["relationship.business_partner.read"],
      }),
...
test("rejects incomplete declarations and cross-plane scope kinds", () => {
  const contract = buildExactScopeCompatibility({
    plane: "studio",
    permissionCodes: ["studio.metadata.contract.view"],
  });
  assert.throws(
    () =>
      validateExactScopeCompatibility(contract, [
  ...
  assert.throws(
    () =>
      validateExactScopeCompatibility(invalid, [
        "studio.metadata.contract.view",
      ]),
...
test("rejects broad propagation for exact-only scope kinds", () => {
  const contract = buildExactScopeCompatibility({
    plane: "mesh",
    permissionCodes: ["mesh.catalog.attachment.read"],
  });
  const invalid = structuredClone(contract);
  invalid.permissions[0]!.scopes = [
    { kind: "resource", propagation: "subtree" },
  ];
  assert.throws(
    () =>
      validateExactScopeCompatibility(invalid, [
        "mesh.catalog.attachment.read",
      ]),
    /resource requires exact propagation/,
  );
});
