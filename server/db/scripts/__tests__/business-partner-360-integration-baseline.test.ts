...
} from "../evaluate-business-partner-360-p0-approvals.js";
import { provisionBusinessPartner360AcceptanceFixtures } from "../provision-business-partner-360-acceptance-fixtures.js";
import { runBusinessPartner360MaterializationEvidence } from "../run-business-partner-360-materialization-evidence.js";
...
test("P0 plans exactly seven deterministic BP360 acceptance families", async () => {
  ...
  assert.deepEqual(plan.families.map((value) => value.family), ["organization", "supplier", "customer", "dual_role", "person_workforce", "external_worker", "mesh_linked"]);
  assert.equal(new Set(plan.families.map((value) => value.businessPartnerId)).size, 7);
});

test("P0 disposable runner refuses shared containers and records manifest hashes", async () => {
  const runner = await read("db/scripts/apply-disposable-ddl-manifest.ts");
  ...
  assert.match(runner, /files: Object\.freeze\(evidence\)/);
});

test("P0 fixture payload is risk-negative and person/workforce remains independent", async () => {
  const fixture = await read("db/scripts/provision-business-partner-360-acceptance-fixtures.ts");
  assert.doesNotMatch(fixture, /risk(?:Score|Band|Class|Trend|Incident|Exposure)/i);
...
test("P0 production repository evidence is local-only, rollback-only, and covers typed plus legacy policy", async () => {
  await assert.rejects(
    runBusinessPartner360MaterializationEvidence({
      neonDatabaseUrl: "postgresql://postgres@shared.example.test:5432/athyper_neon",
      confirmation: "RUN-BS360-MATERIALIZATION-EVIDENCE",
    }),
    /requires local athyper_neon/,
  );
  await assert.rejects(
    runBusinessPartner360MaterializationEvidence({
      neonDatabaseUrl: "postgresql://postgres@127.0.0.1:55432/athyper_neon",
  ...
  const source = await read("db/scripts/run-business-partner-360-materialization-evidence.ts");
  for (const value of [
...
test("P0 approval packet remains fail-closed until browser evidence and seven accountable approvals exist", async () => {
  const packet = JSON.parse(await read("../docs/architecture/evidence/business-partner-360-p0-approvals.json")) as BusinessPartner360P0ApprovalPacket;
  ...
  const complete = evaluateBusinessPartner360P0Approvals({
    ...packet,
    evidence: packet.evidence.map((item) => ({ ...item, status: "passed" as const })),
  ...
  assert.ok(prematureSecurityApproval.invalid.includes("SECURITY_PRIVACY_APPROVED_BEFORE_BROWSER_EVIDENCE"));
  ...
  const notIndependent = evaluateBusinessPartner360P0Approvals({
    ...
    approvals: packet.approvals.map((item, index) => ({
      ...item,
      status: "approved" as const,
      approverId: ["security", "privacy_data_protection"].includes(item.gate) ? sharedApprover : `owner.${item.gate}.${index}`,
      approvedAt: "2026-08-30T14:00:00+08:00",
      approvalRef: `release/bp360/p0/${item.gate}/approval-001`,
    })),
  });
  assert.equal(notIndependent.p0Closed, false);
  assert.ok(notIndependent.invalid.includes("SECURITY_PRIVACY_APPROVERS_NOT_INDEPENDENT"));
});
