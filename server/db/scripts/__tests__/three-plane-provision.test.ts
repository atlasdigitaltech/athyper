...
describe("canonical three-plane provisioning contract", () => {
  ...
  it("resolves every scoped authorization resource before database application", async () => {
    ...
    const coreAccounts = new Set(Array.from({ length: 22 }, (_, index) => String(1_000_000_001 + index))
      .flatMap((suffix) => [`BNA-${suffix}`, `SNA-${suffix}`]));
    assert.equal(accounts.filter((account) => coreAccounts.has(account.scopeKey)).length, 44);
    const tenantCodes = new Set(inputs.manifest.tenants.map((tenant) => tenant.code));
    assert.equal(planeAssignments(inputs.authorizationPacks.studio, "studio", tenantCodes).length, 28);
    assert.equal(planeAssignments(inputs.authorizationPacks.neon, "neon", tenantCodes).length, 151);
    assert.equal(planeAssignments(inputs.authorizationPacks.mesh, "mesh", tenantCodes).length, 54);
  });
  ...
  it("loads distinct simple, medium, and complex Neon tenant scenarios", async () => {
    ...
    assert.deepEqual(
      Object.fromEntries(Object.entries(inputs.scenarioPacks).map(([code, pack]) => [code, {
        ...
        organizationCompanyAssignments: pack.operatingOrganizationCompanyAssignments.length,
        orgUnits: pack.orgUnits.length,
      }])),
      {
        athyper: { legalEntities: 17, companyCodes: 34, operatingOrganizations: 10, organizationCompanyAssignments: 102, orgUnits: 12 },
        technostat: { legalEntities: 4, companyCodes: 4, operatingOrganizations: 3, organizationCompanyAssignments: 12, orgUnits: 9 },
        cirrusatlantic: { legalEntities: 1, companyCodes: 1, operatingOrganizations: 1, organizationCompanyAssignments: 1, orgUnits: 3 },
      },
    );
    for (const pack of Object.values(inputs.scenarioPacks)) {
      assert.equal(pack.legalEntities.every((entity) => Boolean(entity.regionCode)), true);
      assert.equal(pack.companyCodes.every((company) => company.readinessProfile === "finance_baseline_v1"), true);
    }
    const athyper = inputs.scenarioPacks.athyper!;
    assert.equal(athyper.companyCodes.filter((company) => company.companyPurpose === "operations").length, 17);
  });

  it("enriches authorization-backed legal entities from scenario data", async () => {
    const inputs = await loadProvisionInputs();
    const resources = legalEntityResources(inputs);
    const cirrus = resources.find((resource) => resource.tenantCode === "cirrusatlantic")!;
    const technostatEgypt = resources.find((resource) => resource.scopeKey === "ORG-1000000020")!;
    const athyperIndia = resources.find((resource) => resource.scopeKey === "ORG-1000000010")!;
    assert.deepEqual(
      [cirrus.registrationCountryCode, cirrus.functionalCurrency, cirrus.reportingCurrency],
      ["GB", "GBP", "GBP"],
    );
  ...
  it("separates lifecycle resources and admission identity from clean-slate authority", async () => {
    ...
    for (const plane of planeOrder()) {
      const projection = inputs.authorizationPacks[plane].tenantAuthorityProjection;
      assert.equal(projection.plane, plane);
      assert.equal(projection.contractVersion, "athyper.authorization.tenant-authority-projection.v1");
      ...
      assert.equal(projection.definitions.principalGroups.every((group) => group.zeroGrant), true);
      assert.equal(projection.assignments.planeMemberships.length > 0, true);
      assert.equal(projection.assignments.scopeTargets.length >= inputs.manifest.tenants.length, true);
      ...
      for (const grant of projection.definitions.rolePermissions) assert.equal(roleIds.has(grant.roleId), true);
      for (const member of projection.assignments.groupMembers) assert.equal(groupIds.has(member.groupId), true);
      ...
      for (const role of projection.definitions.roles) assert.equal("keycloakSubject" in role, false);
      for (const group of projection.definitions.principalGroups) assert.equal("scopeTargetId" in group, false);
    }
  });
});
