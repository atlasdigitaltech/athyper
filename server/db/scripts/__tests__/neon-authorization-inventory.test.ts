...
function contract(overrides: Partial<NeonReviewedSlicesContract> = {}): NeonReviewedSlicesContract {
  return {
...
test("rejects generic CRUD publication for an aggregate child", () => {
  ...
  assert.ok(result.errors.includes("child published as generic CRUD: master.example/create"));
});
...
test("rejects an operation without its required scope coordinate", () => {
  const result = validateNeonAuthorizationInventory({
    discoveredTables: [rootRow.table],
    coverage: [rootRow],
    contract: contract({ operations: [{ ...operation, scopeCoordinateKeys: {} }] }),
  });
  assert.ok(result.errors.includes("operation scope coordinate missing: example:create/resource"));
});
...
test("strict validation rejects pending review while inventory mode reports it", () => {
  ...
  const inventory = validateNeonAuthorizationInventory({ discoveredTables: [pending.table], coverage: [pending], contract: contract({ tables: [], operations: [] }) });
  assert.deepEqual(inventory.errors, []);
  assert.deepEqual(inventory.releaseBlockers, ["pending table review: master.example"]);
  const strict = validateNeonAuthorizationInventory({ discoveredTables: [pending.table], coverage: [pending], contract: contract({ tables: [], operations: [] }), strict: true });
...
test("rejects a Studio organization boundary that bypasses the Neon plane-local applier", async () => {
  const [artifact, reviewed, boundary] = await Promise.all([
    readFile(resolve(dbRoot, "seed/contracts/authorization/inventory/neon/compiled/table-authorization-coverage.v1.json"), "utf8"),
    readFile(resolve(dbRoot, "seed/contracts/authorization/inventory/neon/reviewed-slices.v1.json"), "utf8"),
    readFile(resolve(dbRoot, "seed/contracts/authorization/inventory/neon/studio-neon-organization-boundary.v1.json"), "utf8"),
  ]);
  ...
  const organizationBoundary = JSON.parse(boundary) as StudioNeonOrganizationBoundaryContract;
  const unsafeBoundary = {
    ...organizationBoundary,
    source: { ...organizationBoundary.source, directNeonSql: true },
    target: { ...organizationBoundary.target, forbidsBusinessActivation: false },
  ...
  assert.ok(result.errors.includes("Studio organization boundary permits direct Neon SQL"));
  assert.ok(result.errors.includes("Studio organization onboarding can activate Neon business state"));
});

test("publishes a deterministic 263-row non-enforcing Neon inventory", async () => {
  ...
  assert.equal(artifact.counts.reviewedTables, 48);
  assert.equal(artifact.counts.pendingReviewTables, 215);
  ...
  assert.equal(new Set(artifact.tables.map((row) => row.table)).size, 263);
});
