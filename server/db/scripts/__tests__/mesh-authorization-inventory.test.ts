...
const operation: MeshOperationDefinition = {
  ...
  coordinates: [{ scopeKind: "network_account", coordinateKey: "networkAccountId", coordinateRole: "target", authorizationRequired: true }],
...
function contract(overrides: Partial<MeshReviewedSlicesContract> = {}): MeshReviewedSlicesContract {
  return {
...
test("rejects generic operation publication for a Mesh aggregate child", () => {
  ...
  const result = validateMeshAuthorizationInventory({
    ...
    coverage: [child],
    contract: contract({ tables: [{ ...contract().tables[0]!, classification: "aggregate_child" }] }),
  });
  assert.ok(result.errors.includes("child published as generic CRUD: mesh.network_account/read"));
});
...
test("rejects an operation without an authorization-bearing coordinate", () => {
  const result = validateMeshAuthorizationInventory({
    discoveredTables: [rootRow.table],
    coverage: [rootRow],
    contract: contract({ operations: [{ ...operation, coordinates: [{ ...operation.coordinates[0]!, authorizationRequired: false }] }] }),
  });
  assert.ok(result.errors.includes("operation has no authorization coordinate: network_account:read"));
});
...
test("rejects a relationship operation missing bilateral coordinates or using initiator acceptance", () => {
  ...
  const unsafe: MeshOperationDefinition = {
    ...
    coordinates: [
      { scopeKind: "network_relationship", coordinateKey: "networkRelationshipId", coordinateRole: "target", authorizationRequired: true },
      { scopeKind: "network_account", coordinateKey: "actorNetworkAccountId", coordinateRole: "actor_account", authorizationRequired: true },
    ],
  ...
  const result = validateMeshAuthorizationInventory({
    ...
    contract: contract({
      tables: [{ ...contract().tables[0]!, table: relationshipRow.table, aggregateRoots: [relationshipRow.table], requiredScopeKinds: relationshipRow.requiredScopeKinds }],
      operations: [unsafe],
    }),
  });
  assert.ok(result.errors.includes("bilateral relationship coordinate missing: network_relationship:accept/buyerNetworkAccountId"));
  assert.ok(result.errors.includes("unsafe relationship participant rule: network_relationship:accept"));
});
...
test("strict validation rejects pending review while inventory mode reports it", () => {
  ...
  const inventory = validateMeshAuthorizationInventory({ discoveredTables: [pending.table], coverage: [pending], contract: contract({ tables: [], operations: [] }) });
  assert.deepEqual(inventory.errors, []);
  assert.deepEqual(inventory.releaseBlockers, ["pending table review: mesh.network_account"]);
  const strict = validateMeshAuthorizationInventory({ discoveredTables: [pending.table], coverage: [pending], contract: contract({ tables: [], operations: [] }), strict: true });
  assert.ok(strict.errors.includes("pending table review: mesh.network_account"));
});
...
test("rejects a Studio boundary that writes Mesh directly or fabricates a relationship", async () => {
  const [artifactSource, reviewedSource, boundarySource] = await Promise.all([
    readFile(resolve(dbRoot, "seed/contracts/authorization/inventory/mesh/compiled/table-authorization-coverage.v1.json"), "utf8"),
    readFile(resolve(dbRoot, "seed/contracts/authorization/inventory/mesh/reviewed-slices.v1.json"), "utf8"),
    readFile(resolve(dbRoot, "seed/contracts/authorization/inventory/mesh/studio-mesh-network-boundary.v1.json"), "utf8"),
  ]);
  const artifact = JSON.parse(artifactSource) as { tables: MeshTableCoverageRow[] };
  const reviewed = JSON.parse(reviewedSource) as MeshReviewedSlicesContract;
  const boundary = JSON.parse(boundarySource) as StudioMeshNetworkBoundaryContract;
  const unsafe = {
    ...boundary,
    source: { ...boundary.source, directMeshSql: true, mayCreateOrAcceptRelationship: true },
    target: { ...boundary.target, forbidsBusinessActivation: false, forbidsRelationshipMutation: false },
  ...
  assert.ok(result.errors.includes("Studio network boundary permits direct Mesh SQL"));
  assert.ok(result.errors.includes("Studio onboarding can mutate a bilateral Mesh relationship"));
