...
test("P7 persists exact desired identity state, fenced attempts, callbacks, and SoD replay evidence", async () => {
  const [tables, constraints, indexes, triggers, rls, grants] = await Promise.all([
    "03_tables.sql", "05_constraints.sql", "06_indexes.sql", "08_triggers.sql", "10_rls.sql", "11_grants.sql",
  ].map(file => readFile(resolve(dbRoot, `ddl/planes/studio/trustiam/${file}`), "utf8")));
  ...
  assert.match(tables, /CREATE TABLE trustiam\.provider_identity_callback_inbox/);
  assert.match(tables, /desired_version bigint NOT NULL, desired_hash char\(64\) NOT NULL/);
  ...
  assert.match(triggers, /identity desired state is fenced by an active saga attempt/);
  assert.match(triggers, /terminal identity saga evidence is immutable/);
  assert.match(rls, /identity_saga_attempt/);
  assert.match(grants, /GRANT SELECT,INSERT,UPDATE ON trustiam\.identity_projection,trustiam\.identity_saga_attempt,trustiam\.provider_identity_callback_inbox TO athyper_trustiam_service/);
});
...
test("P7 keeps provider administration separate from local authorization and business history", async () => {
  ...
  assert.match(contract, /person, employment and BP stores are outside this port/);
  assert.match(contract, /roles: readonly LocalRoleAssignment\[\]/);
  assert.match(contract, /Plane-local admission is closed first/);
  assert.doesNotMatch(repository, /DELETE FROM master\.(person|employment|business_partner)/i);
  assert.match(repository, /provider_attributes[^;]+?'\{\}'::jsonb/);
  assert.doesNotMatch(repository, /provider_attributes\s*->|provider_attributes\s*#>|jsonb_to_record[^;]+provider_attributes/);
});
