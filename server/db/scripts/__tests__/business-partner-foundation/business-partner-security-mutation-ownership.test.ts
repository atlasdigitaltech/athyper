...
test("S5 centralizes lifecycle and decision mutation authority", () => {
  ...
  assert.match(
    functions,
    /Maker cannot approve or decide their own Business Partner record/,
  );
...
test("S5 evidence is immutable, bounded, forced-RLS, and outbox-backed", () => {
  ...
  assert.match(
    functions,
    /Business Partner mutation evidence contains protected identity data/,
  );
...
test("S5 upgrade and runtime repositories use command ownership", () => {
  const migration = read(
    "migrations/20260903_neon_business_partner_security_mutation_ownership.sql",
  );
  ...
  assert.match(
    manifest,
    /^20260903_neon_business_partner_security_mutation_ownership\.sql$/m,
  );
  assert.match(
    manifest,
    /20260903_neon_business_partner_security_mutation_ownership\.sql\n20260903_neon_business_partner_security_certification_hardening\.sql/,
  );
  assert.match(migration, /S5 preflight failed/);
  assert.match(
    migration,
    /Preserve supported-baseline Customer lifecycle evidence/,
  );
  ...
  assert.doesNotMatch(
    eligibility,
    /UPDATE control\.customer_credit_review SET decision/,
  );
