...
test("P1 preserves historical result invariants while application dispatch is organization-only", async () => {
  ...
  const matrix: ReadonlyArray<readonly [string, string]> = [
    ["new_partner", "partner_role_created"],
    ["new_partner", "workforce_created"],
    ["add_supplier", "partner_role_created"],
    ["add_customer", "partner_role_created"],
    ["add_workforce", "workforce_created"],
    ["amend_partner", "partner_amended"],
    ["assign_organization", "organization_assigned"],
    ["configure_company", "company_configured"],
    ["change_bank", "bank_verification_started"],
    ["change_employment", "employment_changed"],
    ["deactivate", "partner_deactivated"],
    ["reactivate", "partner_reactivated"],
    ["archive", "partner_archived"],
  ];
  for (const [kind, result] of matrix) {
    assert.match(
      migration,
      new RegExp(`WHEN '${kind}'[\\s\\S]{0,500}${result}`),
    );
    assert.match(tables, new RegExp(`WHEN '${kind}'[\\s\\S]{0,500}${result}`));
  }
  assert.match(
    migration,
    /requested_role='supplier'[\s\S]*partner_role_created[\s\S]*materialized_supplier_id IS NOT NULL/,
  );
  assert.match(
    migration,
    /requested_role='customer'[\s\S]*partner_role_created[\s\S]*materialized_customer_id IS NOT NULL/,
  );
  assert.match(
    migration,
    /requested_role='workforce'[\s\S]*workforce_created[\s\S]*materialized_person_id IS NOT NULL/,
  );
  assert.match(
    repository,
  ...
  assert.match(
    manifest,
    /^20260829_neon_business_partner_request_application_invariants\.sql$/m,
  );
});
...
test("P1 application retries, stale targets, scope drift, and rollback share one boundary", async () => {
  ...
  assert.match(
    repository,
    /status='applied' AND application_idempotency_key=\$\{idempotencyKey\} AND application_fingerprint=\$\{fingerprint\}/,
  );
  assert.match(
    repository,
  ...
  assert.match(
    service,
    /options\.transactions\.run\("neon"[\s\S]*options\.repository\.apply[\s\S]*effects\(options/,
  );
  assert.match(migration, /^BEGIN;/m);
  assert.match(migration, /COMMIT;/);
  assert.doesNotMatch(
    repository,
    /catch\s*\([^)]*\)[\s\S]{0,200}(failure|failed|ledger)/i,
  );
});
...
test("P1 lifecycle and category correction preserve history", async () => {
  ...
  assert.match(
    migration,
    /business_partner_request_lifecycle_impact_chk[\s\S]*dependencies[\s\S]*evidenceVersion[\s\S]*reasonCode/,
  );
  assert.match(
    repository,
  ...
  assert.match(
    masterFunctions,
    /NEW\.partner_category IS DISTINCT FROM OLD\.partner_category/,
  );
  assert.match(
    migration,
    /business_partner_duplicate_resolution[\s\S]*resolution_kind IN\('merge','rekey','supersede'\)/,
  );
  assert.match(migration, /same-category partners/);
  assert.doesNotMatch(
    repository,
    /DELETE FROM master\.(business_partner|supplier|customer|person|employee|employment|work_assignment)/,
  );
});
...
test("P1 change_bank only starts the dedicated verification authority", async () => {
  ...
  const orchestration =
    repository.match(
      /async function orchestrateBankVerification[\s\S]*?\n}\n\nasync function claimApplication/,
    )?.[0] ?? "";
  assert.match(
    orchestration,
  ...
  assert.doesNotMatch(
    orchestration,
    /UPDATE master\.company_code_supplier_profile SET preferred_remittance_bank_link_id/,
  );
  assert.match(
    bankService,
    /async applyVerification[\s\S]*UPDATE master\.company_code_supplier_profile SET preferred_remittance_bank_link_id/,
  );
  assert.match(
    migration,
    /materialized_bank_verification_id[\s\S]*REFERENCES document\.business_partner_bank_verification/,
  );
});
