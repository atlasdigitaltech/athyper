...
test("WP15 MESH disclosure is directional, approved, immutable and masked", async () => {
  const [tables, snapshots, functions, rls, migration, manifest] =
    await Promise.all([
      ...
      read(
        "migrations/20260828_mesh_business_partner_account_bank_linkage.sql",
      ),
  ...
  for (const value of [
    ...
    assert.match(tables, new RegExp(value));
  ...
  const service = await read(
    "../packages/planes/mesh/src/business-partner-bank-disclosure.ts",
  );
  ...
  assert.match(
    manifest,
    /^20260828_mesh_business_partner_account_bank_linkage\.sql$/m,
  );
});

test("WP15 NEON separates account mapping, masked projection, verification and application", async () => {
  const [control, document, snapshot, functions, migration, guards, manifest] =
    await Promise.all([
      ...
      read(
        "migrations/20260828_neon_business_partner_account_bank_linkage.sql",
      ),
      read(
        "migrations/20260828_neon_business_partner_account_bank_linkage_guards.sql",
      ),
      read("migrations/manifests/neon.txt"),
    ]);
  for (const value of [
    "mesh_business_partner_account_link",
    "mesh_bank_account_disclosure_inbox",
    "mesh_bank_account_projection",
  ])
    assert.match(control, new RegExp(value));
  ...
  assert.match(
    manifest,
    /^20260828_neon_business_partner_account_bank_linkage\.sql$/m,
  );
});
