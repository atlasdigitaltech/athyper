...
test("NEON owns claim intake, canonical service acceptance, reconciliation and invoice binding", async () => {
  const [
    ...
    read("migrations/20260830_neon_external_workforce_mesh_claim_bridge.sql"),
    read("migrations/manifests/neon.txt"),
    read(
      "../packages/services/finance/src/external-workforce/service-sheet-source-service.ts",
    ),
    read(
      "../packages/services/finance/src/external-workforce/kysely-service-sheet-source-repository.ts",
    ),
  ]);
  ...
  assert.match(
    functions,
    /External claim line identity and parent are immutable/,
  );
  assert.match(
    functions,
    /Invoice service-sheet source must identify an accepted canonical line/,
  );
  ...
  assert.doesNotMatch(
    grants,
    /GRANT SELECT,INSERT,UPDATE,DELETE ON[\s\S]{0,120}document\.external_service_entry/,
  );
  assert.match(migration, /Legacy external service-entry backfill/);
  assert.match(migration, /DO \$reconcile\$/);
  assert.match(
    manifest,
    /^20260830_neon_external_workforce_mesh_claim_bridge\.sql$/m,
  );
  assert.match(
    service,
    /never writes the deprecated external_service_entry tables/,
  );
  assert.match(
    repository,
...
test("MESH owns document publications, delivery governance and only a participant-safe projection", async () => {
  const [tables, functions, rls, migration, manifest] = await Promise.all([
    ...
    read("ddl/planes/mesh/control/07_functions.sql"),
    read("ddl/planes/mesh/mesh/10_rls.sql"),
    read("migrations/20260830_mesh_external_workforce_exchange.sql"),
  ...
  for (const forbidden of [
    ...
    assert.doesNotMatch(
      tables.match(
        /CREATE TABLE mesh\.document_business_status_projection[\s\S]*?COMMENT ON TABLE/,
      )?.[0] ?? "",
  ...
  assert.match(
    rls,
    /document_business_status_projection[\s\S]*sender_tenant_id,e\.receiver_tenant_id/,
  );
