...
test("supplier preference is effective-dated, scoped, governed, and tenant-isolated", async () => {
  const [
    ...
    read("ddl/planes/neon/control/10_rls.sql"),
    read("ddl/planes/neon/control/11_grants.sql"),
    read("migrations/20260828_neon_supplier_preference.sql"),
    read("migrations/manifests/neon.txt"),
  ]);
  for (const source of [tables, migration])
    for (const token of [
      "supplier_preference_designation",
  ...
  for (const source of [functions, migration]) {
    assert.match(
      source,
      /Overlapping approved supplier preference scope exists/,
    );
    assert.match(source, /daterange/);
    assert.match(
      source,
      /Supplier preference row version must advance exactly once/,
    );
  }
  ...
  assert.match(
    grants,
    /GRANT SELECT, INSERT, UPDATE ON[\s\S]*control\.supplier_preference_designation/,
  );
...
test("preference authorization and audit contracts are production reference seeds", async () => {
  const [
    ...
    read("migrations/20260828_business_partner_preference_audit_contract.sql"),
    read("migrations/manifests/neon.txt"),
  ...
  for (const source of [audit, auditMigration]) {
    assert.match(
      source,
      /preference\\\.\(created\|approved\|rejected\|revoked\)/,
    );
    assert.match(source, /'revoke'/);
  }
  assert.match(auditBaseline, /ON CONFLICT \(code\) DO NOTHING/);
  assert.match(auditBaseline, /schema_version >= 2/);
  assert.match(audit, /65536, 5/);
  assert.match(auditMigration, /schema_version=3/);
  for (const manifest of [neonManifest, meshManifest, studioManifest]) {
    assert.match(
      manifest,
      /^20260829_business_partner_audit_contract_baseline\.sql$/m,
    );
    assert.ok(
      manifest.indexOf(
        "20260829_business_partner_audit_contract_baseline.sql",
      ) <
        manifest.indexOf(
          "20260828_business_partner_preference_audit_contract.sql",
        ),
    );
  }
});
