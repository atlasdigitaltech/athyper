...
test("supplier registration channels are typed and invitation secrets are never persisted", async () => {
  const [
    ...
    read("migrations/20260829_neon_supplier_registration_channel.sql"),
    read("migrations/20260829_neon_supplier_registration_permissions.sql"),
    ...
    read(
      "../packages/services/master-data/src/business-partner-invitation-jobs.ts",
    ),
  ]);
  for (const token of [
    ...
    assert.match(tables, new RegExp(token));
  for (const token of [
    "supplier_registration_invitation",
    "registration_mode",
    "self_service",
    "on_behalf",
    "representation_evidence_id",
  ...
  assert.match(constraints, /business_partner_invitation_request_fk/);
  assert.match(migration, /supplier_registration_invitation_request_fk/);
  for (const source of [functions, migration])
    for (const evidence of [
      "registration channel identity is immutable",
      "Submitted on-behalf registration requires representation evidence",
      "accepted invitation binding",
    ])
  ...
  assert.match(
    manifest,
    /^20260829_neon_supplier_registration_permissions\.sql$/m,
  );
  assert.match(
    ddlManifest,
  ...
  for (const permission of [
    ...
    assert.match(
      permissions,
      new RegExp(
        `neon\\.supplier_registration\\.${permission.replace(".", "\\.")}`,
      ),
    );
  ...
  assert.match(jobs, /business-partner-invitations-expire/);
