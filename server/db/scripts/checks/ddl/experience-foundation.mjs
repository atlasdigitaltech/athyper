  ...
  return stages
    .map((stage) => {
      const source = readFileSync(
        resolve(databaseRoot, `ddl/common/${table.split(".")[0]}/${stage}`),
        "utf8",
      );
      ...
      if (source.split(start).length !== 2 || source.split(end).length !== 2)
        throw Error(`Missing or duplicate canonical block: ${table}/${stage}`);
...
export const integrityChecks = [
  {
    ...
    legacy:
      "((jsonb_typeof(definition) = 'object'::text) AND ((definition ->> 'schema'::text) = 'atlas-experience-definition/1'::text) AND ((definition ->> 'scope'::text) = scope) AND (octet_length((definition)::text) <= 131072))",
  },
  {
    table: "runtime_meta.experience_surface_projection",
    name: "experience_surface_projection_definition_chk",
    legacy:
      "((jsonb_typeof(definition) = 'object'::text) AND ((definition ->> 'schema'::text) = 'athyper-experience-surface/1'::text) AND ((definition ->> 'id'::text) = surface_key) AND (octet_length((definition)::text) <= 262144))",
  },
  {
    table: "runtime_meta.experience_surface_projection",
...
function checkExpression(check) {
  const line = canonicalBlocks(check.table)
    .split("\n")
...
function alterChecks(table, checks, legacy) {
  return checks
    .map(
      (check) =>
        `ALTER TABLE ${table} DROP CONSTRAINT ${check.name}, ADD CONSTRAINT ${check.name} CHECK (${legacy ? check.legacy : checkExpression(check)});`,
    )
    .join("\n");
}
export function legacyBlocks(table) {
  let source = canonicalBlocks(table);
  for (const check of integrityChecks.filter((c) => c.table === table))
    source = source.replace(
      `CHECK (${checkExpression(check)})`,
      `CHECK (${check.legacy})`,
    );
  return source;
}
  ...
  return tables
    .map((table, i) =>
      canonicalBlocks(table)
        .replaceAll(table, `pg_temp.atlas_experience_expected_${i}`)
...
-- Generate an unapplied candidate for review: pnpm --dir server/db run db:generate:experience-foundation
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  ...
  if (args.length !== 1 || !["--check", "--write"].includes(args[0]))
    throw Error("Expected --check or --write");
  const expected = buildMigration();
  if (args[0] === "--write")
    console.log(
      `Unapplied upgrade candidate: ${writeUpgradeCandidate("experience-foundation.candidate.sql", expected)}`,
    );
  for (const plane of ["studio", "neon", "mesh"]) {
    ...
    if (manifest.includes(migrationName))
      throw Error(
        `Retired baseline migration must not appear in ${plane} manifest`,
      );
    const foundation = readFileSync(
      resolve(databaseRoot, `ddl/planes/${plane}/_manifest.txt`),
      "utf8",
    ).split(/\r?\n/);
    for (const schema of ["ai", "runtime_meta"])
      for (const stage of stages)
        if (
          foundation.filter(
            (line) => line.trim() === `common/${schema}/${stage}`,
          ).length !== 1
        )
          throw Error(
            `Canonical stage missing/duplicated: ${plane}/${schema}/${stage}`,
          );
  }
  console.log(
    "Experience canonical blocks and three-plane foundation manifests verified.",
  );
}
