...
export async function applyDisposableDdlManifest(options: {
  ...
  const manifestPath = resolve(ddlRoot, "planes", options.plane, "_manifest.txt");
  const manifest = await readFile(manifestPath, "utf8");
  const paths = manifest.split(/\r?\n/u).map((value) => value.trim()).filter((value) => value && !value.startsWith("#"));
  const evidence: Array<Readonly<{ path: string; sha256: string }>> = [];
  for (const path of paths) {
    const fullPath = await confined(resolve(ddlRoot, path));
    const sql = await expandSql(fullPath, new Set());
    evidence.push(Object.freeze({ path, sha256: sha256(sql) }));
    const prelude = `SELECT set_config('app.database_plane','${options.plane}',false);\nSELECT set_config('app.current_principal_id','00000000-0000-0000-0000-000000000000',false);\n`;
    try {
      await pipe("docker", ["exec", "-i", options.container, "psql", "-U", user, "-d", database, "--single-transaction", "--no-psqlrc", "-v", "ON_ERROR_STOP=1", "-f", "-"], prelude + sql);
    } catch (error) {
      throw new Error(`DDL failed at ${path}`, { cause: error });
    }
  }
  return Object.freeze({ plane: options.plane, database, manifest: relative(databaseRoot, manifestPath), manifestSha256: sha256(manifest), files: Object.freeze(evidence), appliedAt: new Date().toISOString() });
}
