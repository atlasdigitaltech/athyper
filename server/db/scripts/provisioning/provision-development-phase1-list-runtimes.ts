...
export function buildDevelopmentPhase1ListProjection(plane: Plane, permissionId: string, permissionKind: "entity_operation" | "system_action" = "entity_operation") {
  const definition = definitions[plane], releaseNo = 5, sourceVersion = "development-v5";
  ...
  const contract = { schema: "athyper.meta-entity-contract/2.1", entity: { entityCode: definition.entityCode, entityClass: plane === "studio" ? "metadata" : "business" }, runtime: { plane, backingKind: "table", readMode: "cursor", writeMode: "governed_adapter" }, storage: { schema: definition.storage.schema, object: definition.storage.object }, operations: [{ code: "read", permissionCode: definition.permissionCode },{code:"export",permissionCode:definition.permissionCode},{code:"import",permissionCode:definition.permissionCode}] };
  const contractHash = sha256(contract);
  const descriptor = { schema: "athyper.entity-runtime-descriptor/1.0", entityCode: definition.entityCode, planeKey: plane, storage: definition.storage, fields: definition.fields, listPresentation: {...definition.listPresentation,dataOperations:{exportFormats:["xlsx","csv","json","ndjson"],importAdapterKey:definition.importAdapterKey,importOperations:plane==="mesh"?["create","update","upsert","delete","replace"]:["create","update","upsert","replace"],importOperationPermissions:Object.fromEntries((plane==="mesh"?["create","update","upsert","delete","replace"]:["create","update","upsert","replace"]).map(mode=>[mode,[definition.importPermissionCode]])),importFormats:["xlsx","csv","json"],importMaxRows:plane==="mesh"?50000:1000,importMaxFileBytes:26214400,allowTemplateDownload:true,draftOnly:plane==="studio"}}, operations: { read: { code: "read", permissionCode: definition.permissionCode },export:{code:"export",permissionCode:definition.permissionCode},import:{code:"import",permissionCode:definition.permissionCode} }, source: { entity_id: entityId, release_hash: contractHash }, operation_scope_bindings: [{ bindingId, scopeBindingId, sourceEntityOperationId: operationId, entityCode: definition.entityCode, operationKey: "read", permissionId, permissionCode: definition.permissionCode, permissionKind, decisionMode: "collection", scopeKind: definition.scopeBinding.scopeKind, coordinateSource: definition.scopeBinding.coordinateSource, coordinateKey: definition.scopeBinding.coordinateKey, resolverKey: definition.scopeBinding.resolverKey }] };
...
export async function provisionDevelopmentPhase1ListRuntime(options: { plane: Plane; databaseUrl: string; confirmation?: string; dryRun?: boolean }) {
  ...
  try {
    ...
    await one<{ id:string }>(client,"SELECT id::text AS id FROM authz.permission WHERE canonical_code=$1 AND permission_kind IN ('entity_operation','system_action') AND status='published'",[definition.importPermissionCode]);
