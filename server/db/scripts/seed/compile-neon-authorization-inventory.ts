...
export async function compileNeonAuthorizationInventory(): Promise<{
  ...
  const [contract, organizationBoundary] = await Promise.all([
    loadJson<NeonReviewedSlicesContract>(contractPath),
    loadJson<StudioNeonOrganizationBoundaryContract>(organizationBoundaryPath),
  ]);
  ...
  const discovered = (await Promise.all(ddlSources.map((source) => discoverInventoryTables(dbRoot, source, ["master", "document"])))).flat()
  ...
  const artifactBody = {
    ...
    source: {
      ...
      studioNeonOrganizationBoundary: relativeToDb(dbRoot, organizationBoundaryPath),
    },
    counts: {
...
async function main(): Promise<void> {
  ...
  await publishInventoryArtifacts({
    ...
    staleMessage: "Neon authorization inventory artifacts are stale",
  });
