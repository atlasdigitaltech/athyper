...
const contractPath = resolve(
  dbRoot,
  "seed/contracts/base/seed-contract.v1.json",
);
const baselinePath = resolve(
  dbRoot,
  "seed/contracts/base/pre-contract-baseline.v1.json",
);
const legacyReportPath = resolve(
  dbRoot,
  "seed/migration/wave1-legacy-lint-report.v1.json",
);
...
function writeRelations(
  source: string,
): Array<{ match: RegExpExecArray; relation: string }> {
  const pattern =
    /\b(insert\s+into|update(?!\s+set\b)|delete\s+from|merge\s+into|copy|truncate(?:\s+table)?)\s+(?:only\s+)?(?:"([a-z_][a-z0-9_]*)"|([a-z_][a-z0-9_]*))\s*\.\s*(?:"([a-z_][a-z0-9_]*)"|([a-z_][a-z0-9_]*))/gi;
...
export function lintSeedSource(
  source: string,
  file = "seed.sql",
  ...
  for (const key of activeContract.requiredHeaders) {
    if (!metadata.has(key))
      add("metadata.missing", null, `missing required header '${key}'`);
  }
  if (metadata.get("seed-contract-version") !== "1") {
    add(
      "metadata.contract-version",
      null,
      "seed-contract-version must equal 1",
    );
  }
  if (
    !/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(
      metadata.get("seed-pack-version") ?? "",
    )
  ) {
    add(
      "metadata.pack-version",
      null,
      "seed-pack-version must be semantic versioning",
    );
  }
  if (
    !activeContract.allowedPlanes.includes(metadata.get("seed-plane") ?? "")
  ) {
    add(
      "metadata.plane",
      null,
      `seed-plane must be one of ${activeContract.allowedPlanes.join(", ")}`,
    );
  }
  if (
    !activeContract.allowedTenantScopes.includes(
      metadata.get("seed-tenant-scope") ?? "",
    )
  ) {
    add(
      "metadata.tenant-scope",
      null,
      `seed-tenant-scope must be one of ${activeContract.allowedTenantScopes.join(", ")}`,
    );
  }
  if (
    !activeContract.allowedDataClasses.includes(
      metadata.get("seed-data-class") ?? "",
    )
  ) {
    add(
      "metadata.data-class",
      null,
      `seed-data-class must be one of ${activeContract.allowedDataClasses.join(", ")}`,
    );
  }
      ...
      for (const key of activeContract.provenanceRequiredFields) {
        if (
          ...
          add(
            "metadata.provenance",
            null,
            `seed-provenance requires non-empty '${key}'`,
          );
        }
      }
      if (
        typeof provenance.retrieved_at === "string" &&
        !/^\d{4}-\d{2}-\d{2}$/.test(provenance.retrieved_at)
      ) {
        add(
          "metadata.provenance",
          null,
          "seed-provenance.retrieved_at must use YYYY-MM-DD",
        );
      }
    } catch {
      add(
        "metadata.provenance",
        null,
        "seed-provenance must be valid single-line JSON",
      );
    }
  }

  const assertions = new Set(
    (metadata.get("seed-assertions") ?? "")
      .split(",")
  ...
  for (const assertion of activeContract.requiredAssertions) {
    if (!assertions.has(assertion))
      add(
        "metadata.assertions",
        null,
        `seed-assertions must include '${assertion}'`,
      );
    const marker = new RegExp(
      `--\\s*seed-assertion:\\s*${escaped(assertion)}\\b`,
      "i",
    );
    if (!marker.test(source))
      add("assertion.marker", null, `missing SQL marker for '${assertion}'`);
  }
  if (!/\braise\s+exception\b/i.test(source)) {
    add(
      "assertion.failure",
      null,
      "seed assertions and scope guards must fail with RAISE EXCEPTION",
    );
  }
  ...
  if (!/^(?:exact|minimum):\d+$|^query:[a-z][a-z0-9_.-]+$/i.test(expected)) {
    add(
      "metadata.expected-count",
      null,
      "seed-expected-row-count must be exact:N, minimum:N, or query:identifier",
    );
  }
  const naturalKey = metadata.get("seed-natural-key") ?? "";
  if (
    !/^[a-z_][a-z0-9_]*\.[a-z_][a-z0-9_]*\([a-z0-9_, ]+\)(?:;[a-z_][a-z0-9_]*\.[a-z_][a-z0-9_]*\([a-z0-9_, ]+\))*$/i.test(
      naturalKey,
    )
  ) {
    add(
      ...
      null,
      "seed-natural-key must declare schema.table(columns), separated by semicolons when needed",
    );
  }
  ...
  if (!new Set(["true", "false"]).has(crossFileIds ?? "")) {
    add(
      "identity.cross-file",
      null,
      "seed-cross-file-ids must be true or false",
    );
  }
  const idStrategy = metadata.get("seed-id-strategy") ?? "";
  if (
    !/^(?:natural-key-only|database-generated|deterministic-uuid:[a-z][a-z0-9_.:-]+)$/i.test(
      idStrategy,
    )
  ) {
  ...
  if (
    crossFileIds === "true" &&
    !idStrategy.startsWith("deterministic-uuid:")
  ) {
    add(
      "identity.deterministic",
      null,
      "cross-file IDs require deterministic-uuid:<immutable-namespace>",
    );
  }

  if (
    !/current_setting\s*\(\s*'app\.database_plane'\s*,\s*true\s*\)/i.test(
      source,
    )
  ) {
    add(
      "scope.plane-assertion",
      null,
      "seed must assert app.database_plane before data changes",
    );
  }
  if (metadata.get("seed-tenant-scope") === "tenant") {
    if (
      !/current_setting\s*\(\s*'app\.seed_tenant_id'\s*,\s*true\s*\)/i.test(
        source,
      )
    ) {
      add(
        "scope.tenant-assertion",
        null,
        "tenant seed must resolve app.seed_tenant_id",
      );
    }
    if (!/\braise\s+exception\b/i.test(source)) {
      add(
        "scope.tenant-assertion",
        null,
        "tenant seed must reject a missing or invalid tenant",
      );
    }
  }
  const hardcodedTenant =
    source.match(
      /(?:app\.seed_tenant_id|\btenant_id)\s*(?:=|:=|,)\s*'([0-9a-f]{8}-[0-9a-f-]{27})'/i,
    ) ??
    source.match(
      /set_config\s*\(\s*'app\.seed_tenant_id'\s*,\s*'[0-9a-f-]{36}'/i,
    ) ??
    source.match(
      /insert\s+into\s+[a-z_][a-z0-9_]*\.[a-z_][a-z0-9_]*\s*\([^)]*\btenant_id\b[^)]*\)\s*(?:overriding\s+\w+\s+value\s*)?values\s*\([^)]*'(?!00000000-0000-0000-0000-000000000000)[0-9a-f]{8}-[0-9a-f-]{27}'/i,
    );
  if (hardcodedTenant)
    add(
      "scope.hardcoded-tenant",
      hardcodedTenant,
      "hard-coded tenant identifiers are prohibited",
    );

  const forbiddenDdl =
    structuralSql.match(
      /\b(?:create|alter|drop)\s+(?:or\s+replace\s+)?(?:table|policy|trigger|function|procedure|domain|type|schema|index|extension)\b/i,
    ) ??
    structuralSql.match(
      /\balter\s+table\b[^;]*\b(?:enable|force|disable)\s+row\s+level\s+security\b/i,
    ) ??
    structuralSql.match(/\bgrant\s+|\brevoke\s+/i);
  if (forbiddenDdl)
    add(
      "structure.forbidden-ddl",
      forbiddenDdl,
      "seed packs cannot contain DDL, policies, triggers, grants, or table creation",
    );

  const destructive =
    structuralSql.match(/\bdelete\s+from\b/i) ??
    structuralSql.match(/\btruncate(?:\s+table)?\b/i);
  if (destructive)
    add(
      "lifecycle.destructive-delete",
      destructive,
      "retire obsolete values through status/effectivity; DELETE/TRUNCATE is prohibited",
    );

  const runtimeOnly = new Set(
    activeContract.runtimeOnlyRelations.map((relation) =>
      relation.toLowerCase(),
    ),
  );
  const reconciliationOnly = new Set(
    activeContract.reconciliationOnlyRelations.map((relation) =>
      relation.toLowerCase(),
    ),
  );
  ...
  const relationPattern =
    /\b(?:insert\s+into|update(?!\s+set\b)|delete\s+from|merge\s+into|from|join)\s+(?:only\s+)?(?:lateral\s+)?((?!lateral\b)[a-z_][a-z0-9_.]*)\b/gi;
  for (const match of structuralSql.matchAll(relationPattern)) {
    const name = match[1]!.toLowerCase();
    if (
      !name.includes(".") &&
      !ctes.has(name) &&
      !["excluded", "values"].includes(name)
    ) {
      add(
        "relation.unqualified",
        match,
        `relation '${name}' must be schema-qualified or declared as a CTE`,
      );
    }
  }
  ...
  for (const relation of activeContract.legacyRelations) {
    const match = structuralSql.match(
      new RegExp(`\\b${escaped(relation)}\\b`, "i"),
    );
    if (match)
      add(
        "relation.legacy-name",
        match,
        `legacy relation '${relation}' is prohibited by the new DDL contract`,
      );
  }
  ...
  for (const segment of insertSegments(structuralSql)) {
    ...
    const membership = segment.text.match(
      /^insert\s+into\s+control\.owner_type_purpose\s*\(([^)]+)\)/i,
    );
    const membershipKeys = ["owner_type_id", "capability", "purpose_code"];
    const conflictKeys = segment.text
      .match(/on\s+conflict\s*\(([^)]+)\)/i)?.[1]
      ?.split(",")
      .map((key) => key.trim().toLowerCase());
    const keyOnlyMembership = Boolean(
      membership &&
      membership[1]!
        .split(",")
        .every((column) =>
          [...membershipKeys, "created_at", "created_by"].includes(
            column.trim().toLowerCase(),
          ),
        ) &&
      conflictKeys?.length === 3 &&
      membershipKeys.every((key) => conflictKeys.includes(key)),
    );
    if (
      !keyOnlyMembership &&
      /\bon\s+conflict\b[\s\S]*?\bdo\s+nothing\b/i.test(segment.text)
    ) {
      add(
        "convergence.do-nothing",
        {
          0: "",
          index: segment.index,
          input: source,
    ...
    if (/\bdo\s+update\s+set\b/i.test(segment.text)) {
      ...
      const updateSet =
        segment.text.match(
          /\bdo\s+update\s+set\b([\s\S]*?)(?:\bwhere\b|;|$)/i,
        )?.[1] ?? "";
      if (/\bcreated_(?:at|by)\s*=/i.test(updateSet)) {
        add(
          "convergence.created-audit",
          {
            0: "",
            index: segment.index,
            input: source,
  ...
  if (metadata.get("seed-data-class") === "production_reference") {
    if (metadata.get("seed-demo-data") !== "false") {
      add(
        "fixture.production-demo-flag",
        null,
        "production_reference requires seed-demo-data: false",
      );
    }
    const demo = source.match(
      /\b(?:demo_tenant|test_tenant|example_tenant|technostat|cirrusatlantic)\b|[a-z0-9._%+-]+@example\.(?:com|org|net)/i,
    );
    if (demo)
      add(
        "fixture.production-demo-data",
        demo,
        "production reference seed contains a demo/test/customer fixture marker",
      );
  }
...
async function legacySeedFiles(): Promise<string[]> {
  return [
    ...
    (path) =>
      !path.replaceAll("\\", "/").includes("/seed/platform/003_control/"),
  );
}
  ...
  return Promise.all(
    files.sort().map(async (path) => {
      ...
      return {
        ...
        findings: lintSeedSource(source, repoPath(path)),
      };
    }),
  );
}
...
function summary(results: Awaited<ReturnType<typeof analyze>>) {
  ...
  return {
    ...
    filesWithFindings: results.filter((result) => result.findings.length > 0)
      .length,
    findings: all.length,
    byRule: Object.fromEntries(
      [...new Set(all.map((item) => item.ruleId))]
        .sort()
        .map((ruleId) => [
          ruleId,
          all.filter((item) => item.ruleId === ruleId).length,
        ]),
    ),
  };
}
...
async function main(): Promise<void> {
  ...
  const files = legacyReport
    ? await legacySeedFiles()
    : await strictSeedFiles();
  ...
  if (writeBaseline) {
        ...
        results
          ...
          .map((result) => [
            result.path,
            {
              ...
              ruleIds: [
                ...new Set(result.findings.map((item) => item.ruleId)),
              ].sort(),
            },
          ]),
      ),
    ...
    console.log(
      `Pre-contract baseline written for ${Object.keys(baseline.files).length} files.`,
    );
    return;
  }
  ...
  for (const result of results) {
    ...
    if (
      !accepted ||
      accepted.sourceSha256 !== result.sourceSha256 ||
      ruleIds.some((ruleId) => !accepted.ruleIds.includes(ruleId))
    ) {
      failures.push(result);
    }
  }
  const staleBaseline = Object.keys(baseline.files).filter(
    (path) => !results.some((result) => result.path === path),
  );
  if (failures.length > 0 || staleBaseline.length > 0) {
    for (const result of failures) {
      console.error(`\n${result.path} (${result.findings.length} finding(s))`);
      for (const item of result.findings)
        console.error(`  ${item.line} [${item.ruleId}] ${item.message}`);
    }
    ...
    throw new Error(
      `${failures.length} changed/new seed file(s) violate ${contract.contractVersion}; ${staleBaseline.length} stale baseline entr${staleBaseline.length === 1 ? "y" : "ies"}`,
    );
  }
  console.log(
    `Seed contract lint passed: ${results.length} files checked, ${Object.keys(baseline.files).length} unchanged pre-contract files baseline-bound.`,
  );
}
