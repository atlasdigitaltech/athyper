...
node server/db/scripts/seed/generators/generate-hs-sql.cjs

# Regenerate UNSPSC codes SQL
node server/db/scripts/seed/generators/generate-unspsc-sql.cjs

# Verify reference data counts against live DB (requires DATABASE_URL)
node server/db/scripts/seed/generators/verify-counts.mjs
```
