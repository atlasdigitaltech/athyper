...
function assertLocalDatabase(value: string, database: string) { const url = new URL(value); if (!["localhost", "127.0.0.1", "::1"].includes(url.hostname) || url.pathname !== `/${database}`) throw new Error(`BS360 fixtures require local ${database}`); }
