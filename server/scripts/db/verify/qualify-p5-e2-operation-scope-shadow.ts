...
import { NoopAuthorizationDecisionAuditSink, ProductionAuthorizationDecisionService } from "@athyper/svc-iam";
import { NormalizedOperationScopeAuthorizationRepository } from "@athyper/svc-iam";
import { OperationScopeShadowComparator } from "@athyper/svc-iam";
import { SqlOperationScopeShadowSink } from "@athyper/svc-iam";
...
async function qualify(input: { plane: "neon" | "mesh"; url: string; database: string; corpus: Corpus; runId: string }): Promise<{ samples: number; matches: number; mismatches: number; errors: number; persisted: number }> {
  ...
  try {
    const bindingResult = await pool.query<Binding>(`SELECT binding.plane_code,binding.entity_code,
      ...
      binding.decision_mode::text,scope.scope_kind::text
      FROM authz.entity_operation_binding binding
      JOIN authz.entity_operation_scope_binding scope ON scope.entity_operation_binding_id=binding.id
...
const corpus = JSON.parse(await readFile(resolve(import.meta.dirname, "../../../db/scripts/verify/config/p5-e2-operation-scope-golden-corpus.v1.json"), "utf8")) as Corpus;
