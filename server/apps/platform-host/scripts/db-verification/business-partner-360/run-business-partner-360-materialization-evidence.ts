...
export async function runBusinessPartner360MaterializationEvidence(options: {
  ...
  let retained:
    | Omit<
        BusinessPartner360MaterializationEvidence,
        "capturedAt" | "database" | "repository" | "rolledBack" | "cleanup"
      >
    | undefined;
  ...
  try {
    try {
      await database.transaction().execute(async (transaction) => {
        ...
        assert(
          equalCounts(created.extensionSummary.counts, requestedCounts),
          "production repository did not persist all typed request children",
        );
        const approvedVersion = await approveThroughRepository(
          repository,
          transaction,
          coordinates,
          created,
        ...
        const applicationFingerprint = hash(
          `${created.extensionSummary.fingerprint}:approved:${approvedVersion}`,
        );
        ...
        assert(
          ...
          "production repository did not apply the typed request",
        );
        assert(
          equalCounts(
            applied.materialization.extensionMaterializationCounts,
            requestedCounts,
          ),
          "typed materialization counts differ from the approved request",
        );
        const observation = await observeApplication(
          transaction,
          coordinates.tenantId,
          created.id,
          applied.materialization.businessPartnerId,
        );
        assert(
          observation.evidenceItems === 7,
          "every typed child must have immutable application evidence",
        );
        assert(
          observation.snapshotRows === 1,
          "typed application must capture one immutable snapshot",
        );
        ...
        assert(
          ...
          "exact application replay was not returned",
        );
        const stableCoordinates =
          replay.materialization.businessPartnerId ===
            applied.materialization.businessPartnerId &&
          replay.materialization.snapshotId ===
            applied.materialization.snapshotId;
        assert(
          stableCoordinates,
          "exact replay changed materialization coordinates",
        );
        ...
        const legacy = await createLegacyRequest(
          repository,
          transaction,
          coordinates,
        );
        ...
        assert(
          legacyApplied && !legacyApplied.replayed,
          "legacy policy probe did not apply through the production repository",
        );
        assert(
          equalCounts(
            legacyApplied.materialization.extensionMaterializationCounts,
            ZERO_COUNTS,
          ),
          "legacy JSON was reinterpreted as typed children",
        );
        const legacyEvidence = await countMaterializationItems(
          transaction,
          coordinates.tenantId,
          legacy.requestId,
        );
        assert(
          legacyEvidence === 0,
          "legacy JSON created typed materialization evidence",
        );
      ...
      throw new Error(
        "materialization evidence transaction unexpectedly committed",
      );
    } catch (error) {
    ...
    assert(
      retained,
      "materialization evidence was not captured before rollback",
    );
    const cleanup = await observeCleanup(database);
    assert(
      Object.values(cleanup).every((count) => count === 0),
      "materialization evidence rollback left persistent rows",
    );
    return {
...
async function loadCoordinates(transaction: Tx) {
  ...
  if (!row)
    throw new Error(
      "the Athyper acceptance tenant lacks materialization prerequisites",
    );
  return {
...
async function setContext(transaction: Tx, tenantId: string, actorId: string) {
  await sql`SELECT set_config('app.database_plane','neon',true),
    ...
    set_config('app.current_principal_id',${actorId},true)`.execute(
    transaction,
  );
}
...
async function createPrerequisites(
  ...
  const tax = (
    await sql<{ id: string }>`INSERT INTO master.tax_jurisdiction(
  ...
  const commodity = (
    await sql<{ id: string }>`INSERT INTO master.commodity_category(
  ...
  if (!tax || !commodity)
    throw new Error("materialization prerequisites were not created");
  return {
  ...
  const validated = await repository.recordValidation(
    {
      ...
      result: {
        ...
        ruleset: {
          ...
          version: 1,
          hash: hash(`bs360-p0-${suffix}-ruleset`),
        },
        valid: true,
        findings: [
          {
...
async function createLegacyRequest(
  ...
  const inserted = (
    await sql<{ id: string }>`INSERT INTO document.business_partner_request(
  ...
  const staged = await repository.get(
    coordinates.tenantId,
    inserted.id,
    transaction,
  );
  assert(
    staged && staged.extensionSummary.mode === "legacy_untyped",
    "legacy request mode was not retained",
  );
  const rowVersion = await approveThroughRepository(
    repository,
    transaction,
    coordinates,
    staged,
...
async function observeApplication(
  ...
  return {
    ...
    targetTables: Object.freeze(
      [...new Set(rows.map((row) => row.target_table))].sort(),
    ),
    snapshotRows,
  };
}

async function countMaterializationItems(
  transaction: Tx,
  tenantId: string,
  requestId: string,
) {
  return Number(
    (
      await sql<{ count: number }>`SELECT count(*)::int AS count
    FROM document.business_partner_request_materialization_item
    WHERE tenant_id=${tenantId}::uuid AND request_id=${requestId}::uuid`.execute(
        transaction,
      )
    ).rows[0]?.count ?? 0,
  );
...
async function observeCleanup(database: Kysely<Database>) {
  const row = (
    ...
    (SELECT count(*)::int FROM master.business_partner WHERE code='BP.BS360.P0.LEGACY') AS legacy_partner_rows`.execute(
      database,
    )
  ).rows[0];
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const url =
    option(process.argv.slice(2), "--neon-database-url") ??
  ...
  process.stdout.write(
    `${JSON.stringify(
      await runBusinessPartner360MaterializationEvidence({
        neonDatabaseUrl: url,
