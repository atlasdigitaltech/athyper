...
import { createKyselyWorkflowRepository, createWorkflowService, registerWorkflowRoutes } from "@athyper/server-platform-workflow";
...
import {assertControlServiceRoutePlaneSafety,controlAdminFoundation,createAuthorizationManagementService,createBankValidationService,createConnectorControlService,createCycleConfigService,createEntitlementControlService,createExactPlaneAuthorizationRepositoryProvider,createFeatureFlagService,createLookupService,createParameterService,createRoundingService,createSafeAuthorizationManagementRolloutSelector,KyselyCycleTemplateRepository,registerAuthorizationManagementRoutes,registerControlServiceRoutes,registerCycleConfigRoutes,type ControlServiceRouteFlags}from"@athyper/server-platform-control-admin";
import {ATLAS_KNOWLEDGE_QUEUE,ATLAS_MONITORING_QUEUE,INGEST_ATLAS_KNOWLEDGE_JOB,RUN_ATLAS_DRIFT_JOB,AtlasAgentRuntime,AtlasBindingRegistry,AtlasProviderRegistry,AtlasRegisteredToolCoordinator,AtlasThreadService,AtlasToolRegistry,AtlasToolService,createAtlasA2Services,createAtlasDriftHandler,createAtlasKnowledgeIngestionHandler,KyselyAtlasToolProposalStore,registerAtlasAdminRoutes,registerAtlasRoutes,type AtlasKnowledgeJobAuthority,type AtlasPromptResolver}from"@athyper/server-platform-ai";
...
import {BookPeriodService,CloseReadinessService,FinancePostingGuard,KyselyBookPeriodRepository,KyselyCloseReadinessRepository,KyselyFinanceFoundationReader,KyselyRoundingPolicyReader,RoundingResolver,SnapshotFinanceSourceDocumentReader,financeFoundation,snapshotFinancePermissionChecker}from"@athyper/server-service-finance";
...
export function registerServices(
  ...
  if(container.adapters.neonDatabase){
    ...
    if(financeRoutesEnabled)container.platform.httpRegistrars.push(application=>registerFinanceRoutes(application,{authenticate:createIamAuthenticationMiddleware(iam),readContext:readVerifiedRequestContext,periods,rounding,postingGuard}));
  ...
  container.platform.controlAdmin={...(cycleConfig?{cycleConfig}:{}),...(controlServices?{services:controlServices}:{}),...(authorizationManagement?{authorizationManagement}:{}),routeFlags:controlRouteFlags,routesEnabled:controlRoutesEnabled||cycleConfigRoutesEnabled||authorizationRoutesEnabled};
  ...
  if (dependencies.repository || Object.keys(recordDatabases).length > 0) {
    ...
    container.services.records = { queries, mutations, ...(snapshots?{snapshots}:{}) };
  ...
  if(objectStorage&&objectStorageBucket&&malwareScanner&&Object.keys(metadataDatabases).length>0){
    ...
    const attachments=createAttachmentLifecycle({transactions,repository:attachmentRepository,storage:objectStorage,scanner:malwareScanner,quota,quotaPolicies,scheduler:{scheduleExtraction:async identity=>{await extractionScheduler?.schedule({...identity});},scheduleDerivatives:async request=>{await derivativeScheduler?.schedule(request);},schedulePurge:async()=>undefined}});
...
export function registerAtlas(
  ...
  const runtime = new AtlasAgentRuntime({ admission: dependencies.admission, modelPolicy: dependencies.modelPolicy, bindings, providers, credentials: operations.resolver, threads, runs: dependencies.runs, ledger: dependencies.usageLedger, prompts: dependencies.prompts, ...(coordinator ? { tools: coordinator } : {}), maxInputCharacters: 50_000, maxToolRounds: toolsEnabled ? 3 : 0 });
...
function registerPublication(container:Container,config:HostConfig,databases:Partial<Record<PlaneKey,Kysely<Record<string,never>>>>,iam:NonNullable<Container["platform"]["iam"]>,authorizer:NonNullable<Container["platform"]["authorizer"]>,audit:NonNullable<Container["platform"]["audit"]>):void{
  ...
  if(container.runtimes.jobs&&config.publication.apiEnabled){container.platform.httpRegistrars.push(application=>registerPublicationRoutes(application,{authenticate:createIamAuthenticationMiddleware(iam),readContext:readVerifiedRequestContext,authorizer,audit,authority,jobs:container.runtimes.jobs!,apiEnabled:true}));}
}
...
function createDatabaseOutboxWriter(source: "records" | "workflow" | "documents"|"collaboration"|"integration"|"governance"|"finance"): OutboxWriter<RecordTransaction> {
  return {
