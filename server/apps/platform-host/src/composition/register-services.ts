...
export function registerServices(
  ...
  if (container.adapters.neonDatabase) {
    ...
    const workforce=createWorkforceService<RecordTransaction>({authorizer:businessPartnerAuthorizer,repository:new KyselyWorkforceRepository(),transactions,requests:businessPartnerRequests,audit,outbox:createDatabaseOutboxWriter("business-partner")});
    ...
    container.runtimes.health.register("business-partner-invitations.neon",async()=>{try{const row=(await sql<{invitation_table:string|null;compatibility_view:string|null;policy_table:string|null}>`SELECT to_regclass('document.business_partner_invitation')::text invitation_table,to_regclass('document.supplier_registration_invitation')::text compatibility_view,to_regclass('document.business_partner_invitation_applicant_policy')::text policy_table`.execute(neonDatabase)).rows[0];return row?.invitation_table&&row.compatibility_view&&row.policy_table?{status:"healthy"}:{status:"unhealthy",message:"Generalized invitation aggregate or compatibility policy is incomplete"};}catch(error){return{status:"unhealthy",message:error instanceof Error?error.message:"Business Partner invitation readiness check failed"};}});
