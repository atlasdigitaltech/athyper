...
import {ATLAS_KNOWLEDGE_QUEUE,ATLAS_MONITORING_QUEUE,INGEST_ATLAS_KNOWLEDGE_JOB,RUN_ATLAS_DRIFT_JOB,AtlasAgentRuntime,AtlasBindingRegistry,AtlasProviderRegistry,AtlasRegisteredToolCoordinator,AtlasThreadService,AtlasToolRegistry,AtlasToolService,AtlasExperienceConfigurationService,KyselyAtlasAttachmentContextResolver,KyselyAtlasExperienceConfigurationRepository,KyselyAtlasTenantQuotaManager,createAtlasA2Services,createAtlasDriftHandler,createAtlasKnowledgeIngestionHandler,KyselyAtlasToolProposalStore,registerAtlasAdminRoutes,registerAtlasExperienceRoutes,registerAtlasRoutes,type AtlasKnowledgeJobAuthority,type AtlasPromptResolver}from"@athyper/server-platform-ai";
...
export function registerServices(
  ...
  if (container.adapters.neonDatabase) {
    ...
    const workforce=createWorkforceService<RecordTransaction>({authorizer:businessPartnerAuthorizer,repository:new KyselyWorkforceRepository(),transactions,requests:businessPartnerRequests,audit,outbox:createDatabaseOutboxWriter("business-partner"),...(protectedValues?{protectedValues}:{})});
