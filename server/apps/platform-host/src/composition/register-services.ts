...
import {ATLAS_KNOWLEDGE_QUEUE,ATLAS_MONITORING_QUEUE,INGEST_ATLAS_KNOWLEDGE_JOB,RUN_ATLAS_DRIFT_JOB,AtlasAgentRuntime,AtlasBindingRegistry,AtlasProviderRegistry,AtlasRegisteredToolCoordinator,AtlasSurfaceDraftGenerator,AtlasThreadService,AtlasToolRegistry,AtlasToolService,AtlasExperienceConfigurationService,KyselyAtlasAttachmentContextResolver,KyselyAtlasExperienceConfigurationRepository,KyselyAtlasTenantQuotaManager,createAtlasA2Services,createAtlasDriftHandler,createAtlasKnowledgeIngestionHandler,KyselyAtlasToolProposalStore,registerAtlasAdminRoutes,registerAtlasExperienceRoutes,registerAtlasRoutes,registerAtlasSurfaceDraftRoutes,type AtlasKnowledgeJobAuthority,type AtlasPromptResolver}from"@athyper/server-platform-ai";
...
import {BUSINESS_PARTNER_INVITATION_MAINTENANCE_QUEUE,createBusinessPartnerEligibilityService,createBusinessPartnerInvitationExpiryHandler,createBusinessPartnerInvitationExternalGuard,createBusinessPartnerInvitationService,createBusinessPartnerRequestService,createBusinessPartnerRequestValidator,createSupplierActivationReevaluationHandler,createSupplierQualificationExpiryHandler,createWorkforceService,EXPIRE_BUSINESS_PARTNER_INVITATIONS_JOB,EXPIRE_SUPPLIER_QUALIFICATIONS_JOB,KyselyBusinessPartnerEligibilityRepository,KyselyBusinessPartnerInvitationRepository,KyselyBusinessPartnerRequestRepository,KyselyWorkforceRepository,KyselyWorkforceRequestRepository,MasterDataError,REEVALUATE_SUPPLIER_ACTIVATIONS_JOB,registerBusinessPartnerEligibilityRoutes,registerBusinessPartnerInvitationRoutes,registerBusinessPartnerRequestRoutes,registerWorkforceRoutes,SUPPLIER_READINESS_MAINTENANCE_QUEUE} from "@athyper/server-service-master-data";
import {createBusinessPartner360Service,createHttpBusinessPartner360MeshNetworkAdapter,createLastValidBusinessPartner360DefinitionResolver,createSecretStoreProtectedValueResolver,createUnavailableBusinessPartner360ActivityProviders,KyselyBusinessPartner360Repository,parseBusinessPartner360Definition,registerBusinessPartner360Routes} from "@athyper/server-service-master-data";
...
export function registerServices(
  ...
  if(container.adapters.meshDatabase){
    ...
    const profilePublications=createBusinessPartnerProfilePublicationService({authorizer:meshProfileAuthorizer,repository:new KyselyBusinessPartnerProfilePublicationRepository(),transactions:transactions as never,audit:audit as never,outbox:createDatabaseOutboxWriter("mesh-business-partner") as never,definition:new LocalMeshBusinessPartnerDefinitionConsumer({local:container.services.publication?.projections.mesh??new KyselyLocalProjectionRepository(meshDatabase)})});
  ...
  if (container.adapters.neonDatabase) {
    ...
    const businessPartnerAuthorizer=createPermissionAuthorizer({policyGate:{async evaluate(input){
      ...
      if(input.permissionCode==="neon.customer.credit.create")return{allowed:input.resource?.["creditControl"]===true&&input.resource?.["makerCheckerEnforced"]===true,sodSatisfied:true,reason:"customer_credit_control_required"};
      if(input.permissionCode==="neon.customer.credit.decide"){const creator=input.resource?.["createdBy"],separated=typeof creator==="string"&&creator!==input.context.principalId,controlled=input.resource?.["creditControl"]===true&&input.resource?.["makerCheckerEnforced"]===true;return{allowed:controlled&&separated,sodSatisfied:controlled&&separated,reason:"customer_credit_separation_required"};}
      if(["neon.customer.lifecycle.activate","neon.customer.lifecycle.suspend","neon.customer.lifecycle.reactivate"].includes(input.permissionCode)){const suspend=input.permissionCode.endsWith("suspend"),pinned=input.resource?.["readinessEvidencePinned"]===true;return{allowed:input.resource?.["makerCheckerEnforced"]===true&&(suspend||pinned),sodSatisfied:true,reason:"customer_lifecycle_control_required"};}
      if(input.permissionCode==="neon.relationship.business_partner.activate"){const pinned=input.resource?.["readinessEvidencePinned"]===true,eligible=input.resource?.["eligible"]===true,internal=input.resource?.["externalApplicant"]===false,elevated=input.context.assurance==="elevated";return{allowed:pinned&&eligible&&internal&&elevated,sodSatisfied:pinned&&internal,reason:"supplier_activation_requires_pinned_readiness_and_elevated_internal_authority"};}
      if(["neon.relationship.business_partner_request.create","neon.relationship.business_partner_request.read","neon.relationship.business_partner_request.update","neon.relationship.business_partner_request.validate","neon.relationship.business_partner.read","neon.customer.credit.read","neon.business_partner_profile_match.create","neon.business_partner_profile_match.read","neon.business_partner_profile_match.request","neon.supplier_registration.invitation.create","neon.supplier_registration.invitation.read","neon.supplier_registration.invitation.cancel","neon.supplier_registration.external.respond","neon.customer_registration.invitation.create","neon.customer_registration.invitation.read","neon.customer_registration.invitation.cancel","neon.customer_registration.external.respond","neon.workforce.read","neon.workforce.request.create","neon.workforce.request.read","neon.workforce.review","neon.workforce.onboarding.execute","neon.workforce.offboarding.execute","neon.workforce.pii.read","neon.workforce.iam.retry","neon.workforce.integration.import"].includes(input.permissionCode))return{allowed:true,reason:"bounded_business_partner_operation"};
      if((Object.values(BUSINESS_PARTNER_360_PERMISSIONS) as readonly string[]).includes(input.permissionCode))return{allowed:true,reason:"bounded_business_partner_360_read"};
    ...
    const localDefinitions=new LocalBusinessPartnerDefinitionConsumer({local:container.services.publication?.projections.neon??new KyselyLocalProjectionRepository(neonDatabase),canonicalizer:{canonicalBytes,sha256}});
    const protectedValues=container.adapters.secretStore?createSecretStoreProtectedValueResolver(container.adapters.secretStore):undefined;
    const businessPartner360Definitions=createLastValidBusinessPartner360DefinitionResolver({async resolve(){const descriptors=await localDefinitions.descriptors(),view=descriptors.views["neonPartner360"] as Record<string,unknown>|undefined;if(!view||typeof view["version"]!=="string")throw new MasterDataError(503,"BP_360_DEFINITION_UNAVAILABLE","Verified local Business Partner 360 definition is unavailable");return parseBusinessPartner360Definition({version:view["version"],hash:sha256(canonicalBytes({bundleHash:descriptors.bundleHash,releaseNo:descriptors.releaseNo,view})),view});}});
    const meshNetwork=config?.businessPartner360.meshLiveBaseUrl&&config.businessPartner360.meshLiveCredentialReference&&container.adapters.secretStore?createHttpBusinessPartner360MeshNetworkAdapter({baseUrl:config.businessPartner360.meshLiveBaseUrl,credentialReference:config.businessPartner360.meshLiveCredentialReference,secrets:container.adapters.secretStore}):undefined;
    const businessPartner360=createBusinessPartner360Service<RecordTransaction>({authorizer:businessPartnerAuthorizer,repository:new KyselyBusinessPartner360Repository() as never,transactions,audit:audit as never,...(protectedValues?{protectedValues}:{}),...(meshNetwork?{meshNetwork,meshTimeoutMs:config?.businessPartner360.meshTimeoutMs}:{}),definitions:businessPartner360Definitions,businessActivityProviders:createUnavailableBusinessPartner360ActivityProviders()});
    container.platform.httpRegistrars.push(application=>registerLocalBusinessPartnerDefinitionRoutes(application,{authenticate:createIamAuthenticationMiddleware(iam),readContext:readVerifiedRequestContext,authorizer:businessPartnerAuthorizer,consumer:localDefinitions}));
    const businessPartnerRequests = createBusinessPartnerRequestService<RecordTransaction>({
      ...
      schemas: {
        async resolve(input) {
          try{return await localDefinitions.requestSchema({kind:input.kind,sourceKind:input.sourceKind,...(input.requestedRole?{requestedRole:input.requestedRole}:{})});}catch(error){throw new MasterDataError(503,"BUSINESS_PARTNER_REQUEST_SCHEMA_UNAVAILABLE",error instanceof Error?error.message:"Verified local Business Partner definition is unavailable");}
        },
      },
      validator,
      workflows:{
        async resolve(input,transaction){
          ...
          const artifact=await localDefinitions.workflow({kind:input.request.kind,...(input.request.requestedRole?{requestedRole:input.request.requestedRole}:{})});
          return{code:artifact.code,version:artifact.version,hash:artifact.hash,stageCode:artifact.stageCode,stageName:artifact.stageName,approverPrincipalIds:candidates.rows.map(row=>row.principal_id)};
        },
      },
      audit,
    ...
    const workforce=createWorkforceService<RecordTransaction>({authorizer:businessPartnerAuthorizer,repository:new KyselyWorkforceRepository(),requestRepository:new KyselyWorkforceRequestRepository(),requestValidator:{async validate(input,transaction){
      const evidence=await sql<{legal_entity:boolean;company:boolean;org_unit:boolean;position:boolean;profile:boolean;target:boolean}>`SELECT
        ...
        CASE WHEN ${input.request.kind}='onboard_person' THEN true WHEN ${input.request.kind}='add_employment' THEN EXISTS(SELECT 1 FROM master.person p WHERE p.tenant_id=${input.context.tenantId}::uuid AND p.id=${input.request.targetPersonId??null}::uuid AND p.status='active') ELSE EXISTS(SELECT 1 FROM master.employment e JOIN master.employee m ON m.tenant_id=e.tenant_id AND m.id=e.employee_id WHERE e.tenant_id=${input.context.tenantId}::uuid AND e.id=${input.request.targetEmploymentId??null}::uuid AND e.person_id=${input.request.targetPersonId??null}::uuid AND e.employee_id=${input.request.targetEmployeeId??null}::uuid AND e.status='active' AND m.person_id=e.person_id) END target`.execute(transaction);
      const row=evidence.rows[0]!,checks=[["workforce.legal_entity.effective","$.legalEntityId",row.legal_entity],["workforce.company.effective","$.companyCodeId",row.company],["workforce.org_unit.effective","$.orgUnitId",row.org_unit],["workforce.position.effective","$.positionId",row.position],["workforce.profile.ready","$.protectedProfileContentItemId",row.profile],["workforce.target.consistent","$.target",row.target]] as const;return{evaluationId:randomUUID(),evaluatedAt:new Date().toISOString(),ruleset:{code:"workforce.lifecycle.validation",version:1,hash:createHash("sha256").update("workforce.lifecycle.validation.v1").digest("hex")},valid:checks.every(([, ,ok])=>ok),findings:checks.map(([code,path,ok])=>({ruleCode:code,severity:"error" as const,fieldPath:path,outcome:ok?"passed" as const:"failed" as const,messageCode:ok?"WORKFORCE_RULE_PASSED":"WORKFORCE_RULE_FAILED",evidenceReference:{checked:true}}))};}},requestWorkflows:{async resolve(input,transaction){const candidates=await sql<{principal_id:string}>`SELECT principal_id::text FROM document.fn_workforce_request_approvers(${input.context.tenantId}::uuid,${input.request.legalEntityId}::uuid,${input.request.companyCodeId??null}::uuid,${input.context.principalId}::uuid)`.execute(transaction);return{code:"NeonWorkforceLifecycle",version:1,hash:createHash("sha256").update("neon.workforce.lifecycle.v1").digest("hex"),stageCode:"people_review",stageName:"People review",approverPrincipalIds:candidates.rows.map(row=>row.principal_id)};}},transactions,audit,outbox:createDatabaseOutboxWriter("business-partner"),...(protectedValues?{protectedValues}:{})});
    container.services.workforce=workforce;
    const businessPartnerInvitations=createBusinessPartnerInvitationService<RecordTransaction>({authorizer:businessPartnerAuthorizer,repository:new KyselyBusinessPartnerInvitationRepository(),transactions,schemas:{async resolve(input){try{return await localDefinitions.requestSchema({kind:input.kind,sourceKind:input.sourceKind,...(input.requestedRole?{requestedRole:input.requestedRole}:{})});}catch(error){throw new MasterDataError(503,"BUSINESS_PARTNER_REQUEST_SCHEMA_UNAVAILABLE",error instanceof Error?error.message:"Verified local Business Partner definition is unavailable");}}},audit,outbox:createDatabaseOutboxWriter("business-partner")});
    ...
    const invitationCount=container.adapters.openTelemetry?.metrics.counter("athyper_business_partner_invitation_http_total","Business Partner invitation HTTP operations and rejected external requests");
    const invitationDuration=container.adapters.openTelemetry?.metrics.histogram("athyper_business_partner_invitation_http_duration_ms","Business Partner invitation HTTP operation duration");
    const bp360Count=container.adapters.openTelemetry?.metrics.counter("athyper_bp360_request_total","Business Partner 360 operations by safe outcome and section");
    const bp360Duration=container.adapters.openTelemetry?.metrics.histogram("athyper_bp360_duration_ms","Business Partner 360 operation duration in milliseconds");
    const bp360Payload=container.adapters.openTelemetry?.metrics.histogram("athyper_bp360_payload_bytes","Business Partner 360 response payload bytes");
    const bp360Completeness=container.adapters.openTelemetry?.metrics.counter("athyper_bp360_completeness_total","Business Partner 360 completeness outcomes");
    const bp360Redaction=container.adapters.openTelemetry?.metrics.counter("athyper_bp360_redaction_total","Business Partner 360 redaction notices by classification");
    const bp360Reveal=container.adapters.openTelemetry?.metrics.counter("athyper_bp360_reveal_total","Business Partner 360 purpose-bound reveal outcomes");
    const bp360MeshFallback=container.adapters.openTelemetry?.metrics.counter("athyper_bp360_mesh_fallback_total","Business Partner 360 MESH local-projection fallbacks");
    const requestOldestOpen=container.adapters.openTelemetry?.metrics.gauge("athyper_business_partner_request_oldest_open_seconds","Age of the oldest non-terminal Business Partner request");
    const invitationGuard=createBusinessPartnerInvitationExternalGuard({onRejected:reason=>invitationCount?.increment({operation:"external_guard",outcome:reason,status_code:reason==="rate_limited"?"429":"413"})});
    container.platform.httpRegistrars.push(application=>registerBusinessPartnerRequestRoutes(application,{authenticate:createIamAuthenticationMiddleware(iam),readContext:readVerifiedRequestContext,service:businessPartnerRequests,aggregate360:businessPartner360,telemetry:event=>{const labels={operation:event.operation,outcome:event.outcome,status_code:String(event.statusCode)};requestCount?.increment(labels);requestDuration?.record(event.durationMs,labels);}}));
    container.platform.httpRegistrars.push(application=>registerBusinessPartner360Routes(application,{authenticate:createIamAuthenticationMiddleware(iam),readContext:readVerifiedRequestContext,service:businessPartner360,telemetry:event=>{const labels={operation:event.operation,outcome:event.outcome,status_code:String(event.statusCode),section:event.section??"none",state:event.state??"none",reason_code:event.reasonCode??"none"};bp360Count?.increment(labels);bp360Duration?.record(event.durationMs,labels);if(event.payloadBytes!==undefined)bp360Payload?.record(event.payloadBytes,{operation:event.operation,section:event.section??"none"});if(event.completenessStatus)bp360Completeness?.increment({pack:"composed",status:event.completenessStatus});for(const fieldClass of event.redactionClasses??[])bp360Redaction?.increment({field_class:fieldClass});if(event.revealClass)bp360Reveal?.increment({field_class:event.revealClass,outcome:event.outcome,reason_code:event.reasonCode??"none"});if(event.meshFallbackReason)bp360MeshFallback?.increment({reason_code:event.meshFallbackReason});}}));
    container.platform.httpRegistrars.push(application=>registerWorkforceRoutes(application,{authenticate:createIamAuthenticationMiddleware(iam),readContext:readVerifiedRequestContext,service:workforce}));
    container.platform.httpRegistrars.push(application=>registerBusinessPartnerInvitationRoutes(application,{authenticateInternal:createIamAuthenticationMiddleware(iam),authenticateExternal:createIamAuthenticationMiddleware(iam),externalGuard:invitationGuard,readContext:readVerifiedRequestContext,service:businessPartnerInvitations,telemetry:event=>{const labels={operation:event.operation,outcome:event.outcome,status_code:String(event.statusCode)};invitationCount?.increment(labels);invitationDuration?.record(event.durationMs,labels);}}));
    ...
    container.runtimes.health.register("workforce-requests.neon",async()=>{try{const row=(await sql<{request_table:string|null;validation_table:string|null;guard_function:string|null;approver_function:string|null;permission_count:number}>`SELECT to_regclass('document.workforce_request')::text request_table,to_regclass('document.workforce_request_validation')::text validation_table,to_regprocedure('document.fn_workforce_request_payload_has_restricted_key(jsonb)')::text guard_function,to_regprocedure('document.fn_workforce_request_approvers(uuid,uuid,uuid,uuid)')::text approver_function,(SELECT count(*)::int FROM authz.permission WHERE canonical_code LIKE 'neon.workforce.request.%' AND status='published') permission_count`.execute(neonDatabase)).rows[0];return row?.request_table&&row.validation_table&&row.guard_function&&row.approver_function&&row.permission_count===6?{status:"healthy"}:{status:"unhealthy",message:"People-owned workforce validation, workflow, materializer, or permission contract is incomplete"};}catch(error){return{status:"unhealthy",message:error instanceof Error?error.message:"Workforce request readiness check failed"};}});
    container.runtimes.health.register("business-partner-invitations.neon",async()=>{try{const row=(await sql<{invitation_table:string|null;recovery_table:string|null;access_revocation:boolean}>`SELECT to_regclass('document.business_partner_invitation')::text invitation_table,to_regclass('document.business_partner_invitation_recovery')::text recovery_table,EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='document' AND table_name='business_partner_invitation' AND column_name='applicant_access_revoked_at') access_revocation`.execute(neonDatabase)).rows[0];return row?.invitation_table&&row.recovery_table&&row.access_revocation?{status:"healthy"}:{status:"unhealthy",message:"Generalized invitation aggregate, recovery authority, or applicant-access revocation is incomplete"};}catch(error){return{status:"unhealthy",message:error instanceof Error?error.message:"Business Partner invitation readiness check failed"};}});
    if(container.runtimes.jobs){container.runtimes.jobs.register(BUSINESS_PARTNER_INVITATION_MAINTENANCE_QUEUE,EXPIRE_BUSINESS_PARTNER_INVITATIONS_JOB,createBusinessPartnerInvitationExpiryHandler(businessPartnerInvitations));container.runtimes.jobDefinitions.push({code:EXPIRE_BUSINESS_PARTNER_INVITATIONS_JOB,owner:"@athyper/server-service-master-data",queue:BUSINESS_PARTNER_INVITATION_MAINTENANCE_QUEUE,name:EXPIRE_BUSINESS_PARTNER_INVITATIONS_JOB,scope:"tenant",payloadSchema:{name:EXPIRE_BUSINESS_PARTNER_INVITATIONS_JOB,version:1},timeoutMs:60_000,maxAttempts:5,executionRetentionDays:30});}
    if(container.runtimes.jobs){container.runtimes.jobs.register(SUPPLIER_READINESS_MAINTENANCE_QUEUE,EXPIRE_SUPPLIER_QUALIFICATIONS_JOB,createSupplierQualificationExpiryHandler(businessPartnerEligibility));container.runtimes.jobs.register(SUPPLIER_READINESS_MAINTENANCE_QUEUE,REEVALUATE_SUPPLIER_ACTIVATIONS_JOB,createSupplierActivationReevaluationHandler(businessPartnerEligibility));for(const code of [EXPIRE_SUPPLIER_QUALIFICATIONS_JOB,REEVALUATE_SUPPLIER_ACTIVATIONS_JOB])container.runtimes.jobDefinitions.push({code,owner:"@athyper/server-service-master-data",queue:SUPPLIER_READINESS_MAINTENANCE_QUEUE,name:code,scope:"tenant",payloadSchema:{name:code,version:1},timeoutMs:120_000,maxAttempts:5,executionRetentionDays:90});}
    container.runtimes.health.register("business-partner-eligibility.neon",async()=>{try{const result=await sql<{qualification_table:string|null;preference_table:string|null;block_table:string|null;risk_table:string|null;activation_evidence_table:string|null;row_version:boolean;idempotency_key:boolean;permission_count:number}>`SELECT to_regclass('control.business_partner_qualification')::text AS qualification_table,to_regclass('control.supplier_preference_designation')::text AS preference_table,to_regclass('control.business_partner_block')::text AS block_table,to_regclass('master.party_risk_assessment')::text AS risk_table,to_regclass('document.supplier_activation_evidence')::text AS activation_evidence_table,EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='control' AND table_name='business_partner_qualification' AND column_name='row_version' AND is_nullable='NO') AS row_version,EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='control' AND table_name='business_partner_qualification' AND column_name='idempotency_key' AND is_nullable='NO') AS idempotency_key,(SELECT count(*)::int FROM authz.permission WHERE canonical_code IN ('neon.supplier.qualification.admin','neon.supplier.preference.admin','neon.relationship.business_partner.read','neon.relationship.business_partner.activate')) AS permission_count`.execute(neonDatabase);const row=result.rows[0];return row?.qualification_table&&row.preference_table&&row.block_table&&row.risk_table&&row.activation_evidence_table&&row.row_version&&row.idempotency_key&&row.permission_count===4?{status:"healthy"}:{status:"unhealthy",message:"Business Partner qualification, preference, or activation contract is incomplete"};}catch(error){return{status:"unhealthy",message:error instanceof Error?error.message:"Business Partner eligibility readiness check failed"};}});
    ...
    container.runtimes.health.register("business-partner-request-age.neon",async()=>{try{const row=(await sql<{oldest_open_seconds:number}>`SELECT COALESCE(extract(epoch FROM clock_timestamp()-min(created_at)),0)::int AS oldest_open_seconds FROM document.business_partner_request WHERE status IN ('draft','validating','validation_failed','pending_approval','returned','approved','applying','failed')`.execute(neonDatabase)).rows[0];requestOldestOpen?.set(Number(row?.oldest_open_seconds??0));return{status:"healthy"};}catch(error){return{status:"unhealthy",message:error instanceof Error?error.message:"Business Partner request age check failed"};}});
  }
  ...
  if (dependencies.repository || Object.keys(recordDatabases).length > 0) {
    ...
    const lists = createEntityListService({ metadata: listMetadata, authorizer, listExecutor, queries, ...(collectionScopes ? { collectionScopes } : {}) });
...
export function registerAtlas(
  ...
  const experience = new AtlasExperienceConfigurationService(new KyselyAtlasExperienceConfigurationRepository(transactions));
  container.platform.httpRegistrars.push((application)=>registerAtlasExperienceRoutes(application as never,{authenticate:createIamAuthenticationMiddleware(iam),readContext:readVerifiedRequestContext,authorizeAdmin:async context=>context.permissions.allowed.includes("studio.platform.catalog.manage")||context.permissions.allowed.includes("atlas.admin.manage")||context.permissions.allowed.includes("ai.admin"),experience}));
  ...
  if (!routesEnabled) {
    if (container.platform.experience) container.platform.httpRegistrars.push((application) => registerAtlasSurfaceDraftRoutes(application as never, { authenticate: createIamAuthenticationMiddleware(iam), readContext: readVerifiedRequestContext }));
    container.platform.ai = { ledger, routesEnabled: false, toolsEnabled: false };
    return;
  }
  ...
  const runtime = new AtlasAgentRuntime({ admission: dependencies.admission, modelPolicy: dependencies.modelPolicy, bindings, providers, credentials: operations.resolver, threads, runs: dependencies.runs, ledger: dependencies.usageLedger, prompts: dependencies.prompts, quota:quotas, experience, attachments:new KyselyAtlasAttachmentContextResolver(transactions), ...(coordinator ? { tools: coordinator } : {}), maxInputCharacters: 55_000, maxToolRounds: toolsEnabled ? 3 : 0 });
  ...
  if (container.platform.experience) {
    const surfaceDrafts = new AtlasSurfaceDraftGenerator({ runtime, threads, admission: dependencies.admission, surfaces: container.platform.experience.service });
    container.platform.httpRegistrars.push((application) => registerAtlasSurfaceDraftRoutes(application as never, { authenticate: createIamAuthenticationMiddleware(iam), readContext: readVerifiedRequestContext, generator: surfaceDrafts }));
  }
  ...
  return {
    run(planeKey, actor, work) {
      if (planeKey === "neon" && container.adapters.neonDatabase) {
        return container.adapters.neonDatabase.withSystemTransaction(async (transaction) => {
          await stampTransactionActor(transaction as unknown as Transaction<Record<string, never>>, actor);
          return work(transaction as unknown as RecordTransaction);
        });
      }
      if (planeKey === "mesh" && container.adapters.meshDatabase) {
        return container.adapters.meshDatabase.withSystemTransaction(async (transaction) => {
          await stampTransactionActor(transaction as unknown as Transaction<Record<string, never>>, actor);
          return work(transaction as unknown as RecordTransaction);
        });
      }
      if (planeKey === "studio" && container.adapters.athyperDatabase) {
        return container.adapters.athyperDatabase.withSystemTransaction(async (transaction) => {
          await stampTransactionActor(transaction as unknown as Transaction<Record<string, never>>, actor);
          return work(transaction as unknown as RecordTransaction);
        });
      }
