...
export function registerPlatform(
  ...
  const provisioning =
    ...
    (studioDatabase
      ? createProvisioningVertical({
          ...
          transactions: {
            run: (_actor, work) =>
              studioDatabase.withTenantTransaction((transaction) =>
                work(
  ...
  if (studioDatabase && provisioning) {
    ...
    const options: TrustIamAuthorityOptions<
      Transaction<Record<string, never>>
    > = {
      repository: new KyselyTrustIamAuthorityRepository(),
      transactions,
      authorizer,
      audit,
      outbox: createIamOutboxWriter(),
    };
    container.platform.trustIam = {
      organizations: new OrganizationService(options),
      projections: new OrganizationProjectionService(options),
      scopes: new ProjectionScopeService(options),
      identityProvisioning: new IdentityProvisioningService<
        Transaction<Record<string, never>>
      >({
        create: provisioning,
        repository: new KyselyIdentityProvisioningAttemptRepository(),
        transactions,
        authorizer,
        audit,
        outbox: createIamOutboxWriter(),
    ...
    container.runtimes.health.register(
      "trustiam.studio-authority",
      async () => {
        try {
          await sql`SELECT 1 FROM trustiam.organization LIMIT 1`.execute(
            studioDatabase.database,
          );
          return { status: "healthy" };
        } catch {
          return {
            status: "unhealthy",
            message: "TrustIAM Studio authority database is unavailable",
          };
        }
      },
    );
  ...
  if (studioDatabase)
    container.platform.httpRegistrars.push((application) =>
      registerProjectionReconciliationRoutes(
        application,
        studioDatabase,
        iam,
        authorizer,
  ...
  container.platform.httpRegistrars.push((application) =>
    registerTrustedDeviceRoutes(application, container, iam),
  );
  container.platform.httpRegistrars.push((application) =>
...
function registerIdentitySagaWorker(
  ...
  const studio = container.adapters.jobAthyperDatabase,
    targets = {
    ...
    providerConfig = config.identityProvider;
  if (
    !studio ||
    !targets.studio ||
    !targets.neon ||
    ...
    !providerConfig.keycloakAdminBaseUrl ||
    !providerConfig.keycloakAdminClientId ||
    !providerConfig.keycloakAdminCredentialReference
  )
    return;
  const runStudio = <T>(
    work: (tx: Transaction<Record<string, never>>) => Promise<T>,
  ) =>
    studio.withSystemTransaction((transaction) => work(transaction as never));
  const repository = new KyselyIdentitySagaRepository(runStudio);
  const provider = new KeycloakIdentityProviderAdapter({
    baseUrl: providerConfig.keycloakAdminBaseUrl,
    adminRealm: providerConfig.keycloakAdminRealm,
    clientId: providerConfig.keycloakAdminClientId,
    credentialReference: providerConfig.keycloakAdminCredentialReference,
  ...
  const local = new KyselyPlaneLocalIdentityAuthority(
    ...
    {
      run(plane, work) {
        return targets[plane]!.withSystemTransaction((transaction) =>
  ...
  const metric = container.adapters.openTelemetry?.metrics.counter(
    "trustiam_identity_saga_total",
  );
  const worker = new IdentitySagaWorker({
    workerId: `identity-saga-${process.env["HOSTNAME"] ?? "worker"}`,
    repository,
    provider,
    local,
    organizations: new KyselyDesiredOrganizationReader(runStudio),
    alerts: {
      metric(labels) {
        metric?.increment(labels);
      },
      async deadLetter(event) {
        captureOperationalError(
          Object.assign(new Error(event.errorCode), { code: event.errorCode }),
          {
            component: "trustiam-identity-saga",
            "identity.id": event.identityId,
            "attempt.id": event.attemptId,
            "authority.tenant_id": event.authorityTenantId,
          },
        );
      },
    },
  ...
  if (container.runtimes.jobs)
    container.runtimes.jobs.register(
      IDENTITY_SAGA_QUEUE,
      RUN_IDENTITY_SAGA_JOB,
      {
        async handle() {
          const outcomes: Record<string, number> = {};
          for (let index = 0; index < 100; index++) {
            const outcome = await worker.runOne(randomUUID());
            outcomes[outcome] = (outcomes[outcome] ?? 0) + 1;
            if (outcome === "idle") break;
          }
          return { status: "completed", output: outcomes };
        },
      },
    );
  container.runtimes.jobDefinitions.push({
    code: RUN_IDENTITY_SAGA_JOB,
    owner: "@athyper/server-platform-iam",
    queue: IDENTITY_SAGA_QUEUE,
    name: RUN_IDENTITY_SAGA_JOB,
    scope: "plane",
    payloadSchema: { name: RUN_IDENTITY_SAGA_JOB, version: 1 },
    timeoutMs: 120_000,
    maxAttempts: 1,
  ...
  if (container.runtimes.scheduler)
    container.runtimes.scheduledJobs.push({
      scheduleId: "trustiam-identity-saga",
      queue: IDENTITY_SAGA_QUEUE,
      name: RUN_IDENTITY_SAGA_JOB,
      data: {},
      pattern: { kind: "interval", everyMs: 15_000 },
      options: {
        jobId: "trustiam:identity:saga",
        maxAttempts: 1,
        payloadSchema: { name: RUN_IDENTITY_SAGA_JOB, version: 1 },
        execution: {
          planeKey: "studio",
          scope: "plane",
          principalId: "trustiam-identity-saga",
        },
      },
    });
  container.runtimes.health.register("trustiam.identity-saga", async () => {
    try {
      const row = await runStudio(
        async (transaction) =>
          (
            await sql<{
              pending: number;
              dead_letters: number;
            }>`SELECT count(*) FILTER(WHERE reconciliation_status IN('pending','drifted','failed'))::int pending,(SELECT count(*)::int FROM trustiam.identity_saga_attempt WHERE status='dead_letter' AND replay_requested_at IS NULL) dead_letters FROM trustiam.identity_projection`.execute(
              transaction,
            )
          ).rows[0],
      );
      return row && row.dead_letters === 0
      ...
      return {
        status: "unhealthy",
        message:
          error instanceof Error ? error.message : "Identity saga unavailable",
      };
    }
  });
}

function registerIdentitySagaRoutes(
  ...
  registerContractRoute(
    ...
    defineRouteContract({
      method: "get",
      path: "/api/iam/identity-saga-attempts/:attemptId/evidence",
      operationId: "iam.readIdentitySagaEvidence",
      summary: "Read bounded identity saga audit evidence",
      tags: ["IAM"],
      authenticated: true,
        ...
        if (
          ...
          !(
            await authorizer.authorize({
              ...
              resource: { tenantId: context.tenantId, attemptId },
            })
          ).allowed
        ) {
          response
            ...
            .json(
              problem(
                403,
                "AUTH_IDENTITY_SAGA_EVIDENCE_FORBIDDEN",
                "Identity saga evidence requires Studio projection-read authority",
              ),
            );
          return;
        }
        const row = await studio.withTenantTransaction(
          async (transaction) =>
            (
              await sql<Row>`SELECT id::text,identity_projection_id::text "identityId",desired_version "desiredVersion",desired_hash "desiredHash",attempt_no "attemptNo",fencing_token "fencingToken",status,failure_class "failureClass",error_code "errorCode",started_at::text "startedAt",terminal_at::text "terminalAt",receipt FROM trustiam.identity_saga_attempt WHERE authority_tenant_id=${context.tenantId}::uuid AND id=${attemptId}::uuid`.execute(
                transaction,
              )
            ).rows[0],
        );
        if (!row) {
          response
            ...
            .json(
              problem(
                404,
                "AUTH_IDENTITY_SAGA_EVIDENCE_NOT_FOUND",
                "Identity saga evidence was not found",
              ),
            );
          return;
        }
...
function registerProjectionReconciliationWorker(container: Container): void {
  ...
  if (
    !studio ||
    !targetAdapters.studio ||
    !targetAdapters.neon ||
    !targetAdapters.mesh
  )
    return;
  const repository = new KyselyProjectionReconciliationRepository(
    studio.database as never,
  );
  const targets = new KyselyExactPlaneProjectionApplier(
    {
      run(plane, work) {
        const adapter = targetAdapters[plane]!;
        return adapter.withSystemTransaction((transaction) =>
          work(transaction as never),
        );
      },
  ...
  const worker = new ProjectionReconciliationWorker({
    ...
    alerts: {
      ...
      capture(error, context) {
        captureOperationalError(error, {
          component: "trustiam-projection-reconciliation",
          "target.plane": context.targetPlane,
          "projection.id": context.projectionId,
  ...
  if (container.runtimes.jobs)
    container.runtimes.jobs.register(
      PROJECTION_RECONCILIATION_QUEUE,
  ...
  container.runtimes.jobDefinitions.push({
    code: RECONCILE_PROJECTIONS_JOB,
    owner: "@athyper/server-platform-iam",
    queue: PROJECTION_RECONCILIATION_QUEUE,
    name: RECONCILE_PROJECTIONS_JOB,
    scope: "plane",
    payloadSchema: { name: RECONCILE_PROJECTIONS_JOB, version: 1 },
    timeoutMs: 120_000,
    maxAttempts: 1,
    executionRetentionDays: 90,
  });
  if (container.runtimes.scheduler)
    container.runtimes.scheduledJobs.push({
      scheduleId: "trustiam-projection-reconciliation",
      queue: PROJECTION_RECONCILIATION_QUEUE,
      name: RECONCILE_PROJECTIONS_JOB,
      data: {},
      pattern: { kind: "interval", everyMs: 60_000 },
      options: {
        jobId: "trustiam:projection:reconcile",
        maxAttempts: 1,
        payloadSchema: { name: RECONCILE_PROJECTIONS_JOB, version: 1 },
        execution: {
          planeKey: "studio",
          scope: "plane",
          principalId: "trustiam-projection-reconciler",
        },
      },
    });
}

function registerProjectionReconciliationRoutes(
  application: Parameters<Container["platform"]["httpRegistrars"][number]>[0],
  studio: NonNullable<Container["adapters"]["athyperDatabase"]>,
  iam: ReturnType<typeof createIamService>,
  authorizer: ReturnType<typeof createPermissionAuthorizer>,
  ...
  registerContractRoute(
    ...
    defineRouteContract({
      method: "get",
      path: "/api/iam/projection-health",
      operationId: "iam.getProjectionHealth",
      summary: "Read Studio-authoritative projection reconciliation health",
      tags: ["IAM"],
      authenticated: true,
      permission: "studio.iam.application_projection.read",
      responses: {
        ...
        if (
          ...
          response
            ...
            .json(
              problem(
                403,
                "AUTH_PROJECTION_HEALTH_FORBIDDEN",
                "Projection health requires Studio projection-read authority",
              ),
            );
          return;
        }
        const result = await studio.withTenantTransaction(
          async (transaction) =>
            (
              await sql<Row>`SELECT count(*) FILTER(WHERE reconciliation_status='in_sync')::int "inSync",count(*) FILTER(WHERE reconciliation_status='pending')::int pending,count(*) FILTER(WHERE reconciliation_status='drifted')::int drifted,count(*) FILTER(WHERE reconciliation_status='failed')::int failed,min(coalesce(updated_at,created_at)) FILTER(WHERE reconciliation_status<>'in_sync') "oldestUnreconciledAt",(SELECT count(*)::int FROM trustiam.projection_reconciliation_attempt WHERE authority_tenant_id=${context.tenantId}::uuid AND status='dead_letter' AND replay_requested_at IS NULL) "deadLetters" FROM trustiam.application_projection WHERE authority_tenant_id=${context.tenantId}::uuid`.execute(
                transaction,
              )
            ).rows[0],
        );
  ...
  registerContractRoute(
    ...
    defineRouteContract({
      method: "post",
      path: "/api/iam/projection-reconciliation-attempts/:attemptId/replay",
      operationId: "iam.replayProjectionReconciliation",
      summary:
        "Request a reviewed replay of one dead-lettered projection attempt",
      tags: ["IAM"],
      authenticated: true,
    ...
    async (request, response, next) => {
      try {
        ...
        if (
          ...
          response
            ...
            .json(
              problem(
                403,
                "AUTH_PROJECTION_REPLAY_FORBIDDEN",
                "Projection replay requires Studio projection-replay authority",
              ),
            );
          return;
        }
        const changed = await studio.withTenantTransaction((transaction) =>
          sql`UPDATE trustiam.projection_reconciliation_attempt SET replay_requested_at=clock_timestamp(),replay_requested_by=${context.principalId}::uuid,updated_by=${context.principalId}::uuid WHERE authority_tenant_id=${context.tenantId}::uuid AND id=${request.params.attemptId}::uuid AND status='dead_letter' AND replay_requested_at IS NULL`.execute(
            transaction,
          ),
        );
        if (Number(changed.numAffectedRows ?? 0) !== 1) {
          response
            ...
            .json(
              problem(
                404,
                "AUTH_PROJECTION_DEAD_LETTER_NOT_FOUND",
                "Replayable projection dead letter was not found",
              ),
            );
          return;
        }
        response
          .status(202)
          .json({ accepted: true, attemptId: request.params.attemptId });
      } catch (error) {
        next(error);
      }
    },
...
function registerIdentityContextDiscovery(
  ...
  registerContractRoute(
    ...
    defineRouteContract({
      ...
      responses: {
        200: {
          description: "Sanitized active contexts",
          body: { type: "object" },
        },
        401: { description: "Authentication required" },
        403: { description: "Plane access rejected" },
        503: { description: "Exact-plane identity directory unavailable" },
      },
    }),
    async (request, response) => {
      ...
      try {
        ...
        const plane =
          rawPlane === "neon" || rawPlane === "mesh" || rawPlane === "studio"
            ? rawPlane
            : undefined;
        if (!authorization || !plane) {
          response
            .status(401)
            .json(
              problem(
        ...
        try {
          token = await verifier.verify(authorization);
        } catch {
          response
            .status(401)
            .json(
        ...
        if (!acceptsPlane(token.claims, plane)) {
          response
            .status(403)
            .json(
              problem(
                403,
                "AUTH_ACCESS_DENIED",
                "Account is not authorized for this plane",
              ),
            );
          return;
        }
        const realmKey =
          claim(token.claims, "realm_key") ??
          issuerRealm(token.issuer) ??
          defaultRealmKey;
        const requestedRealm = request.header("x-realm");
        if (
          realmKey !== defaultRealmKey ||
          (requestedRealm && requestedRealm !== realmKey)
        ) {
          response
            .status(403)
            .json(
              problem(
                ...
                "Identity realm does not match this deployment",
              ),
            );
          return;
        }
        ...
        if (!run) {
          response
            ...
            .json(
              problem(
                503,
                "AUTH_CONTEXT_DIRECTORY_UNAVAILABLE",
                "The exact-plane identity directory is unavailable",
              ),
            );
          return;
        }
        ...
        `.execute(transaction);
          ...
          for (const projection of projections.rows) {
            ...
            await sql`SELECT set_config('app.current_tenant_id',${projection.tenantId},true)`.execute(
              transaction,
            );
            const principal = (
              await sql<{ principalId: string; authEpoch: number }>`
          ...
          `.execute(transaction)
            ...
            await sql`SELECT set_config('app.current_principal_id',${principal.principalId},true)`.execute(
              transaction,
            );
            const membership = (
              await sql<{
          ...
          for (const row of memberships) {
            ...
            await sql`SELECT set_config('app.current_tenant_id',${row.tenantId},true),set_config('app.current_principal_id',${row.principalId},true)`.execute(
              transaction,
            );
            presented.push({
              ...row,
              ...(await contextPresentation(transaction, plane, row.tenantId)),
            });
          }
          return presented;
        });
...
function contextTransaction(
  ...
  if (plane === "neon" && container.adapters.neonDatabase)
    return (work) =>
      container.adapters.neonDatabase!.withSystemTransaction((transaction) =>
        work(transaction as unknown as Transaction<Record<string, never>>),
      );
  if (plane === "mesh" && container.adapters.meshDatabase)
    return (work) =>
      container.adapters.meshDatabase!.withSystemTransaction((transaction) =>
        work(transaction as unknown as Transaction<Record<string, never>>),
      );
  if (plane === "studio" && container.adapters.athyperDatabase)
    return (work) =>
      container.adapters.athyperDatabase!.withSystemTransaction((transaction) =>
        work(transaction as unknown as Transaction<Record<string, never>>),
      );
  return undefined;
}

async function contextPresentation(
  transaction: Transaction<Record<string, never>>,
  plane: "neon" | "mesh" | "studio",
  tenantId: string,
): Promise<{ description: string; badges: readonly string[] }> {
  if (plane === "neon") {
    const row = (
      await sql<{
        legalEntities: number;
        operatingOrganizations: number;
      }>`SELECT (SELECT count(*)::int FROM master.legal_entity WHERE tenant_id=${tenantId}::uuid AND status='active') AS "legalEntities", (SELECT count(*)::int FROM master.operating_organization WHERE tenant_id=${tenantId}::uuid AND status='active') AS "operatingOrganizations"`.execute(
        transaction,
      )
    ).rows[0];
    return {
      description: "Business operations, finance and governed execution",
      badges: [
        `${row?.legalEntities ?? 0} legal entities`,
        `${row?.operatingOrganizations ?? 0} operating organizations`,
      ],
    };
  }
  if (plane === "mesh") {
    const rows = (
      await sql<{
        role: string;
        count: number;
      }>`SELECT network_role::text AS role,count(*)::int AS count FROM mesh.network_account WHERE tenant_id=${tenantId}::uuid AND status='active' GROUP BY network_role ORDER BY network_role`.execute(
        transaction,
      )
    ).rows;
    return {
      description: "Verified buyer and supplier network participation",
      badges: rows.map(
        (row) =>
          `${row.count} ${row.role} account${row.count === 1 ? "" : "s"}`,
      ),
    };
  }
  return {
    description: "Platform administration and governed tenant operations",
    badges: ["Administrative authority", "Audited access"],
  };
}

function acceptsPlane(
  ...
  const claimPlane =
    claim(claims, "plane") ?? authorizedParty?.replace(/-web$/, "");
  return (
    claimPlane === plane &&
    authorizedParty === planeClient &&
...
> {
  return {
    async append(event, transaction) {
      if (!transaction)
        throw new Error("IAM outbox writes require the active transaction");
      await sql`
