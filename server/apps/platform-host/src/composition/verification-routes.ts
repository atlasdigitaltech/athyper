...
async function adapterHealth(container: Container): Promise<VerificationCheckResult[]> {
  const candidates: Array<readonly [string, string, CheckCategory, (() => Promise<{ readonly ok: boolean; readonly detail: string }>) | undefined]> = [
    ...
    ["mail.smtp", "SMTP delivery transport", "mail", container.adapters.notificationChannels.get("email") ? async () => { const result = await container.adapters.notificationChannels.get("email")!.health(); return { ok: result.status !== "unhealthy", detail: "SMTP transport verification succeeded" }; } : undefined],
  ];
