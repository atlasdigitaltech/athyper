...
import {
  createBusinessPartnerGovernedImport,
  MasterDataError,
} from "@athyper/server-service-master-data";

/** Owning-service facts only. IAM still checks grants, denials, scope and MFA.
 * The import owner creates governed drafts; it cannot approve or apply them. */
export function businessPartnerImportPolicy(input: AuthorizationRequest) {
  const allowed =
    input.permissionCode === "neon.relationship.bp_target.import" &&
    input.context.planeKey === "neon" &&
    input.resource?.["entityCode"] === "business_partner" &&
    input.resource?.["operationKey"] === "import" &&
    input.resource?.["proposalOnly"] === true &&
    input.resource?.["makerCheckerEnforced"] === true;
  return {
    allowed,
    sodSatisfied: allowed,
    reason: "governed_import_proposal_required",
  };
}
...
export function createBusinessPartnerImportRuntime(options: {
  ...
  return createBusinessPartnerGovernedImport({
    ...
    async authorizeGateway(context) {
      ...
      if (!decision.allowed)
        throw new MasterDataError(
          403,
          "BP_GOVERNED_IMPORT_FORBIDDEN",
          "Governed import is not authorized",
        );
    },
    async resolveRow(context, row) {
      ...
      const scope = await options.scopes.resolve({
        context,
        entityCode: "entity_case",
        operationKey: "create",
        phase: "execute",
        target: "proposed",
        resolver: row.companyCodeId
      ...
      if (
        ...
        Object.entries(coordinates).some(
          ([k, v]) => Reflect.get(scope.coordinates, k) !== v,
        )
      )
        throw new MasterDataError(
          403,
          "BP_GOVERNED_IMPORT_SCOPE_INVALID",
          "Import organization/company is unavailable",
        );
      ...
      if (!decision.allowed)
        throw new MasterDataError(
          403,
          "BP_GOVERNED_IMPORT_ROW_FORBIDDEN",
          "Import row creation is not authorized",
        );
    },
  });
}
