...
export function registerAdapters(
  ...
  if(config.search.baseUrl&&config.search.apiKey){const searchIndex=dependencies.createSearchIndex({baseUrl:config.search.baseUrl,apiKey:config.search.apiKey,indexUid:config.search.indexUid,timeoutMs:config.search.timeoutMs});container.adapters.searchIndex=searchIndex;lifecycle.onReady(async()=>{const health=await searchIndex.health();if(health.status==="unhealthy")throw new Error(health.message??"Configured search index is unhealthy");await searchIndex.initialize();});lifecycle.onShutdown(()=>searchIndex.close());}
