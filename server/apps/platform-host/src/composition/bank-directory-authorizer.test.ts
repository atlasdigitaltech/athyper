...
const permissionCode = "studio.bank_directory.publish";
...
describe("bank directory reviewer policy", () => {
  it("uses the tenant-scoped immutable author for independent approval", async () => {
    const get = vi.fn(async () => ({ createdBy: "author" }));
    const decision = await createBankDirectoryAuthorizer({ get }).authorize({
      context,
      permissionCode,
      resource: { revisionId: "revision", createdBy: "reviewer" },
    });
  ...
  it("rejects self approval despite caller-provided author evidence", async () => {
    ...
    expect(
      await createBankDirectoryAuthorizer({ get }).authorize({
        context,
        permissionCode,
        resource: { revisionId: "revision", createdBy: "other" },
      }),
    ).toMatchObject({
      allowed: false,
  ...
  it("rejects missing or tenant-invisible revisions", async () => {
    expect(
      await createBankDirectoryAuthorizer({ get: async () => null }).authorize({
        context,
        permissionCode,
  ...
  it("does not replace the permission grant or entitlement", async () => {
    const authorizer = createBankDirectoryAuthorizer({
      get: async () => ({ createdBy: "author" }),
    });
    expect(
    ...
    expect(
      await authorizer.authorize({
        context: {
          ...context,
          permissions: { ...context.permissions, planLocked: [permissionCode] },
        },
        permissionCode,
        resource: { revisionId: "revision" },
      }),
    ).toMatchObject({ allowed: false, reason: "plan_locked" });
  });
});
