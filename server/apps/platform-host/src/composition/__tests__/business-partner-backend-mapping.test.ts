  ...
  const selectedProfile = {
    ...
    operations: profile.operations.map((o) =>
      ["identity_read", "contacts_read", "bank_read"].includes(o.key)
        ? { ...o, permissionCode: `neon.relationship.bp_target.${o.key}` }
        : o,
    ),
  };
...
it("normalizes owning case operation names without borrowing a BP record ID", () => {
  expect(
    mapping.target(
      request("neon.relationship.entity_case.create", {
        entityCode: "entity_case",
        operationKey: "create",
  ...
  expect(
    mapping.target(
      request("neon.relationship.entity_case.materialize", {
        ...
        recordId: "case",
        businessPartnerId: "parent",
        operatingOrganizationId: "org",
      }),
    ),
  ).toMatchObject({ operationKey: "case_materialize", recordId: "case" });
  expect(
    mapping.target(
      request("neon.relationship.entity_case.materialize", {
        entityCode: "business_partner",
...
it("maps provider selections without incomplete legacy operation coordinates", () => {
  const resource = {
    businessPartnerId: "bp",
    providerOperationKey: "contacts_read",
    providerFieldPath: "items.*.displayName",
  };
  expect(
    mapping.target(
      request("neon.relationship.business_partner_contact.read", resource),
    ),
  ).toMatchObject({ operationKey: "contacts_read", recordId: "bp" });
  expect(
    mapping.target(
      request("neon.relationship.business_partner_contact.read", {
        ...resource,
        operationKey: "read",
      }),
    ),
  ...
  expect(
    mapping.target(
      request("neon.relationship.business_partner_contact.read", {
        ...resource,
        providerOperationKey: "bank_read",
      }),
    ),
  ).toBeNull();
});
