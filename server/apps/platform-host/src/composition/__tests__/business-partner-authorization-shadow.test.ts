...
import {
  ...
  type BusinessPartnerShadowEvent,
...
const bytes = readFileSync(path),
  profile = parseEntityAuthorizationProfile(JSON.parse(bytes.toString()));
      ...
      evidence: [
        {
          ...
          scopeKind: kind,
          scopeTargetId: "scope",
          targetId: kind === "tenant" ? "tenant" : "organization-secret",
...
function setup(
  authority: Authorizer = createPermissionAuthorizer(),
  ...
  const shadow = createBusinessPartnerAuthorizationShadow({
    config: { mode: "shadow", profile, profileHash },
    scopes: scopes ?? {
      resolve: async (input) => ({
        state: "resolved",
...
describe("BP runtime shadow", () => {
  it("maps predecessor observations without translating grants into dedicated permissions", async () => {
    ...
    const shadow = createBusinessPartnerAuthorizationShadow({
      config: {
        ...
        observationProfileHash: profileHash,
      },
      scopes: { resolve: async () => ({ state: "resolved", coordinates: {} }) },
    ...
    expect(
      (
        await shadow.wrap(createPermissionAuthorizer()).authorize({
          context: c,
          permissionCode: permission,
          resource: { businessPartnerId: recordId },
        })
      ).allowed,
    ).toBe(true);
    ...
    expect(events).toContainEqual(
      expect.objectContaining({
        ...
        operationKey: "read",
        legacy: "allowed",
        candidateTarget: "denied",
        candidateTrace: ["read:permission:denied"],
      }),
    );
  ...
  it("records explicit target deferrals without executing the predecessor operation", async () => {
    ...
    Object.assign(target, {
      operations: target.operations.filter((o) => o.key !== "update"),
      deferredOperations: ["update"],
    });
    const events: BusinessPartnerShadowEvent[] = [];
    ...
    await shadow
      .wrap({ authorize: async () => ({ allowed: true }) } as Authorizer)
      .authorize({
        context: context(),
        permissionCode: operation.permissionCode,
        resource: { businessPartnerId: recordId, operationKey: "update" },
      });
    expect(events).toContainEqual(
      expect.objectContaining({
        kind: "decision",
        operationKey: "update",
        candidateTarget: "unavailable",
        candidateTrace: ["update:deferred:unavailable"],
      }),
    );
  ...
  it("requires an exact NEON profile pin and rejects enforce", () => {
    ...
    expect(() =>
      readBusinessPartnerShadowConfig({ BP_AUTHORIZATION_MODE: "enforce" }),
    ).toThrow();
    expect(() =>
      readBusinessPartnerShadowConfig({
        BP_AUTHORIZATION_MODE: "shadow",
        BP_AUTHORIZATION_PROFILE_PATH: path.pathname,
        BP_AUTHORIZATION_PROFILE_SHA256: "0".repeat(64),
      }),
    ).toThrow("mismatch");
    expect(
      readBusinessPartnerShadowConfig({
        BP_AUTHORIZATION_MODE: "shadow",
        BP_AUTHORIZATION_PROFILE_PATH: path.pathname,
        BP_AUTHORIZATION_PROFILE_SHA256: profileHash,
      }),
    ).toMatchObject({ mode: "shadow", profileHash });
  });
  it("projects bindings only, records installed incompatibility, and never changes the verified snapshot", async () => {
    const c = context(),
      before = structuredClone(c),
      { events, wrapped } = setup();
    const result = await wrapped.authorize({
      context: c,
      permissionCode: permission,
      resource: { tenantId: c.tenantId, businessPartnerId: recordId },
    });
    ...
    expect(events).toContainEqual(
      expect.objectContaining({
        kind: "decision",
        legacy: "allowed",
        installedTarget: "denied",
        candidateTarget: "allowed",
        installedTrace: ["read:binding:denied"],
        candidateTrace: ["read:complete:allowed"],
        authority: "legacy",
        grantsChanged: false,
      }),
    );
    ...
    expect(
      (
        await wrapped.authorize({
          context: context("operating_organization"),
          permissionCode: permission,
        })
      ).allowed,
    ).toBe(true);
  ...
  it("never unions a target allow into a legacy deny", async () => {
    ...
    const authority: Authorizer = {
      authorize: async (request) =>
        request.resource?.["operationKey"]
          ? pure.authorize(request)
          : { allowed: false, reason: "legacy_policy_denied" },
    };
    ...
    ).toEqual({ allowed: false, reason: "legacy_policy_denied" });
    expect(events).toContainEqual(
      expect.objectContaining({ legacy: "denied", candidateTarget: "allowed" }),
    );
  });
  it("retains explicit denials in candidate snapshots", async () => {
    const { wrapped, events } = setup();
    await wrapped.authorize({
      context: context("tenant", true),
      permissionCode: permission,
    });
    expect(events).toContainEqual(
      expect.objectContaining({ legacy: "denied", candidateTarget: "denied" }),
    );
  });
  it("compares commands only as discovery and never invokes preflight", async () => {
    const { wrapped, events, preflight } = setup({
    ...
    expect(events).toContainEqual(
      expect.objectContaining({
        operationKey: "case_submit",
        candidateTarget: "preflight_required",
        evidence: "command_discovery_preview",
  ...
  it("retains authority during resolver and telemetry failures", async () => {
    ...
    expect(
      (
        await shadow
          .wrap(authority)
          .authorize({ context: context(), permissionCode: permission })
      ).allowed,
    ).toBe(true);
  });
  it("skips unrelated entities and reports unknown BP operations without guessing", async () => {
    const authority = {
      ...
      { wrapped, events } = setup(authority);
    await wrapped.authorize({
      context: context(),
      permissionCode: "finance.invoice.read",
    });
    expect(events).toHaveLength(0);
    expect(authority.authorize).toHaveBeenCalledTimes(1);
    await wrapped.authorize({
      context: context(),
      permissionCode: "neon.relationship.business_partner.unknown_operation",
      resource: {
        entityCode: "business_partner",
        operationKey: "unknown_operation",
      },
    });
    expect(events).toContainEqual(
      expect.objectContaining({
        kind: "mapping_gap",
        reason: "operation_not_profiled",
      }),
    );
  });
  it.each(["create", "update", "patch"])(
    "maps %s without authorizing a direct master command",
    async (operationKey) => {
      const { wrapped, events, preflight } = setup({
        authorize: async () => ({ allowed: true }),
      });
      const permissionCode =
        ...
        (operationKey === "patch" ? "update" : operationKey);
      await wrapped.authorize({
        context: context(),
        permissionCode,
        resource: {
          entityCode: "business_partner",
          operationKey,
          ...(operationKey === "create" ? {} : { recordId }),
        },
      });
      expect(events).toContainEqual(
        expect.objectContaining({
          kind: "decision",
          operationKey: operationKey === "patch" ? "update" : operationKey,
          candidateTarget: "preflight_required",
          executionParity: false,
        }),
      );
  ...
  it.each([false, true])(
    "preserves qualification coverage and separation facts (company=%s)",
    async (company) => {
      const calls: Readonly<Record<string, unknown>>[] = [];
      const authority: Authorizer = {
        authorize: async (request) => {
          calls.push(request.resource ?? {});
          return request.resource?.["qualificationControl"] &&
            request.resource?.["makerCheckerEnforced"] &&
            request.resource?.["createdBy"] !== request.context.principalId
            ? { allowed: true }
            : {
                allowed: false,
      ...
      const request = {
        context: context(),
        permissionCode: "neon.supplier.qualification.admin",
        observation: {
          entityCode: "business_partner",
      ...
      expect(events).toContainEqual(
        expect.objectContaining({
          kind: "decision",
          operationKey: company ? "qualification_company" : "qualification",
          candidateTarget: "preflight_required",
        }),
      );
      ...
      expect(
        (
          await wrapped.authorize({
            ...
            resource: {
              ...request.resource,
              createdBy: request.context.principalId,
            },
          })
        ).allowed,
      ).toBe(false);
      expect(events.at(-1)).toMatchObject({ candidateTarget: "denied" });
      expect(preflight).not.toHaveBeenCalled();
    },
  );
  it.each(["supplier", "customer"])(
    "keeps %s company provider reads separate from global record admission",
    async (role) => {
      const resolve = vi.fn(
        async (input: Parameters<EntityScopeAdapter["resolve"]>[0]) => ({
          state: "resolved" as const,
          coordinates: input.coordinates ?? {},
        }),
      );
      const { wrapped, events } = setup(
        { authorize: async () => ({ allowed: true }) },
        { resolve, preflight: async () => "allowed" },
      );
      await wrapped.authorize({
        context: context(),
        permissionCode: permission,
      ...
      expect(events).toContainEqual(
        expect.objectContaining({
          kind: "decision",
          operationKey: role + "_company_read",
        }),
      );
  ...
  it("rejects unsupported ownership resolvers and planes before touching storage", async () => {
    const transaction = vi.fn(),
      adapter = createBusinessPartnerShadowScopes({ transaction } as never);
    for (const overrides of [
      { resolver: "workspace.record.v1" },
      { entityCode: "invoice" },
  ...
  it("carries a BP record through an actual HTTP request and leaves its UI response unchanged", async () => {
    ...
    const { shadow, wrapped, events } = setup(createPermissionAuthorizer(), {
        resolve: resolveScope,
        preflight: async () => "allowed",
      }),
      payload = {
        sections: [{ code: "identity", state: "ready" }],
        actions: [],
        data: { displayName: "Secret test partner" },
      };
    const app = createHttpApplication({
      configure(app) {
        shadow.register(app);
        app.get(
          "/api/neon/business-partners/:id/360/summary",
          async (_req, res) => {
            await wrapped.authorize({
              context: context(),
              permissionCode: permission,
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:${address.port}/api/neon/business-partners/${recordId}/360/summary`,
      );
      expect(await response.json()).toEqual(payload);
      expect(events).toContainEqual(
        expect.objectContaining({ kind: "decision", operationKey: "read" }),
      );
      expect(events).toContainEqual(
        expect.objectContaining({
          kind: "http_outcome",
      ...
      expect(JSON.stringify(events)).not.toContain("Secret test partner");
...
it("observes native target permissions alongside the pinned legacy vocabulary", async () => {
  const targetCode = "neon.relationship.bp_target.read";
  const candidate = parseEntityAuthorizationProfile({
    ...profile,
    operations: profile.operations.map((o) =>
      o.key === "read" ? { ...o, permissionCode: targetCode } : o,
    ),
  });
  const events: BusinessPartnerShadowEvent[] = [];
  const shadow = createBusinessPartnerAuthorizationShadow({
  ...
  expect(
    await shadow
      .wrap({
        authorize: async () => ({
          allowed: false,
          reason: "retained_source_denial",
        }),
      })
      .authorize(input),
  ).toEqual({ allowed: false, reason: "retained_source_denial" });
  expect(events.some((e) => e.kind === "mapping_gap")).toBe(false);
  expect(events).toContainEqual(
    expect.objectContaining({
      kind: "decision",
...
it("compares explicit native list intent with directory authority without changing the source decision", async () => {
  const targetCode = "neon.relationship.bp_target.read";
  const candidate = parseEntityAuthorizationProfile({
    ...profile,
    operations: profile.operations.map((o) =>
  ...
  const source = { allowed: false as const, reason: "source_denied" };
  const result = await shadow
    .wrap({ authorize: async () => source })
    .authorize({
      context: context(),
      permissionCode: targetCode,
      observation: {
      ...
      resource: { entityCode: "business_partner", operationKey: "read" },
    });
