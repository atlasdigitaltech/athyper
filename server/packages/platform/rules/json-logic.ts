...
export function evaluateJsonLogic(rule: unknown, data: Data): boolean {
  ...
  switch (operator) {
    ...
    default:
      // Unknown operators are configuration errors; gates must fail closed.
      throw Object.assign(
        new Error(`JSONLOGIC_UNKNOWN_OPERATOR: ${operator}`),
        { code: "JSONLOGIC_UNKNOWN_OPERATOR", operator },
      );
  }
}
