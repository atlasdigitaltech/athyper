...
export class AtlasExperienceConfigurationService {
  ...
  async publish(context: VerifiedRequestContext, scope: string, expectedRevision: number): Promise<AtlasExperienceRelease> { this.author(context); if (!Number.isSafeInteger(expectedRevision) || expectedRevision < 1) throw new TypeError("A positive expected revision is required."); return this.repository.publish({ context, scope: validCode(scope, "scope"), expectedRevision }); }
  private author(context: VerifiedRequestContext): void { if (context.planeKey !== "studio") throw new Error("Atlas experience authoring is owned by Studio."); }
}
...
export class KyselyAtlasExperienceConfigurationRepository implements AtlasExperienceConfigurationRepository {
  ...
  async saveDraft(input: { readonly context: VerifiedRequestContext; readonly definition: AtlasExperienceDefinition; readonly expectedRevision?: number }): Promise<AtlasExperienceRelease> {
    return this.transactions.run(input.context.planeKey, { tenantId: input.context.tenantId, principalId: input.context.principalId }, async (tx) => {
      ...
      if (input.expectedRevision !== undefined && input.expectedRevision !== currentRevision) throw new Error("Atlas experience revision conflict.");
  ...
  async publish(input: { readonly context: VerifiedRequestContext; readonly scope: string; readonly expectedRevision: number }): Promise<AtlasExperienceRelease> {
    return this.transactions.run(input.context.planeKey, { tenantId: input.context.tenantId, principalId: input.context.principalId }, async (tx) => {
      ...
      if (!draft) throw new Error("Atlas experience draft is missing or stale.");
...
export function validateAtlasExperienceDefinition(value: AtlasExperienceDefinition): AtlasExperienceDefinition {
  if (value.schema !== "atlas-experience-definition/1") throw new TypeError("Unsupported Atlas experience schema.");
  ...
  if (new Set(value.widgets.map((item) => item.kind)).size !== value.widgets.length) throw new TypeError("Each built-in widget kind can be configured only once.");
  value.widgets.forEach(validateWidget); value.searchSources.forEach(validateSource); value.agents.forEach(validateAgent);
  const agents = new Map(value.agents.map((item) => [item.code,item])); value.prompts.forEach((item) => { validatePrompt(item); const agent=agents.get(item.agentCode); if (!agent) throw new TypeError(`Prompt ${item.code} references an unknown agent.`); if(item.planes.some((plane)=>!agent.planes.includes(plane)))throw new TypeError(`Prompt ${item.code} targets a plane where its agent is unavailable.`); });
  if (Buffer.byteLength(JSON.stringify(value), "utf8") > 128 * 1024) throw new TypeError("Atlas experience definition exceeds 128 KiB.");
  return Object.freeze(value);
}

function validateWidget(item: AtlasWidgetConfiguration) { validCode(item.code, "widget code"); if (!["recommendations", "quick-actions", "workspaces", "recent"].includes(item.kind)) throw new TypeError("Widget kind is invalid."); text(item.title, 80, "widget title"); planes(item.planes); if (!Number.isSafeInteger(item.order) || item.order < 0 || item.order > 100) throw new TypeError("Widget order is invalid."); access(item.access); }
function validateSource(item: AtlasSearchSourceConfiguration) { validCode(item.code, "source code"); if (!["navigation", "record", "knowledge"].includes(item.kind)) throw new TypeError("Search source kind is invalid."); text(item.label, 100, "source label"); planes(item.planes); if (item.permissionCode) validCode(item.permissionCode, "permission code"); if (item.entityCode) validCode(item.entityCode, "entity code"); if (item.routePrefix && (!item.routePrefix.startsWith("/") || item.routePrefix.includes(".."))) throw new TypeError("Search source route is invalid."); }
...
function validateAgent(item: AtlasAgentConfiguration) { validCode(item.code, "agent code"); text(item.promptRevision, 128, "prompt revision"); text(item.name, 80, "agent name"); text(item.description, 240, "agent description"); text(item.publicModelId, 128, "public model id"); if (!["public", "internal", "confidential", "restricted"].includes(item.dataClass)) throw new TypeError("Agent data class is invalid."); planes(item.planes); bounded(item.toolCodes, 32, "agent tools"); item.toolCodes.forEach((code) => validCode(code, "tool code")); access(item.access); }
...
function planes(value: readonly string[]) { if (!value.length || value.some((plane) => !PLANES.includes(plane as typeof PLANES[number]))) throw new TypeError("At least one valid target plane is required."); }
function bounded(value: readonly unknown[], maximum: number, label: string) { if (!Array.isArray(value) || value.length > maximum) throw new TypeError(`Atlas experience ${label} exceed the configured limit.`); }
function unique(values: readonly string[], label: string) { const normalized = values.map((value) => validCode(value, `${label} code`)); if (new Set(normalized).size !== normalized.length) throw new TypeError(`Atlas experience ${label} codes must be unique.`); }
function validCode(value: string, label: string): string { if (!CODE.test(value)) throw new TypeError(`Atlas experience ${label} is invalid.`); return value; }
function text(value: string, maximum: number, label: string) { if (!value.trim() || value.length > maximum) throw new TypeError(`Atlas experience ${label} is invalid.`); }
