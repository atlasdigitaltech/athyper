...
export function createAtlasLocalGenerationServices(options:{businessContexts?:AtlasBusinessContextResolver;transactions:PlaneTransactionCoordinator<Transaction<Record<string,never>>>;config:AtlasLocalConfiguration;provider:AtlasModelProvider;tools?:{revalidate?:(context:VerifiedRequestContext,evidence:AtlasReadReplayEvidence)=>Promise<boolean>;coordinator:AtlasRuntimeToolCoordinator;readEnabled:boolean;mutationsEnabled:boolean;available:(access?:"read"|"mutation")=>boolean}}){
  const c=parseAtlasLocalConfiguration(options.config);
  const disclosure = new AtlasDurableMessageAuthorizer({reader:new KyselyAtlasMessageLineageReader(options.transactions),businessContexts:options.businessContexts,...(options.tools?.revalidate?{reads:{revalidate:options.tools.revalidate}}:{})});
  const conversation=createAtlasConversationServices(options.transactions,disclosure);
