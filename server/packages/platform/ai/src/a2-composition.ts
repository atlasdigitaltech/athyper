import type { AtlasCredentialCipher, AtlasCredentialInvalidation, AtlasDriftAlertPublisher, AtlasKnowledgeIndex, AtlasPolicyInvalidation, AtlasProviderCredentialResolver } from "@athyper/server-contract-ai";
...
export function createAtlasA2Services(options:{transactions:PlaneTransactionCoordinator<Tx>;platformCredentials:AtlasProviderCredentialResolver;credentialCipher:AtlasCredentialCipher;credentialInvalidation:AtlasCredentialInvalidation;knowledgeIndex:AtlasKnowledgeIndex;policyInvalidation:AtlasPolicyInvalidation;driftAlerts:AtlasDriftAlertPublisher}){
  ...
  return Object.freeze({credentials,credentialRepository,knowledge,knowledgeRepository,policies,monitoring,drift,resolver,knowledgeIndex:options.knowledgeIndex});
}
