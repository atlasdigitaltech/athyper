...
export function createAtlasRecordDataGateway(options: AtlasRecordDataGatewayOptions): AtlasRecordDataGateway {
  ...
  return {
    async query({ context, request }): Promise<AtlasRecordResult> {
      ...
      const result = await options.records.list({ context, entityCode: request.entityCode, limit: request.limit, filters: request.filters, sort: request.sort, countMode: "none", hydrateReferences: false });
      ...
      const sources = result.data.map((row) => {
        const recordId = row[idField]; const revision = versionField ? row[versionField] : undefined;
