...
export class TenantProviderCredentialRotationService {
  ...
  async rotate(input: {
    ...
    await this.db.transaction().execute(async (tx) => {
      const current = await sql<{ rotation_epoch: number }>`
        INSERT INTO ai.atlas_tenant_provider_credential_epoch
        ...
        ON CONFLICT (tenant_id,provider_id) DO UPDATE
          SET rotation_epoch=ai.atlas_tenant_provider_credential_epoch.rotation_epoch+1,
              revoked=false,updated_at=now()
        RETURNING rotation_epoch
      `.execute(tx);
      epoch = Number(current.rows[0]?.rotation_epoch ?? 1);
      await sql`UPDATE ai.atlas_tenant_provider_credential
        ...
        .execute(tx);
      await sql`INSERT INTO ai.atlas_tenant_provider_credential
      ...
      await sql`INSERT INTO audit.audit_log (
        ...
        actor_principal_id,actor_type,context
      ) VALUES (
        ${input.tenantId}::uuid,'ai.byok.rotated','update',
        'ai.byok_provider_binding',${input.tenantId}::uuid,
        ${input.actorPrincipalId}::uuid,'user',
        jsonb_build_object('provider_id',${input.providerId},'rotation_epoch',${epoch})
      )`
        .execute(tx);
    });
  ...
  async revoke(input: {
    ...
    await this.db.transaction().execute(async (tx) => {
      const current = await sql<{ rotation_epoch: number }>`
        UPDATE ai.atlas_tenant_provider_credential_epoch
      ...
      await sql`INSERT INTO audit.audit_log (
        ...
        ${input.tenantId}::uuid,'ai.byok.revoked','revoke',
        'ai.byok_provider_binding',${input.tenantId}::uuid,
        ${input.actorPrincipalId}::uuid,'user',
        jsonb_build_object('provider_id',${input.providerId},'rotation_epoch',${epoch})
      )`
  ...
  async reEncryptTenant(tenantId: string): Promise<number> {
    ...
    const rows = await sql<{ id: string; encrypted_secret: string }>`
      SELECT id,encrypted_secret FROM ai.atlas_tenant_provider_credential
