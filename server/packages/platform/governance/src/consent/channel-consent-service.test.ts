...
import { brandExactPlaneTransaction,createExactPlaneRepositoryProvider } from "@athyper/server-foundation/transaction";

describe("channel consent destination hashing",()=>{
  it("normalizes email and telephone destinations without retaining the raw value",()=>{expect(hashDestination("email"," User@Example.COM ")).toBe(hashDestination("email","user@example.com"));expect(hashDestination("sms","+60 (12) 345-6789")).toBe(hashDestination("sms","60123456789"));expect(hashDestination("email","user@example.com")).toMatch(/^[a-f0-9]{64}$/);});
