...
export function createCycleRunService(options: CycleExecutionServiceOptions): CycleRunService {
  ...
  return {
    async create(command: CreateCycleRunCommand) {
      ...
      if (command.periodEnd && (!command.periodStart || command.periodEnd < command.periodStart)) fail("GOVERNANCE_INVALID_COMMAND", { field: "periodEnd" });
      if (command.scheduledStartAt && command.dueAt && command.dueAt < command.scheduledStartAt) fail("GOVERNANCE_INVALID_COMMAND", { field: "dueAt" });
      if (!/^[A-Z][A-Z0-9_.-]{1,62}$/.test(command.code)) fail("GOVERNANCE_INVALID_COMMAND", { field: "code" });
      ...
      return exactRepository(options, command.context).transaction(command.context, async (store) => {
        ...
        if (replay) {
          if (replay.cycleTypeId !== command.cycleTypeId || replay.code !== command.code || replay.name !== command.name || replay.periodStart !== command.periodStart || replay.periodEnd !== command.periodEnd || replay.scheduledStartAt !== command.scheduledStartAt || replay.dueAt !== command.dueAt || replay.parentCycleRunId !== command.parentCycleRunId || replay.ownerPrincipalId !== command.ownerPrincipalId || !isDeepStrictEqual(replay.data, command.data ?? {}) || (command.templateVersion !== undefined && replay.templateRevisionNumber !== command.templateVersion)) fail("GOVERNANCE_IDEMPOTENCY_CONFLICT");
        ...
        const tasks: CycleTask[] = template.template.tasks.map((task) => ({ id: taskIds.get(task.id)!, tenantId: run.tenantId, cycleRunId: run.id, cycleTypeId: run.cycleTypeId, taskTemplateId: task.id, phaseId: task.phaseId, code: task.code, name: task.name, completionMode: task.completionMode, isMandatory: task.isMandatory, isWaivable: task.isWaivable, completionEvidence: {}, status: dependencyTemplates.some((edge) => edge.successorTemplateId === task.id && edge.isHard && edge.dependencyType === "finish_to_start") ? "pending" : "ready", createdAt: at, createdBy: command.context.principalId, updatedAt: at, updatedBy: command.context.principalId, version: 1 }));
...
export function createCycleTaskService(options: CycleExecutionServiceOptions): CycleTaskService {
  ...
  const change = async (context: VerifiedRequestContext, taskId: string, action: "claim" | "start" | "complete" | "block" | "waive" | "reopen", evidence: Readonly<Record<string, unknown>> = {}, owner?: string): Promise<CycleTask> => {
    ...
    return exactRepository(options, context).transaction(context, async (store) => {
      ...
      const dependencies = await store.listDependencies(run.id); const tasks = await store.listTasks(run.id); const passed = dependenciesPass(task.id, dependencies, tasks, "start");
      ...
      if (action === "complete") { if (task.status !== "in_progress" || !dependenciesPass(task.id, dependencies, tasks, "finish")) invalidTask(task, action); assertOwner(task, context); nonempty(evidence, "completion evidence"); status = "completed"; }
      ...
      if (action === "reopen") { if (!["completed", "waived", "blocked"].includes(task.status)) invalidTask(task, action); nonempty(evidence, "reopen reason");
        required(String(evidence["reason"] ?? ""), "reason");
        if (dependencies.some((edge) => edge.isHard && edge.predecessorTaskId === task.id && tasks.some((successor) => successor.id === edge.successorTaskId && (["completed", "waived"].includes(successor.status) || edge.dependencyType === "finish_to_start" && successor.status === "in_progress")))) fail("GOVERNANCE_INVALID_TRANSITION", { reason: "dependent_task_active_or_complete" });
      ...
      if (action === "reopen") {
        for (const successor of tasks) {
          if (successor.status === "ready" && dependencies.some((edge) => edge.isHard && edge.dependencyType === "finish_to_start" && edge.predecessorTaskId === task.id && edge.successorTaskId === successor.id))
            await store.putTask({ ...successor, status: "pending", updatedAt: at, updatedBy: context.principalId, version: successor.version + 1 });
        }
      }
...
export function createCycleDeviationService(options: CycleExecutionServiceOptions): CycleDeviationService {
  ...
  const decide = async (context: VerifiedRequestContext, deviationId: string, status: "resolved" | "waived", resolution: string) => {
    ...
    return exactRepository(options, context).transaction(context, async (store) => { const value = await requireDeviation(store, deviationId); await assertRunMutable(store, await requireRun(store, value.cycleRunId)); if (value.status !== "open") fail("GOVERNANCE_DEVIATION_IMMUTABLE"); const updated: CycleDeviation = { ...value, status, resolution, decidedAt: now().toISOString(), decidedBy: context.principalId }; await store.putDeviation(updated); return updated; });
  };
  return {
    ...
    async carryForward(context, deviationId, targetRunId, idempotencyKey) { await permitted(options.authorizer, context, governancePermissions.cycleReview); required(idempotencyKey, "idempotencyKey"); return exactRepository(options, context).transaction(context, async (store) => { const replay = await store.findDeviationCarryByIdempotencyKey(idempotencyKey); if (replay) { if (replay.carriedFromDeviationId !== deviationId || replay.cycleRunId !== targetRunId) fail("GOVERNANCE_IDEMPOTENCY_CONFLICT"); return { kind: "replayed" as const, deviation: replay }; } const source = await requireDeviation(store, deviationId); if (source.status !== "open") fail("GOVERNANCE_DEVIATION_IMMUTABLE"); if (source.cycleRunId === targetRunId) fail("GOVERNANCE_INVALID_COMMAND", { field: "targetRunId" }); await assertRunMutable(store, await requireRun(store, source.cycleRunId)); const target = await requireRun(store, targetRunId); await assertRunMutable(store, target); const { cycleTaskId: _task, carriedToCycleRunId: _target, ...carryBase } = source; const carried: CycleDeviation = { ...carryBase, id: id(), cycleRunId: target.id, status: "open", carriedFromDeviationId: source.id, carryCount: source.carryCount + 1, createdAt: now().toISOString(), createdBy: context.principalId }; const closed: CycleDeviation = { ...source, status: "carried", resolution: `Carried to cycle ${target.id}`, carriedToCycleRunId: target.id, decidedAt: now().toISOString(), decidedBy: context.principalId }; await store.putDeviation(closed); await store.putDeviation(carried); await store.recordDeviationCarryIdempotencyKey(idempotencyKey, carried.id); return { kind: "carried" as const, deviation: carried }; }); },
  };
}
...
async function unlockSuccessors(store: CycleExecutionStore, runId: string, predecessorId: string, at: string, principalId: string): Promise<void> { const dependencies = await store.listDependencies(runId); const successorIds = new Set(dependencies.filter((item) => item.predecessorTaskId === predecessorId).map((item) => item.successorTaskId)); if (!successorIds.size) return; const tasks = await store.listTasks(runId); for (const successorId of successorIds) { const successor = tasks.find((item) => item.id === successorId); if (successor?.status === "pending" && dependenciesPass(successor.id, dependencies, tasks, "start")) await store.putTask({ ...successor, status: "ready", updatedAt: at, updatedBy: principalId, version: successor.version + 1 }); } }
function dependenciesPass(taskId: string, dependencies: readonly CycleTaskDependency[], tasks: readonly CycleTask[], stage: "start" | "finish"): boolean { const predecessors = dependencies.filter((item) => item.successorTaskId === taskId && item.isHard && (stage === "finish" || item.dependencyType === "finish_to_start")); return predecessors.every((edge) => { const task = tasks.find((item) => item.id === edge.predecessorTaskId); return task !== undefined && (["completed", "waived"].includes(task.status) || !edge.isHard); }); }
