...
async function sweep(
  ...
  const result = await sql<PendingMessage>`
    ...
    RETURNING id, tenant_id, plane_key, event_id, template_key, template_version, subject, payload, channels,
...
async function send(
  ...
  const msgResult = await sql<PendingMessage>`
    SELECT id, tenant_id, plane_key, event_id, template_key, template_version, subject, payload, channels,
           rule_id, event_code, entity_type, entity_id
    FROM   event.notification_message
  ...
  for (const row of eligibleRows) {
    ...
    const fromOverride = row.channel === "email" && emailFromMap
      ? emailFromMap.get(msg.plane_key)
      : undefined;
...
async function flush(
  ...
  const claimed = await sql<StagedRow>`
    ...
    RETURNING
      ...
      (SELECT nm.plane_key
        ...
        WHERE nm.id = event.digest_staging.message_id) AS plane_key
  `.execute(db);

  const rows = claimed.rows;
  if (rows.length === 0) return;

  // Group by (tenant_id, plane_key, recipient_id, channel)
  const groups = new Map<string, StagedRow[]>();
  for (const row of rows) {
    const key = `${row.tenant_id}::${row.plane_key}::${row.recipient_id}::${row.channel}`;
  ...
  for (const [, groupRows] of groups) {
    const { tenant_id, plane_key, recipient_id, channel } = groupRows[0]!;
