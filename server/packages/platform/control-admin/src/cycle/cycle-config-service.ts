  ...
  return {
    ...
    async publish(command) {
      ...
      return options.repositories.require(command.context.planeKey).publish({
        ...
        ...(command.expectedLatestVersion !== undefined
          ? { expectedLatestVersion: command.expectedLatestVersion }
          : {}),
        preview,
      });
    },
    async applyDesiredState(command) {
      ...
      if (
        ...
        throw coded("CONTROL_ADMIN_DESIRED_STATE_TARGET_MISMATCH");
      if (cycleTemplateHash(revision.template) !== revision.templateHash)
        throw coded("CONTROL_ADMIN_DESIRED_STATE_HASH_INVALID");
      const verifier = options.desiredStateVerifier;
      if (
        !verifier ||
        !(await verifier.verify(
          desiredStatePayload(revision),
          revision.signature,
        ))
      )
        throw coded("CONTROL_ADMIN_DESIRED_STATE_SIGNATURE_INVALID");
...
export function validateCycleTemplate(
  template: CycleTemplateDraft,
): readonly CycleTemplateIssue[] {
  return validateLocal(normalize(template));
}
export function cycleTemplateHash(template: CycleTemplateDraft): string {
  return createHash("sha256")
    .update(stable(normalize(template)), "utf8")
    .digest("hex");
}
...
function validateLocal(template: CycleTemplateDraft): CycleTemplateIssue[] {
  ...
  duplicates(
    template.tasks,
    (item) => `${item.entityCode}:${item.code}`,
    "tasks",
    "code",
    issues,
  );
  duplicates(
    template.tasks,
    (item) => `${item.phaseId}:${item.sortOrder}`,
    "tasks",
    "sortOrder",
    issues,
    "DUPLICATE_ORDER",
  ...
  duplicates(
    template.carryForwardRules,
    (item) => item.deviationType,
    "carryForwardRules",
    "deviationType",
    issues,
  );
  ...
  template.tasks.forEach((task, index) => {
    ...
    if (!categoryIds.has(task.categoryId))
      issue(
        ...
        `Unknown category ${task.categoryId}`,
      );
    if (task.completionMode !== "manual" && !task.systemCheckHandler?.trim())
      issue(
        issues,
        "INVALID_TASK_HANDLER",
        `tasks[${index}].systemCheckHandler`,
        "System and hybrid tasks require a handler",
      );
  });
  const edges = new Set<string>();
  template.dependencies.forEach((edge, index) => {
    if (!taskIds.has(edge.predecessorTemplateId))
      issue(
        issues,
        "MISSING_REFERENCE",
        `dependencies[${index}].predecessorTemplateId`,
        `Unknown task ${edge.predecessorTemplateId}`,
      );
    if (!taskIds.has(edge.successorTemplateId))
      issue(
        issues,
        "MISSING_REFERENCE",
        `dependencies[${index}].successorTemplateId`,
        `Unknown task ${edge.successorTemplateId}`,
      );
    if (edge.predecessorTemplateId === edge.successorTemplateId)
      issue(
        issues,
        "SELF_DEPENDENCY",
        `dependencies[${index}]`,
  ...
  if (taskIds.size > 0 && topological(template).length !== taskIds.size)
    issue(
      issues,
      "CYCLIC_DEPENDENCY",
      "dependencies",
  ...
  template.carryForwardRules.forEach((rule, index) => {
    if (rule.action === "auto_carry" && !rule.targetCycleTypeId)
      issue(
        issues,
        "INVALID_CARRY_FORWARD_TARGET",
        `carryForwardRules[${index}].targetCycleTypeId`,
        "auto_carry requires a target cycle type",
      );
  });
  return issues;
}
...
async function validateExternal(
  ...
  for (let index = 0; index < template.crossDependencies.length; index += 1) {
    ...
    if (
      edge.predecessorTypeId !== currentType &&
      edge.successorTypeId !== currentType
    ) {
      issue(
        issues,
        "INVALID_CROSS_CYCLE_REFERENCE",
        `crossDependencies[${index}]`,
        "At least one side must reference the template cycle type",
      );
      continue;
    }
    const endpoints = [
      [edge.predecessorTypeId, edge.predecessorPhaseId, "predecessorPhaseId"],
      [edge.successorTypeId, edge.successorPhaseId, "successorPhaseId"],
    ] as const;
    for (const [typeId, phaseId, field] of endpoints) {
      const exists =
        typeId === currentType
          ? localPhases.has(phaseId)
          : await repository.externalPhaseExists(tenantId, typeId, phaseId);
      if (!exists)
        issue(
          issues,
          "INVALID_CROSS_CYCLE_REFERENCE",
          `crossDependencies[${index}].${field}`,
          `Unknown cycle phase ${typeId}/${phaseId}`,
        );
    }
  }
  for (let index = 0; index < template.carryForwardRules.length; index += 1) {
    ...
    if (
      target &&
      target !== currentType &&
      !(await repository.cycleTypeExists(tenantId, target))
    )
      issue(
        issues,
        "INVALID_CARRY_FORWARD_TARGET",
        `carryForwardRules[${index}].targetCycleTypeId`,
        `Unknown cycle type ${target}`,
      );
  }
}
...
function topological(template: CycleTemplateDraft): string[] {
  ...
  for (const edge of template.dependencies)
    if (
      indegree.has(edge.predecessorTemplateId) &&
      indegree.has(edge.successorTemplateId) &&
      edge.predecessorTemplateId !== edge.successorTemplateId
    ) {
      outgoing.get(edge.predecessorTemplateId)!.push(edge.successorTemplateId);
      indegree.set(
        edge.successorTemplateId,
        indegree.get(edge.successorTemplateId)! + 1,
      );
    }
...
function normalize(template: CycleTemplateDraft): CycleTemplateDraft {
  return {
    ...
    tasks: [...template.tasks].sort(
      (a, b) =>
        a.phaseId.localeCompare(b.phaseId) ||
        a.sortOrder - b.sortOrder ||
        a.id.localeCompare(b.id),
    ),
    dependencies: [...template.dependencies].sort((a, b) =>
      `${a.predecessorTemplateId}:${a.successorTemplateId}`.localeCompare(
        `${b.predecessorTemplateId}:${b.successorTemplateId}`,
      ),
    ),
    crossDependencies: [...template.crossDependencies].sort((a, b) =>
      stable(a).localeCompare(stable(b)),
    ),
    carryForwardRules: [...template.carryForwardRules].sort((a, b) =>
      a.deviationType.localeCompare(b.deviationType),
    ),
  };
}
function desiredStatePayload(
  revision: CycleDesiredStatePayload & { readonly signature: unknown },
): CycleDesiredStatePayload {
  const { signature: _signature, ...payload } = revision;
  return payload;
}
function byOrder<T extends { readonly sortOrder: number; readonly id: string }>(
  a: T,
  b: T,
): number {
...
function duplicates<T>(
  ...
  field: string,
  issues: CycleTemplateIssue[],
  code: CycleTemplateIssue["code"] = "DUPLICATE_CODE",
): void {
