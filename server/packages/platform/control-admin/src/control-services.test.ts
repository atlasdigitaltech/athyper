...
describe("C2 control services", () => {
  it("selects catalog reads and tenant override writes from the request plane", async () => {
    ...
    await service.saveOverride({ context, code: "ui.new", enabled: false, reason: "neon only", effectiveFrom: "2026-01-01T00:00:00Z", expectedVersion: 0 });
  ...
  it("fails closed when the requested plane has no control repository", async () => {
    ...
    await expect(service.evaluate(context, "ui.new")).rejects.toMatchObject({ code: "CONTROL_ADMIN_FEATURE_REPOSITORY_UNAVAILABLE", statusCode: 503 });
  });

  it("keeps a zero-percent experiment disabled even when its catalog default is enabled", async () => {
    const repository = {
      ...featureRepository(true),
      getDefinition: vi.fn(async () => ({ id: "flag-360", code: "neon.business_partner.view_360", cohortStrategy: "principal_fnv1a_v2" as const, cohortRevision: 1, defaultEnabled: true, rolloutPct: 0, effectiveFrom: "2026-01-01T00:00:00Z", status: "active" as const })),
  ...
  it("keeps platform catalog publication Studio-owned", async () => {
    const studio = { listApplicable: vi.fn(async () => []), list: vi.fn(async () => []), get: vi.fn(), publish: vi.fn(async (rule: BankValidationRule) => rule) };
  ...
  it("allows only exact-tenant lookup retirement", async () => {
    const retireValue = vi.fn(async () => lookupDomain());
    const repository = { getDomain: vi.fn(async () => ({...lookupDomain(), values:[{id:"value-1",code:"tenant",name:"Tenant",tenantId:context.tenantId,sortOrder:0,metadata:{},status:"active" as const}]})), listDomains: vi.fn(async () => []), publishDesiredState: vi.fn(), isValueReferenced: vi.fn(async () => false), retireValue };
    ...
    await expect(service.retireValue(context, { domainCode: "reason", valueCode: "tenant", tenantId: context.tenantId, expectedVersion:1 })).resolves.toMatchObject({ code: "reason" });
    expect(retireValue).toHaveBeenCalledOnce();
  });

  it("validates parameter types, bounds, allowed values, and durations", () => {
    const numeric = { revision:1, id: "p1", code: "finance.threshold", valueType: "integer" as const, defaultValue: 2, minValue: 1, maxValue: 5, allowedValues: [1, 2, 3], tenantCanOverride: true, reloadMode: "immediate" as const, cacheTtlSeconds: 0, status: "active" as const };
    ...
    expect(() => validateParameterValue({ ...numeric, valueType: "duration", minValue: undefined, maxValue: undefined, allowedValues: undefined }, "PT15M")).not.toThrow();
  });

  it("uses an effective feature override and invalidates only after a successful write", async () => {
    const repository = { getDefinition: vi.fn(async () => ({ id: "flag-1", code: "ui.new", cohortStrategy: "principal_fnv1a_v2" as const, cohortRevision: 1, defaultEnabled: false, effectiveFrom: "2026-01-01T00:00:00Z", status: "active" as const })), listDefinitions: vi.fn(async () => []), getOverride: vi.fn(async () => ({ id: "override-1", version: 1, tenantId: "tenant-1", featureFlagId: "flag-1", enabled: true, reason: "rollout", effectiveFrom: "2026-01-01T00:00:00Z", status: "active" as const })), saveOverride: vi.fn(async (value) => ({ ...value, id: "override-1", version: 1, status: "active" as const })), expireOverride: vi.fn() } satisfies FeatureFlagRepository;
    ...
    await service.saveOverride({ context, code: "ui.new", enabled: true, reason: "pilot", effectiveFrom: "2026-01-01T00:00:00Z", expectedVersion: 0 });
  ...
  it("selects the most-specific rounding context and rejects ambiguous overlap", async () => {
    ...
    const service = createRoundingService({ authorizer: allow, repositories: provider({ getCurrencyDefaults: async () => undefined, list: async () => rules, get: async () => undefined, save: async (value) => ({ ...value, version: 1 }), retire: async () => rules[0]! }), cache: cache() });
  ...
  it("simulates decimal ties without binary floating-point drift", async () => {
    ...
    const service = createRoundingService({ authorizer: allow, repositories: provider({ getCurrencyDefaults: async () => undefined, list: async () => [exact], get: async () => exact, save: async (value) => ({ ...value, version: 2 }), retire: async () => exact }), cache: cache() });
  ...
  it("enforces connector lifecycle, endpoint compatibility, and secret references", async () => {
    ...
    await expect(service.saveDraft(context, { ...connector(), config: { apiToken: "plaintext" } }, 1)).rejects.toMatchObject({ code: "CONTROL_ADMIN_CONNECTOR_INVALID" });
    await expect(service.saveDraft(context, { ...connector(), endpoints: [{ code: "escape", path: "https://evil.test", method: "GET", kind: "health" }] }, 1)).rejects.toMatchObject({ code: "CONTROL_ADMIN_CONNECTOR_INVALID" });
...
function featureRepository(defaultEnabled: boolean) { return { getDefinition: vi.fn(async () => ({ id: "flag-1", code: "ui.new", cohortStrategy: "principal_fnv1a_v2" as const, cohortRevision: 1, defaultEnabled, effectiveFrom: "2026-01-01T00:00:00Z", status: "active" as const })), listDefinitions: vi.fn(async () => []), getOverride: vi.fn(async () => undefined), saveOverride: vi.fn(async (value) => ({ ...value, id: "override-1", version: 1, status: "active" as const })), expireOverride: vi.fn() } satisfies FeatureFlagRepository; }
