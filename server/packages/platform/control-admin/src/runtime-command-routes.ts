...
export function registerRuntimeCommandRoutes(
  ...
  registerContractRoute(
    ...
    contract(
      "post",
      "/api/control-admin/runtime-commands/dry-run",
      "runtimeCommands.dryRun",
      "Preview and diff a runtime command",
      "control.runtime_command.manage",
      controlAdminSchemas.runtimeCommand,
    ),
    options.authenticate,
    route(async (request, response) =>
      options.service.preview(
  ...
  registerContractRoute(
    ...
    contract(
      "post",
      "/api/control-admin/runtime-commands",
      "runtimeCommands.submit",
      "Submit or apply a runtime command",
      "control.runtime_command.manage",
      controlAdminSchemas.runtimeCommand,
    ),
    options.authenticate,
  ...
  registerContractRoute(
    ...
    contract(
      "post",
      "/api/control-admin/runtime-approvals/:id/decisions",
      "runtimeApprovals.decide",
      "Approve or reject a high-risk runtime command",
      "control.runtime_command.approve",
      controlAdminSchemas.runtimeApprovalDecision,
    ),
    options.authenticate,
    route(async (request, response) => {
  ...
  registerContractRoute(
    ...
    contract(
      "get",
      "/api/control-admin/runtime-history",
      "runtimeHistory.list",
      "List immutable runtime change history",
      "control.runtime_history.read",
    ),
    options.authenticate,
    route(async (request, response) =>
      options.service.history(
        options.readContext(response),
        optionalLimit(request.query["limit"]),
      ),
    ),
  );
}
...
function contract(
  ...
  return defineRouteContract({
    method,
    path,
    operationId: `controlAdmin.${operationId}`,
    summary,
    tags: ["Control Administration"],
    authenticated: true,
    permission,
    request: {
      ...(schema ? { body: schema } : {}),
      ...(path.includes(":id")
        ? { params: controlAdminSchemas.runtimeApprovalParams }
        : {}),
      query:
        method === "get"
          ? controlAdminSchemas.runtimeHistoryQuery
          : { type: "object", additionalProperties: false, properties: {} },
    },
    responses: {
      200: {
        description: "Control administration response",
        body:
          operationId === "runtimeCommands.dryRun"
            ? controlAdminSchemas.runtimePreview
            : operationId === "runtimeApprovals.decide"
              ? controlAdminSchemas.runtimeApproval
              : operationId === "runtimeHistory.list"
                ? controlAdminSchemas.runtimeHistory
                : controlAdminSchemas.runtimeSubmission,
      },
      ...(operationId === "runtimeCommands.submit"
        ? {
            202: {
              description: "Approval required or execution pending",
              body: controlAdminSchemas.runtimeSubmission,
            },
          }
        : ({} as Record<number, { description: string; body: RuntimeSchema }>)),
      401: { description: "Authentication required" },
      503: { description: "Persistence or executor unavailable" },
...
function route(
  work: (request: Request, response: Response) => Promise<unknown>,
) {
  return async (
    request: Request,
    response: Response,
...
function approvalDecision(value: unknown): RuntimeApprovalDecision {
  if (value !== "approved" && value !== "rejected") throw invalid();
  return value;
}
