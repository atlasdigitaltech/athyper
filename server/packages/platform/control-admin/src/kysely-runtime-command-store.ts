...
export class KyselyRuntimeCommandStore implements RuntimeCommandStore {
  ...
  async findSubmission(tenantId: string, idempotencyKey: string): Promise<StoredRuntimeCommandSubmission | undefined> {
  ...
  async appendSubmission(record: StoredRuntimeCommandSubmission): Promise<boolean> {
    const outcome = record.submission.outcome === "replayed" ? "applied" : record.submission.outcome;
    ...
    const existing = await this.findSubmission(record.tenantId, record.idempotencyKey);
    if (!existing || existing.fingerprint !== record.fingerprint) throw coded("CONTROL_ADMIN_IDEMPOTENCY_CONFLICT", 409);
    return false;
  }

  async appendApprovalRequest(approval: StoredRuntimeCommandApproval): Promise<void> {
    const result = await sql<Row>`
      INSERT INTO ops.control_runtime_command_approval_request
        (id,tenant_id,plane_code,command_id,kind,command_fingerprint,requested_by,requested_at)
      VALUES
        (${approval.approvalId}::uuid,${approval.tenantId}::uuid,${approval.planeKey},${approval.commandId},
         ${approval.kind},${approval.commandFingerprint},${approval.requestedBy}::uuid,${approval.requestedAt}::timestamptz)
      ON CONFLICT (id) DO NOTHING RETURNING id
    `.execute(this.db);
    if (!result.rows[0]) {
      const existing = await this.getApproval(approval.approvalId);
      if (!existing || existing.commandFingerprint !== approval.commandFingerprint) throw coded("CONTROL_ADMIN_IDEMPOTENCY_CONFLICT", 409);
    }
  }
  ...
  async getApproval(approvalId: string): Promise<StoredRuntimeCommandApproval | undefined> {
    const row = (await sql<Row>`
  ...
  async appendHistory(input: Omit<RuntimeCommandHistoryEntry, "historyId" | "entryHash" | "previousHash">): Promise<RuntimeCommandHistoryEntry> {
    return this.db.transaction().execute(async (transaction) => {
      await sql`SELECT pg_advisory_xact_lock(hashtextextended(${`${input.planeKey}:${input.tenantId}`},0))`.execute(transaction);
      const previousHash = String((await sql<Row>`
      ...
      `.execute(transaction)).rows[0]?.["entry_hash"] ?? "") || undefined;
  ...
  async listHistory(input: { readonly tenantId: string; readonly limit: number }): Promise<readonly RuntimeCommandHistoryEntry[]> {
  ...
  async health(): Promise<void> {
    await sql`SELECT 1 FROM ops.control_runtime_command_history LIMIT 1`.execute(this.db);
  }
}

function submissionRow(tenantId: string, row: Row): StoredRuntimeCommandSubmission {
  return { tenantId, planeKey: String(row["plane_code"]) as StoredRuntimeCommandSubmission["planeKey"], actorId: String(row["created_by"]), idempotencyKey: String(row["idempotency_key"]), fingerprint: String(row["fingerprint"]), submission: object(row["submission"]) as unknown as RuntimeCommandSubmission };
}
function approvalRow(row: Row): StoredRuntimeCommandApproval {
  const decision = row["decision"] == null ? undefined : String(row["decision"]) as RuntimeApprovalDecision;
  return {
    approvalId: String(row["id"]), commandId: String(row["command_id"]), commandFingerprint: String(row["command_fingerprint"]),
    requestedBy: String(row["requested_by"]), requestedAt: new Date(String(row["requested_at"])).toISOString(),
    status: decision ?? "pending", kind: String(row["kind"]), tenantId: String(row["tenant_id"]),
    planeKey: String(row["plane_code"]) as StoredRuntimeCommandApproval["planeKey"],
    ...(decision ? { decidedBy: String(row["decided_by"]), decidedAt: new Date(String(row["decided_at"])).toISOString(), decisionReason: String(row["decision_reason"]) } : {}),
  };
}
function historyRow(row: Row): RuntimeCommandHistoryEntry {
  return {
    historyId: String(row["id"]), commandId: String(row["command_id"]), kind: String(row["kind"]),
    event: String(row["event"]) as RuntimeCommandHistoryEntry["event"], planeKey: String(row["plane_code"]) as RuntimeCommandHistoryEntry["planeKey"],
    tenantId: String(row["tenant_id"]), actorId: String(row["actor_id"]), reason: String(row["reason"]),
    fingerprint: String(row["fingerprint"]), detail: object(row["detail"]) as Readonly<Record<string, JsonValue>>,
    occurredAt: new Date(String(row["occurred_at"])).toISOString(),
    ...(row["previous_hash"] == null ? {} : { previousHash: String(row["previous_hash"]) }), entryHash: String(row["entry_hash"]),
  };
}
