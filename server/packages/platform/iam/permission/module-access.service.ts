...
export async function getEffectiveModuleAccess(
  ...
  // Subscription is commercial state, not tenant identity state. Callers must
  // supply the verified entitlement snapshot/version selected for this session.
  const planVersionId = options.planVersionId ?? null;
  if (!planVersionId) {
    return {
      moduleIds: [],
      moduleCodes: [],
  ...
  if (cache) {
    ...
    WITH group_role_modules AS (
      ...
      JOIN control.workspace_module workspace_module
        ON workspace_module.module_id = module.id
       AND workspace_module.is_primary
       AND workspace_module.status = 'active'
