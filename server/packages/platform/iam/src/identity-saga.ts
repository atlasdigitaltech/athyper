...
export type IamOrganizationPurpose =
  "tenant_employer" | "supplier_portal" | "customer_portal";
export type IdentityRelationship = "employer" | "contact" | "external_worker";
export type IdentityDesiredStatus =
...
export interface DesiredOrganizationMembership {
  ...
  /** Employment, BP-contact, or worker-engagement coordinate. It is correlation evidence, not a provider grant. */
...
export interface IdentitySagaRepository {
  ...
  fail(
    ...
    input: {
      ...
      readonly classification: IdentitySagaFailureClass;
...
export interface PlaneLocalIdentityAuthority {
  ...
  revokeAccess(input: {
    ...
    readonly desiredHash: string;
    readonly applications: readonly DesiredApplicationAccess[];
    readonly reason: "suspended" | "deprovisioned";
  }): Promise<void>;
}
...
export class IdentitySagaWorker {
  ...
  async runOne(
    claimToken: string,
  ): Promise<"idle" | "succeeded" | "retry" | "dead_letter" | "stale"> {
    ...
    const work = await this.options.repository.claim({
      ...
      leaseExpiresAt: new Date(
        claimedAt.getTime() + (this.options.leaseMs ?? 60_000),
      ).toISOString(),
    });
    if (!work) return "idle";
    ...
    try {
      ...
      if (!(await this.options.repository.current(work)))
        throw coded("IDENTITY_DESIRED_STATE_STALE");
      if (
        ...
        throw coded("IDENTITY_ATTEMPT_FENCE_REJECTED");
      const organization = await this.options.organizations.get(
        work.membership.organizationId,
      );
      validateRelationship(work, organization);

      let providerSubject = await this.options.local.providerSubject(
        work.identityId,
      );
      if (work.status === "invited" || work.status === "active") {
        providerSubject ??= (
          await this.options.provider.inviteOrCreate({
        ...
        for (const application of work.applications) {
          await this.options.provider.ensureApplication({
            providerSubject,
            plane: application.plane,
            targetTenantId: application.targetTenantId,
            idempotencyKey: providerKey(
              work,
              "application",
              `${application.plane}:${application.targetTenantId}`,
            ),
          });
        }
        if (!(await this.options.repository.current(work)))
        ...
        await this.options.local.converge({
          ...
          providerSubject,
          realmKey: work.realmKey,
          desiredVersion: work.desiredVersion,
          desiredHash: work.desiredHash,
        ...
        await this.options.local.revokeAccess({
          identityId: work.identityId,
          personId: work.personId,
          desiredVersion: work.desiredVersion,
          desiredHash: work.desiredHash,
        ...
        if (providerSubject) {
          if (work.status === "suspended")
            await this.options.provider.suspend({
              realmKey: work.realmKey,
              providerSubject,
              idempotencyKey: providerKey(work, "suspend"),
            });
          else
            await this.options.provider.deprovision({
              realmKey: work.realmKey,
              providerSubject,
              idempotencyKey: providerKey(work, "deprovision"),
            });
        }
      }
      if (
        !(await this.options.repository.succeed(coordinate, {
      ...
      const nextAttemptAt = deadLetter
        ? undefined
        : new Date(
            this.now().getTime() + identityRetryDelay(work.attemptNo),
          ).toISOString();
      const errorCode = safeErrorCode(error);
      const recorded = await this.options.repository.fail(coordinate, {
      ...
      this.options.alerts.metric({
        classification,
        outcome: deadLetter ? "dead_letter" : "retry",
      });
      if (deadLetter)
        await this.options.alerts.deadLetter({
          authorityTenantId: work.authorityTenantId,
          identityId: work.identityId,
...
export class ProviderIdentityCallbackConsumer {
  ...
  async consume(
    callback: ProviderIdentityCallback,
  ): Promise<"applied" | "duplicate" | "stale" | "out_of_order"> {
    validateCallback(callback);
    if ((await this.repository.accept(callback)) === "duplicate")
      return "duplicate";
...
export function diffIdentityProjection(
  desired: readonly DesiredIdentityProjection[],
  observed: readonly ProviderIdentityObservation[],
): readonly IdentityProjectionDrift[] {
  ...
  for (const item of desired) {
    ...
    if (
      found.desiredVersion !== item.desiredVersion ||
      found.desiredHash !== item.desiredHash ||
    ...
    if (!found.organizationIds.includes(item.membership.organizationId))
      drift.push({
        identityId: item.identityId,
        kind: "missing",
        coordinate: `organization:${item.membership.organizationId}`,
      });
    for (const application of item.applications) {
      const coordinate = `${application.plane}:${application.targetTenantId}`;
      if (!found.applications.includes(coordinate))
        drift.push({
          identityId: item.identityId,
          kind: "missing",
...
export interface PeriodicIdentityReconciliationRepository {
  ...
  recordExact(input: {
    ...
    readonly desiredVersion: number;
    readonly desiredHash: string;
    readonly status: "in_sync" | "drifted";
    readonly drift: readonly IdentityProjectionDrift[];
    readonly observedAt: string;
...
export class PeriodicIdentityReconciliationJob {
  constructor(
    private readonly options: {
      readonly repository: PeriodicIdentityReconciliationRepository;
      readonly provider: {
        inventory(): Promise<readonly ProviderIdentityObservation[]>;
      };
      readonly now?: () => Date;
    },
  ) {}
  async run(): Promise<{
    ...
    const allDrift = diffIdentityProjection(desired, observed),
    ...
    for (const item of desired) {
      const drift = allDrift.filter(
        (entry) => entry.identityId === item.identityId,
      );
      const result = await this.options.repository.recordExact({
        identityId: item.identityId,
        desiredVersion: item.desiredVersion,
        desiredHash: item.desiredHash,
        status: drift.length ? "drifted" : "in_sync",
        drift,
        observedAt,
      });
      if (result === "stale") stale++;
      else if (drift.length) drifted++;
      else inSync++;
    }
    for (const item of observed)
      if (!wantedIds.has(item.identityId)) {
        await this.options.repository.recordExtra({
          identityId: item.identityId,
          observedAt,
        });
        extra++;
...
export function providerKey(
  work: Pick<
    ...
    "identityId" | "desiredVersion" | "desiredHash"
  >,
  operation: string,
  coordinate = "self",
): string {
...
export function classifyIdentitySagaFailure(
  error: unknown,
): IdentitySagaFailureClass {
  ...
  if (
    ...
    code.includes("PROVIDER_REJECTED") ||
    code.includes("AUTH_REJECTED") ||
    ["23503", "23505", "23514", "22023"].includes(code)
  )
    return "permanent";
  return "transient";
}
...
export function identityRetryDelay(attemptNo: number): number {
  return [60_000, 5 * 60_000, 15 * 60_000, 60 * 60_000][
    Math.min(Math.max(attemptNo - 1, 0), 3)
  ]!;
}

function validateRelationship(
  work: DesiredIdentityProjection,
  organization: DesiredOrganizationProjection | undefined,
): asserts organization is DesiredOrganizationProjection {
  if (!organization) throw coded("IDENTITY_ORGANIZATION_MISSING");
  const relationshipAllowed =
    organization.purpose === "tenant_employer"
      ? work.membership.relationship === "employer" ||
        work.membership.relationship === "external_worker"
      : work.membership.relationship === "contact";
  if (!relationshipAllowed)
    throw coded("IDENTITY_ORGANIZATION_RELATIONSHIP_INVALID");
  if (
    work.membership.relationship === "employer" &&
    !work.membership.sourceRef.startsWith("employment:")
  )
    throw coded("IDENTITY_EMPLOYMENT_REFERENCE_INVALID");
  if (
    work.membership.relationship === "contact" &&
    !work.membership.sourceRef.startsWith("business_partner_contact:")
  )
    throw coded("IDENTITY_CONTACT_REFERENCE_INVALID");
  if (
    work.membership.relationship === "external_worker" &&
    !work.membership.sourceRef.startsWith("worker_engagement:")
  )
    throw coded("IDENTITY_WORKER_ENGAGEMENT_REFERENCE_INVALID");
}
function validateWork(work: IdentitySagaWork, claimTokenHash: string): void {
  if (
    work.claimTokenHash !== claimTokenHash ||
    work.fencingToken < 1 ||
    work.desiredVersion < 1 ||
    !/^[a-f0-9]{64}$/.test(work.desiredHash)
  )
    throw coded("IDENTITY_WORK_INVALID");
  if (
    !work.applications.length &&
    (work.status === "invited" || work.status === "active")
  )
    throw coded("IDENTITY_APPLICATIONS_INVALID");
}
function validateCallback(callback: ProviderIdentityCallback): void {
  if (
    ...
    callback.desiredVersion < 1 ||
    callback.providerSequence < 1 ||
    !/^[a-f0-9]{64}$/.test(callback.desiredHash)
  )
    throw coded("IDENTITY_CALLBACK_INVALID");
}
function coordinates(work: IdentitySagaWork): IdentitySagaCoordinate {
  return {
    attemptId: work.attemptId,
    identityId: work.identityId,
    desiredVersion: work.desiredVersion,
    ...
    claimTokenHash: work.claimTokenHash,
  };
}
function safeErrorCode(error: unknown): string {
  if (error && typeof error === "object" && "code" in error)
    return String((error as { code: unknown }).code).slice(0, 160);
  return error instanceof Error
    ? error.message.slice(0, 160)
    : "IDENTITY_SAGA_FAILED";
}
