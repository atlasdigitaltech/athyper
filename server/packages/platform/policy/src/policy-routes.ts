...
export function registerPolicyRoutes(application: Application, options: PolicyRouteOptions): void {
  application.post("/api/policy/evaluate", options.authenticate, async (request, response, next) => {
    try {
      const context = options.readContext(response);
      const authorization = await options.authorizer.authorize({ context, permissionCode: "policy.evaluate" });
      if (!authorization.allowed) { response.status(403).json({ error: "FORBIDDEN", message: "Policy evaluation is not permitted" }); return; }
      ...
      const decision = await options.policy.evaluate({ context, entityType: requiredText(value, "entityType"), ...(entityId ? { entityId } : {}), facts: object(value["facts"]), ...(policyDefinitionIds ? { policyDefinitionIds } : {}), ...(pipelineId ? { pipelineId } : {}) });
...
function object(value: unknown): Readonly<Record<string, unknown>> { if (!value || typeof value !== "object" || Array.isArray(value)) throw new PolicyRouteError(400, "INVALID_FACTS", "Policy facts must be an object"); return value as Readonly<Record<string, unknown>>; }
...
function requiredText(value: Record<string, unknown>, key: string): string { const result = optionalText(value, key); if (!result) throw new PolicyRouteError(400, "MISSING_FIELD", `${key} is required`); return result; }
function uuidArray(value: unknown): readonly string[] | undefined {
  if (value === undefined) return undefined;
  if (!Array.isArray(value) || value.length > 100 || value.some((entry) => typeof entry !== "string" || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(entry))) throw new PolicyRouteError(400, "INVALID_POLICY_IDS", "policyDefinitionIds must contain at most 100 UUIDs");
