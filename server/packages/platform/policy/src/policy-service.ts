...
async function evaluateInTransaction<Transaction>(options: PolicyServiceOptions<Transaction>, evaluator: JsonRuleEvaluator, request: PolicyEvaluationRequest, transaction: Transaction): Promise<PolicyDecision> {
  ...
  const definitions = await options.repository.findActive({ planeKey: request.context.planeKey, tenantId: request.context.tenantId, entityType: request.entityType, ...(request.policyDefinitionIds ? { policyDefinitionIds: request.policyDefinitionIds } : {}), effectiveOn }, transaction);
...
async function simulateInTransaction<Transaction>(options:PolicyServiceOptions<Transaction>,evaluator:JsonRuleEvaluator,request:PolicyEvaluationRequest,transaction:Transaction):Promise<PolicySimulation>{const effectiveOn=(options.now?.()??new Date()).toISOString().slice(0,10);const definitions=await options.repository.findActive({planeKey:request.context.planeKey,tenantId:request.context.tenantId,entityType:request.entityType,...(request.policyDefinitionIds?{policyDefinitionIds:request.policyDefinitionIds}:{}),effectiveOn},transaction);const evaluated=evaluateDefinitions(definitions,factsFor(request),evaluator);return{decision:evaluated.decision,trace:evaluated.trace,effectiveOn,audited:false};}
