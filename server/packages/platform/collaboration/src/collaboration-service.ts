...
const PERMISSION = {
  create: "collaboration.comment.create",
  edit: "collaboration.comment.edit",
  remove: "collaboration.comment.delete",
  react: "collaboration.comment.react",
  draft: "collaboration.draft.write",
  read: "collaboration.comment.read",
  flag: "collaboration.comment.flag",
} as const;
...
export function createCollaborationService<Transaction>(
  ...
  return {
    async create(command) {
      ...
      const key =
        prepared.idempotencyKey === undefined
          ? undefined
          : parseIdempotencyKey(prepared.idempotencyKey);
      if (key && !key.ok)
      ...
      if (key && !options.commandExecutions)
        throw new CollaborationError(
          503,
          "IDEMPOTENCY_UNAVAILABLE",
          "Command receipt storage is unavailable",
        );
      let replayed = false;
      const result = await transact(options, prepared.context, async (tx) => {
        let executionId: string | undefined;
        if (key?.ok && options.commandExecutions) {
          const { context, idempotencyKey: _key, ...input } = prepared;
          const receipt = await options.commandExecutions.begin(
            {
              tenantId: context.tenantId,
              commandCode: "collaboration.comment.create",
              idempotencyKey: key.value,
              requestFingerprint: fingerprintCommand({
                ...input,
          ...
          if (receipt.kind !== "started")
            throw new CollaborationError(
              ...
              "Idempotency key was reused or is still processing",
            );
      ...
      const result = await transact(options, prepared.context, async (tx) => {
        ...
        if (!comment)
          throw new CollaborationError(
            ...
            "Comment was not found, is moderated, or has changed",
          );
        await emit(
          options,
          prepared.context,
    ...
    async remove(input) {
      ...
      return transact(options, input.context, async (tx) => {
        const removed = await options.repository.softDelete(
    ...
    async putReaction(input) {
      ...
      return transact(options, input.context, async (tx) => {
        const inserted = await options.repository.putReaction(
          input.context.tenantId,
          input.commentId,
          input.context.principalId,
          input.code,
    ...
    async deleteReaction(input) {
      ...
      return transact(options, input.context, (tx) =>
        options.repository.deleteReaction(
          input.context.tenantId,
          input.commentId,
          input.context.principalId,
          input.code,
    ...
    async deleteDraft(input) {
      ...
      if (input.parentCommentId !== undefined && !uuid(input.parentCommentId))
        throw new CollaborationError(
          400,
          "INVALID_PARENT_ID",
          "Parent comment ID must be a UUID",
        );
      return transact(options, input.context, (tx) =>
        options.repository.deleteDraft(
          input.context.tenantId,
          input.context.principalId,
          input,
    ...
    async markRead(input) {
      ...
      validateCoordinate(input);
      const requestedReadAt = timestamp(input.readAt);
      const now = new Date().toISOString();
      const readAt =
        requestedReadAt && requestedReadAt < now ? requestedReadAt : now;
      await transact(options, input.context, (tx) =>
        options.repository.markRead(
          input.context.tenantId,
          input.context.principalId,
          input,
          readAt,
    ...
    async flag(input) {
      ...
      if (!/^[a-z][a-z0-9_.-]{1,62}$/.test(input.reasonCode))
        throw new CollaborationError(
          400,
          "INVALID_CODE",
          "Invalid flag reason code",
        );
      return transact(options, input.context, async (tx) => {
        const id = await options.repository.createFlag(
          input.context.tenantId,
          input.commentId,
          input.context.principalId,
          input.reasonCode,
        ...
        await options.moderation?.open(
          {
            context: input.context,
            commentFlagId: id,
            reviewerEvidence: {
              source: "collaboration.comment.flag",
        ...
        await simpleEvent(
          ...
          input.context,
          input.commentId,
          "collaboration.comment.flagged",
          tx,
          { flagId: id, reasonCode: input.reasonCode },
        );
        return id;
      });
    },
...
async function emit<Transaction>(
  options: CollaborationServiceOptions<Transaction>,
...
async function simpleEvent<Transaction>(
  options: CollaborationServiceOptions<Transaction>,
  context: VerifiedRequestContext,
  commentId: string,
  eventType: string,
  ...
  await options.audit.record(
    {
      eventCode: eventType,
      action: eventType.split(".").at(-1) ?? "change",
      outcome: "success",
...
async function resolveMentions(
  ...
  const ids = [...new Set(command.mentionedPrincipalIds ?? [])];
  if (ids.some((id) => !uuid(id)))
    throw new CollaborationError(
      400,
      "INVALID_MENTION",
      "Mention principals must be UUIDs",
    );
  const resolved = await principals.resolveActivePrincipals(
    command.context,
    ids,
    tx,
  );
  if (resolved.length !== ids.length)
    throw new CollaborationError(
      422,
      "MENTION_PRINCIPAL_NOT_FOUND",
      "Every mention must resolve in the plane-local principal directory",
    );
  return resolved;
}
...
function transact<T, R>(
  options: CollaborationServiceOptions<T>,
  context: VerifiedRequestContext,
  work: (tx: T) => Promise<R>,
): Promise<R> {
...
function validateCoordinate(value: {
  ...
  if (value.contextType !== undefined) validateCode(value.contextType);
  if (
    !/^[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)?$/.test(value.entityType) ||
    !value.entityId.trim() ||
    value.entityId.length > 512
  )
    throw new CollaborationError(
      400,
...
function validateBody(text: string): void {
  if (!text.trim() || text.length > 50_000)
    throw new CollaborationError(
      ...
      "Comment text must contain 1-50000 characters",
    );
}
function validateAttachments(ids?: readonly string[]): void {
  if ((ids?.length ?? 0) > 10 || ids?.some((id) => !uuid(id)))
    throw new CollaborationError(
      400,
      "INVALID_ATTACHMENTS",
      "At most 10 attachment UUIDs are allowed",
    );
}
...
>(command: T): T {
  if ((command.format ?? "plain") === "plain") {
    if (command.content !== undefined)
      throw new CollaborationError(
        ...
        "Plain comments cannot include rich-text JSON",
      );
    return {
      ...command,
  ...
  if (command.format !== "rich_json" || !command.content)
    throw new CollaborationError(
      400,
      "INVALID_RICH_TEXT",
  ...
  return {
    ...
    html: projected.html,
    contentSchema: projected.contentSchema,
    mentionedPrincipalIds: projected.mentionIds,
    attachmentIds: [
      ...new Set([
        ...(command.attachmentIds ?? []),
        ...projected.attachmentIds,
      ]),
    ],
  } as T;
}
