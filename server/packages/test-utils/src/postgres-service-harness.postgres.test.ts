...
import { loadPlaneRlsFixtures, loadPlaneTestDatabases, PostgresServiceHarness, postgresServiceTestsEnabled } from "./postgres-service-harness.js";
...
describe.skipIf(!enabled)("DDL-created plane databases", () => {
  ...
  for (const plane of ["studio", "neon", "mesh"] as const) {
    it(`${plane} verifies its physical plane, application role, request context, and RLS fixture`, async () => {
      ...
      await harness.withTenantRollback(plane, fixture, async (client, context) => {
        expect(context).toMatchObject({ plane, tenantId: fixture.tenantId, principalId: fixture.principalId });
        expect(context.requestId).toMatch(/^[0-9a-f-]{36}$/);
        expect(context.correlationId).toMatch(/^[0-9a-f-]{36}$/);
        const visible = await client.query<{ tenant_id: string; principal_id: string }>(
    ...
    it(`${plane} rejects a connection presented as the wrong physical plane before SQL work`, async () => {
      const database = loadPlaneTestDatabases().find((entry) => entry.plane === plane)!;
      const claimedPlane = plane === "studio" ? "neon" : "studio";
      const wrongPlaneHarness = new PostgresServiceHarness([{ ...database, plane: claimedPlane }]);
      try {
        await expect(wrongPlaneHarness.withTenantRollback(claimedPlane, fixtures[claimedPlane], async () => undefined))
