...
export class NoSkippedPostgresReporter implements Reporter {
  onTestRunEnd(modules: readonly TestModule[]): void {
    ...
    if (tests.length === 0)
      throw new Error("Required PostgreSQL qualification executed zero tests");
    ...
    if (skipped.length > 0) {
      ...
      throw new Error(
        `Required PostgreSQL qualification skipped ${skipped.length} integration test(s): ${names}`,
      );
    }
  }
}
