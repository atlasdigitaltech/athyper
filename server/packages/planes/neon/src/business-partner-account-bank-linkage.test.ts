...
describe("native bank verification controls",()=>{
 it("stores the full bank identifier only in protected storage",async()=>{let protectedBytes="",created:any;const repository={protectedRegistrationByKey:async()=>null,protectedRegistrationSource:async()=>({owner_type_id:"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"}),createProtectedRegistration:async(input:any)=>{created=input;return{bank_account_link_id:"bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb",account_last4:input.accountLast4,status:"pending_verification"};}};const value=createBusinessPartnerAccountBankLinkageService({repository:repository as any,authorizer:{async authorize(){return{allowed:true};}},transactions:{async run(_plane:any,_actor:any,work:any){return work({});}},secrets:{async resolve(){throw new Error("unused");},async put(_reference,value){protectedBytes=new TextDecoder().decode(value);return{reference:_reference,version:"1"};}}});const result=await value.registerProtectedBankAccount({context,businessPartnerId:"33333333-3333-4333-8333-333333333333",companyCodeId:"44444444-4444-4444-8444-444444444444",accountHolderName:"Example Supplier",accountIdentifier:"MY12 3456-7890",accountIdType:"iban",currencyCode:"MYR",bankName:"Example Bank",bankCountryCode:"MY",idempotencyKey:"protected-bank-001"});expect(result.registration).toMatchObject({account_last4:"7890"});expect(protectedBytes).toBe("MY1234567890");expect(created).not.toHaveProperty("accountIdentifier");expect(JSON.stringify(created)).not.toContain("MY1234567890");expect(created.protectedValueToken).toMatch(/^bank:/);});
