...
export interface ProfileProjectionResult { readonly eventId: string; readonly disposition: ProjectionDisposition; readonly processingDisposition?: ProjectionDisposition; readonly reasonCode?: string; readonly replayed: boolean; readonly projection?: ProfileProjectionHead; }
...
export interface BusinessPartnerProfileProjectionRepository {
  receiptByEventId(tenantId: string, eventId: string, tx: Tx): Promise<{ id: string; envelopeHash: string; processingDisposition?: ProjectionDisposition } | null>;
  ...
  processingReceipts(tenantId: string, limit: number, tx: Tx): Promise<readonly Readonly<Record<string, unknown>>[]>;
...
export class KyselyBusinessPartnerProfileProjectionRepository implements BusinessPartnerProfileProjectionRepository {
  async receiptByEventId(tenantId: string, eventId: string, tx: Tx) { const row = (await sql<Row>`SELECT inbox.id,inbox.envelope_hash,(SELECT disposition FROM control.mesh_business_partner_profile_processing_attempt WHERE tenant_id=inbox.tenant_id AND inbox_event_id=inbox.id ORDER BY attempt_no DESC LIMIT 1) processing_disposition FROM control.mesh_business_partner_profile_inbox inbox WHERE inbox.tenant_id=${tenantId}::uuid AND event_id=${eventId}::uuid FOR UPDATE`.execute(tx)).rows[0]; return row ? { id: String(row["id"]), envelopeHash: String(row["envelope_hash"]), processingDisposition: row["processing_disposition"] as ProjectionDisposition | undefined } : null; }
  ...
  async processingReceipts(tenantId: string, limit: number, tx: Tx) {
    return (await sql<Row>`SELECT inbox.event_id,inbox.network_relationship_id,inbox.publication_id,inbox.publication_version,inbox.lifecycle_version,attempt.attempt_no,attempt.trigger_kind,attempt.disposition,attempt.reason_code,attempt.processed_at
      FROM control.mesh_business_partner_profile_processing_attempt attempt
      JOIN control.mesh_business_partner_profile_inbox inbox ON inbox.tenant_id=attempt.tenant_id AND inbox.id=attempt.inbox_event_id
      WHERE attempt.tenant_id=${tenantId}::uuid
      ORDER BY attempt.processed_at DESC,inbox.event_id,attempt.attempt_no DESC LIMIT ${limit}`.execute(tx)).rows;
  }
...
export function createBusinessPartnerProfileProjectionService(options: { authorizer: Authorizer; repository: BusinessPartnerProfileProjectionRepository; transactions: ProfileProjectionTransactions }) {
    ...
    return options.transactions.run("neon", actor(context), async tx => {
      ...
      if (!inboxId) {
        ...
        if (existing) {
          ...
          return { eventId: parsed.eventId, disposition: "duplicate", reasonCode: "MESH_PROFILE_EVENT_DUPLICATE", processingDisposition: existing.processingDisposition ?? "quarantined", replayed: true };
        }
  ...
  return Object.freeze({
    ...
    async processingReceipts(input: { context: VerifiedRequestContext; limit?: number }) {
      context(input.context);
      const limit = bounded(input.limit);
      await authorize(options.authorizer, input.context, businessPartnerProfileProjectionPermissions.read);
      return options.transactions.run("neon", actor(input.context), tx => options.repository.processingReceipts(input.context.tenantId, limit, tx));
    },
...
function validate(envelope: MeshBusinessPartnerProfileEnvelope, parsed: ParsedEnvelope): { disposition: "quarantined"; reasonCode: string } | undefined { if (envelope.sourcePlane !== "mesh") return quarantine("MESH_PROFILE_SOURCE_PLANE_INVALID"); if (parsed.schemaCode !== "mesh.business_partner_profile" || ![1,2].includes(parsed.schemaVersion)) return quarantine("MESH_PROFILE_SCHEMA_UNSUPPORTED"); if (parsed.fieldSetCode !== `recipient_safe_v${parsed.schemaVersion}`) return quarantine("MESH_PROFILE_FIELD_SET_UNSUPPORTED"); if (parsed.eventType.endsWith("published")) { if (!envelope.payload) return quarantine("MESH_PROFILE_PAYLOAD_MISSING"); if (hash(envelope.payload) !== parsed.payloadHash) return quarantine("MESH_PROFILE_PAYLOAD_HASH_MISMATCH"); if (!safe(envelope.payload)) return quarantine("MESH_PROFILE_SENSITIVE_FIELD_REJECTED"); const recipient = object(object(envelope.payload)["recipient"]); if (recipient["tenantId"] !== envelope.recipientTenantId || recipient["networkAccountId"] !== envelope.recipientNetworkAccountId || recipient["networkRelationshipId"] !== envelope.networkRelationshipId) return quarantine("MESH_PROFILE_RECIPIENT_BINDING_MISMATCH"); if (parsed.lifecycleVersion !== 1) return quarantine("MESH_PROFILE_LIFECYCLE_INVALID"); } else if (envelope.payload) return quarantine("MESH_PROFILE_WITHDRAWAL_PAYLOAD_FORBIDDEN"); return undefined; }
