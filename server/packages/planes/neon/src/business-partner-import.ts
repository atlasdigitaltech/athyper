...
export function createNeonBusinessPartnerImportAdapter(): GovernedImportAdapter<Tx> {
  const adapter: GovernedImportAdapter<Tx> = {
    ...
    async validate({ row, rowNumber, session, scope }) {
      ...
      if (!constraint)
        errors.push(
          "IMPORT_SCOPE_INVALID:scope:The operating-organization scope is required",
        );
      requiredText(row, "code", errors);
      if (
        session.operation === "create" ||
        session.operation === "upsert" ||
        session.operation === "replace"
      )
        requiredText(row, "name", errors);
      optionalEnum(row, "partner_role", ["supplier", "customer"], errors);
      optionalEnum(
        row,
        "partner_category",
        ["organization", "government", "nonprofit", "internal"],
        errors,
      );
      ...
      optionalEnum(
        ...
        ["government", "nonprofit", "sole_proprietor"],
        errors,
      );
    ...
    async apply({ context, row, session, scope, transaction }) {
      ...
      const existing = (
        await sql<{
          id: string;
        }>`SELECT id FROM master.business_partner WHERE tenant_id=${context.tenantId}::uuid AND code=${code} FOR UPDATE`.execute(
          transaction,
        )
      ).rows[0];
      if (session.operation === "delete") {
        if (!existing)
          return conflict(
            session.conflictPolicy,
            `Business partner ${code} was not found`,
          );
        const ended =
          await sql`UPDATE master.business_partner_operating_organization_assignment
          ...
          WHERE tenant_id=${context.tenantId}::uuid AND business_partner_id=${existing.id}::uuid AND operating_organization_id=${organizationId}::uuid AND status IN ('draft','active')`.execute(
            transaction,
          );
        if (!Number(ended.numAffectedRows))
          return conflict(
            session.conflictPolicy,
            `Business partner ${code} is not assigned to the selected organization`,
          );
        return { outcome: "deleted" as const, recordId: existing.id };
      }
      if (existing && session.operation === "create")
        return conflict(
          session.conflictPolicy,
          `Business partner ${code} already exists`,
        );
      if (!existing && session.operation === "update")
        return conflict(
          session.conflictPolicy,
          `Business partner ${code} was not found`,
        );
      ...
      if (!existing && has(row, "aliases"))
        for (const alias of textArray(row, "aliases"))
          await sql`INSERT INTO master.business_partner_alias(tenant_id,business_partner_id,alias_name,created_by) VALUES(${context.tenantId}::uuid,${partnerId}::uuid,${alias},${context.principalId}::uuid)`.execute(
            transaction,
          );
      if (existing)
      ...
      if (role === "supplier")
        await sql`INSERT INTO master.supplier(tenant_id,business_partner_id,supplier_code,supplier_type,metadata,status,created_by)
        ...
        ON CONFLICT(tenant_id,business_partner_id) DO NOTHING`.execute(
          transaction,
        );
      if (role === "customer")
        await sql`INSERT INTO master.customer(tenant_id,business_partner_id,customer_code,customer_type,metadata,status,created_by)
...
function neonScope(scope: Scope) {
  return scope.constraints.find(
    (item) => item.kind === "neon.business_partner.operating_organization.v1",
  );
}
function requiredNeonScope(scope: Scope) {
  const value = neonScope(scope);
  if (
    !value ||
    value.kind !== "neon.business_partner.operating_organization.v1"
  )
    throw new Error("Neon import scope is unavailable");
  return value.operatingOrganizationId;
}
function conflict(policy: "reject" | "skip", message: string) {
  if (policy === "skip") return { outcome: "skipped" as const };
  throw Object.assign(new Error(message), {
    code: "IMPORT_CONFLICT",
    retryable: false as const,
  });
}
function has(row: Row, key: string) {
  return Object.prototype.hasOwnProperty.call(row, key);
}
function text(row: Row, key: string) {
  const value = row[key];
  if (typeof value !== "string" || !value.trim())
    throw new Error(`IMPORT_FIELD_REQUIRED:${key}`);
  return value.trim();
}
...
function textArray(row: Row, key: string): readonly string[] {
  const value = row[key];
  return Array.isArray(value)
    ? value.filter((item): item is string => typeof item === "string")
    : [];
}
function requiredText(row: Row, key: string, errors: string[]) {
  if (typeof row[key] !== "string" || !String(row[key]).trim())
    errors.push(`IMPORT_FIELD_REQUIRED:${key}:${key} is required`);
}
function optionalEnum(
  ...
  if (row[key] !== undefined && !allowed.includes(String(row[key])))
    errors.push(`IMPORT_FIELD_INVALID:${key}:${key} is invalid`);
}
function optionalJsonObject(row: Row, key: string, errors: string[]) {
  const value = row[key];
  if (
    value !== undefined &&
    (!value || typeof value !== "object" || Array.isArray(value))
  )
    errors.push(`IMPORT_FIELD_INVALID:${key}:${key} must be an object`);
}
function normalizePartnerStructure(row: Row) {
  const raw = nullableText(row, "partner_category") ?? "organization";
  const ownership =
    raw === "internal"
      ? "internal"
      : (nullableText(row, "ownership_class") ?? "external");
  const classification = ["government", "nonprofit"].includes(raw)
    ? raw
    : nullableText(row, "legal_classification");
