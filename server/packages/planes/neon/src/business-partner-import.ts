  ...
  const adapter: GovernedImportAdapter<Tx> = {
    ...
    async apply({ context, row, session, scope, transaction }) {
      ...
      const partnerId = existing?.id ?? (await sql<{ id: string }>`INSERT INTO master.business_partner(
          ...
          ${context.tenantId}::uuid,${code},${text(row,"name")},${nullableText(row,"display_name")},${nullableText(row,"legal_name")},${structural.category}::master.business_partner_category_d,${structural.ownership}::master.business_partner_ownership_d,${structural.classification??null}::master.business_partner_legal_classification_d,${structural.category==="person"?null:nullableText(row,"legal_form")},${structural.category==="person"?null:nullableText(row,"registration_country_code")},${structural.category==="person"?null:nullableText(row,"incorporation_date")}::date,${structural.category==="person"?null:nullableText(row,"website_url")},${nullableText(row,"description")},${JSON.stringify(object(row,"metadata"))}::jsonb,${nullableText(row,"status")??"draft"},${context.principalId}::uuid
        ) RETURNING id`.execute(transaction)).rows[0]!.id;
      if (!existing && has(row,"aliases")) for (const alias of textArray(row,"aliases")) await sql`INSERT INTO master.business_partner_alias(tenant_id,business_partner_id,alias_name,created_by) VALUES(${context.tenantId}::uuid,${partnerId}::uuid,${alias},${context.principalId}::uuid)`.execute(transaction);
