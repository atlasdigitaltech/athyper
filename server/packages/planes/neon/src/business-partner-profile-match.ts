...
export const acceptedMeshBusinessPartnerFieldPaths = Object.freeze(["partner.accountCode", "partner.legalName", "partner.legalForm", "partner.countryCode", "partner.incorporationDate", "partner.websiteUrl", "partner.description"] as const);
...
export interface ProfileMatchCandidate { readonly businessPartnerId: string; readonly code: string; readonly name: string; readonly legalForm?: string; readonly countryCode?: string; readonly incorporationDate?: string; readonly websiteUrl?: string; readonly description?: string; readonly score: number; readonly reasons: readonly string[]; readonly fingerprint: string; }
...
interface CandidateRow { id: string; code: string; name: string; legalForm?: string; countryCode?: string; incorporationDate?: string; websiteUrl?: string; description?: string; }
...
export class KyselyBusinessPartnerProfileMatchRepository implements BusinessPartnerProfileMatchRepository {
  ...
  async currentPartner(tenantId: string, businessPartnerId: string, operatingOrganizationId: string, tx: Tx) {
    const row = (await sql<Row>`SELECT bp.record_version,bp.name,bp.legal_form,bp.registration_country_code,bp.incorporation_date::text,bp.website_url,bp.description
    ...
    return row ? {version: Number(row["record_version"]), fields: {legalName: row["name"], legalForm: row["legal_form"], countryCode: row["registration_country_code"], incorporationDate: row["incorporation_date"], websiteUrl: row["website_url"], description: row["description"]}} : null;
  }
  ...
  async candidates(tenantId: string, operatingOrganizationId: string, tx: Tx) { const rows=(await sql<Row>`SELECT DISTINCT bp.id,bp.code,bp.name,bp.legal_form,bp.registration_country_code,bp.incorporation_date::text,bp.website_url,bp.description FROM master.business_partner bp JOIN master.business_partner_operating_organization_assignment a ON a.tenant_id=bp.tenant_id AND a.business_partner_id=bp.id WHERE bp.tenant_id=${tenantId}::uuid AND a.operating_organization_id=${operatingOrganizationId}::uuid AND bp.is_active AND a.is_active ORDER BY bp.code,bp.id LIMIT 500`.execute(tx)).rows; return rows.map(row=>({id:String(row["id"]),code:String(row["code"]),name:String(row["name"]),...optional(row,"legal_form","legalForm"),...optional(row,"registration_country_code","countryCode"),...optional(row,"incorporation_date","incorporationDate"),...optional(row,"website_url","websiteUrl"),...optional(row,"description","description")})); }
