...
export class KyselyMetadataGenerationEventStore {
  constructor(private readonly database:Kysely<Record<string,never>>,private readonly actorId:string){}
  async append(event:MetadataGenerationEvent):Promise<void>{await sql`SELECT publication.fn_emit_outbox(${event.tenantId}::uuid,'metadata.generation.advanced',${event.eventId},'metadata.entity_release',${event.releaseId}::uuid,${this.actorId}::uuid,NULL::uuid,${JSON.stringify(event)}::jsonb)`.execute(this.database);}
}
