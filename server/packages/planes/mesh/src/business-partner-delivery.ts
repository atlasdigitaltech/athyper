...
export const DELIVER_BUSINESS_PARTNER_EVENTS_JOB =
  "mesh.business-partner.deliver";
export const RECONCILE_BUSINESS_PARTNER_EVENTS_JOB =
  "mesh.business-partner.reconcile";
...
export interface BusinessPartnerRecipientTransport {
  deliver(item: BusinessPartnerDeliveryItem): Promise<{
...
export interface BusinessPartnerDeliveryTelemetry {
  record(labels: {
    readonly operation: "delivery" | "reconciliation";
    readonly kind: BusinessPartnerEventKind;
...
export class KyselyBusinessPartnerDeliveryRepository implements BusinessPartnerDeliveryRepository {
  ...
  reopen(item: BusinessPartnerDeliveryItem, reason: string) {
    return this.run(async (tx) => {
      await sql`UPDATE event.outbox SET status='failed',available_at=clock_timestamp(),processed_at=NULL,published_at=NULL,last_error=${reason},attempts=CASE WHEN attempts>=max_attempts THEN greatest(max_attempts-1,0) ELSE attempts END WHERE id=${item.outboxId}::uuid AND status='completed'`.execute(
        tx,
      );
    });
  }
...
export class BusinessPartnerDeliveryWorker {
  ...
  async deliver(limit = 100): Promise<Readonly<Record<string, number>>> {
    ...
    const items = await this.options.repository.claim({
    ...
    for (const item of items) {
      try {
        ...
        this.options.telemetry?.record({
          operation: "delivery",
          kind: item.kind,
          outcome: failure.permanent ? "dead_letter" : "retry",
        });
  ...
  async reconcile(limit = 250): Promise<Readonly<Record<string, number>>> {
    const summary: Record<string, number> = {};
    for (const item of await this.options.repository.reconciliationCandidates(
      bounded(limit, 250),
    )) {
      try {
        ...
        if (received) increment(summary, "in_sync");
          ...
          await this.options.repository.reopen(
            item,
            "RECIPIENT_RECEIPT_MISSING",
          );
          increment(summary, "reopened");
        }
        this.options.telemetry?.record({
          operation: "reconciliation",
          kind: item.kind,
          outcome: received ? "in_sync" : "reopened",
        });
      } catch (error) {
...
export function createBusinessPartnerDeliveryHandler(
  worker: BusinessPartnerDeliveryWorker,
  repository?: BusinessPartnerDeliveryRepository,
): JobHandler {
  return {
    async handle(job) {
      const data = job.data as Record<string, unknown>,
        replayOutboxId = data["replayOutboxId"],
        reason = data["reason"];
      if (replayOutboxId !== undefined) {
        if (
          !repository ||
          typeof replayOutboxId !== "string" ||
          !uuid(replayOutboxId) ||
          typeof reason !== "string" ||
          reason.trim().length < 8 ||
          reason.length > 1000
        )
          throw new Error("BUSINESS_PARTNER_REPLAY_REQUEST_INVALID");
        if (!(await repository.requestReplay(replayOutboxId, reason.trim())))
          return {
            status: "discarded",
            reason: "BUSINESS_PARTNER_DEAD_LETTER_NOT_FOUND",
          };
      }
      const limit = numeric(data["limit"], 100);
      return { status: "completed", output: await worker.deliver(limit) };
    },
  };
}
export function createBusinessPartnerReconciliationHandler(
  worker: BusinessPartnerDeliveryWorker,
): JobHandler {
  return {
    async handle(job) {
      const limit = numeric(
        (job.data as Record<string, unknown>)["limit"],
        250,
      );
      return { status: "completed", output: await worker.reconcile(limit) };
    },
  };
}
...
function classify(error: unknown, exhausted: boolean) {
  ...
  const code = String(value?.code ?? "BUSINESS_PARTNER_DELIVERY_FAILED").slice(
    0,
    120,
  );
  const message = String(
    value?.message ?? "Business Partner event delivery failed",
  ).slice(0, 2000);
  const permanent =
    exhausted ||
...
function retryAt(now: Date, attempt: number) {
  return new Date(
    now.valueOf() + Math.min(3_600_000, 5_000 * 2 ** Math.max(0, attempt - 1)),
  ).toISOString();
}
function increment(summary: Record<string, number>, key: string) {
  summary[key] = (summary[key] ?? 0) + 1;
}
function numeric(value: unknown, fallback: number) {
  return typeof value === "number" ? bounded(value, fallback) : fallback;
}
function bounded(value: number, max = 100) {
  if (!Number.isSafeInteger(value) || value < 1 || value > max)
    throw new Error(`BUSINESS_PARTNER_DELIVERY_LIMIT_INVALID:${max}`);
  return value;
}
function mapItem(row: Row): BusinessPartnerDeliveryItem {
  ...
  return {
    ...
    eventId,
    eventType,
    kind: eventType.startsWith("mesh.bank_account.") ? "bank" : "profile",
...
function object(value: unknown): Readonly<Record<string, unknown>> {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new Error("BUSINESS_PARTNER_DELIVERY_ENVELOPE_INVALID");
