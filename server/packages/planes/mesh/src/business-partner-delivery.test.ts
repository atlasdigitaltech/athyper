...
function harness(deliver: () => Promise<{ disposition: "applied" | "duplicate" | "stale" | "quarantined"; reasonCode?: string }>, received = true) {
  ...
  const recipient = { deliver: vi.fn(deliver), hasReceipt: vi.fn(async () => received) };
  return { repository, recipient, worker: new BusinessPartnerDeliveryWorker({ workerId: "worker-1", repository, recipient, now: () => new Date("2026-08-28T00:00:00Z") }) };
}
