...
function validateCommon(sourceType: string, performedAt: string) { if (!/^[a-z][a-z0-9_.-]{1,126}$/.test(sourceType) || !Number.isFinite(Date.parse(performedAt))) throw new FinanceContractError("FINANCE_INVALID_COMMAND", "Inventory source or performedAt is invalid"); }
