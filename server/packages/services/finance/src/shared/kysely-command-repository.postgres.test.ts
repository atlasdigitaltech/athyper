...
describe.skipIf(!enabled)("finance durable PostgreSQL idempotency", () => {
  ...
  it("serializes same-key retries, replays the original result, and conflicts on changed input", async () => {
    ...
    const original = command(actor, randomUUID(), commandCode, key, {
      amount: "10.0000",
    });
    let applications = 0;
    const execute = <T extends Readonly<Record<string, unknown>>>(
      value: FinanceCommand<T>,
    ) =>
      database.transaction().execute(async (tx) => {
        await context(actor, tx);
        return repository.execute(value, tx, async () => {
          applications += 1;
          return {
            resourceId: original.commandId,
            version: 7,
            output: { amount: "10.0000" },
          };
        });
      });
    {
      const [left, right] = await Promise.all([
        ...
        execute({
          ...
          actor: { ...actor, correlationId: randomUUID() },
        }),
      ]);
      expect([left.kind, right.kind].sort()).toEqual(["applied", "replayed"]);
      const applied = [left, right].find(
        (result) => result.kind === "applied",
      )!;
      const replayed = [left, right].find(
        (result) => result.kind === "replayed",
      )!;
      ...
      const changed = command(actor, randomUUID(), commandCode, key, {
        amount: "11.0000",
      });
      await expect(execute(changed)).resolves.toMatchObject({
        kind: "idempotency_conflict",
        existingCommandId: applied.commandId,
      });
      expect(applications).toBe(1);
    }
    ...
    await expect(
      database.transaction().execute(async (tx) => {
        await context(actor, tx);
        await sql`DELETE FROM event.command_execution WHERE tenant_id=${actor.tenantId}::uuid AND command_code=${commandCode}`.execute(
          tx,
        );
      }),
  ...
  it("rolls back a failed application and permits a clean retry of the same key", async () => {
    ...
    await expect(
      database.transaction().execute(async (tx) => {
        await context(actor, tx);
        return repository.execute(original, tx, async () => {
          throw new Error("controlled finance application failure");
        });
      }),
    ...
    await database.transaction().execute(async (tx) => {
      await context(actor, tx);
      const rows =
        await sql`SELECT 1 FROM event.command_execution WHERE id=${original.commandId}::uuid`.execute(
          tx,
        );
      expect(rows.rows).toHaveLength(0);
      expect(
        await repository.execute(original, tx, async () => ({
          resourceId: original.commandId,
          version: 1,
          output: { amount: "5.0000" },
        })),
...
function command<Payload extends Readonly<Record<string, unknown>>>(
  actor: FinanceActor,
  commandId: string,
  commandCode: string,
  idempotencyKey: string,
  payload: Payload,
): FinanceCommand<Payload> {
  return {
    actor,
    commandId,
    commandCode,
