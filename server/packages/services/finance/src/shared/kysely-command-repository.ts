...
export class KyselyFinanceCommandRepository implements FinanceCommandRepository<
...
> {
  ...
  >(
    ...
    apply: (transaction: Transaction<Database>) => Promise<{
    ...
    const row =
      inserted.rows[0] ??
      (
        await sql<Row>`SELECT * FROM event.command_execution WHERE tenant_id=${command.actor.tenantId}::uuid AND command_code=${command.commandCode} AND idempotency_key=${command.idempotencyKey} FOR UPDATE`.execute(
          transaction,
        )
      ).rows[0];
    if (!row) throw new Error("FINANCE_COMMAND_RESERVATION_FAILED");
    const existingCommandId = String(row["id"]);
    if (String(row["request_fingerprint"]) !== command.requestFingerprint)
      return {
    ...
    await sql`UPDATE event.command_execution SET status='processing',started_at=${startedAt}::timestamptz,status_changed_at=${startedAt}::timestamptz,status_changed_by=${command.actor.principalId}::uuid,updated_at=${startedAt}::timestamptz,updated_by=${command.actor.principalId}::uuid WHERE id=${command.commandId}::uuid`.execute(
      transaction,
    );
...
function replay<Output extends Readonly<Record<string, unknown>>>(
  row: Row,
  requestedCommandId: string,
): FinanceCommandResult<Output> {
  if (row["status"] !== "succeeded")
    throw Object.assign(new Error("FINANCE_COMMAND_IN_PROGRESS"), {
      retryable: true,
  ...
  return {
    kind: "replayed",
    commandId: String(row["id"] ?? requestedCommandId),
