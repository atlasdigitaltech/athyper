...
import { loadCompanyFxSummary } from "./finance-fx-company-summary.service.js";
...
import { allowedCapability, type FxPermissions } from "./finance-fx-contracts.js";
...
export interface DomainReadiness {
  ...
  blockerCount: number;
  warningCount: number;
  checks: Array<{ code: string; label: string; passed: boolean; classification:"setup" }>;
  operationalHealth?:{
...
const FX_CERTIFICATION_PERMISSIONS:FxPermissions={
  ...
  const state: DomainReadinessState = passed === checks.length
    ...
    : passed === 0
      ...
      : blockerCount > 0
        ? "blocked"
        : "in_progress";
  return { domain: key, label, state, passed, total: checks.length, blockerCount, warningCount: warnings,
    checks:checks.map(check=>({...check,classification:"setup" as const})),
...
export async function loadCompanyCertificationReadiness(
  ...
  const [fx,tax,payments,banking,{ rows: evidenceRows }] = await Promise.all([
    loadCompanyFxSummary(db,{
      tenantId,companyCode,asOfDate,permissions:FX_CERTIFICATION_PERMISSIONS,
    }),
  ...
  const fxDomain=domain("currency_fx","Currency & FX",companyCode,[
    {
      code:"fx_policy_effective",
      label:"Effective tenant or Company FX policy",
      passed:!fx.exposure.hasForeignCurrencyExposure||Boolean(fx.policy.effective),
    },
    {
      code:"fx_setup_complete",
      label:"Deterministic Company FX setup status",
      passed:fx.setupStatus.complete,
    },
    {
      code:"fx_posting_accounts",
      label:"Required FX gain/loss posting accounts resolve",
      passed:fx.postingAccounts.ready,
    },
  ],"/finance/setup/company/:company/currency-fx",fx.operationalHealth.attentionCount);
  fxDomain.operationalHealth={
    state:fx.operationalHealth.state,
    attentionCount:fx.operationalHealth.attentionCount,
    affectsCertification:false,
  };
