 ...
 *
 * Fallback (no user-scope ACL rows): when no principal/group scope grants exist,
...
const USER_SCOPE_ENTITY_TYPES: ScopeEntityType[] = ["principal", "auth_group", "auth_group_role", "team"];
...
export class CompanyCodeScopeService {
  ...
  async resolveScope(
    ...
    // Business ACL rows such as supplier/customer access must not disable RBAC
    // fallback for user company-code scope.
    const anyUserScopeGrant = await this.db
      .selectFrom("master.company_code_access as cca" as never)
      .select("cca.id" as never)
      .where("cca.tenant_id" as never, "=", tenantId as never)
      .where("cca.entity_type" as never, "in" as never, USER_SCOPE_ENTITY_TYPES as never)
      .limit(1)
      .executeTakeFirst() as { id: string } | undefined;

    if (!anyUserScopeGrant) {
