import type { JobEnvelope, JobExecutionCoordinate, JobExecutionFailure, JobExecutionIdentity, JobExecutionLifecycle, JobExecutionResult, JobPayload, JobPayloadSchema, JobSubject } from "@athyper/server-contract-jobs";
...
export interface JobExecutionStore {
  ...
  recordCompleted(job: JobEnvelope, result: JobExecutionResult | void): Promise<void>;
...
export function createJobExecutionLifecycle(options: JobExecutionLifecycleOptions): JobExecutionLifecycle {
  const requireGoverned = options.requireGovernedCoordinates ?? false;
  return {
    async enqueued(input) {
      if (!input.execution) {
        if (requireGoverned) throw new Error(`Job ${input.queue}/${input.name} is not governed`);
        return;
      }
      return options.store.recordEnqueued({
        ...
        ...(input.execution.tenantId ? { tenantId: input.execution.tenantId } : {}),
    ...
    async started(job) {
      ...
      else if (requireGoverned) throw new Error(`Job ${job.queue}/${job.name} is not governed`);
    },
