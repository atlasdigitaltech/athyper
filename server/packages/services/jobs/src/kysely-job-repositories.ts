  ...
  return {
    async recordEnqueued(input) {
      return withCoordinate(
        ...
        async (transaction) => {
          const persistedKey = await resolvePersistedExecutionKey(
            transaction,
            input.tenantId,
            input.queue,
            input.jobId,
          ...
          return {
            executionId: requiredRow(result.rows[0]).id,
            executionKey: persistedKey,
          };
        },
      );
    },
    ...
    async recordCompleted(job, result) {
      const coordinate = requiredCoordinate(job);
      await withCoordinate(transactions, coordinate, (transaction) =>
        recordTerminal(
          transaction,
          job,
          coordinate,
          ...
          return {
            ...
            ...(metadata["subject"]
              ? {
                  subject: objectValue(
                    metadata["subject"],
                  ) as unknown as JobSubject,
                }
              : {}),
            ...(metadata["payloadSchema"]
              ? {
                  payloadSchema: objectValue(
                    metadata["payloadSchema"],
                  ) as unknown as JobPayloadSchema,
                }
              : {}),
          };
        },
...
export function createKyselyJobGovernanceStore(
  transactions: JobTransactionCoordinator,
): JobGovernanceStore {
  return {
    ...
    async listSchedules(execution) {
      return withCoordinate(transactions, execution, async (transaction) => {
        const result = await sql<Record<string, unknown>>`
          ...
          FROM control.cron_schedule WHERE tenant_id IS NOT DISTINCT FROM ${execution.tenantId ?? null}::uuid ORDER BY code`.execute(
          transaction,
        );
    ...
    async createSchedule(input) {
      return withCoordinate(
        ...
        async (transaction) => {
          ...
          RETURNING id,code,name,handler_type,cron_expression,timezone,target_queue,payload_template,is_enabled,next_run_at`.execute(
            transaction,
          );
      ...
      await withCoordinate(
        ...
        async (transaction) => {
          const result = await sql<{
            id: string;
          }>`UPDATE control.cron_schedule SET is_enabled=false,updated_at=now(),updated_by=${input.execution.principalId}::uuid
          WHERE id=${input.scheduleId}::uuid AND tenant_id IS NOT DISTINCT FROM ${input.execution.tenantId ?? null}::uuid AND is_enabled RETURNING id`.execute(
            transaction,
          );
          const row = result.rows[0];
          if (!row) {
            const existing = await sql<{
              id: string;
            }>`SELECT id FROM control.cron_schedule
            WHERE id=${input.scheduleId}::uuid AND tenant_id IS NOT DISTINCT FROM ${input.execution.tenantId ?? null}::uuid`.execute(
              transaction,
            );
    ...
    async listScheduleAudit(input) {
      return withCoordinate(
        ...
        async (transaction) => {
          ...
          >`SELECT action,reason,changed_at,changed_by FROM control.cron_schedule_change_log
          WHERE tenant_id IS NOT DISTINCT FROM ${input.execution.tenantId ?? null}::uuid AND schedule_id=${input.scheduleId}::uuid ORDER BY changed_at DESC`.execute(
            transaction,
          );
          return result.rows.map((row) => ({
            action: stringValue(row["action"], "action") as
              "created" | "updated" | "deactivated",
            reason: stringValue(row["reason"], "reason"),
            changedAt: dateTime(row["changed_at"]),
            changedBy: stringValue(row["changed_by"], "changed_by"),
          }));
        },
      );
    },
...
function scheduleRow(row: Record<string, unknown>) {
  return {
    ...
    handlerType: stringValue(row["handler_type"], "handler_type"),
    cronExpression: stringValue(row["cron_expression"], "cron_expression"),
    timezone: stringValue(row["timezone"], "timezone"),
    targetQueue: stringValue(row["target_queue"], "target_queue"),
    payloadTemplate: objectValue(row["payload_template"]),
    enabled: Boolean(row["is_enabled"]),
    ...(row["next_run_at"] ? { nextRunAt: dateTime(row["next_run_at"]) } : {}),
  };
}
async function recordScheduleChange(
  ...
  await sql`INSERT INTO control.cron_schedule_change_log (tenant_id,schedule_id,action,reason,changed_by) VALUES (${execution.tenantId ?? null}::uuid,${scheduleId}::uuid,${action},${reason.trim().slice(0, 1000)},${execution.principalId}::uuid)`.execute(
    transaction,
  );
}
...
async function recordTerminal(
  ...
  const updated = await sql<{
    ...
    RETURNING id,COALESCE(started_at,created_at) AS started_at,duration_ms`.execute(
    transaction,
  );
...
async function withCoordinate<T>(
  ...
  if (
    !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(
      coordinate.principalId,
    )
  ) {
    throw new TypeError("Job principalId must be a UUID");
  }
  if (coordinate.scope === "tenant") {
    if (!coordinate.tenantId)
      throw new Error("Tenant-scoped job is missing tenantId");
    return transactions.runTenant(
      coordinate.planeKey,
      {
...
function failureStatus(job: JobEnvelope, failure: JobExecutionFailure) {
  ...
  if (
    ...
    (failure.disposition === "retryable" || failure.disposition === "timed_out")
  )
...
function requiredCoordinate(job: JobEnvelope): JobExecutionCoordinate {
  if (!job.execution)
    throw new Error(
      `Job ${job.queue}/${job.name} is missing execution coordinates`,
    );
...
function jobCode(
  ...
  const candidate = (schema?.name ?? `${queue}.${name}`)
    .toLowerCase()
    .replace(/[^a-z0-9_.-]+/g, "-");
...
function resultPayload(
  ...
  return result.status === "completed"
    ? { ...(result.output ?? {}) }
    : { discarded: true, reason: result.reason };
}
...
function stringValue(value: unknown, field: string): string {
  if (typeof value !== "string" || !value.trim())
    throw new Error(`Invalid cron schedule ${field}`);
  return value;
}
...
function dateTime(value: unknown): string {
  ...
  if (Number.isNaN(parsed.valueOf()))
    throw new Error("Invalid job execution timestamp");
...
function scheduleWriteError(error: unknown): never {
  if (
    ...
    error.code === "23505" &&
    "constraint" in error &&
    error.constraint === "cron_schedule_code_uq"
  ) {
...
async function resolvePersistedExecutionKey(
  transaction: JobTransaction,
  ...
  }>`SELECT execution_key FROM ops.job_execution
