...
import {
  ...
  type AtlasToolInvocationRecoveryMetrics,
...
export interface JobsServiceOptions {
  ...
  /** Milliseconds between Atlas conversation purge sweeps (default: 3 600 000) */
  atlasConversationPurgeSweepMs?: number;
  /** Milliseconds between Atlas stale tool invocation recovery sweeps (default: 300 000) */
  atlasToolInvocationRecoverySweepMs?: number;
}
...
export interface JobsServiceDeps {
  ...
  /**
   * Separately-authorized Atlas retention maintenance boundary. The scheduler
   * and worker remain inactive when absent; the normal application DB must
   * never be substituted for this dependency.
   */
  atlasConversationPurge?: {
    service: AtlasConversationPurgeService;
    batchSize: number;
    metrics?: import("./workers/atlas-conversation-purge.worker.js").AtlasConversationPurgeMetrics;
  };
  /**
   * Separately-authorized Atlas read-only tool recovery boundary. This is
   * supplied only when both Atlas persistence and governed tools are enabled.
   */
...
const QUEUE_NAME_TO_KEY: Record<string, keyof JobsQueues> = {
  ...
  [QUEUE_NAME.ATLAS_CONVERSATION_PURGE]: "atlasConversationPurge",
...
export interface JobsQueues {
  ...
  staleLock:       Queue;
  atlasConversationPurge: Queue<AtlasConversationPurgeJobData>;
  atlasToolInvocationRecovery:
    Queue<AtlasToolInvocationRecoveryJobData>;
}
...
export function createJobsService(deps: JobsServiceDeps): JobsService {
  ...
  const {
    ...
    atlasConversationPurgeSweepMs =
      DEFAULT_INTERVALS.ATLAS_CONVERSATION_PURGE_MS,
    atlasToolInvocationRecoverySweepMs =
      DEFAULT_INTERVALS.ATLAS_TOOL_INVOCATION_RECOVERY_MS,
  } = options;
  ...
  const queues: JobsQueues = {
    ...
    atlasConversationPurge:
      new Queue(QUEUE_NAME.ATLAS_CONVERSATION_PURGE, { connection: conn }),
    atlasToolInvocationRecovery:
      new Queue(QUEUE_NAME.ATLAS_TOOL_INVOCATION_RECOVERY, {
        connection: conn,
      }),
  };
  ...
  const workers: Worker[] = workersEnabled ? [
    ...
    ...(atlasConversationPurge
      ? [createAtlasConversationPurgeWorker({
          connection: conn,
          service: atlasConversationPurge.service,
          batchSize: atlasConversationPurge.batchSize,
          logger,
          metrics: atlasConversationPurge.metrics,
        })]
      : []),
    ...(atlasToolInvocationRecovery
      ? [createAtlasToolInvocationRecoveryWorker({
          connection: conn,
          service: atlasToolInvocationRecovery.service,
          options: {
            batchSize: atlasToolInvocationRecovery.batchSize,
            staleRunTimeoutMs:
              atlasToolInvocationRecovery.staleRunTimeoutMs,
          },
          logger,
          metrics: atlasToolInvocationRecovery.metrics,
        })]
      : []),
  ] : [];
  ...
  return {
    ...
    async start(): Promise<void> {
      ...
      // Atlas retention uses a dedicated privileged maintenance authority.
      // Remove a scheduler left by an older deployment whenever that authority
      // is unavailable so queued work cannot accumulate without a safe worker.
      if (atlasConversationPurge) {
        await queues.atlasConversationPurge.upsertJobScheduler(
          SCHEDULER_ID.ATLAS_CONVERSATION_PURGE,
          { every: atlasConversationPurgeSweepMs },
          {
            name: JOB_NAME.ATLAS_CONVERSATION_PURGE,
            data: {} satisfies AtlasConversationPurgeJobData,
          },
        );
        logger?.info("atlas_conversation_purge_scheduler_active", {
          queue: QUEUE_NAME.ATLAS_CONVERSATION_PURGE,
          intervalMs: atlasConversationPurgeSweepMs,
          batchSize: atlasConversationPurge.batchSize,
          workerLocal: workersEnabled,
        });
      } else {
        await queues.atlasConversationPurge.removeJobScheduler(
          SCHEDULER_ID.ATLAS_CONVERSATION_PURGE,
        );
        logger?.info("atlas_conversation_purge_scheduler_inactive", {
          reason: "maintenance_authority_unavailable",
        });
      }
      ...
      // Tool recovery is default-off and exists only when bootstrap has
      // attested the dedicated admin authority with both persistence and tools
      // enabled. Remove stale schedulers whenever any prerequisite disappears.
      if (atlasToolInvocationRecovery) {
        await queues.atlasToolInvocationRecovery.upsertJobScheduler(
          SCHEDULER_ID.ATLAS_TOOL_INVOCATION_RECOVERY,
          { every: atlasToolInvocationRecoverySweepMs },
          {
            name: JOB_NAME.ATLAS_TOOL_INVOCATION_RECOVERY,
            data: {} satisfies AtlasToolInvocationRecoveryJobData,
          },
        );
        logger?.info("atlas_tool_invocation_recovery_scheduler_active", {
          queue: QUEUE_NAME.ATLAS_TOOL_INVOCATION_RECOVERY,
          intervalMs: atlasToolInvocationRecoverySweepMs,
          batchSize: atlasToolInvocationRecovery.batchSize,
          staleRunTimeoutMs:
            atlasToolInvocationRecovery.staleRunTimeoutMs,
          workerLocal: workersEnabled,
        ...
        await queues.atlasToolInvocationRecovery.removeJobScheduler(
          SCHEDULER_ID.ATLAS_TOOL_INVOCATION_RECOVERY,
        );
        logger?.info("atlas_tool_invocation_recovery_scheduler_inactive", {
          reason: "tools_persistence_or_maintenance_authority_unavailable",
        });
      }
      ...
      logger?.info("jobs_service_started", {
        ...
        atlasConversationPurgeEnabled: atlasConversationPurge != null,
        atlasConversationPurgeSweepMs,
        atlasToolInvocationRecoveryEnabled:
          atlasToolInvocationRecovery != null,
        atlasToolInvocationRecoverySweepMs,
        tikaExtractEnabled,
        tikaExtractSweepMs,
        workersEnabled,
