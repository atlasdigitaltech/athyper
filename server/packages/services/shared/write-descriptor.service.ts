...
export async function getWriteDescriptor(
  ...
  const rows = await sql<EntityFieldRow>`
    SELECT ef.name,
           ef.column_name,
           ef.data_type,
           ef.origin,
  ...
  for (const row of rows.rows) {
    const editabilityJson = normalizeJson(row.editability);
    const spec: FieldSpec = {
      ...
      isWriteOnce: row.is_write_once,
      editability: editabilityJson,
    };
    fieldsByName.set(row.name, spec);

    const systemFlagged =
      row.is_read_only || row.is_computed || row.origin === "system";

    if (systemFlagged) {
      systemManaged.add(row.name);
      systemManagedOnUpdate.add(row.name);
    } else {
      writable.add(row.name);
      if (row.is_write_once) {
        // write-once: settable on create, locked after.
        systemManagedOnUpdate.add(row.name);
      }
    }

    // editability.editable_in_status maps a field to the set of statuses in
    // which it may be edited via PATCH. When present with an empty array,
    // the field is locked (never editable through PATCH).
    const statuses = extractEditableStatuses(editabilityJson);
    if (statuses === null) {
      alwaysEditable.add(row.name);
    } else {
      for (const status of statuses) {
        let bucket = editableInStatus.get(status);
        if (!bucket) {
          bucket = new Set();
    ...
    // Required derivation: is_required=true AND not system-managed on create
    // AND editable in the initial 'draft' status (or always editable).
    if (row.is_required && !systemFlagged) {
      const draftEditable =
        alwaysEditable.has(row.name) ||
        (editableInStatus.get("draft")?.has(row.name) ?? false) ||
        statuses === null ||
  ...
  const descriptor: WriteDescriptor = {
    entityCode,
    entityVersionId:       rows.rows[0]!.entity_version_id,
    fieldsByName,
    writable,
    systemManaged,
  ...
  CACHE.set(`${entityCode}:${descriptor.entityVersionId}`, descriptor);
  return descriptor;
}
