...
import { executeLifecycleTransition } from "../lifecycle/execute-lifecycle-transition.js";
import {
      ...
      SELECT
        ...
        (SELECT count(*) FROM audit.audit_log a
          WHERE a.tenant_id = r.tenant_id
            AND a.entity_id IN (c.id, r.id, pi.id, pe.id)
            AND a.context->>'domain' IS NOT NULL)::text AS activity_count,
