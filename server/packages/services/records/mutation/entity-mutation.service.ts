...
function applyTenantScope<TQuery>(query: TQuery, tenantColumn: string | null, tenantId: string): TQuery {
  return tenantColumn
    ? (query as any).where(tenantColumn, "=", tenantId) as TQuery
    : query;
}
...
export class DefaultEntityMutationService implements EntityMutationService {
  ...
  private async createMeasured(command: CreateEntityCommand): Promise<MutationResult> {
    ...
    const primaryField = [...target.descriptor.fields.values()].find((field) => field.column === primaryKey);
    // UUID keys are generated by the mutation kernel. Other scalar keys stay
    // database-generated or must be supplied by the caller; never write a UUID
    // into an integer/code primary key merely because the old route assumed id.
    if (mapped[primaryKey] === undefined && primaryField?.dataType === "uuid") mapped[primaryKey] = recordId;
    if (tenantColumn) mapped[tenantColumn] = command.context.tenantId;
  ...
  private async patchMeasured(command: PatchEntityCommand): Promise<MutationResult> {
    ...
    const result = await executeDurableMutationTransaction(this.deps.db, async (trx) => {
      ...
      const table = fullTable(target);
      const current = await applyTenantScope(
        (trx.selectFrom(table) as any)
          .selectAll()
      ...
      const updateQuery = applyTenantScope(
        (trx.updateTable(table) as any)
          .set(mapped)
          .where(primaryKey, "=", command.recordId),
        tenantColumn,
        command.context.tenantId,
  ...
  private async deleteMeasured(command: DeleteEntityCommand): Promise<MutationResult> {
    ...
    return executeDurableMutationTransaction(this.deps.db, async (trx) => {
      ...
      const table = fullTable(target);
      const current = await applyTenantScope(
        (trx.selectFrom(table) as any).selectAll().where(primaryKey, "=", command.recordId),
        tenantColumn,
        command.context.tenantId,
      )
  ...
  private async invokeRegistered(
    ...
    const result = await executeDurableMutationTransaction(this.deps.db, async (trx) => {
      ...
      if (replay) return replay;
      const current = await applyTenantScope(
        (trx.selectFrom(fullTable(target)) as any).selectAll().where(primaryKey, "=", command.recordId),
        tenantColumn,
        command.context.tenantId,
      )
        .forUpdate().executeTakeFirst() as Record<string, unknown> | undefined;
  ...
  private async resolveTarget(context: VerifiedRequestContext, entityCode: string): Promise<CompiledMutationTarget | null> {
    ...
    try {
      ...
      await Promise.all([...descriptor.relations.values()].map(async (relation) => {
        ...
        if (!child.descriptor.storage.tenantColumn) {
          throw new Error(`Relation target '${relation.targetEntity}' is global and cannot be mutated through a tenant-scoped relation.`);
        }
...
async function applyDeletionStrategy(
  ...
  if (target.manifest.deletionMode === "hard_delete") {
    await applyTenantScope(
      (trx.deleteFrom(table) as any).where(primaryKey, "=", command.recordId),
      tenantColumn,
      command.context.tenantId,
    ).executeTakeFirst();
    return;
  ...
  await applyTenantScope(
    (trx.updateTable(table) as any).set(values).where(primaryKey, "=", command.recordId),
    tenantColumn,
    command.context.tenantId,
  )
...
function committed(
  ...
  const versionColumn = target?.descriptor.storage.rowVersionColumn ?? "row_version";
  const publicRecord = target
    ? exposeMutationIdentityAliases(record, target.descriptor.storage.primaryKey, target.descriptor.storage.tenantColumn)
    : record;
  return {
    kind: "Committed",
    action,
    ...
    recordId,
    record: publicRecord,
    version: typeof publicRecord[versionColumn] === "number" ? publicRecord[versionColumn] : undefined,
    replayed,
    durableSideEffects: true,
  };
}

function exposeMutationIdentityAliases(
  record: Record<string, unknown>,
  primaryKey: string,
