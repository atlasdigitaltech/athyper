...
export function createRecordsRoute(router: Router, deps: RecordsRouteDeps): Router {
  ...
  // Once the production execution-descriptor provider is registered, list
  // and detail reads always enter the descriptor query kernel. query_v1 is
  // retained as a response compatibility marker only; it is no longer an
  // authority switch that can fall back to governed control-table behavior.
  const shouldUseEntityQuery = (_req: Request, _entityCode: string): boolean =>
