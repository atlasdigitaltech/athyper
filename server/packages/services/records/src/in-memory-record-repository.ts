...
function relativeDateMatches(actual: unknown, relative: unknown): boolean { const timestamp = new Date(String(actual ?? "")); if (Number.isNaN(timestamp.valueOf())) return false; const now = new Date(), start = new Date(now.getFullYear(), now.getMonth(), now.getDate()), day = 86_400_000; const ranges: Readonly<Record<string, readonly [number, number]>> = { today: [start.valueOf(), start.valueOf() + day], yesterday: [start.valueOf() - day, start.valueOf()], tomorrow: [start.valueOf() + day, start.valueOf() + 2 * day], last_7_days: [start.valueOf() - 7 * day, start.valueOf() + day], last_30_days: [start.valueOf() - 30 * day, start.valueOf() + day], next_7_days: [start.valueOf(), start.valueOf() + 8 * day], next_30_days: [start.valueOf(), start.valueOf() + 31 * day] }; if (relative === "this_week") { const week = new Date(start); week.setDate(start.getDate() - ((start.getDay() + 6) % 7)); return timestamp >= week && timestamp < new Date(week.valueOf() + 7 * day); } if (relative === "this_month") { const month = new Date(start.getFullYear(), start.getMonth(), 1), next = new Date(start.getFullYear(), start.getMonth() + 1, 1); return timestamp >= month && timestamp < next; } const range = typeof relative === "string" ? ranges[relative] : undefined; return Boolean(range && timestamp.valueOf() >= range[0] && timestamp.valueOf() < range[1]); }
