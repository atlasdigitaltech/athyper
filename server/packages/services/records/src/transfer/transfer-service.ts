...
export interface ImportStagingStore<Transaction = unknown> {
  ...
  restartImport?(tenantId: string, sessionId: string, transaction: Transaction): Promise<"restarted" | "invalid_state" | "not_found">;
  failQueuedImport?(tenantId: string, sessionId: string, errorCode: string, errorDetail: string, failedAt: string, transaction: Transaction): Promise<boolean>;
  ...
  restartExport?(tenantId: string, exportRequestId: string, transaction: Transaction): Promise<"restarted" | "invalid_state" | "not_found">;
  failQueuedExport?(tenantId: string, exportRequestId: string, errorCode: string, errorDetail: string, failedAt: string, transaction: Transaction): Promise<boolean>;
  getExport?(tenantId:string,exportRequestId:string,transaction:Transaction):Promise<{entityCode:string;actorPrincipalId:string;status:"queued"|"running"|"completed"|"cancelled"|"failed";exactFilter?:Readonly<Record<string,unknown>>;artifactKey?:string;rowCount?:number}|null>;
...
export interface ImportWorkbookIntake {
  prepareUpload(input:{readonly tenantId:string;readonly sessionId:string;readonly fileName:string;readonly sizeBytes:number;readonly maxBytes:number}):Promise<{readonly uploadUrl:string;readonly expiresInSeconds:number}>;
  read(input:{readonly tenantId:string;readonly sessionId:string;readonly fileName:string;readonly sizeBytes:number;readonly maxBytes:number;readonly maxRows:number;readonly allowedFields:readonly string[]}):Promise<readonly Row[]>;
  createTemplate(input:{readonly tenantId:string;readonly entityCode:string;readonly descriptorHash:string;readonly fields:readonly {key:string;label:string;required:boolean;example?:string|number|boolean;options?:readonly(string|number|boolean)[]}[]}):Promise<{readonly url:string;readonly expiresInSeconds:number}>;
}
...
export function createRecordTransferService<Transaction>(options: {
  ...
  return {
    ...
    async prepareWorkbookUpload(context:VerifiedRequestContext,sessionId:string,fileName:string,sizeBytes:number){
      if(!options.workbookIntake)throw new RecordServiceError(503,"IMPORT_FILE_UNAVAILABLE","Server-side file import is not configured");
      ...
      if(!staged||staged.session.status!=="uploading"||(staged.session.nextChunkIndex??0)!==0)throw new RecordServiceError(409,"XLSX_IMPORT_STATE_INVALID","Excel upload is unavailable for this import session");requireOwner(staged.session,context);
      const descriptor=await descriptorFor(options.metadata,context,staged.session.entityCode),format=importFileFormat(fileName);requireDescriptorHash(staged.session,descriptor.compiledHash);requireImportFileCapability(descriptor,format);
      const scope=await resolveImportScope(options.collectionScopes,context,descriptor,staged.session.scopeCoordinate);await authorizeDescriptorOperation(options.authorizer,context,descriptor,"import",scope.authorizationResource);await authorizeImportMode(options.authorizer,context,descriptor,staged.session.operation,scope.authorizationResource);
      const maxBytes=descriptor.listPresentation?.dataOperations?.importMaxFileBytes??25*1024*1024;
      return{sessionId,...await options.workbookIntake.prepareUpload({tenantId:context.tenantId,sessionId,fileName,sizeBytes,maxBytes})};
    },

    async completeWorkbookUpload(context:VerifiedRequestContext,sessionId:string,fileName:string,sizeBytes:number){
      if(!options.workbookIntake)throw new RecordServiceError(503,"IMPORT_FILE_UNAVAILABLE","Server-side file import is not configured");requireChunkStore(options.staging);
      const staged=await options.transactions.run(context.planeKey,context,tx=>options.staging.get(context.tenantId,sessionId,tx));
      if(!staged||staged.session.status!=="uploading"||(staged.session.nextChunkIndex??0)!==0)throw new RecordServiceError(409,"XLSX_IMPORT_STATE_INVALID","Excel upload is unavailable for this import session");requireOwner(staged.session,context);
      const descriptor=await descriptorFor(options.metadata,context,staged.session.entityCode),format=importFileFormat(fileName);requireDescriptorHash(staged.session,descriptor.compiledHash);requireImportFileCapability(descriptor,format);
      const scope=await resolveImportScope(options.collectionScopes,context,descriptor,staged.session.scopeCoordinate);await authorizeDescriptorOperation(options.authorizer,context,descriptor,"import",scope.authorizationResource);await authorizeImportMode(options.authorizer,context,descriptor,staged.session.operation,scope.authorizationResource);
      const maxBytes=descriptor.listPresentation?.dataOperations?.importMaxFileBytes??25*1024*1024,maxRows=descriptor.listPresentation?.dataOperations?.importMaxRows??50_000,allowedFields=importFields(descriptor).map(field=>field.key);
      const inspectedRows=await options.workbookIntake.read({tenantId:context.tenantId,sessionId,fileName,sizeBytes,maxBytes,maxRows,allowedFields}),rows=coerceWorkbookRows(inspectedRows,descriptor);if(!rows.length)throw new RecordServiceError(422,"IMPORT_FILE_EMPTY","The import file contains no records");
      return options.transactions.run(context.planeKey,context,async tx=>{const current=await options.staging.get(context.tenantId,sessionId,tx);if(!current||current.session.status!=="uploading"||(current.session.nextChunkIndex??0)!==0)throw new RecordServiceError(409,"IMPORT_FILE_STATE_INVALID","Import session changed while the file was being inspected");requireOwner(current.session,context);let checksum=current.session.checksum,chunkIndex=0;for(let offset=0;offset<rows.length;offset+=500){const chunk=rows.slice(offset,offset+500),chunkChecksum=digest(stable(chunk));checksum=digest(`${checksum}:${chunkIndex}:${chunkChecksum}`);await options.staging.appendChunk!({tenantId:context.tenantId,sessionId,chunkIndex,rows:chunk,chunkChecksum,cumulativeChecksum:checksum},tx);chunkIndex+=1;}await options.staging.seal!({tenantId:context.tenantId,sessionId,expectedChunkCount:chunkIndex,checksum},tx);await appendEvent(options.outbox,context,"records.import.upload_completed",`records:import:${sessionId}:upload-completed`,current.session.entityCode,sessionId,{transferId:sessionId,sessionId,entityCode:current.session.entityCode,format,rowCount:rows.length,checksum},tx);return{...current.session,status:"staged"as const,stagedRowCount:rows.length,nextChunkIndex:chunkIndex,checksum};});
    },

    async createWorkbookTemplate(context:VerifiedRequestContext,entityCode:string){
      if(!options.workbookIntake)throw new RecordServiceError(503,"XLSX_IMPORT_UNAVAILABLE","Excel import is not configured");const descriptor=await descriptorFor(options.metadata,context,entityCode);requireWorkbookCapability(descriptor);await authorizeDescriptorOperation(options.authorizer,context,descriptor,"import",{});const fields=importFields(descriptor).map(field=>({key:field.key,label:field.label??field.key,required:field.required,example:workbookExample(field.type)}));if(!fields.length)throw new RecordServiceError(409,"XLSX_TEMPLATE_EMPTY","This entity has no workbook-importable fields");return options.workbookIntake.createTemplate({tenantId:context.tenantId,entityCode,descriptorHash:descriptor.compiledHash,fields});
    },
    ...
    async validate(context: VerifiedRequestContext, sessionId: string): Promise<RecordImportPreview> {
      ...
      await options.transactions.run(context.planeKey, context, async (tx) => {
        ...
        await recordTransferAudit(options.audit, context, "records.import.validated", "import", staged.session.entityCode, { sessionId, summary, errorReportKey: errorReportKey ?? null }, tx);
    ...
    async commit(context: VerifiedRequestContext, sessionId: string) {
      ...
      const staged = await options.transactions.run(context.planeKey, context, async (tx) => {
        ...
        if (value.session.atomicity === "all_or_nothing" && value.validation.some((row) => !row.valid)) throw new RecordServiceError(422,"IMPORT_ATOMICITY_REJECTED","All-or-nothing import contains invalid rows");
        if (!value.validation.some((row) => row.valid)) throw new RecordServiceError(422,"IMPORT_NO_VALID_ROWS","Import contains no valid rows");
      ...
      await dispatchOrFail("import",context,staged.session.entityCode,sessionId,jobId,()=>options.jobs.enqueue("import", { planeKey:context.planeKey,tenantId: context.tenantId, sessionId, checksum: staged.session.checksum, actorPrincipalId: context.principalId,context }, { jobId }));
    ...
    async cancelImport(context: VerifiedRequestContext, sessionId: string) {
      ...
      const result = await options.transactions.run(context.planeKey, context, async (tx) => {
        ...
        if (state === "cancelled"){await recordTransferAudit(options.audit,context,"records.import.cancelled","import",current.session.entityCode,{sessionId,jobId},tx);await appendEvent(options.outbox, context, "records.import.cancelled", `${jobId}:cancelled`, current.session.entityCode, sessionId, { transferId:sessionId,sessionId,jobId,entityCode:current.session.entityCode,status:"cancelled",url:`/operations/data-transfers?transfer=${encodeURIComponent(sessionId)}` }, tx);}
        return state;
      });
    ...
    async restartImport(context:VerifiedRequestContext,sessionId:string){
      if(!options.staging.restartImport)throw new RecordServiceError(409,"IMPORT_RESTART_UNAVAILABLE","Import restart is not supported by the staging store");
      const retryId=createId(),jobId=`records:import-retry:${retryId}`;
      const staged=await options.transactions.run(context.planeKey,context,async tx=>{const current=await options.staging.get(context.tenantId,sessionId,tx);if(!current)throw new RecordServiceError(404,"IMPORT_NOT_FOUND","Import session is unavailable");requireOwner(current.session,context);if(current.session.status!=="failed")throw new RecordServiceError(409,"IMPORT_RESTART_INVALID_STATE","Only failed imports can be restarted");const descriptor=await descriptorFor(options.metadata,context,current.session.entityCode);requireDescriptorHash(current.session,descriptor.compiledHash);const scope=await resolveImportScope(options.collectionScopes,context,descriptor,current.session.scopeCoordinate);await authorizeDescriptorOperation(options.authorizer,context,descriptor,"import",scope.authorizationResource);await authorizeImportMode(options.authorizer,context,descriptor,current.session.operation,scope.authorizationResource);const state=await options.staging.restartImport!(context.tenantId,sessionId,tx);if(state!=="restarted")throw new RecordServiceError(409,"IMPORT_RESTART_INVALID_STATE","The import is no longer restartable");await recordTransferAudit(options.audit,context,"records.import.restart_requested","import",current.session.entityCode,{sessionId,jobId,retryId},tx);await appendEvent(options.outbox,context,"records.import.dispatch_requested",jobId,current.session.entityCode,sessionId,{tenantId:context.tenantId,sessionId,checksum:current.session.checksum,actorPrincipalId:context.principalId,jobId,retry:true},tx);return current;});
      await dispatchOrFail("import",context,staged.session.entityCode,sessionId,jobId,()=>options.jobs.enqueue("import",{planeKey:context.planeKey,tenantId:context.tenantId,sessionId,checksum:staged.session.checksum,actorPrincipalId:context.principalId,context},{jobId}));
      return{sessionId,jobId,status:"queued"as const};
    },
    ...
    async requestExport(context: VerifiedRequestContext, entityCode: string, filter: Readonly<Record<string, unknown>>, requestId?: string) {
      ...
      await dispatchOrFail("export",context,entityCode,exportRequestId,jobId,()=>options.jobs.enqueue("export", { planeKey:context.planeKey,tenantId: context.tenantId, entityCode, exactFilter, actorPrincipalId: context.principalId, exportRequestId,context }, { jobId }));
    ...
    async cancelExport(context: VerifiedRequestContext, exportRequestId: string) {
      ...
      const state = await options.transactions.run(context.planeKey, context, async (tx) => {
        const current=options.staging.getExport?await options.staging.getExport(context.tenantId,exportRequestId,tx):null;
        if(current&&current.actorPrincipalId!==context.principalId)throw new RecordServiceError(403,"FORBIDDEN","Record transfer belongs to another principal");
        ...
        if (result === "cancelled"){await recordTransferAudit(options.audit,context,"records.export.cancelled","export",current?.entityCode??"records.export_request",{exportRequestId,jobId},tx);await appendEvent(options.outbox, context, "records.export.cancelled", `${jobId}:cancelled`, current?.entityCode??"records.export_request", exportRequestId, { transferId:exportRequestId,exportRequestId,jobId,entityCode:current?.entityCode??"records",status:"cancelled",url:`/operations/data-transfers?transfer=${encodeURIComponent(exportRequestId)}` }, tx);}
        return result;
      });
  ...
  async function dispatchOrFail(kind:"import"|"export",context:VerifiedRequestContext,entityCode:string,transferId:string,jobId:string,enqueue:()=>Promise<unknown>):Promise<void>{
    try{await enqueue();return;}catch(error){
      const failedAt=now().toISOString(),errorCode="TRANSFER_DISPATCH_UNAVAILABLE",errorDetail="The transfer queue is temporarily unavailable. Retry this failed transfer after queue service recovery.";
      const changed=await options.transactions.run(context.planeKey,context,async tx=>{
        const marked=kind==="import"?await options.staging.failQueuedImport?.(context.tenantId,transferId,errorCode,errorDetail,failedAt,tx):await options.staging.failQueuedExport?.(context.tenantId,transferId,errorCode,errorDetail,failedAt,tx);
        if(!marked)return false;
        const eventCode=`records.${kind}.dispatch_failed`;
        await options.audit.record({eventCode,action:kind,outcome:"failure",severity:"error",actor:{kind:"user",principalId:context.principalId},tenantId:context.tenantId,entityType:entityCode,requestId:context.requestId,...(context.correlationId?{correlationId:context.correlationId}:{}),metadata:{transferId,jobId,errorCode}},tx);
        await appendEvent(options.outbox,context,eventCode,`${jobId}:dispatch-failed`,entityCode,transferId,{transferId,jobId,entityCode,status:"failed",errorCode,failedAt,url:`/operations/data-transfers?transfer=${encodeURIComponent(transferId)}`},tx);
        return true;
      }).catch(()=>false);
...
async function resolveTransferScope(resolver:RecordCollectionScopeResolver|undefined,context:VerifiedRequestContext,descriptor:Awaited<ReturnType<typeof descriptorFor>>,_operationCode:"export",coordinate?:import("@athyper/server-contract-records").RecordListScopeCoordinate):Promise<Extract<RecordCollectionScopeResolution,{readonly status:"ready"}>>{const resolution=resolver?await resolver.resolve({context,descriptor,operationCode:"read",...(coordinate?{coordinate}:{})}):{status:"ready"as const,authorizationResource:Object.freeze({}),constraints:Object.freeze([]),labels:Object.freeze([]),fingerprintMaterial:Object.freeze({mode:"tenant"})};if(resolution.status==="context_required")throw new RecordServiceError(409,"EXPORT_SCOPE_REQUIRED","A governed export scope must be selected");if(resolution.status==="forbidden")throw new RecordServiceError(403,resolution.code,resolution.message);return resolution;}
...
function requireWorkbookCapability(descriptor:Awaited<ReturnType<typeof descriptorFor>>){if(!descriptor.listPresentation?.dataOperations?.importFormats?.includes("xlsx"))throw new RecordServiceError(409,"XLSX_IMPORT_NOT_PUBLISHED","Excel import is not published for this entity");}
function requireImportFileCapability(descriptor:Awaited<ReturnType<typeof descriptorFor>>,format:"csv"|"json"|"xlsx"){if(!descriptor.listPresentation?.dataOperations?.importFormats?.includes(format))throw new RecordServiceError(409,"IMPORT_FILE_FORMAT_NOT_PUBLISHED",`${format.toUpperCase()} import is not published for this entity`);}
function importFileFormat(fileName:string):"csv"|"json"|"xlsx"{try{return structuredImportFormat(fileName);}catch(error){if(error instanceof Error&&"code"in error)throw new RecordServiceError(400,String(error.code),error.message);throw error;}}
function workbookExample(type:string):string|number|boolean{return type==="integer"||type==="decimal"||type==="money"?0:type==="boolean"?false:type==="date"?"2026-01-31":type==="datetime"?"2026-01-31T12:00:00Z":"";}
function importFields(descriptor:Awaited<ReturnType<typeof descriptorFor>>){return descriptor.listPresentation?.dataOperations?.importFields??descriptor.fields.filter(field=>field.writableOn.includes("create")||field.writableOn.includes("patch")).map(field=>({key:field.key,type:field.type,required:field.required,label:field.list?.label}));}
function coerceWorkbookRows(rows:readonly Readonly<Record<string,unknown>>[],descriptor:Awaited<ReturnType<typeof descriptorFor>>){const jsonFields=new Set(importFields(descriptor).filter(field=>field.type==="json").map(field=>field.key));return rows.map((row,index)=>Object.freeze(Object.fromEntries(Object.entries(row).map(([key,value])=>{if(!jsonFields.has(key)||typeof value!=="string"||!value.trim())return[key,value];try{return[key,JSON.parse(value)];}catch{throw new RecordServiceError(422,"XLSX_JSON_CELL_INVALID",`Data row ${index+2}, field ${key}, must contain valid JSON`);}}))));}
