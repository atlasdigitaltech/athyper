...
describe("record transfer workers",()=>{
  ...
  it("re-authorizes exports and neutralizes spreadsheet formulas in streamed CSV",async()=>{
    ...
    const store={claimExport:async()=>{if(claimed)return null;claimed=true;return{entityCode:"party",actorPrincipalId:"principal-1",exactFilter:{scopeCoordinate:{operatingOrganizationId:"org-1"},_transfer:{format:"csv",fields:["code","name"],headings:true}}};},completeExport:async(_tenant:string,_request:string,_key:string,rowCount:number)=>{completed=rowCount;},failExport:async()=>undefined,releaseExport:async()=>undefined}as unknown as KyselyRecordTransferStore;
