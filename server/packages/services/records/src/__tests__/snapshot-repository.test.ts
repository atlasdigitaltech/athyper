...
function setup(previous: Record<string, unknown> | null = row) {
  ...
  const query = vi.fn(async (sql: string, parameters: readonly unknown[]) => {
    if(sql.includes("fn_capture_entity")) captured={...row,id:"new",chain_seq:2,payload_json:JSON.parse(String(parameters[7]))};
    return {rows: sql.includes("fn_capture_entity") ? [{id:"new"}] : sql.includes("SELECT i.*") ? (captured ? [captured] : previous ? [previous] : []) : [],command:"SELECT",rowCount:1};
  });
  ...
  return { repository: new KyselyRecordSnapshotRepository({ run } as never), query, run, database };
}
describe("snapshot persistence", () => {
  ...
  it.each([
    ...
    expect(h.query.mock.calls.filter(([sql]) => sql.includes("fn_capture_entity"))).toHaveLength(1);
    expect(h.query.mock.calls.some(([sql]) => sql.includes("INSERT"))).toBe(false);
    await h.database.destroy();
  });
  it("hashes and returns the serialized payload, including Date values", async () => {
    ...
    const evidence = h.query.mock.calls.find(([sql]) => sql.includes("fn_capture_entity"));
