...
describe.skipIf(!enabled)("standard view PostgreSQL complete-result semantics",()=>{
 it("paginates 137 documents, deduplicates tasks and respects actor, tenant, status and request evidence",async()=>{
  ...
  try {
   psql(name,`CREATE SCHEMA document; CREATE SCHEMA master; CREATE SCHEMA snapshot;
   ...
   const repository=createKyselyRecordRepository({databases:{neon:db}});
   const input={descriptor,tenantId:tenant,limit:25,filters:[],sort:[{field:"id",direction:"desc"}],countMode:"exact",projection:["id","status"],collectionScope:[],cursorScope:"test",viewRelationships:[{kind:"workflow.actionable_documents.v1",principalId:principal,sourceEntityCode:"case_document",workflowKeys:["review.flow"],workTypeCodes:["case.approval"],link:"workflow_request"}]} as RecordRepositoryListInput;
   const ids:string[]=[];let cursor:string|undefined;
   do {const page=await repository.list({...input,...(cursor?{cursor}:{})});expect(page.pagination.total).toBe(137);ids.push(...page.data.map(row=>String(row['id'])));cursor=page.pagination.nextCursor;}while(cursor);
   expect(ids).toHaveLength(137);expect(new Set(ids).size).toBe(137);
   const grouped=await repository.list({...input,group:"status"});expect(grouped.groups).toEqual([{value:"draft",count:137}]);
   const mine=await repository.list({...input,viewRelationships:[{kind:"document.case_requests.v1",principalId:principal,role:{field:"requestedRole",values:["supplier"]}}]});expect(mine.pagination.total).toBe(142);
   const otherActor=await repository.list({...input,viewRelationships:[{...input.viewRelationships![0]!,principalId:other}]});expect(otherActor.pagination.total).toBe(2);
   const empty=await repository.list({...input,viewRelationships:[{...input.viewRelationships![0]!,principalId:"44444444-4444-4444-8444-444444444444"}]});expect(empty.data).toEqual([]);expect(empty.pagination.total).toBe(0);
   const first=await repository.list(input);await expect(repository.list({...input,cursor:first.pagination.nextCursor!,viewRelationships:[{...input.viewRelationships![0]!,principalId:other}]})).rejects.toThrow();
  } finally {await db?.destroy();psql("postgres",`DROP DATABASE ${name}`);}
 },30000);
});
