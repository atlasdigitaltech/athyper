...
const transactions = {
  run<T>(
    ...
    if (plane !== "neon") return Promise.reject(new Error("WRONG_PLANE"));
  ...
  it("serializes concurrent captures and replays an identical latest payload", async () => {
    const entityId = randomUUID(),
      common = {
        ...
        entityType: "master.wave0_test",
        entityId,
        entityCode: "wave0_test",
        entityContractHash: "a".repeat(64),
        captureEvent: "records.snapshot.capture",
        captureKind: "manual" as const,
        retentionClass: "temporary" as const,
        captureSource: "postgres_test",
      };
    const receipts = await Promise.all([
      repository.capture({
        ...common,
        sourceRecordVersion: 1,
        payload: { id: entityId, row_version: 1, value: "first" },
      }),
      repository.capture({
        ...common,
        sourceRecordVersion: 2,
        payload: { id: entityId, row_version: 2, value: "second" },
      }),
    ]);
    expect(receipts.map((item) => item.snapshot.chainSequence).sort()).toEqual([
      1, 2,
    ]);
    const latest = await repository.latest(
      { tenantId: tenantId!, principalId: principalId!, planeKey: "neon" },
      common.entityType,
      entityId,
    );
    const replay = await repository.capture({
      ...common,
      sourceRecordVersion: latest!.sourceRecordVersion,
      payload: latest!.payload,
    });
    expect(replay).toMatchObject({
      kind: "replayed",
      snapshot: { id: latest!.id, chainSequence: 2 },
    });
    expect(
      await repository.get(
        { tenantId: randomUUID(), principalId: principalId!, planeKey: "neon" },
        latest!.id,
      ),
    ).toBeNull();
    await expect(
      repository.get(
        { tenantId: tenantId!, principalId: principalId!, planeKey: "studio" },
        latest!.id,
      ),
    ).rejects.toThrow("WRONG_PLANE");
  });
  it("persists canonical JSONB evidence and creates new evidence for retention changes", async () => {
    ...
    expect(legal).toMatchObject({
      ...
      snapshot: {
        chainSequence: 2,
        retentionClass: "legal",
        previousSnapshotId: captured.snapshot.id,
      },
    });
  });
});
