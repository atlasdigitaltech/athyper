...
function relativeDateCondition(ref: RawBuilder<unknown>, value: unknown): RawBuilder<unknown> { switch (value) { case "today": return sql`${ref} >= CURRENT_DATE AND ${ref} < CURRENT_DATE + INTERVAL '1 day'`; case "yesterday": return sql`${ref} >= CURRENT_DATE - INTERVAL '1 day' AND ${ref} < CURRENT_DATE`; case "tomorrow": return sql`${ref} >= CURRENT_DATE + INTERVAL '1 day' AND ${ref} < CURRENT_DATE + INTERVAL '2 days'`; case "last_7_days": return sql`${ref} >= CURRENT_DATE - INTERVAL '7 days' AND ${ref} < CURRENT_DATE + INTERVAL '1 day'`; case "last_30_days": return sql`${ref} >= CURRENT_DATE - INTERVAL '30 days' AND ${ref} < CURRENT_DATE + INTERVAL '1 day'`; case "last_90_days": return sql`${ref} >= CURRENT_DATE - INTERVAL '90 days' AND ${ref} < CURRENT_DATE + INTERVAL '1 day'`; case "last_365_days": return sql`${ref} >= CURRENT_DATE - INTERVAL '365 days' AND ${ref} < CURRENT_DATE + INTERVAL '1 day'`; case "next_7_days": return sql`${ref} >= CURRENT_DATE AND ${ref} < CURRENT_DATE + INTERVAL '8 days'`; case "next_30_days": return sql`${ref} >= CURRENT_DATE AND ${ref} < CURRENT_DATE + INTERVAL '31 days'`; case "next_90_days": return sql`${ref} >= CURRENT_DATE AND ${ref} < CURRENT_DATE + INTERVAL '91 days'`; case "next_365_days": return sql`${ref} >= CURRENT_DATE AND ${ref} < CURRENT_DATE + INTERVAL '366 days'`; case "this_week": return sql`${ref} >= date_trunc('week', CURRENT_DATE) AND ${ref} < date_trunc('week', CURRENT_DATE) + INTERVAL '1 week'`; case "this_month": return sql`${ref} >= date_trunc('month', CURRENT_DATE) AND ${ref} < date_trunc('month', CURRENT_DATE) + INTERVAL '1 month'`; case "this_quarter": return sql`${ref} >= date_trunc('quarter', CURRENT_DATE) AND ${ref} < date_trunc('quarter', CURRENT_DATE) + INTERVAL '3 months'`; case "last_year": return sql`${ref} >= date_trunc('year', CURRENT_DATE) - INTERVAL '1 year' AND ${ref} < date_trunc('year', CURRENT_DATE)`; case "this_year": return sql`${ref} >= date_trunc('year', CURRENT_DATE) AND ${ref} < date_trunc('year', CURRENT_DATE) + INTERVAL '1 year'`; case "next_year": return sql`${ref} >= date_trunc('year', CURRENT_DATE) + INTERVAL '1 year' AND ${ref} < date_trunc('year', CURRENT_DATE) + INTERVAL '2 years'`; default: return sql`FALSE`; } }
