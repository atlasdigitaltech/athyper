...
describe("attachment lifecycle", () => {
  it("issues a short-lived download only for an active verified attachment",async()=>{const createDownloadUrl=vi.fn(async()=>"https://objects.example/signed"),identity={planeKey:"neon"as const,tenantId:"11111111-1111-4111-8111-111111111111",attachmentId:"22222222-2222-4222-8222-222222222222",principalId:"33333333-3333-4333-8333-333333333333"},record={id:identity.attachmentId,status:"active"as const,storageKey:"attachments/file",sha256:"a".repeat(64),isCurrent:true,isActive:true,hasLegalHold:false},lifecycle=createAttachmentLifecycle({transactions:{run:async(_plane,_actor,work)=>work({})},repository:{createStaged:async()=>{throw new Error("unused");},load:async()=>null,loadForDownload:async()=>record,finalizeClean:async()=>{throw new Error("unused");},quarantine:async()=>undefined,deactivate:async()=>undefined,expire:async()=>undefined,markPurged:async()=>undefined},storage:{get:async()=>new Uint8Array(),put:async()=>undefined,delete:async()=>undefined,exists:async()=>true,createDownloadUrl,createUploadUrl:async()=>"",copy:async()=>undefined},scanner:{scan:async()=>({status:"clean",scanner:"fixture",scannedAt:"2026-08-30T00:00:00.000Z",durationMs:1})},quota:{reserve:async()=>"created",commit:async()=>undefined,release:async()=>false,expire:async()=>[]},quotaPolicies:{resolve:async()=>({kind:"attachment.storage",limitBytes:1_000,limitItems:10,reservationTtlSeconds:60,retryAfterSeconds:60})},outbox:{append:async()=>undefined},now:()=>new Date("2026-08-30T00:00:00.000Z")});await expect(lifecycle.createAuthorizedDownload(identity,600)).resolves.toEqual({attachmentId:identity.attachmentId,url:"https://objects.example/signed",expiresAt:"2026-08-30T00:05:00.000Z"});expect(createDownloadUrl).toHaveBeenCalledWith("attachments/file",300);});
