...
export function createKyselyAttachmentRepository(
  ...
  return {
    ...
    async createStaged(input, tx) {
      let parent:
        { id: string; version_no: number; series_id: string } | undefined;
      ...
      if (input.parentAttachmentId) {
        const loaded = await sql<{
          ...
          version_no: number;
          series_id: string;
        }>`SELECT id,version_no,series_id FROM document.attachment WHERE tenant_id=${input.tenantId}::uuid AND id=${input.parentAttachmentId}::uuid AND is_active FOR UPDATE`.execute(
          tx,
        );
      ...
      const result =
        await sql<Row>`INSERT INTO document.attachment(id,tenant_id,file_name,original_filename,content_type,size_bytes,kind,storage_bucket,storage_key,version_no,parent_attachment_id,series_id,is_active,is_virus_scanned,expires_at,uploaded_by,metadata,status,created_by) VALUES(${input.attachmentId}::uuid,${input.tenantId}::uuid,${input.fileName},${input.fileName},${input.contentType},${input.sizeBytes ?? null},'attachment',${storageBucket},${input.storageKey},${parent ? parent.version_no + 1 : 1},${parent?.id ?? null}::uuid,${seriesId}::uuid,true,false,${input.expiresAt}::timestamptz,${input.principalId}::uuid,${JSON.stringify({ staged_size_bytes: input.sizeBytes ?? null, provenance: input.provenance ?? { source: "user_upload" }, ...(input.entityType ? { entity_type: input.entityType } : {}), ...(input.entityId ? { entity_id: input.entityId } : {}) })}::jsonb,'uploading',${input.principalId}::uuid) RETURNING *,false AS is_current,false AS has_legal_hold`.execute(
          tx,
        );
    ...
    async finalizeClean(identity, input, tx) {
      ...
      const row = required(result.rows[0]);
      await sql`UPDATE document.attachment_series SET current_attachment_id=${identity.attachmentId}::uuid,updated_at=clock_timestamp(),updated_by=${identity.principalId}::uuid WHERE tenant_id=${identity.tenantId}::uuid AND id=${row.series_id}::uuid`.execute(
        tx,
      );
    ...
    async expire(identity, tx) {
      await sql`UPDATE document.attachment SET status='expired',is_active=false,status_changed_at=clock_timestamp(),status_changed_by=${identity.principalId}::uuid,updated_by=${identity.principalId}::uuid WHERE tenant_id=${identity.tenantId}::uuid AND id=${identity.attachmentId}::uuid`.execute(
        tx,
      );
    },
    async markPurged(identity, tx) {
      await sql`UPDATE document.attachment SET status='deleted',is_active=false,storage_key='purged/'||id::text,status_changed_at=CASE WHEN status IS DISTINCT FROM 'deleted' THEN clock_timestamp() ELSE status_changed_at END,status_changed_by=CASE WHEN status IS DISTINCT FROM 'deleted' THEN ${identity.principalId}::uuid ELSE status_changed_by END,updated_by=${identity.principalId}::uuid WHERE tenant_id=${identity.tenantId}::uuid AND id=${identity.attachmentId}::uuid`.execute(
        tx,
      );
    },
    ...
    async listRetentionCandidates(input, tx) {
      const result = await sql<{
        id: string;
      }>`SELECT attachment.id FROM document.attachment attachment WHERE attachment.tenant_id=${input.tenantId}::uuid AND attachment.status IN('expired','deleted','orphaned') AND COALESCE(attachment.retention_until,'-infinity'::timestamptz)<=${input.before}::timestamptz AND NOT EXISTS(SELECT 1 FROM document.attachment_legal_hold hold WHERE hold.tenant_id=attachment.tenant_id AND hold.attachment_series_id=attachment.series_id AND hold.released_at IS NULL) ORDER BY attachment.status_changed_at NULLS LAST,attachment.created_at LIMIT ${input.limit} FOR UPDATE SKIP LOCKED`.execute(
        tx,
      );
