 ...
 *                       Resolves all active principals in auth groups that carry
 *                       that role for the tenant. Returns one
...
export class ApproverResolverService {
  ...
  /**
   * Find all active principals in the tenant that belong to an auth group carrying
   * the specified role. Works via:
   *   shared.role (code → id)  →  master.auth_group_role  →  master.auth_group_member
   *
  ...
  private async resolveByRole(
    ...
    try {
      ...
      const members = await (trx as any)
        ...
        .innerJoin(
          "master.principal as p" as never,
          "p.id" as never,
          "agm.principal_id" as never,
        )
        .select("agm.principal_id" as never)
        .where("agr.tenant_id" as never, "=", tenantId as never)
      ...
      const principalIds = members
        .map((member) => member["principal_id"] as string)
        .filter(Boolean)
        .filter((id, index, values) => values.indexOf(id) === index);

      return principalIds.map((id) => ({ type: "principal" as const, id }));
    } catch (err) {
  ...
  private async resolveByGroup(
    ...
    try {
      ...
      if (isUuid) {
        ...
        const row = await trx
          .selectFrom("master.auth_group as ag" as never)
