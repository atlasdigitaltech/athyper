...
export class ReportPackService {
  ...
  async generateHtmlPack(
    ...
    const certifications = await this.db
      .selectFrom("governance.cycle_certification as cc" as never)
      .selectAll("cc" as never)
      .where("cc.cycle_run_id" as never, "=", cycleRunId as never)
      .where("cc.tenant_id" as never, "=", tenantId as never)
      .orderBy("cc.created_at" as never, "asc")
      .execute() as Record<string, unknown>[];

    const deviations = await this.db
      .selectFrom("governance.cycle_deviation as cd" as never)
      .selectAll("cd" as never)
      .where("cd.cycle_run_id" as never, "=", cycleRunId as never)
      .where("cd.tenant_id" as never, "=", tenantId as never)
      .orderBy("cd.created_at" as never, "asc")
      .execute() as Record<string, unknown>[];

    const html = buildHtmlReport(cycleRun, tasks, certifications, deviations, packRow);
...
function buildHtmlReport(
  ...
  const taskRows = tasks.map((t) => `
    <tr>
      <td>${escapeHtml(t["task_code"])}</td>
      <td>${escapeHtml(t["task_name"] ?? t["task_code"])}</td>
      <td>${escapeHtml(t["status"])}</td>
      <td>${escapeHtml(t["assigned_to"])}</td>
      <td>${escapeHtml(t["due_at"])}</td>
    </tr>`).join("");
  const certificationRows = certifications.map((c) => `<tr><td>${escapeHtml(c["cert_code"])}</td><td>${escapeHtml(c["cert_version"])}</td><td>${escapeHtml(c["status"])}</td><td>${escapeHtml(c["content_hash"])}</td></tr>`).join("");
  const deviationRows = deviations.map((d) => `<tr><td>${escapeHtml(d["deviation_code"] ?? d["deviation_type"])}</td><td>${escapeHtml(d["title"])}</td><td>${escapeHtml(d["severity"])}</td><td>${escapeHtml(d["status"])}</td></tr>`).join("");
...
<body>
  ...
  <table><thead><tr><th>Code</th><th>Version</th><th>Status</th><th>Content hash</th></tr></thead>
    <tbody>${certificationRows || "<tr><td colspan='4'>No certifications</td></tr>"}</tbody></table>
  <h2>Deviations (${deviations.length})</h2>
  <table><thead><tr><th>Code</th><th>Title</th><th>Severity</th><th>Status</th></tr></thead>
    <tbody>${deviationRows || "<tr><td colspan='4'>No deviations</td></tr>"}</tbody></table>
  <footer>
