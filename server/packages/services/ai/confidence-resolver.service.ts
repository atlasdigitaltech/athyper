...
const PLATFORM_DEFAULT_THRESHOLDS: Record<string, ConfidenceThreshold> = {
  ...
  atlas_tool_read:  { min_for_suggest: 0.00, min_for_assist: 0.00, min_for_auto: 0.00, drift_alert_below: null, drift_window_hours: 72 },
};
...
export class ConfidenceResolver {
  ...
  /**
   * Uncached, fail-closed resolution for governed server actions. A database
   * error must not be mistaken for absence of a tenant override.
   */
  async resolveStrict(
    tenantId: string,
    actionCode: string,
