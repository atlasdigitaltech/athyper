...
export interface DispatchNotificationInput {
  ...
  priority?:      number;
  /**
   * Application plane this notification originates from.
   * Controls the email FROM address via emailFromMap in the notification worker.
   * Values: 'neon' | 'mesh' | 'admin'. Omit for in_app-only notifications.
   */
...
export class NotificationOrchestrator {
  ...
  private async createMessage(opts: {
    ...
    const storedPayload: Record<string, unknown> = {
      ...
      ...(opts.sourcePlane ? { source_plane: opts.sourcePlane } : {}),
    };
