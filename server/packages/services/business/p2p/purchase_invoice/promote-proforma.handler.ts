...
import { resolvePurchaseInvoiceCommitmentDefaults } from "@athyper/svc-shared";
...
const COMMITMENT_DEFAULT_FIELDS = [
  "company_code_id",
  "supplier_id",
  "currency_code",
  "base_currency_code",
...
export async function handlePromoteProforma(
  ...
  try {
    const updatedRecord = await db.transaction().execute(async (trx) => {
      ...
      const commitmentWasProvided = Object.prototype.hasOwnProperty.call(body, "commitment_id");
      const effectiveCommitmentId = commitmentWasProvided
        ? (commitment_id ?? null)
        : ((invoice["commitment_id"] as string | null | undefined) ?? null);
      const invoiceSource = String(invoice["invoice_source"] ?? "");
      if (
        commitmentWasProvided &&
        !effectiveCommitmentId &&
        (invoiceSource === "po_based" || invoiceSource === "contract_based")
      ) {
        const e = new Error("commitment_id is required to promote po_based and contract_based proforma invoices") as Error & { code: string; httpStatus: number };
        e.code = "VALIDATION_ERROR";
        e.httpStatus = 400;
        throw e;
      }

      const commitmentDefaults = commitmentWasProvided && effectiveCommitmentId
        ? await resolvePurchaseInvoiceCommitmentDefaults(
            { commitment_id: effectiveCommitmentId },
            {
              db:       trx,
              tenantId,
        ...
        : {};
      const dedupCompanyId = String(commitmentDefaults["company_code_id"] ?? invoice["company_code_id"] ?? "");
      const dedupSupplierId = (commitmentDefaults["supplier_id"] as string | null | undefined)
        ?? (invoice["supplier_id"] as string | null | undefined)
        ?? null;
      ...
      const dupResult = await sql<{ id: string }>`
        ...
        WHERE  tenant_id               = ${tenantId}
          AND  company_code_id         = ${dedupCompanyId}::uuid
          AND  supplier_id IS NOT DISTINCT FROM ${dedupSupplierId}::uuid
      ...
      const paymentTermId = (commitmentDefaults["payment_term_id"] as string | null | undefined)
        ?? (invoice["payment_term_id"] as string | null | undefined);
      if (paymentTermId) {
      ...
      if (commitmentWasProvided) {
        for (const field of COMMITMENT_DEFAULT_FIELDS) {
          if (Object.prototype.hasOwnProperty.call(commitmentDefaults, field)) {
            setClause[field] = commitmentDefaults[field] ?? null;
          }
        }
      }
