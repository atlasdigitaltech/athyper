 ...
 * Consumed by runtime flow UI to render the metadata-driven intake experience.
 *
 * 404 when no active flow exists — client falls back to the generic runtime form.
 *
  ...
  const handler: RequestHandler = async (req, res, next) => {
    try {
      ...
      const steps = stepRows.map((s: any) => {
        // Flat field bindings for this step
