...
async function supplierControlsWithCommodities(
  ...
  const capabilityById = new Map(
    ...
    qualificationCapabilityById = new Map(
      qualificationScopeResult.rows.map((row) => [
        text(row, "id"),
        optional(row, "commodity_capability_id"),
      ]),
    );
  const qualifications = baseData.qualifications.map((item) => {
    const commodityCapabilityId = qualificationCapabilityById.get(item.id),
      capability = commodityCapabilityId
        ? capabilityById.get(commodityCapabilityId)
        : undefined;
    return {
      ...item,
      ...(commodityCapabilityId ? { commodityCapabilityId } : {}),
      ...(capability
        ? {
    ...
    return result(
      {
        ...
        accounts: [],
        manageHref: href(input, "banking"),
        verifyHref: href(input, "bank-verification"),
      } satisfies BusinessPartner360BankingData,
      "empty",
  ...
  const data: BusinessPartner360SupplierControlsData = {
    ...
    preferences: p.rows.map((row) => ({
      ...
      ...(optional(row, "company_code_id")
        ? { companyCodeId: optional(row, "company_code_id") }
        : {}),
      ...(optional(row, "commodity_category_id")
        ? { commodityCategoryId: optional(row, "commodity_category_id") }
        : {}),
