...
async function supplierControls(input: Input, tx: Tx) {
  ...
  const [q, p, b, c] = await Promise.all([
    sql<Row>`SELECT qualification.id::text,qualification.partner_role::text,qualification.qualification_type_code,qualification.decision::text,qualification.decision_reason,organization_scope.operating_organization_id::text,company_scope.company_code_id::text,qualification.effective_from,qualification.effective_until,qualification.next_review_at FROM control.business_partner_qualification qualification JOIN control.business_partner_decision_scope organization_scope ON organization_scope.tenant_id=qualification.tenant_id AND organization_scope.qualification_id=qualification.id AND organization_scope.scope_kind='operating_organization' AND organization_scope.scope_mode='include' AND organization_scope.operating_organization_id=${input.operatingOrganizationId}::uuid LEFT JOIN control.business_partner_decision_scope company_scope ON company_scope.tenant_id=qualification.tenant_id AND company_scope.qualification_id=qualification.id AND company_scope.scope_kind='company_code' AND company_scope.scope_mode='include' WHERE qualification.tenant_id=${input.tenantId}::uuid AND qualification.business_partner_id=${input.businessPartnerId}::uuid AND(${input.companyCodeId ?? null}::uuid IS NULL OR company_scope.id IS NULL OR company_scope.company_code_id=${input.companyCodeId ?? null}::uuid) AND(qualification.effective_from IS NULL OR qualification.effective_from<=${input.asOf}::date) AND(qualification.effective_until IS NULL OR qualification.effective_until>${input.asOf}::date) ORDER BY qualification.qualification_type_code,qualification.id`.execute(
      tx,
    ),
    sql<Row>`SELECT preference.id::text,preference.status::text,organization_scope.operating_organization_id::text,company_scope.company_code_id::text,commodity_scope.commodity_category_id::text,preference.rationale,preference.effective_from,preference.effective_until FROM control.supplier_preference_designation preference JOIN control.business_partner_decision_scope organization_scope ON organization_scope.tenant_id=preference.tenant_id AND organization_scope.supplier_preference_id=preference.id AND organization_scope.scope_kind='operating_organization' AND organization_scope.scope_mode='include' AND organization_scope.operating_organization_id=${input.operatingOrganizationId}::uuid LEFT JOIN control.business_partner_decision_scope company_scope ON company_scope.tenant_id=preference.tenant_id AND company_scope.supplier_preference_id=preference.id AND company_scope.scope_kind='company_code' AND company_scope.scope_mode='include' LEFT JOIN control.business_partner_decision_scope commodity_scope ON commodity_scope.tenant_id=preference.tenant_id AND commodity_scope.supplier_preference_id=preference.id AND commodity_scope.scope_kind='commodity_category' AND commodity_scope.scope_mode='include' WHERE preference.tenant_id=${input.tenantId}::uuid AND preference.business_partner_id=${input.businessPartnerId}::uuid AND(${input.companyCodeId ?? null}::uuid IS NULL OR company_scope.id IS NULL OR company_scope.company_code_id=${input.companyCodeId ?? null}::uuid) AND preference.effective_from<=${input.asOf}::date AND(preference.effective_until IS NULL OR preference.effective_until>${input.asOf}::date) ORDER BY preference.effective_from DESC,preference.id`.execute(
      tx,
    ),
