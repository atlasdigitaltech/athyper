...
export class KyselyBusinessPartnerRequestRepository implements BusinessPartnerRequestRepository<Tx> {
  ...
  async apply(input: Parameters<BusinessPartnerRequestRepository<Tx>["apply"]>[0], transaction: Tx) {
    ...
    if(request.kind==="change_employment")return applyEmploymentChange(input,request,transaction);
    ...
    const aggregate={businessPartner:masterSnapshot(partner),...(person?{person:masterSnapshot(person)}:{}),partnerRole:role,roleExtension:masterSnapshot(roleRow),operatingOrganizationAssignment:masterSnapshot(assignment),onboarding:{requestId:request.id,requestNo:request.requestNo,requestKind:request.kind,workflowRequestId:request.workflowRequestId,decisionFingerprint:request.decisionFingerprint,approvedAt:request.approvedAt,approvedBy:request.approvedBy,source:request.source,schema:request.schema}};
...
async function applyWorkforceOnboarding(input:Parameters<BusinessPartnerRequestRepository<Tx>["apply"]>[0],request:BusinessPartnerRequest,transaction:Tx){
  ...
  const managerEmployeeId=payloadUuid(payload,"managerEmployeeId","manager_employee_id");
  ...
  const assignment=(await sql<Row>`INSERT INTO master.work_assignment(tenant_id,code,name,employee_id,employment_id,position_id,org_unit_id,manager_employee_id,company_code_id,assignment_type,fte,effective_from,metadata,status,created_by) VALUES(${input.tenantId}::uuid,${assignmentCode},${displayName},${text(employee,"id")}::uuid,${text(employment,"id")}::uuid,${request.positionId??null}::uuid,${request.orgUnitId}::uuid,${managerEmployeeId??null}::uuid,${request.companyCodeId}::uuid,'primary',${payloadNumber(payload,"fte")??1},${hireDate}::date,${JSON.stringify({onboardingRequestId:request.id})}::jsonb,'active',${input.appliedBy}::uuid) RETURNING *`.execute(transaction)).rows[0];
  if(!assignment)throw new Error("WORKFORCE_ASSIGNMENT_CREATE_FAILED");
  const defaultChecklist=[
    {code:"payroll",label:"Payroll enrollment",category:"payroll",required:true,status:"pending"},
    {code:"benefits",label:"Benefits enrollment",category:"benefits",required:false,status:"pending"},
    {code:"equipment",label:"Equipment issued",category:"equipment",required:true,status:"pending"},
    {code:"policy",label:"Policies acknowledged",category:"policy",required:true,status:"pending"},
    {code:"training",label:"Required training",category:"training",required:true,status:"pending"},
    {code:"application_access",label:"Application access provisioned",category:"application_access",required:false,status:"pending"},
  ];
  const onboardingDraft=(await sql<Row>`INSERT INTO document.onboarding_case(tenant_id,code,name,person_id,employee_id,target_start_date,checklist,workflow_request_id,business_partner_request_id,business_partner_id,employment_id,work_assignment_id,status,created_by) VALUES(${input.tenantId}::uuid,${`ONB.${request.requestNo}`},${`Onboarding ${displayName}`},${text(person,"id")}::uuid,${text(employee,"id")}::uuid,${hireDate}::date,${JSON.stringify(defaultChecklist)}::jsonb,${request.workflowRequestId??null}::uuid,${request.id}::uuid,${text(partner,"id")}::uuid,${text(employment,"id")}::uuid,${text(assignment,"id")}::uuid,'draft',${input.appliedBy}::uuid) RETURNING *`.execute(transaction)).rows[0];
  if(!onboardingDraft)throw new Error("WORKFORCE_ONBOARDING_CASE_CREATE_FAILED");
  const onboarding=(await sql<Row>`UPDATE document.onboarding_case SET status='active',status_changed_at=now(),status_changed_by=${input.appliedBy}::uuid,updated_by=${input.appliedBy}::uuid WHERE tenant_id=${input.tenantId}::uuid AND id=${text(onboardingDraft,"id")}::uuid RETURNING *`.execute(transaction)).rows[0];
  if(!onboarding)throw new Error("WORKFORCE_ONBOARDING_CASE_ACTIVATION_FAILED");
  const aggregate={businessPartner:masterSnapshot(partner),person:masterSnapshot(person),employee:masterSnapshot(employee),employment:masterSnapshot(employment),workAssignment:masterSnapshot(assignment),onboarding:{id:text(onboarding,"id"),requestId:request.id,requestNo:request.requestNo,workflowRequestId:request.workflowRequestId,decisionFingerprint:request.decisionFingerprint,approvedAt:request.approvedAt,approvedBy:request.approvedBy,source:request.source,schema:request.schema}};
  ...
  const applied=(await sql<Row>`UPDATE document.business_partner_request SET status='applied',materialized_business_partner_id=${text(partner,"id")}::uuid,materialized_person_id=${text(person,"id")}::uuid,materialized_employee_id=${text(employee,"id")}::uuid,materialized_employment_id=${text(employment,"id")}::uuid,materialized_work_assignment_id=${text(assignment,"id")}::uuid,materialized_onboarding_case_id=${text(onboarding,"id")}::uuid,materialization_snapshot_id=${snapshot}::uuid,application_idempotency_key=${input.command.idempotencyKey},application_fingerprint=${input.applicationFingerprint},application_result_kind='workforce_created',applied_at=now(),applied_by=${input.appliedBy}::uuid,updated_by=${input.appliedBy}::uuid WHERE tenant_id=${input.tenantId}::uuid AND id=${request.id}::uuid AND status='applying' RETURNING *`.execute(transaction)).rows[0];
...
async function applyPartnerAmendment(input:Parameters<BusinessPartnerRequestRepository<Tx>["apply"]>[0],request:BusinessPartnerRequest,transaction:Tx){
  ...
  const snapshot=await captureApplicationSnapshot(input,request,updated,{snapshotType:"business_partner.amendment",businessPartner:masterSnapshot(updated)},transaction);
...
async function applyPartnerLifecycle(input:Parameters<BusinessPartnerRequestRepository<Tx>["apply"]>[0],request:BusinessPartnerRequest,transaction:Tx){
  ...
  const live=(await sql<Row>`SELECT
    ...
    (SELECT count(*)::int FROM master.employment employment JOIN master.person person ON person.tenant_id=employment.tenant_id AND person.id=employment.person_id WHERE person.tenant_id=${input.tenantId}::uuid AND person.business_partner_id=${request.targetBusinessPartnerId}::uuid AND employment.status='active') AS active_employments,
  ...
  const expectedDependencies=["supplier_roles","customer_roles","active_organization_assignments","active_employments","effective_blocks","effective_qualifications"].map(code=>({code,count:Number(live[code]??0)}));
  ...
  const claimed=await claimApplication(input,request,transaction);if(!claimed)return null;
  const updated=(await sql<Row>`UPDATE master.business_partner SET status=${desired}::master.business_partner_status_d,status_changed_at=now(),status_changed_by=${input.appliedBy}::uuid,updated_by=${input.appliedBy}::uuid WHERE tenant_id=${input.tenantId}::uuid AND id=${request.targetBusinessPartnerId}::uuid RETURNING *`.execute(transaction)).rows[0];
...
async function applyEmploymentChange(input:Parameters<BusinessPartnerRequestRepository<Tx>["apply"]>[0],request:BusinessPartnerRequest,transaction:Tx){
  if(!request.targetBusinessPartnerId||!request.legalEntityId||!request.companyCodeId||!request.orgUnitId)return null;
  const effectiveFrom=requiredPayloadText(request.proposedPayload,"effectiveFrom","effective_from");if(!/^\d{4}-\d{2}-\d{2}$/.test(effectiveFrom))throw new MasterDataError(409,"BUSINESS_PARTNER_REQUEST_PAYLOAD_INVALID","effectiveFrom must be an ISO date");
  const managerEmployeeId=payloadUuid(request.proposedPayload,"managerEmployeeId","manager_employee_id");
  const compatible=(await sql<{compatible:boolean}>`SELECT EXISTS(SELECT 1 FROM master.company_code company JOIN master.legal_entity legal ON legal.tenant_id=company.tenant_id AND legal.id=company.legal_entity_id JOIN master.org_unit unit ON unit.tenant_id=company.tenant_id WHERE company.tenant_id=${input.tenantId}::uuid AND company.id=${request.companyCodeId}::uuid AND company.legal_entity_id=${request.legalEntityId}::uuid AND unit.id=${request.orgUnitId}::uuid AND company.status='active' AND legal.status='active' AND unit.status='active' AND (${request.positionId??null}::uuid IS NULL OR EXISTS(SELECT 1 FROM master.position position WHERE position.tenant_id=company.tenant_id AND position.id=${request.positionId??null}::uuid AND position.company_code_id=company.id AND (position.legal_entity_id IS NULL OR position.legal_entity_id=legal.id) AND (position.org_unit_id IS NULL OR position.org_unit_id=unit.id) AND position.status='active')) AND (${managerEmployeeId??null}::uuid IS NULL OR EXISTS(SELECT 1 FROM master.work_assignment manager_assignment WHERE manager_assignment.tenant_id=company.tenant_id AND manager_assignment.employee_id=${managerEmployeeId??null}::uuid AND manager_assignment.company_code_id=company.id AND manager_assignment.status='active' AND manager_assignment.assignment_type='primary' AND daterange(manager_assignment.effective_from,COALESCE(manager_assignment.effective_until,'infinity'::date),'[)') @> ${effectiveFrom}::date))) AS compatible`.execute(transaction)).rows[0]?.compatible===true;
  if(!compatible)throw new MasterDataError(409,"BUSINESS_PARTNER_REQUEST_SCOPE_INCOMPATIBLE","Employment-change legal entity, company, organization unit, or position is no longer compatible");
  const current=(await sql<Row>`SELECT partner.code AS partner_code,person.id AS person_id,employee.id AS employee_id,employee.name AS employee_name,employment.*,assignment.id AS assignment_id FROM master.business_partner partner JOIN master.person person ON person.tenant_id=partner.tenant_id AND person.business_partner_id=partner.id JOIN master.employee employee ON employee.tenant_id=person.tenant_id AND employee.person_id=person.id JOIN master.employment employment ON employment.tenant_id=employee.tenant_id AND employment.employee_id=employee.id AND employment.status='active' LEFT JOIN master.work_assignment assignment ON assignment.tenant_id=employee.tenant_id AND assignment.employment_id=employment.id AND assignment.status='active' AND assignment.assignment_type='primary' WHERE partner.tenant_id=${input.tenantId}::uuid AND partner.id=${request.targetBusinessPartnerId}::uuid AND partner.status='active' FOR UPDATE OF partner,employee,employment,assignment`.execute(transaction)).rows[0];
  if(!current)throw new MasterDataError(409,"BUSINESS_PARTNER_EMPLOYMENT_CURRENT_REQUIRED","An active workforce employment is required");
  if(effectiveFrom<=dateOnly(current["hire_date"]))throw new MasterDataError(409,"BUSINESS_PARTNER_EMPLOYMENT_DATE_CONFLICT","Employment change must begin after the current hire date");
  const claimed=await claimApplication(input,request,transaction);if(!claimed)return null;
  await sql`UPDATE master.work_assignment SET effective_until=${effectiveFrom}::date,status='inactive',updated_by=${input.appliedBy}::uuid WHERE tenant_id=${input.tenantId}::uuid AND id=${current["assignment_id"]??null}::uuid`.execute(transaction);
  await sql`UPDATE master.employment SET termination_date=${effectiveFrom}::date,termination_reason='governed_change',employment_status='terminated',status='inactive',updated_by=${input.appliedBy}::uuid WHERE tenant_id=${input.tenantId}::uuid AND id=${text(current,"id")}::uuid`.execute(transaction);
  const employment=(await sql<Row>`INSERT INTO master.employment(tenant_id,code,name,person_id,employee_id,legal_entity_id,company_code_id,employment_number,employment_type,is_primary,employment_status,hire_date,metadata,status,created_by) VALUES(${input.tenantId}::uuid,${`EMT.${request.requestNo}`},${text(current,"employee_name")},${text(current,"person_id")}::uuid,${text(current,"employee_id")}::uuid,${request.legalEntityId}::uuid,${request.companyCodeId}::uuid,${`${text(current,"employment_number")}.${request.requestNo}`},${payloadEnum(request.proposedPayload,"employmentType","employment_type",["full_time","part_time","contract","casual","intern","volunteer"],text(current,"employment_type"))},true,'active',${effectiveFrom}::date,${JSON.stringify({changeRequestId:request.id,priorEmploymentId:text(current,"id")})}::jsonb,'active',${input.appliedBy}::uuid) RETURNING *`.execute(transaction)).rows[0];
  if(!employment)throw new Error("BUSINESS_PARTNER_EMPLOYMENT_CHANGE_CREATE_FAILED");
  const assignment=(await sql<Row>`INSERT INTO master.work_assignment(tenant_id,code,name,employee_id,employment_id,position_id,org_unit_id,manager_employee_id,company_code_id,assignment_type,fte,effective_from,metadata,status,created_by) VALUES(${input.tenantId}::uuid,${`ASN.${request.requestNo}`},${text(current,"employee_name")},${text(current,"employee_id")}::uuid,${text(employment,"id")}::uuid,${request.positionId??null}::uuid,${request.orgUnitId}::uuid,${managerEmployeeId??null}::uuid,${request.companyCodeId}::uuid,'primary',${payloadNumber(request.proposedPayload,"fte")??1},${effectiveFrom}::date,${JSON.stringify({changeRequestId:request.id,priorAssignmentId:current["assignment_id"]??null})}::jsonb,'active',${input.appliedBy}::uuid) RETURNING *`.execute(transaction)).rows[0];
  if(!assignment)throw new Error("BUSINESS_PARTNER_WORK_ASSIGNMENT_CHANGE_CREATE_FAILED");
  const partner=(await sql<Row>`SELECT * FROM master.business_partner WHERE tenant_id=${input.tenantId}::uuid AND id=${request.targetBusinessPartnerId}::uuid`.execute(transaction)).rows[0]!;
  const snapshot=await captureApplicationSnapshot(input,request,partner,{snapshotType:"business_partner.employment_change",priorEmploymentId:text(current,"id"),priorAssignmentId:current["assignment_id"]??null,employment:masterSnapshot(employment),workAssignment:masterSnapshot(assignment)},transaction);
  return finalizeApplication(input,request,{partnerId:request.targetBusinessPartnerId,snapshot,resultKind:"employment_changed",personId:text(current,"person_id"),employeeId:text(current,"employee_id"),employmentId:text(employment,"id"),workAssignmentId:text(assignment,"id")},transaction);
}
...
function applicationResponse(request:BusinessPartnerRequest,replayed:boolean){if(!request.materializedBusinessPartnerId||!request.materializationSnapshotId||!request.applicationFingerprint||!request.applicationResultKind)throw new Error("BUSINESS_PARTNER_REQUEST_APPLICATION_ROW_INVALID");const common={businessPartnerId:request.materializedBusinessPartnerId,resultKind:request.applicationResultKind,snapshotId:request.materializationSnapshotId,applicationFingerprint:request.applicationFingerprint,...(request.applicationReasonCode?{reasonCode:request.applicationReasonCode}:{}),...(request.materializedBankVerificationId?{bankVerificationId:request.materializedBankVerificationId}:{}),...(request.materializedPersonId?{personId:request.materializedPersonId}:{}),...(request.materializedEmployeeId?{employeeId:request.materializedEmployeeId,roleId:request.materializedEmployeeId,partnerRole:"workforce" as const}:{}),...(request.materializedEmploymentId?{employmentId:request.materializedEmploymentId}:{}),...(request.materializedWorkAssignmentId?{workAssignmentId:request.materializedWorkAssignmentId}:{}),...(request.materializedOnboardingCaseId?{onboardingCaseId:request.materializedOnboardingCaseId}:{}),...(request.materializedPrincipalId?{principalId:request.materializedPrincipalId}:{}),...(request.materializedSupplierId?{supplierId:request.materializedSupplierId,roleId:request.materializedSupplierId,partnerRole:"supplier" as const}:{}),...(request.materializedCustomerId?{customerId:request.materializedCustomerId,roleId:request.materializedCustomerId,partnerRole:"customer" as const}:{}),...((request.materializedSupplierCompanyProfileId??request.materializedCustomerCompanyProfileId)?{companyProfileId:request.materializedSupplierCompanyProfileId??request.materializedCustomerCompanyProfileId}:{}),...(request.materializedOperatingOrganizationAssignmentId?{operatingOrganizationAssignmentId:request.materializedOperatingOrganizationAssignmentId}:{})};return{request,materialization:common,replayed};}
