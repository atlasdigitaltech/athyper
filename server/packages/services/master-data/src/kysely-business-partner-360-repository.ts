  ...
  async readFragments(
    ...
    const [address, contact, identifiers, work, recent] = await Promise.all([
      ...
      can(BUSINESS_PARTNER_360_PERMISSIONS.identifierMasked)
        ...
        : Promise.resolve({ rows: [] } as { rows: Row[] }),
      sql<Row>`SELECT count(*) FILTER(WHERE status IN('draft','submitted','in_review','approved','materializing','conflicted'))::int active_count,count(*) FILTER(WHERE status='conflicted')::int returned_count FROM document.entity_case WHERE tenant_id=${input.tenantId}::uuid AND entity_code='master.business_partner' AND target_entity_id=${input.core.id}::uuid`.execute(
        transaction,
      ),
