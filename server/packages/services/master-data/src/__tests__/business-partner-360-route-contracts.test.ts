...
async function fixture({ deny = false, malformed = false, forbidden = false } = {}) {
  ...
  const app = createHttpApplication({ openApi: { title: 'BP360', version: '1', enforceContracts: true, enforceResponses: true }, configure(app) {
    registerBusinessPartner360Routes(app, {
      ...
      service: { summary: vi.fn(), section: vi.fn(), revealTaxRegistration: vi.fn(), revealBankAccount: reveal } as never,
    });
  } });
  const server = createServer(app); servers.push(server);
  await new Promise<void>((resolve) => server.listen(0, '127.0.0.1', resolve));
  const address = server.address(); if (!address || typeof address === 'string') throw new Error('Missing address');
  return { url: `http://127.0.0.1:${address.port}`, reveal };
}
...
describe('Business Partner 360 registered contracts', () => {
  it('publishes all four operations with request, response and permission metadata', async () => {
