...
const primaryRelationships={addresses:[{clientItemKey:"address-1",isPrimary:true}],contactPersons:[{clientItemKey:"contact-1",isPrimary:true}],contactChannels:[{contactClientItemKey:"contact-1",isPrimary:true}]};
const context = {
...
const schema = { code: "neon.business_partner_request", version: 1, hash: "a".repeat(64), releaseId: "00000000-0000-4000-8000-000000000001" } as const;
...
class MemoryRepository implements BusinessPartnerRequestRepository<object> {
  ...
  submissionCount = 0;
  workflowTask: { readonly requestId: string; readonly stageId: string; readonly workItemId: string; readonly ownerPrincipalId: string; workItemVersion: number; workItemStatus: string } | undefined;
  ...
  async getView(tenantId:string,requestId:string){const request=await this.get(tenantId,requestId);if(!request)return null;const validation=this.validations.at(-1),task=this.workflowTask;return{request,validationFindings:validation?.findings??[],...(request.workflowRequestId&&task?{workflow:{...task,definition:{code:"neon.business_partner.onboarding",version:1,hash:"b".repeat(64)}}}:{})};}
  ...
  async submit(input: Parameters<BusinessPartnerRequestRepository<object>["submit"]>[0]) { const row=this.rows.get(input.requestId); if(row?.status==="pending_approval") return null; if(!row||row.rowVersion!==input.expectedVersion||row.validationSummary["outcome"]!=="passed")return null; const updated:BusinessPartnerRequest={...row,status:"pending_approval",rowVersion:row.rowVersion+2,workflowRequestId:"77777777-7777-4777-8777-777777777777",decisionFingerprint:input.decisionFingerprint,submittedAt:"2026-08-28T02:00:00.000Z",submittedBy:input.submittedBy};this.rows.set(row.id,updated);this.submissionCount+=1;this.workflowTask={requestId:updated.workflowRequestId!,stageId:this.submissionCount===1?"88888888-8888-4888-8888-888888888888":"88888888-8888-4888-8888-888888888887",workItemId:this.submissionCount===1?"99999999-9999-4999-8999-999999999999":"99999999-9999-4999-8999-999999999998",workItemVersion:1,workItemStatus:"open",ownerPrincipalId:input.definition.approverPrincipalIds[0]!};return{request:updated,workflow:{requestId:updated.workflowRequestId!,stageId:this.workflowTask.stageId,workItemId:this.workflowTask.workItemId,workItemVersion:this.workflowTask.workItemVersion,definition:{code:input.definition.code,version:input.definition.version,hash:input.definition.hash},decisionFingerprint:input.decisionFingerprint},replayed:false}; }
  async decide(input: Parameters<BusinessPartnerRequestRepository<object>["decide"]>[0]) {const row=this.rows.get(input.command.requestId),task=this.workflowTask;if(!row||!task||row.status!=="pending_approval"||row.rowVersion!==input.command.expectedRequestVersion||task.workItemId!==input.command.workItemId||task.workItemVersion!==input.command.expectedWorkItemVersion||task.ownerPrincipalId!==input.decidedBy||task.workItemStatus!=="open")return null;task.workItemStatus="completed";task.workItemVersion+=1;const status=input.command.decision==="approve"?"approved":input.command.decision==="reject"?"rejected":"returned";const updated:BusinessPartnerRequest={...row,status,rowVersion:row.rowVersion+1,...(status==="approved"?{approvedAt:"2026-08-28T03:00:00.000Z",approvedBy:input.decidedBy}:{})};this.rows.set(row.id,updated);return{request:updated,workflow:{requestId:input.command.workflowRequestId,stageId:task.stageId,workItemId:input.command.workItemId,workItemVersion:task.workItemVersion,definition:{code:"neon.business_partner.onboarding",version:1,hash:"b".repeat(64)},decisionFingerprint:row.decisionFingerprint!},decision:input.command.decision,decisionFingerprint:input.decisionFingerprint,replayed:false};}
...
function command(overrides: Partial<CreateBusinessPartnerRequestCommand> = {}): CreateBusinessPartnerRequestCommand {
  return { context, idempotencyKey: "request-key-001", kind: "new_partner", source: { kind: "manual" }, requestedRole: "supplier", operatingOrganizationId: "55555555-5555-4555-8555-555555555555", ...overrides, proposedPayload: { ...(overrides.proposedPayload===undefined?{legalName:"Acme"}:{}), ownershipClass: "internal", [overrides.requestedRole==="customer"?"customerType":"supplierType"]: overrides.proposedPayload?.["ownershipClass"]==="external"?(overrides.requestedRole==="customer"?"corporate":"general"):"intercompany", ...overrides.proposedPayload } };
}

function fixture(allowed: boolean | ((permissionCode:string, resource?:Readonly<Record<string,unknown>>)=>boolean) = true, denialReason = "test") {
  const repository = new MemoryRepository(), permissions: string[] = [], effects: string[] = [], outboxEvents: Array<Readonly<Record<string, unknown>>> = [];
  const service = createBusinessPartnerRequestService({
    repository,
    authorizer: { async authorize(request) { permissions.push(request.permissionCode); return (typeof allowed==="function"?allowed(request.permissionCode,request.resource):allowed) ? { allowed: true } : { allowed: false, reason: denialReason }; } },
    ...
    validator: {async validate(input,tx){return createBusinessPartnerRequestValidator({duplicates:{async findExactLegalName(){return[];}},createEvaluationId:()=>"66666666-6666-4666-8666-666666666666",now:()=>new Date("2026-08-28T01:00:00.000Z")}).validate({...input,request:{...input.request,proposedPayload:{relationshipProposals:primaryRelationships,...input.request.proposedPayload}}},tx);}},
    ...
    outbox: { async append(input) { effects.push(input.eventType); outboxEvents.push(input as unknown as Readonly<Record<string, unknown>>); } },
...
describe("Business Partner request service", () => {
  ...
  it("creates once and replays the same internal NEON command without duplicate effects", async () => {
    ...
    expect(value.effects).toEqual(["business_partner.case.created", "business_partner.case.created"]);
    expect(value.permissions).toEqual(["neon.relationship.entity_case.create", "neon.relationship.entity_case.create"]);
  });

  it("rejects a stale viewed form release before creating a case", async () => {
    const value = fixture();
    await expect(value.service.create(command({ expectedForm: { ...schema, hash: "c".repeat(64) } }))).rejects.toMatchObject({
  ...
  it("persists identity extensions as typed coordinates and keeps them outside the general payload",async()=>{
    ...
    expect(created.request.proposedPayload).toMatchObject({legalName:"Acme"});
  ...
  it("uses expected versions and organization-scoped update permission", async () => {
    ...
    expect(value.permissions.at(-1)).toBe("neon.relationship.entity_case.update");
  });
  ...
  it("persists a ruleset-pinned validation evaluation and request summaries atomically", async () => {
    ...
    expect(result.validation).toMatchObject({evaluationId:"66666666-6666-4666-8666-666666666666",valid:true,ruleset:{code:"neon.business_partner.entity_case.phase1",version:3}});
    expect(result.validation.findings).toHaveLength(18);
    expect(value.repository.validations).toHaveLength(1);
    expect(value.permissions.at(-1)).toBe("neon.relationship.entity_case.validate");
    expect(value.effects.slice(-2)).toEqual(["business_partner.case.validated","business_partner.case.validated"]);
  });

  it("lets only the invitation-bound restricted applicant validate and submit their portal proposal",async()=>{const value=fixture(permission=>permission==="neon.supplier_registration.external.respond");const created=await value.repository.create({tenantId:context.tenantId,requestNo:"BPR-EXTERNAL-001",createdBy:context.principalId,schema,command:{idempotencyKey:"external-owned-001",kind:"new_partner",source:{kind:"portal"},registrationMode:"self_service",invitationId:"12121212-1212-4121-8121-121212121212",applicantPrincipalId:context.principalId,requestedRole:"supplier",operatingOrganizationId:"55555555-5555-4555-8555-555555555555",proposedPayload:{relationshipProposals:primaryRelationships,legalName:"Invited Supplier",ownershipClass:"external",supplierType:"general",qualificationTypeCode:"standard"}}},{});value.permissions.length=0;const other={...context,principalId:"cccccccc-cccc-4ccc-8ccc-cccccccccccc"}as VerifiedRequestContext;await expect(value.service.validate({context:other,requestId:created.id,expectedVersion:1})).rejects.toMatchObject({code:"FORBIDDEN",status:403});expect(value.permissions).toEqual(["neon.relationship.entity_case.validate"]);value.permissions.length=0;const validated=await value.service.validate({context,requestId:created.id,expectedVersion:1});const submitted=await value.service.submit({context,requestId:created.id,expectedVersion:validated.request.rowVersion,idempotencyKey:"external-submit-001"});expect(submitted.request.status).toBe("pending_approval");expect(value.permissions).toEqual(["neon.supplier_registration.external.respond","neon.supplier_registration.external.respond"]);});

  it("labels duplicate evidence as a deterministic exact-name master lookup",async()=>{const value=fixture(),created=(await value.service.create(command())).request,validator=createBusinessPartnerRequestValidator({duplicates:{async findExactLegalName(){return[{id:"10101010-1010-4010-8010-101010101010",code:"BP.EXACT",name:"Acme",legalName:"Acme"}];}},createEvaluationId:()=>"67676767-6767-4676-8676-676767676767",now:()=>new Date("2026-08-28T01:30:00.000Z")});const result=await validator.validate({context,request:{...created,proposedPayload:{...created.proposedPayload,relationshipProposals:primaryRelationships}}},{});expect(result.duplicateSummary).toEqual({matchMethod:"exact_legal_name_case_insensitive",sourceEntity:"master.business_partner",sourceField:"legal_name_or_name",scored:false,exactLegalNameCandidateCount:1,requiresResolution:true,candidates:[{id:"10101010-1010-4010-8010-101010101010",code:"BP.EXACT",name:"Acme",legalName:"Acme"}]});expect(result.findings).toContainEqual(expect.objectContaining({ruleCode:"identity.legal_name.duplicate",outcome:"failed",severity:"warning",evidenceReference:expect.objectContaining({matchMethod:"exact_legal_name_case_insensitive",sourceEntity:"master.business_partner"})}));expect(result.valid).toBe(true);});
  ...
  it("requires a qualification type before an external supplier can be submitted",async()=>{const value=fixture(),created=(await value.service.create(command({proposedPayload:{legalName:"External Supplier",ownershipClass:"external"}}))).request;const result=await value.service.validate({context,requestId:created.id,expectedVersion:1});expect(result.request.status).toBe("validation_failed");expect(result.validation.findings).toContainEqual(expect.objectContaining({ruleCode:"supplier.qualification_type.required",fieldPath:"$.qualificationTypeCode",outcome:"failed",severity:"error"}));});

  it("requires exactly one primary address and a primary contact with a primary channel",async()=>{const value=fixture(),created=(await value.service.create(command())).request,validator=createBusinessPartnerRequestValidator({duplicates:{async findExactLegalName(){return[];}},createEvaluationId:()=>"67676767-6767-4676-8676-676767676767",now:()=>new Date("2026-08-28T01:30:00.000Z")});const request={...created,proposedPayload:{...created.proposedPayload,relationshipProposals:{addresses:[{clientItemKey:"address-1",isPrimary:false}],contactPersons:[{clientItemKey:"contact-1",isPrimary:true}],contactChannels:[{contactClientItemKey:"contact-1",isPrimary:false}]}}};const invalid=await validator.validate({context,request},{});expect(invalid.findings).toEqual(expect.arrayContaining([expect.objectContaining({ruleCode:"relationship.primary_address.required",outcome:"failed"}),expect.objectContaining({ruleCode:"relationship.primary_contact.required",outcome:"failed"})]));const valid=await validator.validate({context,request:{...request,proposedPayload:{...request.proposedPayload,relationshipProposals:{addresses:[{clientItemKey:"address-1",isPrimary:true}],contactPersons:[{clientItemKey:"contact-1",isPrimary:true}],contactChannels:[{contactClientItemKey:"contact-1",isPrimary:true}]}}}},{});expect(valid.findings.filter(item=>item.ruleCode.startsWith("relationship.")).every(item=>item.outcome==="passed")).toBe(true);});

  it("submits a passing request into a pinned workflow and approves it with a distinct maker/checker",async()=>{const value=fixture(),created=(await value.service.create(command())).request;const validated=await value.service.validate({context,requestId:created.id,expectedVersion:1});const submitted=await value.service.submit({context,requestId:created.id,expectedVersion:validated.request.rowVersion,idempotencyKey:"submit-key-001"});expect(submitted).toMatchObject({replayed:false,request:{status:"pending_approval"},workflow:{definition:{code:"neon.business_partner.onboarding",version:1}}});const approverContext={...context,principalId:"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",assurance:"elevated"} as VerifiedRequestContext;const decided=await value.service.decide({context:approverContext,requestId:created.id,workflowRequestId:submitted.workflow.requestId,workItemId:submitted.workflow.workItemId,expectedRequestVersion:submitted.request.rowVersion,expectedWorkItemVersion:1,decision:"approve",reason:"Validated onboarding evidence",idempotencyKey:"decision-key-001"});expect(decided).toMatchObject({decision:"approve",request:{status:"approved",approvedBy:approverContext.principalId}});expect(value.effects.slice(-4)).toEqual(["business_partner.case.submitted","business_partner.case.submitted","business_partner.case.approved","business_partner.case.approved"]);});
  ...
  it("reports high-risk decision step-up without weakening the permission",async()=>{const value=fixture(permission=>permission!=="neon.relationship.entity_case.decide","mfa_required"),created=(await value.service.create(command())).request,validated=await value.service.validate({context,requestId:created.id,expectedVersion:1}),submitted=await value.service.submit({context,requestId:created.id,expectedVersion:validated.request.rowVersion,idempotencyKey:"submit-mfa-boundary-001"}),approverContext={...context,principalId:"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"} as VerifiedRequestContext;await expect(value.service.decide({context:approverContext,requestId:created.id,workflowRequestId:submitted.workflow.requestId,workItemId:submitted.workflow.workItemId,expectedRequestVersion:submitted.request.rowVersion,expectedWorkItemVersion:1,decision:"approve",reason:"MFA boundary evidence",idempotencyKey:"decision-mfa-boundary-001"})).rejects.toMatchObject({code:"BUSINESS_PARTNER_REQUEST_STEP_UP_REQUIRED",status:403,message:expect.stringContaining("MFA verification")});expect(value.permissions.at(-1)).toBe("neon.relationship.entity_case.decide");expect(value.repository.rows.get(created.id)?.status).toBe("pending_approval");});

  it("denies a decision when a previously visible task is reassigned",async()=>{const value=fixture(),created=(await value.service.create(command())).request,validated=await value.service.validate({context,requestId:created.id,expectedVersion:1}),submitted=await value.service.submit({context,requestId:created.id,expectedVersion:validated.request.rowVersion,idempotencyKey:"submit-owner-boundary-001"});const approverContext={...context,principalId:"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",assurance:"elevated"} as VerifiedRequestContext;expect((await value.service.getView({context:approverContext,requestId:created.id})).workflow).toMatchObject({ownerPrincipalId:approverContext.principalId,workItemStatus:"open",workItemVersion:1});value.repository.workflowTask={...value.repository.workflowTask!,ownerPrincipalId:"bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb",workItemVersion:2};await expect(value.service.decide({context:approverContext,requestId:created.id,workflowRequestId:submitted.workflow.requestId,workItemId:submitted.workflow.workItemId,expectedRequestVersion:submitted.request.rowVersion,expectedWorkItemVersion:2,decision:"approve",reason:"Stale visible assignment",idempotencyKey:"decision-owner-boundary-001"})).rejects.toMatchObject({code:"BUSINESS_PARTNER_REQUEST_TASK_OWNER_REQUIRED",status:403});expect(value.repository.rows.get(created.id)?.status).toBe("pending_approval");});

  it("denies stale, wrong, and closed workflow task coordinates",async()=>{const value=fixture(),created=(await value.service.create(command())).request,validated=await value.service.validate({context,requestId:created.id,expectedVersion:1}),submitted=await value.service.submit({context,requestId:created.id,expectedVersion:validated.request.rowVersion,idempotencyKey:"submit-task-boundary-001"});const approverContext={...context,principalId:"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",assurance:"elevated"} as VerifiedRequestContext,base={context:approverContext,requestId:created.id,workflowRequestId:submitted.workflow.requestId,workItemId:submitted.workflow.workItemId,expectedRequestVersion:submitted.request.rowVersion,decision:"approve" as const,reason:"Task boundary evidence"};await expect(value.service.decide({...base,expectedWorkItemVersion:2,idempotencyKey:"decision-stale-task-001"})).rejects.toMatchObject({code:"BUSINESS_PARTNER_REQUEST_TASK_VERSION_CONFLICT",status:409});await expect(value.service.decide({...base,workItemId:"99999999-9999-4999-8999-999999999997",expectedWorkItemVersion:1,idempotencyKey:"decision-wrong-task-001"})).rejects.toMatchObject({code:"BUSINESS_PARTNER_REQUEST_TASK_MISMATCH",status:409});value.repository.workflowTask!.workItemStatus="completed";await expect(value.service.decide({...base,expectedWorkItemVersion:1,idempotencyKey:"decision-closed-task-001"})).rejects.toMatchObject({code:"BUSINESS_PARTNER_REQUEST_TASK_NOT_OPEN",status:409});expect(value.repository.rows.get(created.id)?.status).toBe("pending_approval");});
  ...
  it("opens a new owned task version when a returned case is resubmitted",async()=>{const value=fixture(),created=(await value.service.create(command())).request,validated=await value.service.validate({context,requestId:created.id,expectedVersion:1}),first=await value.service.submit({context,requestId:created.id,expectedVersion:validated.request.rowVersion,idempotencyKey:"submit-new-task-001"}),approverContext={...context,principalId:"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",assurance:"elevated"} as VerifiedRequestContext,returned=await value.service.decide({context:approverContext,requestId:created.id,workflowRequestId:first.workflow.requestId,workItemId:first.workflow.workItemId,expectedRequestVersion:first.request.rowVersion,expectedWorkItemVersion:1,decision:"return",reason:"Correct governed identity",idempotencyKey:"return-new-task-001"}),corrected=await value.service.patch({context,requestId:created.id,expectedVersion:returned.request.rowVersion,proposedPayload:{legalName:"Acme Corrected"}}),revalidated=await value.service.validate({context,requestId:created.id,expectedVersion:corrected.rowVersion}),second=await value.service.submit({context,requestId:created.id,expectedVersion:revalidated.request.rowVersion,idempotencyKey:"submit-new-task-002"}),view=await value.service.getView({context:approverContext,requestId:created.id});expect(second.workflow.requestId).toBe(first.workflow.requestId);expect(second.workflow.workItemId).not.toBe(first.workflow.workItemId);expect(view.workflow).toMatchObject({workItemId:second.workflow.workItemId,workItemVersion:1,workItemStatus:"open",ownerPrincipalId:approverContext.principalId});});

  it("materializes an approved Phase 1B supplier exactly once and returns stable result coordinates",async()=>{const value=fixture(),created=(await value.service.create(command())).request;const validated=await value.service.validate({context,requestId:created.id,expectedVersion:1});const submitted=await value.service.submit({context,requestId:created.id,expectedVersion:validated.request.rowVersion,idempotencyKey:"submit-apply-001"});const approverContext={...context,principalId:"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",assurance:"elevated"} as VerifiedRequestContext;const approved=await value.service.decide({context:approverContext,requestId:created.id,workflowRequestId:submitted.workflow.requestId,workItemId:submitted.workflow.workItemId,expectedRequestVersion:submitted.request.rowVersion,expectedWorkItemVersion:1,decision:"approve",reason:"Approved for supplier onboarding",idempotencyKey:"decision-apply-001"});const applyContext={...context,principalId:"bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb",assurance:"elevated"} as VerifiedRequestContext;const first=await value.service.apply({context:applyContext,requestId:created.id,expectedVersion:approved.request.rowVersion,idempotencyKey:"application-key-001"});const second=await value.service.apply({context:applyContext,requestId:created.id,expectedVersion:approved.request.rowVersion,idempotencyKey:"application-key-001"});expect(first).toMatchObject({replayed:false,request:{status:"applied"},materialization:{businessPartnerId:"10101010-1010-4010-8010-101010101010",supplierId:"11111111-1010-4010-8010-101010101010"}});expect(second).toMatchObject({replayed:true,materialization:first.materialization});expect(value.permissions.at(-1)).toBe("neon.relationship.entity_case.materialize");expect(value.effects.slice(-2)).toEqual(["business_partner.case.materialized","business_partner.case.materialized"]);const event=value.outboxEvents.at(-1)!;expect(event["eventKey"]).toBe(`business_partner.case.materialized:${created.id}:v${first.request.rowVersion}`);expect(event["payload"]).toMatchObject({notification:{templateKey:"business_partner.case.materialized.v1",templateData:{caseNo:"BPR-TEST-001",status:"applied"}},recipient_principal_ids:[created.createdBy]});});
  ...
  ] as const)("%s materializes only the approved role against the target identity",async(scenario,kind,role)=>{const value=fixture(),targetBusinessPartnerId="10101010-1010-4010-8010-101010101010",created=(await value.service.create(command({idempotencyKey:`${scenario.toLowerCase()}-create`,kind,targetBusinessPartnerId,requestedRole:role,proposedPayload:role==="supplier"?{ownershipClass:"external",qualificationTypeCode:"standard",supplierCode:`SUP.${scenario.replaceAll("-","")}`,supplierType:"general"}:{ownershipClass:"external",customerCode:`CUS.${scenario.replaceAll("-","")}`,customerType:"corporate"}}))).request,validated=await value.service.validate({context,requestId:created.id,expectedVersion:1}),submitted=await value.service.submit({context,requestId:created.id,expectedVersion:validated.request.rowVersion,idempotencyKey:`${scenario.toLowerCase()}-submit`}),approverContext={...context,principalId:"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",assurance:"elevated"} as VerifiedRequestContext,approved=await value.service.decide({context:approverContext,requestId:created.id,workflowRequestId:submitted.workflow.requestId,workItemId:submitted.workflow.workItemId,expectedRequestVersion:submitted.request.rowVersion,expectedWorkItemVersion:1,decision:"approve",reason:`${scenario} independently approved`,idempotencyKey:`${scenario.toLowerCase()}-decide`}),applied=await value.service.apply({context:{...context,principalId:"bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb",assurance:"elevated"} as VerifiedRequestContext,requestId:created.id,expectedVersion:approved.request.rowVersion,idempotencyKey:`${scenario.toLowerCase()}-apply`});expect(applied.materialization).toMatchObject({businessPartnerId:targetBusinessPartnerId,partnerRole:role});expect(applied.request.materializedBusinessPartnerId).toBe(targetBusinessPartnerId);if(role==="supplier"){expect(applied.request.materializedSupplierId).toBeDefined();expect(applied.request.materializedCustomerId).toBeUndefined();}else{expect(applied.request.materializedCustomerId).toBeDefined();expect(applied.request.materializedSupplierId).toBeUndefined();}});
...
it("requires authority over the stored child before changing its organization", async()=>{
 let restrict=false;
 const source="55555555-5555-4555-8555-555555555555",target="77777777-7777-4777-8777-777777777777";
 const value=fixture((_permission,resource)=>!restrict || resource?.["operatingOrganizationId"]===target);
 const created=(await value.service.create(command())).request;restrict=true;
 await expect(value.service.patch({context,requestId:created.id,expectedVersion:1,proposedPayload:{legalName:"Moved"},operatingOrganizationId:target})).rejects.toMatchObject({code:"FORBIDDEN"});
 expect(value.repository.rows.get(created.id)?.operatingOrganizationId).toBe(source);
});
it("authorizes each independently scoped case in a list",async()=>{
 let restrict=false;
 const value=fixture((_permission,resource)=>!restrict || resource?.["recordId"]===undefined);
 await value.service.create(command());restrict=true;
 expect(await value.service.list({context,operatingOrganizationId:"55555555-5555-4555-8555-555555555555"})).toEqual([]);
});


describe("commercial role ownership validation",()=>{
 it.each([
  ["supplier","internal","general"], ["customer","internal","corporate"],
  ["supplier","external","intercompany"], ["customer","external","intercompany"],
 ] as const)("rejects %s %s/%s before workflow submission",async(role,ownership,subtype)=>{
  const value=fixture();const created=(await value.service.create(command({requestedRole:role,proposedPayload:{legalName:"Invalid ownership",ownershipClass:ownership,[role==="supplier"?"supplierType":"customerType"]:subtype,qualificationTypeCode:"standard"}}))).request;
  const validation=await value.service.validate({context,requestId:created.id,expectedVersion:created.rowVersion});
  expect(validation.validation.valid).toBe(false);
  expect(validation.validation.findings).toContainEqual(expect.objectContaining({ruleCode:"role.ownership_subtype.compatible",outcome:"failed",severity:"error"}));
  await expect(value.service.submit({context,requestId:created.id,expectedVersion:validation.request.rowVersion,idempotencyKey:"invalid-role-submit"})).rejects.toMatchObject({code:"BUSINESS_PARTNER_REQUEST_NOT_SUBMITTABLE"});
  expect(value.repository.submissionCount).toBe(0);
 });
 it("rechecks legacy passed validation before opening an approval workflow",async()=>{
  const value=fixture();const created=(await value.service.create(command())).request;
  const payload={...created.proposedPayload};delete payload["supplierType"];
  value.repository.rows.set(created.id,{...created,proposedPayload:payload,validationSummary:{outcome:"passed"}});
  await expect(value.service.submit({context,requestId:created.id,expectedVersion:1,idempotencyKey:"legacy-validation-submit"})).rejects.toMatchObject({code:"BUSINESS_PARTNER_REQUEST_SUBMISSION_VALIDATION_FAILED"});
  expect(value.repository.submissionCount).toBe(0);
 });
});

it("rejects missing relationship proposals before approval",async()=>{const value=fixture(),created=(await value.service.create(command())).request;const payload={...created.proposedPayload};payload["relationshipProposals"]=null;value.repository.rows.set(created.id,{...created,proposedPayload:payload});const result=await value.service.validate({context,requestId:created.id,expectedVersion:1});expect(result.validation.valid).toBe(false);expect(result.validation.findings).toEqual(expect.arrayContaining([expect.objectContaining({ruleCode:"relationship.primary_address.required",outcome:"failed"}),expect.objectContaining({ruleCode:"relationship.primary_contact.required",outcome:"failed"})]));await expect(value.service.submit({context,requestId:created.id,expectedVersion:result.request.rowVersion,idempotencyKey:"missing-relationships"})).rejects.toMatchObject({code:"BUSINESS_PARTNER_REQUEST_NOT_SUBMITTABLE"});});
