...
function fixture(allowed: boolean | ((permissionCode:string)=>boolean) = true, denialReason = "test") {
  ...
  const service = createBusinessPartnerRequestService({
    repository,
    authorizer: { async authorize(request) { permissions.push(request.permissionCode); return (typeof allowed==="function"?allowed(request.permissionCode):allowed) ? { allowed: true } : { allowed: false, reason: denialReason }; } },
...
describe("Business Partner request service", () => {
  ...
  it("persists a ruleset-pinned validation evaluation and request summaries atomically", async () => {
    ...
    expect(result.validation).toMatchObject({evaluationId:"66666666-6666-4666-8666-666666666666",valid:true,ruleset:{code:"neon.business_partner.entity_case.phase1",version:1}});
