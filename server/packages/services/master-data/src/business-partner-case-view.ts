...
export function toGovernedBusinessPartnerCaseView(
  ...
  const required = sections.filter((section) => section.state !== "not_applicable");
  ...
  try {
    return parseGovernedCaseView({
      ...
      subject: {
        ...
        ...(request.materializedBusinessPartnerId || request.targetBusinessPartnerId
          ? { id: request.materializedBusinessPartnerId ?? request.targetBusinessPartnerId }
          : {}),
      ...
      progress: {
        completed: required.filter((section) => section.state === "complete").length,
        required: required.length,
        blockers: required.reduce((total, section) => total + section.errors, 0),
      },
      sections,
    ...
    throw invalidProducer(
      error instanceof Error ? error.message : "The governed case projection is invalid",
    );
  }
}
...
function lifecycleSections(
  ...
  const approvalComplete = ["approved", "applying", "applied", "failed"].includes(
    request.status,
  );
      ...
      request.status === "validating"
        ...
        : request.status === "validation_failed"
          ...
          : validated || afterSubmit
            ? "complete"
            : "not_started",
      request.status === "validation_failed" ? Math.max(1, validationErrors) : 0,
    ),
    section(
      "approval",
      "Approval",
...
function allowedActions(
  ...
  const actions: Array<Readonly<{ id: string; label: string; requiresElevation?: boolean }>> = [];
  const editable = ["draft", "validation_failed", "returned"].includes(request.status);
  if (editable && !request.proposedPayload["meshChangeResolutionId"] && permissions.has(permission.update)) actions.push(action("edit", "Edit"));
  if (editable && permissions.has(permission.validate)) actions.push(action("validate", "Validate"));
  if (
    request.status === "draft" &&
    (request.validationSummary["valid"] === true ||
      request.validationSummary["outcome"] === "passed") &&
    permissions.has(permission.submit)
  ) {
  ...
  if (
    ...
    permissions.has(permission.decide)
  ) {
    actions.push(
  ...
  if (request.status === "approved" && permissions.has(permission.materialize)) {
    actions.push(action("materialize", "Materialize", true));
  }
  if (request.materializedBusinessPartnerId && permissions.has(permission.readPartner)) {
...
function action(id: string, label: string, requiresElevation?: boolean) {
  return Object.freeze({ id, label, ...(requiresElevation ? { requiresElevation } : {}) });
}
...
function subjectName(request: BusinessPartnerRequest): string {
  for (const key of ["displayName", "display_name", "legalName", "legal_name", "name"] as const) {
...
function governedStatus(status: BusinessPartnerRequest["status"]): GovernedCaseStatusV1 {
  switch (status) {
    case "draft":
    case "validating":
    ...
    case "superseded":
      return status;
    default:
      throw invalidProducer(`Unsupported governed case status: ${String(status)}`);
  }
}
