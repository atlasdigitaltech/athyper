...
export function createBusinessPartnerRequestService<Transaction>(
  ...
  return {
    async preflightCreate(command) {
      ...
      if (!options.validator.validateProposed)
      ...
      if (
        ...
        throw new MasterDataError(
          ...
          "Published request schema release identity is invalid",
        );
      if (
        command.expectedForm &&
        JSON.stringify([
          command.expectedForm.code,
          command.expectedForm.version,
          command.expectedForm.hash,
          command.expectedForm.releaseId,
        ]) !==
          JSON.stringify([
            schema.code,
            schema.version,
        ...
        throw new MasterDataError(
          409,
          "BUSINESS_PARTNER_REQUEST_FORM_CHANGED",
          "Reload the published request form",
        );
      const validation = await options.transactions.run(
        "neon",
        actor(command.context),
        (transaction) =>
          options.validator.validateProposed!(
    ...
    async create(command) {
      ...
      if (
        command.expectedForm &&
        (command.expectedForm.code !== schema.code ||
          command.expectedForm.version !== schema.version ||
          command.expectedForm.hash !== schema.hash ||
          command.expectedForm.releaseId !== schema.releaseId)
      )
        throw new MasterDataError(
          409,
          "BUSINESS_PARTNER_REQUEST_FORM_CHANGED",
          "The published request form changed after it was opened; reload before creating the request",
        );
          ...
          if (existing) {
            const matches =
              ...
              options.repository.matchesGovernedImportCreation
                ? await options.repository.matchesGovernedImportCreation(
                    {
                      context: command.context,
                      command: withoutContext(command),
                      schema,
                      existing,
                    },
                    transaction,
                  )
                : creationFingerprint(existing) ===
                  commandFingerprint(command, schema);
            if (!matches)
              throw new MasterDataError(
                409,
    ...
    async explainCase(query) {
      ...
      if (
        ...
        (!Number.isSafeInteger(query.expectedVersion) ||
      ...
      return options.transactions.run(
        ...
        actor(query.context),
        async (transaction) => {
          const view = await options.repository.getView(
            query.context.tenantId,
            query.requestId,
            transaction,
          );
          if (!view)
            throw new MasterDataError(
              404,
              "BUSINESS_PARTNER_REQUEST_NOT_FOUND",
              "Case explanation is unavailable",
            );
          await authorize(
            options.authorizer,
            query.context,
          ...
          if (
            ...
            query.businessPartnerId !== view.request.targetBusinessPartnerId &&
            query.businessPartnerId !==
              view.request.materializedBusinessPartnerId
          )
            throw new MasterDataError(
              403,
              "FORBIDDEN",
          ...
          if (
            query.expectedVersion !== undefined &&
            query.expectedVersion !== view.request.rowVersion
          )
            throw new MasterDataError(
              409,
              "BUSINESS_PARTNER_REQUEST_VERSION_CONFLICT",
              "Saved case changed; refresh the explanation",
            );
          ...
          if (view.previousSnapshot) {
            ...
            const decision = await options.authorizer.authorize({
              ...
              permissionCode: businessPartnerRequestPermissions.read,
              resource: scope(
                typeof payload.operatingOrganizationId === "string"
                  ? payload.operatingOrganizationId
                  : undefined,
                typeof payload.companyCodeId === "string"
                  ? payload.companyCodeId
                  : undefined,
              ),
            });
    ...
    async list(query) {
      assertContext(query.context);
      if (
        query.beforeCreatedAt !== undefined &&
        !Number.isFinite(parseInstant(query.beforeCreatedAt))
      )
        throw invalid("beforeCreatedAt must be a valid timestamp");
      if (
      ...
      return options.transactions.run(
        ...
        async (transaction) => {
          ...
          for (const row of rows) {
            if (row.tenantId !== query.context.tenantId)
              throw new MasterDataError(
                403,
                "FORBIDDEN",
    ...
    async getAggregate(query) {
      ...
      await authorize(
        options.authorizer,
        query.context,
        businessPartnerPermissions.read,
        scope(query.operatingOrganizationId),
      );
      return options.transactions.run(
        "neon",
        actor(query.context),
        async (transaction) => {
          const aggregate = await options.repository.getAggregate(
            query.context.tenantId,
            query.businessPartnerId,
            query.operatingOrganizationId,
            transaction,
          ...
          if (!aggregate)
            throw new MasterDataError(
              ...
              "Business Partner was not found in the selected operating organization",
            );
          return aggregate;
        },
      );
    ...
    async patch(command) {
      ...
      return options.transactions.run(
        ...
        async (transaction) => {
          ...
          if((command.draftCapture || command.extensions?.bankAccounts?.length || command.extensions?.supportingDocuments?.length) && (current.kind!=="new_partner" || current.source.kind!=="manual"))throw invalid("Additional request capture is only available for internal new-partner requests");
          if (command.extensions !== undefined)
          ...
          } as CreateBusinessPartnerRequestCommand);
          if (
            !["draft", "validation_failed", "returned"].includes(current.status)
          )
            throw new MasterDataError(
              409,
              "BUSINESS_PARTNER_REQUEST_NOT_EDITABLE",
              "Only draft, validation-failed, or returned requests can be edited",
            );
          await authorize(
            options.authorizer,
            command.context,
            businessPartnerRequestPermissions.update,
            {...caseScope(current), operationKey: "update"},
          );
          ...
          const updated = await options.repository.patch(
            {
              ...
              ...(command.companyCodeId !== undefined
                ...
                : {}),
              ...(command.requestedRole !== undefined
                ? { requestedRole: command.requestedRole }
                : {}),
              updatedBy: command.context.principalId,
            },
            transaction,
          );
          if (!updated)
            throw new MasterDataError(
              409,
              "BUSINESS_PARTNER_REQUEST_VERSION_CONFLICT",
              "Request version or editable state changed",
            );
          if (updated.rowVersion === current.rowVersion) return updated;
          await effects(
            options,
            command.context,
            transaction,
      ...
      return options.transactions.run(
        ...
        async (transaction) => {
          ...
          if (
            ...
            throw new MasterDataError(
              409,
              "BUSINESS_PARTNER_REQUEST_NOT_VALIDATABLE",
          ...
          validateValidationResult(validation);
          const request = await options.repository.recordValidation(
            {
              tenantId: command.context.tenantId,
              requestId: command.requestId,
              expectedVersion: command.expectedVersion,
          ...
          await effects(
            ...
            request,
            {
              evaluationId: validation.evaluationId,
      ...
      return options.transactions.run(
        ...
        async (transaction) => {
          ...
          if (
            ...
            const replay = await options.repository.submit(
              {
                tenantId: command.context.tenantId,
                requestId: command.requestId,
                expectedVersion: command.expectedVersion,
            ...
            throw new MasterDataError(
              409,
              "BUSINESS_PARTNER_REQUEST_ALREADY_SUBMITTED",
          ...
          if (current.status !== "draft" && !(internalSupplier && options.supplierSubmission && current.status === "returned"))
            throw new MasterDataError(
              ...
              "Only a validated draft can be submitted",
            );
          if (current.rowVersion !== command.expectedVersion)
            throw new MasterDataError(
              409,
              "BUSINESS_PARTNER_REQUEST_VERSION_CONFLICT",
              "Request version changed before submission",
            );
          assertPassedValidation(current);
          const submissionValidation = await options.validator.validate(
            { context: command.context, request: current },
            transaction,
          );
          if (!submissionValidation.valid)
            throw new MasterDataError(
              409,
              "BUSINESS_PARTNER_REQUEST_SUBMISSION_VALIDATION_FAILED",
              "Request no longer satisfies current validation rules; validate and correct the draft before submission",
            );
          if (
            current.registrationMode === "on_behalf" &&
            !current.representationEvidenceId
          )
            throw new MasterDataError(
              409,
          ...
          await effects(
            ...
            result.request,
            {
              cycleRunId: result.workflow.requestId,
              cycleTaskId: result.workflow.workItemId,
              definition: result.workflow.definition,
              decisionFingerprint: fingerprint,
              currentApproverPrincipalIds: definition.approverPrincipalIds,
            },
          );
          return result;
        },
      ...
      return options.transactions.run(
        ...
        async (transaction) => {
          ...
          if (current.workflowRequestId !== command.workflowRequestId)
            throw new MasterDataError(
              409,
              "BUSINESS_PARTNER_REQUEST_WORKFLOW_MISMATCH",
              "Decision does not address the request's pinned workflow",
            );
          if (
            ...
            throw new MasterDataError(
              403,
              "BUSINESS_PARTNER_REQUEST_SELF_APPROVAL_FORBIDDEN",
              "The submitter cannot approve their own Business Partner request",
            );
          ...
          const result = await options.repository.decide(
            {
              tenantId: command.context.tenantId,
              command: withoutDecisionContext(command),
          ...
          if (!result)
            throw new MasterDataError(
              409,
              "BUSINESS_PARTNER_REQUEST_DECISION_CONFLICT",
              "Request or work-item state/version changed, or the actor is not eligible",
            );
          if (!result.replayed) {
            const eventCode =
              result.request.status === "approved"
                ...
                : result.request.status === "rejected"
                  ? "business_partner.case.rejected"
                  : result.request.status === "returned"
                    ? "business_partner.case.returned"
                    : result.workflow.workItemStatus === "open"
                      ? "business_partner.workflow.stage.activated"
                      : "business_partner.workflow.vote.recorded";
            await effects(
              options,
              command.context,
              transaction,
              ...
              result.request,
              {
                cycleRunId: result.workflow.requestId,
                cycleTaskId: result.workflow.workItemId,
                stageId: result.workflow.stageId,
                decision: command.decision,
                decisionFingerprint: fingerprint,
                ...(result.workflow.ownerPrincipalId
                  ? {
                      currentApproverPrincipalIds: [
                        result.workflow.ownerPrincipalId,
                      ],
                    }
                  : {}),
      ...
      return options.transactions.run(
        ...
        async (transaction) => {
          ...
          if (current.status !== "applied") {
            ...
            if (!volatileValidation.valid)
              throw new MasterDataError(
                409,
                "BUSINESS_PARTNER_REQUEST_VOLATILE_VALIDATION_FAILED",
                "Request validity changed after approval; review and approval must be repeated",
              );
          }
          ...
          if (!result)
            throw new MasterDataError(
              409,
              "BUSINESS_PARTNER_REQUEST_APPLICATION_CONFLICT",
              "Request version, approval, validation, duplicate, source, or organization evidence changed before materialization",
            );
          const appliedEvent =
            current.kind === "deactivate"
              ? "business_partner.lifecycle.deactivated"
              : current.kind === "reactivate"
                ? "business_partner.lifecycle.reactivated"
                : current.kind === "archive"
                  ? "business_partner.lifecycle.archived"
                  : current.kind === "change_bank"
                    ? "business_partner.bank_verification.requested"
                    : current.kind === "activate_supplier"
          ...
          if (!result.replayed)
            await effects(
              ...
              result.request,
              {
                ...
                ...(result.materialization.supplierId
                  ? { supplierId: result.materialization.supplierId }
                  : {}),
                ...(result.materialization.customerId
                  ? { customerId: result.materialization.customerId }
                  : {}),
                operatingOrganizationAssignmentId:
                  result.materialization.operatingOrganizationAssignmentId,
                snapshotId: result.materialization.snapshotId,
                ...(result.materialization.bankVerificationId
                  ? {
                      bankVerificationId:
                        result.materialization.bankVerificationId,
                    }
                  : {}),
                ...(result.materialization.activationEvidenceId
                  ? {
                      activationEvidenceId:
                        result.materialization.activationEvidenceId,
                    }
                  : {}),
                ...(result.materialization.reasonCode
                  ? { reasonCode: result.materialization.reasonCode }
                  : {}),
                applicationFingerprint:
                  result.materialization.applicationFingerprint,
              },
            );
          return result;
        },
...
async function authorizeCaseMutation(
  ...
  if (
    request.source.kind === "portal" &&
    request.registrationMode === "self_service" &&
    request.applicantPrincipalId === context.principalId &&
    ...
    await authorize(
        ...
        : "neon.customer_registration.external.respond",
      {
        ...
        restrictedSessionRequired: true,
...
function assertContext(context: VerifiedRequestContext): void {
  if (context.planeKey !== "neon")
    throw new MasterDataError(
      400,
      "BUSINESS_PARTNER_REQUEST_NEON_REQUIRED",
      "Business Partner requests execute only in NEON",
    );
}
function validateCreate(command: CreateBusinessPartnerRequestCommand): void {
  ...
  if (
    ...
    throw invalid(
      "idempotencyKey must be trimmed and contain 8 to 200 characters",
    );
  if ((command.kind === "new_partner") !== !command.targetBusinessPartnerId)
    throw invalid(
      "new_partner must not have a target; every other request kind requires one",
    );
  if (command.kind === "add_supplier" && command.requestedRole !== "supplier")
    throw invalid("add_supplier requires requestedRole supplier");
  ...
  if (
    ...
    throw invalid("activate_supplier requires requestedRole supplier");
  if (
    ["amend_partner", "deactivate", "reactivate", "archive"].includes(
      command.kind,
    ) &&
    command.requestedRole
  )
    throw invalid(
      "Partner amendment and lifecycle requests must not claim a role result",
    );
  if (command.kind === "configure_company" && !command.companyCodeId)
    throw invalid("configure_company requires companyCodeId");
  const registrationMode =
    command.registrationMode ?? defaultRegistrationMode(command.source.kind);
  if (
    registrationMode === "self_service" &&
    (command.source.kind !== "portal" ||
      !command.invitationId ||
      !command.applicantPrincipalId ||
      command.representedPartyName ||
      command.representationEvidenceId)
  )
    throw invalid(
      "Self-service registration requires portal source, invitation, and applicant coordinates only",
    );
  if (
    registrationMode === "on_behalf" &&
    (command.source.kind !== "manual" ||
      !command.representedPartyName?.trim() ||
      command.invitationId ||
      command.applicantPrincipalId)
  )
    throw invalid(
      "On-behalf registration requires manual source and represented party coordinates",
    );
  if (
    registrationMode === "integration" &&
    !["mesh", "import", "api"].includes(command.source.kind)
  )
    throw invalid(
      "Integration registration requires mesh, import, or api source",
    );
  if (
    ...
    throw invalid(
      "Direct maintenance requires a manual source without representation coordinates",
    );
  if (
    command.source.kind === "mesh" &&
    (command.source.systemCode !== "athyper_mesh" ||
      !command.source.entityCode ||
      !command.source.entityId ||
      !command.source.projectionId ||
      !command.source.version ||
      !isHash(command.source.payloadHash))
  )
    throw invalid(
      "MESH source coordinates, pinned version, and payload hash are required",
    );
...
function validateValidationResult(
  result: Awaited<
    ReturnType<BusinessPartnerRequestValidator<unknown>["validate"]>
  >,
): void {
  ...
  if (
    ...
    throw new MasterDataError(
      503,
      "BUSINESS_PARTNER_REQUEST_VALIDATOR_INVALID",
      "Validator returned invalid evaluation coordinates",
    );
  for (const finding of result.findings) {
    if (
      ...
      finding.fieldPath.length > 512 ||
      Buffer.byteLength(JSON.stringify(finding.evidenceReference), "utf8") >
        65_536
    )
      throw new MasterDataError(
        503,
...
function validatePayload(payload: Readonly<Record<string, unknown>>): void {
  if (!payload || Array.isArray(payload) || typeof payload !== "object")
    throw invalid("proposedPayload must be an object");
  if (Buffer.byteLength(JSON.stringify(payload), "utf8") > 1_048_576)
    throw invalid("proposedPayload exceeds 1 MiB");
}
...
function validateExtensions(value: BusinessPartnerRequestExtensions, draft = false): void {
  validateBusinessPartnerProfile(value,draft);
  validateRequestCapture(value,draft);
  ...
  for (const [group, rows] of groups) {
    if (!Array.isArray(rows) || rows.length > 100)
      throw invalid(`${group} must contain at most 100 typed items`);
    total += rows.length;
    for (const raw of rows) {
      if (!raw || typeof raw !== "object" || Array.isArray(raw))
        throw invalid(`${group} contains an invalid typed item`);
      const item = raw as unknown as Record<string, unknown>,
        key = item["clientItemKey"],
        field = item["definitionFieldCode"];
      if (
        typeof key !== "string" ||
        !/^[A-Za-z0-9][A-Za-z0-9_.:-]{0,99}$/.test(key) ||
        itemKeys.has(`${group}:${key}`)
      )
        throw invalid(`${group} clientItemKey is invalid or duplicated`);
      itemKeys.add(`${group}:${key}`);
      if (typeof field !== "string" || !/^[a-z][a-z0-9_.-]{1,126}$/.test(field))
        throw invalid(`${group} definitionFieldCode is invalid`);
      if (group === "contactPersons") contacts.add(key);
      for (const name of ["normalizedHash", "valueHash"]) {
        if (
          item[name] !== undefined &&
          (typeof item[name] !== "string" ||
            !/^[a-f0-9]{64}$/.test(item[name] as string))
        )
          throw invalid(`${group}.${name} must be SHA-256`);
      }
      for (const name of ["protectedValueToken", "certificateNumberToken"]) {
        if (
          item[name] !== undefined &&
          (typeof item[name] !== "string" ||
            !/^[A-Za-z0-9._:-]{8,512}$/.test(item[name] as string))
        )
          throw invalid(
            `${group}.${name} must be an opaque protected-data token`,
          );
      }
      for (const [name, fieldValue] of Object.entries(item))
        if (typeof fieldValue === "string" && fieldValue.length > (name === "notes" ? 4000 : 2048))
          throw invalid(`${group}.${name} exceeds the typed field limit`);
    }
  }
  if (total > 300)
    throw invalid("Typed request extensions must contain at most 300 items");
  for (const channel of value.contactChannels ?? [])
    if (!contacts.has(channel.contactClientItemKey))
      throw invalid(
        "Every contact channel must reference a contact person in the same request",
      );
  for (const identifier of value.identifiers ?? []) {
    if ((!draft || identifier.value || identifier.protectedValueToken) && Boolean(identifier.value) === Boolean(identifier.protectedValueToken))
      throw invalid(
        "Identifier requires exactly one ordinary value or protected token",
      );
    const scheme = (identifier.schemeCode??"").replace(/[_-]/g, "").toLowerCase();
    if (
      identifier.value &&
      [
        "nationalid",
      ...
      throw invalid(
        "Restricted identifier schemes require an opaque protected token",
      );
  }
  for (const tax of value.taxRegistrations ?? [])
    if ((!draft || tax.protectedValueToken || tax.maskedValue) && (!tax.protectedValueToken || !tax.maskedValue))
      throw invalid(
        "Tax registration requires a protected token and masked presentation",
      );
  for (const tax of value.taxRegistrations ?? [])
    if (tax.protectedValueToken?.length > 128)
      throw invalid(
        "Tax registration protected token exceeds the canonical field limit",
      );
  for (const classification of value.classifications ?? [])
    if (
      ...
      throw invalid(
        "Classification confidence must be an integer from 0 to 100",
      );
  for (const rows of groups.map(([, items]) => items))
    for (const item of rows) {
      if (
        item.effectiveFrom &&
        item.effectiveUntil &&
        item.effectiveUntil <= item.effectiveFrom
      )
        throw invalid(
          "Typed extension effectiveUntil must be later than effectiveFrom",
        );
    }
  for (const certificate of value.certifications ?? [])
    if (
      (!draft || certificate.certificationTypeId || certificate.customName) && Boolean(certificate.certificationTypeId) ===
      Boolean(certificate.customName)
    )
      throw invalid(
        "Certification requires exactly one registered type or custom name",
      );

  const required = (field: unknown, name: string, max = 512): string => {
    if(draft && (field===undefined||field===null||field===""))return "";
  ...
  const uuid = (field: unknown, name: string): void => {
    if(draft && (field===undefined||field===null||field===""))return;
    if (
      typeof field !== "string" ||
      !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(
  ...
  const code = (field: unknown, name: string): void => {
    if(draft && (field===undefined||field===null||field===""))return;
    if (typeof field !== "string" || !/^[a-z][a-z0-9_.-]{1,62}$/.test(field))
      throw invalid(`${name} must be a lowercase code`);
  };
  for (const item of value.addresses ?? []) {
    code(item.purpose, "Address purpose");
    ...
    if (item.addressKind && !["street","po_box","rural","military","other"].includes(item.addressKind)) throw invalid("Invalid address kind");
    for (const key of ["buildingName","floor","unit","houseNumber","streetName","poBox"] as const) {
      if (item[key] !== undefined && (typeof item[key] !== "string" || item[key]!.length > 160)) throw invalid(`Address ${key} must be text of at most 160 characters`);
    }
    if (item.addressKind === "po_box") required(item.poBox, "Address poBox", 160);
    if (item.addressKind !== "po_box" && item.poBox) throw invalid("PO box requires PO box address kind");
    if (item.addressKind && item.addressKind !== "street" && [item.buildingName,item.floor,item.unit,item.houseNumber,item.streetName].some(Boolean)) throw invalid("Building and street details require street address kind");
    if (item.validationEvidenceId)
      uuid(item.validationEvidenceId, "Address validationEvidenceId");
  }
  for (const item of value.contactPersons ?? []) {
    required(item.contactName, "Contact name", 240);
    if (item.roleCode) code(item.roleCode, "Contact roleCode");
  }
  for (const item of value.contactChannels ?? []) {
    required(
    ...
    if (
      !["email", "phone", "fax", "sms", "whatsapp", "website"].includes(
        item.channelType,
      )
    )
      throw invalid("Contact channel type is invalid");
    required(item.value, "Contact channel value", 512);
    code(item.purpose, "Contact channel purpose");
  }
  for (const item of value.identifiers ?? []) {
    code(item.schemeCode, "Identifier schemeCode");
    required(item.valueHash, "Identifier valueHash", 64);
    required(item.maskedValue, "Identifier maskedValue", 256);
  }
  for (const item of value.taxRegistrations ?? []) {
    uuid(item.jurisdictionId, "Tax jurisdictionId");
    if (item.taxTypeId) uuid(item.taxTypeId, "Tax taxTypeId");
    code(item.registrationTypeCode, "Tax registrationTypeCode");
    required(item.valueHash, "Tax valueHash", 64);
  }
  for (const item of value.classifications ?? []) {
    uuid(item.referenceId, "Classification referenceId");
    if ((!draft||item.classificationKind) && !["commodity", "industry"].includes(item.classificationKind))
      throw invalid("Classification kind is invalid");
    if (
      item.classificationKind === "industry" &&
      !(["isic", "naics"] as const).includes(
        item.domainCode as "isic" | "naics",
      )
    )
      throw invalid("Industry classification domainCode must be isic or naics");
  }
  for (const item of value.certifications ?? []) {
    if (item.certificationTypeId)
      uuid(item.certificationTypeId, "Certification certificationTypeId");
    if (item.attachmentId)
      uuid(item.attachmentId, "Certification attachmentId");
    if (item.companyCodeId)
      uuid(item.companyCodeId, "Certification companyCodeId");
  }
}
function validateExtensionApplicability(
  kind: CreateBusinessPartnerRequestCommand["kind"],
  _role: CreateBusinessPartnerRequestCommand["requestedRole"],
  extensions: BusinessPartnerRequestExtensions,
): void {
  if (kind !== "new_partner" && [extensions.aliases, extensions.governanceRelations, extensions.relationships].some(rows => rows?.length))
    throw invalid("Full profile collections require a new partner request");
  const count = Object.values(extensions).reduce(
  ...
  if (
    ...
    throw invalid(
      "Typed identity extensions are not applicable to this request kind",
    );
}
...
function validateDecisionCommand(
  command: DecideBusinessPartnerRequestCommand,
): void {
  ...
  if (
    !Number.isSafeInteger(command.expectedWorkItemVersion) ||
    command.expectedWorkItemVersion < 1
  )
    throw invalid("expectedWorkItemVersion must be a positive integer");
  if (
    !command.reason.trim() ||
...
function assertRoleMaterializable(request: BusinessPartnerRequest): void {
  if (request.status !== "approved" && request.status !== "applied")
    throw new MasterDataError(
      409,
      "BUSINESS_PARTNER_REQUEST_NOT_APPLICABLE",
      "Only an approved request can be materialized",
    );
  const supported =
    ...
    (request.kind === "add_supplier" &&
      ...
      request.requestedRole === "supplier") ||
    (request.kind === "add_customer" &&
      Boolean(request.targetBusinessPartnerId) &&
      request.requestedRole === "customer") ||
    (request.kind === "assign_organization" &&
      Boolean(request.targetBusinessPartnerId) &&
      (request.requestedRole === "supplier" ||
        request.requestedRole === "customer")) ||
    (request.kind === "configure_company" &&
      Boolean(request.targetBusinessPartnerId) &&
      Boolean(request.companyCodeId) &&
      (request.requestedRole === "supplier" ||
        request.requestedRole === "customer")) ||
    (request.kind === "change_bank" &&
      ...
      Boolean(request.companyCodeId) &&
      request.requestedRole === "supplier") ||
    (request.kind === "activate_supplier" &&
      Boolean(request.targetBusinessPartnerId) &&
      request.requestedRole === "supplier") ||
    (["amend_partner", "deactivate", "reactivate", "archive"].includes(
      request.kind,
  ...
  if (!supported)
    throw new MasterDataError(
      409,
      "BUSINESS_PARTNER_REQUEST_KIND_UNSUPPORTED",
      "The request kind, target, role, and scope combination has no application authority",
    );
  if (
    !request.approvedAt ||
    !request.approvedBy ||
    !request.decisionFingerprint ||
    (["supplier", "customer"].includes(request.requestedRole ?? "") &&
      !request.operatingOrganizationId)
  )
    throw new MasterDataError(
      409,
      "BUSINESS_PARTNER_REQUEST_APPROVAL_EVIDENCE_INVALID",
      "Pinned approval and request-kind-specific scope evidence are required",
    );
  assertPassedValidation(request);
}
function assertPassedValidation(request: BusinessPartnerRequest): void {
  if (request.validationSummary["outcome"] !== "passed")
    throw new MasterDataError(
      409,
      "BUSINESS_PARTNER_REQUEST_VALIDATION_REQUIRED",
      "A current passing validation evaluation is required before submission",
    );
  if (request.duplicateSummary["blocking"] === true)
    throw new MasterDataError(
      409,
      "BUSINESS_PARTNER_REQUEST_DUPLICATE_BLOCKED",
      "Blocking duplicate evidence must be resolved before submission",
    );
}
function assertDecisionTask(
  workflow: BusinessPartnerRequestView["workflow"],
  command: DecideBusinessPartnerRequestCommand,
): void {
  if (
    !workflow ||
    workflow.requestId !== command.workflowRequestId ||
    workflow.workItemId !== command.workItemId
  )
    throw new MasterDataError(
      409,
      "BUSINESS_PARTNER_REQUEST_TASK_MISMATCH",
      "Decision does not address the current workflow task",
    );
  if (
    !Number.isSafeInteger(workflow.workItemVersion) ||
    workflow.workItemVersion !== command.expectedWorkItemVersion
  )
    throw new MasterDataError(
      409,
      "BUSINESS_PARTNER_REQUEST_TASK_VERSION_CONFLICT",
      "Workflow task version changed before the decision",
    );
  if (!["open", "claimed"].includes(workflow.workItemStatus))
    throw new MasterDataError(
      409,
      "BUSINESS_PARTNER_REQUEST_TASK_NOT_OPEN",
      "Workflow task is no longer open for decision",
    );
  if (workflow.ownerPrincipalId !== command.context.principalId)
    throw new MasterDataError(
      403,
      "BUSINESS_PARTNER_REQUEST_TASK_OWNER_REQUIRED",
      "Only the current workflow task owner may decide this request",
    );
}
function validateWorkflowDefinition(
  ...
  const stages = definition.stages?.filter((stage) => stage.routed) ?? [
    {
      code: definition.stageCode,
      name: definition.stageName,
      mode: "parallel" as const,
      quorum: { kind: "any" as const },
  ...
  if (!stages.length)
    throw new MasterDataError(
      409,
      "BUSINESS_PARTNER_REQUEST_WORKFLOW_ROUTE_EMPTY",
      "Configured workflow conditions selected no approval stage",
    );
  for (const stage of stages) {
    const approvers = stage.approverPrincipalIds;
    if (
      !/^[A-Za-z][A-Za-z0-9_.-]{0,62}$/.test(stage.code) ||
      !stage.name.trim() ||
      approvers.length === 0 ||
      new Set(approvers).size !== approvers.length ||
      approvers.includes(submitterId) ||
      approvers.some((id) => !/^[0-9a-f-]{36}$/i.test(id))
    )
      throw new MasterDataError(
        409,
        "BUSINESS_PARTNER_REQUEST_APPROVER_UNAVAILABLE",
        `Workflow stage ${stage.code} requires at least one distinct eligible approver`,
      );
    const value = stage.quorum.value;
    if (
      !["all", "any", "count", "percentage"].includes(stage.quorum.kind) ||
      (stage.quorum.kind === "count" &&
        (!Number.isInteger(value) ||
          value! < 1 ||
          value! > approvers.length)) ||
      (stage.quorum.kind === "percentage" &&
        (!Number.isFinite(value) || value! <= 0 || value! > 100))
    )
      throw new MasterDataError(
        409,
        "BUSINESS_PARTNER_REQUEST_QUORUM_INVALID",
        `Workflow stage ${stage.code} has an invalid quorum`,
      );
    if (
      stage.slaMinutes !== undefined &&
      (!Number.isInteger(stage.slaMinutes) || stage.slaMinutes < 1)
    )
      throw new MasterDataError(
        409,
        "BUSINESS_PARTNER_REQUEST_SLA_INVALID",
        `Workflow stage ${stage.code} has an invalid SLA`,
      );
  }
}
function placeholderDefinition(): BusinessPartnerRequestWorkflowDefinition {
  return {
    code: "replay",
    version: 1,
    hash: "0".repeat(64),
...
function invalid(message: string): MasterDataError {
...
async function authorize(
  ...
  const decision = await authorizer.authorize({
    ...
    observation: {
      entityCode: "business_partner",
      surface: "command",
      phase: permissionCode.endsWith(".read") ? "discover" : "execute",
    },
  });
  if (!decision.allowed) {
    if (decision.reason === "mfa_required")
      throw new MasterDataError(
        403,
        "BUSINESS_PARTNER_REQUEST_STEP_UP_REQUIRED",
        `MFA verification is required for ${permissionCode}`,
      );
    throw new MasterDataError(
      403,
      "FORBIDDEN",
...
function withoutContext(
  command: CreateBusinessPartnerRequestCommand,
): Omit<CreateBusinessPartnerRequestCommand, "context"> {
  const { context: _context, ...result } = command;
  return result;
}
...
function commandFingerprint(
  command: CreateBusinessPartnerRequestCommand,
  schema: BusinessPartnerRequest["schema"],
): string {
  return hash({
    kind: command.kind,
    ...
    applicantPrincipalId: command.applicantPrincipalId,
    representedPartyName: command.representedPartyName,
    representationEvidenceId: command.representationEvidenceId,
    targetBusinessPartnerId: command.targetBusinessPartnerId,
    requestedRole: command.requestedRole,
    operatingOrganizationId: command.operatingOrganizationId,
    companyCodeId: command.companyCodeId,
    proposedPayload: command.proposedPayload,
...
function creationFingerprint(request: BusinessPartnerRequest): string {
  return hash({
    kind: request.kind,
...
): BusinessPartnerRequest["registrationMode"] {
...
function submissionFingerprint(
  ...
  return hash({
    ...
    schema: request.schema,
    proposedPayload: request.proposedPayload,
    extensionSummary: request.extensionSummary,
    validationSummary: request.validationSummary,
    duplicateSummary: request.duplicateSummary,
...
function decisionFingerprint(
  ...
  return hash({
    ...
    requestFingerprint: request.decisionFingerprint,
...
function applicationFingerprint(
  request: BusinessPartnerRequest,
  command: ApplyBusinessPartnerRequestCommand,
): string {
  return hash({
    requestId: request.id,
    requestKind: request.kind,
    baseRecordVersion: request.baseRecordVersion,
    expectedVersion: command.expectedVersion,
    applicationIdempotencyKey: command.idempotencyKey,
    decisionFingerprint: request.decisionFingerprint,
    approvedAt: request.approvedAt,
    approvedBy: request.approvedBy,
    schema: request.schema,
    proposedPayload: request.proposedPayload,
    extensionFingerprint: request.extensionSummary.fingerprint,
...
function withoutDecisionContext(
  command: DecideBusinessPartnerRequestCommand,
): Omit<DecideBusinessPartnerRequestCommand, "context"> {
  const { context: _context, ...result } = command;
  return result;
}
function withoutApplyContext(
  command: ApplyBusinessPartnerRequestCommand,
): Omit<ApplyBusinessPartnerRequestCommand, "context"> {
  const { context: _context, ...result } = command;
  return result;
}
async function effects<Transaction>(
  ...
  const safeMetadata = {
    extensionMode: request.extensionSummary.mode,
    extensionCounts: request.extensionSummary.counts,
    ...metadata,
  };
  ...
  await options.outbox.append(
    {
      tenantId: context.tenantId,
      topic: "business-partner-governed-case",
      eventType: eventCode,
      ...(notification
        ? { eventKey: notification.event.deduplicationKey }
        : {}),
      ...
      payload: {
        ...
        ...(notification
          ? {
              ...
              recipient_principal_ids: notification.recipientPrincipalIds,
            }
          : {}),
      },
    },
  ...
  const action = eventCode.endsWith("created")
    ? "create"
    : eventCode.endsWith("approved")
      ? "approve"
...
function caseScope(
  ...
  return {
    ...scope(request.operatingOrganizationId, request.companyCodeId),
    tenantId: request.tenantId,
    entityCode: "entity_case",
