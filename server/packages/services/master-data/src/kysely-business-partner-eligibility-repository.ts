...
export class KyselyBusinessPartnerEligibilityRepository implements BusinessPartnerEligibilityRepository<Tx> {
  ...
  async activateSupplier(
    ...
    if (input.expectedSupplierVersion !== undefined && Number(supplier.record_version) !== input.expectedSupplierVersion)
      throw new MasterDataError(409,"SUPPLIER_ACTIVATION_VERSION_CHANGED","Supplier version changed before activation");
    const updated = (
