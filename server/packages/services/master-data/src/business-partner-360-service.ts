...
} from "./business-partner-provider-projection.js";
...
export interface BusinessPartner360Repository<Transaction> {
  ...
  readCommercialControlSection(
    input: {
      readonly authorizeCompany?: (companyCodeId: string) => Promise<boolean>;
  ...
  claimRestrictedReveal?(
    input: {
      ...
      readonly commandCode:
        "business_partner.360.tax_reveal" | "business_partner.360.bank_reveal";
...
export function createBusinessPartner360Service<Transaction>(options: {
  ...
  const resolve = async (
    ...
    try {
      await options.admitDirectoryRecord?.(query);
    } catch (error) {
    ...
    // Directory admission has already enforced the published row scope. An
    // organization-bound grant must not re-impose shell context on that record.
    // Explicit denials and section/reveal permissions never use this fallback.
    if (
      !visible.allowed &&
    ...
    if (!core || core.category !== "organization") throw notFound();
    if (!core.scopeValid)
      throw new MasterDataError(
        400,
        "BP_360_SCOPE_INVALID",
  ...
  const service: BusinessPartner360Service & {
    ...
    async summary(query) {
      return options.transactions.run(
        ...
        async (transaction) => {
          const { core, roleLens, asOf, directoryAdmitted } = await resolve(
              query,
              transaction,
            ),
            coordinates = {
          ...
          try {
            ...
            /* Completeness fails closed; canonical identity remains available. */
          }
          ...
          let presentation:
            | import("@athyper/contract-platform-entity-runtime").EntityRecordPresentationV1
            | undefined;
          try {
            presentation = (
              await options.metadata?.getEntityDescriptor(
            ...
            /* Canonical summary remains available during metadata outages. */
          }
          const canComment =
        ...
        async (transaction) => {
          ...
          if (query.sectionCode === "network") {
            ...
            return {
              ...
              definitionHash,
              state,
              data: { local: local.data, live } as never,
              provenance: local.provenance,
              redactions: [
                {
          ...
          if (isCommercialControlSection(query.sectionCode)) {
            const result =
              await options.repository.readCommercialControlSection(
                {
                  ...bankCompanyVisibility(query),
                  authorizeCompany: async (companyCodeId) =>
                    (
                      await options.authorizer.authorize({
                        context: query.context,
                        permissionCode:
            ...
            return {
              ...
              data: (query.sectionCode === "qualifications-certificates"
                ? authorizedCertificates(result.data, policy.granted)
                : query.sectionCode === "banking"
                  ? authorizedBankAccounts(result.data, policy.granted)
  ...
  return {
    ...
    async section<T = unknown>(
      ...
      query = await freshQuery(query);
      const result = await service.section<T>(query);
      if (
        !options.authorizer.enforcedEntityProfile?.("neon", "business_partner")
      )
        return result;
    ...
    async preflightReveal(query) {
      if (
        ...
        !["bank", "tax"].includes(query.kind)
      )
        return "not_applicable";
      if (
        query.historical ||
        (query.kind === "tax" && query.context.assurance !== "elevated")
      )
  ...
  const base = `/mdg/business-partner/${encodeURIComponent(businessPartnerId)}`,
    ...
    definitions: readonly Readonly<{
      ...
      },
      {
        code: "change_bank",
        label: "Change bank",
        href: `${base}?section=banking&action=change-bank`,
        authority: "entity_case",
        permission: requestPermission,
        requestKind: "change_bank",
      },
      {
        code: "lifecycle",
...
function authorizedCertificates(
  value: unknown,
  ...
  return {
    ...
    ...(!permissions.has(BUSINESS_PARTNER_360_PERMISSIONS.qualification)
      ...
      : {}),
    certifications: certifications.map((item: Record<string, unknown>) => {
      if (permissions.has(BUSINESS_PARTNER_360_PERMISSIONS.attachment))
        return item;
...
function bankCompanyVisibility(query: BusinessPartner360Query): {
  authorizedCompanyIds?: readonly string[];
} {
  if (query.companyCodeId)
    return { authorizedCompanyIds: [query.companyCodeId] };
  const scopes = query.context.permissions.authorizationScopes;
  if (
    !scopes ||
    scopes.some(
      (scope) =>
        scope.permissionCode === BUSINESS_PARTNER_360_PERMISSIONS.bankMasked &&
        scope.tenantWide,
    )
  )
    return {};
    ...
    authorizedCompanyIds: [
      ...new Set(
        scopes
          ...
          .flatMap((scope) => scope.companyCodeIds),
      ),
    ],
  };
}
...
function revealScope(command: {
  ...
  return command.operatingOrganizationId && command.companyCodeId
    ? {
