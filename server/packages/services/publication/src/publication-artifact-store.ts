...
export class ImmutablePublicationArtifactStore implements PublicationArtifactStore {
  constructor(private readonly options: PublicationArtifactStoreOptions) {
    if (!options.bucket.trim())
      throw new TypeError("Publication artifact bucket is required");
    if (!options.storage.putIfAbsent)
      throw new TypeError(
        "Publication artifact storage requires atomic putIfAbsent support",
      );
  }
  ...
  async putImmutable(input: {
    ...
    const created = await this.options.storage.putIfAbsent!(
      input.key,
      input.bytes,
      {
  ...
  async get(input: {
    ...
    const bytes = await this.options.storage.get(
      parseArtifactUri(input.uri, this.options.bucket),
    );
    if (this.options.canonicalizer.sha256(bytes) !== input.expectedSha256) {
      throw conflict(
        "Stored artifact checksum does not match its authority record",
      );
    }
    return bytes;
  }
  ...
  async health(): Promise<{
    ...
    try {
      await this.options.storage.exists(
        this.options.healthProbeKey ?? "publication/.health-access-probe",
      );
      return { healthy: true };
    } catch {
      return {
        healthy: false,
        message: "Publication object storage is unavailable",
      };
    }
  }
}
...
export function publicationArtifactKey(input: {
  ...
  const tenantId = requireSegment(input.tenantId, "tenant ID");
  const publicationKey = requireSegment(
    input.publicationKey,
    "publication key",
  );
  const releaseId = requireSegment(input.releaseId, "release ID");
  if (!Number.isSafeInteger(input.releaseNo) || input.releaseNo < 1)
    throw new TypeError("Release number must be a positive safe integer");
  if (!/^[a-f0-9]{64}$/.test(input.contentHash))
    throw new TypeError("Artifact content hash must be lowercase SHA-256");
  const artifactKind = requireSegment(
    input.artifactKind ?? "entity_runtime",
    "artifact kind",
  ).replaceAll("_", "-");
  return `publication/${input.targetPlane}/${tenantId}/v1/${publicationKey}/releases/${input.releaseNo}-${releaseId}/${artifactKind}-${input.contentHash}.json`;
}
...
function parseArtifactUri(uri: string, expectedBucket: string): string {
  ...
  if (!match || match[1] !== expectedBucket || !match[2]) {
    throw new PublicationContractError(
      ...
      "Publication artifact URI is invalid",
    );
  }
  return match[2];
}
...
function conflict(message: string): PublicationContractError {
  return new PublicationContractError(
