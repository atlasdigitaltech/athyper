...
export interface KyselyPublicationAuthorityWorkOptions {
  ...
  readonly authorizationCompilation?: {
    ...
    ): Promise<EntityAuthorizationPublicationInput["catalog"]>;
  };
}
...
export class KyselyPublicationAuthorityWork implements PublicationAuthorityWork {
  ...
  compile(id: string) {
    return this.scoped((worker) => worker.compileScoped(id));
  }
  sign(id: string) {
    return this.scoped((worker) => worker.signScoped(id));
  }
  dispatch(id: string) {
    return this.scoped((worker) => worker.dispatchScoped(id));
  }
  ...
  private async compileScoped(
    ...
    if (!result.rows.length) {
      ...
      WHERE pr.id=${releaseId}::uuid AND pr.status IN ('approved','published') ORDER BY a.plane_key`.execute(
          this.options.database,
        );
      artifactKind = "entity_runtime";
    }
    ...
    const selected = result.rows.filter((row) =>
      this.options.targetPlanes.includes(planeValue(row["plane_key"])),
    );
    if (
      selected.length !==
      (caseContract ||
    ...
    for (const sourceRow of selected) {
      ...
      if (
        ...
        row = {
          ...row,
          contract_hash: this.options.canonicalizer.sha256(contractBytes),
          compiled_hash: this.options.canonicalizer.sha256(
            this.options.canonicalizer.canonicalBytes(row["compiled_json"]),
          ),
          contract_signature: signed.signature,
          signature_algorithm: "Ed25519",
          contract_signing_key_id: this.options.signingKeyId,
        };
      }
      ...
      if (
        artifactKind === "entity_runtime" &&
        object(row, "compiled_json")["collectionRelationship"] !== undefined &&
        object(row, "compiled_json")["entity"] !== undefined
      ) {
        if (!this.options.authorizationCompilation)
          throw permanent("DOCUMENT_COLLECTION_CATALOG_UNAVAILABLE");
        const descriptor = withDocumentCollectionSource(
          compileDocumentCollection(
            object(row, "compiled_json"),
            plane,
            await this.options.authorizationCompilation.catalog(plane),
          ),
          {
            entityId: string(row, "entity_id"),
        ...
        const contractHash = this.options.canonicalizer.sha256(contractBytes);
        parseEntityRuntimeDescriptor({
          entity_code: string(row, "entity_code"),
          release_id: releaseId,
          release_no: number(row, "release_no"),
        ...
        consumerRow = {
          ...
          compiled_json: descriptor,
          compiled_hash: hash,
          native_descriptor_hash: string(row, "compiled_hash"),
          native_contract_hash: string(row, "contract_hash"),
      ...
      if (
        ...
        if (!configuration)
          throw permanent("ENTITY_AUTHORIZATION_COMPILER_UNAVAILABLE");
        if (row["release_kind"] !== "publish")
          throw permanent(
            "ENTITY_AUTHORIZATION_ROLLBACK_REQUIRES_COMPATIBLE_ARTIFACT",
          );
        const native = buildUnsigned(
          row,
          plane,
          this.options.signingKeyId,
          this.options.canonicalizer,
        ...
        if (!Array.isArray(authored))
          throw permanent("ENTITY_AUTHORIZATION_AUTHORED_OPERATIONS_REQUIRED");
        ...
        for (const operation of authored) {
          ...
          if (
            ...
            Object.hasOwn(operationIds, operation.operationKey)
          )
            throw permanent("ENTITY_AUTHORIZATION_AUTHORED_OPERATIONS_INVALID");
          operationIds[operation.operationKey] = operation.id;
        }
        const compiled = await compileEntityAuthorizationPublication({
          projection: native.envelope.payload as EntityRuntimeProjection,
          operationIds,
          catalog: await configuration.catalog(plane),
      ...
      const id = stableUuid(
        `publication-compilation:${releaseId}:${plane}:${artifactKind}`,
      );
      ...
      const replay =
        await sql<Row>`SELECT id,unsigned_hash FROM publication.artifact_compilation
        WHERE publication_release_id=${releaseId}::uuid AND plane_code=${plane} AND artifact_kind=${artifactKind}`.execute(
          this.options.database,
        );
  ...
  private async signScoped(
    ...
    if (
      projection?.entityDescriptor?.descriptor["authorizationRuntime"] !==
      undefined
    ) {
      const configuration = this.options.authorizationCompilation;
      if (!configuration?.review)
        throw permanent("ENTITY_AUTHORIZATION_SIGNING_REVIEW_REQUIRED");
      const c = projection.entityContract,
        d = projection.entityDescriptor;
      const profile = d.descriptor["authorization"] as {
        operations: readonly { key: string }[];
      };
      ...
      const review = await configuration.review.qualify({
        ...
        catalogHash: hash(await configuration.catalog(d.plane)),
        operationKeys: profile.operations.map((o) => o.key).sort(),
      });
      const evidence = (unsigned as any).manifest?.evidence;
      if (review.receiptSha256 !== evidence?.authorizationReviewReceiptSha256)
        throw permanent("ENTITY_AUTHORIZATION_SIGNING_REVIEW_CHANGED");
    }
    const signed = await this.options.signer.sign({
      keyId: this.options.signingKeyId,
      algorithm: "Ed25519",
    ...
    } as unknown as PublicationArtifactDocumentV1;
    ...
    const artifactId = stableUuid(
      `publication-artifact:${releaseId}:${plane}:${artifactKind}`,
    );
    const key = publicationArtifactKey({
    ...
    await this.options.store.putImmutable({
      key,
      bytes,
      contentType: PUBLICATION_ARTIFACT_MEDIA_TYPE_V1,
      sha256: contentHash,
      metadata: {
        releaseId,
        targetPlane: plane,
    ...
    const artifact = await this.options.authority.createArtifact({
      ...
      plane,
      artifactKind,
      artifactUri: publicationArtifactUri(this.options.bucket, key),
      contentHash,
      actorId: string(row, "created_by"),
    });
    if (artifact.status === "compiled")
      await this.options.authority.transitionArtifact({
        artifactId,
        status: "validated",
      });
    await this.options.authority.transitionArtifact({
    ...
    const deployment = await this.options.authority.createDeployment({
      commandId,
      artifactId,
      targetPlane: plane,
  ...
  private async dispatchScoped(
    deploymentId: string,
  ): Promise<PublicationCoordinatePayload> {
    ...
    if (deployment.deploymentStatus === "pending")
      await this.options.authority.transitionDeployment({
        deploymentId,
        status: "dispatched",
        evidence: { authorityWorker: "publication.v1" },
      });
  ...
  private async assertActivationApproved(deploymentId: string): Promise<void> {
    const rows = (
      ...
      WHERE d.id=${deploymentId}::uuid AND c.unsigned_document #> '{envelope,payload,entityDescriptor,descriptor,authorizationRuntime}' IS NOT NULL`.execute(
        this.options.database,
      )
    ).rows;
    if (rows.length) {
      if (!this.options.authorizeEntityActivation)
        throw permanent("ENTITY_AUTHORIZATION_ACTIVATION_APPROVAL_REQUIRED");
      await this.options.authorizeEntityActivation(
        string(rows[0]!, "publication_release_id"),
      );
    }
  }
...
function buildUnsigned(
  ...
  const payload = {
    entityContract: {
      ...
      releaseNo: number(row, "release_no"),
      contractSchemaCode: string(row, "contract_schema_code"),
      contractSchemaVersion: string(row, "contract_schema_version"),
      contractHash: string(row, "contract_hash"),
      contract: object(row, "contract_json"),
      publicationKey: string(row, "release_key"),
      signature: {
        algorithm: string(row, "signature_algorithm"),
        keyId: string(row, "contract_signing_key_id"),
    ...
    entityDescriptor: {
      ...
      compilerVersion: "athyper.entity-release-artifact/1.0.0",
  ...
  const evidence = {
    ...
    ...(row["native_descriptor_hash"]
      ? {
          ...
          nativeContractHash: string(row, "native_contract_hash"),
          documentCollectionCompiler: "registered-case-summary.v1",
        }
      : {}),
  };
  const manifest = {
    ...
    compiler: { name: "athyper.entity-release-artifact", version: "1.0.0" },
    contractSchemaVersion: string(row, "contract_schema_version"),
    descriptorSchemaVersion: "1.0.0",
    ...(row["minimum_runtime_version"]
      ? { minimumRuntimeVersion: string(row, "minimum_runtime_version") }
...
function required(row: Row | undefined, code: string) {
...
function string(row: Row, key: string) {
  ...
  if (typeof value !== "string" || !value)
    throw permanent(`PUBLICATION_SOURCE_INVALID_${key.toUpperCase()}`);
  return value;
}
...
function planeValue(value: unknown): PublicationPlane {
  if (value !== "studio" && value !== "neon" && value !== "mesh")
    throw permanent("PUBLICATION_SOURCE_PLANE_INVALID");
  return value;
}
function artifactKindValue(
  value: unknown,
