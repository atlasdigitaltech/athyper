...
const { KyselyPublicationAuthorityRepository } = await import(
  process.env.PUBLICATION_REPOSITORY_MODULE ?? "../index.ts"
);
...
try {
  const coordinate =
    await sql`SELECT id FROM publication.deployment WHERE command_id='40000000-0000-4000-8000-000000000001'::uuid`.execute(
      database,
    );
