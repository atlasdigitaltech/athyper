...
describe("immutable Publication artifact storage", () => {
  it("creates the required immutable key and stable URI", () => {
    ...
    expect(key).toBe(
      `publication/neon/tenant-1/v1/metadata.entity.invoice/releases/2-release-id/entity-runtime-${"a".repeat(64)}.json`,
    );
    expect(publicationArtifactUri("publication", key)).toBe(
      `s3://publication/${key}`,
    );
  });
  ...
  it("writes once and treats a same-checksum replay as success", async () => {
    ...
    const first = fixture();
    await first.store.putImmutable({
      key: "publication/item",
      bytes,
  ...
  it("rejects conflicting immutable writes and corrupted reads", async () => {
    ...
    await expect(
      conflicting.store.putImmutable({
        key: "publication/item",
        bytes: expected,
        contentType: "application/json",
    ...
    ).rejects.toMatchObject({ code: "ARTIFACT_STORAGE_IMMUTABILITY_CONFLICT" });
    await expect(
      conflicting.store.get({
        uri: "s3://publication/publication/item",
  ...
  it("rejects foreign buckets and reports health without writing data", async () => {
    ...
    await expect(
      f.store.get({
        uri: "s3://other/publication/item",
        expectedSha256: "a".repeat(64),
      }),
    ).rejects.toMatchObject({ code: "ARTIFACT_STORAGE_URI_INVALID" });
    await expect(f.store.health()).resolves.toEqual({ healthy: true });
    expect(f.storage.exists).toHaveBeenCalledWith(
      "publication/.health-access-probe",
    );
