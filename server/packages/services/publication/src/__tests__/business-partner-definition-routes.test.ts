...
describe("Studio definition approval boundary", () => {
  it("records authoring through the native audit validator before returning the saved revision", async () => {
    ...
    await s.invoke("/business-partner-definitions", { bundle: {}, targetPlanes: ["neon"] });
  ...
  it("returns a duplicate immutable version as conflict without another approval job", async () => {
    const s = setup();
    s.options.service.author.mockRejectedValue(Object.assign(new Error("database detail"), {
      code: "23505", constraint: "business_partner_definition_revision_version_uq",
    }));
    await s.invoke("/business-partner-definitions", { bundle: {}, targetPlanes: ["neon", "mesh"] });
