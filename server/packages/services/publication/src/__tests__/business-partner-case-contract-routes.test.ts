...
import { registerBusinessPartnerCaseContractRoutes } from "../business-partner-case-contract-routes.js";
...
function setup(allowed = true) {
  const options = {
    ...
    audit: { record: vi.fn(createAuditService({ sink: { append: vi.fn(async () => {}) } }).record) },
  ...
  registerBusinessPartnerCaseContractRoutes({} as any, options as any);
  const response = {
...
describe("Studio case-contract approval boundary", () => {
  it("records authoring through the native audit validator before returning the saved revision", async () => {
    const s = setup();
    s.options.service.author.mockResolvedValue({ id: revisionId, bundleHash: "hash" });
    await s.invoke("/business-partner-case-contracts", { bundle: {}, targetPlanes: ["neon"] });
    expect(s.options.audit.record).toHaveBeenCalledWith(expect.objectContaining({
      eventCode: "studio.business_partner_definition.author", action: "author", entityId: revisionId,
    }));
    expect(s.response.status).toHaveBeenCalledWith(201);
    expect(s.next).not.toHaveBeenCalled();
  });
  it("returns an immutable command conflict without another approval job", async () => {
    const s = setup();
    s.options.service.author.mockRejectedValue(new BusinessPartnerDefinitionError("CASE_CONTRACT_IDEMPOTENCY_CONFLICT"));
    await s.invoke("/business-partner-case-contracts", { bundle: {}, targetPlanes: ["neon", "mesh"] });
    expect(s.response.status).toHaveBeenCalledWith(409);
    expect(s.response.json).toHaveBeenCalledWith(expect.objectContaining({
      error: "CASE_CONTRACT_IDEMPOTENCY_CONFLICT",
    }));
  ...
  it("keeps unrelated database errors on the internal error path", async () => {
    const s = setup();
    const error = Object.assign(new Error("other constraint"), { code: "23505", constraint: "other" });
