...
describe("Publication authority jobs",()=>{
  it("persists each step before deterministically enqueueing the next",async()=>{
    const enqueued:unknown[][]=[];const jobs:JobPublisher={enqueue:vi.fn(async(...args:unknown[])=>{enqueued.push(args);return String((args[3] as {enqueueKey:string}).enqueueKey);})};
    ...
    expect((enqueued[2]?.[3] as {enqueueKey:string}).enqueueKey).toBe("publication:22222222-2222-4222-8222-222222222222:apply:neon:1");
  });
  it("carries tenant execution through compilation, signing and apply",async()=>{
    const execution={planeKey:"studio" as const,scope:"tenant" as const,tenantId:"tenant",principalId:"worker"};
    const enqueue=vi.fn(async()=>"transport-safe-id");
    const work:PublicationAuthorityWork={compile:async()=>({compilationIds:["compilation"]}),sign:async()=>({deploymentId:"deployment"}),dispatch:async()=>({deploymentId:"deployment",targetPlane:"neon"}),acknowledge:async()=>{},recoverStalled:async()=>[]};
    const handlers=createPublicationAuthorityHandlers(work,{enqueue});
    for(const [name,data] of [[COMPILE_PUBLICATION_ARTIFACT_JOB,{releaseId:"release"}],[SIGN_PUBLICATION_ARTIFACT_JOB,{compilationId:"compilation"}],[DISPATCH_PUBLICATION_JOB,{deploymentId:"deployment"}]] as const){
      await handlers[name]!.handle({...envelope(name,data),execution},context);
    }
    expect(enqueue.mock.calls.map(call=>(call as unknown as unknown[])[3])).toEqual([
      expect.objectContaining({execution,enqueueKey:expect.any(String)}),
      expect.objectContaining({execution,enqueueKey:expect.any(String)}),
      expect.objectContaining({execution:{...execution,planeKey:"neon"},enqueueKey:expect.any(String)}),
    ]);
  });
