...
import {
  ...
  type SesEventSqsClient,
} from "../ses-event-sqs.adapter.js";
...
describe("createSesEventSqsAdapter", () => {
  it("long polls a bounded batch and deletes only acknowledged messages", async () => {
    ...
    const adapter = createSesEventSqsAdapter(
      config(),
      { process },
      dependencies(send),
    );
    ...
    expect((deletes[0] as DeleteMessageCommand).input.ReceiptHandle).toBe(
      "receipt-1",
    );
  });
  ...
  it("leaves transient handler failures and invalid queue messages for redrive", async () => {
    const process = vi
      .fn()
      .mockRejectedValue(new Error("contains recipient@example.test"));
    const warn = vi.fn();
    const send = vi.fn(async (command: unknown) =>
      command instanceof ReceiveMessageCommand
    ...
    const adapter = createSesEventSqsAdapter(
      config({ logger: { info: vi.fn(), warn, error: vi.fn() } }),
      { process },
      dependencies(send),
    );
  ...
  it("checks queue access without receiving messages", async () => {
    const send = vi.fn().mockResolvedValue({
      Attributes: { QueueArn: "arn:aws:sqs:region:account:queue" },
    });
    const adapter = createSesEventSqsAdapter(
      config(),
      { process: vi.fn() },
  ...
  it("validates polling and queue configuration", () => {
    expect(() =>
      createSesEventSqsAdapter(config({ queueUrl: "http://localhost/queue" }), {
        process: vi.fn(),
      }),
    ).toThrow("must be HTTPS");
    expect(() =>
      createSesEventSqsAdapter(config({ maxMessages: 11 }), {
        process: vi.fn(),
      }),
    ).toThrow("1 to 10");
    expect(() =>
      createSesEventSqsAdapter(config({ waitTimeSeconds: 21 }), {
        process: vi.fn(),
      }),
    ).toThrow("0 to 20");
  });
...
function config(overrides: Record<string, unknown> = {}) {
  return {
    ...
    queueUrl: "https://sqs.ap-southeast-1.amazonaws.com/123/events",
    ...overrides,
  };
}
