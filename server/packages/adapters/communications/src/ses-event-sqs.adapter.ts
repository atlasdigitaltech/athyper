...
type SqsCommand = ReceiveMessageCommand | DeleteMessageCommand | GetQueueAttributesCommand;
type SqsOutput = ReceiveMessageCommandOutput | DeleteMessageCommandOutput | GetQueueAttributesCommandOutput;

export interface SesEventSqsClient {
  send(command: SqsCommand, options?: { readonly abortSignal?: AbortSignal }): Promise<SqsOutput>;
  destroy?(): void;
}
...
export interface SesEventSqsAdapter {
  ...
  health(signal?: AbortSignal): Promise<{ readonly status: "healthy" | "unhealthy"; readonly latencyMs: number; readonly message?: string }>;
  close(): void;
}
...
export function createSesEventSqsAdapter(
  ...
  const waitTimeSeconds = integer(config.waitTimeSeconds ?? 20, "SQS wait time", 0, 20);
  const visibilityTimeoutSeconds = integer(config.visibilityTimeoutSeconds ?? 60, "SQS visibility timeout", 1, 43_200);
  const maxMessages = integer(config.maxMessages ?? 10, "SQS maximum messages", 1, 10);
  const failureBackoffMs = integer(config.failureBackoffMs ?? 1_000, "SQS failure backoff", 1, 60_000);
  const client = dependencies.createClient({ region });
  const shutdown = new AbortController();
  let closed = false;
  let running = false;

  async function pollOnce(signal?: AbortSignal): Promise<SesEventSqsPollResult> {
    ...
    }), { abortSignal: operationSignal }) as ReceiveMessageCommandOutput;
    const totals = { received: 0, acknowledged: 0, retrying: 0, permanentFailures: 0 };
  ...
  async function processMessage(message: Message): Promise<SesEventMessageDisposition> {
    const fields = messageFields(message);
    if (!message.Body?.trim() || !message.ReceiptHandle?.trim()) {
      config.logger?.warn("communications.ses_sqs.permanent_failure", { ...fields, reasonCode: "invalid_sqs_message" });
      return { outcome: "permanent_failure", reasonCode: "invalid_sqs_message" };
    }
    try {
      ...
      config.logger?.warn("communications.ses_sqs.retry", { ...fields, reasonCode: "handler_failure" });
      return { outcome: "retry", reasonCode: "handler_failure" };
    }
  }

  async function deleteMessage(message: Message, signal: AbortSignal): Promise<void> {
    const receiptHandle = message.ReceiptHandle?.trim();
    if (!receiptHandle) return;
    await client.send(new DeleteMessageCommand({ QueueUrl: queueUrl, ReceiptHandle: receiptHandle }), { abortSignal: signal });
  }
  ...
  return {
      ...
      try {
        while (!runSignal.aborted) {
          try {
            ...
            config.logger?.error("communications.ses_sqs.poll_failed", { reasonCode: "sqs_operation_failure" });
    ...
    async health(signal) {
      ...
      if (closed) return { status: "unhealthy", latencyMs: 0, message: "SES event SQS adapter is closed" };
      try {
        ...
        return { status: "unhealthy", latencyMs: Date.now() - startedAt, message: "Amazon SQS queue verification failed" };
      }
    },
    close() {
      if (closed) return;
...
function messageFields(message: Message): Readonly<Record<string, unknown>> {
  ...
  return {
    ...(id ? { messageRef: createHash("sha256").update(id).digest("hex").slice(0, 16) } : {}),
    ...(receiveCount ? { receiveCount } : {}),
  };
}
function combinedSignal(first?: AbortSignal, second?: AbortSignal): AbortSignal {
  const signals = [first, second].filter((value): value is AbortSignal => value !== undefined);
...
function queue(value: string) { const result = required(value, "SQS queue URL"); try { const url = new URL(result); if (url.protocol !== "https:") throw new Error(); } catch { throw new TypeError("SQS queue URL must be HTTPS"); } return result; }
function integer(value: number, name: string, minimum: number, maximum: number) { if (!Number.isInteger(value) || value < minimum || value > maximum) throw new TypeError(`${name} must be an integer from ${minimum} to ${maximum}`); return value; }
function safeReason(value: string) { return value.toLowerCase().replace(/[^a-z0-9_.-]+/g, "_").slice(0, 80) || "unspecified"; }
