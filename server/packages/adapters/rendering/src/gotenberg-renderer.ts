...
export function createGotenbergRenderer(
  ...
  const timeoutMs = positiveInteger(config.timeoutMs ?? 120_000, "render timeout");
  const healthTimeoutMs = positiveInteger(config.healthTimeoutMs ?? 5_000, "health timeout");
  const maxHtmlBytes = positiveInteger(config.maxHtmlBytes ?? 5 * 1_024 * 1_024, "maximum HTML bytes");
  const maxPdfBytes = positiveInteger(config.maxPdfBytes ?? 50 * 1_024 * 1_024, "maximum PDF bytes");
  ...
  return {
    async renderPdf(request) {
      ...
      if (!html) {
        throw new RenderingError("invalid_input", "HTML input is required", { retryable: false });
      }
      ...
      if (totalHtmlBytes > maxHtmlBytes) {
        throw new RenderingError("invalid_input", `HTML input exceeds ${maxHtmlBytes} bytes`, {
          retryable: false,
        });
      }
      const startedAt = now();
      ...
      try {
        response = await dependencies.fetch(`${baseUrl}/forms/chromium/convert/html`, {
          method: "POST",
          ...(request.documentName
            ? { headers: { "Gotenberg-Output-Filename": outputName(request.documentName) } }
            : {}),
        ...
        throw new RenderingError(
          ...
          timedOut ? `Gotenberg timed out after ${timeoutMs}ms` : "Gotenberg is unavailable",
      ...
      if (Number.isFinite(declaredLength) && declaredLength > maxPdfBytes) {
        throw new RenderingError("invalid_output", "Rendered PDF exceeds the output limit", {
          retryable: false,
          statusCode: response.status,
        });
      }
      const bytes = new Uint8Array(await response.arrayBuffer());
      if (bytes.byteLength === 0 || bytes.byteLength > maxPdfBytes) {
        throw new RenderingError("invalid_output", "Gotenberg returned an empty or oversized PDF", {
          retryable: false,
          statusCode: response.status,
        });
      }
      if (Buffer.from(bytes.subarray(0, 5)).toString("ascii") !== "%PDF-") {
        throw new RenderingError("invalid_output", "Gotenberg returned non-PDF content", {
          retryable: false,
          statusCode: response.status,
        });
      }
...
function buildForm(html: string, options: RequiredRenderOptions): FormData {
  ...
  form.append("paperWidth", String(options.landscape ? paper.height : paper.width));
  form.append("paperHeight", String(options.landscape ? paper.width : paper.height));
...
function validateOptions(options: PdfRenderOptions | undefined): RequiredRenderOptions {
  const scale = options?.scale ?? 1;
  if (!Number.isFinite(scale) || scale < 0.1 || scale > 2) {
    throw new RenderingError("invalid_input", "PDF scale must be between 0.1 and 2", {
      retryable: false,
    });
  }
  const margin = (value: number | undefined): number => {
    ...
    if (!Number.isFinite(normalized) || normalized < 0 || normalized > 5) {
      throw new RenderingError("invalid_input", "PDF margins must be between 0 and 5 inches", {
        retryable: false,
      });
    }
    return normalized;
...
async function responseError(response: Response): Promise<RenderingError> {
  ...
  if (response.status === 504) {
    return new RenderingError("timeout", `Gotenberg conversion timed out${suffix}`, {
      retryable: true,
      statusCode: response.status,
    });
  }
...
function validateBaseUrl(raw: string): string {
  ...
  if (url.username || url.password || url.search || url.hash) {
    throw new Error("Gotenberg base URL cannot contain credentials, query, or fragment");
  }
