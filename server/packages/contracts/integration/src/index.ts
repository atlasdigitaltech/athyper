...
export interface ConnectorType {
  ...
  readonly healthCheckConfig: JsonObject;
...
export interface RedactedConnectorError {
  readonly classification: "provider" | "transport" | "validation" | "security";
  readonly code: string;
...
export interface DeliveryAttemptEvidence {
  ...
  readonly redactedError?: RedactedConnectorError;
...
export interface IntegrationDlqReplayReceipt {
  readonly kind: "prepared" | "replayed";
  readonly tenantId: string;
  readonly deliveryId: string;
...
export interface ConnectorCatalog {
  listTypes(categoryCode?: string): Promise<readonly ConnectorType[]>;
  getType(id: string): Promise<ConnectorType | undefined>;
}
export interface IntegrationRepository extends ConnectorCatalog {
  getInstance(
    tenantId: string,
    id: string,
  ...
  getDelivery(tenantId: string, id: string): Promise<Delivery | undefined>;
  beginAttempt(tenantId: string, deliveryId: string): Promise<Delivery>;
  appendAttempt(evidence: DeliveryAttemptEvidence): Promise<void>;
  markDelivered(tenantId: string, id: string): Promise<void>;
  scheduleRetry(
    tenantId: string,
    id: string,
...
export interface SecretResolver {
  resolve(
    reference: string,
  ): Promise<{ readonly bytes: Uint8Array; readonly version: string }>;
}
...
export interface InboundWebhookPolicyResolver {
  resolve(subscriptionId: string): Promise<InboundWebhookPolicy | undefined>;
}
...
export interface ProvisioningCommandEnvelope<
...
export interface ProvisioningCommandReceipt {
  readonly commandId: string;
...
export interface ProvisioningCommandTransport {
  execute(
...
export interface ProvisioningCommandExecutionStore<Transaction = unknown> {
  executeAtomically(
    ...
    | {
        ...
        readonly receipt: ProvisioningCommandReceipt;
      }
    | {
...
export interface ProvisioningCommandAuthenticator {
  authenticate(command: ProvisioningCommandEnvelope): Promise<
    ...
    | {
        ...
        readonly errorCode:
          | "PROVISIONING_UNAUTHENTICATED"
          | "PROVISIONING_AUDIENCE_MISMATCH"
          | "PROVISIONING_PLANE_MISMATCH";
      }
  >;
}
