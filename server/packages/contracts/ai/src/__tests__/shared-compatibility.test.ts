...
it("preserves the existing server parser entry points and their validation", () => {
  ...
  for (const parser of [parseAtlasIntent, parseAtlasResponseFeedback, parseAtlasInsightResult, parseAtlasBusinessContext]) {
    expect(() => parser({ schemaVersion: 999, tenantId: "forged" })).toThrow();
  }
  expect(() => parseAtlasAnswerEnvelope({}, { evidenceIds: [], actionIds: [] })).toThrow();
});
