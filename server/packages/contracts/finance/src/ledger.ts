...
export interface CanonicalJournalSourcePort {
  ...
  ): Promise<CanonicalJournalSnapshot | undefined>;
  validateDimensionScope(
    actor: FinanceActor,
    journal: CanonicalJournalSnapshot,
...
export interface GlBalanceQuery extends Pick<
  GlCoordinate,
  "companyCodeId" | "ledgerBookId" | "fiscalPeriodId"
> {
  readonly glAccountId?: string;
  readonly currencyCode?: string;
}
export interface GlReconciliation {
  readonly query: GlBalanceQuery;
  readonly balances: readonly GlBalance[];
  readonly periodDebit: FinanceDecimal;
  readonly periodCredit: FinanceDecimal;
...
export type GlPostingOutput = Readonly<Record<string, unknown>> & {
  readonly journalEntryId: string;
  readonly balances: readonly GlBalance[];
...
export interface GlBalanceRepository<Transaction = unknown> {
  applyAggregate(
    input: {
      ...
      readonly coordinate: GlCoordinate;
      readonly debit: FinanceDecimal;
      readonly credit: FinanceDecimal;
      readonly postingSequence: number;
  ...
  ): Promise<{ readonly balance: GlBalance; readonly replayed: boolean }>;
  list?(
    actor: FinanceActor,
    query: GlBalanceQuery,
    transaction: Transaction,
  ): Promise<readonly GlBalance[]>;
}

export interface GlPostingServiceContract {
  post(
    command: GlPostingCommand,
  ): Promise<FinanceCommandResult<GlPostingOutput>>;
}
...
export interface CrossBookPostingPayload extends Readonly<
...
export type CrossBookExecutionStatus =
...
export interface CrossBookPolicyPort {
  loadImmutable(
    actor: FinanceActor,
    evidence: CrossBookPolicyEvidence,
  ): Promise<CrossBookPolicyEvidence | undefined>;
}
export interface DestinationJournalCommandPort {
  createDerivedJournal(input: {
    readonly actor: FinanceActor;
    readonly commandId: string;
    readonly idempotencyKey: string;
    readonly source: CanonicalJournalSnapshot;
    readonly policy: CrossBookPolicyEvidence;
  }): Promise<{
    readonly journalEntryId: string;
...
export interface CrossBookExecutionRepository<Transaction = unknown> {
  create(
    command: CrossBookPostingCommand,
    input: {
      readonly destinationCommandId: string;
      readonly evidencePayload: Readonly<Record<string, unknown>>;
    },
    ...
    readonly execution: CrossBookPostingExecution;
    readonly replayed: boolean;
  }>;
  claim(
    actor: FinanceActor,
    executionId: string,
    transaction: Transaction,
  ): Promise<CrossBookPostingExecution | undefined>;
  succeed(
    actor: FinanceActor,
    executionId: string,
    ...
    transaction: Transaction,
  ): Promise<CrossBookPostingExecution>;
  fail(
    actor: FinanceActor,
    executionId: string,
    errorCode: string,
...
export type CommitmentFulfillmentType =
  | "goods_receipt"
  | "service_acceptance"
  | "invoice_match"
  | "payment"
  ...
  | "advance_recovery"
  | "retention_release";
export interface CommitmentSourceEvidence extends SourceDocumentEvidence {
...
export interface CommitmentFulfillment {
  ...
  readonly commitmentId: string;
  readonly commitmentLineId: string;
  readonly fulfillmentType: CommitmentFulfillmentType;
  ...
  readonly source: CommitmentSourceEvidence;
  readonly reversesFulfillmentId?: string;
  readonly idempotencyKey: string;
  readonly metadata: Readonly<Record<string, unknown>>;
}
export type CommitmentFulfillmentPayload = Readonly<Record<string, unknown>> &
  Omit<CommitmentFulfillment, "id" | "idempotencyKey" | "metadata"> & {
    readonly postingCoordinates: FinanceCoordinates;
    readonly sourceSnapshot: CommitmentLineSnapshot;
    readonly metadata?: Readonly<Record<string, unknown>>;
  };
export type CommitmentFulfillmentCommand =
  FinanceCommand<CommitmentFulfillmentPayload>;
export interface CommitmentPosition {
  readonly commitmentId: string;
  readonly commitmentLineId: string;
  readonly committedQuantity?: FinanceDecimal;
  readonly actualQuantity?: FinanceDecimal;
  readonly remainingQuantity?: FinanceDecimal;
  readonly committedAmount: FinanceDecimal;
  readonly actualAmount: FinanceDecimal;
  readonly remainingAmount: FinanceDecimal;
  readonly currencyCode: string;
  readonly committedBaseAmount: FinanceDecimal;
  readonly actualBaseAmount: FinanceDecimal;
  readonly remainingBaseAmount: FinanceDecimal;
  readonly baseCurrencyCode: string;
}
export interface CommitmentSourcePort {
  ...
  ): Promise<CommitmentLineSnapshot | undefined>;
  loadFulfillmentSource(
    actor: FinanceActor,
...
export interface CommitmentRepository<Transaction = unknown> {
  append(
    command: CommitmentFulfillmentCommand,
    transaction: Transaction,
  ...
  get(
    ...
    transaction: Transaction,
  ): Promise<CommitmentFulfillment | undefined>;
  list(
    actor: FinanceActor,
    commitmentId: string,
    commitmentLineId: string,
    transaction?: Transaction,
  ): Promise<readonly CommitmentFulfillment[]>;
}
export interface CommitmentServiceContract {
  fulfill(command: CommitmentFulfillmentCommand): Promise<
