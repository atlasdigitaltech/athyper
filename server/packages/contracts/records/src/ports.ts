...
export type StandardViewRelationshipConstraint =
  | Readonly<{
  ...
  | Readonly<{
      ...
      readonly role?: {
        readonly field: "requestedRole" | "roleCode";
...
export type RecordCollectionScopeResolution =
  | Readonly<{
      readonly status: "context_required";
...
export interface GovernedImportAdapter<Transaction = unknown> {
  ...
  ): readonly RecordImportOperation[];
  validate(input: {
    readonly context: VerifiedRequestContext;
    readonly descriptor: EntityRuntimeDescriptor;
  ...
  apply(input: {
    ...
    readonly outcome:
      "created" | "updated" | "deleted" | "requested" | "drafted" | "skipped";
...
export interface RecordMutationService {
  ...
  mutateAggregate(
    command: AggregateRecordCommand,
...
export interface RecordRepository<Transaction = unknown> {
  ...
  patch(
    ...
    readonly versionConflict?: number;
  }>;
  delete(
    descriptor: EntityRuntimeDescriptor,
    tenantId: string,
    ...
    expectedVersion: number | undefined,
    transaction: Transaction,
  ): Promise<{ readonly deleted: boolean; readonly versionConflict?: number }>;
}
...
export type RecordTransactionCoordinator<Transaction> =
  PlaneTransactionCoordinator<Transaction>;

export interface RecordBulkService {
  preflight(command: BulkCommand): Promise<BulkPreflightResult>;
  execute(command: BulkCommand): Promise<BulkExecutionResult>;
}
export interface GovernedRecordJobDispatcher {
  enqueue(
    kind: "bulk" | "import" | "export",
    payload: Readonly<Record<string, unknown>>,
    options?: { readonly jobId: string },
  ): Promise<string>;
}
export interface RecordActionHandler {
  execute(command: RegisteredActionCommand): Promise<RegisteredActionResult>;
}
export interface RecordActionService {
  execute(command: RegisteredActionCommand): Promise<RegisteredActionResult>;
}
...
export interface RecordSnapshotService {
  ...
  ): Promise<SnapshotCaptureReceipt>;
  query(
    context: VerifiedRequestContext,
    snapshotId: string,
...
export interface RecordLockRepository {
  acquire(input: {
    readonly tenantId: string;
