...
export type PartnerQualificationDecision =
  "pending" | "approved" | "conditional" | "rejected" | "suspended" | "expired";
export type SupplierPreferenceStatus =
...
export type CustomerCreditDecision =
  "pending" | "approved" | "conditional" | "rejected" | "suspended" | "expired";
export interface CustomerCreditReview {
  ...
  readonly decision: CustomerCreditDecision;
  readonly decisionReason?: string;
...
export interface CreateCustomerCreditReviewCommand {
  readonly context: VerifiedRequestContext;
  readonly idempotencyKey: string;
  readonly businessPartnerId: string;
  readonly customerId: string;
...
export interface DecideCustomerCreditReviewCommand {
  ...
  readonly expectedVersion: number;
  readonly decision: Exclude<CustomerCreditDecision, "pending" | "expired">;
  readonly reason: string;
  readonly approvedCreditLimit?: number;
  readonly approvedCurrencyCode?: string;
  readonly conditions?: readonly Readonly<Record<string, unknown>>[];
...
export interface CustomerLifecycleResult {
  readonly customerId: string;
  readonly status:
    "prospect" | "active" | "suspended" | "inactive" | "archived";
...
export interface SupplierActivationEvidence {
  ...
  readonly resultingStatus: "active";
...
export interface ActivateSupplierCommand {
  ...
  readonly businessDate: string;
  readonly requirePaymentReadiness?: boolean;
  readonly idempotencyKey: string;
}
...
export interface SupplierPreferenceDesignation {
  ...
  readonly effectiveUntil?: string;
  readonly rationale: string;
  readonly status: SupplierPreferenceStatus;
  readonly temporalStatus: SupplierPreferenceTemporalStatus;
  readonly decisionReason?: string;
  readonly reviewedAt?: string;
  readonly reviewedBy?: string;
  readonly approvedAt?: string;
...
export interface DecideBusinessPartnerQualificationCommand {
  ...
  readonly expectedVersion: number;
  readonly decision: Exclude<PartnerQualificationDecision, "pending">;
  readonly reason: string;
  readonly idempotencyKey: string;
}

export interface CreateSupplierPreferenceCommand {
  readonly context: VerifiedRequestContext;
  readonly idempotencyKey: string;
  readonly businessPartnerId: string;
  readonly supplierId: string;
...
export interface DecideSupplierPreferenceCommand {
  readonly context: VerifiedRequestContext;
...
export interface RevokeSupplierPreferenceCommand {
  readonly context: VerifiedRequestContext;
  readonly preferenceId: string;
  readonly expectedVersion: number;
  readonly reason: string;
...
export interface ListSupplierPreferencesQuery {
  readonly context: VerifiedRequestContext;
  readonly businessPartnerId: string;
  readonly operatingOrganizationId: string;
  readonly companyCodeId?: string;
...
export interface BusinessPartnerEligibilityService {
  ...
  activateSupplier(command: ActivateSupplierCommand): Promise<{
    readonly activation: SupplierActivationEvidence;
    readonly replayed: boolean;
  }>;
  createQualification(
    command: CreateBusinessPartnerQualificationCommand,
  ): Promise<{
    readonly qualification: BusinessPartnerQualification;
    readonly replayed: boolean;
  }>;
  decideQualification(
    command: DecideBusinessPartnerQualificationCommand,
  ): Promise<{
    readonly qualification: BusinessPartnerQualification;
    readonly replayed: boolean;
  }>;
  createPreference(command: CreateSupplierPreferenceCommand): Promise<{
    readonly preference: SupplierPreferenceDesignation;
    readonly replayed: boolean;
  }>;
  decidePreference(command: DecideSupplierPreferenceCommand): Promise<{
    readonly preference: SupplierPreferenceDesignation;
    readonly replayed: boolean;
  }>;
  revokePreference(command: RevokeSupplierPreferenceCommand): Promise<{
    readonly preference: SupplierPreferenceDesignation;
    readonly replayed: boolean;
  }>;
  listPreferences(
    query: ListSupplierPreferencesQuery,
  ): Promise<readonly SupplierPreferenceDesignation[]>;
  expireQualifications(input: {
    readonly tenantId: string;
    readonly actorId: string;
    readonly businessDate: string;
    readonly limit?: number;
  }): Promise<readonly BusinessPartnerQualification[]>;
  reevaluateSupplierActivations(input: {
    readonly tenantId: string;
    readonly actorId: string;
    readonly businessDate: string;
    readonly scopes: readonly SupplierActivationReevaluationScope[];
  }): Promise<readonly PartnerEligibilityDecision[]>;
  createCustomerCreditReview(
    command: CreateCustomerCreditReviewCommand,
  ): Promise<{
    readonly review: CustomerCreditReview;
    readonly replayed: boolean;
  }>;
  decideCustomerCreditReview(
    command: DecideCustomerCreditReviewCommand,
  ): Promise<{
    readonly review: CustomerCreditReview;
    readonly replayed: boolean;
  }>;
  listCustomerCreditReviews(query: {
    readonly context: VerifiedRequestContext;
    readonly businessPartnerId: string;
    readonly operatingOrganizationId: string;
    readonly companyCodeId: string;
