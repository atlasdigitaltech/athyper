...
export interface BusinessPartnerRequestRepository<Transaction = unknown> {
  ...
  ): Promise<BusinessPartnerRequest | null>;
  create(
    input: {
  ...
  getView(
    ...
    requestId: string,
    transaction: Transaction,
  ): Promise<BusinessPartnerRequestView | null>;
  list(
    query: Omit<BusinessPartnerRequestListQuery, "context"> & {
      readonly tenantId: string;
    },
    transaction: Transaction,
  ): Promise<readonly BusinessPartnerRequest[]>;
  getAggregate(
    tenantId: string,
  ...
  patch(
    input: Omit<PatchBusinessPartnerRequestCommand, "context"> & {
      readonly tenantId: string;
  ...
  submit(
    ...
    transaction: Transaction,
  ): Promise<SubmitBusinessPartnerRequestResponse | null>;
  decide(
    input: {
      readonly tenantId: string;
    ...
    transaction: Transaction,
  ): Promise<DecideBusinessPartnerRequestResponse | null>;
  apply(
    input: {
      readonly tenantId: string;
    ...
    transaction: Transaction,
  ): Promise<ApplyBusinessPartnerRequestResponse | null>;
}

export interface BusinessPartnerRequestSchemaResolver {
  resolve(input: {
    readonly context: CreateBusinessPartnerRequestCommand["context"];
    readonly kind: CreateBusinessPartnerRequestCommand["kind"];
    readonly sourceKind: CreateBusinessPartnerRequestCommand["source"]["kind"];
    readonly requestedRole?: CreateBusinessPartnerRequestCommand["requestedRole"];
  }): Promise<
    BusinessPartnerRequestSchemaReference & { readonly releaseId: string }
  >;
}
...
export interface BusinessPartnerRequestValidator<Transaction = unknown> {
  validate(
    ...
    transaction: Transaction,
  ): Promise<BusinessPartnerRequestValidationResult>;
}

export interface BusinessPartnerRequestWorkflowResolver<Transaction = unknown> {
  resolve(
    input: {
      readonly context: SubmitBusinessPartnerRequestCommand["context"];
      readonly request: BusinessPartnerRequest;
    },
    transaction: Transaction,
  ): Promise<BusinessPartnerRequestWorkflowDefinition>;
}
...
export interface BusinessPartnerRequestService {
  ...
  getView(
    query: BusinessPartnerRequestQuery,
  ): Promise<BusinessPartnerRequestView>;
  list(
    query: BusinessPartnerRequestListQuery,
  ): Promise<readonly BusinessPartnerRequest[]>;
  getAggregate(
    query: BusinessPartnerAggregateQuery,
  ): Promise<BusinessPartnerAggregate>;
  patch(
    command: PatchBusinessPartnerRequestCommand,
  ): Promise<BusinessPartnerRequest>;
  validate(
    command: ValidateBusinessPartnerRequestCommand,
  ): Promise<ValidateBusinessPartnerRequestResponse>;
  submit(
    command: SubmitBusinessPartnerRequestCommand,
  ): Promise<SubmitBusinessPartnerRequestResponse>;
  decide(
    command: DecideBusinessPartnerRequestCommand,
  ): Promise<DecideBusinessPartnerRequestResponse>;
  apply(
    command: ApplyBusinessPartnerRequestCommand,
  ): Promise<ApplyBusinessPartnerRequestResponse>;
}

export type BusinessPartnerRequestTransactionCoordinator<Transaction> =
