 ...
 *   - VAPID only: always returns "healthy" (no external endpoint to ping)
 */
...
export function createPushAdapter(config: PushAdapterConfig, db: DB): NotificationChannelHandler {
  ...
  async function sendWebPush(endpoint: string): Promise<void> {
    ...
    if (res.status === 410 || res.status === 404) {
      throw new Error(`Web push endpoint expired (HTTP ${res.status}) — subscription should be removed`);
    }
  ...
  return {
      ...
      for (const sub of subscriptions) {
        try {
          if (sub.platform === "web" && sub.endpoint) {
            ...
            dispatched++;
          } else if ((sub.platform === "android" || sub.platform === "ios") && sub.device_token) {
          ...
          errors.push(`sub ${sub.id}: ${String(err).slice(0, 120)}`);
        }
      }
    ...
    async healthCheck() {
      ...
      // VAPID-only mode — no external health check available
      return "healthy";
    },
  };
}
