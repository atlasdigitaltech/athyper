// server/framework/adapters/auth/src/keycloak/auth-adapter.ts
//
...
import { type AdapterLogger, type JwksHealthStatus, JwksManager } from "./jwks-manager.js";
...
export function createAuthAdapter(config: AuthAdapterConfig): AuthAdapter {
  ...
  try { new URL(config.issuerUrl); } catch {
  ...
  async function verifyWithRealm(
    token: string,
    realm: RealmEntry,
    ...
    getJwksHealth(realmKey?: string): Record<string, JwksHealthStatus> {
      ...
      if (realmKey) {
        ...
        for (const [key, entry] of realms) {
          result[key === defaultRealmKey ? "default" : key] =
            entry.jwksManager.getHealth();
        }
      }
      return result;
    },
