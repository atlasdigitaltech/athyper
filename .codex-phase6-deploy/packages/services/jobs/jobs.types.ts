...
export const QUEUE_NAME = {
  ...
  /** Atlas conversation retention expiry and hard-purge maintenance */
  ATLAS_CONVERSATION_PURGE: "jobs-atlas-conversation-purge",
  /** Atlas read-only tool invocation crash recovery maintenance */
  ATLAS_TOOL_INVOCATION_RECOVERY: "jobs-atlas-tool-invocation-recovery",
} as const;
...
export const JOB_NAME = {
  ...
  DB_BACKUP: "pg-dump",
  /** Atlas conversation retention sweep; payload is intentionally empty */
  ATLAS_CONVERSATION_PURGE: "atlas-conversation-purge",
  /** Atlas stale read-only tool invocation recovery; payload is intentionally empty */
  ATLAS_TOOL_INVOCATION_RECOVERY: "atlas-tool-invocation-recovery",
} as const;
...
export const SCHEDULER_ID = {
  ...
  ATLAS_CONVERSATION_PURGE:    "sched:atlas-conversation-purge",
  ATLAS_TOOL_INVOCATION_RECOVERY: "sched:atlas-tool-invocation-recovery",
} as const;
...
/**
 * Atlas retention is always a cross-tenant maintenance sweep. Selectors are
 * deliberately absent so queued data can never choose a tenant, principal,
 * plane, thread, or retention cutoff.
 */
export type AtlasConversationPurgeJobData = Record<string, never>;

/**
 * Atlas tool recovery is a cross-tenant maintenance sweep. Its queued payload
 * is deliberately empty: tenant, principal, plane, age, and row selectors are
 * resolved by the separately-authorized server maintenance service.
 */
export type AtlasToolInvocationRecoveryJobData = Record<string, never>;
...
export const DEFAULT_INTERVALS = {
  ...
  ATLAS_CONVERSATION_PURGE_MS:   3_600_000,      // 1 h    — Atlas retention expiry and purge
  ATLAS_TOOL_INVOCATION_RECOVERY_MS: 300_000,    // 5 min  — stale read-only Atlas tool invocation recovery
} as const;
