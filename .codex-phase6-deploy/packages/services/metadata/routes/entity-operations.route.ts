...
export function createEntityOperationsRoute(router: Router, deps: EntityOperationsRoutesDeps): Router {
  ...
  const handler: RequestHandler = async (req, res, next) => {
    try {
      ...
      const contractV21 = compiledProjection
        ? readCompiledEntityContractV21(compiledProjection)
        : null;
      const contract = compiledProjection ? readCompiledEntityContract(compiledProjection) : null;
      const rows = contractV21
        ? projectContractOperationRowsV21(contractV21)
        : contract ? projectContractOperationRows(contract)
...
function projectContractOperationRowsV21(contract: MetaEntityContractV21): EntityOperationRow[] {
  return contract.operations
    .filter((operation) => operation.enabled && operation.surface !== "HIDDEN")
    .map((operation) => ({
      id: operation.id,
      ...
      is_enabled: operation.enabled,
      selection_config: operation.selection,
      tenant_id: null,
      permission_risk_level: "normal",
      permission_metadata: {
        intent: operation.intent,
