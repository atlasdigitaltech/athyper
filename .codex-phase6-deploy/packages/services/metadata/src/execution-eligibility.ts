...
export function evaluateExecutionEligibility(entity: CanonicalEntity): ExecutionEligibilityResult {
  ...
  if (entity.backing_type === "table") {
    if (entity.physical_primary_key.length !== 1 || entity.physical_primary_key[0] !== entity.primary_key) {
      error("primary_key", "primary_key must match the physical scalar primary key.");
    }
  } else if (entity.primary_key && !entity.physical_columns.includes(entity.primary_key)) {
    error("primary_key", "View runtime entities require a physical scalar projection key.");
  }
  ...
  if (entity.backing_type === "table" && entity.physical_primary_key.length > 1) error("primary_key", "Composite primary keys are not supported by the execution descriptor.");
