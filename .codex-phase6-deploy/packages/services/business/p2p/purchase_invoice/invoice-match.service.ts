 ...
 *
 * Three-way match: purchase_invoice_line ↔ commitment_line ↔ receipt_line
 * Two-way match:   purchase_invoice_line ↔ commitment_line (no receipt required)
...
export async function matchInvoice(
  ...
  const linesResult = await sql<{
    ...
    commitment_line_id: string | null; receipt_line_id: string | null;
  }>`
    SELECT id, line_no, quantity, unit_price, net_amount, commitment_line_id, receipt_line_id
  ...
  for (const line of lines) {
    ...
    if (line.commitment_line_id) {
      ...
      if (cl) {
        ...
        if (matchType === "three_way" && line.receipt_line_id) {
          const grResult = await sql<{ received_qty: number }>`
            ...
            WHERE  id = ${line.receipt_line_id} AND tenant_id = ${tenantId}
            LIMIT  1
          `.execute(db);
  ...
  if (matchCaseId) {
    // imc_status_chk constraint at document/01e_tables_invoice.sql:469 only
    // allows 'pending'|'in_progress'|'completed'|'exception'|'resolved'|'cancelled'
    // — the prior 'complete' (without trailing 'd') would have tripped CHECK
    // on every successful match.
    await sql`
      UPDATE document.invoice_match_case
         SET status = ${anyException ? "exception" : "completed"}, updated_at = now()
