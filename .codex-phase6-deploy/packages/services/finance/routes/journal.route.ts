...
async function autoApproveAndPostJournalEntry(
  ...
  if (!controlledBypass) {
    ...
    logger?.info?.("finance_posting_readiness_gate_evaluated",{
      tenantId,
      companyCodeId:journal.companyCodeId,
      journalEntryId,
      source:readiness.source,
      rolloutMode:readiness.rolloutMode,
      enabled:readiness.enabled,
      certified:readiness.certified,
      decision:readiness.enabled&&!readiness.certified?"blocked":"allowed",
    });
  ...
  router.post("/finance/journals/:jeId/submit", (async (req, res, next) => {
    try {
      ...
      if (shouldAutoApproveAndPost) {
        const posted = await autoApproveAndPostJournalEntry(db, tenantId, jeId, principalId, logger);
