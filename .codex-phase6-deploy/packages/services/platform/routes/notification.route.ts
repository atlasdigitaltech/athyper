...
export interface NotificationRoutesDeps {
  ...
  capabilities?: Partial<Record<"in_app" | "email" | "sms" | "push" | "webhook" | "whatsapp" | "digest", boolean>>;
}
...
function requireTrustedPlane(
  headers: Record<string, unknown>,
  res: Parameters<RequestHandler>[1],
): PlaneKey | null {
  const plane = normalizeSourcePlane(stringHeader(headers["x-plane"]));
  if (!plane) {
    res.status(400).json({
...
export function registerNotificationRoutes(router: Router, deps: NotificationRoutesDeps): Router {
  const { db, auth, logger, bounceWebhookSecret, notificationQueue, webPush, capabilities } = deps;
  ...
  const getPreferencesHandler: RequestHandler = async (req, res, next) => {
    try {
      const claims = await verifyBearer(req.headers.authorization ?? "", auth, res);
      if (!claims) return;
      const planeKey = requireTrustedPlane(req.headers as Record<string, unknown>, res);
      if (!planeKey) return;
      ...
      const rows = await db
        ...
        .where("p.plane_key"    as never, "=", planeKey as never)
    ...
    try {
      ...
      for (const pref of preferences as Record<string, unknown>[]) {
        ...
        await db
          ...
          .onConflict((oc) =>
            oc.columns(["tenant_id", "principal_id", "plane_key", "event_code", "channel"] as never[])
              .doUpdateSet({
                is_enabled:     is_enabled !== undefined ? Boolean(is_enabled) : null,
                frequency_code: frequency_code ? String(frequency_code) : null,
                updated_at:     now,
  ...
  const capabilitiesHandler: RequestHandler = async (req, res, next) => {
    try {
      const claims = await verifyBearer(req.headers.authorization ?? "", auth, res);
      if (!claims) return;
      const planeKey = requireTrustedPlane(req.headers as Record<string, unknown>, res);
      ...
      const principalId = sub && tenantId
        ? await resolvePrincipalIdOrNull(db, sub, tenantId, xRealm)
        : null;
      if (!principalId) {
      ...
      const channels = ["in_app", "email", "sms", "push", "webhook", "whatsapp"].map((code) => {
        const available = code === "push"
          ? Boolean(webPush?.configured && capabilities?.push !== false)
          : Boolean(capabilities?.[code as keyof typeof capabilities]);
        return {
          code,
          available,
          ...(!available ? { reason: `${code}_adapter_unavailable` } : {}),
        };
      });
      const digestAvailable = capabilities?.digest === true;
      const effectivePreferences = await db
        .selectFrom("master.principal_notification_preference as p")
        .select([
          "p.id", "p.event_code", "p.channel", "p.is_enabled", "p.frequency_code", "p.status",
        ] as never[])
      ...
      res.json({
        data: {
          ...
          digest_frequencies: [
            ...
            code,
            available: code === "immediate" || digestAvailable,
            ...(code !== "immediate" && !digestAvailable
              ? { reason: "digest_scheduler_unavailable" }
              : {}),
          })),
      ...
      logger?.error("notif_capabilities_error", { err: String(err) });
      next(err);
    }
  };
    ...
    try {
      ...
      const row = await sql<{ id: string }>`
        INSERT INTO event.push_subscription
          (tenant_id,    principal_id,   plane_key,  platform,   device_id,   endpoint,
           p256dh_key,   auth_key,       device_token, user_agent,
           is_active,    last_used_at,   created_by)
        VALUES
          (${tenantId}::uuid,   ${principalId}::uuid, ${planeKey},
        ...
        ON CONFLICT (tenant_id, principal_id, plane_key, platform, device_id)
          DO UPDATE SET
  ...
  const adminListMessagesHandler: RequestHandler = async (req, res, next) => {
    try {
      ...
      let base = db
        ...
        .where("m.plane_key" as never, "=", planeKey as never);
      ...
      const [countRow, rows] = await Promise.all([
        ...
        base
          .select([
            "m.id", "m.plane_key", "m.event_code", "m.template_key", "m.subject",
    ...
    try {
      ...
      const rows = await sql<{
        ...
        WHERE  nd.tenant_id  = ${tenantId}::uuid
          AND  nd.message_id = ${messageId}::uuid
          AND EXISTS (
            SELECT 1 FROM event.notification_message nm
  ...
  router.get("/notifications/capabilities",            capabilitiesHandler);
