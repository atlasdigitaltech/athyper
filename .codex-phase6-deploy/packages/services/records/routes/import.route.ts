 ...
 *
 * Completes the backend behind runtime entity import flows.
 * Frontend clients call these via BFF relay:
 *
...
export interface ImportRouteDeps {
  ...
  maxUploadMb?:         number;
  logger?:              JobLogger;
  /** RBAC batch check — if absent, all import requests are denied at the route level. */
...
export function createImportRoutes(router: Router, deps: ImportRouteDeps): void {
  const { db, auth, importQueue, objectStorage, logger, checkPermissionBatch } = deps;
  ...
  router.post("/records/:entity/import/upload", (async (req, res, next) => {
    try {
      ...
      const allowed = await isEntityOperationAllowed(
        db, entityCode, "import", tenantId, principalId, checkPermissionBatch,
      );
      if (!allowed) {
        res.status(403).json({ error: "FORBIDDEN", message: "Import is not permitted for this entity." });
        return;
      }
  ...
  router.post("/records/:entity/import", (async (req, res, next) => {
    try {
      ...
      // ── RBAC guard ────────────────────────────────────────────────────────────
      const importAllowed = await isEntityOperationAllowed(
        db, entityCode, "import", tenantId, principalId, checkPermissionBatch,
      );
      if (!importAllowed) {
        res.status(403).json({ error: "FORBIDDEN", message: "Import is not permitted for this entity." });
