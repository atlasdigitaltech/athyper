...
const read = (relative) =>
  YAML.parse(readFileSync(join(repoRoot, relative), "utf8"));
...
test("shared platform ingress exclusively owns workstation HTTP ports", () => {
  ...
  assert.deepEqual(platform.services.ingress.ports.sort(), [
    "127.0.0.1:443:8443",
    "127.0.0.1:80:8080",
  ]);
  assert.equal(
    platform.services.ingress.user,
    "${ATHYPER_RUNTIME_UID:-1000}:${ATHYPER_RUNTIME_GID:-1000}",
  );
  ...
  assert.deepEqual(
    instance.services.gateway.healthcheck.test,
    platform.services.ingress.healthcheck.test,
  );
  assert.deepEqual(instance.services.gateway.ports ?? [], []);
  assert.equal(
    platform.networks["platform-ingress"].name,
    "athyper-platform-ingress",
  );
  assert.equal(instance.networks["platform-ingress"].external, true);
  assert.equal(
    instance.networks["platform-ingress"].name,
    "athyper-platform-ingress",
  );
  assert.equal(
    platform.networks["platform-observability"].name,
    "athyper-platform-observability",
  );
  assert.equal(
    instance.networks["platform-observability"].name,
    "athyper-platform-observability",
  );
  assert.deepEqual(platform.services["platform-outage"].ports ?? [], []);
  assert.equal(platform.services["platform-outage"].read_only, true);
  assert.ok(
    platform.services["platform-outage"].security_opt.includes(
      "no-new-privileges:true",
    ),
  );
});

test("ingress image pins and memory budgets agree with the service catalog", () => {
  const platform = read("deploy/compose/platform/compose.yaml");
  const instance = read("deploy/compose/instance/compose.yaml");
  const catalog = read("deploy/catalog/capabilities.yaml");
  for (const [id, service] of [
    ["platform-ingress", platform.services.ingress],
    ["platform-outage", platform.services["platform-outage"]],
    ["gateway", instance.services.gateway],
    ["gateway-outage", instance.services["gateway-outage"]],
  ]) {
    const capability = catalog.services.find((entry) => entry.id === id);
    const image =
      service.image.match(/^\$\{[^:]+:-(.+)\}$/u)?.[1] ?? service.image;
    if (service.build)
      assert.match(
        readFileSync("deploy/config/traefik/Dockerfile", "utf8"),
        /@sha256:[a-f0-9]{64}/u,
      );
    else assert.match(image, /@sha256:[a-f0-9]{64}$/u);
    assert.equal(image, capability.image);
    assert.equal(service.mem_limit, `${capability.resources.memoryMiB}m`);
  }
  assert.equal(
    instance.services.gateway.image,
    platform.services.ingress.image,
  );
  assert.equal(
    instance.services["gateway-outage"].image,
    platform.services["platform-outage"].image,
  );
  const publication = read(
    "deploy/compose/instance/compose.publication-secretstore.yaml",
  );
  assert.equal(
    publication.services["publication-secretstore-tls"].image,
    instance.services.gateway.image,
  );
});

test("core infrastructure images are immutable and match the catalog across overlays", () => {
  const catalog = read("deploy/catalog/capabilities.yaml");
  const families =
    /^(?:postgres:|redis:|edoburu\/pgbouncer:|axllent\/mailpit:|(?:quay\.io\/)?minio\/(?:minio|mc):)/u;
  for (const file of [
    "compose.yaml",
    "compose.parity.yaml",
    ...
    for (const [id, service] of Object.entries(
      read(`deploy/compose/instance/${file}`).services,
    )) {
      if (!families.test(service.image ?? "")) continue;
      assert.match(service.image, /@sha256:[a-f0-9]{64}$/u, `${file}: ${id}`);
      assert.equal(
        service.image,
        catalog.services.find((entry) => entry.id === id)?.image,
        `${file}: ${id}`,
      );
    }
  }
...
test("worker and scheduler metrics use private cross-project discovery", () => {
  const parity = read("deploy/compose/instance/compose.parity.yaml"),
    operations = read("deploy/compose/operations/compose.yaml"),
    prometheus = readFileSync(
      join(repoRoot, "deploy/compose/operations/config/prometheus.yaml"),
      "utf8",
    );
  assert.deepEqual(
    parity.services.worker.networks["platform-observability"].aliases,
    ["worker-${ATHYPER_INSTANCE:-dev}"],
  );
  assert.deepEqual(
    parity.services.scheduler.networks["platform-observability"].aliases,
    ["scheduler-${ATHYPER_INSTANCE:-dev}"],
  );
  assert.equal(
    operations.services.metrics.networks.includes("platform-observability"),
    true,
  );
  assert.match(
    operations.services.metrics.volumes.join("\n"),
...
test("instance gateway has a unique shared-network alias and no Docker socket", () => {
  ...
  assert.deepEqual(gateway.networks["platform-ingress"].aliases, [
    "gateway-${ATHYPER_INSTANCE:-dev}",
  ]);
  assert.ok(
    !(gateway.volumes ?? []).some((volume) =>
      String(volume).includes("docker.sock"),
    ),
  );
...
test("instance Prometheus targets share a network with the scraper", () => {
  const base = read("deploy/compose/instance/compose.yaml");
  const parity = read("deploy/compose/instance/compose.parity.yaml");
  const optional = read("deploy/compose/instance/compose.optional.yaml");
  const services = {
    ...base.services,
    ...parity.services,
    ...optional.services,
  ...
  const config = read(
    "deploy/compose/instance/config/prometheus/prometheus.yaml",
  );
  ...
  for (const job of config.scrape_configs) {
    for (const group of job.static_configs ?? []) {
      for (const target of group.targets) {
        ...
        assert.ok(services[host], `missing target service: ${host}`);
        assert.ok(
          networks(services[host]).some((network) =>
            networks(services.metrics).includes(network),
          ),
...
test("queue console uses the runtime Redis and waits for authenticated readiness", () => {
  const base = read("deploy/compose/instance/compose.yaml");
  const optional = read("deploy/compose/instance/compose.optional.yaml");
  const console = optional.services.queueconsole;
  const runtime = readFileSync(
    join(repoRoot, "deploy/compose/instance/scripts/start-runtime.sh"),
    "utf8",
  );
  ...
  const catalog = read("deploy/catalog/capabilities.yaml");
  assert.deepEqual(
    catalog.services.find(({ id }) => id === "queueconsole").secrets,
    console.secrets,
  );
  assert.equal(optional.services["memorycache-jobs"], undefined);
  assert.equal(optional.secrets["jobs-redis-password"], undefined);
  assert.equal(optional.volumes["memorycache-jobs-data"], undefined);
  assert.ok(!catalog.services.some(({ id }) => id === "memorycache-jobs"));
  const workloads = read("deploy/catalog/workload-sets.yaml");
  assert.ok(
    !workloads.sets.some(({ services }) =>
      services.includes("memorycache-jobs"),
    ),
  );
...
test("platform ingress routes DEV, QA, and staging through isolated gateways", () => {
  ...
  for (const environment of ["dev", "qa", "stg"]) {
    assert.equal(
      dynamic.http.routers[environment].service,
      `gateway-${environment}-failover`,
    );
    assert.deepEqual(
      dynamic.http.services[`gateway-${environment}-failover`].failover,
      {
        healthCheck: {},
        service: `gateway-${environment}`,
        fallback: "platform-outage",
      },
    );
    assert.equal(
      dynamic.http.services[`gateway-${environment}`].loadBalancer.servers[0]
        .url,
      `http://gateway-${environment}:8080`,
    );
    assert.deepEqual(
      dynamic.http.services[`gateway-${environment}`].loadBalancer.healthCheck,
      {
        path: "/ping",
        port: 8082,
        interval: "10s",
  ...
  assert.equal(
    dynamic.http.services["platform-outage"].loadBalancer.servers[0].url,
    "http://platform-outage:8080",
  );
});
...
test("unhealthy browser planes fail over to branded HTML while APIs retain problem JSON", () => {
  for (const environment of ["dev", "qa", "stg"]) {
    const dynamic = read(
      `deploy/compose/instance/config/traefik/${environment}.yaml`,
    );
    for (const plane of ["neon", "mesh", "studio"]) {
      ...
      assert.equal(
        dynamic.http.services[plane].loadBalancer.healthCheck.path,
        "/",
      );
    }
    assert.equal(dynamic.http.routers.api.service, "api-failover");
    assert.equal(
      dynamic.http.services["api-failover"].failover.fallback,
      "problem",
    );
    assert.equal(
      dynamic.http.services.api.loadBalancer.healthCheck.path,
      "/livez",
    );
    assert.equal(
      dynamic.http.services.outage.loadBalancer.servers[0].url,
      "http://gateway-outage:8080",
    );
    assert.equal(
      dynamic.http.services.maintenance.loadBalancer.servers[0].url,
      "http://gateway-outage:8081",
    );
    assert.equal(
      dynamic.http.services.problem.loadBalancer.servers[0].url,
      "http://gateway-outage:8082",
    );
    ...
    assert.deepEqual(
      maintenanceRouters,
      environment === "dev" ? ["neon-maintenance-preview"] : [],
    );
    const outageRouters = Object.entries(dynamic.http.routers)
      .filter(([, { service }]) => service === "outage")
      .map(([name]) => name);
    assert.deepEqual(
      outageRouters,
      environment === "dev" ? ["neon-outage-preview"] : [],
    );
  }

  const nginx = readFileSync(
    join(repoRoot, "deploy/compose/instance/config/nginx/outage.conf"),
    "utf8",
  );
  const page = readFileSync(
    join(repoRoot, "deploy/compose/instance/config/nginx/status.html"),
    "utf8",
  );
  const favicon = readFileSync(
    join(
      repoRoot,
      "packages/platform/foundation/brand/assets/master-marks/athyper-favicon.svg",
    ),
  ).toString("base64");
  assert.match(
    nginx,
    /listen 8080;[\s\S]*sub_filter '__STATUS_MODE__' 'outage'/u,
  );
  assert.match(
    nginx,
    /listen 8081;[\s\S]*sub_filter '__STATUS_MODE__' 'maintenance'/u,
  );
  assert.match(
    nginx,
    /listen 8082;[\s\S]*default_type application\/problem\+json/u,
  );
  assert.match(nginx, /Cache-Control "no-store" always/u);
  assert.match(nginx, /Retry-After "60" always/u);
  assert.match(
    page,
    /studio: \{ label: "Studio", description: "Business Technology Platform" \}/u,
  );
  assert.match(
    page,
    /neon: \{ label: "Neon", description: "Business Operating Platform" \}/u,
  );
  assert.match(
    page,
    /mesh: \{ label: "Mesh", description: "Business Collaboration Network" \}/u,
  );
  ...
  assert.equal(
    (page.match(/src="data:image\/svg\+xml;base64,/gu) ?? []).length,
    3,
  );
  for (const plane of ["neon", "mesh", "studio"]) {
    const lockup = readFileSync(
      join(
        repoRoot,
        `packages/platform/foundation/brand/assets/plane-lockups/${plane}.svg`,
      ),
    ).toString("base64");
    assert.ok(
      page.includes(
        `data-plane-brand="${plane}" src="data:image/svg+xml;base64,${lockup}"`,
      ),
    );
  }
...
test("DEV exposes explicit Neon maintenance and outage previews without replacing the live plane", () => {
  ...
  assert.equal(
    dynamic.http.routers["neon-maintenance-preview"].service,
    "maintenance",
  );
  assert.equal(dynamic.http.routers["neon-outage-preview"].service, "outage");
  assert.equal(
    dynamic.http.routers["neon-maintenance-preview"].priority,
    10000,
  );
  assert.equal(dynamic.http.routers["neon-outage-preview"].priority, 10000);
  assert.match(
    dynamic.http.routers["neon-maintenance-preview"].rule,
    /PathPrefix\(`\/__maintenance-preview`\)/u,
  );
  assert.match(
    dynamic.http.routers["neon-outage-preview"].rule,
    /PathPrefix\(`\/__outage-preview`\)/u,
  );
});

test("platform outage page uses the same self-contained monochrome recovery design", () => {
  const page = readFileSync(
    join(repoRoot, "deploy/compose/platform/outage/status.html"),
    "utf8",
  );
  const favicon = readFileSync(
    join(
  ...
  for (const plane of ["neon", "mesh", "studio"])
    assert.match(page, new RegExp(`data-plane-brand="${plane}"`, "u"));
  assert.match(page, /--contrast:#151515/u);
  assert.match(page, /--brand:#234B84/u);
...
test("database role membership is granted only after foundation roles exist", () => {
  const init = readFileSync(
    join(repoRoot, "deploy/compose/instance/scripts/init-postgres.sh"),
    "utf8",
  );
  const foundation = readFileSync(
    join(repoRoot, "deploy/compose/instance/scripts/run-foundation.sh"),
    "utf8",
  );
  assert.doesNotMatch(init, /GRANT athyperapp TO athyper_runtime/u);
  assert.doesNotMatch(
    init,
    /GRANT athyper_trustiam_service TO athyper_runtime/u,
  );
  assert.doesNotMatch(init, /GRANT athyper_jobs_service TO athyper_worker/u);
  assert.match(
    foundation,
    /apply_plane mesh[\s\S]*GRANT athyperapp TO athyper_runtime/u,
  );
  assert.match(
    foundation,
    /apply_plane mesh[\s\S]*GRANT athyper_trustiam_service TO athyper_runtime/u,
  );
  assert.match(
    foundation,
    /apply_plane mesh[\s\S]*GRANT athyper_jobs_service TO athyper_worker/u,
  );
  assert.match(foundation, /runtime-worker-grants-v1\.sql/u);
  const workerGrants = readFileSync(
    join(
      repoRoot,
      "deploy/compose/instance/scripts/runtime-worker-grants-v1.sql",
    ),
    "utf8",
  );
  assert.match(
    workerGrants,
    /GRANT USAGE ON SCHEMA shared, event TO athyper_jobs_service/u,
  );
...
test("release instances use an image-contained fail-closed forward migration runner", () => {
  ...
  const dockerfile = readFileSync(
    join(repoRoot, "server/Dockerfile.prod"),
    "utf8",
  );
  const runner = readFileSync(
    join(repoRoot, "server/db/runtime/run-forward-migrations.sh"),
    "utf8",
  );
  const service = parity.services["db-forward-migration"];
  assert.deepEqual(service.entrypoint, [
    "/bin/sh",
    "/app/bin/run-forward-migrations.sh",
  ]);
  ...
  assert.deepEqual(
    catalog.services
      .find(({ id }) => id === "db-forward-migration")
      .presets.sort(),
  ...
  assert.match(
    dockerfile,
    /run-forward-migrations\.sh \/app\/bin\/run-forward-migrations\.sh/u,
  );
  ...
  for (const plane of ["studio", "neon", "mesh"]) {
    ...
    assert.equal(
      new Set(names).size,
      names.length,
      `${plane}: duplicate migrations`,
    );
    for (const name of names) {
      assert.match(
        name,
        /^[A-Za-z0-9][A-Za-z0-9_.-]*\.sql$/u,
        `${plane}: unsafe migration filename`,
      );
      const sql = readFileSync(
        join(repoRoot, "server/db/migrations", name),
        "utf8",
      );
      if (legacyUnwrapped.has(name)) {
        assert.equal(
          createHash("sha256").update(sql).digest("hex"),
          legacyUnwrapped.get(name),
          `${plane}/${name}: legacy migration changed`,
        );
        continue;
      }
      // Structural convention only; rollback semantics require database integration tests.
      ...
      assert.match(
        statements,
        /^BEGIN\s*;/iu,
        `${plane}/${name}: missing opening transaction`,
      );
      assert.match(
        statements,
        /COMMIT\s*;\s*$/iu,
        `${plane}/${name}: missing closing transaction`,
      );
    }
  }
});
...
test("QA has a fail-closed one-time migration baseline initializer", () => {
  ...
  const runner = readFileSync(
    join(
      repoRoot,
      "deploy/compose/instance/scripts/baseline-forward-migrations.sh",
    ),
    "utf8",
  );
  assert.deepEqual(service.entrypoint, [
    "/bin/sh",
    "/athyper/bin/baseline-forward-migrations.sh",
  ]);
  assert.equal(service.read_only, true);
  assert.deepEqual(service.secrets, ["postgres-password"]);
  assert.deepEqual(
    catalog.services.find(({ id }) => id === "db-migration-baseline").presets,
    ["qa-standard"],
  );
  assert.match(runner, /foundation receipt does not match the approved DDL/u);
  assert.match(
    runner,
    /already has a migration ledger; refusing baseline adoption/u,
  );
  assert.match(runner, /sha256sum/u);
});

test("S3 bootstrap has enough memory for concurrent full-stack startup", () => {
  const instance = read("deploy/compose/instance/compose.yaml");
  const catalog = read("deploy/catalog/capabilities.yaml");
  const service = catalog.services.find(
    ({ id }) => id === "objectstorage-init",
  );
  assert.equal(instance.services["objectstorage-init"].mem_limit, "256m");
  assert.equal(service.resources.memoryMiB, 256);
});

test("DEV exposes the S3 endpoint without an obsolete MinIO console route", () => {
  const dynamic = read("deploy/compose/instance/config/traefik/dev.yaml");
  assert.equal(dynamic.http.routers.minio, undefined);
  assert.equal(dynamic.http.services.minio, undefined);
  assert.equal(
    dynamic.http.routers.objects.rule,
    "Host(`objects.dev.athyper.test`)",
  );
  assert.equal(
    dynamic.http.services.objects.loadBalancer.servers[0].url,
  ...
  for (const environment of ["qa", "stg"]) {
    ...
    if (environment === "stg") {
      ...
      assert.equal(
        routes.http.services.objects.loadBalancer.servers[0].url,
        "http://objectstorage:9000",
      );
    }
  }
...
test("read-only Alloy stores runtime state only in tmpfs", () => {
  ...
  assert.deepEqual(alloy.volumes, [
...
test("operations UIs and receiver use loopback ports through a non-internal network", () => {
  ...
  for (const service of ["metrics", "logshipper", "telemetry"]) {
    ...
    assert.ok(
      operations.services[service].ports.every((port) =>
  ...
  const alloy = readFileSync(
    join(repoRoot, "deploy/compose/operations/config/alloy.alloy"),
    "utf8",
  );
  for (const label of [
    "instance",
...
test("Grafana reads the owner-only secret only during privilege-dropping bootstrap", () => {
  ...
  const bootstrap = readFileSync(
    join(repoRoot, "deploy/compose/operations/scripts/start-grafana.sh"),
    "utf8",
  );
  assert.equal(grafana.user, "0:0");
  assert.deepEqual(grafana.entrypoint, [
    "/bin/bash",
    "/athyper/bin/start-grafana.sh",
  ]);
  assert.ok(
    grafana.volumes.includes(
      "./scripts/start-grafana.sh:/athyper/bin/start-grafana.sh:ro",
    ),
  );
  assert.match(
    bootstrap,
    /GF_SECURITY_ADMIN_PASSWORD="\$\(<"\$\{secret_file\}"\)"/u,
  );
  assert.match(bootstrap, /unset GF_SECURITY_ADMIN_PASSWORD__FILE/u);
  assert.match(
    bootstrap,
    /exec su -p -s \/bin\/bash grafana -c "exec \/run\.sh"/u,
  );
});
...
test("web session storage reaches Redis without joining the data network", () => {
  const instance = read("deploy/compose/instance/compose.yaml");
  const parity = read("deploy/compose/instance/compose.parity.yaml");
  assert.deepEqual(instance.services.memorycache.networks.sort(), [
    "app",
    "data",
  ]);
...
test("OIDC uses public HTTPS issuers with private backchannel endpoints", () => {
  ...
  const web = readFileSync(
    join(repoRoot, "deploy/compose/instance/scripts/start-web.sh"),
    "utf8",
  );
  const runtime = readFileSync(
    join(repoRoot, "deploy/compose/instance/scripts/start-runtime.sh"),
  ...
  assert.equal(
    instance.services.iam.environment.KC_HOSTNAME,
    "https://iam.${ATHYPER_DOMAIN_SUFFIX:-dev.athyper.test}",
  );
  assert.match(
    web,
    /KEYCLOAK_BASE_URL="https:\/\/iam\.\$\{ATHYPER_DOMAIN_SUFFIX:-dev\.athyper\.test\}"/u,
  );
  assert.match(web, /KEYCLOAK_INTERNAL_BASE_URL=http:\/\/iam:8080/u);
  assert.match(web, /PUBLIC_BASE_URL="https:\/\/\$\{ATHYPER_APP_DOMAIN\}"/u);
  assert.match(
    runtime,
    /IAM_ISSUER_URL="https:\/\/iam\.\$\{ATHYPER_DOMAIN_SUFFIX:-dev\.athyper\.test\}\/realms\/athyper"/u,
  );
  assert.match(runtime, /export KEYCLOAK_REALM=athyper/u);
  assert.match(
    runtime,
    /KEYCLOAK_JWKS_URL=http:\/\/iam:8080\/realms\/athyper\/protocol\/openid-connect\/certs/u,
  );
  const realm = read("deploy/config/iam/realm-athyper-clean-slate.json");
  for (const plane of ["studio", "neon", "mesh"]) {
    const client = realm.clients.find(
      ({ clientId }) => clientId === `${plane}-web`,
    );
    ...
    assert.ok(
      client.attributes["post.logout.redirect.uris"]
        .split("##")
        .includes(`${origin}/api/auth/logout/callback`),
    );
    assert.equal(client.frontchannelLogout, false);
    assert.equal(
      client.attributes["backchannel.logout.url"],
      `http://${plane}-web:3000/api/auth/backchannel-logout`,
    );
    assert.equal(
...
test("runtime communication providers preserve overrides and support secret files", () => {
  ...
  const providers = read(
    "deploy/compose/instance/compose.notification-providers.yaml",
  );
  assert.match(runtime, /SMTP_HOST="\$\{SMTP_HOST:-mailtrap\}"/u);
  assert.match(
    runtime,
    /SMTP_FROM="\$\{SMTP_FROM:-noreply@\$\{ATHYPER_DOMAIN_SUFFIX:-dev\.athyper\.test\}\}"/u,
  );
  ...
  for (const variable of [
    ...
    assert.match(
      runtime,
      new RegExp(`(?:^|\\s)${variable}(?:\\s|;|\\\\|$)`, "u"),
    );
  }
  assert.match(
    runtime,
    /Secret file configured by \$\{variable\}_FILE is unavailable/u,
  );
  assert.match(
    runtime,
    /Secret file configured by \$\{variable\}_FILE is empty/u,
  );
  assert.equal(providers.services.api.secrets, undefined);
  assert.equal(providers.services.worker.secrets, undefined);
  assert.equal(providers.secrets, undefined);
  assert.equal(providers.services.scheduler, undefined);
  assert.equal(
    providers.services.api.environment.EMAIL_PROVIDER,
    "${EMAIL_PROVIDER:-ses}",
  );
  assert.equal(providers.services.api.environment.ATHYPER_ENV, "staging");
  assert.equal(
    providers.services.worker.environment.VAPID_PRIVATE_KEY_FILE,
    undefined,
  );
  assert.match(runtime, /export ATHYPER_ENV PORT=/u);
  assert.match(runtime, /ATHYPER_ENV must be explicit outside local DEV\/QA/u);
});

test("notification capture is an explicit local overlay for every runtime", () => {
  const overlay = read(
    "deploy/compose/instance/compose.notification-capture.yaml",
  );
  for (const service of ["api", "worker", "scheduler"]) {
    assert.equal(
      overlay.services[service].environment.NOTIFICATION_CAPTURE,
      "true",
    );
...
test("default DEV ingress excludes isolated review backends", () => {
  const config = read("deploy/compose/instance/config/traefik/dev.yaml");
  for (const kind of ["operation", "role"]) {
    const name = `bp-${kind}-review`;
    assert.equal(config.http.routers[name], undefined);
    assert.equal(config.http.services[name], undefined);
    const routes = read(
      `deploy/compose/instance/config/traefik/reviews/${kind}.yaml`,
    );
    assert.equal(routes.http.routers[name].service, name);
    const overlay = YAML.parse(
      readFileSync(
        join(repoRoot, `deploy/compose/instance/reviews/${kind}.compose.yaml`),
        "utf8",
      ).replace("!override", ""),
    );
    assert.equal(
      overlay.services.gateway.depends_on[name].condition,
      "service_started",
    );
  }
});
