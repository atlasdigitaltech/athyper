...
KEYCLOAK_BASE_URL="https://iam.${ATHYPER_DOMAIN_SUFFIX:-dev.athyper.test}"
KEYCLOAK_INTERNAL_BASE_URL=http://iam:8080
KEYCLOAK_REALM=athyper
KEYCLOAK_CLIENT_ID="$ATHYPER_IAM_CLIENT_ID"
RUNTIME_API_URL=http://api:4000
PUBLIC_BASE_URL="https://${ATHYPER_APP_DOMAIN}"
PUBLIC_WEB_URL="$PUBLIC_BASE_URL"
APP_ORIGIN="$PUBLIC_BASE_URL"
ALLOWED_HOSTS="$ATHYPER_APP_DOMAIN"
NODE_ENV=production
PORT=3000
export REDIS_URL SESSION_TOKEN_ENCRYPTION_KEY KEYCLOAK_CLIENT_SECRET KEYCLOAK_BASE_URL KEYCLOAK_INTERNAL_BASE_URL
