...
import {
  ...
  stopForwarder,
} from "../src/operations-execution.mjs";
...
function fixture() {
  ...
  return {
    ...
    dependencies: {
      ...
      startForwarder: (_repo, directory) => {
        mkdirSync(directory, { recursive: true });
        writeFileSync(join(directory, "forwarder.pid"), "4242\n");
        return 4242;
      },
...
test("operations up is confirmed, receipt-owned, and starts Compose before forwarding", () => {
  ...
  const receipt = executeOperationsOperation(
    defaultRepoRoot,
    "up",
    "lite",
    { confirm: "lite" },
  ...
  assert.ok(
    context.calls[2]
      .join(" ")
      .includes("until wget -q --spider http://logging:3100/ready"),
  );
  assert.ok(
    context.calls[3]
      .join(" ")
      .includes("reset-admin-password --password-from-stdin"),
  );
  assert.ok(!context.calls[3].join(" ").includes("fixture-secret"));
  const active = JSON.parse(
    readFileSync(join(context.root, "operations/receipts/active.json"), "utf8"),
  );
...
test("forwarder lifecycle refuses a stale PID belonging to another process", () => {
  const context = fixture();
  const path = join(context.root, "forwarder.pid");
  writeFileSync(path, `${process.pid}\n`);
  assert.throws(() => stopForwarder(path), /does not belong/);
});
