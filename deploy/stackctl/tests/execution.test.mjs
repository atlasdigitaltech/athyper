...
function fixture(options = {}) {
  ...
  const run = (program, args) => {
    ...
    if (
      options.platformObjects &&
      args[0] === "ps" &&
      args.includes("label=com.docker.compose.project=athyper-platform")
    ) {
      return { ok: true, status: 0, stdout: "existing-platform-container" };
    }
    if (
      options.failInstanceUp &&
      args.includes("athyper-dev") &&
      args.includes("up")
    ) {
    ...
    if (
      options.failRestore &&
      args.some((value) => value.includes("pg_restore"))
    ) {
      return { ok: false, status: 1, stderr: "fixture restore failure" };
    }
    if (
      options.failRestoreCleanup &&
      (args.includes("rm") ||
        (args.includes("down") &&
          args.some((value) => value.includes("restore"))))
    ) {
        ...
        stdout: JSON.stringify({
          services: {
            db: {
              ports: [
                {
                  host_ip: options.unsafeBind ? "0.0.0.0" : "127.0.0.1",
                  target: 5432,
                  published: "5432",
                },
              ],
    ...
    if (cpIndex >= 0 && args[cpIndex + 1]?.startsWith("db:/tmp/")) {
      writeFileSync(
        args[cpIndex + 2],
        `PostgreSQL fixture: ${args[cpIndex + 1]}\n`,
      );
    }
  ...
  return {
    ...
    dependencies: {
      ...
      plan: () => ({
        blockers: [],
        services: [{ id: "db-migration" }],
        sources: {
          platformCompose: join(
            defaultRepoRoot,
...
test("up starts platform before instance and writes an ownership receipt", () => {
  ...
  const receipt = executeStackOperation(
    defaultRepoRoot,
    "up",
    "dev",
    { confirm: "dev" },
  ...
  assert.ok(
    invocations[0].includes(
      "label=com.docker.compose.project=athyper-platform",
    ),
  );
  assert.ok(
    invocations[1].startsWith("network ls") &&
      invocations[1].includes("athyper-platform"),
  );
  assert.ok(
    invocations[2].includes("--project-name athyper-platform") &&
      invocations[2].endsWith("config --quiet"),
  );
  assert.ok(
    invocations[3].includes("--project-name athyper-dev") &&
      invocations[3].endsWith("config --format json"),
  );
  assert.ok(
    invocations[4].includes("--project-name athyper-platform") &&
      invocations[4].includes(" up "),
  );
  assert.ok(
    invocations[5].includes("--project-name athyper-dev") &&
      invocations[5].endsWith("up --detach --wait db"),
  );
  assert.ok(
    invocations[6].includes("--project-name athyper-dev") &&
      invocations[6].endsWith("run --rm db-init"),
  );
  assert.ok(
    invocations[7].includes("--project-name athyper-dev") &&
      invocations[7].endsWith("run --rm db-migration"),
  );
  assert.ok(
    invocations[8].includes("--project-name athyper-dev") &&
      invocations[8].includes("--remove-orphans"),
  );
  const active = JSON.parse(
    readFileSync(
      join(context.root, "instances/dev/receipts/active.json"),
      "utf8",
    ),
  );
  ...
  const platform = JSON.parse(
    readFileSync(join(context.root, "platform/receipts/active.json"), "utf8"),
  );
  assert.equal(platform.spec.project, "athyper-platform");
  const migration = JSON.parse(
    readFileSync(
      join(context.root, "instances/dev/receipts/migration.json"),
      "utf8",
    ),
  );
  ...
  const targets = JSON.parse(
    readFileSync(
      join(context.root, "operations/prometheus-targets/dev.json"),
      "utf8",
    ),
  );
  assert.deepEqual(
    targets.map((item) => item.targets[0]),
    ["api-dev:9464", "worker-dev:9464", "scheduler-dev:9464"],
  );
  assert.ok(
    targets.every(
      (item) =>
        item.labels.environment === "development" &&
        item.labels.source_revision === "1".repeat(40),
    ),
  );
  assert.equal(
    statSync(join(context.root, "operations/prometheus-targets")).mode & 0o777,
    0o755,
  );
  assert.equal(
    statSync(join(context.root, "operations/prometheus-targets/dev.json"))
      .mode & 0o777,
    0o644,
  );
});
...
test("up failure rolls the instance back without stopping the platform", () => {
  ...
  assert.ok(
    invocations.some(
      (line) =>
        line.includes("--project-name athyper-dev") && line.includes(" down "),
    ),
  );
  assert.ok(
    !invocations.some(
      (line) =>
        line.includes("--project-name athyper-platform") &&
        line.includes(" down "),
    ),
  );
  const receiptPath = join(
    context.root,
    "instances/dev/receipts/20260821T123456Z-up.json",
  );
...
test("DEV preserved-database up validates the existing migration receipt and skips clean-slate migration", () => {
  ...
  writeFileSync(
    join(receiptDirectory, "migration.json"),
    `${JSON.stringify({
  ...
  assert.equal(receipt.spec.status, "succeeded");
  assert.equal(
    receipt.spec.artifacts.migrationMode,
    "preserved-existing-database",
  );
  assert.ok(invocations.some((line) => line.endsWith("run --rm db-init")));
  assert.ok(
    !invocations.some((line) => line.endsWith("run --rm db-migration")),
  );
});
...
test("QA initialization foundations, baselines, then verifies forward migrations exactly once", () => {
  ...
  const invocations = context.calls.map(({ args }) => args.join(" "));
  const foundation = invocations.findIndex((line) =>
    line.endsWith("run --rm db-migration"),
  );
  const baseline = invocations.findIndex((line) =>
    line.endsWith("run --rm db-migration-baseline"),
  );
  const forward = invocations.findIndex((line) =>
    line.endsWith("run --rm db-forward-migration"),
  );
  ...
  const migration = JSON.parse(
    readFileSync(
      join(context.root, "instances/qa/receipts/migration.json"),
      "utf8",
    ),
  );
...
test("release operations record the immutable image-set revision instead of controller HEAD", () => {
  ...
  assert.equal(
    receipt.spec.sourceRevision,
    loadModel(defaultRepoRoot, "qa").imageSet.spec.sourceRevision,
  );
});
...
test("down is receipt-owned and retains Docker volumes", () => {
  ...
  assert.equal(
    existsSync(join(context.root, "operations/prometheus-targets/dev.json")),
    false,
  );
});
...
test("backup is checksummed and restore targets a new retained volume", () => {
  ...
  const backupReceipt = executeStackOperation(
    defaultRepoRoot,
    "backup",
    "dev",
    { confirm: "dev" },
  ...
  const manifestPath = join(
    context.root,
    "backups/dev/20260821T123456Z/backup.json",
  );
  ...
  assert.ok(
    manifest.spec.database.dumps.every(({ sha256 }) =>
      /^[a-f0-9]{64}$/u.test(sha256),
    ),
  );

  const restoreReceipt = executeStackOperation(
    defaultRepoRoot,
    "restore",
    "dev",
    {
  ...
  assert.match(
    restoreReceipt.spec.artifacts.restoreProject,
    /^athyper-dev-restore-/u,
  );
  assert.match(restoreReceipt.spec.artifacts.restoreVolume, /_db-data$/u);
  const restoreCommands = restoreReceipt.spec.commands.map(
    ({ arguments: args }) => args.join(" "),
  );
  assert.ok(
    restoreCommands.some(
      (line) => line.includes(" up ") && line.endsWith(" db"),
    ),
  );
  const globalsCopy = restoreCommands.findIndex((line) =>
    line.includes("postgres-globals.sql db:/tmp/postgres-globals.sql"),
  );
  const globalsRestore = restoreCommands.findIndex(
    (line) =>
      line.includes("sed '/^CREATE ROLE postgres;$/d'") &&
      line.includes("ON_ERROR_STOP=1"),
  );
  const databaseInit = restoreCommands.findIndex((line) =>
    line.endsWith("run --rm db-init"),
  );
  const firstDatabaseRestore = restoreCommands.findIndex((line) =>
  ...
  assert.equal(
    restoreCommands.filter((line) => line.includes(" pg_restore ")).length,
    4,
  );
  assert.ok(
    restoreCommands.some(
      (line) =>
        line.includes(" rm -f ") && line.includes("postgres-globals.sql"),
    ),
  );
  assert.ok(
    restoreCommands.some((line) => line.includes(" down --remove-orphans")),
  );
...
test("restore cleanup cannot mask the primary pg_restore failure", () => {
  ...
  const receipt = JSON.parse(
    readFileSync(
      join(
        context.root,
        "instances/dev/receipts/20260821T123456Z-restore.json",
      ),
      "utf8",
    ),
  );
  ...
  assert.ok(
    context.calls.filter(
      ({ args }) => args.includes("rm") || args.includes("down"),
    ).length >= 2,
  );
});
...
test("a global controller lock rejects concurrent mutations", () => {
  const context = fixture();
  mkdirSync(join(context.root, "locks/stackctl-operation.lock"), {
    recursive: true,
    mode: 0o700,
  });
  assert.throws(
...
test("unsafe rendered override is rejected before any startup", () => {
  const context = fixture({ unsafeBind: true });
  assert.throws(
    () =>
      executeStackOperation(
        defaultRepoRoot,
