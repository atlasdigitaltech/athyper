...
export function createCapabilityPlan(repoRoot, instanceId, profile) {
  if (!PROFILES.has(profile))
    throw new Error(`Unsupported optional capability profile: ${profile}`);
  ...
  const optionalCompose = YAML.parse(readFileSync(optionalPath, "utf8"), {
    merge: true,
  });
  ...
  for (const service of services) {
    if (!optionalCompose.services?.[service.id])
      blockers.push(`Optional service is not composed: ${service.id}.`);
  }

  const additionalMemoryMiB = services.reduce(
    (total, service) => total + service.resources.memoryMiB,
    0,
  );
  const additionalCpu = services.reduce(
  ...
  const totalCpu = Number(
    (deployment.resources.cpu + additionalCpu).toFixed(2),
  );
  if (totalMemoryMiB > deployment.resources.limitMemoryMiB) {
    blockers.push(
      `Capability profile requires ${totalMemoryMiB} MiB with ${instanceId}; host profile permits ${deployment.resources.limitMemoryMiB} MiB.`,
    );
  }
  if (totalCpu > deployment.resources.limitCpu) {
    blockers.push(
      `Capability profile requires ${totalCpu} CPU with ${instanceId}; host profile permits ${deployment.resources.limitCpu}.`,
    );
  }

  const requiredSecrets = [
    ...new Set(services.flatMap((service) => service.secrets)),
  ].sort();
  for (const secret of requiredSecrets) {
    const problem = secretProblem(
      join(runtimeRoot(), "instances", instanceId, "secrets", secret),
    );
  ...
  const recoveryEvidence = {
    secretstore: join(
      qualificationRoot(),
      "capabilities",
      "infisical-recovery.yaml",
    ),
    analytics: join(
      qualificationRoot(),
      "capabilities",
      "analytics-backup.yaml",
    ),
  }[profile];
  if (recoveryEvidence && !existsSync(recoveryEvidence)) {
    blockers.push(
      `Capability recovery evidence is absent: ${recoveryEvidence}.`,
    );
  }
  ...
  if (!portBase)
    blockers.push(
      `No optional-capability port allocation exists for instance ${instanceId}.`,
    );
  ...
  const activeBindings =
    bindingId && bindings[bindingId]
      ? { [bindingId]: bindings[bindingId] }
      : {};
  const occupiedResult = runReadOnly(
    "powershell.exe",
    [
      "-NoProfile",
      "-NonInteractive",
  ...
  for (const [id, binding] of Object.entries(activeBindings)) {
    ...
    if (occupiedPorts.has(port))
      blockers.push(
        `Optional loopback binding is already listening: ${id} (${binding}).`,
      );
  }
  const environment =
    {
      ...
      infisical: {
        ...
        ATHYPER_INFISICAL_URL: activeBindings.infisical
          ? `http://${activeBindings.infisical}`
          : undefined,
      },
  ...
  return {
    ...
    composeFiles: [
      ...deployment.sources.compose,
      optionalPath,
      ...(profile === "observability"
        ? [join(repoRoot, "deploy/compose/instance/compose.observability.yaml")]
        : []),
    ],
    composeProfiles: [profile],
    services: services.map(({ id, image, lifecycle, resources, secrets }) => ({
      id,
      image,
      lifecycle,
      resources,
