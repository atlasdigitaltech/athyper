import { loadQaPublication } from "./qa-publication.mjs";
...
const publishedImageId = (serviceId) =>
  [
    ...
    : ({
        db: "postgres",
        "db-init": "postgres",
        "secretstore-db-init": "postgres",
        "analytics-db-init": "postgres",
        ...
        gateway: "traefik",
        "platform-ingress": "traefik",
        "gateway-outage": "nginx",
        "platform-outage": "nginx",
        mailtrap: "mailpit",
        searchcore: "meilisearch",
        docrender: "gotenberg",
        docparser: "tika",
        objectstorage: "seaweedfs",
        "objectstorage-init": "s3-tools",
      }[serviceId] ?? serviceId);
const PARITY_PRESETS = new Set([
  "dev-full",
  "qa-standard",
  "stg-standard",
  "validation-full",
...
function staticCollisionChecks(repoRoot) {
  const templates = loadInstanceTemplates(repoRoot).map(
    ({ document }) => document,
  );
  const debugPorts = templates.flatMap((item) =>
    Object.values(item.spec.debugPorts ?? {}),
  );
  return {
...
function liveProjectObjects(project) {
  ...
  return Object.fromEntries(
    Object.entries(commands).map(([kind, args]) => {
...
function projectOwnership(repoRoot, root, instanceId, project) {
  ...
  try {
    const receipt = createValidator(repoRoot)(
...
function secretProblem(path) {
  ...
  if (/^(changeme|placeholder|example|todo|replace-me|<.+>)$/iu.test(value))
...
function listeningPorts() {
  ...
  return new Set(
    result.ok
      ? result.stdout.split(/\s+/u).map(Number).filter(Number.isInteger)
      : [],
  );
}
  ...
  return {
    ...
    spec: {
      ...
      providerContracts: Object.fromEntries(
        Object.entries(model.instance.spec.providers).map(([id, mode]) => [
          id,
...
export function createPlan(repoRoot, instanceId, probes = {}) {
  ...
  const qaPublication = loadQaPublication(
    probes.runtimeRoot ?? runtimeRoot(),
    instance,
  );
  const localMasterData = loadLocalMasterData(
    repoRoot,
    probes.runtimeRoot ?? runtimeRoot(),
    instance,
  );
  const localAtlas = join(
    probes.runtimeRoot ?? runtimeRoot(),
    "instances",
    instanceId,
    "config",
    "local-atlas.compose.json",
  );
  ...
  const resourceServices = model.selected.filter(
    ({ id }) => id !== "db-migration-baseline",
  );
  const memoryMiB = resourceServices.reduce(
    (total, service) => total + service.resources.memoryMiB,
    0,
  );
  const cpu = resourceServices.reduce(
    (total, service) => total + service.resources.cpu,
    0,
  );
  const blockers = [];
  const infrastructure =
    probes.infrastructureGates ??
    inspectInfrastructureGates(repoRoot, {
  ...
  for (const [kind, values] of Object.entries(collisions)) {
    if (values.length)
      blockers.push(`Template ${kind} collide: ${values.join(", ")}`);
  }
  if (memoryMiB > model.resources.spec.maxContainerMemoryMiB) {
    blockers.push(
      `Preset requires ${memoryMiB} MiB; host profile permits ${model.resources.spec.maxContainerMemoryMiB} MiB.`,
    );
  }
  if (cpu > model.resources.spec.maxContainerCpu) {
    blockers.push(
      `Preset requires ${cpu} CPU; host profile permits ${model.resources.spec.maxContainerCpu}.`,
    );
  }
  if (RELEASE_MODES.has(instance.spec.mode)) {
    ...
    const derivedNames = new Set([
      ...
      "nginx",
      "mailpit",
      ...derived.map((image) => image.id),
    ]);
    const requiredImageIds = new Set([
      ...PUBLISHED_IMAGE_IDS,
      ...model.selected
        .map((service) => publishedImageId(service.id))
        .filter((id) => derivedNames.has(id)),
    ]);
    for (const id of requiredImageIds) {
      if (!imageIds.includes(id))
        blockers.push(`Release image set is incomplete: ${id} is absent.`);
    }
    if (/^0{40}$/u.test(model.imageSet.spec.sourceRevision)) {
      blockers.push(
        "Release image set source revision is an all-zero placeholder.",
      );
    }
    for (const image of model.imageSet.spec.images) {
      if (!/@sha256:[a-f0-9]{64}$/u.test(image.reference))
        blockers.push(`Release image is mutable: ${image.id}`);
      if (/@sha256:0{64}$/u.test(image.reference))
        blockers.push(
          `Release image digest is an all-zero placeholder: ${image.id}`,
        );
    }
  }
  ...
  const externalNotificationProviders =
    instance.spec.providers.mail === "external" ||
    instance.spec.providers.push === "external";
  if (instance.spec.providers.objectStorage === "external") {
    const cloud = probes.storageEnvironment ?? process.env;
    for (const key of [
      "S3_REGION",
      "S3_BUCKET_DOCUMENTS",
      "S3_BUCKET_ARTIFACTS",
      ...
      if (!cloud[key]?.trim())
        blockers.push(
          `STG/PROD storage pending provisioning: ${key} is required.`,
        );
    }
  }
  const requiredSecrets = [
    ...
    .filter(
      (secret) =>
        instance.spec.providers.objectStorage !== "external" ||
        !secret.startsWith("objectstorage-"),
    )
    .sort();
  const secretProblems = requiredSecrets
    .map((secret) => ({
      secret,
      problem: (probes.secretProblem ?? secretProblem)(
        join(
          probes.runtimeRoot ?? runtimeRoot(),
          "instances",
          instanceId,
  ...
  for (const { secret, problem } of secretProblems)
    blockers.push(`Required secret file ${problem}: ${secret}`);
  const providerConfiguration =
    instance.spec.mode === "staging"
      ? (probes.providerConfiguration ??
  ...
  for (const secret of localMasterData?.secrets ?? []) {
    const problem = (probes.secretProblem ?? secretProblem)(
      join(
        probes.runtimeRoot ?? runtimeRoot(),
        "instances",
        instanceId,
    ...
    if (problem)
      blockers.push(`Local master-data secret ${secret}: ${problem}`);
  }
  const occupiedPorts = probes.listeningPorts?.() ?? listeningPorts();
  for (const [name, binding] of Object.entries(
    instance.spec.debugPorts ?? {},
  )) {
    const port = Number(binding.slice(binding.lastIndexOf(":") + 1));
    if (occupiedPorts.has(port))
      blockers.push(`Debug port ${binding} for ${name} is already listening.`);
  }
  const live =
    probes.liveProjectObjects?.(instance.spec.composeProject) ??
    liveProjectObjects(instance.spec.composeProject);
  const ownership =
    probes.projectOwnership ??
    projectOwnership(
      repoRoot,
      probes.runtimeRoot ?? runtimeRoot(),
      instanceId,
  ...
  if (Object.values(live).some((values) => values.length) && !ownership.owned) {
    blockers.push(
      `Compose project ${instance.spec.composeProject} already owns Docker resources; adopt it with a receipt or choose another ID.`,
    );
  }

  const ingressServices = ["api", "iam", "mesh", "neon", "studio"];
  if (instance.spec.mode === "development") {
    if (model.selected.some((service) => service.id === "mailtrap"))
      ingressServices.push("mail");
    if (model.selected.some((service) => service.id === "objectstorage"))
      ingressServices.push("objects");
  }
  const domains = ingressServices.map(
    (service) => `${service}.${instance.spec.domainSuffix}`,
  );
  const imageOverrides = new Map(
    model.imageSet.spec.images.map(({ id, reference }) => [id, reference]),
  );
  ...
  return {
    ...
    sources: {
      ...
      ...(providerConfiguration
        ? { providerConfiguration: providerConfiguration.path }
        : {}),
      ...
      compose: [
        ...
        ...(externalNotificationProviders
          ...
          : []),
        ...(instance.spec.providers.objectStorage === "external"
          ? [join(repoRoot, "deploy/compose/instance/compose.aws-storage.yaml")]
          : []),
        ...(!localMasterData &&
        ["internal", "mock"].includes(instance.spec.providers.mail)
          ? [
              join(
                repoRoot,
    ...
    services: model.selected.map((service) => ({
      ...
      image:
        localMasterData &&
        ["api", "worker", "scheduler", "neon-web"].includes(service.id)
          ? service.id === "neon-web"
            ? localMasterData.neonImage
            : localMasterData.runtimeImage
    ...
    resources: {
      ...
      limitMemoryMiB: model.resources.spec.maxContainerMemoryMiB,
      limitCpu: model.resources.spec.maxContainerCpu,
    },
    networks: ["edge", "app", "data", "ops"].map(
      (name) => `${instance.spec.composeProject}_${name}`,
    ),
    volumes: model.selected
      .filter((service) => service.stateful)
      .map((service) => `${instance.spec.composeProject}_${service.id}-data`)
      .sort(),
