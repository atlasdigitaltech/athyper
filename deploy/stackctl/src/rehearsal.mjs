...
function createProductionRehearsalPlan(repoRoot,sourceInstance){
  ...
  for(const [id,path] of Object.entries(evidence)){
    ...
    try{const document=validate(JSON.parse(readFileSync(path,"utf8")),path);const evidenceRevision=document.spec?.sourceRevision??document.sourceRevision;if(evidenceRevision!==sourceModel.imageSet.spec.sourceRevision)blockers.push(`Production evidence source revision differs from STG: ${id}.`);if(document.kind==="BusinessPartnerReleaseCertificationReport"&&document.status!=="certified")blockers.push("Business Partner P9 release certification is not certified.");}catch(error){blockers.push(`Production evidence is invalid: ${id} (${error.message}).`);}
  }
