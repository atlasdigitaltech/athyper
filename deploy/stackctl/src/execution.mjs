...
import { assertRenderedBindings } from "./rendered-bindings.mjs";
...
function assertSecret(path) {
  ...
  if (!stat.isFile())
    throw new Error(`Required secret is not a regular file: ${path}`);
  if ((stat.mode & 0o077) !== 0)
    throw new Error(`Required secret must be owner-only: ${path}`);
...
function instanceEnvironment(model, root) {
  ...
  const providerEnvironment =
    model.instance.spec.mode === "staging"
...
function deploymentRevision(repoRoot, model) {
  ...
  return /^[a-f0-9]{40}$/u.test(imageRevision) &&
    !/^0{40}$/u.test(imageRevision)
    ? imageRevision
...
function safeBackupDirectory(root, instanceId, backupId) {
  ...
  if (!target.startsWith(`${parent}${sep}`))
    throw new Error("Backup path escapes the instance backup root.");
  return target;
}
...
function readActive(validate, root, instanceId, project, requiredState) {
  ...
  if (!existsSync(path))
    throw new Error(
      `No controller ownership receipt exists for ${instanceId}: ${path}`,
    );
  const receipt = validate(readJson(path), path);
  if (
    receipt.metadata.instance !== instanceId ||
    receipt.spec.project !== project
  ) {
    throw new Error(
      `Ownership receipt does not match ${instanceId}/${project}.`,
    );
  }
  if (requiredState && receipt.spec.state !== requiredState) {
    throw new Error(
      `Instance ${instanceId} is ${receipt.spec.state}; ${requiredState} is required.`,
    );
  }
  return { path, receipt };
}
...
function readPlatformActive(validate, root, requiredState) {
  ...
  if (!existsSync(path))
    throw new Error(
      `No controller ownership receipt exists for ${PLATFORM_PROJECT}: ${path}`,
    );
  const receipt = validate(readJson(path), path);
  if (
    receipt.metadata.instance !== "platform" ||
    receipt.spec.project !== PLATFORM_PROJECT
  ) {
    throw new Error(
      `Ownership receipt does not match platform/${PLATFORM_PROJECT}.`,
    );
  }
  if (requiredState && receipt.spec.state !== requiredState) {
    throw new Error(
      `Platform is ${receipt.spec.state}; ${requiredState} is required.`,
    );
  }
  return { path, receipt };
}
...
function readMigrationReceipt(validate, root, instanceId, project) {
  ...
  if (!existsSync(path)) {
    throw new Error(
      `Preserved-database startup requires an existing migration receipt: ${path}`,
    );
  }
  const receipt = validate(readJson(path), path);
  if (
    ...
    throw new Error(
      `Migration receipt does not match ${instanceId}/${project}.`,
    );
  }
  return { path, receipt };
}
...
function admissionBlockers(repoRoot, instanceId, receiptState, dependencies) {
  ...
  if (!plan.services?.some(({ id }) => MIGRATION_RUNNERS.has(id))) {
    blockers.push(
      "No executable database migration service is selected; application startup cannot bypass the migration gate.",
    );
  }
...
function executeLocked(
  ...
  const started = now();
  const operationLocation = uniqueOperationPath(
    receiptDirectory,
    `${timestamp(started)}-${operation}`,
  );
  const operationId = operationLocation.id;
  const operationPath = operationLocation.path;
  const revision =
    dependencies.sourceRevision ?? deploymentRevision(repoRoot, model);
  ...
  const forwardMigrationSha256 = treeSha256(
    join(repoRoot, "server", "db", "migrations"),
  );
  ...
  const compose = (args, selectedProject = project, selectedFiles = files) =>
    invoke("docker", [
      ...composePrefix(selectedProject, selectedFiles),
      ...args,
    ]);
  ...
  const metricsTargetPath = join(
    root,
    "operations",
    "prometheus-targets",
    `${instanceId}.json`,
  );
  const writeMetricsTarget = () => {
    atomicJson(metricsTargetPath, [
      {
        targets: [`api-${instanceId}:9464`],
        labels: {
          instance: instanceId,
      ...
      {
        targets: [`worker-${instanceId}:9464`],
        labels: {
          instance: instanceId,
          environment: model.instance.spec.mode,
          service: "worker",
    ...
    chmodSync(dirname(metricsTargetPath), 0o755);
    chmodSync(metricsTargetPath, 0o644);
  };
  const removeMetricsTarget = () => {
    if (existsSync(metricsTargetPath)) unlinkSync(metricsTargetPath);
  };
  ...
  try {
    if (operation === "up") {
      if (options.preserveDatabase && instanceId !== "dev") {
        throw new Error(
          "--preserve-database is restricted to the DEV instance.",
        );
      }
      if (options.initializeDatabase && !["qa", "stg"].includes(instanceId)) {
        throw new Error(
          "--initialize-database is restricted to QA and STG instances.",
        );
      }
      if (options.initializeDatabase && options.preserveDatabase) {
        throw new Error(
          "--initialize-database and --preserve-database are mutually exclusive.",
        );
      }
      if (
        options.initializeDatabase &&
        existsSync(join(receiptDirectory, "migration.json"))
      ) {
        throw new Error(
          "Database initialization already has a migration receipt; use normal forward-only startup.",
        );
      }
      const preservedMigration = options.preserveDatabase
        ? readMigrationReceipt(validate, root, instanceId, project)
        : null;
      const admission = admissionBlockers(
        repoRoot,
        instanceId,
        existingActive?.spec.state,
        dependencies,
      );
      if (admission.blockers.length) {
        throw new Error(
          `Execution gates are blocked:\n- ${admission.blockers.join("\n- ")}`,
        );
      }
      for (const name of PLATFORM_SECRETS)
        assertSecret(join(root, "platform", "secrets", name));
      const platformFile = admission.plan.sources.platformCompose;
      const platformArgs = composePrefix(PLATFORM_PROJECT, [platformFile]);
      const platformContainers = invoke("docker", [
        "ps",
        "-aq",
        "--filter",
        `label=com.docker.compose.project=${PLATFORM_PROJECT}`,
      ]);
      const platformNetworks = invoke("docker", [
        "network",
        "ls",
        "-q",
        "--filter",
      ...
      assertRenderedBindings(
        JSON.parse(compose(["config", "--format", "json"]).stdout),
      );
      invoke("docker", [...platformArgs, "up", "--detach", "--wait"]);
      writePlatformActive(platformFile);
      try {
        const migrationRunner = model.selected.some(
          ({ id }) => id === "db-forward-migration",
        )
          ? "db-forward-migration"
          : "db-migration";
        const migrationMode =
          migrationRunner === "db-forward-migration"
            ? "forward-migrations"
        ...
        if (options.initializeDatabase) {
          ...
          writeMigrationReceipt(
            "fresh-database-foundation-baselined",
            forwardMigrationSha256,
          );
        ...
        const result = run(
          "docker",
          [...composePrefix(project, files), "down", "--remove-orphans"],
          { cwd: repoRoot, env },
        );
        commands.push({
          program: "docker",
          arguments: [
            ...composePrefix(project, files),
            "down",
            "--remove-orphans",
          ],
          status: result.status ?? (result.ok ? 0 : 1),
      ...
      if (
        service &&
        !new Set(model.selected.map(({ id }) => id)).has(service)
      ) {
        throw new Error(
          `Service ${service} is not selected by instance ${instanceId}.`,
        );
      }
      ...
      if (existsSync(directory))
        throw new Error(`Backup already exists: ${directory}`);
      mkdirSync(directory, { recursive: true, mode: 0o700 });
      chmodSync(directory, 0o700);
      const globals = join(directory, "postgres-globals.sql");
      const dumps = DATABASES.map((database) => ({
        database,
      ...
      compose([
        ...
        "sh",
        "-ec",
        'export PGPASSWORD="$(cat /run/secrets/postgres-password)"; pg_dumpall --username postgres --globals-only --no-role-passwords --file /tmp/athyper-postgres-globals.sql',
      ]);
      for (const { database } of dumps) {
        compose([
          ...
          "sh",
          "-ec",
          `export PGPASSWORD=\"$(cat /run/secrets/postgres-password)\"; pg_dump --username postgres --format custom --dbname ${database} --file /tmp/${database}.dump`,
        ]);
      }
      let copyError;
      let cleanup;
      try {
        ...
        for (const { database, path } of dumps)
          compose(["cp", `db:/tmp/${database}.dump`, path]);
      } catch (error) {
        copyError = error;
      } finally {
        const cleanupArgs = [
          ...
          "rm",
          "-f",
          "/tmp/athyper-postgres-globals.sql",
          ...DATABASES.map((database) => `/tmp/${database}.dump`),
        ];
        cleanup = run("docker", cleanupArgs, { cwd: repoRoot, env });
        commands.push({
          program: "docker",
          arguments: cleanupArgs,
          status: cleanup.status ?? (cleanup.ok ? 0 : 1),
        });
      }
      ...
      if (!cleanup.ok)
        throw commandError(
          "docker",
          ["compose", "exec", "db", "rm", "backup-temporary-files"],
          cleanup,
        );
      for (const path of [globals, ...dumps.map(({ path }) => path)])
      ...
      if (options.confirmRestore !== backupId)
        throw new Error(
          `Refusing restore: pass --confirm-restore ${backupId}.`,
        );
      const directory = safeBackupDirectory(root, instanceId, backupId);
      const manifestPath = join(directory, "backup.json");
      const backup = validate(readJson(manifestPath), manifestPath);
      if (
        backup.metadata.instance !== instanceId ||
        backup.metadata.id !== backupId
      )
        throw new Error("Backup identity does not match the restore target.");
      const backupArtifacts = [
        backup.spec.database.globals,
        ...backup.spec.database.dumps,
      ];
      ...
      assertRenderedBindings(
        JSON.parse(
          compose(["config", "--format", "json"], restoreProject).stdout,
        ),
      );
      ...
      try {
        ...
        compose(
          [
            ...
            "sh",
            "-ec",
            `export PGPASSWORD=\"$(cat /run/secrets/postgres-password)\"; sed '/^CREATE ROLE postgres;$/d' /tmp/${globals.file} | psql --set=ON_ERROR_STOP=1 --username postgres --dbname postgres`,
          ],
          restoreProject,
        );
        ...
        for (const item of backup.spec.database.dumps) {
          ...
          compose(
            [
              ...
              "sh",
              "-ec",
              `export PGPASSWORD=\"$(cat /run/secrets/postgres-password)\"; pg_restore --username postgres --clean --if-exists --dbname ${item.database} /tmp/${item.file}`,
            ],
            restoreProject,
          );
        }
        ...
        for (const args of [
          [
            ...
            "-f",
            `/tmp/${globals.file}`,
            ...backup.spec.database.dumps.map((item) => `/tmp/${item.file}`),
          ],
          ["down", "--remove-orphans"],
        ]) {
          try {
            compose(args, restoreProject);
          } catch (error) {
...
export function executeStackOperation(
  repoRoot,
  operation,
  instanceId,
  options = {},
  ...
  if (!OPERATIONS.has(operation))
    throw new Error(`Unsupported stack operation: ${operation}`);
  if (!/^[a-z][a-z0-9-]{1,31}$/u.test(instanceId))
    throw new Error(`Invalid instance ID: ${instanceId}`);
  assertConfirmation(instanceId, options);
  const root = dependencies.runtimeRoot ?? configuredRuntimeRoot();
  if (
    instanceId === "dev" &&
    ["up", "down", "restart", "restore"].includes(operation)
  ) {
    const workspace = join(root, "instances/dev/workspace");
    if (existsSync(join(workspace, "operation.lock"))) {
      throw new Error("DEV workspace mode switch is in progress");
    }
    if (
      existsSync(join(workspace, "mode.json")) &&
      ["source", "container"].includes(
        JSON.parse(readFileSync(join(workspace, "mode.json"), "utf8")).mode,
      )
    ) {
      throw new Error(
        "DEV workspace owns the application services. Use pnpm dev:workspace legacy before a Stack v2 image deployment.",
      );
    }
  }
  ...
  try {
    ...
    if (error.code === "EEXIST") {
      throw new Error(
        `Another stackctl mutation holds the controller lock: ${lockPath}`,
      );
    }
    throw error;
  }
  try {
    return executeLocked(repoRoot, operation, instanceId, options, {
      ...dependencies,
      runtimeRoot: root,
    });
  } finally {
