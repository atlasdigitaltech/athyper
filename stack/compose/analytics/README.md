# Analytics Profile — analyticsboard (Metabase, DEFERRED hedge stage)
...
resolved, enabling `analyticsboard` is a profile change rather than a fresh
infra ticket.
...
- **Disconnected** — no data source is wired. Admins point analyticsboard at
...
| Developer DB debugging | `dbconsole` (admin profile, dev-only) |
...
analyticsboard / Metabase **must not** point at the primary database. Postgres uses MVCC,
...
processing. A naive analyticsboard query like
...
- Statement timeout enforced on the replica role used by analyticsboard
  (recommendation: `SET statement_timeout = '5min'` in the replica
  role's default settings).
- analyticsboard's data-source role granted `CONNECT` on the replica only —
  no write grants anywhere.

### 1a. Least-privilege DB role — curated views, not raw tables

Even on a read replica, analyticsboard's DB role **must not** be given blanket
...
2. Grant analyticsboard's DB role `USAGE` on the `analytics` schema and
...
- **Account provisioning** — Who can create analyticsboard accounts? Tied
...
profile unless `ANALYTICSBOARD_GOVERNANCE_APPROVED=approved` is set in the
...
If no finance/ops user has actually asked for analyticsboard, this is
...
1. Provision a Postgres database for analyticsboard's app data (separate from
   `athyper_neon` — Metabase rewrites schema aggressively across version
   upgrades). Suggested name: `athyper_analytics`.
2. Run a one-time migration from H2 → Postgres using Metabase's built-in
   ...
   docker compose --profile analytics run --rm \
     ...
     -e MB_DB_CONNECTION_URI=jdbc:postgresql://dbpool-session:6433/athyper_analytics?user=...&password=... \
     analyticsboard load-from-h2 /metabase-data/metabase.db
   ```
3. Set `MB_DB_TYPE=postgres` and `MB_DB_CONNECTION_URI=…` in the
   environment file, restart the service.
4. Back up the H2 file under `${ATHYPER_DATA}/analyticsboard` before deleting.
...
analyticsboard boots in ~60–90 s on first launch (schema migrations on the H2
...
- `ANALYTICSBOARD_HOST` — Traefik hostname
- `ANALYTICSBOARD_MEMORY_LIMIT` — JVM container limit (default `1g`)
...
- `ANALYTICSBOARD_GOVERNANCE_APPROVED` — `approved` to clear the
