...
#   7. Application planes (neon/mesh/admin)
...
#   APPS_ATHYPER_ADMIN_HOST — Admin hostname (skipped if unset on non-local)
#   IAM_HOST                — Keycloak hostname, e.g. iam-stg.athyper.com
#   IAM_DEFAULT_REALM       — fallback realm name (default: athyper)
#   {NEON,MESH,ADMIN}_KEYCLOAK_REALM — per-plane realms (checked when set)
...
ADMIN_REALM="${ENV_MAP[ADMIN_KEYCLOAK_REALM]:-}"
...
printf "  hosts       : api=%s neon=%s mesh=%s admin=%s iam=%s\n" \
  "${API_HOST:-<unset>}" "${NEON_HOST:-<unset>}" \
  "${MESH_HOST:-<unset>}" "${ADMIN_HOST:-<unset>}" "${IAM_HOST:-<unset>}"
printf "============================================================\n\n"

# ----------------------------
# [1/8] Container status
# ----------------------------
# Infra + runtime planes. Runtime containers (api/iam/neon/mesh/admin) are
...
if [[ "$ENVIRONMENT" == "local" || -n "${ENV_MAP[APPS_ATHYPER_ADMIN_HOST]:-}" ]]; then
  EXPECTED_CONTAINERS+=("${PROJECT}-admin-1")
fi
...
if [[ -n "$ADMIN_HOST" ]]; then
  ...
  skip "Admin host not configured (set APPS_ATHYPER_ADMIN_HOST)"
fi
...
if [[ -z "$IAM_HOST" ]]; then
  ...
  REALMS_TO_CHECK=()
  for r in "$IAM_DEFAULT_REALM" "$NEON_REALM" "$MESH_REALM" "$ADMIN_REALM"; do
    [[ -z "$r" ]] && continue
