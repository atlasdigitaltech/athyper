...
REM ============================================================
REM athyper Stack - CONTAINER START - Windows Batch
REM Location: stack\scripts\stack-service\start.bat
REM Usage:
...
echo Starting: !CNAME!
docker start "!CNAME!"
if errorlevel 1 echo WARNING: docker start returned non-zero for !CNAME!
...
REM -------------------------------------------------------
REM :resolve  alias  outVar
REM   Sets outVar to the resolved container name.
REM   Falls through to raw name if alias is not recognised.
REM -------------------------------------------------------
:resolve
set "A=%~1"
set "RESULT="
