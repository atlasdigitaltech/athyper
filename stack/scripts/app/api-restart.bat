...
REM ============================================================
REM athyper - Backend API RESTART - Windows Batch
REM Location: stack\scripts\app\api-restart.bat
REM Usage:
REM   api-restart.bat             : restart API
REM   api-restart.bat worker      : restart worker
REM   api-restart.bat scheduler   : restart scheduler
REM
REM Behaviour (auto-detected from ENVIRONMENT in .env):
REM   local      : prints guidance (tsx watch handles hot reload)
REM   staging    : docker compose restart selected api/worker/scheduler service
REM   production : docker compose restart selected api/worker/scheduler service
REM ============================================================
REM
...
set "SERVICE_TARGET=api"
set "SERVICE_TARGET_SEEN=0"

:parse_args
if "%~1"=="" goto :after_parse_args
if /I "%~1"=="-h" goto :arg_help
if /I "%~1"=="--help" goto :arg_help
if /I "%~1"=="api" goto :arg_target
if /I "%~1"=="worker" goto :arg_target
if /I "%~1"=="scheduler" goto :arg_target
...
:arg_target
if "!SERVICE_TARGET_SEEN!"=="1" (
  echo ERROR: only one service target can be supplied.
  call :usage
  exit /b 1
)
if /I "%~1"=="api" set "SERVICE_TARGET=api"
if /I "%~1"=="worker" set "SERVICE_TARGET=worker"
if /I "%~1"=="scheduler" set "SERVICE_TARGET=scheduler"
set "SERVICE_TARGET_SEEN=1"
shift
goto :parse_args
...
if /I "!ENVIRONMENT!"=="local" (
  ...
  echo For a hard restart: Ctrl+C in the !SERVICE_TARGET! dev terminal, then run api-up.bat !SERVICE_TARGET!.
  echo.
  goto :end
)
...
docker compose --project-directory "%COMPOSE_DIR%" --env-file "%ENV_FILE%" ^
  !COMPOSE_FILES! restart !SERVICE_TARGET!
...
docker compose --project-directory "%COMPOSE_DIR%" --env-file "%ENV_FILE%" ^
  !COMPOSE_FILES! ps !SERVICE_TARGET!

goto :end

:usage
echo Usage:
echo   api-restart.bat [api^|worker^|scheduler]
echo.
