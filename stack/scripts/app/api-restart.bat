...
REM   staging    -> docker compose restart athyper-api
REM   production -> docker compose restart athyper-api
REM ============================================================
REM
...
if /I "!ENVIRONMENT!"=="local" (
  ...
  echo For a hard restart: Ctrl+C in the dev terminal, then run api-up.bat.
  echo.
  goto :end
)
...
docker compose --project-directory "%COMPOSE_DIR%" --env-file "%ENV_FILE%" ^
  !COMPOSE_FILES! restart athyper-api
