...
#   ./web-up.sh studio --build  -> rebuild and start Studio
#   ./web-up.sh all             -> start Neon, Mesh, and Studio
#
...
start_plane_dev() {
  ...
  (
    ...
    if [[ "$plane" == "studio" ]]; then
      # Studio is the only confidential browser client. The stack secret uses
      # the deployment-facing name, while the shared BFF consumes the
      # plane-qualified runtime name.
