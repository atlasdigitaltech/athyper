...
const canonicalRoot = path.join(
  repoRoot,
  "packages/platform/foundation/brand/assets",
);
const obsoleteAssets = [
  "app-icon-inverse.png",
  "app-icon.svg",
  "icon.png",
  "wordmark-inverse.png",
...
async function main(): Promise<void> {
  for (const plane of planes) {
    ...
    const atlasTarget = path.join(repoRoot, `apps/${plane}/public/brand/atlas`);
    await fs.mkdir(atlasTarget, { recursive: true });
    await fs.copyFile(
      path.join(canonicalRoot, "atlas/app-icon.svg"),
      path.join(atlasTarget, "app-icon.svg"),
    );
    ...
    await Promise.all([
      fs.copyFile(
        path.join(canonicalRoot, `plane-lockups/${plane}.svg`),
        path.join(target, "identity-lockup.svg"),
      ),
      fs.copyFile(
        path.join(canonicalRoot, "master-marks/mark-blue-transparent-2048.png"),
        path.join(target, "app-icon.png"),
      ),
      fs.copyFile(
        path.join(canonicalRoot, "master-marks/athyper-favicon.svg"),
        path.join(target, "athyper-favicon.svg"),
      ),
    ]);
    await fs.writeFile(
      path.join(target, "manifest.webmanifest"),
      `${JSON.stringify({
