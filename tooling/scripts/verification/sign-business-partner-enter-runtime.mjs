...
const report = { baseImage: before.Image, imageId: before.Image };
if (
  before.Image !==
  "sha256:671d0d5abecbfc16790e2d8e643222171eed8d1b5ab485b3b4048b466439fc47"
)
  throw Error("Reviewed adapter image changed");
const root = join(
  homedir(),
...
console.log(JSON.stringify({kind:'bp_enter_correction_native_handler_signing',releaseId:id,checks,reviewReceipt,compile,signed,dispatchHeld:scheduled.filter(j=>j.name==='publication.dispatch').map(j=>j.data),queueConsumersStarted:false,dispatchExecuted:false,grantsChanged:false,activationAuthorized:false}));process.exit(0);`;
