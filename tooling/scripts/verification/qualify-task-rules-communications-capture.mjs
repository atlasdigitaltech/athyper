...
const cases=JSON.parse(readFileSync("governance/policy/reports/task-rules-journeys-live.dev.json","utf8")).cases;
const ids=cases.map(c=>{assert.match(c.id,/^[0-9a-f-]{36}$/);return "'"+c.id+"'::uuid";}).join(',');
const sql =
  "SELECT coalesce(json_agg(x),'[]'::json) FROM (SELECT d.id,d.external_id,d.recipient_id,d.recipient_addr,m.event_code,m.payload->>'caseNo' case_no,m.payload->'communication' pin FROM event.notification_delivery d JOIN event.notification_message m ON m.id=d.message_id WHERE m.event_code LIKE 'supplier.onboarding.notice.%' AND d.channel='email' AND d.status='delivered' AND m.entity_id IN ("+ids+")) x";
...
writeFileSync(
  "governance/policy/reports/task-rules-communications-capture.dev.json",
