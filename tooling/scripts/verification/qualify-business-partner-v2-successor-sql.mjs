...
const head = {
  base: {
    releaseId: proposal.predecessor.releaseId,
    releaseNo: proposal.predecessor.releaseNo,
    appliedReleaseId: proposal.predecessor.releaseId,
    headVersion: 1,
  },
};
const lit = (v) => "'" + String(v).replaceAll("'", "''") + "'";
let migration = readFileSync(
  "server/db/scripts/tests/integration/fixtures/legacy-upgrades/20260911_successor_active_baseline_sequence.sql",
  "utf8",
).replace(/COMMIT;\s*$/, "");
const sql =
  migration +
...
INSERT INTO publication.entity_authorization_successor_payload(content_hash,tenant_id,descriptor) VALUES(${lit(proposal.descriptorHash)},'44444444-4444-4444-8444-444444444444',${lit(JSON.stringify(proposal.descriptor))}::jsonb) ON CONFLICT(content_hash) DO NOTHING;
DO $test$ BEGIN
 BEGIN
...
DO $assert$ BEGIN
 IF NOT EXISTS(SELECT 1 FROM publication.release WHERE id='ba383d04-9a18-4e59-ab4e-3d9726e934c6' AND release_no=19 AND status='approved') THEN RAISE EXCEPTION 'EXISTING_PUBLICATION_CHANGED'; END IF;
 IF NOT EXISTS(SELECT 1 FROM publication.entity_authorization_successor_link WHERE publication_release_id='11111111-1111-4111-8111-111111111119' AND predecessor_release_id=${lit(proposal.predecessor.releaseId)}::uuid) THEN RAISE EXCEPTION 'REVIEWED_PREDECESSOR_CHANGED'; END IF;
 IF NOT EXISTS(SELECT 1 FROM publication.fn_authorization_successor_compilation_source('11111111-1111-4111-8111-111111111119') WHERE release_no=20 AND compiled_json->'authorization' IS NOT NULL AND jsonb_array_length(contract_json->'operations')=42) THEN RAISE EXCEPTION 'SUCCESSOR_COMPILATION_SOURCE_MISSING'; END IF;
END $assert$;
...
const out = execFileSync(
  ...
  { input: sql, encoding: "utf8", maxBuffer: 1000000 },
);
const checks = JSON.parse(out.split("\n").find((l) => l.startsWith("{")));
const report = {
  schemaVersion: 1,
  kind: "bp_v2_successor_sql_rehearsal",
...
writeFileSync(
  "governance/policy/reports/business-partner-v2-successor-sql.dev.json",
