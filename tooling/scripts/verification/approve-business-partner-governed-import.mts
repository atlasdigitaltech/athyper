/** Record the user's explicit import approval through real NEON MFA and revision-bound review. */
...
const packet=JSON.parse(readFileSync('governance/policy/reviews/business-partner-governed-import-workflow.dev.json','utf8'));
if(packet.packetRevision!=='ef285c3c15543c113fb7321c5034c40a6be643e59c6f922cb98a7d8571bf104d'||packet.rows.length!==1||packet.rows[0].operation!=='import')throw Error('Approved import packet changed');
...
const statePath=`tests/e2e/.auth/${account}-import-review.json`;
const fallbackPath=`tests/e2e/.auth/${account}-correction-review.json`;
...
try{
 ...
 const check=async()=>{const r=await context.request.get('/api/governance/operation-review');if(!r.ok())throw Error('Review identity or revision unavailable');const data=await r.json();if(data.packetRevision!==packet.packetRevision||data.reviewerId!==account||data.rows.length!==1||data.rows.some((r:any)=>!packet.rows.some((p:any)=>p.operation===r.operation&&p.proposalSha256===r.proposalSha256)))throw Error('Exact reviewed proposal changed');return data;};
 ...
 if(data.receipts.some((r:any)=>r.reviewerId===account)){console.log(JSON.stringify({account,alreadyRecorded:true}));}
  ...
  if(data.assurance!=='elevated'){
   ...
   console.log(JSON.stringify({account,status:'MFA_REQUIRED',operations:1}));
  ...
  await page.getByLabel('Reason for the selected decisions',{exact:true}).fill('Explicit user approval of import revision ef285c3c15543c113fb7321c5034c40a6be643e59c6f922cb98a7d8571bf104d: bounded supplier-request drafts only; exact tenant gateway plus existing per-row case-create authority, stored scope checks, full pre-write validation, pinned schema, stable idempotency and fresh checks with stop on revocation. Submission, MFA, approval and application stay separate. No grants, publication or activation approved.');
  ...
  await page.getByRole('button',{name:'Record 1 explicit decisions',exact:true}).click();
 ...
 mkdirSync('tests/e2e/.auth',{recursive:true});await context.storageState({path:`tests/e2e/.auth/${account}-import-review.json`});
