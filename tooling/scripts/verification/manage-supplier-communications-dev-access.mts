/** Proposed P7 DEV grants. Run only after explicit approval of the P7 access proposal; no business outcome fixtures. */
...
const path =
  "governance/policy/reports/supplier-communications-dev-access.dev.json";
  ...
  await db.transaction().execute(async (tx: any) => {
    ...
    const command = async (
      ...
      const receipt = await repository.apply({
        context,
        kind,
        commandId: `p7-dev-access:${key}`,
        idempotencyKey: `p7-dev-access:${key}`,
        payload,
        ...extra,
      });
    ...
    for (const [name, principal, codes] of [
      ...
      const role = await command("role.create", `${name}:role`, {
        code: `dev.p7.temporary.${name}`,
        name: `DEV P7 temporary ${name}`,
        description:
          "User-approved P7 qualification; assignments expire within 24 hours",
      });
