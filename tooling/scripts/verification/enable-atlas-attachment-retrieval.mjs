...
const state = "tests/e2e/.auth/dev/neon/catl.admin.json",
  ...
  path = "docs/examples/atlas-f5/cirrus-retrieval-enabled.json";
...
try {
  ...
  for (const [step, route, data] of [
    ["reindex", "attachments/reindex", body],
    ["search", "search", { ...body, query: "synthetic" }],
  ]) {
    ...
    report.steps.push({ step, status: r.status(), response });
    console.log(JSON.stringify(report.steps.at(-1)));
    if (!r.ok()) throw Error(step + " failed");
  }
} finally {
