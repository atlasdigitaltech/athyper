/** Controlled upstream risk assessments for fresh P6 suppliers. Company, qualification and activation use owning APIs separately. */
...
const fixtures=JSON.parse(readFileSync("governance/policy/reports/supplier-onboarding-activation-live.dev.json","utf8")).cases;
try {for(const c of fixtures){await db.transaction().execute(async(tx:any)=>{
 ...
 const target=(await sql`SELECT s.id,b.name FROM master.supplier s JOIN master.business_partner b ON b.tenant_id=s.tenant_id AND b.id=s.business_partner_id WHERE s.tenant_id=${tenant}::uuid AND b.id=${c.businessPartnerId}::uuid`.execute(tx)).rows[0];assert.match(target.name,/^DEV P6 (basic|standard|enhanced) /);
 let risk=(await sql`SELECT id FROM master.party_risk_assessment WHERE tenant_id=${tenant}::uuid AND business_partner_id=${c.businessPartnerId}::uuid AND notes='P6 controlled upstream risk fixture' ORDER BY assessed_at DESC LIMIT 1`.execute(tx)).rows[0];
 if(!risk)risk=(await sql`INSERT INTO master.party_risk_assessment(tenant_id,subject_type,subject_id,business_partner_id,assessment_context,model_code,model_version,risk_band,status,assessed_at,assessed_by,approved_at,approved_by,next_review_at,notes,created_by) VALUES(${tenant}::uuid,'supplier',${target.id}::uuid,${c.businessPartnerId}::uuid,'supplier_role','standard_supplier','1.0','low','approved',now(),${maker}::uuid,now(),${checker}::uuid,${date}::date+30,'P6 controlled upstream risk fixture',${maker}::uuid) RETURNING id`.execute(tx)).rows[0];
 report.cases.push({caseId:c.id,riskId:risk.id});
 });}writeFileSync("governance/policy/reports/supplier-onboarding-risk-fixture.dev.json",JSON.stringify(report,null,2)+"\n");console.log("Three scoped upstream risk fixtures ready; no activation performed.");}finally{await db.destroy();}
