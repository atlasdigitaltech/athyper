...
DO $$
DECLARE base runtime_meta.applied_release%ROWTYPE; descriptor runtime_meta.entity_descriptor%ROWTYPE;
DECLARE candidate uuid:=gen_random_uuid(); count_before integer; staged integer;
BEGIN
 SELECT * INTO STRICT base FROM runtime_meta.applied_release WHERE id='01a08532-31a1-7493-8953-ca4528dda3de';
 SELECT * INTO STRICT descriptor FROM runtime_meta.entity_descriptor WHERE applied_release_id=base.id AND tenant_id='44444444-4444-4444-8444-444444444444';
 SELECT count(*) INTO count_before FROM authz.entity_operation_binding WHERE applied_release_id=base.id;
 ...
 VALUES(candidate,'verification.operation_projection.'||candidate,gen_random_uuid(),18,gen_random_uuid(),base.artifact_hash,base.manifest);
 staged:=authz.fn_stage_entity_operation_projection(candidate,descriptor.tenant_id,'neon',descriptor.release_id,descriptor.compiled_hash,descriptor.compiled_json);
 IF staged<>count_before OR staged=0 THEN RAISE EXCEPTION 'Projection count mismatch'; END IF;
 IF authz.fn_stage_entity_operation_projection(candidate,descriptor.tenant_id,'neon',descriptor.release_id,descriptor.compiled_hash,descriptor.compiled_json)<>staged THEN RAISE EXCEPTION 'Retry not idempotent'; END IF;
 IF (SELECT count(*) FROM authz.entity_operation_binding WHERE applied_release_id=base.id)<>count_before THEN RAISE EXCEPTION 'Baseline changed'; END IF;
 IF EXISTS(SELECT source_entity_operation_id,permission_id,decision_mode FROM authz.entity_operation_binding WHERE applied_release_id=base.id EXCEPT SELECT source_entity_operation_id,permission_id,decision_mode FROM authz.entity_operation_binding WHERE applied_release_id=candidate) THEN RAISE EXCEPTION 'Operation semantics changed'; END IF;
 RAISE NOTICE 'PASS: baseline retained, successive projection coexists, operation semantics preserved, retry idempotent (% operations)',staged;
END $$;
ROLLBACK;
