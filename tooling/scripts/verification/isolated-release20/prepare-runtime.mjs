...
const env = {
  ...
  ATHYPER_ENV: "local",
  PORT: "4000",
  BP_ISOLATED_EXECUTION: "release20",
  BP_AUTHORIZATION_MODE: "off",
  REDIS_URL: "redis://athyper-bp-r20-redis:6379/0",
  REDIS_BULLMQ_URL: "redis://athyper-bp-r20-redis:6379/1",
  REDIS_KEY_PREFIX: "bp-r20:",
  S3_ENDPOINT: "http://athyper-bp-r20-objectstore:9000",
  S3_BUCKET: "bp-release20-qualification",
...
for (const p of ["studio", "neon", "mesh"]) {
  env[p === "neon" ? "DATABASE_URL" : p.toUpperCase() + "_DATABASE_URL"] =
    `postgresql://athyper_runtime:${password}@athyper-bp-r20-db:5432/athyper_${p}`;
  env[p.toUpperCase() + "_WORKER_DATABASE_URL"] =
    `postgresql://athyper_worker:${password}@athyper-bp-r20-db:5432/athyper_${p}`;
}
