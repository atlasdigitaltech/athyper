...
export function assertIsolatedEnvironment(env) {
  if (
    env.BP_ISOLATED_EXECUTION !== "release19" ||
  ...
  for (const key of databaseKeys) {
    ...
    if (
      url.hostname !== "athyper-bp-r19-db" ||
      url.pathname !==
        "/athyper_" +
  ...
  for (const key of ["REDIS_URL", "REDIS_BULLMQ_URL"])
    if (new URL(env[key]).hostname !== "athyper-bp-r19-redis")
      throw Error("SHARED_QUEUE_FORBIDDEN");
  if (
    new URL(env.S3_ENDPOINT).hostname !== "athyper-bp-r19-objectstore" ||
    env.S3_BUCKET !== "bp-release19-qualification"
  )
