...
const processes = JSON.parse(
  docker(["inspect", "athyper-bp-r19s-api", "athyper-bp-r19s-worker"]),
...
const save = () =>
  fs.writeFileSync(
    "governance/policy/reports/business-partner-release-19-successor-revocation.dev.json",
...
try {
  ...
  docker(["pause", "athyper-bp-r19s-worker"]);
  paused = true;
  const queued = send(
  ...
  cp.execFileSync(
    "node",
    [
      "tooling/scripts/verification/isolated-successor/revoke-test-grants.mjs",
      "--revoke",
    ],
    { stdio: ["pipe", "pipe", "pipe"] },
  );
  ...
  docker(["unpause", "athyper-bp-r19s-worker"]);
  paused = false;
  let job;
  for (let n = 0; n < 60; n++) {
    job = JSON.parse(
  ...
  if (paused) docker(["unpause", "athyper-bp-r19s-worker"]);
  save();
  console.log(
