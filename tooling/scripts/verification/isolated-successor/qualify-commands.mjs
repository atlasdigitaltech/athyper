...
const report = existsSync(output)
  ...
  : {
      schemaVersion: 1,
      kind: "release19_authenticated_commands_and_import",
      runId: randomUUID(),
      releaseId,
      artifactHash,
      checks: [],
...
const send = (account, name, path, method = "GET", data, expected = [200]) => {
  ...
  if (!passed) {
    writeFileSync(
      "/tmp/bp-successor-build/last-command-response.json",
...
const payload = {
  ...
  proposedPayload: {
    legalName: "Isolated Release 19 " + report.runId,
...
try {
  ...
  for (let step = 0; step < 8; step++) {
    ...
    const decision = {
      ...
      decision: "approve",
      reason: "Isolated signed release-19 command qualification",
        ...
        rows: [
          {
            ...
            proposedPayload: {
              ...
              legalName: "Imported Release19 " + report.runId,
