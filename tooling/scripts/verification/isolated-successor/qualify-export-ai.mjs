...
try {
  const journey = JSON.parse(
    readFileSync(
      "governance/policy/reports/business-partner-release-19-successor-commands.dev.json",
      "utf8",
    ),
  );
  if (
  ...
  const content = JSON.parse(
    execFileSync(
      ...
      [
        ...
        "--input-type=module",
        "-e",
        `import{readFileSync}from'node:fs';import{createHash}from'node:crypto';const url=new URL(readFileSync(0,'utf8'));if(url.hostname!=='athyper-bp-r19s-objectstore')throw Error('WRONG_STORE');const r=await fetch(url);if(!r.ok)throw Error('DOWNLOAD_FAILED');const b=Buffer.from(await r.arrayBuffer()),rows=JSON.parse(b.toString());console.log(JSON.stringify({sha256:createHash('sha256').update(b).digest('hex'),rows:rows.length,keys:[...new Set(rows.flatMap(row=>Object.keys(row)))]}));`,
      ],
      {
  ...
  const events = execFileSync("docker", ["logs", "athyper-bp-r19s-worker"], {
    encoding: "utf8",
    stdio: ["pipe", "pipe", "pipe"],
  })
    .split("\n")
