...
console.log(JSON.stringify({kind:'bp_worker_runtime_smoke',checks,selectedOperations:exact.coordinate.operationKeys.length,runtimeBindingQualificationPendingCompilation:true,reviewReceipt,consumersStarted:false,grantsChanged:false,activationAuthorized:false}));process.exit(0);`;
...
try {
  ...
  const result = run(
    [
      ...
      "--rm",
      "--network",
      Object.keys(before.NetworkSettings.Networks).find((n) =>
        n.endsWith("_data"),
      ),
      "--env-file",
      envFile,
      ...
      "-v",
      join(root, "smoke.mjs") + ":/app/server/bp-smoke.mjs:ro",
      "--entrypoint",
      "node",
      report.imageId,
      "/app/server/bp-smoke.mjs",
    ],
    { timeout: 60000 },
  );
  const receipt = JSON.parse(result.split("\n").find((l) => l.startsWith("{")));
  writeFileSync(
    "governance/policy/reports/business-partner-v2-worker-image-qualification.dev.json",
    JSON.stringify(
      {
        ...receipt,
