...
const published=JSON.parse(readFileSync('governance/policy/reports/task-response-escalation-release-live.dev.json','utf8'));
...
try{
 ...
 const section=page.locator('section[aria-labelledby="task-edit-policy-heading"]');await section.waitFor().catch(async error=>{console.log(JSON.stringify({path:new URL(page.url()).pathname,text:(await page.locator("body").innerText()).slice(0,1800)}));throw error;});
 ...
 const responseRoles=controls.getByLabel('Overdue response supervisor role',{exact:true});assert.equal(await responseRoles.count(),8);for(const input of await responseRoles.all())assert.equal(await input.inputValue(),'dev.bp.approvers.operating_organization');
 assert.ok(await controls.getByRole('checkbox').count()>0);assert.ok(await controls.getByRole('combobox').count()>0);
 await page.screenshot({path:'governance/policy/reports/task-response-escalation-release-live.dev/controls.png',fullPage:true});
 ...
 writeFileSync('governance/policy/reports/task-response-escalation-controls-live.dev.json',JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result));await context.close();
