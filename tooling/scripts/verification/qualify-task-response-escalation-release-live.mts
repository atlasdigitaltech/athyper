...
const origin = "https://studio.dev.athyper.test", prefix = "governance/policy/reports/task-response-escalation-release-live";
...
try {
 ...
 const previous=JSON.parse(readFileSync('governance/policy/reports/task-rules-bound-release-live.dev.json','utf8'));
 const input={...fixture,predecessorId:previous.policyId,definition:{...fixture.definition,versionNo:2,name:'DEV requester response supervisor notification '+randomUUID()},processBinding:{basePublicationId:base.id,expectedReleaseId:base.release_id??null,tasks:base.publication.manifests.flatMap((m:any)=>m.tasks.filter((t:any)=>['review','approval'].includes(t.executionKind)).map((t:any)=>({profile:m.profile.code,code:t.code,informationPolicy:{schema:'athyper.task-information-policy/1',clockMode:'bounded_pause',responseHours:48,overdueSupervisorRole:'dev.bp.approvers.operating_organization'},caseAuthority:{schema:'athyper.task-case-authority/1',returnForChanges:true,rejectProposal:true}})))}};
