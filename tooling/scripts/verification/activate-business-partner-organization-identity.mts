...
try {
  ...
  const current = await r.json(),
    graph = withBusinessPartnerOrganizationIdentity(current.graph);
  const issues = validateGraph(graph).issues;
  if (graph.fields.some(field => ["legal_name", "display_name", "details_legal_name", "details_display_name"].includes(field.fieldKey))) throw Error("Retired identity fields remain");
  ...
  if (receipt.beforeHash !== receipt.contractHash || current.preview?.state !== "active" || current.preview?.contractHash !== receipt.contractHash) {
    ...
    writeFileSync(
      "governance/policy/reports/business-partner-organization-identity-activation.dev.json",
