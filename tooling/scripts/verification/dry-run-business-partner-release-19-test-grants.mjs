/** Transactional grant rehearsal only. Always rolls back; no --apply switch. */
import {readFileSync,writeFileSync} from 'node:fs';
import {createHash} from 'node:crypto';
import {execFileSync} from 'node:child_process';
if(process.argv.length!==2)throw Error('Dry run only; no apply supported');
...
if(!output.trim().endsWith('ROLLBACK'))throw Error('Rollback not confirmed');
const receipt=JSON.parse(output.split('\n').find(l=>l.startsWith('{')));
const report={schemaVersion:1,kind:'bp_release_19_test_grants_dry_run',checkedAt:new Date().toISOString(),...receipt,rollbackConfirmed:true,grantsChanged:false,activationAuthorized:false,approvalRecorded:false};
writeFileSync('governance/policy/reports/business-partner-release-19-test-grants-dry-run.dev.json',JSON.stringify(report,null,2)+'\n');
console.log(report);
