...
import { BUSINESS_PARTNER_REQUEST_HEADER_LABELS } from "../../../server/db/scripts/provisioning/business-partner-request-capture.js";
import {
  sha256,
  validateGraph,
  runContractTests,
...
try {
  ...
  const current = await r.json(),
    graph = structuredClone(current.graph);
  const details = graph.surfaces.find((s: any) => s.surfaceKey === "intake_details");
  if (!details) throw Error("Details surface unavailable");
  details.layoutConfig.formLabels = {...details.layoutConfig.formLabels, ...BUSINESS_PARTNER_REQUEST_HEADER_LABELS};
  const flow = graph.flows?.find((f: any) => f.flowKey === "request_intake");
  if (flow) flow.allowDraftResume = true;
  ...
  if (receipt.beforeHash !== receipt.contractHash) {
    ...
    writeFileSync(
      "governance/policy/reports/business-partner-request-header-activation.dev.json",
      JSON.stringify(receipt, null, 2) + "\n",
    );
    if (!result.ok()) throw Error("Save failed");
  }
  const verified = await (await c.get(path)).json();
  const verifiedLabels=verified.graph.surfaces.find((s:any)=>s.surfaceKey==="intake_details")?.layoutConfig?.formLabels;
  if (!Object.entries(BUSINESS_PARTNER_REQUEST_HEADER_LABELS).every(([key,value])=>verifiedLabels?.[key]===value) || !verified.graph.flows.find((f:any)=>f.flowKey==="request_intake")?.allowDraftResume)
    throw Error("Saved header metadata mismatch");
