...
const signed = JSON.parse(
  readFileSync(
    "governance/policy/reports/business-partner-enter-correction-runtime-signing.dev.json",
  ),
);
const report = { baseImage: before.Image, imageId: signed.imageId };
...
const execution={tenantId:exact.coordinate.tenantId,principalId:exact.durable.publishedBy,planeKey:'studio',scope:'tenant',requestId:'bp-enter-correction-signing'};
const invoke=(name,data)=>runWithJobContext(execution,()=>registered.get(name).handle({id:'bp-enter-correction:'+name,name,data,execution}));
...
console.log(JSON.stringify({kind:'bp_enter_correction_runtime_verification',releaseId:id,artifactId:a.id,artifactHash:a.content_hash,verification:loaded.verification,runtimeVersion:d.authorizationRuntime.runtimeVersion,operationCount:d.authorization.operations.length,reviewReceipt,alteredHashRejected,readOnly:true,grantsChanged:false,activationAuthorized:false}));process.exit(0);`;
