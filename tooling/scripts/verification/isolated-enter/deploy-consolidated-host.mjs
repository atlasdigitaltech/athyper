...
fs.writeFileSync(
  root + "/consolidated-deployment.json",
...
for (const mode of ["worker", "api"]) {
  const name = "athyper-bp-enter-" + mode,
    prior = name + "-before-consolidated";
...
fs.writeFileSync(
  "governance/policy/reports/business-partner-consolidated-execution-deployment-20260912.dev.json",
  JSON.stringify(
    {
