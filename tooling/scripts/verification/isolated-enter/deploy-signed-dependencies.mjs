...
const verification = JSON.parse(
  fs.readFileSync(
    "governance/policy/reports/business-partner-dependency-artifact-verification-1789167957359.dev.json",
  ),
);
...
const rows=await container.adapters.athyperDatabase.database.transaction().execute(async tx=>{await sql.raw('SET TRANSACTION READ ONLY').execute(tx);await sql.raw("SET LOCAL app.current_tenant_id='44444444-4444-4444-8444-444444444444'").execute(tx);return(await sql.raw("SELECT a.*,r.release_key,r.release_no FROM publication.artifact a JOIN publication.release r ON r.id=a.publication_release_id WHERE r.id IN ('4bc3b11e-fcc2-4220-82e3-8ed528d9af18','9827ce80-766d-4262-9b4f-835307ac33d2') AND a.status='signed'").execute(tx)).rows;});
if(rows.length!==2)throw Error('DEPENDENCY_SET_MISMATCH');
...
return out;});
console.log(JSON.stringify({kind:'isolated-dependency-projection',capturedAt:new Date().toISOString(),results,sharedActivation:false}));process.exit(0);
`;
const r = spawnSync(
  "docker",
  [
