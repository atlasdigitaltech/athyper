...
for (const file of manifest.harness)
  assert.equal(
    sha(fs.readFileSync(root + "/reveal-coordinates-harness/" + file.name)),
    file.sha256,
  );
const output =
  "governance/policy/reports/business-partner-reveal-coordinates-deployment-20260912.dev.json";
...
for (const mode of ["worker", "api", "neon-ui"]) {
  ...
  const previous = name + "-before-reveal-coordinates";
  const mounts = old.Mounts.flatMap((m) => [
    "--mount",
    "type=bind,src=" +
      (m.Destination === "/app/server/qualification"
        ? root + "/reveal-coordinates-harness"
