...
try {
  ...
  if (!state.changeSet) {
    state.changeSet = await call("change-sets", "POST", {
      ...
      branchCode: "bp-consolidated-20260912",
      title: "BP company setup catalog identity; governed case service only",
      registration: {
        schemaVersion: 1,
        moduleCode: "rel",
        entityClass: "process",
  ...
  if (!state.graphStored) {
    const graph = JSON.parse(
      fs.readFileSync(
        "governance/policy/reviews/bp-consolidated-20260912/company-registration-ordered.candidate.json",
        "utf8",
      ),
    );
