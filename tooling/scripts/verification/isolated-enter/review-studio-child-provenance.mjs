...
const review = JSON.parse(
  fs.readFileSync(
    "governance/policy/reviews/bp-dependencies-20260912/child-provenance-review.json",
    "utf8",
  ),
);
assert.equal(
  review.nativeContractHash,
  "27e2132ce6b729ddae3ddacd9c2d14da5911558cc6cd18cf5cc63f5e4c2d709c",
);
const preflight = JSON.parse(
  cp.execFileSync(
    ...
    [
      ...
      "athyper_studio",
      "-c",
      "SELECT jsonb_build_object('status',c.status,'revision',c.lock_version,'ledgerHash',r.contract_hash,'graph',r.contract_json,'author',c.created_by,'submitter',c.submitted_by) FROM metadata.entity_change_set c JOIN LATERAL(SELECT contract_hash,contract_json FROM snapshot.entity_contract_revision WHERE change_set_id=c.id ORDER BY revision_no DESC LIMIT 1)r ON true WHERE c.id='bab653ec-e2a2-4dfe-9b00-d4e00611f9d9';",
    ],
    { encoding: "utf8" },
  ),
);
...
try {
  ...
  console.log({
    ...
    onboardingPublicationRequested: false,
