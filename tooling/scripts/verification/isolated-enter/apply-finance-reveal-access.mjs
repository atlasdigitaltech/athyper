...
for (const m of [p.referenceMigration, p.additionalFixtures])
...
for (const mode of ["api", "worker"]) {
  ...
  assert.ok(mount.Source.endsWith("/finance-reveal-v2-harness"));
...
let sql = accessInsertionSql(p)
  .replace(
    ...
    "BEGIN;SET LOCAL app.database_plane='neon';" +
      fs.readFileSync(p.referenceMigration.path, "utf8") +
      fs.readFileSync(p.additionalFixtures.path, "utf8"),
  )
  .replace("END $guard$;", "END $guard$;" + registrations);
...
fs.writeFileSync(
  "governance/policy/reports/business-partner-finance-reveal-access-application-20260912.dev.json",
  JSON.stringify(report, null, 2) + "\n",
  { flag: "wx" },
);
assert.equal(rows[0].permissionAssignments, 52);
