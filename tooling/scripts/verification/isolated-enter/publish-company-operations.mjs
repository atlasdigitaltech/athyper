...
const statePath =
  "governance/policy/reports/business-partner-company-operations-publication-20260912.dev.json";
...
try {
  if (!state.source) {
    const rows = JSON.parse(
      cp.execFileSync(
        ...
        [
          ...
          "athyper_studio",
          "-c",
          "BEGIN READ ONLY; SELECT jsonb_build_object('changeSet',jsonb_build_object('id',c.id,'revision',c.lock_version,'status',c.status),'graph',r.contract_json,'contractHash',r.contract_hash) FROM metadata.entity_change_set c JOIN LATERAL(SELECT contract_json,contract_hash FROM snapshot.entity_contract_revision WHERE change_set_id=c.id ORDER BY revision_no DESC LIMIT 1)r ON true WHERE c.id='c49fec5f-76b6-44ac-a760-48e306006a7d'; ROLLBACK;",
        ],
        { encoding: "utf8" },
      ),
    );
    ...
    assert.equal(state.source.graph.operations.length, 8);
    save();
  }
  if (!state.review) {
