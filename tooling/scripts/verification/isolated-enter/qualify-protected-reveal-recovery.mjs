...
assert.ok(artifact);
const bad = root + "/protected-reveal-recovery-incompatible-artifact.json";
...
const failedName = "athyper-bp-protected-reveal-incompatible-recovery-probe";
docker([
  "run",
  "-d",
  "--name",
...
for (const mode of ["worker", "api"]) {
  const name = "athyper-bp-enter-" + mode,
    prior = name + "-before-protected-reveal-recovery";
  docker(["stop", name]);
...
const output =
  "governance/policy/reports/business-partner-protected-reveal-compatible-recovery-20260912.dev.json";
fs.writeFileSync(
  output,
  JSON.stringify(
    {
