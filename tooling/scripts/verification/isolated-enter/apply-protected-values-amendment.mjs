...
const proposalPath =
  ...
  approval = JSON.parse(
    fs.readFileSync(
      "governance/policy/reviews/business-partner-protected-values-amendment-20260912.user-approval.dev.json",
    ),
  ),
  parent = JSON.parse(
    fs.readFileSync(
...
for (const f of manifest.harness)
  assert.equal(
    sha(fs.readFileSync(root + "/protected-values-harness/" + f.name)),
    f.sha256,
  );
...
const output =
  "governance/policy/reports/business-partner-protected-values-amendment-application-20260912.dev.json";
...
const secret = JSON.parse(
  docker(
    ["exec", "-i", manifest.canary.name, "node", "--input-type=module", "-"],
    `import {createHash} from 'node:crypto';const reference=${reference};const url=new URL('/api/v3/secrets/raw/'+encodeURIComponent(reference),process.env.INFISICAL_URL);const configuration={workspaceId:process.env.INFISICAL_WORKSPACE_ID,environment:process.env.INFISICAL_ENVIRONMENT||'dev',secretPath:process.env.INFISICAL_SECRET_PATH||'/'};for(const[k,v]of Object.entries(configuration))url.searchParams.set(k,v);const headers={Authorization:'Bearer '+process.env.INFISICAL_TOKEN,'Content-Type':'application/json'};const prior=await fetch(url,{headers});if(prior.status!==404)throw Error('SECRET_REFERENCE_NOT_NEW:'+prior.status);const created=await fetch(url,{method:'POST',headers,body:JSON.stringify({...configuration,type:'shared',secretValue:Buffer.from('GB82WEST12345698765432').toString('base64')})});if(!created.ok){console.log(JSON.stringify({created:false,status:created.status}));process.exit(1);}const verified=await fetch(url,{headers});if(!verified.ok)throw Error('SECRET_READBACK_FAILED');const payload=await verified.json();const secret=payload.secret??payload;const bytes=Buffer.from(secret.secretValue,secret.secretEncoding==='utf8'?'utf8':'base64');const hash=createHash('sha256').update(bytes).digest('hex');if(hash!==${JSON.stringify(expectedHash)})throw Error('SECRET_READBACK_MISMATCH');console.log(JSON.stringify({created:true,reference,version:secret.version??secret.secretVersion??'current',sha256:hash,tlsVerified:true}));`,
  ),
);
...
for (const mode of ["worker", "api"]) {
  const name = "athyper-bp-enter-" + mode,
    prior = name + "-before-protected-values",
