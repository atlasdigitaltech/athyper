...
const migration = fs
  ...
  .replace(/BEGIN;/, "")
  .replace(/COMMIT;/, "");
const dryRun = true; // Actual signed company artifact rehearsal; never activates persistently.
const script = `
...
const out =
  "governance/policy/reports/business-partner-company-signed-projection-preview-" +
  Date.now() +
  ".dev.json";
