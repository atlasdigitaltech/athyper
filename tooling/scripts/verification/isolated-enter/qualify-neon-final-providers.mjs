...
const report = journal("providers", (check, report) => {
  for (const actor of ["catl.admin", "catl.owner"]) {
    ...
    for (const kind of ["bank", "tax"]) {
      ...
      check(
        name + "_" + kind + "_reveal",
        actor,
        base + (kind === "bank" ? "banking" : "identifiers-tax") + "/reveal",
        "POST",
        body,
        (r) => {
  ...
  report.positiveBusinessActivityQualified = false;
  report.businessActivityGap =
    "Production owning-module summary providers are not configured; case activity is separate.";
  report.sqlFilteredCountsQualified = false;
});
