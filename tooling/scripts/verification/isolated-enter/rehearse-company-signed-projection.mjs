...
const rows=await container.adapters.athyperDatabase.database.transaction().execute(async tx=>{await sql.raw('SET TRANSACTION READ ONLY').execute(tx);await sql.raw("SET LOCAL app.current_tenant_id='44444444-4444-4444-8444-444444444444'").execute(tx);return(await sql.raw("SELECT a.*,r.release_key,r.release_no FROM publication.artifact a JOIN publication.release r ON r.id=a.publication_release_id WHERE r.id IN ('ed6a7433-b41d-4ad8-8dda-44e1bdfba4a8') AND a.status='signed'").execute(tx)).rows;});
