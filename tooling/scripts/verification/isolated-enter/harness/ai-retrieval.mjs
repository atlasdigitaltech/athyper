...
export function registerAiRetrieval(
  app,
  {
    authenticate,
    readContext,
  ...
  const gateway = createAtlasRecordDataGateway({
    ...
    fieldSecurity: {
      async project({ rows, requestedFields }) {
    ...
    async (req, res) => {
      try {
        ...
        if (
          ...
          return res.status(403).json({ code: "AI_ADMISSION_DENIED" });
        const args = req.body;
        tool.validateArguments(args);
        if (
          args.entityCode !== "business_partner" ||
          args.descriptorHash !== descriptorHash
        )
          return res.status(409).json({ code: "AI_RELEASE_MISMATCH" });
        ...
        res.status(403).json({
          code: "AI_RETRIEVAL_DENIED",
          reason: error.code ?? "RETRIEVAL_UNAVAILABLE",
        });
      }
    },
  );
