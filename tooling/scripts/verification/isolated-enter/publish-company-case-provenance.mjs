...
const staged = JSON.parse(
  fs.readFileSync(
    "governance/policy/reports/business-partner-company-case-provenance-stage-20260912.1789183181595.dev.json",
  ),
).result.body;
assert.equal(staged.id, "2d735de5-0675-4e2f-9ce0-6f7e783e7f01");
...
const events = [];
const out =
  "governance/policy/reports/business-partner-company-case-provenance-review-" +
  Date.now() +
  ".dev.json";
try {
        ...
        const r = await fetch("/api/relay/" + path, {
          ...
          headers: {
            ...
            ...(method === "POST"
              ? {
                  "idempotency-key":
                    "bp-company-initial-publish-20260912-provenance",
                }
              : {}),
          },
