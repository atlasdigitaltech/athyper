...
const verification = JSON.parse(
  fs.readFileSync(
    "governance/policy/reports/business-partner-company-case-provenance-signing-1789183444279.dev.json",
  ),
);
const pins = Object.fromEntries(
  verification.verified.map((r) => [r.releaseId, r.artifactHash]),
);
const migration = fs
  .readFileSync(
...
if (
  ...
  throw Error("APPROVAL_WINDOW_REQUIRED");
const dryRun = false; // Approved isolated candidate projection only; no enforcement mode change.
const script = `
...
const rows=await container.adapters.athyperDatabase.database.transaction().execute(async tx=>{await sql.raw('SET TRANSACTION READ ONLY').execute(tx);await sql.raw("SET LOCAL app.current_tenant_id='44444444-4444-4444-8444-444444444444'").execute(tx);return(await sql.raw("SELECT a.*,r.release_key,r.release_no FROM publication.artifact a JOIN publication.release r ON r.id=a.publication_release_id WHERE r.id IN ('ecbf538d-a1b7-4ba3-bb28-147d7263ed76') AND a.status='signed'").execute(tx)).rows;});
if(rows.length!==1)throw Error('DEPENDENCY_SET_MISMATCH');
...
const out =
  "governance/policy/reports/business-partner-company-signed-projection-application-" +
  Date.now() +
  ".dev.json";
