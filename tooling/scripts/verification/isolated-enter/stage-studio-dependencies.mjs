...
try {
  ...
  const p = JSON.parse(
    fs.readFileSync(
      "governance/policy/reviews/bp-dependencies-20260912/proposal.json",
      "utf8",
    ),
  );
  const result = await page.evaluate(
    async ({ bundle }) => {
      ...
      const r = await fetch("/api/relay/studio/business-partner-definitions", {
        method: "POST",
        headers: {
          ...
          "idempotency-key": "bp-onboarding-dependency-20260912-fd57fb23",
        },
        body: JSON.stringify({ bundle, targetPlanes: ["neon"] }),
      });
      return { status: r.status, body: await r.json() };
    },
    { bundle: p.onboarding.bundle },
  );
  const report = {
  ...
  const out =
    "governance/policy/reports/business-partner-onboarding-stage-20260912." +
    Date.now() +
    ".dev.json";
