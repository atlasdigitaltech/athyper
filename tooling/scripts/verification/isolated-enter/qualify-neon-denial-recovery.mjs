...
const p = JSON.parse(
  ...
  revocation = JSON.parse(
    fs.readFileSync(
      "governance/policy/reports/business-partner-neon-final-revocation-20260912.dev.json",
    ),
  );
...
assert.ok(artifact);
const bad = root + "/neon-denial-recovery-incompatible-artifact.json";
...
const failedName = "athyper-bp-neon-denial-incompatible-recovery-probe";
docker([
  "run",
  "-d",
  "--name",
...
for (const mode of ["worker", "api"]) {
  const name = "athyper-bp-enter-" + mode,
    prior = name + "-before-neon-denial-recovery";
  docker(["stop", name]);
...
const output =
  "governance/policy/reports/business-partner-neon-denial-compatible-recovery-20260912.dev.json";
fs.writeFileSync(
  output,
  JSON.stringify(
    {
