...
const statePath =
  "governance/policy/reports/business-partner-child-provenance-stage-20260912.dev.json";
...
try {
  ...
  if (!state.changeSet) {
    state.changeSet = await call("change-sets", "POST", {
      ...
      branchCode: "bp-provenance-20260912",
      title: "BP child publication provenance correction; unchanged policy",
    });
    save();
  }
  if (!state.graphStored) {
    const graph = JSON.parse(
      fs.readFileSync(
        "governance/policy/reviews/bp-dependencies-20260912/child-provenance.candidate.json",
        "utf8",
      ),
    );
