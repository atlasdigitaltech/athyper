...
assert.ok(artifact);
const bad = root + "/affordance-count-recovery-incompatible-artifact.json";
...
const failedName = "athyper-bp-affordance-count-incompatible-recovery-probe";
docker([
  "run",
  "-d",
  "--name",
...
for (const mode of ["worker", "api"]) {
  const name = "athyper-bp-enter-" + mode,
    prior = name + "-before-affordance-count-recovery";
  docker(["stop", name]);
...
const output = `governance/policy/reports/business-partner-affordance-count-compatible-recovery-${run}.dev.json`;
fs.writeFileSync(
  output,
  JSON.stringify(
    {
