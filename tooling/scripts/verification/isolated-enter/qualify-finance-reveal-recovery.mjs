...
const p = JSON.parse(
  ...
  revocation = JSON.parse(
    fs.readFileSync(
      "governance/policy/reports/business-partner-finance-reveal-revocation-20260912.dev.json",
    ),
  );
...
assert.ok(artifact);
const bad = root + "/finance-reveal-recovery-incompatible-artifact.json";
...
const failedName = "athyper-bp-finance-reveal-incompatible-recovery-probe";
docker([
  "run",
  "-d",
  "--name",
...
for (const mode of ["worker", "api"]) {
  const name = "athyper-bp-enter-" + mode,
    prior = name + "-before-finance-reveal-recovery";
  docker(["stop", name]);
...
for (const account of ["catl.admin", "catl.owner"]) {
  const r = JSON.parse(
    docker([
      ...
      account,
      "/api/records/business_partner/01a092d1-8242-7948-9ce9-6f19c38c4b27",
      "GET",
    ]),
  );
...
const atlas = JSON.parse(
  docker(
    ...
    JSON.stringify({
      ...
      recordId: "01a092d1-8242-7948-9ce9-6f19c38c4b27",
      descriptorHash:
        "1e1f4dd20f900b220b644eb936baf0474c01357be4f3b18290340efea6ad9cda",
    }),
  ),
);
assert.equal(atlas.status, 403);
assert.equal(atlas.body.code, "AI_ADMISSION_DENIED");
assert.equal(atlas.releaseSet, p.releaseSetHash);
checks.push({
  account: "catl.admin",
  surface: "scopedAtlas",
  status: atlas.status,
  code: atlas.body.code,
  releaseSet: atlas.releaseSet,
});
const after = fp();
assert.deepEqual(after, before);
const output =
  "governance/policy/reports/business-partner-finance-reveal-compatible-recovery-20260912.dev.json";
fs.writeFileSync(
  output,
  JSON.stringify(
    {
