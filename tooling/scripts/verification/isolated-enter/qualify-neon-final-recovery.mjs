...
assert.ok(artifact);
const bad = root + "/neon-final-recovery-incompatible-artifact.json";
...
const failedName = "athyper-bp-neon-final-incompatible-recovery-probe";
docker([
  "run",
  "-d",
  "--name",
...
for (const mode of ["worker", "api"]) {
  const name = "athyper-bp-enter-" + mode,
    prior = name + "-before-neon-final-recovery";
  docker(["stop", name]);
...
const output =
  "governance/policy/reports/business-partner-neon-final-compatible-recovery-20260912.dev.json";
fs.writeFileSync(
  output,
  JSON.stringify(
    {
