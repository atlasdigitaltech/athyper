...
const p = JSON.parse(
  fs.readFileSync(
    "governance/policy/reviews/bp-dependencies-20260912/case-initial.proposal.json",
  ),
);
const staged = JSON.parse(
  fs.readFileSync(
    "governance/policy/reports/business-partner-case-initial-stage-20260912.1789168268619.dev.json",
  ),
).result.body;
assert.equal(staged.id, "5e5f938b-d332-4a68-9e74-0c2a716212b9");
...
const events = [];
const out =
  "governance/policy/reports/business-partner-case-initial-review-" +
  Date.now() +
  ".dev.json";
try {
        ...
        const r = await fetch("/api/relay/" + path, {
          ...
          headers: {
            ...
            ...(method === "POST"
              ? {
                  "idempotency-key":
                    "bp-case-initial-publish-20260912-1811da8d",
                }
              : {}),
          },
  ...
  const current = await call(
    "studio/business-partner-case-contracts/" + staged.id,
    "GET",
  );
  ...
  const result = await call(
    "studio/business-partner-case-contracts/" + staged.id + "/publish",
    "POST",
