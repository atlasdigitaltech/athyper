...
const root =
  os.homedir() +
  "/.athyper/instances/dev/deployments/bp-enter-isolated-20260911/consolidated-backend-build-v2";
...
const installed = JSON.parse(
  run([
    ...
    "--input-type=module",
    "-e",
    "import fs from 'node:fs'; const root='/app/server/node_modules/@athyper';console.log(JSON.stringify(Object.fromEntries(fs.readdirSync(root).map(n=>['@athyper/'+n,fs.realpathSync(root+'/'+n)]))));",
  ]),
);
const manifests = cp
...
const report =
  "governance/policy/reports/business-partner-consolidated-backend-image-20260912-v2.dev.json";
fs.writeFileSync(
  report,
  JSON.stringify(
    {
