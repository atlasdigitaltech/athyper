...
const build = JSON.parse(
  fs.readFileSync(
    "governance/policy/reports/business-partner-company-correction-backend-image-20260912.dev.json",
  ),
);
...
const rb = root + "/company-correction-harness/release-boundary.mjs";
fs.writeFileSync(
  rb,
...
const manifest = {
  ...
  harness: fs
    .readdirSync(root + "/company-correction-harness")
    .filter((n) =>
      fs.statSync(root + "/company-correction-harness/" + n).isFile(),
    )
    .map((name) => ({
      name,
      sha256: sha(
        fs.readFileSync(root + "/company-correction-harness/" + name),
      ),
    })),
  reason:
...
write(
  "governance/policy/reviews/business-partner-company-image-correction-20260912.proposal.dev.json",
  amendment,
);
const docker = (args) =>
