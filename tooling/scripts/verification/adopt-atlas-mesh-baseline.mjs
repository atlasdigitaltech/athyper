...
import { planMeshBaseline as planAdoption, assertCurrent } from './atlas-mesh-baseline-model.mjs';

const args = process.argv.slice(2);
const apply = args.includes('--apply');
const rollbackIndex = args.indexOf('--rollback');
const rollbackId = rollbackIndex < 0 ? null : args[rollbackIndex + 1];
if (args.some((arg, i) => !['--apply', '--rollback'].includes(arg) && !(rollbackIndex >= 0 && i === rollbackIndex + 1)) || (rollbackIndex >= 0 && !/^[0-9a-f-]{36}$/.test(rollbackId ?? ''))) throw Error('Usage: adopt-atlas-mesh-baseline.mjs [--apply] [--rollback <import-id>]');
const tenantId = '44444444-4444-4444-8444-444444444444';
const base = {publicationKey:'metadata.entity.network_relationship',releaseId:'205b53cd-ed90-5f8b-a20e-b72234d92ddb',releaseNo:5,compiledHash:'c6e7b8875fd6d72287b413f384aade42cc532fb4d8a94c0304d6b2b433bd8e29'};
const expected = { ...base, tenantId, entityCode: 'network_relationship' };
...
const preamble = `BEGIN; SET LOCAL lock_timeout='5s'; SET LOCAL statement_timeout='15s';
 ...
 SELECT NOT EXISTS(SELECT 1 FROM public.athyper_schema_migration_v1 WHERE migration_name='20260910_mesh_global_baseline_import.sql' AND status='applied') AS install_schema \\gset
 \\if :install_schema
 ${schema}
...
if (rollbackId) {
  ...
  writeFileSync(`docs/examples/atlas-f6/mesh-baseline-revocation.${apply?'applied':'dry-run'}.json`,JSON.stringify(receipt,null,2)+'\n');
  ...
  const source = spawn('docker', psqlArgs('mesh'), {stdio:['pipe','pipe','pipe']});
  ...
  const query = `WITH locked AS (SELECT jsonb_build_object('contract',to_jsonb(c),'descriptor',to_jsonb(d),'head',to_jsonb(h),'applied',to_jsonb(a)) AS capture
    ...
    WHERE h.publication_key=${quote(base.publicationKey)} AND c.tenant_id IS NULL
      AND d.tenant_id IS NULL AND d.plane_code='mesh' AND d.descriptor_kind='entity_runtime' AND d.status='active'
  ...
  try {
    ...
    const result=runStudio(`${preamble}
      ...
      VALUES(${quote(importId)}::uuid,${quote(tenantId)}::uuid,${quote(imported.publicationKey)},${quote(imported.sourceReleaseId)}::uuid,${imported.sourceReleaseNo},${quote(imported.entityCode)},${quote(imported.sourceEntityId)}::uuid,'mesh',${quote(imported.contentHash)},${quote(JSON.stringify(imported))}::jsonb)
    ...
    writeFileSync(`docs/examples/atlas-f6/mesh-baseline-adoption.${apply?'applied':'dry-run'}.json`,JSON.stringify(receipt,null,2)+'\n');
    writeFileSync('docs/examples/atlas-f6/mesh-baseline-import.json',JSON.stringify(imported,null,2)+'\n');
    console.log(JSON.stringify(receipt));
  } finally {
