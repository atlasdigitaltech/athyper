...
const out=execFileSync('docker',['exec','-i','athyper-dev-db-1','psql','-X','-U','postgres','-d','athyper_neon','-At','-v','ON_ERROR_STOP=1'],{input:sql,encoding:'utf8'});
...
const report={schemaVersion:1,kind:'bp_release_19_test_grant_cleanup',capturedAt:new Date().toISOString(),proposalRevision:p.proposalRevision,...counts,applied:apply,rollbackRehearsal:!apply,oldGrantsRestored:false,activationAuthorized:false};
writeFileSync('governance/policy/reports/business-partner-release-19-test-grants-cleanup.'+(apply?'applied':'rehearsal')+'.dev.json',JSON.stringify(report,null,2)+'\n');console.log(report);
