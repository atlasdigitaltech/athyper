...
const sql =
  "SELECT coalesce(json_agg(x),'[]'::json) FROM (SELECT d.id,d.external_id,d.recipient_id,d.recipient_addr,m.event_code,m.payload->>'caseNo' case_no,m.payload->'communication' pin FROM event.notification_delivery d JOIN event.notification_message m ON m.id=d.message_id WHERE m.event_code LIKE 'supplier.onboarding.notice.%' AND d.channel='email' AND d.status='delivered') x";
