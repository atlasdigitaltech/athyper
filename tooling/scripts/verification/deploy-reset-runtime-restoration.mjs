import fs from "node:fs";
import path from "node:path";
import os from "node:os";
import cp from "node:child_process";
import assert from "node:assert/strict";
import { createHash } from "node:crypto";
const run = (args) =>
  cp.execFileSync("docker", args, {
    encoding: "utf8",
    stdio: ["pipe", "pipe", "pipe"],
    maxBuffer: 4000000,
  });
const hash = (bytes) => createHash("sha256").update(bytes).digest("hex");
const reportPath =
  "governance/policy/reports/business-partner-reset-runtime-deployment.dev.json";
assert.ok(
  !fs.existsSync(reportPath),
  "Inspect existing deployment, do not replay",
);
const names = ["athyper-dev-api-1", "athyper-dev-worker-1"];
const before = JSON.parse(run(["inspect", ...names]));
assert.equal(before[0].Image, before[1].Image);
assert.equal(
  before[0].Config.Labels["com.docker.compose.project.config_files"],
  before[1].Config.Labels["com.docker.compose.project.config_files"],
);
const baseConfig =
  before[0].Config.Labels["com.docker.compose.project.config_files"];
assert.ok(!baseConfig.includes(","));
const config = JSON.parse(fs.readFileSync(baseConfig));
assert.equal(config.name, "athyper-dev");
for (const name of ["api", "worker"])
  assert.equal(config.services[name].environment.BP_AUTHORIZATION_MODE, "off");
const fingerprint = () => {
  const state = {};
  for (const database of ["athyper_neon", "athyper_studio"]) {
    const tables = [
      "role",
      "role_permission",
      "group_member",
      "group_role",
      "plane_membership",
      "delegation",
      "delegation_grant",
      "permission",
      "permission_scope_kind",
      "deny_rule",
      "record_acl",
      "override",
      "scope_target",
      "principal_group",
    ];
    state[database] = run([
      "exec",
      "athyper-dev-db-1",
      "psql",
      "-X",
      "-U",
      "postgres",
      "-d",
      database,
      "-At",
      "-v",
      "ON_ERROR_STOP=1",
      "-c",
      [
        ...tables.map(
          (t) =>
            `SELECT '${t}',md5(coalesce(jsonb_agg(to_jsonb(r) ORDER BY to_jsonb(r)::text),'[]'::jsonb)::text) FROM authz.${t} r`,
        ),
        "SELECT 'activation',md5(coalesce(jsonb_agg(to_jsonb(r) ORDER BY to_jsonb(r)::text),'[]'::jsonb)::text) FROM runtime_meta.release_activation_head r",
      ].join(" UNION ALL "),
    ]);
  }
  return hash(JSON.stringify(state));
};
const authorityBefore = fingerprint();
const root = path.join(
  os.homedir(),
  ".athyper/instances/dev/deployments/bp-reset-runtime-" + Date.now(),
);
fs.mkdirSync(root, { recursive: true, mode: 0o700 });
const destination =
  "/app/server/node_modules/.pnpm/@athyper+server-plane-studio-meta-entity-authoring@file+server+packages+planes+studio+meta-entity-authoring/node_modules/@athyper/server-plane-studio-meta-entity-authoring/dist";
const files = [];
const add = (name, destination, bytes) => {
  const staged = "file-" + files.length + ".js";
  fs.writeFileSync(path.join(root, staged), bytes);
  files.push({ name, staged, destination, sha256: hash(bytes) });
};
for (const name of [
  "index.js",
  "deterministic.js",
  "runtime-restoration.js",
  "runtime-restoration-publication.js",
])
  add(
    name,
    destination + "/" + name,
    fs.readFileSync(
      "server/packages/planes/studio/meta-entity-authoring/dist/" + name,
    ),
  );
const pub =
  "/app/server/node_modules/.pnpm/@athyper+server-service-publication@file+server+packages+services+publication/node_modules/@athyper/server-service-publication/dist/kysely-publication-authority-work.js";
add(
  "kysely-publication-authority-work.js",
  pub,
  fs.readFileSync(
    "server/packages/services/publication/dist/kysely-publication-authority-work.js",
  ),
);
const host = "/app/server/dist/composition/register-services.js";
let composition = run([
  "exec",
  "athyper-dev-api-1",
  "node",
  "--input-type=module",
  "-e",
  `import fs from'node:fs';process.stdout.write(fs.readFileSync('${host}','utf8'));`,
]);
const local = fs.readFileSync(
  "server/apps/platform-host/dist/composition/register-services.js",
  "utf8",
);
const begin = local.indexOf(
    "const restored = await prepareRuntimeRestorationRelease(",
  ),
  end = local.indexOf(
    "const successor = await prepareAuthorizationSuccessorRelease(",
    begin,
  );
assert.ok(begin >= 0 && end > begin);
assert.ok(!composition.includes("prepareRuntimeRestorationRelease"));
assert.equal(
  composition.split("prepareAuthorizationSuccessorRelease,").length,
  2,
);
composition = composition.replace(
  "prepareAuthorizationSuccessorRelease,",
  "prepareAuthorizationSuccessorRelease, prepareRuntimeRestorationRelease,",
);
assert.equal(
  composition.split(
    "const successor = await prepareAuthorizationSuccessorRelease(",
  ).length,
  2,
);
composition = composition.replace(
  "const successor = await prepareAuthorizationSuccessorRelease(",
  local.slice(begin, end) +
    "const successor = await prepareAuthorizationSuccessorRelease(",
);
add("register-services.js", host, Buffer.from(composition));

const baseTag =
    "athyper/runtime:reset-runtime-base-" + before[0].Image.slice(7, 19),
  tag = "athyper/runtime:reset-runtime-" + Date.now();
run(["tag", before[0].Image, baseTag]);
fs.writeFileSync(
  path.join(root, "Dockerfile"),
  `FROM ${baseTag}\n` +
    files.map((f) => `COPY ${f.staged} ${f.destination}`).join("\n") +
    "\n",
);
run(["build", "--pull=false", "--network=none", "-t", tag, root]);
run([
  "run",
  "--rm",
  "--network",
  "none",
  "--entrypoint",
  "node",
  tag,
  "--input-type=module",
  "-e",
  `await import('${destination}/routes.js');await import('${destination}/runtime-restoration-publication.js');await import('/app/server/dist/composition/register-services.js');`,
]);
const image = run(["image", "inspect", "--format", "{{.Id}}", tag]).trim();
for (const name of ["api", "worker"]) {
  config.services[name].image = image;
  config.services[name].pull_policy = "never";
}
const nextConfig = path.join(root, "compose.private.json");
fs.writeFileSync(nextConfig, JSON.stringify(config, null, 2) + "\n", {
  mode: 0o600,
});
const report = {
  schemaVersion: 1,
  startedAt: new Date().toISOString(),
  baseImage: before[0].Image,
  image,
  files,
  moduleLoadPassed: true,
  baseConfig,
  nextConfig,
  authorityBefore,
  deployed: false,
  grantsChanged: false,
  activationChanged: false,
};
const save = () =>
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2) + "\n");
save();
try {
  assert.equal(
    fingerprint(),
    authorityBefore,
    "Authority changed during image preparation",
  );
  run([
    "compose",
    "-p",
    "athyper-dev",
    "-f",
    nextConfig,
    "up",
    "-d",
    "--no-deps",
    "--force-recreate",
    "api",
    "worker",
  ]);
  let current;
  for (let i = 0; i < 50; i++) {
    current = JSON.parse(run(["inspect", ...names]));
    if (current.every((c) => c.State.Health?.Status === "healthy")) break;
    await new Promise((r) => setTimeout(r, 1000));
  }
  assert.ok(
    current.every((c) => c.State.Health?.Status === "healthy"),
    "Health check failed",
  );
  report.authorityAfter = fingerprint();
  assert.equal(
    report.authorityAfter,
    authorityBefore,
    "Authority or activation drift",
  );
  report.deployed = true;
  report.healthy = true;
} catch (e) {
  report.failure = String(e.stderr ?? e.message).slice(0, 1200);
  process.exitCode = 1;
  try {
    run([
      "compose",
      "-p",
      "athyper-dev",
      "-f",
      baseConfig,
      "up",
      "-d",
      "--no-deps",
      "--force-recreate",
      "api",
      "worker",
    ]);
    report.configurationRestored = true;
  } catch {
    report.configurationRestored = false;
  }
}
report.finishedAt = new Date().toISOString();
save();
console.log(JSON.stringify(report));
