...
const save=()=>writeFileSync("governance/policy/reports/task-response-escalation-browser-live.dev.json",JSON.stringify(report,null,2)+"\n");
async function call(
  actor: any,
  path: string,
  data?: any,
...
async function create(level: string) {
  ...
  const created = await post(endpoint, {
    ...
    proposedPayload: {
      ...
      name: `DEV R1 response escalation ${level} ${key}`,
...
try{
 ...
 const before=await view(id);assert.equal(before.selection.manifest.revision.version,3);report.manifest=before.selection.manifest.revision;
 ...
 await p.getByLabel('Reason for supervisor attention',{exact:true}).fill('catl.admin has not responded; catl.owner supervisor follow-up is requested.');await click(p,'Escalate pending response');
 await delivered(id,'information_escalated');const escalated=await view(id);assert.equal(escalated.information.at(-1).escalated_to,'645b6a55-3355-526a-9643-3900425bde47');assert.equal(escalated.information.at(-1).state,'open');assert.ok(escalated.executions.some((e:any)=>e.waitingForInformation));
 assert.deepEqual(escalated.executions.map((e:any)=>[e.work_item_id,e.assignee_principal_id]),before.executions.map((e:any)=>[e.work_item_id,e.assignee_principal_id]));report.checks.push('catl.admin response escalates to catl.owner by live browser; one supervisor email/inbox notice; assignment unchanged and approval blocked');save();
 mkdirSync('governance/policy/reports/task-response-escalation-browser.dev',{recursive:true});await p.screenshot({path:'governance/policy/reports/task-response-escalation-browser.dev/escalated.png',fullPage:true});
 ...
 mkdirSync('governance/policy/reports/task-response-escalation-browser.dev',{recursive:true});await p.screenshot({path:'governance/policy/reports/task-response-escalation-browser.dev/resolved.png',fullPage:true});report.checks.push('Reviewer browser accepts clarification and resumes review');
