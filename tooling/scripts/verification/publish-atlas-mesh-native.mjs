...
const id=JSON.parse(readFileSync('docs/examples/atlas-f6/mesh-native-draft.json','utf8')).body.id,tenant='44444444-4444-4444-8444-444444444444';
const receiptPath='docs/examples/atlas-f6/mesh-authenticated-publication.json';
...
try{
 const initial=await c.storageState(),csrf=initial.cookies.find(x=>x.name==='__Host-athyper-csrf'&&x.domain==='studio.dev.athyper.test');
 ...
 const state=await c.storageState(),cookie=state.cookies.find(x=>x.name==='__Host-athyper-csrf'&&x.domain==='studio.dev.athyper.test');if(!cookie)throw Error('Missing CSRF token');
 ...
 const data=step==='stage'?JSON.parse(readFileSync('docs/examples/atlas-f6/mesh-baseline-native-graph.json','utf8')):step==='activate'?{plane:'mesh'}:{expectedRevision:current.revision,...(step==='redispatch'?{releaseId:current.releaseId}:{}),...(['publish','redispatch'].includes(step)?{targetPlanes:['mesh']}:{})};
