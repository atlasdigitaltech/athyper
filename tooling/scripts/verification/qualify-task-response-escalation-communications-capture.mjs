...
const source=JSON.parse(readFileSync("governance/policy/reports/task-response-escalation-browser-live.dev.json","utf8"));
const cases=[{id:source.caseId}];
const ids=cases.map(c=>{assert.match(c.id,/^[0-9a-f-]{36}$/);return "'"+c.id+"'::uuid";}).join(',');
const sql =
  "SELECT coalesce(json_agg(x),'[]'::json) FROM (SELECT d.id,d.external_id,d.recipient_id,d.recipient_addr,m.event_code,m.payload->>'caseNo' case_no,m.payload->'communication' pin FROM event.notification_delivery d JOIN event.notification_message m ON m.id=d.message_id WHERE m.event_code LIKE 'supplier.onboarding.notice.%' AND d.channel='email' AND d.status='delivered' AND m.event_code IN ('supplier.onboarding.notice.information_requested','supplier.onboarding.notice.information_answered','supplier.onboarding.notice.information_escalated') AND m.entity_id IN ("+ids+")) x";
...
writeFileSync(
  "governance/policy/reports/task-response-escalation-communications-capture.dev.json",
