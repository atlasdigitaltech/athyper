...
const name='20260910_operation_projection_release_identity.sql',migration=readFileSync('server/db/migrations/'+name,'utf8'),hash=createHash('sha256').update(migration).digest('hex');
...
const receipt={observedAt:new Date().toISOString(),environment:'dev',plane:'neon',migration:name,sha256:hash,applied:apply,runtimeHeadChanged:false};writeFileSync(`docs/examples/atlas-f5/cirrus-operation-projection-identity.${apply?'applied':'dry-run'}.json`,JSON.stringify(receipt,null,2)+'\n');console.log(JSON.stringify(receipt));
