...
import {createAuthenticatedOperationReview} from './entity-authorization/authenticated-operation-review.mjs';
...
const handler=createAuthenticatedOperationReview({enabled:true,origin,nominationPath:'/review/nomination.json',packetPath:'/review/packet.json',packetSha256:process.env.LOCAL_BP_OPERATION_REVIEW_PACKET_SHA256!,outputDirectory:'/review/output',resolveSession:r=>runtime.resolveRelaySession(r),async currentIdentity(r){const response=await relay(new Request(origin+'/api/relay/iam/me',{headers:{cookie:r.headers.get('cookie')??''}}),{params:Promise.resolve({path:['iam','me']})});if(!response.ok)throw Object.assign(Error('Identity unavailable'),{status:response.status===401?401:503,code:'REVIEW_IDENTITY_UNAVAILABLE'});return response.json();}});
createServer(async(req,res)=>{try{
 const url=new URL(req.url??'/',origin);
 let result:Response;
 if(url.pathname==='/livez')result=new Response('ok');
 else if(url.pathname==='/mdg/operation-review/review.js')result=new Response(await readFile('/app/review.js'),{headers:{'content-type':'text/javascript','cache-control':'no-store'}});
 else if(url.pathname==='/mdg/operation-review')result=new Response('<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Business Partner operation review</title></head><body><nav><a href="/home">NEON home</a> · <a href="/api/auth/login?returnTo=%2Fmdg%2Foperation-review">Sign in</a></nav><div id="review-root"></div><script type="module" src="/mdg/operation-review/review.js"></script></body></html>',{headers:{'content-type':'text/html','cache-control':'no-store','x-content-type-options':'nosniff'}});
 else if(url.pathname==='/api/governance/operation-review'){
