...
const origin = "https://studio.dev.athyper.test", prefix = "governance/policy/reports/task-policy-authoring-live";
const browser = await chromium.launch({headless:true});
const report:any={at:new Date().toISOString(),passed:false,checks:[],boundary:"Real deployed Studio browser, BFF, authenticated API, policy tables and independent publication. This fixture publishes an unbound policy; process release cutover is separately qualified."};
const contexts=[];
try {
 ...
 const input={...exampleTaskEditPolicy,definition:{...exampleTaskEditPolicy.definition,name:`DEV task policy ${randomUUID()}`,effectiveFrom:new Date().toISOString().slice(0,10)}};
 ...
 assert.equal(response.status(),200,JSON.stringify(body));assert.equal(body.status,'pending_approval');report.policyId=body.definition.id;report.policyHash=body.hash;report.checks.push('Browser creates one immutable revision with four passing fixtures');
 ...
 await reviewSection.locator('input').fill(body.definition.id);
 ...
 report.passed=true;
} catch(error){report.error=String(error); const page=contexts.at(-1)?.pages()[0]; if(page){ report.ui={path:new URL(page.url()).pathname,alerts:await page.getByRole("alert").allTextContents(),buttons:await page.getByRole("button").allTextContents()}; await page.screenshot({path:"/tmp/task-policy-failure.png",fullPage:true}); } throw error;}
