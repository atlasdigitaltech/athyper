...
import {
  ...
  writeOrCheck,
} from "./generate-server-route-manifest.mjs";
...
function snapshot(t) {
  ...
  f.write(
    ...
    'router.get("/audit/status", handler);\nrouter.post("/retired", handler);',
  );
...
test("source snapshot checksums and paths are deterministic across checkout locations", (t) => {
  const a = snapshot(t),
    b = snapshot(t);
  assert.deepEqual(
    JSON.parse(readFileSync(join(a.root, baselinePath))),
    JSON.parse(readFileSync(join(b.root, baselinePath))),
  );
  b.write(
    "server-backup/routes.ts",
    'router.get("/audit/status", handler); // changed source',
  );
  const updated = refreshLegacyBaseline(b);
  assert.notEqual(
    updated.provenance.sha256,
    JSON.parse(readFileSync(join(a.root, baselinePath))).provenance.sha256,
  );
});
...
test("malformed baseline evidence fails instead of silently dropping legacy routes", (t) => {
  ...
  for (const invalid of [
    { ...baseline, schemaVersion: 9 },
    { ...baseline, provenance: {} },
    { ...baseline, routes: [] },
    {
      ...baseline,
      routes: [
        { ...baseline.routes[0], source: "server-backup/../server/routes.ts" },
      ],
    },
...
test("missing current source fails rather than publishing every legacy route as retired", (t) => {
  ...
  assert.throws(
    () => buildRouteManifest(root),
    /Current server source directory is required/,
  );
});

test("retains native and compatibility paths declared in Express arrays", () => {
  const routes = extractRoutesFromSource(
    'app.post(["/api/neon/business-partner-cases/:requestId/materialize", "/api/neon/business-partner-requests/:requestId/apply"], handler);',
  );
  assert.deepEqual(
    routes.map(({ method, declaredPath }) => [method, declaredPath]),
    [
...
test("normalizes parameter names and the legacy API mount", () => {
  assert.equal(
    normalizeRoutePath("/records/:entityCode/:recordId/", {
      defaultMount: "/api",
    }),
    "/api/records/:param/:param",
  );
  assert.equal(
    normalizeRoutePath("/health/live", { defaultMount: "/api" }),
    "/health/live",
  );
});
...
test("resolves router and nested-router mounts", () => {
  ...
  `;
  assert.deepEqual(
    extractRoutesFromSource(source).map(({ method, path }) => [method, path]),
    [["GET", "/api/iam/sessions/:param"]],
  );
});
...
test("extracts defineRouteContract routes", () => {
  ...
  assert.deepEqual(
    extractRoutesFromSource(source).map(({ method, path, kind }) => [
      method,
      path,
      kind,
    ]),
    [["PATCH", "/api/preferences/:param", "contract"]],
  );
});
...
test("extracts local contract-factory routes and expands their loops", () => {
  ...
  `;
  assert.deepEqual(
    extractRoutesFromSource(source)
...
test("expands inline and named finite route loops", () => {
  ...
  `;
  assert.deepEqual(
    extractRoutesFromSource(source, { defaultMount: "/api" })
      .map(({ path }) => path)
      .sort(),
    [
...
test("current manifests include finance descriptor routes alongside host contracts", (t) => {
  ...
  const routes = buildRouteManifest(f.root, {
    currentOnly: true,
  }).routes.filter((route) => route.path.startsWith("/api/neon/finance/"));
  assert.equal(routes.length, 29);
  assert.ok(routes.every((route) => route.current.length === 1));
  assert.ok(
    routes.some((route) => route.path === "/api/neon/finance/budget/command"),
  );
});

test("current manifests retain named contract factories without adding unmounted router paths", (t) => {
  const { root, write } = snapshot(t);
  write(
    "server/control.ts",
    `
    ...
    const router = Router();
    app.use('/api', router);
    router.get('/mounted', handler);
    function featureContract(method, path) {
      return defineRouteContract({method, path, operationId: 'features.read', responses: {200: {description: 'OK'}}});
    }
    registerContractRoute(application, featureContract('get', '/api/features'), handler);
  `,
  );
  const manifest = buildRouteManifest(root);
  assert.ok(
    manifest.routes.some(
      (route) => route.identity === "GET /api/features" && route.current.length,
    ),
  );
  assert.ok(
    manifest.routes.some(
      (route) => route.identity === "GET /api/mounted" && route.current.length,
    ),
  );
  assert.ok(
    !manifest.routes.some((route) => route.identity === "GET /mounted"),
  );
});

test("current inventory excludes DB and host verification route fixtures", (t) => {
  const { root, write } = fixture(t);
  write(
    "server/db/scripts/tests/integration/probe.ts",
    'app.post("/api/db-fixture", handler);',
  );
  write(
    "server/apps/platform-host/scripts/probe.ts",
    'app.post("/api/host-fixture", handler);',
  );
  const manifest = buildRouteManifest(root, { currentOnly: true });
  assert.deepEqual(
    manifest.routes.map((route) => route.identity),
