...
export function extractStaticUrlRoutes(source) {
  ...
  function collectReceivers(node) {
    if (
      ...
      [...receiverTypes].some((type) =>
        new RegExp(`\\b${type}\\b`).test(node.type.getText(file)),
      )
    )
    ...
    if (
      ...
      if (
        (ts.isIdentifier(callee) && constructors.has(callee.text)) ||
        (ts.isPropertyAccessExpression(callee) &&
          constructors.has(callee.expression.getText(file)) &&
  ...
  function bind(parameters, args, env) {
    ...
    parameters.forEach((p, i) => {
      if (ts.isIdentifier(p.name))
        local.set(
          p.name.text,
          i >= args.length && p.initializer
            ? value(p.initializer, local)
            : args[i],
        );
    });
    return local;
  ...
  function value(node, env, depth = 0) {
    ...
    if (
      ...
      return value(node.expression, env, depth + 1);
    if (ts.isArrayLiteralExpression(node))
      return node.elements.map((n) => value(n, env, depth + 1));
      ...
      return Object.fromEntries(
        node.properties
          ...
          .map((p) => [
            ...
            value(p.initializer, env, depth + 1),
          ]),
      );
    ...
    if (
      ...
      const left = value(node.left, env, depth + 1),
        right = value(node.right, env, depth + 1);
    ...
    if (ts.isConditionalExpression(node)) {
      ...
      // A deployment flag may choose between finite path arrays. Inventory both
      // branches only when every path is statically known; never omit a dynamic arm.
      const yes = value(node.whenTrue, env, depth + 1);
      const no = value(node.whenFalse, env, depth + 1);
      if (
        Array.isArray(yes) &&
        Array.isArray(no) &&
        [...yes, ...no].every(
    ...
    if (ts.isTemplateExpression(node)) {
      ...
      for (const span of node.templateSpans) {
        const part = value(span.expression, env, depth + 1);
    ...
    if (ts.isCallExpression(node)) {
      if (
        ...
        if (
          ...
          return receiver[node.expression.name.text](...args);
      }
      const fn = value(node.expression, env, depth + 1);
      if (fn?.arrow && !ts.isBlock(fn.arrow.body))
        return value(
          fn.arrow.body,
          bind(
            fn.arrow.parameters,
            node.arguments.map((n) => value(n, env, depth + 1)),
            fn.env,
          ),
          depth + 1,
        );
  ...
  function visit(node, env, stack = []) {
    ...
    if (ts.isArrowFunction(node) || ts.isFunctionExpression(node)) {
      visit(node.body, new Map(env), stack);
      return;
    }
    if (ts.isVariableDeclaration(node) && ts.isIdentifier(node.name)) {
      env.set(node.name.text, value(node.initializer, env));
    }
    if (ts.isObjectLiteralExpression(node)) {
      const descriptor = value(node, env);
      if (descriptor?.kind === "route" && descriptor.method && descriptor.path)
        emit(node, descriptor.method, descriptor.path, "contract");
    }
        ...
        for (const row of rows) {
          ...
          if (name && ts.isIdentifier(name)) local.set(name.text, row);
            ...
            name.elements.forEach((element, i) =>
              local.set(element.name?.getText(file), row?.[i]),
            );
    ...
    if (ts.isCallExpression(node)) {
      ...
      if (
        ...
        if (node.arguments.length > 0 && node.arguments.every(isCallback))
          return;
      }
      if (
        ts.isPropertyAccessExpression(expression) &&
        receivers.has(expression.expression.getText(file)) &&
        ["route", "all", "use"].includes(expression.name.text)
      ) {
        attempted.set(node.pos, node);
        return;
      }
      if (
        ts.isPropertyAccessExpression(expression) &&
        receivers.has(expression.expression.getText(file)) &&
        methods.has(expression.name.text)
      ) {
        emit(
          node,
          expression.name.text,
          value(node.arguments[0], env),
          "direct",
        );
      ...
      if (ts.isIdentifier(expression)) {
        ...
        if (name === "defineRouteContract") {
          ...
          const property = (key) =>
            properties.find((p) => p.name?.getText(file) === key);
          const read = (key) => {
            const p = property(key);
            return p && ts.isShorthandPropertyAssignment(p)
              ? env.get(key)
              : value(p?.initializer, env);
          };
        ...
        if (fn && !stack.includes(name)) {
          visit(
            ...
            bind(
              fn.parameters,
              node.arguments.map((n) => value(n, env)),
              env,
            ),
            [...stack, name],
          );
  ...
  return {
    ...
    unresolved: [...attempted]
      .filter(([pos]) => !resolved.has(pos))
      .map(([, node]) => ({
        line: file.getLineAndCharacterOfPosition(node.getStart(file)).line + 1,
        expression: node.getText(file).slice(0, 160),
      })),
  };
}
