...
import {
  importSpecifiers,
  packageDependencyCounts,
  referencesBackupRoot,
} from "./frontend-spine-governance.mjs";

describe("frontend spine governance", () => {
  it("detects imports from every reference-only root", () => {
    for (const value of [
      "../../apps-backup/neon",
      "../packages-backup/platform/ui",
      "server-backup/src/app",
    ]) {
...
it("application code cannot reach tooling through an undeclared relative import", async () => {
  const { crossesAppPackage } = await import("./frontend-spine-governance.mjs");
  assert.equal(
    crossesAppPackage(
      "/repo",
      "/repo/apps/neon/lib/review.ts",
      "../../../tooling/review.mjs",
    ),
    true,
  );
  assert.equal(
    crossesAppPackage("/repo", "/repo/apps/neon/lib/review.ts", "./auth"),
    false,
  );
  assert.equal(
    crossesAppPackage(
      "/repo",
      "/repo/apps/neon/lib/review.ts",
      "@athyper/platform-iam-governance-review",
    ),
    false,
  );
});
