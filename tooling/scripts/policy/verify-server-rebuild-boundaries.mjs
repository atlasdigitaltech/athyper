...
function segments(path) {
  return posix(path).split("/");
}
function ignored(path) {
  return segments(path).some((part) => IGNORED_SEGMENTS.has(part));
}
  ...
  for (const entry of readdirSync(root)) {
    ...
    if (stat.isDirectory()) {
      if (!IGNORED_SEGMENTS.has(entry))
        yield* walk(absolute, includePackageJson);
      continue;
    }
...
function discoverPackages(repoRoot) {
  ...
  for (const searchRoot of [
    join(serverRoot, "apps"),
    join(serverRoot, "packages"),
    join(serverRoot, "db"),
  ]) {
    for (const path of walk(searchRoot, true)) {
      if (
        path.endsWith(`${sep}package.json`) &&
        !ignored(relative(serverRoot, path))
      )
        manifests.push(path);
    }
  }
...
function layer(pkg, repoRoot) {
  ...
  if (path === "package.json" || pkg.name === ROOT_ORCHESTRATOR)
    return "orchestrator";
...
function packageForFile(path, packages) {
  return packages
    .filter((pkg) => inside(path, pkg.root))
...
export function analyzeServerRebuild(repoRoot) {
  ...
  for (const pkg of packages) {
    ...
    if (typeof pkg.name !== "string" || !pkg.name.startsWith(SERVER_PREFIX)) {
      add(
        violations,
        "PACKAGE_NAME",
        `${rel} must use an ${SERVER_PREFIX}* name`,
      );
      continue;
    }
    if (byName.has(pkg.name))
      add(
        violations,
        "DUPLICATE_PACKAGE",
        `${pkg.name} is declared by ${byName.get(pkg.name)} and ${rel}`,
      );
  ...
  if (
    !orchestrator ||
    orchestrator.name !== ROOT_ORCHESTRATOR ||
    orchestrator.manifest.private !== true ||
    orchestrator.manifest.exports !== undefined ||
    orchestrator.manifest.dependencies !== undefined
  ) {
    add(
      violations,
      "ROOT_ORCHESTRATOR",
      "server/package.json must be private @athyper/server-workspace with no exports or runtime dependencies",
    );
  }
  for (const pkg of packages) {
    if (
      pkg !== orchestrator &&
      dependencyNames(pkg.manifest).has(ROOT_ORCHESTRATOR)
    ) {
      add(
        violations,
        "ROOT_ORCHESTRATOR",
        `${pkg.name} must not depend on ${ROOT_ORCHESTRATOR}`,
      );
    }
  }
  ...
  for (const pkg of packages) {
    ...
    const sourceRoot = existsSync(join(pkg.root, "src"))
      ? join(pkg.root, "src")
      : pkg.root;
    const roots =
      sourceRoot === pkg.root
        ? [sourceRoot]
        : [sourceRoot, join(pkg.root, "scripts")];
    ...
    for (const specifier of imports(source, file)) {
      if (
        ...
        add(
          violations,
          "KERNEL",
          `${relFile} imports forbidden kernel namespace ${specifier}`,
        );
      }
      if (specifier.startsWith(".")) {
        ...
        if (targetOwner && targetOwner !== owner)
          add(
            ...
            `${relFile} crosses into ${targetOwner.name} through ${specifier}`,
          );
        continue;
      }
      ...
      if (
        !dependencyNames(owner.manifest).has(importedName) &&
        importedName !== owner.name
      ) {
        add(
          violations,
          "UNDECLARED_DEPENDENCY",
          `${relFile} imports undeclared dependency ${importedName}`,
        );
      }
      if (!target) {
        if (specifier.startsWith(SERVER_PREFIX))
          add(
            violations,
            "UNRESOLVED_SERVER_PACKAGE",
            `${relFile} imports unknown server package ${specifier}`,
          );
        continue;
      }
      if (specifier !== target.name) {
        const subpath = `.${specifier.slice(target.name.length)}`;
        if (!exportedSubpaths(target.manifest).has(subpath))
          add(
            violations,
            "DEEP_IMPORT",
            `${relFile} imports unexported subpath ${specifier}`,
          );
      }
      const ownerLayer = layer(owner, root);
      const targetLayer = layer(target, root);
      if (targetLayer === "host" && ownerLayer !== "host")
        add(
          violations,
          "HOST_DIRECTION",
          `${owner.name} must not import the platform host`,
        );
      const testUtility =
        /\.(?:test|spec)\.[cm]?[jt]sx?$/.test(file) &&
        targetLayer === "test-utils" &&
        owner.manifest.devDependencies?.[target.name];
      if (
        !testUtility &&
        (RANK.get(ownerLayer) ?? -1) < (RANK.get(targetLayer) ?? -1)
      ) {
        add(
          violations,
          "LAYER_DIRECTION",
          `${owner.name} (${ownerLayer}) must not depend on ${target.name} (${targetLayer})`,
        );
      }
      if (ownerLayer === "foundation" && target !== owner)
        add(
          violations,
          "FOUNDATION_ISOLATION",
          `Foundation must not import ${target.name}`,
        );
    }
  }
  ...
  if (orchestrator) {
    for (const name of dependencyNames(orchestrator.manifest)) {
      if (!NODE_BUILTINS.has(name) && name.startsWith(SERVER_PREFIX))
        add(
          violations,
          "ROOT_ORCHESTRATOR",
          `Root orchestrator must not depend on ${name}`,
        );
    }
  }
  ...
  if (foundation) {
    for (const section of [
      ...
      for (const name of Object.keys(foundation.manifest[section] ?? {}))
        add(
          violations,
          "FOUNDATION_ISOLATION",
          `Foundation manifest declares ${section}.${name}`,
        );
    }
  }
  ...
  if (!existsSync(ownershipPath))
    add(
      violations,
      "CONTRACT_OWNERSHIP",
      "Missing server/architecture/contract-ownership.json",
    );
  else {
    const registry = readJson(ownershipPath);
    const contractPackages = packages.filter(
      (pkg) => layer(pkg, root) === "contracts",
    );
    ...
    for (const pkg of contractPackages)
      if (!owners.includes(pkg.name))
        add(
          violations,
          "CONTRACT_OWNERSHIP",
          `${pkg.name} has no canonical domain registration`,
        );
    for (const owner of owners)
      if (!contractPackages.some((pkg) => pkg.name === owner))
        add(
          violations,
          "CONTRACT_OWNERSHIP",
          `${owner} is registered but is not an active contract package`,
        );
    if (new Set(owners).size !== owners.length)
      add(
        violations,
        "CONTRACT_OWNERSHIP",
        "A contract package owns more than one canonical domain",
      );

    const symbols = new Map();
    const allow = new Set(registry.duplicateSymbolAllowlist ?? []);
    const declaration =
      /\bexport\s+(?:declare\s+)?(?:interface|type|class|function|const|enum)\s+([A-Za-z_$][\w$]*)/g;
    for (const pkg of contractPackages)
      for (const file of walk(join(pkg.root, "src"))) {
        for (const match of readFileSync(file, "utf8").matchAll(declaration)) {
          const name = match[1];
          if (allow.has(name)) continue;
          const prior = symbols.get(name);
          if (prior && prior !== pkg.name)
            add(
              violations,
              "DUPLICATE_CONTRACT",
              `${name} is exported by both ${prior} and ${pkg.name}`,
            );
  ...
  const ignoreText = existsSync(dockerIgnore)
    ? readFileSync(dockerIgnore, "utf8")
    : "";
  if (!/^\/?server-backup\/?$/m.test(ignoreText))
    add(
      violations,
      "BACKUP_ISOLATION",
      ".dockerignore must exclude server-backup",
    );
  const workspacePath = join(root, "pnpm-workspace.yaml");
  if (
    existsSync(workspacePath) &&
    /server-backup/.test(readFileSync(workspacePath, "utf8"))
  )
    add(
      violations,
      "BACKUP_ISOLATION",
      "pnpm-workspace.yaml must not reference server-backup",
    );
  for (const file of sourceFiles) {
    if (/server-backup/.test(readFileSync(file, "utf8")))
      add(
        violations,
        "BACKUP_ISOLATION",
        `${posix(relative(root, file))} references server-backup`,
      );
  }
...
function run() {
  ...
  console.log(
    `Server rebuild boundaries verified (${result.packages} packages, ${result.sourceFiles} source files).`,
  );
}
