...
const BASELINE =
  "governance/config/governance/server-legacy-route-baseline.json";
...
function stringConstants(source) {
  ...
  for (const match of source.matchAll(/\bconst\s+([A-Za-z_$][\w$]*)\s*=\s*/g)) {
    ...
    if (parsed && !parsed.value.includes("${"))
      values.set(match[1], parsed.value);
  }
  return values;
}
...
function expandTemplate(value, environment, constants) {
  const expanded = value.replace(
    /\$\{\s*([A-Za-z_$][\w$]*)\s*\}/g,
    (_match, name) => environment[name] ?? constants.get(name) ?? `\${${name}}`,
  );
...
function receiverPrefix(receiver, mounts, seen = new Set()) {
  ...
  return mount
    ? `${receiverPrefix(mount.parent, mounts, seen)}/${mount.prefix}`
    : "";
}
...
export function normalizeRoutePath(path, { defaultMount = "" } = {}) {
  ...
  if (
    defaultMount &&
    !normalized.startsWith(`${defaultMount}/`) &&
    normalized !== defaultMount &&
...
function directRoutes(source, environment, constants, mounts, offset = 0) {
  ...
  for (const match of source.matchAll(pattern)) {
    ...
    const paths = parsed
      ? [parsed.value]
      : end > start
        ? parseArray(source.slice(start, end + 1))
        : [];
    for (const value of paths ?? []) {
      ...
      routes.push({
        method: match[2],
        declaredPath:
          `${receiverPrefix(match[1], mounts)}/${declaredPath}`.replace(
            /\/+/g,
            "/",
          ),
        kind: Object.keys(environment).length ? "loop" : "direct",
...
function callBodies(source, callName) {
  ...
  for (const match of source.matchAll(pattern)) {
    ...
    const end = matching(source, start, "(", ")");
    if (end > start)
      bodies.push({ source: source.slice(start + 1, end), index: match.index });
  }
  return bodies;
}
...
function contractRoutes(
  ...
  environment = {},
  offset = 0,
  localFactory = hasLocalContractFactory(source),
) {
  const routes = [];
  for (const body of callBodies(source, "defineRouteContract")) {
    const method = body.source.match(
      /\bmethod\s*:\s*["'](get|post|put|patch|delete)["']/,
    )?.[1];
    const pathStart = body.source.match(/\bpath\s*:\s*/);
    const parsed = pathStart
      ? readQuoted(body.source, pathStart.index + pathStart[0].length)
      : null;
    const declaredPath = parsed
      ? expandTemplate(parsed.value, environment, constants)
      : null;
    if (method && declaredPath?.startsWith("/"))
      routes.push({
        method,
        declaredPath,
        kind: "contract",
        index: offset + body.index,
      });
  }
    ...
    for (const body of callBodies(source, "contract")) {
      ...
      const pathStart =
        comma < 0
          ...
          : comma +
            1 +
            (body.source.slice(comma + 1).match(/^\s*/)?.[0].length ?? 0);
...
function loopRoutes(source, constants, mounts, arrays) {
  ...
  for (const match of source.matchAll(pattern)) {
    ...
    const expression = source
      .slice(expressionStart, close)
      .replace(/\s+as\s+const\s*$/, "")
      .trim();
    const values = expression.startsWith("[")
      ? parseArray(expression)
      : arrays.get(expression);
    if (!Array.isArray(values)) continue;
    const statementStart =
      close + 1 + (source.slice(close + 1).match(/^\s*/)?.[0].length ?? 0);
    const braced = source[statementStart] === "{";
    const statementEnd = braced
      ? matching(source, statementStart, "{", "}")
      : source.indexOf(";", statementStart);
    ...
    for (const item of values) {
      ...
      routes.push(
        ...directRoutes(body, environment, constants, mounts, bodyStart),
      );
      routes.push(
        ...contractRoutes(
          body,
...
function hasLocalContractFactory(source) {
  return /function\s+contract\s*\([^)]*\)[^{]*\{[\s\S]{0,4000}?\bdefineRouteContract\s*\(/.test(
    source,
  );
}
...
export function extractRoutesFromSource(source, options = {}) {
  ...
  return candidates.flatMap((route) => {
    const path = normalizeRoutePath(route.declaredPath, {
      defaultMount: options.defaultMount ?? "",
    });
    ...
    return [
      {
        ...
        kind: route.kind,
        line: lineAt(source, route.index),
      },
    ];
  });
}
...
function collectTree(root, tree, workspaceRoot) {
  ...
  for (const path of walk(root)) {
    const segments = posix(relative(root, path)).split("/");
    // Verification/provisioning programs can create Express fixtures, but those
    // declarations are not part of the deployable server source surface.
    if (
      tree === "current" &&
      (segments[0] === "db" ||
    ...
    // AST extraction includes named contract factories and descriptor routes.
    // Supplement only contracts: raw router paths still require mount resolution.
    // Historical baseline extraction remains stable.
    if (tree === "current") {
      for (const route of extractStaticUrlRoutes(source).routes.filter(
        (route) => route.kind === "contract",
      )) {
        const normalized = normalizeRoutePath(route.declaredPath);
        if (
          !routes.some(
            (existing) =>
              existing.method === route.method && existing.path === normalized,
          )
        )
          routes.push({ ...route, path: normalized });
      }
    }
    for (const route of routes)
      occurrences.push({
        ...route,
        source: posix(relative(workspaceRoot, path)),
      });
  }
  return occurrences;
}
...
function validateBaseline(baseline) {
  ...
  if (
    ...
    !["historical-manifest", "source-tree"].includes(provenance?.kind) ||
    ...
    throw new Error(
      "Invalid legacy route baseline: nonempty routes and verifiable provenance are required",
    );
  }
  const seen = new Set();
  for (const route of baseline.routes) {
    if (
      ...
      !HTTP_METHODS.has(route.method.toLowerCase()) ||
      route.method !== route.method.toUpperCase() ||
      typeof route.path !== "string" ||
      !route.path.startsWith("/") ||
      normalizeRoutePath(route.path) !== route.path ||
      typeof route.declaredPath !== "string" ||
      !route.declaredPath.startsWith("/") ||
      typeof route.source !== "string" ||
      !route.source.startsWith("server-backup/") ||
      route.source.includes("\\") ||
      route.source.split("/").includes("..") ||
      !Number.isSafeInteger(route.line) ||
      route.line < 1 ||
      !["direct", "contract", "loop"].includes(route.kind)
    ) {
    ...
    if (seen.has(key))
      throw new Error("Duplicate legacy route baseline occurrence");
    seen.add(key);
  }
  return baseline;
}
...
export function refreshLegacyBaseline({ root = ROOT } = {}) {
  ...
  if (!existsSync(legacyRoot))
    throw new Error(
      "Restore the authentic server-backup snapshot before refreshing the legacy baseline",
    );
  ...
  console.log(
    `Refreshed legacy baseline (${baseline.routes.length} occurrences). Review and commit the baseline and regenerated manifest.`,
  );
  return baseline;
}
...
export function buildRouteManifest(root = ROOT, { currentOnly = false } = {}) {
  ...
  const baseline =
    !currentOnly && existsSync(baselinePath)
      ? validateBaseline(JSON.parse(sourceText(baselinePath)))
      : null;
  const legacy = baseline?.routes ?? [];
  if (!existsSync(join(root, "server")))
    throw new Error(
      "Current server source directory is required to generate the route manifest",
    );
  const current = collectTree(join(root, "server"), "current", root);
  const grouped = new Map();
  for (const [tree, routes] of [
    ["legacy", legacy],
    ["current", current],
  ]) {
    for (const route of routes) {
  ...
  for (const route of routes) {
    ...
    route.status = !baseline
      ? "current-uncompared"
      : route.legacy.length && route.current.length
        ? "matched"
        : route.legacy.length
          ? "legacy-only"
          : "current-only";
  ...
  return {
    schemaVersion: 2,
    normalization: {
      legacyDefaultMount: "/api",
      parameterNames: ":param",
      excluded: ["tests", "fixtures", "dist", "node_modules"],
    },
    sources: { legacy: baseline ? BASELINE : null, current: "server" },
    legacyParity: baseline
      ? { status: "available", provenance: baseline.provenance }
      : {
          status: "unavailable",
          reason: currentOnly
            ? "Explicit current-routes-only mode"
            : "Legacy route baseline is missing",
        },
    summary: {
      identities: routes.length,
      matched: baseline
        ? routes.filter((route) => route.status === "matched").length
        : null,
      legacyOnly: baseline
        ? routes.filter((route) => route.status === "legacy-only").length
        : null,
      currentOnly: baseline
        ? routes.filter((route) => route.status === "current-only").length
        : null,
...
function markdown(manifest) {
  const lines = [
    ...
    ...manifest.routes.map(
      (route) =>
        `| ${route.status} | ${route.method} | \`${route.path}\` | ${route.legacy.length} | ${route.current.length} |`,
    ),
    "",
  ];
  return lines.join("\n");
...
export function writeOrCheck({
  ...
  if (!currentOnly && manifest.legacyParity.status !== "available")
    throw new Error(
      "Legacy route baseline is missing. Restore the committed baseline, or use --current-only to explicitly report legacy parity as unavailable.",
    );
  const dispositions = reconcileRouteDispositions(
    manifest,
    readRouteDispositionDecisions(root),
    root,
  );
  const outputs = [
    [
      join(root, "docs/architecture/server-route-dispositions.json"),
      renderJson(dispositions),
    ],
    [
      join(root, "docs/architecture/server-route-manifest.json"),
      renderJson(manifest),
    ],
    [
      join(root, "docs/architecture/server-route-manifest.md"),
      markdown(manifest),
    ],
  ];
  if (write) {
    ...
    console.log(
      `Wrote normalized route manifest (${manifest.routes.length} identities; legacy parity ${manifest.legacyParity.status}).`,
    );
    return manifest;
  }
  const stale = outputs
    .filter(
      ([path, content]) => !existsSync(path) || sourceText(path) !== content,
    )
    .map(([path]) => posix(relative(root, path)));
  if (stale.length)
    throw new Error(
      `Route manifest is stale. Run pnpm routes:server-manifest.\n${stale.join("\n")}`,
    );
  console.log(
    `Normalized route manifest verified (${manifest.routes.length} identities; legacy parity ${manifest.legacyParity.status}).`,
  );
  return manifest;
}
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  try {
    ...
    if (
      args.some(
        (arg) =>
          !["--write", "--current-only", "--refresh-legacy-baseline"].includes(
            arg,
          ),
      ) ||
      (args.includes("--refresh-legacy-baseline") && args.length !== 1)
    )
      throw new Error(
        "Use --write, --current-only, or --refresh-legacy-baseline (alone)",
      );
