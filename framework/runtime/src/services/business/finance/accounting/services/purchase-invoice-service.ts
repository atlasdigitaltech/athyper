...
import type { ServiceResult, OperationContext } from "../../../engines/shared/engine-base.js";
import type { CreateJournalEntryInput, CreateJournalLineInput } from "../../../engines/posting-engine/domain/types.js";
...
import type { PurchaseInvoiceRepo, PurchaseInvoiceLineRepo } from "../persistence/purchase-invoice-repo.js";
...
export interface BudgetOps {
    reserve(ctx: OperationContext, fpId: string, amount: string, currencyCode: string, txnId: string): Promise<ServiceResult<unknown>>;
    commit(ctx: OperationContext, fpId: string, amount: string, currencyCode: string, txnId: string): Promise<ServiceResult<unknown>>;
    consume(ctx: OperationContext, fpId: string, amount: string, currencyCode: string, txnId: string): Promise<ServiceResult<unknown>>;
    release(ctx: OperationContext, fpId: string, amount: string, currencyCode: string, txnId: string): Promise<ServiceResult<unknown>>;
}

/** Tax Engine — calculation + posting splits */
export interface TaxOps {
    calculateBatch(ctx: OperationContext, inputs: unknown[]): Promise<ServiceResult<{ calculations: any[]; totalTaxAmount: string }>>;
}

/** Asset Engine — WIP asset creation for CAPEX lines */
export interface AssetOps {
    createAsset(ctx: OperationContext, input: {
...
export interface InventoryOps {
    receiveStock(ctx: OperationContext, input: {
        itemId: string;
        warehouseId: string;
        quantity: string;
        unitCost: string;
...
export interface CommissionOps {
    calculateCommission(ctx: OperationContext, input: {
        docId: string;
        supplierId: string;
        baseAmount: string;
        currencyCode: string;
...
export interface FederationOps {
    createICTransaction(ctx: OperationContext, input: {
        sourceEntity: string;
        destEntity: string;
        amount: string;
        currencyCode: string;
...
export interface ApprovalOps {
    createInstance(ctx: OperationContext, input: {
        entityType: string;
        entityId: string;
        triggerEvent: string;
    ...
    cancelInstance(ctx: OperationContext, instanceId: string): Promise<ServiceResult<void>>;
}
...
export interface PurchaseInvoiceService {
    create(ctx: OperationContext, input: CreatePurchaseInvoiceInput): Promise<ServiceResult<PurchaseInvoice>>;
    update(ctx: OperationContext, id: string, input: UpdatePurchaseInvoiceInput): Promise<ServiceResult<PurchaseInvoice>>;
    ...
    setLines(ctx: OperationContext, invoiceId: string, lines: CreateInvoiceLineInput[], version?: number): Promise<ServiceResult<PurchaseInvoiceLine[]>>;

    /** Resolve OU+Intent defaults for a new line (called by UI) */
    resolveLineDefaults(ctx: OperationContext, invoiceId: string): Promise<ServiceResult<Record<string, unknown>>>;

    /** DRAFT → SUBMITTED: Decision Grid + Budget RESERVE + Approval */
    submit(ctx: OperationContext, invoiceId: string): Promise<ServiceResult<PurchaseInvoice>>;

    /** Workflow callback */
    onApprovalComplete(ctx: OperationContext, invoiceId: string, outcome: "approved" | "rejected"): Promise<ServiceResult<PurchaseInvoice>>;

    /** APPROVED → POSTED: Tax final + JE + GL + Budget CONSUME + Asset + Inventory + Commission + Federation */
    post(ctx: OperationContext, invoiceId: string): Promise<ServiceResult<PurchaseInvoice>>;

    /** Cancel (reversal cascade) */
    cancel(ctx: OperationContext, invoiceId: string): Promise<ServiceResult<PurchaseInvoice>>;
}
...
export class DefaultPurchaseInvoiceService implements PurchaseInvoiceService {
    ...
    async create(
        ...
        const existing = await this.documentControl.checkIdempotency(
            this.invoiceRepo, ctx.tenantId, input.entityCode, input.idempotencyKey,
        );
    ...
    async update(
        ...
        if (invoice.status !== "DRAFT") {
            return fail("INVALID_STATUS", `Cannot update invoice in ${invoice.status} status`);
        }
        this.documentControl.assertVersion(input.version, invoice.version);

        const updated = await this.invoiceRepo.updateStatus(ctx.tenantId, id, "DRAFT", {
            ...input,
    ...
    async setLines(
        ...
        if (invoice.status !== "DRAFT") {
            return fail("INVALID_STATUS", `Cannot modify lines in ${invoice.status} status`);
        }
        ...
        const savedLines = await this.lineRepo.bulkUpsert(ctx.tenantId, invoiceId, lines);
    ...
    async resolveLineDefaults(
        ...
        const result = await this.ouIntentResolver.resolveDefaults(
            ctx, invoice.ouId, invoice.intentId, invoice.spendCategoryId,
        );
    ...
    async submit(
        ...
        if (!this.canTransition(invoice.status, "SUBMITTED")) {
            return fail("INVALID_TRANSITION", `Cannot submit invoice in ${invoice.status} status`);
        }
        ...
        if (evalResult.approvalRoute === "BLOCKED") {
            return fail("BLOCKED_BY_POLICY",
                ...
                { policyResults: evalResult.policyResults, exceptions: evalResult.exceptions },
            );
        }
        ...
        for (const { fpId, amount } of fpIds) {
            const reserveResult = await this.budgetOps.reserve(
                ctx, fpId, amount, invoice.currencyCode, invoice.txnId,
            );
            if (!reserveResult.ok) {
                return fail("BUDGET_RESERVE_FAILED",
        ...
        if (evalResult.approvalRoute === "ZERO_APPROVAL") {
            ...
            const approved = await this.invoiceRepo.updateStatus(ctx.tenantId, invoiceId, "APPROVED", {
            ...
            for (const { fpId, amount } of fpIds) {
                await this.budgetOps.commit(ctx, fpId, amount, invoice.currencyCode, invoice.txnId);
            }
        ...
        const approvalInstanceId = approvalResult.ok ? approvalResult.value.id : null;

        const submitted = await this.invoiceRepo.updateStatus(ctx.tenantId, invoiceId, "SUBMITTED", {
            decisionScore: evalResult.compositeScore,
            approvalRoute: evalResult.approvalRoute,
            approvalInstanceId,
            submittedAt: new Date(),
    ...
    async onApprovalComplete(
        ...
        const rejected = await this.invoiceRepo.updateStatus(ctx.tenantId, invoiceId, "DRAFT", {
        ...
        const fpIds = this.collectFundingProfileIds(invoice, lines);
        for (const { fpId, amount } of fpIds) {
            await this.budgetOps.release(ctx, fpId, amount, invoice.currencyCode, invoice.txnId);
        }
    ...
    async post(
        ...
        if (!this.canTransition(invoice.status, "POSTED")) {
            return fail("INVALID_TRANSITION", `Cannot post invoice in ${invoice.status} status`);
        }
        ...
        for (const line of lines) {
            ...
            const addToBaseAmount = taxSplits
                .filter((s) => s.mode === "ADD_TO_BASE_LINE" && s.sourceLineId === line.id)
                .map((s) => s.amount);
            const lineTotal = addToBaseAmount.length > 0
        ...
        if (!postResult.ok) {
            return fail(postResult.error.code, `Posting failed: ${postResult.error.message}`);
        }
        ...
        const posted = await this.invoiceRepo.updateStatus(ctx.tenantId, invoiceId, "POSTED", {
            jeId: je.id,
            postedAt: new Date(),
            postedBy: ctx.actorId,
            postingDate: invoice.postingDate ?? invoice.invoiceDate,
    ...
    async cancel(
        ...
        if (invoice.status === "POSTED" || invoice.status === "PARTIALLY_PAID") {
            ...
            if (invoice.jeId) {
                const reversalNumber = await this.documentControl.generateNumber(
                    ctx.tenantId, entityCode, "JER",
                );
                const reverseResult = await this.postingService.reverse(ctx, invoice.jeId, reversalNumber);
                if (!reverseResult.ok) {
                    return fail("REVERSAL_FAILED", `JE reversal failed: ${reverseResult.error.message}`);
                }
            }
        ...
        const cancelled = await this.invoiceRepo.updateStatus(ctx.tenantId, invoiceId, "CANCELLED", {
