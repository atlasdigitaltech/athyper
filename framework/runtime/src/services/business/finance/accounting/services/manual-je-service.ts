...
} from "../../../engines/posting-engine/domain/double-entry-validator.js";
...
export class DefaultManualJEService implements ManualJEService {
  ...
  async create(
    ...
    const accountError = await this.validateAccounts(
      ctx.tenantId,
      normalised.lines,
    );
    if (accountError) {
  ...
  async submit(
    ...
    const evalResult = await this.decisionGrid.evaluate(ctx, {
      ...
      metadata: {
        ...
        lineCount: (await this.jeRepo.getLinesByJeId(ctx.tenantId, je.id))
          .length,
      },
    });
    ...
    if (
      evaluation.approvalRoute === "ZERO_APPROVAL" &&
      options?.directPost === true
    ) {
