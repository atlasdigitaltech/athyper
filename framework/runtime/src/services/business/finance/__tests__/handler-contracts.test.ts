...
function createMockCtx(overrides = {}): any {
  return {
    tenant: { tenantId: "t-1", realmKey: "t-1", orgKey: "ACME" },
    auth: { userId: "user-1" },
...
describe("PostPaymentHandler — contract", () => {
  it("returns 400 with CROSS_SUPPLIER error code when payment references cross-supplier invoice", async () => {
    const mockPaymentService = {
      post: vi.fn(async () => ({
        ...
        error: {
          code: "CROSS_SUPPLIER",
          message:
            "Invoice INV-2026-00001 belongs to supplier supplier-2, but payment is for supplier supplier-1",
    ...
    const req = createMockReq({ id: "pay-1" }, {}, { entityCode: "ACME" });
    const res = createMockRes();
    ...
    const mockPaymentService = {
      post: vi.fn(async () => ({
        ...
        error: {
          code: "OVERPAYMENT",
          message:
            "Overpayment on invoice INV-2026-00001: allocated 1000 but only 200 remaining",
