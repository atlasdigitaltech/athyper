...
import type { PaymentEntry, PaymentAllocation } from "../payments/domain/types.js";
...
function createPaymentMocks(overrides?: {
    ...
    const defaultInvoices: PurchaseInvoice[] = (overrides?.invoices ?? [
        {
            id: "inv-1",
            tenantId: "t-1",
            entityCode: "ACME",
    ...
    const paymentEntryRepo = {
        ...
        updateStatus: vi.fn(async (_t: string, _id: string, status: string, fields?: any) => ({
            ...basePayment,
            status,
            ...fields,
        })),
        list: vi.fn(async () => ({ items: [], total: 0, limit: 50, offset: 0, hasMore: false })),
    };
    ...
    const purchaseInvoiceRepo = {
        ...
        updatePaidAmount: vi.fn(async (_t: string, id: string, paidAmount: string, status: string) => {
            const inv = invoiceMap.get(id) ?? defaultInvoices[0];
            return { ...inv, paidAmount, status };
        }),
        lockForUpdate: vi.fn(async (ids: string[]) => {
            return ids.map((id) => invoiceMap.get(id)).filter(Boolean) as PurchaseInvoice[];
        }),
        list: vi.fn(async () => ({ items: [], total: 0, limit: 50, offset: 0, hasMore: false })),
    };
...
describe("PaymentEntryService — posting lifecycle", () => {
    ...
    it("post() generates JE with Dr AP and Cr Bank lines", async () => {
        ...
        const crBankLines = lines.filter(
            (l) => l.creditAmount !== "0" && l.debitAmount === "0"
    ...
    it("post() builds balanced JE: Dr AP 1000 = Cr Bank 850 + Cr WHT 100 + Cr Disc 50", async () => {
        ...
        expect(
            Number(crBank!.creditAmount) + Number(crWHT!.creditAmount) + Number(crDisc!.creditAmount),
