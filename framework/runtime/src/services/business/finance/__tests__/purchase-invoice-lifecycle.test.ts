...
function makeDecisionResult(
  overrides: Partial<DecisionEvaluationResult> = {},
): DecisionEvaluationResult {
  return {
    compositeScore: 0.35,
...
function createInvoiceMocks() {
  ...
  const invoiceRepo = {
    ...
    updateStatus: vi.fn(
      async (_t: string, _id: string, status: InvoiceStatus, fields?: any) => ({
        ...baseInvoice,
        status,
        ...fields,
      }),
  ...
  const federationOps = {
    createICTransaction: vi.fn(async () => ({
      ok: true,
      value: { id: "ic-1" },
    })),
...
describe("PurchaseInvoiceService — lifecycle", () => {
  ...
  it("T2: submit evaluates Decision Grid and creates approval instance", async () => {
    ...
    expect(m.decisionGrid.evaluate).toHaveBeenCalledWith(
      m.ctx,
    ...
    expect(m.approvalOps.createInstance).toHaveBeenCalledWith(
      m.ctx,
      expect.objectContaining({
  ...
  it("T5: approval complete (approved) commits budget reservation", async () => {
    ...
    const result = await m.service.onApprovalComplete(
      m.ctx,
      "inv-1",
      "approved",
    );
  ...
  it("T7: posting with recoverable tax creates a tax input credit debit line", async () => {
    ...
    const jeInput = m.postingService.createAndPost.mock.calls[0][1];
    const taxLine = jeInput.lines.find((l: any) =>
      l.description?.includes("Tax input credit"),
    );
  ...
  it("T24: cancelling a SUBMITTED invoice cancels approval and releases budget", async () => {
    ...
    expect(m.approvalOps.cancelInstance).toHaveBeenCalledWith(
      m.ctx,
      "approval-1",
    );
  ...
  it("T28: posting an already-posted invoice returns ALREADY_POSTED error", async () => {
    ...
    m.postingService.createAndPost.mockResolvedValueOnce({
      ...
      error: {
        code: "ALREADY_POSTED",
        message:
          "Document inv-1 (PURCHASE_INVOICE) already has a non-reversed JE",
