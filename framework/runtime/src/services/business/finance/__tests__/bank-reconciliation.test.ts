...
} from "../banking/persistence/bank-statement-repo.js";
...
function makeCandidate(
  overrides: Partial<PaymentMatchCandidate> = {},
...
describe("BankReconciliationService — lifecycle", () => {
  ...
  it("startReconciliation creates session and marks statement IN_PROGRESS", async () => {
    ...
    expect(mocks.statementRepo.updateStatus).toHaveBeenCalledWith(
      "t1",
      "stmt-1",
      "IN_PROGRESS",
    );
  ...
  it("runAutoMatch dispatches to autoMatch and applies high-confidence matches", async () => {
    ...
    expect(mocks.lineRepo.updateMatch).toHaveBeenCalledWith(
      "t1",
      "L-A",
      expect.objectContaining({
        matchStatus: "AUTO_MATCHED",
  ...
  it("completeReconciliation calls reconcilePayment for each matched line", async () => {
    ...
    mocks.lineRepo.updateMatch.mockImplementation(
      async (_t: string, _id: string, updates: Record<string, unknown>) => {
    ...
    expect(mocks.paymentQueryRepo.reconcilePayment).toHaveBeenCalledWith(
      "t1",
      "pay-R1",
    );
    expect(mocks.paymentQueryRepo.reconcilePayment).toHaveBeenCalledWith(
    ...
    expect(mocks.reconRepo.complete).toHaveBeenCalledWith(
      "t1",
      "sess-1",
      "user-1",
    );
