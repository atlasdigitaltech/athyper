...
export class CreatePaymentHandler implements RouteHandler {
    async handle(req: Request, res: Response, ctx: HttpHandlerContext): Promise<void> {
        try {
            const service = await ctx.container.resolve<PaymentEntryService>(PAY_TOKEN);
...
export class GetPaymentHandler implements RouteHandler {
    async handle(req: Request, res: Response, ctx: HttpHandlerContext): Promise<void> {
        try {
            ...
            if (!payment) {
                res.status(404).json({ error: { code: "NOT_FOUND", message: `Payment ${id} not found` } });
                return;
            }
    ...
    async handle(req: Request, res: Response, ctx: HttpHandlerContext): Promise<void> {
        try {
            ...
            if (!Array.isArray(allocations)) {
                res.status(400).json({ error: { code: "INVALID_ALLOCATIONS", message: "Request body must contain an allocations array" } });
                return;
            }

            const version = typeof req.body.version === "number" ? req.body.version : undefined;
            const result = await service.addAllocations(opCtx, id, allocations, version);
