...
import { ok, fail, validateTransition } from "../../../engines/shared/engine-base.js";
import { sumAmounts, subtractAmounts, compareAmounts } from "../../../engines/shared/money.js";
import type { PostingService, TransactionContext } from "../../../engines/posting-engine/services/posting-service.js";
...
export class DefaultPaymentEntryService implements PaymentEntryService {
    ...
    async addAllocations(
        ...
        const payment = await this.paymentEntryRepo.getById(ctx.tenantId, paymentId);
        if (!payment) {
    ...
    async submit(
        ...
        if (allocations.length === 0) {
            return fail("NO_ALLOCATIONS", "Payment must have at least one allocation");
        }
    ...
    async post(
        ...
        try {
            ...
            const apControlAccountId = payment.clearingAccountId ?? payment.bankAccountId!;
            ...
            const whtPayableAccountId = payment.clearingAccountId ?? payment.bankAccountId!;
            ...
            const discountIncomeAccountId = payment.clearingAccountId ?? payment.bankAccountId!;
            ...
            if (!jeResult.ok) {
                throw Object.assign(
                    ...
                    { statusCode: 500, code: jeResult.error.code, details: jeResult.error.details },
                );
            }
