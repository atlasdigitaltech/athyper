...
export class DefaultBankStatementRepo implements BankStatementRepo {
  ...
  async getById(tenantId: string, id: string): Promise<BankStatement | null> {
    const db = await this.container.resolve<any>("db");
    return db.queryOne(
      `SELECT * FROM fin.bank_statement WHERE tenant_id=$1 AND id=$2`,
      [tenantId, id],
    );
  }
  ...
  async updateStatus(
    ...
    const db = await this.container.resolve<any>("db");
    return db.queryOne(
      `UPDATE fin.bank_statement SET status=$3, updated_at=NOW() WHERE tenant_id=$1 AND id=$2 RETURNING *`,
      [tenantId, id, status],
    );
  }
  ...
  async list(
    ...
    const countResult = await db.queryOne(
      `SELECT COUNT(*)::int AS total FROM fin.bank_statement WHERE ${where}`,
      params,
    );
    const total = countResult?.total ?? 0;
    const rows = await db.query(
      `SELECT * FROM fin.bank_statement WHERE ${where} ORDER BY created_at DESC LIMIT $${idx++} OFFSET $${idx++}`,
...
export class DefaultBankStatementLineRepo implements BankStatementLineRepo {
  ...
  async getByStatementId(
    tenantId: string,
    statementId: string,
  ): Promise<BankStatementLine[]> {
    const db = await this.container.resolve<any>("db");
    return db.query(
      `SELECT * FROM fin.bank_statement_line WHERE tenant_id=$1 AND statement_id=$2 ORDER BY line_no`,
  ...
  async getById(
    ...
    const db = await this.container.resolve<any>("db");
    return db.queryOne(
      `SELECT * FROM fin.bank_statement_line WHERE tenant_id=$1 AND id=$2`,
      [tenantId, id],
    );
  }
  ...
  async getUnmatched(
    ...
    const db = await this.container.resolve<any>("db");
    return db.query(
      `SELECT * FROM fin.bank_statement_line WHERE tenant_id=$1 AND statement_id=$2 AND match_status='UNMATCHED' ORDER BY line_no`,
      [tenantId, statementId],
    );
  }
}
