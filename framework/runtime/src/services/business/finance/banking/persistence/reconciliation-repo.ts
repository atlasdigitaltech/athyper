...
import type { ReconciliationSession } from "../domain/types.js";
...
export class DefaultReconciliationSessionRepo implements ReconciliationSessionRepo {
  ...
  async getById(
    ...
    const db = await this.container.resolve<any>("db");
    return db.queryOne(
      `SELECT * FROM fin.reconciliation_session WHERE tenant_id=$1 AND id=$2`,
      [tenantId, id],
    );
  }
  ...
  async getByStatementId(
    ...
    const db = await this.container.resolve<any>("db");
    return db.queryOne(
      `SELECT * FROM fin.reconciliation_session WHERE tenant_id=$1 AND statement_id=$2 ORDER BY created_at DESC LIMIT 1`,
