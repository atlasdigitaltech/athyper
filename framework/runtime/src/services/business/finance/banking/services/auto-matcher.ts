...
export function autoMatch(
    ...
    for (const line of unmatchedLines) {
        ...
        for (const candidate of candidates) {
            ...
            const candidateRef = (candidate.reference ?? candidate.paymentNumber).trim().toUpperCase();
            if (normalizedRef === candidateRef || normalizedRef.includes(candidateRef) || candidateRef.includes(normalizedRef)) {
                results.push({
                    lineId: line.id,
    ...
    for (const line of unmatchedLines) {
        ...
        for (const candidate of candidates) {
            ...
            const diffMs = Math.abs(line.transactionDate.getTime() - candidate.paymentDate.getTime());
            const diffDays = diffMs / (1000 * 60 * 60 * 24);
