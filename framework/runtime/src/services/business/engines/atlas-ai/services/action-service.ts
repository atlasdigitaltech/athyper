...
export interface ActionService {
  ...
  ): Promise<ServiceResult<AIAction>>;
...
export class DefaultActionService implements ActionService {
  ...
  async propose(
    ...
    if (!input.reasoningChain || input.reasoningChain.trim().length === 0) {
      return fail(
        "MISSING_REASONING",
        "Governance: reasoning_chain is mandatory for all AI actions",
      );
    }

    const reversalMinutes =
      input.reversalWindowMinutes ?? DEFAULT_REVERSAL_WINDOW_MINUTES;
    const reversalWindowExpiresAt = new Date(
      Date.now() + reversalMinutes * 60 * 1000,
    );
  ...
  async execute(
    ...
    if (!action)
      return fail("ACTION_NOT_FOUND", `Action ${actionId} not found`);

    if (action.status !== "PROPOSED") {
      return fail(
        "INVALID_STATUS",
        `Action is ${action.status}, must be PROPOSED`,
      );
    }
  ...
  async reverse(
    ...
    if (action.status !== "EXECUTED") {
      return fail(
        "INVALID_STATUS",
        `Action is ${action.status}, must be EXECUTED to reverse`,
      );
    }

    // Check reversal window
    if (
      action.reversalWindowExpiresAt &&
      new Date() > action.reversalWindowExpiresAt
    ) {
      return fail(
        "REVERSAL_WINDOW_EXPIRED",
        "Reversal window has expired for this action",
      );
    }
  ...
  async reject(
    ...
    if (action.status !== "PROPOSED") {
      return fail(
        "INVALID_STATUS",
        `Action is ${action.status}, must be PROPOSED to reject`,
      );
    }
