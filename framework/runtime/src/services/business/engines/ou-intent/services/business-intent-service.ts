...
import type { BusinessIntent, CreateBusinessIntentInput, IntentDomain } from "../domain/types.js";
...
export interface BusinessIntentService {
    create(ctx: OperationContext, input: CreateBusinessIntentInput): Promise<ServiceResult<BusinessIntent>>;
    ...
    update(ctx: OperationContext, id: string, input: Partial<CreateBusinessIntentInput>): Promise<ServiceResult<BusinessIntent>>;
    list(tenantId: string, filters: { domain?: IntentDomain; isActive?: boolean }, pagination: PaginationParams): Promise<PaginatedResult<BusinessIntent>>;
...
export class DefaultBusinessIntentService implements BusinessIntentService {
    ...
    async create(ctx: OperationContext, input: CreateBusinessIntentInput): Promise<ServiceResult<BusinessIntent>> {
        // Check code uniqueness
        const existing = await this.repo.getByCode(ctx.tenantId, input.code);
        if (existing) {
            return fail("INTENT_CODE_EXISTS", `Intent code "${input.code}" already exists`);
        }
        ...
        if (input.parentId) {
            ...
            if (!parent) {
                return fail("INTENT_PARENT_NOT_FOUND", `Parent intent ${input.parentId} not found`);
            }
        }
    ...
    async getByCode(tenantId: string, code: string): Promise<BusinessIntent | null> {
        return this.repo.getByCode(tenantId, code);
    }

    async update(ctx: OperationContext, id: string, input: Partial<CreateBusinessIntentInput>): Promise<ServiceResult<BusinessIntent>> {
    ...
    async deactivate(ctx: OperationContext, id: string): Promise<ServiceResult<void>> {
        const existing = await this.repo.getById(ctx.tenantId, id);
        if (!existing) {
            return fail("INTENT_NOT_FOUND", `Intent ${id} not found`);
        }
        ...
        if (activeChildren.length > 0) {
            return fail("INTENT_HAS_ACTIVE_CHILDREN", `Cannot deactivate intent with ${activeChildren.length} active children`);
        }
