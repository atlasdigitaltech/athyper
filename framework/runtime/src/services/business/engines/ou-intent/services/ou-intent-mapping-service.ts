...
export interface OUIntentMappingService {
  assign(
    ...
    input: CreateOUIntentMappingInput,
  ): Promise<ServiceResult<OUIntentMapping>>;
  unassign(
    ctx: OperationContext,
    ouId: string,
    intentId: string,
...
export class DefaultOUIntentMappingService implements OUIntentMappingService {
  ...
  async assign(
    ...
    if (ou.status !== "ACTIVE" && ou.status !== "DRAFT") {
      return fail(
        "OU_NOT_ASSIGNABLE",
        `OU is ${ou.status}, must be DRAFT or ACTIVE`,
      );
    }
  ...
  async setDefault(
    ...
    if (!exists) {
      return fail(
        "MAPPING_NOT_FOUND",
        `Intent ${intentId} is not assigned to OU ${ouId}`,
      );
    }
