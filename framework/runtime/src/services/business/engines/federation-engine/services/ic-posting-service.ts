...
} from "../../posting-engine/domain/types.js";
...
export interface ICPostingService {
  ...
  createMirrorJE(
    ...
    ServiceResult<{ destJeId: string; icTransaction: IntercompanyTransaction }>
  >;
}
...
export class DefaultICPostingService implements ICPostingService {
  ...
  > {
    ...
    if (!icTxn) {
      return fail(
        "IC_TXN_NOT_FOUND",
        `IC transaction ${icTransactionId} not found`,
      );
    }
    ...
    if (icTxn.destJeId) {
      return fail(
        "ALREADY_MIRRORED",
        `IC transaction ${icTransactionId} already has a mirror JE`,
      );
    }

    // ── Step 2: Validate destination entity ──
    const destEntity = await this.entityRepo.getByCode(
      ctx.tenantId,
      icTxn.destEntityCode,
    );
    if (!destEntity) {
      return fail(
        "DEST_ENTITY_NOT_FOUND",
        `Destination entity ${icTxn.destEntityCode} not found`,
      );
    }
    ...
    const icReceivableAccountId =
      await this.accountResolver.getICReceivableAccountId(icTxn.destEntityCode);
    const icRevenueAccountId = await this.accountResolver.getICRevenueAccountId(
      icTxn.destEntityCode,
    );
    ...
    if (!postResult.ok) {
      return fail(
        ...
        `Mirror JE posting failed: ${postResult.error.message}`,
      );
    }
    ...
    const updated = await this.icRepo.updateStatus(
      ctx.tenantId,
      icTransactionId,
      "POSTED",
      {
