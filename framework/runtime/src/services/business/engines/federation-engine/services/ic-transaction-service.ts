...
import type { IntercompanyTransaction, CreateICTransactionInput } from "../domain/types.js";
...
export interface ICTransactionService {
    ...
    create(ctx: OperationContext, input: CreateICTransactionInput): Promise<ServiceResult<IntercompanyTransaction>>;
    ...
    markPosted(ctx: OperationContext, id: string, sourceJeId: string, destJeId: string): Promise<ServiceResult<IntercompanyTransaction>>;
}
...
export class DefaultICTransactionService implements ICTransactionService {
    ...
    async create(ctx: OperationContext, input: CreateICTransactionInput): Promise<ServiceResult<IntercompanyTransaction>> {
        // Validate both entities exist
        const source = await this.entityRepo.getByCode(ctx.tenantId, input.sourceEntityCode);
        if (!source) return fail("SOURCE_NOT_FOUND", `Source entity ${input.sourceEntityCode} not found`);

        const dest = await this.entityRepo.getByCode(ctx.tenantId, input.destEntityCode);
        if (!dest) return fail("DEST_NOT_FOUND", `Dest entity ${input.destEntityCode} not found`);

        if (input.sourceEntityCode === input.destEntityCode) {
            return fail("SAME_ENTITY", "Source and destination entities cannot be the same");
        }

        const txn = await this.icRepo.create(input);

        // Auto-mirror (set status to MIRRORED)
        const mirrored = await this.icRepo.updateStatus(ctx.tenantId, txn.id, "MIRRORED");
        return ok(mirrored);
    }

    async getById(tenantId: string, id: string): Promise<IntercompanyTransaction | null> {
        return this.icRepo.getById(tenantId, id);
    }

    async markPosted(ctx: OperationContext, id: string, sourceJeId: string, destJeId: string): Promise<ServiceResult<IntercompanyTransaction>> {
