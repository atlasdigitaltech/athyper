...
import type { IntercompanyTransaction, LegalEntity, ICTxnStatus } from "../domain/types.js";
...
function makeICTxn(overrides: Partial<IntercompanyTransaction> = {}): IntercompanyTransaction {
    return {
        id: "ic-1",
        tenantId: "t-1",
...
function createMocks() {
    const icRepo = {
        ...
        updateStatus: vi.fn(async (_t: string, id: string, status: ICTxnStatus, updates?: any) => ({
            ...makeICTxn(),
            id,
            status,
            ...updates,
    ...
    return {
        service, icRepo, entityRepo, postingService,
        documentControl, accountResolver, fxRateProvider, ctx,
    };
}
...
describe("ICPostingService — mirror JE creation", () => {
    ...
    it("creates mirror JE with Dr IC Receivable and Cr IC Revenue", async () => {
        const result = await m.service.createMirrorJE(m.ctx, "ic-1", "je-source-1", now);
    ...
    it("skips FX when source and dest currencies match", async () => {
        ...
        m.entityRepo.getByCode.mockResolvedValue(makeEntity({
    ...
    it("fails if IC transaction not found", async () => {
        ...
        const result = await m.service.createMirrorJE(m.ctx, "ic-999", "je-source-1", now);
