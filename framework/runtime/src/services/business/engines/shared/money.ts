...
export function money(amount: string, currencyCode: string, precision = 4): Money {
    if (!/^-?\d+(\.\d+)?$/.test(amount)) {
        throw new Error(`Invalid money amount: "${amount}" — must be a numeric string`);
    }
...
export function multiplyMoney(m: Money, scalar: string): Money {
    ...
    return { amount: result, currencyCode: m.currencyCode, precision: m.precision };
}
...
export function multiplyByRate(
    ...
    return { amount: fromScaledBigInt(rounded, precision), currencyCode: m.currencyCode, precision };
}
 ...
 */
export function sumAmounts(amounts: string[], precision = DEFAULT_PRECISION): string {
    let total = 0n;
 ...
 */
export function subtractAmounts(a: string, b: string, precision = DEFAULT_PRECISION): string {
 ...
 */
export function compareAmounts(a: string, b: string, precision = DEFAULT_PRECISION): -1 | 0 | 1 {
...
export function formatMoney(m: Money, displayPrecision = 2): string {
    ...
    const fracPart = (parts[1] ?? "").padEnd(displayPrecision, "0").slice(0, displayPrecision);
 ...
 */
function applyRounding(rawResult: bigint, factor: bigint, mode: RoundingMode): bigint {
...
function bigAdd(a: string, b: string, precision: number): string {
    return fromScaledBigInt(toScaledBigInt(a, precision) + toScaledBigInt(b, precision), precision);
}
