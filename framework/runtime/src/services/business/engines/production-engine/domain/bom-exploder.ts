...
export function explodeBom(
    ...
    if (level >= maxDepth) {
        throw new Error(`BOM explosion exceeded max depth of ${maxDepth}. Possible circular reference.`);
    }
    ...
    for (const line of rootBomLines) {
        ...
        const scrapFactor = multiplyAmounts(hundredPlusScrap, "0.01", QTY_PRECISION);
        const qtyTimesScrap = multiplyAmounts(line.quantityPer, scrapFactor, QTY_PRECISION);
        const totalQtyStr = multiplyAmounts(parentQty, qtyTimesScrap, QTY_PRECISION);
 ...
 */
export function consolidateExplodedBom(lines: ExplodedBomLine[]): ExplodedBomLine[] {
    ...
    for (const line of lines) {
        ...
        if (existing) {
            ...
            const newQty = sumAmounts([existing.totalQuantity, line.totalQuantity], QTY_PRECISION);
