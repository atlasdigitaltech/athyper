...
export interface PipelineOrchestrator {
    ...
    submit(ctx: OperationContext, input: SubmitTransactionInput): Promise<ServiceResult<TransactionPipeline>>;
    /** Resume a failed/paused pipeline from its current step */
    resume(ctx: OperationContext, pipelineId: string): Promise<ServiceResult<TransactionPipeline>>;
    /** Get pipeline status */
    getStatus(tenantId: string, txnId: string): Promise<TransactionPipeline | null>;
}
...
export class DefaultPipelineOrchestrator implements PipelineOrchestrator {
    ...
    async getStatus(tenantId: string, txnId: string): Promise<TransactionPipeline | null> {
    ...
    private async executeSteps(
        ...
        for (let step = current.currentStep as PipelineStep; step <= 12; step = (step + 1) as PipelineStep) {
            ...
            if (!executor) {
                ...
                if (status) {
                    current = await this.pipelineRepo.updateStep(
                        ctx.tenantId, current.id, step, status, {},
                    );
                }
                continue;
            }
            ...
            try {
                ...
                if (!result.success) {
                    ...
                    current = await this.pipelineRepo.updateStep(
                        ctx.tenantId, current.id, step, "FAILED",
                        { rejectionReason: result.error, ...result.updates },
                    );
                    return fail("STEP_FAILED", `Pipeline failed at step ${step}: ${result.error}`);
                }

                // Step succeeded — update pipeline
                const status = STEP_TO_STATUS[step] ?? current.status;
                current = await this.pipelineRepo.updateStep(
                    ctx.tenantId, current.id, step, status, result.updates,
                );
        ...
        // All steps completed
        current = await this.pipelineRepo.updateStatus(ctx.tenantId, current.id, "COMPLETED");
        return ok(current);
    }
}
