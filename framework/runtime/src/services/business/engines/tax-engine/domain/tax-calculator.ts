...
import { multiplyByRate, money, sumAmounts, toRatio } from "../../shared/money.js";
...
function resolveActualRate(rate: TaxRate, vendorCountryCode?: string): string {
    ...
    if (rate.treatyRate && vendorCountryCode && rate.taxType === "WHT") {
        ...
        return compareAmounts(rate.treatyRate, rate.rate) < 0 ? rate.treatyRate : rate.rate;
    }
    return rate.rate;
}
 ...
 */
export function aggregateTaxResults(calculations: Array<{ baseAmount: string; taxAmount: string }>): {
