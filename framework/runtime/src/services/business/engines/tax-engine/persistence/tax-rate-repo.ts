...
export interface TaxRateRepo {
    ...
    getEffectiveRates(tenantId: string, jurisdictionId: string, taxCode: string, asOfDate: Date): Promise<TaxRate[]>;

    /** Get all rates for a jurisdiction */
    listByJurisdiction(tenantId: string, jurisdictionId: string, filters?: { taxType?: TaxType }): Promise<TaxRate[]>;

    /** Get rates by tax code across all jurisdictions */
    getByTaxCode(tenantId: string, taxCode: string, asOfDate: Date): Promise<TaxRate[]>;
}
