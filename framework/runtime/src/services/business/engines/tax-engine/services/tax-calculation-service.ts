...
export class DefaultTaxCalculationService implements TaxCalculationService {
  ...
  async calculate(
    ...
    const jurisdiction = await this.jurisdictionRepo.getByCode(
      ctx.tenantId,
    ...
    if (!jurisdiction) {
      return fail(
        ...
        `Tax jurisdiction "${input.jurisdictionCode}" not found`,
      );
    }
    ...
    if (rates.length === 0) {
      return fail(
        "NO_RATES",
        `No effective tax rates found for ${input.taxCode} in ${input.jurisdictionCode}`,
      );
    }
