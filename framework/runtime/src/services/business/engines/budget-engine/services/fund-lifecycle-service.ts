...
export interface FundLifecycleService {
  ...
  ): Promise<ServiceResult<FundingTransaction>>;
...
export class DefaultFundLifecycleService implements FundLifecycleService {
  ...
  private async executeAction(
    ...
    if (input.idempotencyKey) {
      const exists = await this.txnRepo.existsByIdempotencyKey(
    ...
    if (!fp)
      return fail("FP_NOT_FOUND", `Funding profile ${input.fpId} not found`);
    ...
    if (fp.parentId) {
      ...
      for (const ancestor of ancestors) {
        if (isParentBlocked(ancestor)) {
          return fail(
            "FP_PARENT_BLOCKED",
            `Parent FP "${ancestor.code}" health is BLACK`,
          );
        }
      }
    }
    ...
    if (input.action !== "RELEASE") {
      ...
      if (wouldBreach(newBalance.available, "0", fp.currencyCode)) {
        return fail(
          "FP_BREACH",
          `Action would exceed funding limit for "${fp.code}"`,
        );
      }
    }
