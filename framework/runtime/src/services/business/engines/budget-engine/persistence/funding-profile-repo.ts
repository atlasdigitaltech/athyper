...
import type { FundingProfile, CreateFundingProfileInput, FPStatus, HealthStatus, FPStateSnapshot } from "../domain/types.js";
...
export interface FundingProfileRepo {
    ...
    getByCode(tenantId: string, entityCode: string, code: string, fiscalYear: number): Promise<FundingProfile | null>;

    /** Update amounts and health after a fund action */
    updateState(tenantId: string, id: string, state: FPStateSnapshot): Promise<FundingProfile>;

    /** Update status */
    updateStatus(tenantId: string, id: string, status: FPStatus): Promise<FundingProfile>;

    /** Update health and trend */
    updateHealth(tenantId: string, id: string, health: {
    ...
    list(tenantId: string, filters: {
        ...
        ouId?: string;
    }, pagination: PaginationParams): Promise<PaginatedResult<FundingProfile>>;
