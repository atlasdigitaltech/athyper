...
import type { CommitmentSchedule, CreateScheduleInput, ScheduleStatus } from "../domain/types.js";
...
export interface CommitmentScheduleRepo {
    ...
    getByCommitmentId(tenantId: string, commitmentId: string): Promise<CommitmentSchedule[]>;

    updateStatus(tenantId: string, id: string, status: ScheduleStatus, timestamps?: {
        triggeredAt?: Date;
        fulfilledAt?: Date;
    }): Promise<CommitmentSchedule>;

    /** Get schedules due on or before a date with PENDING status */
    getDueSchedules(tenantId: string, dueDate: Date): Promise<CommitmentSchedule[]>;
