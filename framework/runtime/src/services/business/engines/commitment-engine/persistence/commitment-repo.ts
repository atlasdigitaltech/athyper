...
import type { Commitment, CreateCommitmentInput, CommitmentStatus } from "../domain/types.js";

/**
 * Commitment Repository.
 */
export interface CommitmentRepo {
    create(input: CreateCommitmentInput & { docNumber: string }): Promise<Commitment>;
    getById(tenantId: string, id: string): Promise<Commitment | null>;
    getByDocNumber(tenantId: string, entityCode: string, docNumber: string): Promise<Commitment | null>;
    getByTxnId(tenantId: string, txnId: string): Promise<Commitment | null>;

    updateStatus(tenantId: string, id: string, status: CommitmentStatus): Promise<Commitment>;
    updateFulfilledAmount(tenantId: string, id: string, fulfilledAmount: string): Promise<Commitment>;
    updateApproval(tenantId: string, id: string, approvedBy: string): Promise<Commitment>;
    ...
    list(tenantId: string, filters: {
        ...
        fpId?: string;
        docType?: string;
    }, pagination: PaginationParams): Promise<PaginatedResult<Commitment>>;
