...
function generateFixedRecurring(
  ...
  const installmentScaled = toScaledBigInt(
    installmentAmount,
    DEFAULT_PRECISION,
  );
  const count = Number(
...
function generateEscalating(
  ...
  const escalationFactor = sumAmounts([
    "1",
    multiplyAmounts(escalationPct, "0.01"),
  ]);
  ...
  while (compareAmounts(remaining, "0") > 0) {
    ...
    const amount =
      compareAmounts(currentAmount, remaining) <= 0 ? currentAmount : remaining;
    remaining = subtractAmounts(remaining, amount);
