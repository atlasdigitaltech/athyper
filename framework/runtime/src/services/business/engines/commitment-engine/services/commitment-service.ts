...
import { matchFulfillmentToSchedule, calculateFulfilledAmount } from "../domain/fulfillment-matcher.js";
import { isValidCommitmentTransition, determineStatusFromFulfillment } from "../domain/lifecycle.js";
...
export interface CommitmentService {
    create(ctx: OperationContext, input: CreateCommitmentInput, docNumber: string, schedulePattern?: SchedulePattern): Promise<ServiceResult<Commitment>>;
    getById(tenantId: string, id: string): Promise<Commitment | null>;
    transition(ctx: OperationContext, id: string, targetStatus: CommitmentStatus): Promise<ServiceResult<Commitment>>;
    recordFulfillment(ctx: OperationContext, input: CreateFulfillmentInput): Promise<ServiceResult<CommitmentFulfillment>>;
    approve(ctx: OperationContext, id: string): Promise<ServiceResult<Commitment>>;
}
...
export class DefaultCommitmentService implements CommitmentService {
    ...
    async create(
        ...
        // Create commitment
        const commitment = await this.commitmentRepo.create({ ...input, docNumber });
    ...
    async transition(
        ...
        if (!isValidCommitmentTransition(commitment.status, targetStatus)) {
            return fail("INVALID_TRANSITION", `Cannot transition from ${commitment.status} to ${targetStatus}`);
        }
        ...
        const updated = await this.commitmentRepo.updateStatus(ctx.tenantId, id, targetStatus);
        return ok(updated);
    }
    ...
    async recordFulfillment(
        ...
        const commitment = await this.commitmentRepo.getById(ctx.tenantId, input.commitmentId);
        if (!commitment) {
            return fail("COMMITMENT_NOT_FOUND", `Commitment ${input.commitmentId} not found`);
        }

        if (commitment.status !== "ACTIVE" && commitment.status !== "PARTIALLY_FULFILLED") {
            return fail("COMMITMENT_NOT_ACTIVE", `Commitment is ${commitment.status}, must be ACTIVE or PARTIALLY_FULFILLED`);
        }

        // Match to schedule if applicable
        if (!input.scheduleId) {
            const schedules = await this.scheduleRepo.getByCommitmentId(ctx.tenantId, commitment.id);
        ...
        const allFulfillments = await this.fulfillmentRepo.getByCommitmentId(ctx.tenantId, commitment.id);
        const fulfilledAmount = calculateFulfilledAmount(allFulfillments);
        await this.commitmentRepo.updateFulfilledAmount(ctx.tenantId, commitment.id, fulfilledAmount);

        // Auto-transition status
        const newStatus = determineStatusFromFulfillment(commitment.totalAmount, fulfilledAmount);
        if (newStatus !== commitment.status) {
            await this.commitmentRepo.updateStatus(ctx.tenantId, commitment.id, newStatus);
        }

        // Update schedule status if matched
        if (input.scheduleId) {
            await this.scheduleRepo.updateStatus(ctx.tenantId, input.scheduleId, "FULFILLED", {
    ...
    async approve(ctx: OperationContext, id: string): Promise<ServiceResult<Commitment>> {
        const commitment = await this.commitmentRepo.getById(ctx.tenantId, id);
        if (!commitment) {
            return fail("COMMITMENT_NOT_FOUND", `Commitment ${id} not found`);
        }
        ...
        if (commitment.status !== "PENDING") {
            return fail("NOT_PENDING", `Commitment is ${commitment.status}, must be PENDING for approval`);
        }

        await this.commitmentRepo.updateApproval(ctx.tenantId, id, ctx.actorId);
        const updated = await this.commitmentRepo.updateStatus(ctx.tenantId, id, "ACTIVE");
        return ok(updated);
    }
}
