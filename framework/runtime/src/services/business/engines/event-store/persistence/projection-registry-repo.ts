...
export interface ProjectionRegistryRepo {
     ...
     */
    getById(tenantId: string, projectionId: string): Promise<ProjectionRegistration | null>;

    /**
     * Get all active projections for an engine.
     */
    getByEngine(tenantId: string, owningEngine: string): Promise<ProjectionRegistration[]>;

    /**
     * Get all active projections that subscribe to a given event type.
     */
    getByEventType(tenantId: string, eventType: string): Promise<ProjectionRegistration[]>;
