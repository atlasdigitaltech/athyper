...
import type { UniversalEventEnvelope, EventQuery, EventPage } from "../domain/types.js";
...
export interface EventRepo {
     ...
     */
    append(event: Omit<UniversalEventEnvelope, "id" | "createdAt" | "sequenceNo">): Promise<UniversalEventEnvelope>;
     ...
     */
    getById(tenantId: string, eventId: string): Promise<UniversalEventEnvelope | null>;

    /**
     * Get events by transaction ID.
     */
    getByTxnId(tenantId: string, txnId: string): Promise<UniversalEventEnvelope[]>;

    /**
     * Get events by correlation ID (full causal chain).
     */
    getByCorrelationId(tenantId: string, correlationId: string): Promise<UniversalEventEnvelope[]>;
     ...
     */
    existsByIdempotencyKey(tenantId: string, idempotencyKey: string): Promise<boolean>;
}
