...
import type { ProjectionRegistration, ProjectionSnapshot } from "../domain/types.js";
...
export interface ProjectionManager {
     ...
     */
    register(tenantId: string, registration: ProjectionRegistration): Promise<void>;

    /**
     * Rebuild a projection from scratch (delete checkpoints + catchup from start).
     */
    rebuild(tenantId: string, projectionId: string): Promise<{ eventsProcessed: number }>;
     ...
     */
    listByEngine(tenantId: string, engineCode: string): Promise<ProjectionRegistration[]>;
}
...
export class DefaultProjectionManager implements ProjectionManager {
    ...
    async register(tenantId: string, registration: ProjectionRegistration): Promise<void> {
        await this.registryRepo.upsert(tenantId, registration);
    }

    async rebuild(tenantId: string, projectionId: string): Promise<{ eventsProcessed: number }> {
    ...
    async takeSnapshot(tenantId: string, snapshot: ProjectionSnapshot): Promise<void> {
    ...
    async getLatestSnapshot(
        ...
        return this.snapshotRepo.getLatest(tenantId, projectionId, partitionDomain, partitionKey);
    }
    ...
    async listByEngine(tenantId: string, engineCode: string): Promise<ProjectionRegistration[]> {
