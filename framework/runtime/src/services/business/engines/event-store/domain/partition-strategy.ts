...
export const PartitionKeys = {
  ...
  ouFlow(tenantId: string, entityCode: string, ouId: string): string {
    return computePartitionKey("OU_FLOW", {
      tenantId,
      entityCode,
      primaryId: ouId,
    });
  ...
  entityFlow(tenantId: string, entityCode: string): string {
    return computePartitionKey("ENTITY_FLOW", {
      tenantId,
      entityCode,
      primaryId: "global",
    });
  ...
  inventoryFlow(
    ...
    return computePartitionKey("INVENTORY_FLOW", {
      tenantId,
      entityCode,
  ...
  commissionFlow(
    ...
    return computePartitionKey("COMMISSION_FLOW", {
      tenantId,
      entityCode,
      primaryId: partnerId,
    });
  ...
  icFlow(
    ...
    return computePartitionKey("IC_FLOW", {
      tenantId,
      entityCode: sorted[0]!,
      primaryId: sorted[1]!,
    });
