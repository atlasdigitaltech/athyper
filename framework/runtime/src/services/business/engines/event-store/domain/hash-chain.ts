 ...
 */
export async function verifyHash(payload: unknown, expectedHash: string): Promise<boolean> {
...
export async function computeChainHash(
    ...
    const input = previousHash ? `${previousHash}:${currentPayloadHash}` : currentPayloadHash;
    const encoder = new TextEncoder();
...
export async function verifyChain(
    ...
    for (let i = 0; i < payloadHashes.length; i++) {
        const expectedChainHash = await computeChainHash(previousHash, payloadHashes[i]!);
