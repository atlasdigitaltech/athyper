...
class DelegateTaskHandler {
  async handle(req: any, res: any, ctx: any) {
    const service = (await ctx.container.resolve(
      SHARE_TOKENS.taskDelegationService,
    )) as TaskDelegationService;
    const { delegateId, scopeType, scopeRef, permissions, reason, expiresAt } =
      req.body;
...
class RevokeDelegationHandler {
  async handle(req: any, res: any, ctx: any) {
    ...
    if (!revoked)
      return res
        .status(404)
        .json({ success: false, error: "Delegation not found" });
...
class AdminReassignHandler {
  async handle(req: any, res: any, ctx: any) {
    const service = (await ctx.container.resolve(
      SHARE_TOKENS.adminReassignmentService,
    )) as AdminReassignmentService;
...
class ShareRecordHandler {
  async handle(req: any, res: any, ctx: any) {
    const service = (await ctx.container.resolve(
      SHARE_TOKENS.recordShareService,
    )) as RecordShareService;
    const {
      entityType,
      entityId,
      sharedWithId,
...
class UnshareRecordHandler {
  async handle(req: any, res: any, ctx: any) {
    ...
    const revoked = await service.unshare(
      ctx.tenant?.id,
      req.params.id,
      ctx.auth?.principalId,
    );
...
class CreateTemporaryGrantHandler {
  async handle(req: any, res: any, ctx: any) {
    const service = (await ctx.container.resolve(
      SHARE_TOKENS.temporaryAccessService,
    )) as TemporaryAccessService;
    const {
      entityType,
      entityId,
      sharedWithId,
    ...
    if (!expiresAt) {
      return res.status(400).json({
        success: false,
        error: "expiresAt is required for temporary grants",
      });
    }
...
class ListShareAuditHandler {
  async handle(req: any, res: any, ctx: any) {
    const service = (await ctx.container.resolve(
      SHARE_TOKENS.shareAuditService,
    )) as ShareAuditService;
    const { startDate, endDate, actorId, action, limit, offset } =
      req.query ?? {};
...
class CreateExternalShareHandler {
  async handle(req: any, res: any, ctx: any) {
    const service = (await ctx.container.resolve(
      SHARE_TOKENS.crossTenantShareService,
    )) as CrossTenantShareService;
    const {
      targetEmail,
      entityType,
      entityId,
...
export const module: RuntimeModule = {
  ...
  async register(c: Container) {
    ...
    // ── Repos ─────────────────────────────────────────────────────
    c.register(
      SHARE_TOKENS.delegationGrantRepo,
      async () => new DelegationGrantRepo(db),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.recordShareRepo,
      async () => new RecordShareRepo(db),
      "singleton",
    );
    ...
    c.register(
      SHARE_TOKENS.shareAuditService,
      async () => new ShareAuditService(db),
      "singleton",
    );
    ...
    c.register(
      SHARE_TOKENS.taskDelegationService,
      async () => {
        const delegationRepo = await c.resolve<DelegationGrantRepo>(
          SHARE_TOKENS.delegationGrantRepo,
        );
        const auditService = await c.resolve<ShareAuditService>(
          SHARE_TOKENS.shareAuditService,
        );
    ...
    c.register(
      SHARE_TOKENS.sharePolicyResolver,
      async () => {
        const shareRepo = await c.resolve<RecordShareRepo>(
          SHARE_TOKENS.recordShareRepo,
        );
        const delegationRepo = await c.resolve<DelegationGrantRepo>(
          SHARE_TOKENS.delegationGrantRepo,
    ...
    c.register(
      SHARE_TOKENS.shareEnforcementService,
      async () => {
        const resolver = await c.resolve<SharePolicyResolver>(
          SHARE_TOKENS.sharePolicyResolver,
        );
        const auditService = await c.resolve<ShareAuditService>(
          SHARE_TOKENS.shareAuditService,
    ...
    c.register(
      SHARE_TOKENS.temporaryAccessService,
      async () => {
        const recordShareService = await c.resolve<RecordShareService>(
          SHARE_TOKENS.recordShareService,
        );
        const delegationService = await c.resolve<TaskDelegationService>(
          SHARE_TOKENS.taskDelegationService,
        );
        const auditService = await c.resolve<ShareAuditService>(
          SHARE_TOKENS.shareAuditService,
    ...
    // ── Handlers ──────────────────────────────────────────────────
    c.register(
      SHARE_TOKENS.delegateTaskHandler,
      async () => new DelegateTaskHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.revokeDelegationHandler,
      async () => new RevokeDelegationHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.listMyDelegationsHandler,
      async () => new ListMyDelegationsHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.listDelegatedToMeHandler,
      async () => new ListDelegatedToMeHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.adminReassignHandler,
      async () => new AdminReassignHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.adminBulkReassignHandler,
      async () => new AdminBulkReassignHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.shareRecordHandler,
      async () => new ShareRecordHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.unshareRecordHandler,
      async () => new UnshareRecordHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.listEntitySharesHandler,
      async () => new ListEntitySharesHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.listMySharesHandler,
      async () => new ListMySharesHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.createTemporaryGrantHandler,
      async () => new CreateTemporaryGrantHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.listShareAuditHandler,
      async () => new ListShareAuditHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.getRecordShareHistoryHandler,
      async () => new GetRecordShareHistoryHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.createExternalShareHandler,
      async () => new CreateExternalShareHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.verifyExternalShareHandler,
      async () => new VerifyExternalShareHandler(),
      "singleton",
    );
    c.register(
      SHARE_TOKENS.revokeExternalShareHandler,
      async () => new RevokeExternalShareHandler(),
      "singleton",
    );

    // Also register under global tokens for cross-module access
    c.register(
      TOKENS.shareTaskDelegationService,
      async () => {
        return c.resolve<TaskDelegationService>(
          SHARE_TOKENS.taskDelegationService,
        );
      },
      "singleton",
    ...
    c.register(
      TOKENS.shareEnforcementService,
      async () => {
        return c.resolve<ShareEnforcementService>(
          SHARE_TOKENS.shareEnforcementService,
        );
      },
      "singleton",
    ...
    c.register(
      TOKENS.shareRecordShareService,
      async () => {
    ...
    c.register(
      TOKENS.shareTemporaryAccessService,
      async () => {
        return c.resolve<TemporaryAccessService>(
          SHARE_TOKENS.temporaryAccessService,
        );
      },
      "singleton",
    ...
    c.register(
      TOKENS.shareCrossTenantService,
      async () => {
        return c.resolve<CrossTenantShareService>(
          SHARE_TOKENS.crossTenantShareService,
        );
      },
      "singleton",
    ...
    c.register(
      TOKENS.shareAdminReassignmentService,
      async () => {
        return c.resolve<AdminReassignmentService>(
          SHARE_TOKENS.adminReassignmentService,
        );
      },
      "singleton",
    ...
    c.register(
      TOKENS.shareDelegationGrantRepo,
      async () => {
