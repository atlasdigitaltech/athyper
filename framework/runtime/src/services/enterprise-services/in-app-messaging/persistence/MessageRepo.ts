...
export class MessageRepo {
   ...
   */
  async getById(tenantId: string, id: MessageId): Promise<Message | undefined> {
   ...
   */
  async update(tenantId: string, id: MessageId, body: string): Promise<void> {
    await this.db
   ...
   */
  async softDelete(tenantId: string, id: MessageId): Promise<void> {
    await this.db
      .updateTable(TABLE as any)
   ...
   */
  async countBySender(tenantId: string, senderId: string): Promise<number> {
  ...
  async searchMessages(
    ...
    let query = this.db
      ...
      .where("tenant_id", "=", tenantId)
      .where(sql`body_tsv @@ to_tsquery('english', ${sanitizedQuery})` as any);
