...
} from "../../persistence/MessageDeliveryRepo";
import {
  MessageRepo,
...
import {
  MessagingMetrics,
  MetricTimer,
} from "../observability/MessagingMetrics";
import {
...
} from "../rate-limit/MessagingRateLimiter";
...
export class MessageService {
  ...
  async sendMessage(input: CreateMessageInput): Promise<MessageWithDeliveries> {
    ...
    try {
      // Check rate limit
      await this.rateLimiter.enforceMessageSendLimit(
        input.tenantId,
        input.senderId,
      );
      ...
      try {
        ...
        this.metrics.trackAccessGranted(
          input.tenantId,
          "conversation",
          "send_message",
        );
        ...
        this.metrics.trackAccessDenied(
          input.tenantId,
          "conversation",
          "send_message",
        );
      ...
      const deliveryInputs: CreateDeliveryInput[] = recipientIds.map(
        (recipientId) => ({
  ...
  async editMessage(input: EditMessageInput): Promise<Message> {
    const message = await this.messageRepo.getById(
      input.tenantId,
      input.messageId,
    );
    if (!message) {
  ...
  async markAllAsRead(
    ...
    // Get latest message
    const latestMessage = await this.messageRepo.getLatest(
      tenantId,
      conversationId,
    );
    if (!latestMessage) {
    ...
    if (unreadMessageIds.length > 0) {
      await this.deliveryRepo.markBatchAsRead(
        tenantId,
        unreadMessageIds,
        userId,
      );
  ...
  async listForConversation(
    ...
    // List messages
    return this.messageRepo.listForConversation(
      tenantId,
      conversationId,
      options,
    );
  ...
  async listThreadReplies(
    ...
    // Get parent message
    const parentMessage = await this.messageRepo.getById(
      tenantId,
      parentMessageId,
    );
    if (!parentMessage) {
    ...
    // List thread replies
    return this.messageRepo.listThreadReplies(
      tenantId,
      parentMessageId,
      options,
    );
  ...
  async searchMessages(
    ...
    try {
      ...
      if (options?.conversationId) {
        ...
        const participants =
          await this.participantRepo.listActiveForConversation(
            tenantId,
            options.conversationId,
          );
      ...
      const userParticipations = await this.participantRepo.listForUser(
        tenantId,
        userId,
      );
