...
export class OrchestrationEngine {
  ...
  private evaluateCondition(
    ...
    if (!step.condition)
      throw new Error(`Step ${step.id}: condition config missing`);
  ...
  private async executeParallel(
    ...
    if (!step.parallel)
      throw new Error(`Step ${step.id}: parallel config missing`);
  ...
  private async executeDelay(step: OrchestrationStep): Promise<void> {
    ...
    await new Promise<void>((resolve) =>
      setTimeout(resolve, step.delay!.durationMs),
    );
  }
  ...
  private interpolatePayload(
    ...
    for (const [key, value] of Object.entries(template)) {
      if (
        ...
        result[key] = this.interpolatePayload(
