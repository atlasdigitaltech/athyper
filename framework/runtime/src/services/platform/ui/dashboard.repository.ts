...
export class DashboardRepository {
  ...
  async listForUser(params: {
    ...
    for (const p of params.personas)
      principalMatches.push({ type: "persona", key: p });
    for (const r of params.roles)
      principalMatches.push({ type: "role", key: r });
    for (const g of params.groups)
      principalMatches.push({ type: "group", key: g });
  ...
  async getPublished(dashboardId: string): Promise<{
    ...
    const row = await this.db
      ...
      .select([
        ...
        "d.code as d_code",
        "d.title_key as d_title_key",
        "d.description_key as d_description_key",
        "d.module_code as d_module_code",
        "d.workbench as d_workbench",
        "d.visibility as d_visibility",
        "d.icon as d_icon",
        "d.sort_order as d_sort_order",
        "d.is_hidden as d_is_hidden",
        "d.forked_from_id as d_forked_from_id",
        ...
        "v.id as v_id",
        "v.tenant_id as v_tenant_id",
        "v.dashboard_id as v_dashboard_id",
        "v.version_no as v_version_no",
        "v.status as v_status",
        "v.layout as v_layout",
        "v.published_at as v_published_at",
        "v.published_by as v_published_by",
  ...
  async duplicate(params: {
    ...
    if (!source)
      throw new Error(`Dashboard not found: ${params.sourceDashboardId}`);
  ...
  async update(
    ...
    if (updates.descriptionKey !== undefined)
      set.description_key = updates.descriptionKey;
