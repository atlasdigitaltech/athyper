...
describe("DashboardContributionSeeder", () => {
  ...
  beforeEach(() => {
    ...
    seeder = new DashboardContributionSeeder(
      mockRepo as any,
      mockLogger as any,
    );
  });
  ...
  it("should find contribution files recursively", () => {
    const tmpDir = createTempDir([
      {
        path: "business/crm/dashboard.contribution.json",
        content: makeContribution({ module_code: "CRM" }),
      },
      {
        path: "business/hr/dashboard.contribution.json",
        content: makeContribution({ module_code: "HR" }),
      },
      {
        path: "platform/ref/dashboard.contribution.json",
        content: makeContribution({ module_code: "REF" }),
      },
    ]);

    const files = seeder.findContributionFiles(tmpDir);
    expect(files).toHaveLength(3);
    expect(files.every((f) => f.endsWith("dashboard.contribution.json"))).toBe(
      true,
    );
  });
