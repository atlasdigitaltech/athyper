      ...
      if (!result) {
        res.status(404).json({
          ...
          error: {
            ...
            message: `Dashboard ${id} not found`,
          },
        });
        return;
      }
    ...
    if (!body.code || !body.titleKey || !body.moduleCode || !body.workbench) {
      res.status(400).json({
        ...
        error: {
          ...
          message: "code, titleKey, moduleCode, and workbench are required",
        },
      });
      return;
    }
  ...
  async handle(
    ...
    if (!body.principalType || !body.principalKey || !body.permission) {
      res.status(400).json({
        ...
        error: {
          code: "MISSING_REQUIRED_FIELDS",
          message: "principalType, principalKey, and permission are required",
        },
      });
      return;
    }
    ...
    if (!validTypes.includes(body.principalType)) {
      res.status(400).json({
        ...
        error: {
          ...
          message: `principalType must be one of: ${validTypes.join(", ")}`,
        },
      });
      return;
    }
