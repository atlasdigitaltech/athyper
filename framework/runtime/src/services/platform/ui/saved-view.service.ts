...
export class SavedViewService {
   ...
   */
  async getView(user: UserContext, viewId: string): Promise<SavedViewRow> {
  ...
  async createView(
    ...
    if (!created) {
      throw new HttpError(
        500,
        "CREATE_FAILED",
        "Failed to retrieve created view",
      );
    }
    return created;
  }
   ...
   */
  async deleteView(user: UserContext, viewId: string): Promise<void> {
    const existing = await this.repo.getById(user.tenantId, viewId);
    if (!existing) {
      throw new HttpError(404, "VIEW_NOT_FOUND", `View "${viewId}" not found`);
    }
  ...
  private assertWriteScope(user: UserContext, scope: string): void {
    ...
    if (scope === "SHARED" && user.roles.includes("ui:preset:write_shared"))
      return;
    if (scope === "SYSTEM" && user.roles.includes("ui:preset:write_system"))
      return;
  ...
  private assertWriteAccess(user: UserContext, view: SavedViewRow): void {
    ...
    if (
      view.scope === "SHARED" &&
      user.roles.includes("ui:preset:write_shared")
    )
      return;
    if (
      view.scope === "SYSTEM" &&
      user.roles.includes("ui:preset:write_system")
    )
      return;
  ...
  private assertDeleteAccess(user: UserContext, view: SavedViewRow): void {
    ...
    if (
      view.scope === "SHARED" &&
      user.roles.includes("ui:preset:delete_shared")
    )
      return;
    if (
      view.scope === "SYSTEM" &&
