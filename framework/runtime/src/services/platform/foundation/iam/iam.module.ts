...
import { ListPlatformTenantsHandler } from "./api/platform-tenants.handler.js";
...
import { PersonaRegistryService } from "./persona-model/persona-registry.service.js";
import { SessionInvalidationService } from "./session-invalidation.js";
import {
  loadTenantIAMProfile,
  validateTenantIAMProfile,
} from "./tenant-iam-profile.js";
...
const IAM_HANDLER_TOKENS = {
  ...
  // Platform Control
  listPlatformTenants: "iam.handler.listPlatformTenants",
} as const;
...
class GetPrincipalEntitlementsHandler implements RouteHandler {
  async handle(
    ...
    const db = await ctx.container.resolve<Kysely<DB>>(TOKENS.db);
    const capService = await ctx.container.resolve<IPersonaCapabilityService>(
      TOKENS.iamCapabilityService,
    );
    const tenantId = ctx.tenant.tenantKey ?? "default";
    ...
    const roles = await db
      ...
      .select([
        ...
        "core.role.name",
        "core.role.code",
        "core.principal_role.assigned_at",
        "core.principal_role.expires_at",
      ])
    ...
    const groups = await db
      ...
      .innerJoin(
        ...
        "core.principal_group.id",
        "core.group_member.group_id",
      )
      .select([
        "core.principal_group.id",
        "core.principal_group.name",
        "core.principal_group.code",
      ])
    ...
    const effectivePersona =
      await capService.resolveEffectivePersona(roleCodes);
...
class CheckCapabilityHandler implements RouteHandler {
  async handle(
    ...
    if (!persona || !operation) {
      res.status(400).json({
        success: false,
        error: "persona and operation query params required",
      });
      return;
    }
...
class GetPersonaCapabilitiesHandler implements RouteHandler {
  async handle(
    ...
    const capabilities = await capService.getPersonaCapabilities(
...
class StubHandler implements RouteHandler {
  ...
  async handle(
    ...
    res
      .status(501)
      .json({ success: false, error: `Not implemented: ${this.description}` });
  }
}
...
class MfaStatusHandler implements RouteHandler {
  async handle(
    ...
    const mfaService = await ctx.container.resolve<MfaService>(
      TOKENS.iamMfaService,
    );
...
class ListGroupsHandler implements RouteHandler {
  async handle(
    ...
    if (req.query.search) {
      const search = `%${req.query.search}%`;
      qb = qb.where((eb) =>
        eb.or([eb("name", "ilike", search), eb("code", "ilike", search)]),
      );
    }
...
class DeleteGroupHandler implements RouteHandler {
  async handle(
    ...
    const result = await db
      .deleteFrom("core.principal_group")
      .where("id", "=", req.params.id)
      .where("tenant_id", "=", tenantId)
      .executeTakeFirst();
...
class AddGroupMemberHandler implements RouteHandler {
  async handle(
    ...
    if (!body.principalId) {
      res.status(400).json({
        success: false,
        error: { code: "MISSING_FIELDS", message: "principalId is required" },
      });
      return;
    }
    ...
    try {
      ...
      if (message.includes("duplicate") || message.includes("unique")) {
        res.status(409).json({
          success: false,
          error: { code: "DUPLICATE", message: "Member already in group" },
        });
      } else {
        throw err;
      }
...
class DeleteRoleHandler implements RouteHandler {
  async handle(
    ...
    await db
      .deleteFrom("core.principal_role")
  ...
  async handle(
    ...
    if (!body.principalId || !body.roleId) {
      res.status(400).json({
        ...
        error: {
          code: "MISSING_FIELDS",
          message: "principalId and roleId are required",
        },
      });
      return;
    }
  ...
  async handle(
    ...
    if (body.parentId) {
      ...
      if (!parent) {
        res.status(400).json({
          success: false,
          error: { code: "INVALID_PARENT", message: "Parent OU not found" },
        });
        return;
      }
    }
...
class GetOuHandler implements RouteHandler {
  async handle(
    ...
    if (!ou) {
      res
        .status(404)
        .json({ success: false, error: "Organization unit not found" });
      return;
    }
...
class UpdateOuHandler implements RouteHandler {
  async handle(
    ...
    if (body.parentId) {
      if (body.parentId === req.params.id) {
        res.status(400).json({
          ...
          error: {
            code: "CIRCULAR_REF",
            message: "OU cannot be its own parent",
          },
        });
        return;
      }
    ...
    const ou = await db
      .updateTable("core.organizational_unit")
      .set(updates)
      .where("id", "=", req.params.id)
      .where("tenant_id", "=", tenantId)
      .returningAll()
...
class DeleteOuHandler implements RouteHandler {
  async handle(
    ...
    if (children) {
      res.status(400).json({
        ...
        error: {
          code: "HAS_CHILDREN",
          message:
            "Cannot delete OU with children. Move or delete children first.",
        },
      });
      return;
    }
    ...
    const result = await db
      .deleteFrom("core.organizational_unit")
      .where("id", "=", req.params.id)
      .where("tenant_id", "=", tenantId)
      .executeTakeFirst();
...
class CreateFieldPolicyHandler implements RouteHandler {
  async handle(
    ...
    const body = req.body as {
      ...
      policyType?: string;
      roleList?: string[];
      abacCondition?: Record<string, unknown>;
      maskStrategy?: string;
    ...
    if (!body.entityId || !body.fieldPath || !body.policyType) {
      res.status(400).json({
        ...
        error: {
          code: "MISSING_FIELDS",
          message: "entityId, fieldPath, and policyType are required",
        },
      });
      return;
    }
    ...
    const policy = await db
      .insertInto("meta.field_security_policy")
      .values({
        id: crypto.randomUUID(),
        tenant_id: tenantId,
        ...
        abac_condition: body.abacCondition
          ? JSON.stringify(body.abacCondition)
          : null,
...
class UpdateFieldPolicyHandler implements RouteHandler {
  async handle(
    ...
    if (body.roleList !== undefined)
      updates.role_list = JSON.stringify(body.roleList);
    if (body.abacCondition !== undefined)
      updates.abac_condition = JSON.stringify(body.abacCondition);
    if (body.maskStrategy !== undefined)
      updates.mask_strategy = body.maskStrategy;
    if (body.maskConfig !== undefined)
      updates.mask_config = JSON.stringify(body.maskConfig);
    ...
    const policy = await db
      .updateTable("meta.field_security_policy")
      .set(updates)
      .where("id", "=", req.params.id)
      .where("tenant_id", "=", tenantId)
      .returningAll()
...
class DeleteFieldPolicyHandler implements RouteHandler {
  async handle(
    ...
    const result = await db
      .deleteFrom("meta.field_security_policy")
      .where("id", "=", req.params.id)
      .where("tenant_id", "=", tenantId)
      .executeTakeFirst();
...
export const module: RuntimeModule = {
  ...
  async register(c: Container) {
    ...
    // ── Persona Registry (DB-driven priorities) ────────────────────
    c.register(
      TOKENS.iamPersonaRegistry,
      async () => {
        const repo = new DatabasePersonaCapabilityRepository(db);
        return new PersonaRegistryService(repo, logger);
      },
      "singleton",
    );
    ...
    c.register(
      ...
      async () => {
        const repo = new DatabasePersonaCapabilityRepository(db);
        const registry = await c.resolve<PersonaRegistryService>(
          TOKENS.iamPersonaRegistry,
        );
        return new PersonaCapabilityService(repo, logger, { registry });
      },
      "singleton",
    );
    ...
    c.register(
      TOKENS.iamTenantProfile,
      async () => {
        return {
          load: (tenantId: string) => loadTenantIAMProfile(db as any, tenantId),
          validate: validateTenantIAMProfile,
        } satisfies {
          load: (tenantId: string) => Promise<TenantIAMProfile>;
          validate: (profile: TenantIAMProfile) => void;
        };
      },
      "singleton",
    );
    ...
    c.register(
      TOKENS.policyGate,
      async () => createPolicyGate(db),
      "singleton",
    );
    c.register(
      TOKENS.fieldSecurityRepo,
      async () => new DatabaseFieldSecurityRepository(db),
      "singleton",
    );
    c.register(
      TOKENS.fieldAccessService,
      async () => {
    ...
    c.register(
      TOKENS.fieldProjectionBuilder,
      async () => {
        const fas = await c.resolve<FieldAccessService>(
          TOKENS.fieldAccessService,
        );
    ...
    // Principals
    c.register(
      IAM_HANDLER_TOKENS.listPrincipals,
      async () => new ListPrincipalsHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.getPrincipal,
      async () => new GetPrincipalHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.getPrincipalEntitlements,
      async () => new GetPrincipalEntitlementsHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.getPrincipalRoles,
      async () => new GetPrincipalRolesHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.getPrincipalGroups,
      async () => new GetPrincipalGroupsHandler(),
      "singleton",
    );

    // Capabilities
    c.register(
      IAM_HANDLER_TOKENS.capabilityMatrix,
      async () => new CapabilityMatrixHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.listPersonas,
      async () => new ListPersonasHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.checkCapability,
      async () => new CheckCapabilityHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.getPersonaCapabilities,
      async () => new GetPersonaCapabilitiesHandler(),
      "singleton",
    );

    // MFA
    c.register(
      IAM_HANDLER_TOKENS.mfaStatus,
      async () => new MfaStatusHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.mfaRequired,
      async () => new MfaRequiredHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.mfaEnroll,
      async () => new MfaEnrollHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.mfaEnrollVerify,
      async () => new MfaEnrollVerifyHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.mfaDevices,
      async () => new MfaDevicesHandler(),
      "singleton",
    );

    // Groups
    c.register(
      IAM_HANDLER_TOKENS.listGroups,
      async () => new ListGroupsHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.createGroup,
      async () => new CreateGroupHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.getGroup,
      async () => new GetGroupHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.updateGroup,
      async () => new UpdateGroupHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.deleteGroup,
      async () => new DeleteGroupHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.listGroupMembers,
      async () => new ListGroupMembersHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.addGroupMember,
      async () => new AddGroupMemberHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.removeGroupMember,
      async () => new RemoveGroupMemberHandler(),
      "singleton",
    );

    // Roles
    c.register(
      IAM_HANDLER_TOKENS.listRoles,
      async () => new ListRolesHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.createRole,
      async () => new CreateRoleHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.getRole,
      async () => new GetRoleHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.updateRole,
      async () => new UpdateRoleHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.deleteRole,
      async () => new DeleteRoleHandler(),
      "singleton",
    );

    // Role Bindings
    c.register(
      IAM_HANDLER_TOKENS.listRoleBindings,
      async () => new ListRoleBindingsHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.createRoleBinding,
      async () => new CreateRoleBindingHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.getRoleBinding,
      async () => new GetRoleBindingHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.deleteRoleBinding,
      async () => new DeleteRoleBindingHandler(),
      "singleton",
    );

    // Organization Units
    c.register(
      IAM_HANDLER_TOKENS.getOuTree,
      async () => new GetOuTreeHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.createOu,
      async () => new CreateOuHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.getOu,
      async () => new GetOuHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.updateOu,
      async () => new UpdateOuHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.deleteOu,
      async () => new DeleteOuHandler(),
      "singleton",
    );

    // Field Security Policies
    c.register(
      IAM_HANDLER_TOKENS.listFieldPolicies,
      async () => new ListFieldPoliciesHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.createFieldPolicy,
      async () => new CreateFieldPolicyHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.getFieldPolicy,
      async () => new GetFieldPolicyHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.updateFieldPolicy,
      async () => new UpdateFieldPolicyHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.deleteFieldPolicy,
      async () => new DeleteFieldPolicyHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.mfaCancelEnroll,
      async () => new MfaCancelEnrollHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.mfaChallenge,
      async () => new MfaChallengeHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.mfaVerify,
      async () => new MfaVerifyHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.mfaDisable,
      async () => new MfaDisableHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.mfaBackupCodes,
      async () => new MfaBackupCodesRegenerateHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.mfaRevokeDevice,
      async () => new MfaRevokeDeviceHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.mfaRevokeAllDevices,
      async () => new MfaRevokeAllDevicesHandler(),
      "singleton",
    );
    c.register(
      IAM_HANDLER_TOKENS.mfaCheckDevice,
      async () => new MfaCheckDeviceHandler(),
      "singleton",
    );

    // Platform Control
    c.register(
      IAM_HANDLER_TOKENS.listPlatformTenants,
      async () => new ListPlatformTenantsHandler(),
      "singleton",
    );
  ...
  async contribute(c: Container) {
    ...
    routes.add({
      method: "GET",
      path: "/api/iam/principals",
      handlerToken: IAM_HANDLER_TOKENS.listPrincipals,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/principals/:id",
      handlerToken: IAM_HANDLER_TOKENS.getPrincipal,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/principals/:id/entitlements",
      handlerToken: IAM_HANDLER_TOKENS.getPrincipalEntitlements,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/principals/:id/roles",
      handlerToken: IAM_HANDLER_TOKENS.getPrincipalRoles,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/principals/:id/groups",
      handlerToken: IAM_HANDLER_TOKENS.getPrincipalGroups,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    ...
    routes.add({
      method: "GET",
      path: "/api/iam/groups",
      handlerToken: IAM_HANDLER_TOKENS.listGroups,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "POST",
      path: "/api/iam/groups",
      handlerToken: IAM_HANDLER_TOKENS.createGroup,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/groups/:id",
      handlerToken: IAM_HANDLER_TOKENS.getGroup,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "PATCH",
      path: "/api/iam/groups/:id",
      handlerToken: IAM_HANDLER_TOKENS.updateGroup,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "DELETE",
      path: "/api/iam/groups/:id",
      handlerToken: IAM_HANDLER_TOKENS.deleteGroup,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/groups/:id/members",
      handlerToken: IAM_HANDLER_TOKENS.listGroupMembers,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "POST",
      path: "/api/iam/groups/:id/members",
      handlerToken: IAM_HANDLER_TOKENS.addGroupMember,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "DELETE",
      path: "/api/iam/groups/:id/members/:principalId",
      handlerToken: IAM_HANDLER_TOKENS.removeGroupMember,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    ...
    routes.add({
      method: "GET",
      path: "/api/iam/roles",
      handlerToken: IAM_HANDLER_TOKENS.listRoles,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "POST",
      path: "/api/iam/roles",
      handlerToken: IAM_HANDLER_TOKENS.createRole,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/roles/:id",
      handlerToken: IAM_HANDLER_TOKENS.getRole,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "PATCH",
      path: "/api/iam/roles/:id",
      handlerToken: IAM_HANDLER_TOKENS.updateRole,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "DELETE",
      path: "/api/iam/roles/:id",
      handlerToken: IAM_HANDLER_TOKENS.deleteRole,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    ...
    routes.add({
      method: "GET",
      path: "/api/iam/role-bindings",
      handlerToken: IAM_HANDLER_TOKENS.listRoleBindings,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "POST",
      path: "/api/iam/role-bindings",
      handlerToken: IAM_HANDLER_TOKENS.createRoleBinding,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/role-bindings/:id",
      handlerToken: IAM_HANDLER_TOKENS.getRoleBinding,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "DELETE",
      path: "/api/iam/role-bindings/:id",
      handlerToken: IAM_HANDLER_TOKENS.deleteRoleBinding,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    ...
    routes.add({
      method: "GET",
      path: "/api/iam/ous",
      handlerToken: IAM_HANDLER_TOKENS.getOuTree,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "POST",
      path: "/api/iam/ous",
      handlerToken: IAM_HANDLER_TOKENS.createOu,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/ous/:id",
      handlerToken: IAM_HANDLER_TOKENS.getOu,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "PATCH",
      path: "/api/iam/ous/:id",
      handlerToken: IAM_HANDLER_TOKENS.updateOu,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "DELETE",
      path: "/api/iam/ous/:id",
      handlerToken: IAM_HANDLER_TOKENS.deleteOu,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    ...
    routes.add({
      method: "GET",
      path: "/api/iam/field-policies",
      handlerToken: IAM_HANDLER_TOKENS.listFieldPolicies,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "POST",
      path: "/api/iam/field-policies",
      handlerToken: IAM_HANDLER_TOKENS.createFieldPolicy,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/field-policies/:id",
      handlerToken: IAM_HANDLER_TOKENS.getFieldPolicy,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "PATCH",
      path: "/api/iam/field-policies/:id",
      handlerToken: IAM_HANDLER_TOKENS.updateFieldPolicy,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    routes.add({
      method: "DELETE",
      path: "/api/iam/field-policies/:id",
      handlerToken: IAM_HANDLER_TOKENS.deleteFieldPolicy,
      authRequired: true,
      tags: ["iam", "admin"],
    });
    ...
    routes.add({
      method: "GET",
      path: "/api/iam/capabilities/matrix",
      handlerToken: IAM_HANDLER_TOKENS.capabilityMatrix,
      authRequired: true,
      tags: ["iam"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/capabilities/personas",
      handlerToken: IAM_HANDLER_TOKENS.listPersonas,
      authRequired: true,
      tags: ["iam"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/capabilities/check",
      handlerToken: IAM_HANDLER_TOKENS.checkCapability,
      authRequired: true,
      tags: ["iam"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/capabilities/persona/:code",
      handlerToken: IAM_HANDLER_TOKENS.getPersonaCapabilities,
      authRequired: true,
      tags: ["iam"],
    });
    ...
    routes.add({
      method: "GET",
      path: "/api/iam/mfa/status",
      handlerToken: IAM_HANDLER_TOKENS.mfaStatus,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/mfa/required",
      handlerToken: IAM_HANDLER_TOKENS.mfaRequired,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    routes.add({
      method: "POST",
      path: "/api/iam/mfa/enroll",
      handlerToken: IAM_HANDLER_TOKENS.mfaEnroll,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    routes.add({
      method: "POST",
      path: "/api/iam/mfa/enroll/verify",
      handlerToken: IAM_HANDLER_TOKENS.mfaEnrollVerify,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    routes.add({
      method: "DELETE",
      path: "/api/iam/mfa/enroll",
      handlerToken: IAM_HANDLER_TOKENS.mfaCancelEnroll,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    routes.add({
      method: "POST",
      path: "/api/iam/mfa/challenge",
      handlerToken: IAM_HANDLER_TOKENS.mfaChallenge,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    routes.add({
      method: "POST",
      path: "/api/iam/mfa/verify",
      handlerToken: IAM_HANDLER_TOKENS.mfaVerify,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    routes.add({
      method: "DELETE",
      path: "/api/iam/mfa",
      handlerToken: IAM_HANDLER_TOKENS.mfaDisable,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    routes.add({
      method: "POST",
      path: "/api/iam/mfa/backup-codes/regenerate",
      handlerToken: IAM_HANDLER_TOKENS.mfaBackupCodes,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    routes.add({
      method: "GET",
      path: "/api/iam/mfa/devices",
      handlerToken: IAM_HANDLER_TOKENS.mfaDevices,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    routes.add({
      method: "DELETE",
      path: "/api/iam/mfa/devices/:deviceId",
      handlerToken: IAM_HANDLER_TOKENS.mfaRevokeDevice,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    routes.add({
      method: "DELETE",
      path: "/api/iam/mfa/devices",
      handlerToken: IAM_HANDLER_TOKENS.mfaRevokeAllDevices,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    routes.add({
      method: "POST",
      path: "/api/iam/mfa/devices/check",
      handlerToken: IAM_HANDLER_TOKENS.mfaCheckDevice,
      authRequired: true,
      tags: ["iam", "mfa"],
    });
    ...
    routes.add({
      ...
      handlerToken: IAM_HANDLER_TOKENS.listPlatformTenants,
      authRequired: true,
      tags: ["iam", "platform"],
    });
