...
export class TenantIAMProfileViolation extends Error {
  constructor(
    ...
    super(
      `IAM profile violation: ${field} — ${reason} (got ${JSON.stringify(tenantValue)}, minimum ${JSON.stringify(minimumValue)})`,
    );
...
export function validateTenantIAMProfile(profile: TenantIAMProfile): void {
  ...
  if (mp.requireSpecial && !pp.requireSpecial) {
    throw new TenantIAMProfileViolation(
      ...
      "platform requires special character",
      pp.requireSpecial,
      mp.requireSpecial,
    );
  }
  ...
  if (profile.mfaRequired && profile.allowedMfaTypes.length === 0) {
    throw new TenantIAMProfileViolation(
      "allowedMfaTypes",
      "MFA is required but no types are allowed",
      profile.allowedMfaTypes,
      ["totp"],
    );
  }
}
 ...
 */
export function mergeTenantIAMDefaults(
  partial: Partial<TenantIAMProfile>,
  defaults: TenantIAMProfile = DEFAULT_IAM_PROFILE,
): TenantIAMProfile {
  const merged: TenantIAMProfile = {
    ...
    lockoutDurationMinutes:
      partial.lockoutDurationMinutes ?? defaults.lockoutDurationMinutes,
    passwordPolicy: {
      minLength:
        partial.passwordPolicy?.minLength ?? defaults.passwordPolicy.minLength,
      history:
        partial.passwordPolicy?.history ?? defaults.passwordPolicy.history,
      requireUppercase:
        partial.passwordPolicy?.requireUppercase ??
        defaults.passwordPolicy.requireUppercase,
      requireLowercase:
        partial.passwordPolicy?.requireLowercase ??
        defaults.passwordPolicy.requireLowercase,
      requireDigit:
        partial.passwordPolicy?.requireDigit ??
        defaults.passwordPolicy.requireDigit,
      requireSpecial:
        partial.passwordPolicy?.requireSpecial ??
        defaults.passwordPolicy.requireSpecial,
    },
  };

  // Enforce platform minimums (clamp values)
  merged.maxLoginFailures = Math.min(
    merged.maxLoginFailures,
    PLATFORM_MINIMUMS.maxLoginFailures,
  );
  merged.lockoutDurationMinutes = Math.max(
    merged.lockoutDurationMinutes,
    PLATFORM_MINIMUMS.lockoutDurationMinutes,
  );
  merged.passwordPolicy.minLength = Math.max(
    merged.passwordPolicy.minLength,
    PLATFORM_MINIMUMS.passwordPolicy.minLength,
  );
  merged.passwordPolicy.history = Math.max(
    merged.passwordPolicy.history,
    PLATFORM_MINIMUMS.passwordPolicy.history,
  );

  if (PLATFORM_MINIMUMS.passwordPolicy.requireUppercase)
    merged.passwordPolicy.requireUppercase = true;
  if (PLATFORM_MINIMUMS.passwordPolicy.requireLowercase)
    merged.passwordPolicy.requireLowercase = true;
  if (PLATFORM_MINIMUMS.passwordPolicy.requireDigit)
    merged.passwordPolicy.requireDigit = true;
  if (PLATFORM_MINIMUMS.passwordPolicy.requireSpecial)
    merged.passwordPolicy.requireSpecial = true;
...
export async function loadTenantIAMProfile(
  ...
  const raw = result.rows[0]?.iam_profile as
    | Partial<TenantIAMProfile>
    | undefined;
