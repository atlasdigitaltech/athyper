...
export type SessionCreateInput = Omit<
  StoredSession,
  "version" | "sid" | "createdAt" | "lastSeenAt"
>;
...
export class RedisSessionStore {
  ...
  async touch(tenantId: string, sid: string): Promise<void> {
    ...
    await this.cache.set(sessionKey(tenantId, sid), JSON.stringify(session), {
      EX: this.ttl,
    });
  }
  ...
  async update(
    ...
    await this.cache.set(sessionKey(tenantId, sid), JSON.stringify(updated), {
      EX: this.ttl,
    });
  }
}
