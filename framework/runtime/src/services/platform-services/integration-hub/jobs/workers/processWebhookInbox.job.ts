...
import type {
  EventGateway,
  DomainEvent,
} from "../../domain/services/EventGateway.js";
...
export function createProcessWebhookHandler(
  ...
  return async (job: Job<ProcessWebhookPayload>): Promise<void> => {
    ...
    logger.debug(
      { batchSize },
      "[int:worker:webhook] Processing webhook inbox",
    );

    const events = await eventRepo.claimPending(
      batchSize,
      `webhook-worker-${process.pid}`,
    );
    ...
    if (events.length > 0) {
      logger.info(
        ...
        "[int:worker:webhook] Batch complete",
      );
    }
  };
}
