...
export class IntegrationMetrics {
  ...
  deliveryDuration(
    durationMs: number,
    labels: { tenant: string; endpoint: string },
  ): void {
    this.registry.recordHistogram(
      "int_delivery_duration_ms",
      durationMs,
      labels,
    );
  }
  ...
  flowStepDuration(
    durationMs: number,
    labels: { tenant: string; flow: string; stepType: string },
  ): void {
    this.registry.recordHistogram(
      "int_flow_step_duration_ms",
      durationMs,
      labels,
    );
  }
