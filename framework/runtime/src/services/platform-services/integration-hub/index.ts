...
export const module: RuntimeModule = {
  ...
  async register(c: Container) {
    ...
    // ── Persistence ─────────────────────────────────────────────────
    c.register(
      REPO_TOKENS.endpoint,
      async () => new EndpointRepo(db),
      "singleton",
    );
    c.register(REPO_TOKENS.flow, async () => new FlowRepo(db), "singleton");
    c.register(
      REPO_TOKENS.webhookSubscription,
      async () => new WebhookSubscriptionRepo(db),
      "singleton",
    );
    c.register(
      REPO_TOKENS.webhookEvent,
      async () => new WebhookEventRepo(db),
      "singleton",
    );
    c.register(REPO_TOKENS.outbox, async () => new OutboxRepo(db), "singleton");
    c.register(
      REPO_TOKENS.deliveryLog,
      async () => new DeliveryLogRepo(db),
      "singleton",
    );
    ...
    c.register(
      SERVICE_TOKENS.rateLimiter,
      async () => {
        const redis = await c.resolve<Redis>(TOKENS.cache);
        return new IntegrationRateLimiter(
          redis,
          createIntLogger(baseLogger, "rate-limit"),
        );
      },
      "singleton",
    );
    ...
    c.register(
      SERVICE_TOKENS.httpClient,
      async () => {
        const deliveryLogRepo = await c.resolve<DeliveryLogRepo>(
          REPO_TOKENS.deliveryLog,
        );
        return new HttpConnectorClient(
          createIntLogger(baseLogger, "connector"),
          deliveryLogRepo,
        );
      },
      "singleton",
    ...
    // Webhook Service
    c.register(
      SERVICE_TOKENS.webhookService,
      async () => {
        const subRepo = await c.resolve<WebhookSubscriptionRepo>(
          REPO_TOKENS.webhookSubscription,
        );
        const eventRepo = await c.resolve<WebhookEventRepo>(
          REPO_TOKENS.webhookEvent,
        );
        const httpClient = await c.resolve<HttpConnectorClient>(
          SERVICE_TOKENS.httpClient,
        );
        return new WebhookService(
          ...
          eventRepo,
          httpClient,
          createIntLogger(baseLogger, "webhook"),
        );
      },
      "singleton",
    );
    ...
    // Delivery Scheduler
    c.register(
      SERVICE_TOKENS.deliveryScheduler,
      async () => {
        const outboxRepo = await c.resolve<OutboxRepo>(REPO_TOKENS.outbox);
        const httpClient = await c.resolve<HttpConnectorClient>(
          SERVICE_TOKENS.httpClient,
        );
        const endpointRepo = await c.resolve<EndpointRepo>(
          REPO_TOKENS.endpoint,
        );
        const rateLimiter = await c.resolve<IntegrationRateLimiter>(
          SERVICE_TOKENS.rateLimiter,
        );
        const metrics = await c.resolve<IntegrationMetrics>(
          SERVICE_TOKENS.metrics,
        );
    ...
    c.register(
      SERVICE_TOKENS.orchestrationRuntime,
      async () => {
        const httpClient = await c.resolve<HttpConnectorClient>(
          SERVICE_TOKENS.httpClient,
        );
        ...
        const mappingEngine = await c.resolve<MappingEngine>(
          SERVICE_TOKENS.mappingEngine,
        );
    ...
    c.register(
      TOKENS.integrationHttpClient,
      async () => c.resolve<HttpConnectorClient>(SERVICE_TOKENS.httpClient),
      "singleton",
    );
    c.register(
      TOKENS.integrationRateLimiter,
      async () => c.resolve<IntegrationRateLimiter>(SERVICE_TOKENS.rateLimiter),
      "singleton",
    );
    c.register(
      TOKENS.integrationMetrics,
      async () => c.resolve<IntegrationMetrics>(SERVICE_TOKENS.metrics),
      "singleton",
    );
    c.register(
      TOKENS.integrationDeliveryScheduler,
      async () =>
        c.resolve<DeliveryScheduler>(SERVICE_TOKENS.deliveryScheduler),
      "singleton",
    );
    c.register(
      TOKENS.integrationOrchestrator,
      async () =>
        c.resolve<OrchestrationRuntime>(SERVICE_TOKENS.orchestrationRuntime),
      "singleton",
    );
    c.register(
      TOKENS.integrationWebhookService,
      async () => c.resolve<WebhookService>(SERVICE_TOKENS.webhookService),
      "singleton",
    );
    c.register(
      TOKENS.integrationEventGateway,
      async () => c.resolve<EventGateway>(SERVICE_TOKENS.eventGateway),
      "singleton",
    );
    ...
    // Endpoint handlers
    c.register(
      HANDLER_TOKENS.listEndpoints,
      async () => new ListEndpointsHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.getEndpoint,
      async () => new GetEndpointHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.createEndpoint,
      async () => new CreateEndpointHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.updateEndpoint,
      async () => new UpdateEndpointHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.deleteEndpoint,
      async () => new DeleteEndpointHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.testEndpoint,
      async () => new TestEndpointHandler(),
      "singleton",
    );

    // Flow handlers
    c.register(
      HANDLER_TOKENS.listFlows,
      async () => new ListFlowsHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.getFlow,
      async () => new GetFlowHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.createFlow,
      async () => new CreateFlowHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.updateFlow,
      async () => new UpdateFlowHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.executeFlow,
      async () => new ExecuteFlowHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.getFlowRunLogs,
      async () => new GetFlowRunLogsHandler(),
      "singleton",
    );

    // Webhook handlers
    c.register(
      HANDLER_TOKENS.listWebhookSubs,
      async () => new ListWebhookSubscriptionsHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.createWebhookSub,
      async () => new CreateWebhookSubscriptionHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.updateWebhookSub,
      async () => new UpdateWebhookSubscriptionHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.deleteWebhookSub,
      async () => new DeleteWebhookSubscriptionHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.rotateWebhookSecret,
      async () => new RotateWebhookSecretHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.inboundWebhook,
      async () => new InboundWebhookHandler(),
      "singleton",
    );
  },
  ...
  async contribute(c: Container) {
    ...
    try {
      ...
      const scheduler = await c.resolve<DeliveryScheduler>(
        SERVICE_TOKENS.deliveryScheduler,
      );
      const webhookEventRepo = await c.resolve<WebhookEventRepo>(
        REPO_TOKENS.webhookEvent,
      );
      const eventGateway = await c.resolve<EventGateway>(
        SERVICE_TOKENS.eventGateway,
      );
      ...
      logger.warn(
        { error: String(err) },
        "Could not register integration workers",
      );
    }
    ...
    try {
      ...
      logger.warn(
        { error: String(err) },
        "Could not register integration schedules",
      );
    }

    logger.info(
      "Integration Hub module contributed — routes, workers, and schedules registered",
    );
  },
};
