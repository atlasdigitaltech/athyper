...
export class SendSmsHandler implements RouteHandler {
  async handle(
    ...
    const smsAdapter =
      await ctx.container.resolve<ISmsProviderAdapter>(SMS_ADAPTER_TOKEN);
    const smsLogRepo =
      await ctx.container.resolve<SmsLogRepo>(SMS_LOG_REPO_TOKEN);
    ...
    try {
      ...
      res.status(201).json({
        success: true,
        data: { id: logEntry.id, messageRef: result.messageRef },
      });
    } catch (err) {
...
export class ListSmsLogsHandler implements RouteHandler {
  async handle(
    ...
    const logs = await smsLogRepo.list(ctx.tenant.tenantKey!, {
      direction,
      status,
      limit,
      offset,
