...
export class TwilioCtiAdapter implements ICtiAdapter {
  ...
  async initiateCall(params: CallInitiateParams): Promise<CallInitiateResult> {
    ...
    if (!response.ok) {
      ...
      this.logger.error(
        ...
        "[twilio:cti] Call initiation failed",
      );
    ...
    this.logger.info(
      { sid: data.sid, to: params.to },
      "[twilio:cti] Call initiated",
    );
  ...
  async endCall(sessionRef: string): Promise<void> {
    ...
    if (!response.ok) {
      this.logger.error(
        { status: response.status, sessionRef },
        "[twilio:cti] End call failed",
      );
  ...
  parseWebhookEvent(rawBody: Record<string, unknown>): CtiStatusEvent {
    return {
      ...
      durationSeconds: rawBody.CallDuration
        ? Number(rawBody.CallDuration)
        : undefined,
