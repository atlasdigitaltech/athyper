...
export class ListRenderJobsHandler implements RouteHandler {
  async handle(
    ...
    const repo = await ctx.container.resolve<DocRenderJobRepo>(
      RENDER_JOB_REPO_TOKEN,
    );
...
export class GetRenderJobHandler implements RouteHandler {
  async handle(
    ...
    const outputRepo =
      await ctx.container.resolve<DocOutputRepo>(OUTPUT_REPO_TOKEN);
...
export class RetryRenderJobHandler implements RouteHandler {
  async handle(
    ...
    const outputRepo =
      await ctx.container.resolve<DocOutputRepo>(OUTPUT_REPO_TOKEN);
    await outputRepo.updateStatus(job.outputId, "QUEUED", {
      error_message: null,
    });

    res
      .status(202)
      .json({ success: true, message: "Render job retry enqueued" });
  }
}
