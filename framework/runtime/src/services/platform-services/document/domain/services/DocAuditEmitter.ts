...
export class DocAuditEmitter {
  ...
  async templateCreated(
    actor: AuditActor,
    tenantId: string,
    templateId: string,
    code: string,
  ): Promise<void> {
    await this.emit("doc.template.created", actor, {
      tenantId,
      templateId,
      code,
    });
  ...
  async templatePublished(
    ...
    await this.emit("doc.template.published", actor, {
      tenantId,
      templateId,
      versionId,
    });
  ...
  async versionCreated(
    actor: AuditActor,
    tenantId: string,
    templateId: string,
    version: number,
  ): Promise<void> {
    await this.emit("doc.template.version_created", actor, {
      tenantId,
      templateId,
      version,
    });
  ...
  async letterheadChanged(
    actor: AuditActor,
    tenantId: string,
    letterheadId: string,
  ): Promise<void> {
    await this.emit("doc.letterhead.changed", actor, {
      tenantId,
      letterheadId,
    });
  }

  async brandProfileChanged(
    actor: AuditActor,
    tenantId: string,
    profileId: string,
  ): Promise<void> {
    await this.emit("doc.brand_profile.changed", actor, {
      tenantId,
      profileId,
    });
  }
  ...
  async renderCompleted(
    ...
    await this.emit(
      "doc.render.completed",
      { kind: "system" },
      { tenantId, outputId, checksum, durationMs },
    );
  }
