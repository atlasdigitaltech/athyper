...
export class DocHtmlComposer {
  ...
  compose(
    ...
    const headerHtml = this.composeHeader(
      templateVersion,
      letterhead,
      variables,
    );
    const footerHtml = this.composeFooter(
      templateVersion,
      letterhead,
      variables,
    );
...
<body>
  ...
  private evictOldest(): void {
    const entries = [...this.compiledCache.entries()].sort(
  ...
  private validateVariables(
    ...
    if (Array.isArray(required)) {
      for (const field of required) {
        if (
          typeof field === "string" &&
          (variables[field] === undefined || variables[field] === null)
        ) {
    ...
    if (errors.length > 0) {
      ...
      this.logger.warn(
        { errors, templateId: templateVersion.templateId },
        "[doc:composer] " + message,
      );
