...
} from "../../persistence/DocLetterheadRepo.js";
...
export class DocLetterheadService {
  ...
  async create(
    tenantId: string,
    data: Omit<CreateLetterheadInput, "tenantId">,
  ): Promise<DocLetterhead> {
    const existing = await this.repo.getByCode(tenantId, data.code);
    if (existing) {
      throw new Error(
        `Letterhead with code '${data.code}' already exists for this tenant`,
      );
    }

    const letterhead = await this.repo.create({ tenantId, ...data });
    this.logger.info(
      { letterheadId: letterhead.id, code: letterhead.code },
      "[doc:letterhead] Created",
    );
    return letterhead;
  }
