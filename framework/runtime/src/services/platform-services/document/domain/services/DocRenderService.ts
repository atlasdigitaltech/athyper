...
import { classifyDocError } from "../types.js";
...
export class DocRenderService {
  ...
  async renderSync(request: RenderRequest): Promise<RenderSyncResult> {
    ...
    try {
      const { templateVersion, letterhead, brandProfile } =
        await this.resolveRenderInputs(request);
  ...
  async executeRender(
    ...
    try {
      ...
      if (!tplVersion) {
        throw new Error(
          `Template version not found: ${output.templateVersionId}`,
        );
      }
      ...
      // Emit audit event
      await this.auditEmitter?.renderCompleted(
        output.tenantId,
        outputId,
        checksum,
        durationMs,
      ...
      const isFinalAttempt = renderJob
        ? (renderJob.attempts ?? 0) >=
          (renderJob.maxAttempts ?? this.config.maxRetries)
        : true;
  ...
  private async resolveRenderInputs(request: RenderRequest): Promise<{
    ...
    if (request.letterheadId) {
      letterhead =
        (await this.letterheadService.getById(request.letterheadId)) ?? null;
    } else {
      letterhead =
        (await this.letterheadService.resolveDefault(request.tenantId)) ?? null;
    }
    ...
    if (request.brandProfileId) {
      brandProfile =
        (await this.brandService.getById(request.brandProfileId)) ?? null;
    } else {
      brandProfile =
        (await this.brandService.resolveDefault(request.tenantId)) ?? null;
    }
    ...
    if (template) {
      ...
      if (
        template.allowedOperations?.length &&
        !template.allowedOperations.includes(request.operation)
      ) {
        throw new Error(
      ...
      if (
        template.supportedLocales?.length &&
  ...
  private async resolveFromManifest(output: {
    ...
    const letterhead = output.letterheadId
      ...
      : ((await this.letterheadService.resolveDefault(output.tenantId)) ??
        null);
