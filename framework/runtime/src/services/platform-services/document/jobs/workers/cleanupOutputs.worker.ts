...
export function createCleanupOutputsHandler(
  ...
  return async (): Promise<void> => {
    ...
    const archiveCutoff = new Date(
      now.getTime() - config.archiveAfterDays * 86_400_000,
    );
    ...
    try {
      ...
      const archivedCount =
        archiveResult.length > 0
          ? ((archiveResult[0] as any).numUpdatedRows ?? 0)
          : 0;
    ...
    const retentionCutoff = new Date(
      now.getTime() - config.defaultRetentionDays * 86_400_000,
    );
