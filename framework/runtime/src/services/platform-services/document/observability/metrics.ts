...
export class DocMetrics {
  ...
  recordStageDuration(
    stage: "compose" | "render" | "upload" | "db_update",
    durationMs: number,
  ): void {
    this.registry.recordHistogram("doc_render_stage_duration_ms", durationMs, {
      stage,
    });
  }

  incrementFailuresByCode(errorCode: string): void {
    this.registry.incrementCounter("doc_render_failures_by_code", 1, {
  ...
  recordStorageDuration(operation: "put" | "get", durationMs: number): void {
    this.registry.recordHistogram("doc_storage_duration_ms", durationMs, {
      operation,
    });
  }
}
