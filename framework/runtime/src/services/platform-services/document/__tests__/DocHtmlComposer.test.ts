...
import {
  DocHtmlComposer,
  type ComposeOptions,
} from "../domain/services/DocHtmlComposer.js";
...
function baseVersion(
  overrides?: Partial<DocTemplateVersion>,
): DocTemplateVersion {
  return {
...
function baseBrandProfile(
  overrides?: Partial<DocBrandProfile>,
): DocBrandProfile {
  return {
    id: "bp-1" as any,
    tenantId: "t1",
...
describe("DocHtmlComposer", () => {
  ...
  describe("header and footer", () => {
    ...
    it("should render variables in header/footer", () => {
      const version = baseVersion({
        headerHtml: "<div>To: {{recipient}}</div>",
      });
      const html = composer.compose(
        version,
        null,
        null,
        { name: "X", recipient: "John" },
        defaultOptions,
      );
