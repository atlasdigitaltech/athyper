...
export const module: RuntimeModule = {
  ...
  async register(c: Container) {
    ...
    c.register(
      ...
      async () => new DocTemplateRepo(db),
      "singleton",
    );
    c.register(
      REPO_TOKENS.templateVersion,
      async () => new DocTemplateVersionRepo(db),
      "singleton",
    );
    c.register(
      REPO_TOKENS.templateBinding,
      async () => new DocTemplateBindingRepo(db),
      "singleton",
    );
    c.register(
      REPO_TOKENS.letterhead,
      async () => new DocLetterheadRepo(db),
      "singleton",
    );
    c.register(
      REPO_TOKENS.brandProfile,
      async () => new DocBrandProfileRepo(db),
      "singleton",
    );
    c.register(
      REPO_TOKENS.output,
      async () => new DocOutputRepo(db),
      "singleton",
    );
    c.register(
      REPO_TOKENS.renderJob,
      async () => new DocRenderJobRepo(db),
      "singleton",
    );
    c.register(
      REPO_TOKENS.renderDlq,
      async () => new DocRenderDlqRepo(db),
      "singleton",
    );

    // ── PDF Renderer Adapter ────────────────────────────────────────
    c.register(
      TOKENS.documentPdfRenderer,
      async () => {
    ...
    // ── Audit Emitter ───────────────────────────────────────────────
    c.register(
      TOKENS.documentAuditEmitter,
      async () => {
        const auditWriter = await c.resolve<AuditWriter>(TOKENS.auditWriter);
        return new DocAuditEmitter(
          auditWriter,
          createDocLogger(baseLogger, "audit"),
        );
      },
      "singleton",
    );
    ...
    // ── Letterhead Service ──────────────────────────────────────────
    c.register(
      TOKENS.documentLetterheadService,
      async () => {
        const repo = await c.resolve<DocLetterheadRepo>(REPO_TOKENS.letterhead);
        return new DocLetterheadService(
          repo,
          createDocLogger(baseLogger, "letterhead"),
        );
      },
      "singleton",
    );
    ...
    // ── Brand Service ───────────────────────────────────────────────
    c.register(
      TOKENS.documentBrandService,
      async () => {
        const repo = await c.resolve<DocBrandProfileRepo>(
          REPO_TOKENS.brandProfile,
        );
    ...
    // ── Template Service ────────────────────────────────────────────
    c.register(
      TOKENS.documentTemplateService,
      async () => {
        const templateRepo = await c.resolve<DocTemplateRepo>(
          REPO_TOKENS.template,
        );
        const versionRepo = await c.resolve<DocTemplateVersionRepo>(
          REPO_TOKENS.templateVersion,
        );
        const bindingRepo = await c.resolve<DocTemplateBindingRepo>(
          REPO_TOKENS.templateBinding,
        );
        const auditEmitter = await c.resolve<DocAuditEmitter>(
          TOKENS.documentAuditEmitter,
        );
    ...
    // ── DLQ Manager ─────────────────────────────────────────────────
    c.register(
      TOKENS.documentDlqManager,
      async () => {
        const dlqRepo = await c.resolve<DocRenderDlqRepo>(
          REPO_TOKENS.renderDlq,
        );
    ...
    c.register(
      TOKENS.documentRenderService,
      async () => {
        const config = await c.resolve<RuntimeConfig>(TOKENS.config);
        const templateService = await c.resolve<DocTemplateService>(
          TOKENS.documentTemplateService,
        );
        const letterheadService = await c.resolve<DocLetterheadService>(
          TOKENS.documentLetterheadService,
        );
        const brandService = await c.resolve<DocBrandService>(
          TOKENS.documentBrandService,
        );
        const htmlComposer = await c.resolve<DocHtmlComposer>(
          TOKENS.documentHtmlComposer,
        );
        const pdfRenderer = await c.resolve<PuppeteerPdfRenderer>(
          TOKENS.documentPdfRenderer,
        );
        const objectStorage = await c.resolve<any>(TOKENS.objectStorage);
        const storageAdapter = new DefaultDocStorageAdapter(
        ...
        const outputRepo = await c.resolve<DocOutputRepo>(REPO_TOKENS.output);
        const renderJobRepo = await c.resolve<DocRenderJobRepo>(
          REPO_TOKENS.renderJob,
        );
        const jobQueue = await c.resolve<JobQueue>(TOKENS.jobQueue);
        const auditWriter = await c.resolve<AuditWriter>(TOKENS.auditWriter);
        const docMetrics = await c.resolve<DocMetrics>(TOKENS.documentMetrics);
        const dlqManager = await c.resolve<DocRenderDlqManager>(
          TOKENS.documentDlqManager,
        );
        const auditEmitter = await c.resolve<DocAuditEmitter>(
          TOKENS.documentAuditEmitter,
        ...
        return new DocRenderService(
          ...
          {
            paperFormat: (config.document?.rendering?.paperFormat ??
              "A4") as any,
            timeoutMs: config.document?.rendering?.timeoutMs ?? 30000,
            maxRetries: config.document?.rendering?.maxRetries ?? 3,
            storagePathPrefix:
              config.document?.storage?.pathPrefix ?? "documents",
            presignedUrlExpirySeconds:
              config.document?.storage?.presignedUrlExpirySeconds ?? 3600,
            composeTimeoutMs:
              config.document?.rendering?.composeTimeoutMs ?? 5000,
            uploadTimeoutMs:
              config.document?.rendering?.uploadTimeoutMs ?? 30000,
          },
          docMetrics,
          dlqManager,
          auditEmitter,
    ...
    c.register(
      HANDLER_TOKENS.getTemplate,
      async () => new GetTemplateHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.updateTemplate,
      async () => new UpdateTemplateHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.createVersion,
      async () => new CreateVersionHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.publishTemplate,
      async () => new PublishTemplateHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.retireTemplate,
      async () => new RetireTemplateHandler(),
      "singleton",
    );
    c.register(
    ...
    c.register(
      HANDLER_TOKENS.resolveTemplate,
      async () => new ResolveTemplateHandler(),
      "singleton",
    );

    // Letterheads
    c.register(
      HANDLER_TOKENS.listLetterheads,
      async () => new ListLetterheadsHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.createLetterhead,
      async () => new CreateLetterheadHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.getLetterhead,
      async () => new GetLetterheadHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.updateLetterhead,
      async () => new UpdateLetterheadHandler(),
      "singleton",
    );

    // Brand profiles
    c.register(
      HANDLER_TOKENS.listBrandProfiles,
      async () => new ListBrandProfilesHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.createBrandProfile,
      async () => new CreateBrandProfileHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.updateBrandProfile,
      async () => new UpdateBrandProfileHandler(),
      "singleton",
    );

    // Render
    c.register(
      HANDLER_TOKENS.renderDocument,
      async () => new RenderDocumentHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.renderSync,
      async () => new RenderSyncHandler(),
      "singleton",
    );

    // Outputs
    c.register(
      HANDLER_TOKENS.listOutputs,
      async () => new ListOutputsHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.getOutput,
      async () => new GetOutputHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.downloadOutput,
      async () => new DownloadOutputHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.markDelivered,
      async () => new MarkDeliveredHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.revokeOutput,
      async () => new RevokeOutputHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.verifyOutput,
      async () => new VerifyOutputHandler(),
      "singleton",
    );

    // Render jobs
    c.register(
      HANDLER_TOKENS.listRenderJobs,
      async () => new ListRenderJobsHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.getRenderJob,
      async () => new GetRenderJobHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.retryRenderJob,
      async () => new RetryRenderJobHandler(),
      "singleton",
    );

    // DLQ admin
    c.register(
      HANDLER_TOKENS.listDlqEntries,
      async () => new ListDlqEntriesHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.inspectDlqEntry,
      async () => new InspectDlqEntryHandler(),
      "singleton",
    );
    c.register(
      HANDLER_TOKENS.retryDlqEntry,
      async () => new RetryDlqEntryHandler(),
      "singleton",
    );
    c.register(
  ...
  async contribute(c: Container) {
    ...
    const config = await c.resolve<RuntimeConfig>(TOKENS.config);
    const jobQueue = await c.resolve<JobQueue>(TOKENS.jobQueue);
    const renderService = await c.resolve<DocRenderService>(
      TOKENS.documentRenderService,
    );
    const db = await c.resolve<Kysely<DB>>(TOKENS.db);
    ...
    logger.info(
      "Document module contributed — routes, workers, schedules, DLQ, health checks registered",
    );
  },
};
