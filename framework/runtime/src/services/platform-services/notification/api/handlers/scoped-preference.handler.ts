...
export class SetTenantPreferenceHandler {
  async handle(
    ...
    const repo = await ctx.container.resolve<ScopedNotificationPreferenceRepo>(
      SCOPED_PREF_REPO_TOKEN,
    );
    ...
    if (
      ...
      res.status(400).json({
        success: false,
        error: "eventCode, channel, and isEnabled are required",
      });
      return;
    }
...
export class GetEffectivePreferenceHandler {
  async handle(
    ...
    const resolver = await ctx.container.resolve<ScopedPreferenceResolver>(
      SCOPED_RESOLVER_TOKEN,
    );
    const tenantId = ctx.tenant.tenantId;
    ...
    const body = req.body as
      | { eventCode?: string; channel?: string }
      | undefined;
