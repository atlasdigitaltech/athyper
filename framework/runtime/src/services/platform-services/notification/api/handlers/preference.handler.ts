...
export class GetPreferencesHandler implements RouteHandler {
  async handle(
    ...
    const repo =
      await ctx.container.resolve<NotificationPreferenceRepo>(REPO_TOKEN);
    ...
    if (!userId) {
      res.status(401).json({
        success: false,
        error: { code: "UNAUTHENTICATED", message: "User ID required" },
      });
      return;
    }
  ...
  async handle(
    ...
    if (!body.eventCode || !body.channel || body.enabled === undefined) {
      res.status(400).json({
        ...
        error: {
          code: "MISSING_FIELDS",
          message: "eventCode, channel, and enabled are required",
        },
      });
      return;
    }
