...
export class NotificationMetrics {
  ...
  deliveryDuration(
    durationMs: number,
    labels: { channel: string; provider: string },
  ): void {
  ...
  planningDuration(durationMs: number): void {
    this.registry.recordHistogram(
      "notification_planning_duration_ms",
      durationMs,
    );
  }
  ...
  rulesEvaluated(
    count: number,
    labels: { tenant: string; event_type: string },
  ): void {
  ...
  templateRenderDuration(
    ...
    this.registry.recordHistogram(
      "notification_template_render_duration_ms",
      durationMs,
      labels,
    );
  }
  ...
  templateNotFound(labels: { template_key: string; channel: string }): void {
    this.registry.incrementCounter(
      "notification_template_not_found_total",
      1,
      labels,
    );
  }
  ...
  suppressionHit(labels: { channel: string; reason: string }): void {
    this.registry.incrementCounter(
      "notification_suppression_hits_total",
      1,
      labels,
    );
  }
  ...
  preferenceDisabled(labels: { channel: string }): void {
    this.registry.incrementCounter(
      "notification_preference_disabled_total",
      1,
      labels,
    );
  }
  ...
  queueDepth(channel: string, depth: number): void {
    this.registry.setGauge("notification_delivery_queue_depth", depth, {
      channel,
    });
  }
