...
export class WhatsAppConsentRepo {
  ...
  private mapRow(row: any): WhatsAppConsent {
    return {
      ...
      conversationWindowStart: row.conversation_window_start
        ? new Date(row.conversation_window_start)
        : null,
      conversationWindowEnd: row.conversation_window_end
        ? new Date(row.conversation_window_end)
        : null,
