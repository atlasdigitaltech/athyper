...
export class NotificationRuleRepo {
    ...
    await this.db
      ...
      .values({
        ...
        condition_expr: input.conditionExpr
          ? JSON.stringify(input.conditionExpr)
          : null,
  ...
  async update(id: RuleId, input: UpdateRuleInput): Promise<void> {
    ...
    if (input.lifecycleState !== undefined)
      data.lifecycle_state = input.lifecycleState;
    if (input.conditionExpr !== undefined)
      data.condition_expr = JSON.stringify(input.conditionExpr);
    ...
    if (input.recipientRules !== undefined)
      data.recipient_rules = JSON.stringify(input.recipientRules);
    if (input.slaMinutes !== undefined) data.sla_minutes = input.slaMinutes;
    if (input.dedupWindowMs !== undefined)
      data.dedup_window_ms = input.dedupWindowMs;
