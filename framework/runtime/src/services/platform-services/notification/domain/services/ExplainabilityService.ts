...
export class ExplainabilityService {
  ...
  async explain(
    tenantId: string,
    messageId: string,
  ): Promise<NotificationExplainTrace | null> {
    ...
    const storedTrace = (message.metadata as any)?.explainTrace as
      | ExplainStep[]
      | undefined;

    // Get delivery records for the delivery phase
    const deliveries = await this.deliveryRepo.listByMessageId(
      tenantId,
      messageId as MessageId,
    );

    const steps: ExplainStep[] = [...(storedTrace ?? [])];
