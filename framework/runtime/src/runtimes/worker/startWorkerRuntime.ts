...
export async function startWorkerRuntime(container: Container) {
  ...
  const workerCount =
    (jobQueue as any).getRegisteredWorkerCount?.() ?? "unknown";
  if (logger.info) {
    logger.info(
      { service: config.serviceName, registeredWorkers: workerCount },
      "[worker] started",
    );
  } else {
    logger.log?.(
      `[worker] ${config.serviceName} started — registered workers: ${workerCount}`,
    );
  }
