...
export const RuntimeConfigSchema = z.object({
    ...
    eventStore: z.object({
        ...
        crossPartitionStalenessMs: z.coerce.number().int().positive().default(5000),
        enableHashChain: Bool.default(true),
        enablePiiEncryption: Bool.default(true),
    }).default({}),

    financeEngines: z.object({
        enabled: Bool.default(false),
        decisionGrid: z.object({
            ...
            compositeScoring: z.object({
                zeroApprovalThreshold: z.coerce.number().min(0).max(1).default(0.90),
                standardThreshold: z.coerce.number().min(0).max(1).default(0.75),
                enhancedThreshold: z.coerce.number().min(0).max(1).default(0.50),
                executiveThreshold: z.coerce.number().min(0).max(1).default(0.25),
            }).default({}),
        }).default({}),
        budget: z.object({
            reserveExpiryMinutes: z.coerce.number().int().positive().default(1440),
        ...
        commitment: z.object({
            ...
            renewalNoticeBeforeDays: z.coerce.number().int().positive().default(30),
        ...
        atlasAi: z.object({
            enabled: Bool.default(false),
            autonomousConfidenceThreshold: z.coerce.number().min(0).max(1).default(0.95),
            defaultReversalWindowMinutes: z.coerce.number().int().positive().default(60),
