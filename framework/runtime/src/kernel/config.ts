...
function mapSuperstarEnv(): Record<string, unknown> {
  return {
    db: {
      url: superEnv("DATABASE_URL"),
      adminUrl: superEnv("DATABASE_ADMIN_URL"),
    },
    ...
    telemetry: {
      otlpEndpoint: superEnv("OTLP_ENDPOINT"),
      enabled: superEnv("TELEMETRY_ENABLED"),
    },
    iam: {
...
function sanitizeForAudit(input: unknown): unknown {
  ...
  if (isObject(clone) && isObject(clone.db)) {
    ...
    if ("adminUrl" in clone.db)
      clone.db.adminUrl = clone.db.adminUrl ? "[redacted]" : undefined;
  }
 ...
 */
export function resolveRealmClientSecret(
  cfg: RuntimeConfig,
  realmKey: string,
): string | undefined {
