...
export async function registerAdapters(
    ...
    async () => {
      ...
      healthRegistry.register(
        ...
        async () => {
          ...
          const saturation =
            poolStats.max > 0 ? activeCount / poolStats.max : 0;
  ...
  container.register(
    ...
    async () => {
      const { createRedisClient } =
        await import("@athyper/adapter-memorycache");
  ...
  container.register(
    ...
    async () => {
      const { createS3ObjectStorageAdapter } =
        await import("@athyper/adapter-objectstorage");
      ...
      const protectedAdapter = protectObjectStorageAdapter(
        adapter,
        circuitBreakers,
      );
  ...
  container.register(
    TOKENS.telemetry,
    async () => {
      const { createTelemetryAdapter } =
        await import("@athyper/adapter-telemetry");
  ...
  container.register(
    ...
    async () => {
      ...
      const adapter = createAuthAdapter({
        ...
        additionalRealms:
          Object.keys(additionalRealms).length > 0
            ? additionalRealms
            : undefined,
        redisClient,
      });
