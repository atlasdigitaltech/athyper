...
function resolveExitCode(err: unknown): number {
  const code =
    err instanceof KernelConfigError
      ? err.code
...
function bootSummary(config: any) {
  ...
  const tenantsPerRealm = realmKeys.reduce<Record<string, number>>(
    (acc, rk) => {
...
export async function bootstrap(): Promise<BootstrapResult> {
  ...
  if (!PROCESS_GUARDS_INSTALLED) {
    ...
    process.on("unhandledRejection", (reason) => {
      boot.error("boot.unhandledRejection", {
        bootId,
        reason: isObject(reason) ? reason : String(reason),
      });
  ...
  try {
    ...
    {
      const { assertEnvironmentGuardrails } =
        await import("../services/platform/foundation/security/realm-safety");
    ...
    if (config.mode === "api") {
      const { startApiRuntime } =
        await import("../runtimes/api/startApiRuntime");
      await startApiRuntime(container);
    } else if (config.mode === "worker") {
      const { startWorkerRuntime } =
        await import("../runtimes/worker/startWorkerRuntime");
      await startWorkerRuntime(container);
    } else if (config.mode === "scheduler") {
      const { startSchedulerRuntime } =
        await import("../runtimes/scheduler/startSchedulerRuntime");
      await startSchedulerRuntime(container);
    } else {
      throw new KernelConfigError(
        "CONFIG_VALIDATION_ERROR",
        `Unknown mode: ${String((config as any).mode)}`,
      );
    }

    boot.info("boot.success", {
      bootId,
      mode: config.mode,
      serviceName: config.serviceName,
    });
