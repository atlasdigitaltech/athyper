...
import type {
  ...
  RouteHandler,
  RoutePolicy,
} from "../../services/platform/foundation/http/types";
...
export function createExpressHttpServer(root: Container): HttpServer {
  ...
  function mountRoutes(routes: readonly RouteDef[]) {
    ...
    for (const r of routes) {
      ...
      ](r.path, makeRouteMiddleware(root, r));
    }
...
function makeRouteMiddleware(root: Container, def: RouteDef) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const authCtx = await buildAuthContext(
        scope,
        tenant,
      ...
      logger.error(
        { err, requestId: reqCtx.requestId, path: req.path },
        "[http] route error",
      );
      return next(err);
    }
...
async function buildAuthContext(
  ...
  try {
    ...
    if (err instanceof Error) {
      ...
      if (msg.includes("expired") || msg.includes('"exp"'))
        ...
        e.code = "JWT_ISSUER_MISMATCH";
      else if (msg.includes("audience") || msg.includes('"aud"'))
        e.code = "JWT_AUDIENCE_MISMATCH";
  ...
  if (typ && typ !== "Bearer") {
    const e = new Error(
      `JWT typ "${typ}" is not "Bearer" — only access tokens are accepted`,
    ) as Error & { code?: string };
  ...
  if (azp) {
    const config = await scope.resolve<{
      iam: {
        realms: Record<
          string,
          { iam: { clientId: string; allowedAzp?: string[] } }
        >;
      };
    }>(TOKENS.config);
    ...
    if (allowedAzp && allowedAzp.length > 0 && !allowedAzp.includes(azp)) {
      const e = new Error(
        `JWT azp "${azp}" is not in allowedAzp [${allowedAzp.join(", ")}]`,
      ) as Error & { code?: string };
  ...
  const claimTenantKey =
    typeof claims.tenant_key === "string" ? claims.tenant_key : undefined;
  if (
    claimTenantKey &&
    tenant.tenantKey &&
    claimTenantKey !== tenant.tenantKey
  ) {
    const e = new Error(
      `JWT tenant "${claimTenantKey}" does not match context "${tenant.tenantKey}"`,
    ) as Error & { code?: string };
  ...
  try {
    const capService = await scope.resolve<{
      resolveEffectivePersona(roles: string[]): string | Promise<string>;
    }>(TOKENS.iamCapabilityService);
    authCtx.effectivePersona = await capService.resolveEffectivePersona(
      authCtx.roles,
    );
  } catch {
 ...
 */
function normalizeClaimsToAuthContext(
...
function makeErrorMiddleware(root: Container) {
  return async (
    ...
    if (err instanceof KernelConfigError) {
      return res
        .status(500)
        .json({ error: err.code, requestId, message: "Kernel config error" });
    }
...
function isAuthRequiredError(err: unknown): err is { code: "AUTH_REQUIRED" } {
  return (
    !!err &&
    typeof err === "object" &&
    (err as Record<string, unknown>).code === "AUTH_REQUIRED"
  );
}
...
function isJwtError(err: unknown): boolean {
  return (
    !!err &&
    typeof err === "object" &&
    JWT_ERROR_CODES.has((err as Record<string, unknown>).code as string)
  );
}
