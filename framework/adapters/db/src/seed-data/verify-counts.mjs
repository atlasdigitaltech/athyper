import pg from 'pg';
const { Client } = pg;
const client = new Client('postgresql://athyperadmin:athyperadmin@localhost:5432/athyper_dev1');
...
for (const r of res.rows) {
  console.log(`  ${r.domain_code} level ${r.level_no}: ${r.cnt} codes (${r.leaf_cnt} leaf)`);
}
...
for (const r of sample.rows) {
  console.log(`  ${r.code} (${r.name}) -> parent: ${r.parent_code} (${r.parent_name})`);
}
