...
const canonical = await readFile(faviconSource);

for (const relative of faviconTargets) {
  const target = join(repoRoot, relative);
  if (!check) await copyFile(faviconSource, target);
  const actual = await readFile(target);
  if (!actual.equals(canonical)) throw new Error(`Universal SVG favicon is stale: ${relative}`);
}
const blueMark = await readFile(blueMarkSource);
...
for (const relative of embeddedPages) {
  ...
  if (!check) {
    html = html.replace(/(<link rel="icon"[^>]*href=")data:image\/(?:png|svg\+xml);base64,[^"]+("[^>]*>)/u, `$1${dataUri}$2`);
