...
const params = new URLSearchParams(location.search), empty = params.has("empty"), multi = params.has("multi");
...
const navigation: DerivedShellNavigation = Object.freeze({ workspaces: empty ? [] : [Object.freeze({ code: "fin", name: "Finance", iconKey: "home", sortOrder: 1, routes: [route] })], routes: empty ? [] : [route], ...(empty ? {} : { landingHref: "/" }), unknownActiveModules: [] });
createRoot(document.getElementById("root")!).render(<ShellChrome applicationName="Athyper Test" tenantId="tenant-alpha" tenantLabel="Tenant Alpha" accountLabel="User One" navigation={navigation} contexts={multi ? [{ tenantId: "tenant-alpha", label: "Tenant Alpha" }, { tenantId: "tenant-beta", label: "Tenant Beta" }] : undefined}><section><h1>Dashboard</h1><button type="button">Page action</button></section></ShellChrome>);
