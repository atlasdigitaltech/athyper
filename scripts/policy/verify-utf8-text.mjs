...
const roots = ["server/packages", "docs/architecture/server-packages-business-function-improvement-status-report.md", "docs/architecture/server-route-manifest.md", "docs/architecture/server-route-manifest.json"];
const ignored = new Set(["node_modules", "dist", "coverage", ".git", "archive"]);
const extensions = new Set([".ts", ".mts", ".cts", ".js", ".mjs", ".json", ".md", ".yml", ".yaml"]);
const failures = [];

for (const path of roots.map((item) => join(root, item))) scan(path);
if (failures.length) {
  console.error(`UTF-8 mojibake markers found in ${failures.length} file(s):\n${failures.map((item) => `- ${item}`).join("\n")}`);
  process.exitCode = 1;
...
function inspect(path) {
  ...
  if (/\u00c2[\u0080-\u00bf]|\u00e2[\u0080-\u00bf]/.test(source)) failures.push(relative(root, path).replaceAll("\\", "/"));
}
